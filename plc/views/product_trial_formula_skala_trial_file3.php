...
$("#<?php echo $tableId;?> .file_remove").click(function(){
	...
	var tmpDel = $("#skala_trial_mikrobiologi_del");
	li.remove();
	var v = tmpDel.val();
	v+=','+fileid;
	tmpDel.val( v );
	alert( $("#skala_trial_mikrobiologi_del").val() );
});

function del_upload_skala_trial_mikrobiologi(dis, conname, table_id) {
	if($('.'+conname).length > 1) {
		...
		var n = $("span."+table_id+"_num:last").text();
			...
			row_content	 += '<span class="delete_btn"><a href="javascript:;" class="skala_trial_mikrobiologi_del" onclick="del_upload_skala_trial_mikrobiologi(this, \'skala_trial_mikrobiologi_del\', \'skala_trial_mikrobiologi\')">[Hapus]</a></span></td>';		
			row_content  += '</tr>';
...
function add_row_upload_mikrobiologi(table_id){		
		//alert(table_id);
		var row = $('table#'+table_id+' tbody tr:last').clone();
		$("span."+table_id+"_num:first").text('1');
		var n = $("span."+table_id+"_num:last").text();
...
<table class="hover_table" id="skala_trial_mikrobiologi" cellspacing="0" cellpadding="1" style="width: 98%; border: 1px solid #dddddd; text-align: center; margin-left: 5px; border-collapse: collapse">
	<thead>
	...
	<tbody>
		...
		if(in_array('PD', $mydept)) {
			...
			$path = "mikrobiologi";	
			if($value != '') {
				if (file_exists('./files/plc/product_trial/skala_trial/mikrobiologi/'.$idsp.'/'.$value)) {
		...
		?>
				<tr style="border: 1px solid #dddddd; border-collapse: collapse; background: #ffffff; ">
					<td style="border: 1px solid #dddddd; width: 3%; text-align: center;">
						<span class="skala_trial_mikrobiologi_num"><?php echo $i ?></span>
					</td>		
					<td colspan="2" style="border: 1px solid #dddddd; width: 27%">
						<?php echo $row['filename'] ?> 
						<input type="hidden" name="namafile3[]" style="width: 70%" value="<?php echo $row['filename'] ?>" />
					...
					<td style="border: 1px solid #dddddd; width: 10%">
						<span class="delete_btn"><a href="javascript:;" class="skala_trial_mikrobiologi_del" onclick="del_upload_skala_trial_mikrobiologi(this, 'skala_trial_mikrobiologi_del','skala_trial_mikrobiologi')">[Hapus]</a></span>
		...
		<tr style="border: 1px solid #dddddd; border-collapse: collapse; background: #ffffff; ">
					<td style="border: 1px solid #dddddd; width: 3%; text-align: center;">
						<span class="skala_trial_mikrobiologi_num">1</span>
					</td>		
					<td colspan="2" style="border: 1px solid #dddddd; width: 27%">
						<input type="file" class="fileupload multi multifile" name="fileupload3[]" style="width: 70%" /> *max 5 mb
						<input type="hidden" name="namafile3[]" style="width: 70%" value="" />
	...
	<tfoot>
		<tr>
			<td colspan="5"></td><td style="text-align: center"><a href="javascript:;" onclick="javascript:add_row_upload_mikrobiologi('skala_trial_mikrobiologi')">Tambah</a></td>
		</tr>
	</tfoot>
</table>