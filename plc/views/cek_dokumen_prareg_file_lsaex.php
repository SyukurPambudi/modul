...
function add_row_lsaex(table_id){		
		//alert(table_id);
		...
		if (n.length == 0) {
			...
			row_content  +=	'<select name="confirmlsaex[]"><option value="0">Haven\'t been checked</option><option value="1">Yes</option><option value="3">No</option><option value="2">Unnecessary</option></select>';
			<?php } 
				else{?>
			row_content  +=	'<select name="confirmlsaex[]"><option value="0">Haven\'t been checked</option></select>';
			<?php		}
				}?>
			row_content  +=	'</td>';
			row_content	 += '<td colspan="2" style="border: 1px solid #dddddd; width: 50%">';
			row_content	 += '<input type="file" id="fileupload'+table_id+c+'" class="fileupload multi multifile required" name="fileuploadlsaex[]" style="width: 50%" />*';
			row_content  += '<input type="hidden" name="namafiledlsaex[]" style="width: 70%" value="" />';
			row_content  += '<input type="hidden" name="fileidlsaex[]" style="width: 70%" value="" /></td>';	
			row_content	 += '<td colspan="2" style="border: 1px solid #dddddd; width: 8%">';
			row_content	 += '<textarea class="" name="fileketeranganlsaex[]" style="width: 240px; height: 50px;"></textarea></td>';
			row_content	 += '<td style="border: 1px solid #dddddd; width: 10%">';
			row_content	 += '<span class="delete_btn"><a href="javascript:;" class="lsaex_del" onclick="del_row1(this, \'lsaex_del\')">[Hapus]</a></span></td>';		
			row_content  += '</tr>';
			...
			row_content	 += '<input type="file" id="fileupload'+table_id+c+'" class="fileupload multi multifile required" name="fileuploadlsaex[]" style="width: 50%" />*';
			row_content  += '<input type="hidden" name="namafilelsaex[]" style="width: 70%" value="" />';
			row_content  += '<input type="hidden" name="fileidlsaex[]" style="width: 70%" value="" /></td>';
			row_content	 += '<td colspan="2" style="border: 1px solid #dddddd; width: 8%">';
			row_content	 += '<textarea class="" name="fileketeranganlsaex[]" style="width: 240px; height: 50px;"></textarea></td>';
			row_content	 += '<td style="border: 1px solid #dddddd; width: 10%">';
...
<table class="hover_table" id="lsaex" cellspacing="0" cellpadding="1" style="width: 98%; border: 1px solid #dddddd; text-align: center; margin-left: 5px; border-collapse: collapse">
	<thead>
	<tr style="width: 98%; border: 1px solid #dddddd; background: #548cb6; border-collapse: collapse">
		<th colspan="7" style="border: 1px solid #dddddd;"><span style="font-weight: bold; color: #ffffff; text-shadow: 0 1px 1px rgba(0, 0, 0, 0.3); text-transform: uppercase;">Upload File LSA Excipient</span></th>
	</tr>
	...
	<tbody>
		...
		if((in_array('BD', $mydept))||(in_array('QC', $mydept))) {
			...
			$type = "lsaex";
			if($value != '') {
				if (file_exists('./files/plc/lsaex/'.$idsp.'/'.$value)) {
		...
		?>
				<tr style="border: 1px solid #dddddd; border-collapse: collapse; background: #ffffff; ">
					...
					<td style="border: 1px solid #dddddd; width: 4%; text-align: center;">
						...
						?>
						<input type="hidden" name="confirmlsaex[]" value="<?php echo $row['iconfirm_busdev'];?>">
						<span>
					...
					<td colspan="2" style="border: 1px solid #dddddd; width: 27%">
						...
						<input type="hidden" name="namafilelsaex[]" style="width: 70%" value="<?php echo $row['filename'] ?>" />
						<input type="hidden" name="fileidlsaex[]" style="width: 70%" value="<?php echo $row['id'] ?>" />
					</td>	
					...
					<td style="border: 1px solid #dddddd; width: 10%">
						<?php if (($this->input->get('action') != 'view') && (in_array('QC', $mydept)) && (($row['iconfirm_busdev']==0)||($row['iconfirm_busdev']==3))) { ?>
							<span class="delete_btn"><a href="javascript:;" class="lsaex_del" onclick="del_row1(this, 'lsaex_del')">[Hapus]</a></span>	
						<?php	} ?>
		...
		?><!--
				<tr style="border: 1px solid #dddddd; border-collapse: collapse; background: #ffffff; ">
					...
					<td colspan="2" style="border: 1px solid #dddddd; width: 27%">
						<input type="file" id="fileuploadlsaex1" class="fileupload multi multifile required" name="fileuploadlsaex[]" style="width: 50%" /> *
						<input type="hidden" name="namafilelsaex[]" style="width: 70%" value="" />
						<input type="hidden" name="fileidlsaex[]" style="width: 70%" value="" />
						</td>	
					<td colspan="2" style="border: 1px solid #dddddd; width: 15%">
						<textarea class="" name="fileketeranganlsaex[]" style="width: 240px; height: 50px;"></textarea>
					</td>
					<td style="border: 1px solid #dddddd; width: 10%">
						<span class="delete_btn"><a href="javascript:;" class="lsaex_del" onclick="del_row1(this, 'lsaex_del')">[Hapus]</a></span>
					</td>	
	...
	<tfoot>
		<tr>
			...
			<?php if(($iDone<>1) && ((in_array('BD', $mydept))||(in_array('QC', $mydept)))) {?>
			<td colspan="2" style="text-align: center"><a href="javascript:;" onclick="javascript:add_row_lsaex('lsaex')">Tambah</a></td>
			<?php }?>
			<?php if((in_array('BD', $mydept))&&($iDone==0)&&($cekconf==0)) {?>
			<td style="text-align: center">
				<?php echo '&nbsp;&nbsp;&nbsp; <span onclick="javascript:load_popup(\''.base_url().'processor/plc/cek/dokumen/prareg?action=dokDone&dok=lsaex&id='.$idsp.'&upb_id='.$iupb_id.'&group_id='.$this->input->get('group_id').'&modul_id='.$this->input->get('modul_id').'\')" class="ui-button-text icon-save ui-button ui-widget ui-state-default ui-corner-all ui-button-text-icon-primary" id="button_donelsaex">Done</span>';?>
			</td>
			<?php }?>
		</tr>
		<?php if(($iDone==1)) {?>
...
<script>
	$("#fileuploadlsaex1").change(function () {
	    var fileExtension = ["pdf","jpeg", "jpg", "png", "gif", "bmp"];
	    if ($.inArray($(this).val().split(".").pop().toLowerCase(), fileExtension) == -1) {
