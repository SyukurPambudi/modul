...
$("#<?php echo $tableId;?> .file_remove").click(function(){
	...
	var tmpDel = $("#launching_upb_del");
	li.remove();
	var v = tmpDel.val();
	v+=','+fileid;
	tmpDel.val( v );
	alert( $("#launching_upb_del").val() );
});

function add_row_launching_upb(table_id){		
		//alert(table_id);
		...
		if (n.length == 0) {
			...
			row_content	 += '<span class="delete_btn"><a href="javascript:;" class="launching_upb_del" onclick="del_row1(this, \'launching_upb_del\')">[Hapus]</a></span></td>';		
			row_content  += '</tr>';
...
<table class="hover_table" id="launching_upb" cellspacing="0" cellpadding="1" style="width: 98%; border: 1px solid #dddddd; text-align: center; margin-left: 5px; border-collapse: collapse">
	<thead>
	...
	<tbody>
		...
		$linknya = "";
		if (isset($mydept)) {
		if((in_array('PD', $mydept))||(in_array('BD', $mydept))||(in_array('DR', $mydept))) {
			//if(!empty($rows)) {
			if(!empty($rows)) {
				foreach($rows as $row) {
			...
			if($value != '') {
				if (file_exists('./files/plc/bahan_kemas/'.$idsp.'/'.$value)) {
					$link = base_url().'processor/plc/launching/upb?action=download&id='.$idsp.'&path='.$type.'&file='.$value;
		...
		?>
				<tr style="border: 1px solid #dddddd; border-collapse: collapse; background: #ffffff; ">
					<td style="border: 1px solid #dddddd; width: 3%; text-align: center;">
						<span class="launching_upb_num"><?php echo $i ?></span>
					</td>							
		...
		?>
				<tr style="border: 1px solid #dddddd; border-collapse: collapse; background: #ffffff; ">
					<td colspan="6" style="border: 1px solid #dddddd; width: 3%; text-align: center;">
						<span class="launching_upb_num">Tidak ada File</span>
					</td>	
				</tr>
		<?php }
		} 	
