...
$("#<?php echo $tableId;?> .file_remove").click(function(){
	...
	var tmpDel = $("#bahan_baku_mikro_del");
	li.remove();
	var v = tmpDel.val();
	v+=','+fileid;
	tmpDel.val( v );
	alert( $("#bahan_baku_mikro_del").val() );
});
function del_bahan_baku_mikro(dis, conname, table_id) {
	//if($('.'+conname).length > 1) {
		custom_confirm('Delete Selected Record?', function(){
			...
			if($('.'+conname).length==0) {
				...
				var n = $("span."+table_id+"_num:last").text();
					...
					row_content	 += '<span class="delete_btn"><a href="javascript:;" class="bahan_baku_mikro_del" onclick="del_row(this, \'bahan_baku_mikro_del\')">[Hapus]</a></span></td>';		
					row_content  += '</tr>';
...
function add_row_bahan_baku_mikro(table_id){		
		//alert(table_id);
		var row = $('table#'+table_id+' tbody tr:last').clone();
		$("span."+table_id+"_num:first").text('1');
		var n = $("span."+table_id+"_num:last").text();
		if (n.length == 0) {
			...
			row_content	 += '<textarea class="" name="fileketerangan[]" style="width: 240px; height: 50px;"></textarea></td>';
			row_content	 += '<td style="border: 1px solid #dddddd; width: 10%">';
			row_content	 += '<span class="delete_btn"><a href="javascript:;" class="bahan_baku_mikro_del" onclick="del_bahan_baku_mikro(this, \'bahan_baku_mikro_del\', \'bahan_baku_mikro\')">[Hapus]</a></span></td>';		
			row_content  += '</tr>';
...
<table class="hover_table" id="bahan_baku_mikro" cellspacing="0" cellpadding="1" style="width: 98%; border: 1px solid #dddddd; text-align: center; margin-left: 5px; border-collapse: collapse">
	<thead>
	...
	<tbody>
		...
		if(in_array('QA', $mydept)) {
			...
			$path = "mikro";	
			if($value != '') {
				if (file_exists('./files/plc/bahan_baku/mikro/'.$idsp.'/'.$value)) {
					$link = base_url().'processor/plc/bahan/baku/analisa/sample?action=download&id='.$idsp.'&path='.$path.'&file='.$value;
		...
		?>
				<tr style="border: 1px solid #dddddd; border-collapse: collapse; background: #ffffff; ">
					<td style="border: 1px solid #dddddd; width: 3%; text-align: center;">
						<span class="bahan_baku_mikro_num"><?php echo $i ?></span>
					</td>							
					...
					<td style="border: 1px solid #dddddd; width: 10%">
						<span class="delete_btn"><a href="javascript:;" class="bahan_baku_mikro_del" onclick="del_bahan_baku_mikro(this, 'bahan_baku_mikro_del', 'bahan_baku_mikro')">[Hapus]</a></span>
		...
		?>
				<tr style="border: 1px solid #dddddd; border-collapse: collapse; background: #ffffff; ">
					<td style="border: 1px solid #dddddd; width: 3%; text-align: center;">
						<span class="bahan_baku_mikro_num">1</span>
					</td>		
					<td colspan="2" style="border: 1px solid #dddddd; width: 27%">
	...
	<tfoot>
		...
		?>
			<tr>
				<td colspan="5"></td><td style="text-align: center"><a href="javascript:;" onclick="javascript:add_row_bahan_baku_mikro('bahan_baku_mikro')">Tambah</a></td>
			</tr>
		<?php } ?>
	</tfoot>
</table>