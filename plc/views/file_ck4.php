...
function add_row_study_pd_upload1ss1(table_id){		
		...
		if (n.length == 0) {
			...
			row_content	 += '<input type="file" id="fileupload1'+table_id+c+'" class="fileupload1 multi multifile required" name="fileid_upload[]" style="width: 70%" />*';
			row_content  += '<input type="hidden" name="namefile[]" style="width: 70%" value="" />';	
			...
			row_content	 += '<textarea id= "filekt1'+c+'" class="required" name="fileketerangan[]" style="width: 240px; height: 50px;"></textarea><br/>tersisa <span id="len1'+c+'">250</span> karakter<br/></td>';
...
<table class="hover_table" id="study_pd_upload1" cellspacing="0" cellpadding="1" style="width: 98%; border: 1px solid #dddddd; text-align: center; margin-left: 5px; border-collapse: collapse">
	...
	<tr style="width: 98%; border: 1px solid #dddddd; background: #548cb6; border-collapse: collapse">
		<th colspan="4" style="border: 1px solid #dddddd;"><span style="font-weight: bold; color: #ffffff; text-shadow: 0 1px 1px rgba(0, 0, 0, 0.3); text-transform: uppercase;">Upload Dokumen</span></th>
	</tr>
	...
	<tbody>
		<?php
			...
			if(!empty($rows)) {
				...
				foreach($rows as $row) {
					...
					$value = $row['vFilename'];	
					if($value != '') {
						if (file_exists('./files/plc/ck4_file/'.$id.'/'.$value)) {
							$link = base_url().'processor/plc/kelengkapan/dokumen4/export?action=download&id='.$id.'&file='.$value;
		...
		?>
				<tr style="border: 1px solid #dddddd; border-collapse: collapse; background: #ffffff; ">
					...
					<td colspan="1" style="border: 1px solid #dddddd; width: 27%">
						...
						<input type="hidden" name="namefile[]" style="width: 70%" value="<?php echo $row['vFilename'] ?>" />
						<input type="hidden" name="fileid[]" style="width: 70%" value="<?php echo $row['idAttachment'] ?>" />
					</td>
		...
		<tr style="border: 1px solid #dddddd; border-collapse: collapse; background: #ffffff; ">
					...
					<td colspan="1" style="border: 1px solid #dddddd; width: 27%">
						<input type="file" id="fileupload1_study_pd_1" class="fileupload1 multi multifile required" name="fileid_upload[]" style="width: 70%" /> *
						<input type="hidden" name="namefile[]" style="width: 70%" value="" />
