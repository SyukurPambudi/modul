	...
	<tbody>
		...
		foreach ($row as $kr => $vr) {
				...
				foreach ($reqOrigi_p as $v) {
					 ...
					 echo "</td>"; 
					 	if($v['iapppd']==2){ //Approve
							...
							echo "<td> <b>".$v['vreq_ori_no']."</b><br> Waiting For Approval</td>";
						}
					 echo "<tr>";
				}
				echo "</table></td>"; 
			...
			//sAMPLE
				...
				$purchase = "SELECT ro.`iapprove`,ro.`vro_nomor`,ro.`vnip_pur`, ro.tapp_pur, ps.`iupb_id` 
					...
					ro.`tapp_pur` IS NOT NULL AND ro.`vro_nomor` <> '' AND ro.`vnip_pur` <> '' AND
					pu.`ldeleted` = 0 AND pd.`ldeleted` = 0 AND ps.lDeleted = 0 AND ro.`ldeleted` = 0 AND ps.`iupb_id` = '".$vr['iupb_id']."' GROUP BY ro.`vro_nomor`";
				$purchase_p = $this->db_plc0->query($purchase)->result_array();
 			...
 			//Input Terima
	    		...
	    		$tujuan = array(''=>'-','0'=>'-','1'=>'Sample','2'=>'Pilot I','3'=>'Pilot II', '4'=>'Re-Sample');
			...
			//Uji Mikro
				...
				$soibb = "SELECT p.`vkode_surat`, p.`vnip_qc`, p.`iappqc`,p.`tapp_qc`, p.`vnip_qa`,p.`iappqa`, p.`tapp_qa` FROM plc2.plc2_upb_soi_bahanbaku p 
		...
		//Best Formula
				$bestformula = "SELECT pu.`iupb_id`,pf.`vbest_nip_apppd`,pf.`tbest_apppd` FROM plc2.plc2_upb pu 
							JOIN plc2.`plc2_upb_formula` pf ON pf.`iupb_id` = pu.`iupb_id`
								...
								AND pu.`iupb_id` IN (SELECT up.iupb_id FROM plc2.plc2_upb up
							...
							if(!empty($bestformula_p['vbest_nip_apppd']==2)){ //Approve
								echo "<td> Confirm Oleh ".$bestformula_p['vbest_nip_apppd']."<br>Pada ".$bestformula_p['tbest_apppd']."</td>";
							}else{
								echo '<td></td>';
							} 
				...
				$basic = "SELECT pf.`iapppd_basic`, pf.`vbasic_nip_apppd`, pf.`tbasic_apppd` FROM plc2.`plc2_upb_formula` pf JOIN
						...
						AND pf.`iupb_id` =   '".$vr['iupb_id']."' LIMIT 1";
							...
							if($basic_p['iapppd_basic']==2){ //Approve
								...
								echo '<td>Waiting for Approved</td>';
							}else{
								echo '<td></td>';
							}
