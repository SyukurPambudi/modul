...
$("#<?php echo $tableId;?> .file_remove").click(function(){
	...
	var tmpDel = $("#stress_test_disolusi_del");
	li.remove();
	var v = tmpDel.val();
	v+=','+fileid;
	tmpDel.val( v );
	alert( $("#stress_test_disolusi_del").val() );
});

function del_upload_stress_test_disolusi(dis, conname, table_id) {
	if($('.'+conname).length > 1) {
		...
		var n = $("span."+table_id+"_num:last").text();
			...
			row_content	 += '<span class="delete_btn"><a href="javascript:;" class="stress_test_disolusi_del" onclick="del_upload_stress_test_disolusi(this, \'stress_test_disolusi_del\', \'stress_test_disolusi\')">[Hapus]</a></span></td>';		
			row_content  += '</tr>';
...
function add_row_stress_test_disolusi(table_id){		
		//alert(table_id);
		var row = $('table#'+table_id+' tbody tr:last').clone();
		$("span."+table_id+"_num:first").text('1');
		var n = $("span."+table_id+"_num:last").text();
...
<table class="hover_table" id="stress_test_disolusi" cellspacing="0" cellpadding="1" style="width: 98%; border: 1px solid #dddddd; text-align: center; margin-left: 5px; border-collapse: collapse">
	<thead>
	...
	<tbody>
		...
		if(in_array('PD', $mydept)) {
			...
			$path = "filename";	
			if($value != '') {
				if (file_exists('./files/plc/product_trial/stress_test/disolusi/'.$idsp.'/'.$value)) {
		...
		?>
				<tr style="border: 1px solid #dddddd; border-collapse: collapse; background: #ffffff; ">
					<td style="border: 1px solid #dddddd; width: 3%; text-align: center;">
						<span class="stress_test_disolusi_num"><?php echo $i ?></span>
					</td>							
					...
					<td style="border: 1px solid #dddddd; width: 10%">
						<span class="delete_btn"><a href="javascript:;" class="stress_test_disolusi_del" onclick="del_upload_stress_test_disolusi(this, 'stress_test_disolusi_del','stress_test_disolusi')">[Hapus]</a></span>
		...
		?>
				<tr style="border: 1px solid #dddddd; border-collapse: collapse; background: #ffffff; ">
					<td style="border: 1px solid #dddddd; width: 3%; text-align: center;">
						<span class="stress_test_disolusi_num">1</span>
					</td>		
					<td colspan="2" style="border: 1px solid #dddddd; width: 27%">
	...
	<tfoot>
		<tr>
			<td colspan="5"></td><td style="text-align: center"><a href="javascript:;" onclick="javascript:add_row_stress_test_disolusi('stress_test_disolusi')">Tambah</a></td>
		</tr>
	</tfoot>
</table>