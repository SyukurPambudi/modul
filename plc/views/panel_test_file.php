		...
		<?php
			...
			if(!empty($rows)) {
				...
				foreach($rows as $row) {
					...
					$id  = $row['iplc2_upb_paneltest_id'];
					$value = $row['vFilename'];	
					if($value != '') {
						if (file_exists('./files/plc/test_panel/'.$id.'/'.$value)) {
							$link = base_url().'processor/plc/product/trial/test/panel?action=download&id='.$id.'&file='.$value;
		...
		?>
				<tr style="border: 1px solid #dddddd; border-collapse: collapse; background: #ffffff; ">
					...
					<td colspan="1" style="border: 1px solid #dddddd; width: 27%">
						...
						<input type="hidden" name="fileid[]" style="width: 70%" value="<?php echo $row['iplc2_upb_paneltest_file_id'] ?>" />
					</td>	
