...
$("#<?php echo $tableId;?> .file_remove").click(function(){
	...
	var tmpDel = $("#uji_mikro_fg_del");
	li.remove();
	var v = tmpDel.val();
	v+=','+fileid;
	tmpDel.val( v );
	alert( $("#uji_mikro_fg_del").val() );
});

function add_row_uji_mikro_fg_upload(table_id){		
		//alert(table_id);
		...
		if (n.length == 0) {
			...
			row_content	 += '<textarea id= "filekt'+c+'" class="required" name="fileketerangan[]" style="width: 240px; height: 50px;"></textarea><br/>tersisa <span id="len'+c+'">250</span> karakter<br/></td>';
			row_content	 += '<td style="border: 1px solid #dddddd; width: 10%">';
			row_content	 += '<span class="delete_btn"><a href="javascript:;" class="uji_mikro_fg_del" onclick="del_row(this, \'uji_mikro_fg_del\')">[Hapus]</a></span></td>';		
			row_content  += '</tr>';
			...
			row_content  += '<input type="hidden" name="fileid[]" style="width: 70%" value="" /></td>';
			row_content	 += '<td colspan="3" style="border: 1px solid #dddddd; width: 8%">';
			row_content	 += '<textarea id= "filekt'+c+'"class="required" name="fileketerangan[]" style="width: 240px; height: 50px;"></textarea><br/>tersisa <span id="len'+c+'">250</span> karakter<br/>';
			row_content  += '</td>';
...
<table class="hover_table" id="uji_mikro_fg_upload" cellspacing="0" cellpadding="1" style="width: 98%; border: 1px solid #dddddd; text-align: center; margin-left: 5px; border-collapse: collapse">
	<thead>
	...
	<tbody>
		<?php
			...
			if(!empty($rows)) {
				...
				foreach($rows as $row) {
					...
					$value = $row['vFilename'];	
					if($value != '') {
						if (file_exists('./files/plc/uji_mikro_fg/'.$id.'/'.$value)) {
							$link = base_url().'processor/plc/uji/mikro/fg?action=download&id='.$id.'&file='.$value;
		...
		?>
				<tr style="border: 1px solid #dddddd; border-collapse: collapse; background: #ffffff; ">
					<td style="border: 1px solid #dddddd; width: 3%; text-align: center;">
						<span class="uji_mikro_fg_upload_num"><?php echo $i ?></span>
					</td>		
					<td colspan="1" style="border: 1px solid #dddddd; width: 27%">
						...
						<input type="hidden" name="fileid[]" style="width: 70%" value="<?php echo $row['iuji_mikro_fg_file_id'] ?>" />
					</td>	
					...
					<td style="border: 1px solid #dddddd; width: 10%">
						<?php if ($this->input->get('action') != 'view') { ?>
							<span class="delete_btn"><a href="javascript:;" class="uji_mikro_fg_del" onclick="del_row1(this, 'uji_mikro_fg_del')">[Hapus]</a></span>	
						<?php	} ?>
		...
		<tr style="border: 1px solid #dddddd; border-collapse: collapse; background: #ffffff; ">
					<td style="border: 1px solid #dddddd; width: 3%; text-align: center;">
						<span class="uji_mikro_fg_upload_num">1</span>
					</td>		
					<td colspan="1" style="border: 1px solid #dddddd; width: 27%">
						<input type="file" id="fileupload_uji_mikro_fg1" class="fileupload multi multifile required" name="fileupload[]" style="width: 70%" /> *
					...
					<td colspan="3" style="border: 1px solid #dddddd; width: 15%">
						<textarea class="required" id="filekt1" name="fileketerangan[]" style="width: 240px; height: 50px;"size="250"></textarea>
						<br/>
	...
	<tfoot>
		<tr>
			<td colspan="5" style="text-align: left">*) Max File 5 MB : Format = pdf,jpeg,jpg,png,gif,bmp</td><td style="text-align: center">
				<?php if ($this->input->get('action') != 'view') { ?>
					<a href="javascript:;" onclick="javascript:add_row_uji_mikro_fg_upload('uji_mikro_fg_upload')">Tambah</a>	
				<?php } ?>
	...
	<script>
		...
		$("#fileupload_uji_mikro_fg1").change(function () {
	        var fileExtension = ["pdf","jpeg", "jpg", "png", "gif", "bmp"];
	        if ($.inArray($(this).val().split(".").pop().toLowerCase(), fileExtension) == -1) {
	        	_custom_alert("Only formats are allowed : "+fileExtension.join(", "),"info", "info", "uji_mikro_fg", 1, 20000);
	        	$(this).val('');
	        }
    	});
	</script>