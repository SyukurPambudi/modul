		...
		custom_confirm(comfirm_message,function(){
			if(isUpload && !isValidAFileSize('#form_create_'+grid+' input.multifile', uploadLimit)) {
				...
				success: function(data) {
					...
					if(isUpload) {
								var iframe = $('<iframe name="product_lab_frame"/>');
								iframe.attr({'id':'product_lab_frame'});
								...
								$('#form_create_'+grid).attr('target','product_lab_frame');								
								//$('#form_create_'+grid).submit();
								product_trial_formula_skala_lab_tryUPload('form_update_'+grid , grid, formAction, base_url+'processor/plc/product/trial/formula/skala/lab?action=update&id='+o.last_id+'&foreign_key='+o.foreign_id+'&company_id='+o.company_id+'&group_id='+o.group_id+'&modul_id='+o.modul_id );
							} else{
								_custom_alert(o.message,header,info, grid, 1, 20000);
								$.get(base_url+'processor/plc/product/trial/formula/skala/lab?action=update&id='+last_id+'&foreign_key='+foreign_id+'&company_id='+company_id+'&group_id='+group_id+'&modul_id='+modul_id, function(data) {
			...
			if(isUpload && !isValidAFileSize('#form_update_'+grid+' input.multifile', uploadLimit)) {
				...
				success: function(data) {
					...
					if(isUpload > 0) {				
						...
						$('#form_update_'+grid).attr('target','product_lab_frame');
	...
	custom_confirm('Anda Yakin Confirm UPB : '+upbno,function(){
		$.ajax({
			...
			success: function(data) {
				...
				$.get(base_url+'processor/plc/product/trial/formula/skala/lab?action=view&id='+o.last_id+'&foreign_key='+o.foreign_id+'&company_id='+o.company_id+'&group_id='+o.group_id+'&modul_id='+o.modul_id, function(data) {
                        $('div#form_'+grid).html(data);
                        $('html, body').animate({scrollTop:$('#'+grid).offset().top - 20}, 'slow');
                });
			}		
...
<iframe name="product_lab_frame" id="product_lab_frame" height="0" width="0">
</iframe>
