<script type="text/javascript">
	...
	$('.vName_member_div_add1').live('click', function() {
		...
		var row = '<tr><td style="text-align: right"><span class="numberlist"></span></td><td style="text-align: center"><input class="vName_member_div_cNip" name="nip[]" type="hidden" ><input class="input_rows-table vName_member_div" style="width: 98%" name="name[]" type="text" ></td><td><select class="form-control" name="iapprove[]"><option value="1">Ya</option><option value="0">Tidak</option></select></td><td style="text-align: center">[ <a href="javascript:;" class="vName_member_div_del">Hapus</a> ]</td></tr>';		
		...
		<tbody>
								...
								<td>
									<?php 
									 ...
									 ?>
											...
											<?php $dataarray=array(0=>"Tidak",1=>'Iya'); ?>
											<select class="form-control" name="iapprove[]">
												<?php 
													foreach ($dataarray as $index => $value) {
													$select = $index==$v['iapprove']?'selected':'';
													//echo "<option value='".$kzz	."' ".$select.">".$vzz."</option>";
												?>


													<option <?php echo $select ?> value="<?php echo $index ?> "  ><?php echo $value ?></option>
									...
									<?php 
									 ...
									 ?>
									 	...
									 	$sql='select a.iapprove, if(a.iapprove=0,"Tidak","IYA") as tampil from plc2_upb_team_item a where a.iteami_id="'.$v['iteami_id'].'" ';
									 	//$sql='select * from plc2.plc2_upb_team_item a where a.iteami_id="'.$v['iteami_id'].'" ';
			...
			?>
							<tr>
								...
								<td><?php  $zzz=array(0=>"Tidak",1=>'Iya'); ?>
									<select class="form-control" name="iapprove[]">
										<?php foreach ($zzz as $kzz => $vzz) {
											$select=$kzz==$v['iapprove']?'selected':'';
											echo "<option value='".$kzz	."' ".$select.">".$vzz."</option>";
										}
										?>
									</select>
