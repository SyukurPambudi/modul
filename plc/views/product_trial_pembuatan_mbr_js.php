		...
		custom_confirm(comfirm_message,function(){
				...
				success: function(data) {
					...
					if(o.status == true){
						if(isUpload) {
							...
							var iframe = $('<iframe name="product_trial_pembuatan_mbr_frame"/>');
							iframe.attr({'id':'product_trial_pembuatan_mbr_frame'});
							...
							$('#form_update_'+grid).attr('target','product_trial_pembuatan_mbr_frame');								
							$('#form_update_'+grid).submit();
						}
						_custom_alert('Data Berhasil Disimpan !',header,info, grid, 1, 20000);
						$('#grid_'+grid).trigger('reloadGrid');	

						$.get(base_url+'processor/plc/product/trial/pembuatan/mbr?action=update&id='+o.last_id+'&foreign_key='+o.foreign_id+'&company_id='+o.company_id+'&group_id='+o.group_id+'&modul_id='+o.modul_id, function(data) {
			...
			.done(function(data) {
					...
					$('#grid_'+grid).trigger('reloadGrid');
						...
						$.get(base_url+'processor/plc/product/trial/pembuatan/mbr?action=update&id='+last_id+'&foreign_key='+foreign_id+'&company_id='+company_id+'&group_id='+group_id+'&modul_id='+modul_id, function(data) {
	                            $('div#form_'+grid).html(data);
	                            $('html, body').animate({scrollTop:$('#'+grid).offset().top - 20}, 'slow');
	                    });
				});
...
<iframe name="product_trial_pembuatan_mbr_frame" id="product_trial_pembuatan_mbr_frame" height="0" width="0"></iframe>
