...
function add_row_dmf(table_id){		
		...
		if (n.length == 0) {
			...
			row_content  +=	'<select name="confirmdmf[]" id="confirmdmf'+c+'"><option value="0">Haven\'t been checked</option><option value="1">Yes</option><option value="3">No</option><option value="2">Unnecessary</option></select>';
			<?php } 
				else{?>
			row_content  +=	'<select name="confirmdmf[]" id="confirmdmf'+c+'"><option value="0">Haven\'t been checked</option></select>';
			<?php		}
				}?>
			row_content  +=	'</td>';
			row_content	 += '<td colspan="2" style="border: 1px solid #dddddd; width: 50%">';
			row_content	 += '<input type="file" id="fileupload'+table_id+c+'" class="fileupload multi multifile required" name="fileuploaddmf[]" style="width: 50%" />*';
			row_content  += '<input type="hidden" name="namafileddmf[]" style="width: 70%" value="" />';
			row_content  += '<input type="hidden" name="fileiddmf[]" style="width: 70%" value="" /></td>';	
			row_content	 += '<td colspan="2" style="border: 1px solid #dddddd; width: 8%">';
			row_content	 += '<textarea class="" name="fileketerangandmf[]" style="width: 240px; height: 50px;"></textarea></td>';
			row_content	 += '<td style="border: 1px solid #dddddd; width: 10%">';
			row_content	 += '<span class="delete_btn"><a href="javascript:;" class="dmf_del" onclick="del_row1(this, \'dmf_del\')">[Hapus]</a></span></td>';		
			row_content  += '</tr>';
			...
			row_content	 += '<input type="file" id="fileupload'+table_id+c+'" class="fileupload multi multifile required" name="fileuploaddmf[]" style="width: 50%" />*';
			row_content  += '<input type="hidden" name="namafiledmf[]" style="width: 70%" value="" />';
			row_content  += '<input type="hidden" name="fileiddmf[]" style="width: 70%" value="" /></td>';
			row_content	 += '<td colspan="2" style="border: 1px solid #dddddd; width: 8%">';
			row_content	 += '<textarea class="" name="fileketerangandmf[]" style="width: 240px; height: 50px;"></textarea></td>';
			row_content	 += '<td style="border: 1px solid #dddddd; width: 10%">';
		...
		/*$("#confirmdmf"+c).change(function(){
			var v = $(this).val();
			var file = $(this).parent().next();
			var f = file.find("input[type='file']").attr('id');
			if(v==2){
...
<table class="hover_table" id="dmf" cellspacing="0" cellpadding="1" style="width: 98%; border: 1px solid #dddddd; text-align: center; margin-left: 5px; border-collapse: collapse">
	<thead>
	<tr style="width: 98%; border: 1px solid #dddddd; background: #548cb6; border-collapse: collapse">
		<th colspan="7" style="border: 1px solid #dddddd;"><span style="font-weight: bold; color: #ffffff; text-shadow: 0 1px 1px rgba(0, 0, 0, 0.3); text-transform: uppercase;">Upload File DMF</span></th>
	</tr>
	...
	<tbody>
		...
		if((in_array('BD', $mydept))||(in_array('PR', $mydept))) {
			...
			$type = "dmf";
			if($value != '') {
				if (file_exists('./files/plc/dmf/'.$idsp.'/'.$value)) {
		...
		?>
				<tr style="border: 1px solid #dddddd; border-collapse: collapse; background: #ffffff; ">
					...
					<td style="border: 1px solid #dddddd; width: 4%; text-align: center;">
						...
						?>
						<input type="hidden" name="confirmdmf[]" value="<?php echo $row['iconfirm_busdev'];?>">
						<span>
					...
					<td colspan="2" style="border: 1px solid #dddddd; width: 27%">
						...
						<input type="hidden" name="namafiledmf[]" style="width: 70%" value="<?php echo $row['filename'] ?>" />
						<input type="hidden" name="fileiddmf[]" style="width: 70%" value="<?php echo $row['id'] ?>" />
					</td>	
					<td colspan="2" style="border: 1px solid #dddddd; width: 8%">
						<?php echo $row['vketerangan'] ?>
					</td>
					<td style="border: 1px solid #dddddd; width: 10%">
						<?php if (($this->input->get('action') != 'view') && (in_array('PR', $mydept)) && (($row['iconfirm_busdev']==0)||($row['iconfirm_busdev']==3))) { ?>
							<span class="delete_btn"><a href="javascript:;" class="dmf_del" onclick="del_row1(this, 'dmf_del')">[Hapus]</a></span>	
						<?php	} ?>
		...
		?><!--
				<tr style="border: 1px solid #dddddd; border-collapse: collapse; background: #ffffff; ">
					<td style="border: 1px solid #dddddd; width: 3%; text-align: center;">
						<span class="dmf_num">1</span>
					</td>
					<td style="border: 1px solid #dddddd; width: 4%; text-align: center;">
						<select name="confirmdmf[]" id="confirmdmf1">
					...
					<td colspan="2" style="border: 1px solid #dddddd; width: 27%">
						<input type="file" id="fileupload_cek_dokumen_prareg1" class="fileupload multi multifile required" name="fileuploaddmf[]" style="width: 50%" /> *
						<input type="hidden" name="namafiledmf[]" style="width: 70%" value="" />
						<input type="hidden" name="fileiddmf[]" style="width: 70%" value="" />
						</td>	
					<td colspan="2" style="border: 1px solid #dddddd; width: 15%">
						<textarea class="" name="fileketerangandmf[]" style="width: 240px; height: 50px;"></textarea>
					</td>
					<td style="border: 1px solid #dddddd; width: 10%">
						<span class="delete_btn"><a href="javascript:;" class="dmf_del" onclick="del_row1(this, 'dmf_del')">[Hapus]</a></span>
					</td>	
	...
	<tfoot>
		<tr>
			...
			<td colspan="2" style="text-align: center"><a href="javascript:;" onclick="javascript:add_row_dmf('dmf')">Tambah</a></td>
			<?php }?>
			<?php if((in_array('BD', $mydept))&&($iDone==0)&&($cekconf==0)) {?>
			<td style="text-align: center">
				<?php echo '&nbsp;&nbsp;&nbsp; <span onclick="javascript:load_popup(\''.base_url().'processor/plc/cek/dokumen/prareg?action=dokDone&dok=dmf&id='.$idsp.'&upb_id='.$idsp.'&group_id='.$this->input->get('group_id').'&modul_id='.$this->input->get('modul_id').'\')" class="ui-button-text icon-save ui-button ui-widget ui-state-default ui-corner-all ui-button-text-icon-primary" id="button_doneDMF">Done</span>';?>
			</td>
			<?php }?>
		</tr>
		<?php if(($iDone==1)) {?>
			<tr>
			 <td colspan="3" style="text-align: left"><b><p style="color:green;font-size:120%;">Status Dokumen DMF : DONE </p></b></td>
			</tr>
		<?php }?>
...
<script>
	...
	$("#fileupload_cek_dokumen_prareg1").change(function () {
	    var fileExtension = ["pdf","jpeg", "jpg", "png", "gif", "bmp"];
	    if ($.inArray($(this).val().split(".").pop().toLowerCase(), fileExtension) == -1) {
