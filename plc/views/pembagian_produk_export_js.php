				...
				$.ajax({
					...
					success: function(data) {	
						...
						if(o.status == true) {
							...
							$.get(base_url+'processor/plc/upd/prioritas/export?action=update&foreign_key=0&company_id='+company_id+'&id='+last_id+'&group_id='+group_id+'&modul_id='+modul_id, function(data) {
			...
			//if(!isUpload) {
				...
				.done(function(data) {
					...
					$('#grid_'+grid).trigger('reloadGrid');
						...
						$.get(base_url+'processor/plc/pembagian/produk/export?action=update&id='+last_id+'&foreign_key='+foreign_id+'&company_id='+company_id+'&group_id='+group_id+'&modul_id='+modul_id, function(data) {
	                            $('div#form_'+grid).html(data);
	                            $('html, body').animate({scrollTop:$('#'+grid).offset().top - 20}, 'slow');
	                    });
				});
...
function setuju(grid, url, dis, upbid, upbno, iTeam_andev){
	custom_confirm('Anda Yakin Confirm UPD : '+upbno,function(){
		$.ajax({
			url: url,
			type: 'post',
			data: 'grid='+grid+'&last_id='+upbid+'&iTeam_andev='+iTeam_andev,
			success: function(data) {
				var o = $.parseJSON(data);
				if(o.status==true){
					var info = 'success';
				...
				$.get(base_url+'processor/plc/pembagian/produk/export?action=view&id='+o.last_id+'&foreign_key='+o.foreign_id+'&company_id='+o.company_id+'&group_id='+o.group_id+'&modul_id='+o.modul_id, function(data) {
                        $('div#form_'+grid).html(data);
                        $('html, body').animate({scrollTop:$('#'+grid).offset().top - 20}, 'slow');
                });
			}		
