...
<table class="hover_table" id="ls" cellspacing="0" cellpadding="1" style="width: 98%; border: 1px solid #dddddd; text-align: center; margin-left: 5px; border-collapse: collapse">
	...
	<tbody>
		...
		$linknya = "";
		if (isset($mydept)) {
		if((in_array('BD', $mydept))||(in_array('QA', $mydept)) ||(in_array('PD', $mydept))) {
			//if(!empty($rows)) {
			if(!empty($rows)) {
				foreach($rows as $row) {
	...
	<tfoot>
		...
		<tr>
			<td colspan="5"></td><td style="text-align: center"><a href="javascript:;" onclick="javascript:add_row_ls('ls')">Tambah</a></td>
		</tr><?php //}?>
	</tfoot>
</table>