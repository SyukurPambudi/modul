...
$("#<?php echo $tableId;?> .file_remove").click(function(){
	...
	var tmpDel = $("#proses_produksi_del");
	li.remove();
	var v = tmpDel.val();
	v+=','+fileid;
	tmpDel.val( v );
	alert( $("#proses_produksi_del").val() );
});
function del_proses_produksi(dis, conname, table_id) {
	//if($('.'+conname).length > 1) {
		custom_confirm('Delete Selected Record?', function(){
			...
			if($('.'+conname).length==0) {
				...
				var n = $("span."+table_id+"_num:last").text();
					...
					row_content	 += '<span class="delete_btn"><a href="javascript:;" class="proses_produksi_del" onclick="del_proses_produksi(this, \'proses_produksi_del\',\'proses_produksi\')">[Hapus]</a></span></td>';		
					row_content  += '</tr>';
...
function add_row_proses_produksi(table_id){		
		//alert(table_id);
		var row = $('table#'+table_id+' tbody tr:last').clone();
		$("span."+table_id+"_num:first").text('1');
		var n = $("span."+table_id+"_num:last").text();
...
<table class="hover_table" id="proses_produksi" cellspacing="0" cellpadding="1" style="width: 98%; border: 1px solid #dddddd; text-align: center; margin-left: 5px; border-collapse: collapse">
	<thead>
	...
	<tbody>
		...
		if(in_array('PD', $mydept)) {
			...
			if($value != '') {
				if (file_exists('./files/plc/product_trial/basic_formula/pros_prod/'.$idsp.'/'.$value)) {
					$link = base_url().'processor/plc/product/trial/basic/formula?action=download1&id='.$idsp.'&path='.$type.'&file='.$value;
		...
		?>
				<tr style="border: 1px solid #dddddd; border-collapse: collapse; background: #ffffff; ">
					<td style="border: 1px solid #dddddd; width: 3%; text-align: center;">
						<span class="proses_produksi_num"><?php echo $i ?></span>
					</td>							
					...
					<td style="border: 1px solid #dddddd; width: 10%">
						<span class="delete_btn"><a href="javascript:;" class="proses_produksi_del" onclick="del_proses_produksi(this, 'proses_produksi_del','proses_produksi')">[Hapus]</a></span>
		...
		?>
				<tr style="border: 1px solid #dddddd; border-collapse: collapse; background: #ffffff; ">
					<td style="border: 1px solid #dddddd; width: 3%; text-align: center;">
						<span class="proses_produksi_num">1</span>
					</td>		
					<td colspan="2" style="border: 1px solid #dddddd; width: 27%">
	...
	<tfoot>
		<tr>
			<td colspan="5"></td><td style="text-align: center"><a href="javascript:;" onclick="javascript:add_row_proses_produksi('proses_produksi')">Tambah</a></td>
		</tr>
	</tfoot>
</table>