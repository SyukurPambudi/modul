...
<table class="hover_table" id="study_ad_upload" cellspacing="0" cellpadding="1" style="width: 98%; border: 1px solid #dddddd; text-align: center; margin-left: 5px; border-collapse: collapse">
	...
	<tr style="width: 98%; border: 1px solid #dddddd; background: #548cb6; border-collapse: collapse">
		<th colspan="4" style="border: 1px solid #dddddd;"><span style="font-weight: bold; color: #ffffff; text-shadow: 0 1px 1px rgba(0, 0, 0, 0.3); text-transform: uppercase;">Upload Memo Launching</span></th>
	</tr>
	<tr style="width: 100%; border: 1px solid #dddddd; background: #b3d2ea; border-collapse: collapse">
		...
		<th colspan="1" style="border: 1px solid #dddddd;width: 50%;">Keterangan</th>
		...
		<?php
			...
			if(!empty($rows)) {
				...
				foreach($rows as $row) {
					...
					$id  = $row['iLaunching_id'];
					$value = $row['vFileLaunching'];	
					if($value != '') {
						if (file_exists('./files/plc/memo_launching/'.$id.'/'.$value)) {
							$link = base_url().'processor/plc/import/data/lounching?action=downloadad&id='.$id.'&filead='.$value;
		...
		?>
				<tr style="border: 1px solid #dddddd; border-collapse: collapse; background: #ffffff; ">
					...
					<td colspan="1" style="border: 1px solid #dddddd; width: 27%">
						<?php echo $row['vFileLaunching'] ?> 
						<input type="hidden" name="namafile_studyad[]" style="width: 70%" value="<?php echo $row['vFileLaunching'] ?>" />
						<input type="hidden" name="fileid_studyad[]" style="width: 70%" value="<?php echo $row['iLaunchingfile_id'] ?>" />
					</td>	
