...
function add_row_accel(table_id){		
		//alert(table_id);
		...
		if (n.length == 0) {
			...
			row_content	 += '<input type="file" id="fileupload'+table_id+c+'" class="fileupload multi multifile required" name="fileuploada[]" style="width: 70%" />*';
			row_content  += '<input type="hidden" name="namafilea[]" style="width: 70%" value="" />';
			row_content  += '<input type="hidden" name="fileida[]" style="width: 70%" value="" /></td>';	
			row_content	 += '<td colspan="2" style="border: 1px solid #dddddd; width: 8%">';
			row_content	 += '<textarea class="" name="fileketerangana[]" style="width: 240px; height: 50px;"></textarea></td>';
			row_content	 += '<td style="border: 1px solid #dddddd; width: 10%">';
			row_content	 += '<span class="delete_btn"><a href="javascript:;" class="accel_del" onclick="del_row1(this, \'accel_del\')">[Hapus]</a></span></td>';		
			row_content  += '</tr>';
...
<table class="hover_table" id="accel" cellspacing="0" cellpadding="1" style="width: 98%; border: 1px solid #dddddd; text-align: center; margin-left: 5px; border-collapse: collapse">
	<thead>
	<tr style="width: 98%; border: 1px solid #dddddd; background: #548cb6; border-collapse: collapse">
		<th colspan="6" style="border: 1px solid #dddddd;"><span style="font-weight: bold; color: #ffffff; text-shadow: 0 1px 1px rgba(0, 0, 0, 0.3); text-transform: uppercase;">Upload File Accelerated</span></th>
	</tr>
	...
	<tbody>
		...
		?>
				<tr style="border: 1px solid #dddddd; border-collapse: collapse; background: #ffffff; ">
					<td style="border: 1px solid #dddddd; width: 3%; text-align: center;">
						<span class="accel_num"><?php echo $i ?></span>
					</td>							
					<td colspan="2" style="border: 1px solid #dddddd; width: 27%">
						<?php echo $row['filename'] ?> 
						<input type="hidden" name="namafilea[]" style="width: 70%" value="<?php echo $row['filename'] ?>" />
						<input type="hidden" name="fileida[]" style="width: 70%" value="<?php echo $row['id'] ?>" />
					</td>	
					...
					<td style="border: 1px solid #dddddd; width: 10%">
						...
						<span class="delete_btn"><a href="javascript:;" class="accel_del" onclick="del_row1(this, 'accel_del')">[Hapus]</a></span>
						<?php } ?>
					</td>		
				</tr>
		<?php
		...
		?>
				<tr style="border: 1px solid #dddddd; border-collapse: collapse; background: #ffffff; ">
					<td style="border: 1px solid #dddddd; width: 3%; text-align: center;">
						<span class="accel_num">1</span>
					</td>		
					<td colspan="2" style="border: 1px solid #dddddd; width: 27%">
						<input type="file" id="fileupload_accel_del1" class="fileupload multi multifile required" name="fileuploada[]" style="width: 70%" />*
						<input type="hidden" name="namafilea[]" style="width: 70%" value="" />
						<input type="hidden" name="fileida[]" style="width: 70%" value="" />
						</td>	
					<td colspan="2" style="border: 1px solid #dddddd; width: 15%">
						<textarea class="" name="fileketerangana[]" style="width: 240px; height: 50px;"></textarea>
					</td>
					<td style="border: 1px solid #dddddd; width: 10%">
						<?php if($this->input->get('action')!='view'){ ?> 
						<span class="delete_btn"><a href="javascript:;" class="accel_del" onclick="del_row1(this, 'accel_del')">[Hapus]</a></span>
	...
	<tfoot>
		...
		<tr>
			...
			<td style="text-align: center">
				<?php if($this->input->get('action')!='view'){ ?> 
					<a href="javascript:;" onclick="javascript:add_row_accel('accel')">Tambah</a>
				<?php } ?>
			</td>
		</tr><?php //}?>
	</tfoot>
...
$("#fileupload_accel_del1").change(function () {
    var fileExtension = ["pdf","jpeg", "jpg", "png", "gif", "bmp"];
    if ($.inArray($(this).val().split(".").pop().toLowerCase(), fileExtension) == -1) {
