...
$("#<?php echo $tableId;?> .file_remove").click(function(){
	...
	var tmpDel = $("#launching_upb1_del");
	li.remove();
	var v = tmpDel.val();
	v+=','+fileid;
	tmpDel.val( v );
	alert( $("#launching_upb1_del").val() );
});

function add_row_launching_upb1(table_id){		
		//alert(table_id);
		...
		if (n.length == 0) {
			...
			row_content	 += '<span class="delete_btn"><a href="javascript:;" class="launching_upb1_del" onclick="del_row1(this, \'launching_upb1_del\')">[Hapus]</a></span></td>';		
			row_content  += '</tr>';
...
<table class="hover_table" id="launching_upb1" cellspacing="0" cellpadding="1" style="width: 98%; border: 1px solid #dddddd; text-align: center; margin-left: 5px; border-collapse: collapse">
	<thead>
	...
	<tbody>
		...
		$linknya = "";
		if (isset($mydept)) {
		if((in_array('BD', $mydept))||(in_array('DR', $mydept))) {
		//print_r($mydept);
			//if(!empty($rows)) {
			if(!empty($rows)) {
				foreach($rows as $row) {
			...
			$type = "launching_upb1";
			if($value != '') {
				if (file_exists('./files/plc/memo_launching/'.$idsp.'/'.$value)) {
					$link = base_url().'processor/plc/launching/upb?action=download1&id='.$idsp.'&path='.$type.'&file='.$value;
		...
		?>
				<tr style="border: 1px solid #dddddd; border-collapse: collapse; background: #ffffff; ">
					<td style="border: 1px solid #dddddd; width: 3%; text-align: center;">
						<span class="launching_upb1_num"><?php echo $i ?></span>
					</td>							
					...
					<td style="border: 1px solid #dddddd; width: 10%">
						<span class="delete_btn"><a href="javascript:;" class="launching_upb1_del" onclick="del_row1(this, 'launching_upb1_del')">[Hapus]</a></span>
		...
		?>
				<tr style="border: 1px solid #dddddd; border-collapse: collapse; background: #ffffff; ">
					<td style="border: 1px solid #dddddd; width: 3%; text-align: center;">
						<span class="launching_upb1_num">1</span>
					</td>		
					<td colspan="2" style="border: 1px solid #dddddd; width: 27%">
	...
	<tfoot>
		<?php if((in_array('BD', $mydept))||(in_array('DR', $mydept))) {?>
		<tr>
			<td colspan="5"></td><td style="text-align: center"><a href="javascript:;" onclick="javascript:add_row_launching_upb1('launching_upb1')">Tambah</a></td>
		</tr><?php }?>
	</tfoot>
</table>