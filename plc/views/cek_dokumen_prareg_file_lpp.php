...
function add_row_lpp(table_id){		
		...
		if (n.length == 0) {
			...
			row_content  +=	'<select name="confirmlpp[]"><option value="0">Haven\'t been checked</option><option value="1">Yes</option><option value="3">No</option><option value="2">Unnecessary</option></select>';
			<?php } 
				else{?>
			row_content  +=	'<select name="confirmlpp[]"><option value="0">Haven\'t been checked</option></select>';
			<?php		}
				}?>
			row_content  +=	'</td>';
			row_content	 += '<td colspan="2" style="border: 1px solid #dddddd; width: 50%">';
			row_content	 += '<input type="file" id="fileupload'+table_id+c+'" class="fileupload multi multifile required" name="fileuploadlpp[]" style="width: 50%" />*';
			row_content  += '<input type="hidden" name="namafiledlpp[]" style="width: 70%" value="" />';
			row_content  += '<input type="hidden" name="fileidlpp[]" style="width: 70%" value="" /></td>';	
			row_content	 += '<td colspan="2" style="border: 1px solid #dddddd; width: 8%">';
			row_content	 += '<textarea class="" name="fileketeranganlpp[]" style="width: 240px; height: 50px;"></textarea></td>';
			...
			row_content	 += '<input type="file" id="fileupload'+table_id+c+'" class="fileupload multi multifile required" name="fileuploadlpp[]" style="width: 50%" />*';
			row_content  += '<input type="hidden" name="namafilelpp[]" style="width: 70%" value="" />';
			row_content  += '<input type="hidden" name="fileidlpp[]" style="width: 70%" value="" /></td>';
			row_content	 += '<td colspan="2" style="border: 1px solid #dddddd; width: 8%">';
			row_content	 += '<textarea class="" name="fileketeranganlpp[]" style="width: 240px; height: 50px;"></textarea></td>';
			row_content	 += '<td style="border: 1px solid #dddddd; width: 10%">';
...
<table class="hover_table" id="lpp" cellspacing="0" cellpadding="1" style="width: 98%; border: 1px solid #dddddd; text-align: center; margin-left: 5px; border-collapse: collapse">
	...
	<tr style="width: 98%; border: 1px solid #dddddd; background: #548cb6; border-collapse: collapse">
		<th colspan="7" style="border: 1px solid #dddddd;"><span style="font-weight: bold; color: #ffffff; text-shadow: 0 1px 1px rgba(0, 0, 0, 0.3); text-transform: uppercase;">Upload File Laporan Pengembangan Produksi</span></th>
	</tr>
	...
	<tbody>
		...
		?>
				<tr style="border: 1px solid #dddddd; border-collapse: collapse; background: #ffffff; ">
					...
					<td style="border: 1px solid #dddddd; width: 4%; text-align: center;">
						...
						?>
						<input type="hidden" name="confirmlpp[]" value="<?php echo $row['iconfirm_busdev'];?>">
						<span>
					...
					<td colspan="2" style="border: 1px solid #dddddd; width: 27%">
						...
						<input type="hidden" name="namafilelpp[]" style="width: 70%" value="<?php echo $row['vFilename'] ?>" />
						<input type="hidden" name="fileidlpp[]" style="width: 70%" value="<?php echo $row['ilpp_file_id'] ?>" />
					</td>	
		...
		?><!--
				<tr style="border: 1px solid #dddddd; border-collapse: collapse; background: #ffffff; ">
					...
					<td colspan="2" style="border: 1px solid #dddddd; width: 27%">
						<input type="file" id="fileuploadlpp1" class="fileupload multi multifile required" name="fileuploadlpp[]" style="width: 50%" /> *
						<input type="hidden" name="namafilelpp[]" style="width: 70%" value="" />
						<input type="hidden" name="fileidlpp[]" style="width: 70%" value="" />
						</td>	
					<td colspan="2" style="border: 1px solid #dddddd; width: 15%">
						<textarea class="" name="fileketeranganlpp[]" style="width: 240px; height: 50px;"></textarea>
					</td>
					<td style="border: 1px solid #dddddd; width: 10%">
						<span class="delete_btn"><a href="javascript:;" class="lpp_del" onclick="del_row1(this, 'lpp_del')">[Hapus]</a></span>
					</td>	
	...
	<tfoot>
		<tr>
			...
			<td style="text-align: center">
				<?php echo '&nbsp;&nbsp;&nbsp; <span onclick="javascript:load_popup(\''.base_url().'processor/plc/cek/dokumen/prareg?action=dokDone&dok=lpp&id='.$idsp.'&upb_id='.$iupb_id.'&group_id='.$this->input->get('group_id').'&modul_id='.$this->input->get('modul_id').'\')" class="ui-button-text icon-save ui-button ui-widget ui-state-default ui-corner-all ui-button-text-icon-primary" id="button_donelpp">Done</span>';?>
			</td>
			<?php }?>
		</tr>
		<?php if(($iDone==1)) {?>
			<tr>
			 <td colspan="3" style="text-align: left"><b><p style="color:green;font-size:120%;">Status Dokumen Laporan Pengembangan Produksi : DONE </p></b></td>
			</tr>
		<?php }?>
...
<script>
	$("#fileuploadlpp1").change(function () {
	    var fileExtension = ["pdf","jpeg", "jpg", "png", "gif", "bmp"];
	    if ($.inArray($(this).val().split(".").pop().toLowerCase(), fileExtension) == -1) {
