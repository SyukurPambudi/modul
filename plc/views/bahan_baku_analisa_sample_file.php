...
$("#<?php echo $tableId;?> .file_remove").click(function(){
	...
	var tmpDel = $("#bahan_baku_moa_del");
	li.remove();
	var v = tmpDel.val();
	v+=','+fileid;
	tmpDel.val( v );
	alert( $("#bahan_baku_moa_del").val() );
});
function del_bahan_baku_moa(dis, conname, table_id) {
	//if($('.'+conname).length > 1) {
		custom_confirm('Delete Selected Record?', function(){
			...
			if($('.'+conname).length==0) {
				...
				var n = $("span."+table_id+"_num:last").text();
					...
					row_content	 += '<span class="delete_btn"><a href="javascript:;" class="bahan_baku_moa_del" onclick="del_bahan_baku_moa(this, \'bahan_baku_moa_del\', \'bahan_baku_moa\')">[Hapus]</a></span></td>';		
					row_content  += '</tr>';
...
function add_row_bahan_baku_moa(table_id){		
		//alert(table_id);
		var row = $('table#'+table_id+' tbody tr:last').clone();
		$("span."+table_id+"_num:first").text('1');
		var n = $("span."+table_id+"_num:last").text();
...
<table class="hover_table" id="bahan_baku_moa" cellspacing="0" cellpadding="1" style="width: 98%; border: 1px solid #dddddd; text-align: center; margin-left: 5px; border-collapse: collapse">
	<thead>
	...
	<tbody>
		...
		if(in_array('PD', $mydept)) {
			...
			$path = "moa";	
			if($value != '') {
				if (file_exists('./files/plc/bahan_baku/moa/'.$idsp.'/'.$value)) {
		...
		?>
				<tr style="border: 1px solid #dddddd; border-collapse: collapse; background: #ffffff; ">
					<td style="border: 1px solid #dddddd; width: 3%; text-align: center;">
						<span class="bahan_baku_moa_num"><?php echo $i ?></span>
					</td>							
					...
					<td style="border: 1px solid #dddddd; width: 10%">
						<span class="delete_btn"><a href="javascript:;" class="bahan_baku_moa_del" onclick="del_bahan_baku_moa(this, 'bahan_baku_moa_del', 'bahan_baku_moa')">[Hapus]</a></span>
		...
		?>
				<tr style="border: 1px solid #dddddd; border-collapse: collapse; background: #ffffff; ">
					<td style="border: 1px solid #dddddd; width: 3%; text-align: center;">
						<span class="bahan_baku_moa_num">1</span>
					</td>		
					<td colspan="2" style="border: 1px solid #dddddd; width: 27%">
	...
	<tfoot>
		...
		?>
		<tr>
			<td colspan="5"></td><td style="text-align: center"><a href="javascript:;" onclick="javascript:add_row_bahan_baku_moa('bahan_baku_moa')">Tambah</a></td>
		</tr>
		<?php } ?>
	</tfoot>
</table>