...
$("#<?php echo $tableId;?> .file_remove").click(function(){
	...
	var tmpDel = $("#skala_lab_hud_del");
	li.remove();
	var v = tmpDel.val();
	v+=','+fileid;
	tmpDel.val( v );
	alert( $("#skala_lab_hud_del").val() );
});

function add_row_skala_lab_hud_upload(table_id){		
		//alert(table_id);
		...
		if (n.length == 0) {
			...
			row_content	 += '<input type="file" id="fileupload'+table_id+c+'" class="fileupload multi multifile required" name="fileupload_hud[]" style="width: 70%" />*';
			row_content  += '<input type="hidden" name="namafile_hud[]" style="width: 70%" value="" />';	
			row_content  += '<input type="hidden" name="fileid_hud[]" style="width: 70%" value="" /></td>';
			row_content	 += '<td colspan="3" style="border: 1px solid #dddddd; width: 8%">';
			row_content	 += '<textarea id= "filekt_hud'+c+'" class="" name="fileketerangan_hud[]" style="width: 240px; height: 50px;"></textarea><br/>tersisa <span id="len_hud'+c+'">250</span> karakter<br/></td>';
			row_content	 += '<td style="border: 1px solid #dddddd; width: 10%">';
			row_content	 += '<span class="delete_btn"><a href="javascript:;" class="skala_lab_hud_del" onclick="del_row(this, \'skala_lab_hud_del\')">[Hapus]</a></span></td>';		
			row_content  += '</tr>';
			...
			row_content  += '<input type="hidden" name="fileid_hud[]" style="width: 70%" value="" /></td>';
			row_content	 += '<td colspan="3" style="border: 1px solid #dddddd; width: 8%">';
			row_content	 += '<textarea id= "filekt_hud'+c+'"class="" name="fileketerangan_hud[]" style="width: 240px; height: 50px;"></textarea><br/>tersisa <span id="len_hud'+c+'">250</span> karakter<br/>';
			row_content  += '</td>';
			...
			$("span."+table_id+"_num:last").text(c);		
				...
				$("#filekt_hud"+c).keyup(function() {
					var len = this.value.length;
					if (len >= 250) {
					this.value = this.value.substring(0, 250);
					}
					$("#len_hud"+c).text(250 - len);
				});
...
<table class="hover_table" id="skala_lab_hud_upload" cellspacing="0" cellpadding="1" style="width: 98%; border: 1px solid #dddddd; text-align: center; margin-left: 5px; border-collapse: collapse">
	<thead>
	...
	<tbody>
		<?php
			...
			if(!empty($rows)) {
				...
				foreach($rows as $row) {
					...
					$path = "disolusi";	
					if($value != '') {
						if (file_exists('./files/plc/product_trial/skala_lab/disolusi/'.$idsp.'/'.$value)) {
		...
		?>
				<tr style="border: 1px solid #dddddd; border-collapse: collapse; background: #ffffff; ">
					<td style="border: 1px solid #dddddd; width: 3%; text-align: center;">
						<span class="skala_lab_hud_upload_num"><?php echo $i ?></span>
					</td>		
					<td colspan="1" style="border: 1px solid #dddddd; width: 27%">
						<?php echo $row['filename'] ?> 
						<input type="hidden" name="namafile_hud[]" style="width: 70%" value="<?php echo $row['filename'] ?>" />
						<input type="hidden" name="fileid_hud[]" style="width: 70%" value="<?php echo $row['id'] ?>" />
					</td>	
					...
					<td style="border: 1px solid #dddddd; width: 10%">
						<?php if ($this->input->get('action') != 'view') { ?>
							<span class="delete_btn"><a href="javascript:;" class="skala_lab_hud_del" onclick="del_row1(this, 'skala_lab_hud_del')">[Hapus]</a></span>	
						<?php	} ?>
		...
		<tr style="border: 1px solid #dddddd; border-collapse: collapse; background: #ffffff; ">
					<td style="border: 1px solid #dddddd; width: 3%; text-align: center;">
						<span class="skala_lab_hud_upload_num">1</span>
					</td>		
					<td colspan="1" style="border: 1px solid #dddddd; width: 27%">
						<input type="file" id="fileupload_skala_lab_hud_1" class="fileupload multi multifile required" name="fileupload_hud[]" style="width: 70%" /> *
						<input type="hidden" name="namafile_hud[]" style="width: 70%" value="" />
						<input type="hidden" name="fileid_hud[]" style="width: 70%" value="" />
						</td>	
					<td colspan="3" style="border: 1px solid #dddddd; width: 15%">
						<textarea class="" id="filekt_hud1"name="fileketerangan_hud[]" style="width: 240px; height: 50px;"size="250"></textarea>
						<br/>
						tersisa <span id="len_hud1">250</span> karakter<br/>
	...
	<tfoot>
		<tr>
			<td colspan="5" style="text-align: left">*) Max File 5 MB : Format = pdf,jpeg,jpg,png,gif,bmp</td><td style="text-align: center">
				<?php if ($this->input->get('action') != 'view') { ?>
					<a href="javascript:;" onclick="javascript:add_row_skala_lab_hud_upload('skala_lab_hud_upload')">Tambah</a>	
				<?php } ?>
	...
	<script>
		$('#filekt_hud1').keyup(function() {
		var len = this.value.length;
		if (len >= 250) {
		this.value = this.value.substring(0, 250);
		}
		$('#len_hud1').text(250 - len);
		});
		$("#fileupload_skala_lab_hud_1").change(function () {
	        var fileExtension = ["pdf","jpeg", "jpg", "png", "gif", "bmp"];
	        if ($.inArray($(this).val().split(".").pop().toLowerCase(), fileExtension) == -1) {
	        	_custom_alert("Only formats are allowed : "+fileExtension.join(", "),"info", "info", "skala_lab_hud_upload", 1, 20000);
	        	$(this).val('');
	        }
    	});
	</script>