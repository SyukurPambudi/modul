...
$gridurl=base_url()."processor/plc/cek_dokumen_registrasi?action=getuploadfile&id=".$get['id']."&field=".$id."&acc=".$get['action'];
...
/*Modify Grid */
   ...
   <?php 
    ...
    function addrow_detail_bd_files_<?php echo $id ?>_edit(){
        ...
        ?> 
        var opt="<select id='cek_dokumen_registrasi_<?php echo $id ?>_iconfirm_registrasi_"+rlast+"' name='cek_dokumen_registrasi_<?php echo $id ?>_iconfirm_registrasi[0][]' >";
        opt+="<?php echo $opt; ?>";
        opt+="</select>";
        var opt2="<select id='cek_dokumen_registrasi_<?php echo $id ?>_istatus_"+rlast+"' name='cek_dokumen_registrasi_<?php echo $id ?>_istatus[]' disabled='disabled'>";
        opt2+="<?php echo $opt2; ?>";
        opt2+="</select>";
    ...
    function addrow_detail_files_<?php echo $id ?>_edit(){
        ...
        ?> 
        var opt="<select id='cek_dokumen_registrasi_<?php echo $id ?>_iconfirm_registrasi_"+rlast+"' name='cek_dokumen_registrasi_<?php echo $id ?>_iconfirm_registrasi[]' disabled='disabled' >";
        opt+="<?php echo $opt; ?>";
        opt+="</select>";
        var opt2="<select id='cek_dokumen_registrasi_<?php echo $id ?>_istatus_"+rlast+"' name='cek_dokumen_registrasi_<?php echo $id ?>_istatus[0][]'>";
        opt2+="<?php echo $opt2; ?>";
        opt2+="</select>";
        var sa=[[opt,opt2,"<input type='file' id='fileupload_local_<?php echo $id ?>_"+rlast+"' class='fileupload multi multifile required' name='fileupload_local_<?php echo $id ?>[]' style='width: 90%' />","<textarea id='file_keterangan_local_<?php echo $id ?>_"+rlast+"' name='file_keterangan_local_<?php echo $id ?>[]' style='width: 240px; height: 50px;'></textarea>","<input type='hidden' class='num_rows_details_file_<?php echo $id ?>_edit' value='"+rlast+"' /><button id='ihapus_<?php echo $nmTable ?>' class='ui-button-text icon_hapus' style='width:75px' onclick='javascript:hapus_row_details_file_<?php echo $id ?>_edit("+rlast+")' type='button'>Hapus</button>"]];
        var lastr=jQuery("#<?php echo $nmTable; ?>").jqGrid('getGridParam', 'records');
    ...
    function hapus_row_details_file_<?php echo $id ?>_edit(rowId){
        ...
        if(i<=1){
            _custom_alert('Minimal Satu Upload','Error!','info','cek_dokumen_registrasi', 1, 5000);
            return false;
        }else{
    ...
    function done_process_<?php echo $id ?>_edit(){
        setuju('cek_dokumen_registrasi', '<?php echo base_url() ?>processor/plc/cek/dokumen/registrasi?action=doneprocess&field=<?php echo $id ?>&last_id=<?php echo $get['id'] ?>&foreign_key=<?php echo $get['foreign_key'] ?>&company_id=<?php echo $get['company_id'] ?>&group_id=<?php echo $get['group_id'] ?>&modul_id=<?php echo $get['modul_id'] ?>', this, '<?php echo $rowData['iupb_id'] ?>', '<?php echo $rowData['vupb_nomor'] ?>','processor/plc/cek/dokumen/registrasi?action=update','Apakah Yakin akan Done',false);        
    }
</script>