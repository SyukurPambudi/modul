...
$("#<?php echo $tableId;?> .file_remove").click(function(){
	...
	var tmpDel = $("#uji_mikro_bb_file_filename_del");
	li.remove();
	var v = tmpDel.val();
	v+=','+fileid;
	tmpDel.val( v );
	alert( $("#uji_mikro_bb_file_filename_del").val() );
});

function del_upload_uji_mikro_bb_file_filename(dis, conname, table_id) {
	if($('.'+conname).length > 1) {
		...
		var n = $("span."+table_id+"_num:last").text();
			...
			row_content	 += '<span class="delete_btn"><a href="javascript:;" class="uji_mikro_bb_file_filename_del" onclick="del_upload_uji_mikro_bb_file_filename(this, \'uji_mikro_bb_file_filename_del\', \'uji_mikro_bb_file_filename\')">[Hapus]</a></span></td>';		
			row_content  += '</tr>';
...
function add_row_uji_mikro_bb_file_filename(table_id){		
		//alert(table_id);
		var row = $('table#'+table_id+' tbody tr:last').clone();
		$("span."+table_id+"_num:first").text('1');
		var n = $("span."+table_id+"_num:last").text();
...
<table class="hover_table" id="uji_mikro_bb_file_filename" cellspacing="0" cellpadding="1" style="width: 98%; border: 1px solid #dddddd; text-align: center; margin-left: 5px; border-collapse: collapse">
	<thead>
	...
	<tbody>
		...
		if (isset($mydept)) {
			...
			if(!empty($rows)) {
				...
				//tambahan untuk download file
					$iuji_mikro_bb  = $row['iuji_mikro_bb'];
					$value = $row['filename'];
					$path = "rpt_mikro_bb";	
					if($value != '') {
						if (file_exists('./files/plc/local/uji_mikro_bb/rpt_mikro_bb/'.$iuji_mikro_bb.'/'.$value)) {
							$link = base_url().'processor/plc/uji/mikro/bb?action=download&id='.$iuji_mikro_bb.'&path='.$path.'&file='.$value;
		...
		?>
				<tr style="border: 1px solid #dddddd; border-collapse: collapse; background: #ffffff; ">
					<td style="border: 1px solid #dddddd; width: 3%; text-align: center;">
						<span class="uji_mikro_bb_file_filename_num"><?php echo $i ?></span>
					</td>							
					...
					<td style="border: 1px solid #dddddd; width: 10%">
						<span class="delete_btn"><a href="javascript:;" class="uji_mikro_bb_file_filename_del" onclick="del_upload_uji_mikro_bb_file_filename(this, 'uji_mikro_bb_file_filename_del' , 'uji_mikro_bb_file_filename')">[Hapus]</a></span>
		...
		?>
				<tr style="border: 1px solid #dddddd; border-collapse: collapse; background: #ffffff; ">
					<td style="border: 1px solid #dddddd; width: 3%; text-align: center;">
						<span class="uji_mikro_bb_file_filename_num">1</span>
					</td>		
					<td colspan="2" style="border: 1px solid #dddddd; width: 27%">
	...
	<tfoot>
		<tr>
			<td colspan="5"></td><td style="text-align: center"><a href="javascript:;" onclick="javascript:add_row_uji_mikro_bb_file_filename('uji_mikro_bb_file_filename')">Tambah</a></td>
		</tr>
	</tfoot>
</table>