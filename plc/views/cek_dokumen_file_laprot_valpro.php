...
$("#<?php echo $tableId;?> .file_remove").click(function(){
	...
	var tmpDel = $("#lapprot_valpro_del");
	li.remove();
	var v = tmpDel.val();
	v+=','+fileid;
	tmpDel.val( v );
	alert( $("#lapprot_valpro_del").val() );
});

function add_row_lapprot_valpro(table_id){		
		//alert(table_id);
		...
		if (n.length == 0) {
			...
			row_content  +=	'<select name="confirm7[]"><option value="0">Haven\'t been checked</option><option value="1">Yes</option><option value="3">No</option><option value="2">Unnecessary</option></select>';
			<?php } 
				else{?>
			row_content  +=	'<select name="confirm7[]"><option value="0">Haven\'t been checked</option></select>';
			<?		}
				}?>
			row_content  +=	'</td>';
			row_content	 += '<td colspan="2" style="border: 1px solid #dddddd; width: 50%">';
			row_content	 += '<input type="file" class="fileupload multi multifile required" name="fileupload7[]" style="width: 70%" />*max 5 mb';
			...
			row_content	 += '<span class="delete_btn"><a href="javascript:;" class="lapprot_valpro_del" onclick="del_row1(this, \'lapprot_valpro_del\')">[Hapus]</a></span></td>';		
			row_content  += '</tr>';
...
<table class="hover_table" id="lapprot_valpro" cellspacing="0" cellpadding="1" style="width: 98%; border: 1px solid #dddddd; text-align: center; margin-left: 5px; border-collapse: collapse">
	<thead>
	<tr style="width: 98%; border: 1px solid #dddddd; background: #548cb6; border-collapse: collapse">
		<th colspan="7" style="border: 1px solid #dddddd;"><span style="font-weight: bold; color: #ffffff; text-shadow: 0 1px 1px rgba(0, 0, 0, 0.3); text-transform: uppercase;">Upload File lapprot_valpro</span></th>
	</tr>
	...
	<tbody>
		...
		?>
				<tr style="border: 1px solid #dddddd; border-collapse: collapse; background: #ffffff; ">
					...
					<td style="border: 1px solid #dddddd; width: 4%; text-align: center;">
						...
						?>
						<input type="hidden" name="confirm7[]" value="<?php echo $row['iconfirm_busdev'];?>">
						<span>
		...
		?>
				<tr style="border: 1px solid #dddddd; border-collapse: collapse; background: #ffffff; ">
					...
					<td colspan="2" style="border: 1px solid #dddddd; width: 27%">
						<input type="file" class="fileupload multi multifile required" name="fileupload7[]" style="width: 70%" /> *max 5 mb
					...
					<td style="border: 1px solid #dddddd; width: 10%">
						<span class="delete_btn"><a href="javascript:;" class="lapprot_valpro_del" onclick="del_row1(this, 'lapprot_valpro_del')">[Hapus]</a></span>
					</td>	
				</tr>
		<?php }
		} 	
	...
	<tfoot>
		<td colspan="4"></td>
			...
			<td colspan="2" style="text-align: center"><a href="javascript:;" onclick="javascript:add_row_lapprot_valpro('lapprot_valpro')">Tambah</a></td>
		<?php } 
			if((in_array('BD', $mydept))&&($iDone==0)&&($cekconf==0)) {?>
			<td style="text-align: center">
				<?php echo '&nbsp;&nbsp;&nbsp; <span onclick="javascript:load_popup(\''.base_url().'processor/plc/cek/dokumen?action=dokDone&dok=lvalpro&id='.$idsp.'&upb_id='.$iupb_id.'&group_id='.$this->input->get('group_id').'&modul_id='.$this->input->get('modul_id').'\')" class="ui-button-text icon-save ui-button ui-widget ui-state-default ui-corner-all ui-button-text-icon-primary" id="button_doneLValpro">Done</span>';?>
			</td>
			<?php }?>
		</tr>
		<?php if(($iDone==1)) {?>
		<tr>
			 <td colspan="3" style="text-align: left"><b><p style="color:green;font-size:120%;">Status Dokumen Laporan dan Protokol Validasi Proses : DONE </p></b></td>
		</tr>
		<?php }?>
	</tfoot>
</table>