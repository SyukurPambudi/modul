		...
		custom_confirm(comfirm_message,function(){
			if(isUpload && !isValidAFileSize('#form_update_'+grid+' input.multifile', uploadLimit)) {
				...
				success: function(data) {
					...
					if(o.status == true){
						if(isUpload > 0) {								
							var iframe = $('<iframe name="cek_dokumen_prareg_frame"/>');
							iframe.attr({'id':'cek_dokumen_prareg_frame'});
							...
							$('#form_update_'+grid).attr('target','cek_dokumen_prareg_frame');
							
							//$('#form_update_'+grid).submit();	
							//cek_dokumen_prareg_tryUPload('form_update_'+grid , grid, formAction, base_url+'processor/plc/cek/dokumen/prareg?action=update&id='+o.last_id+'&foreign_key='+o.foreign_id+'&company_id='+o.company_id+'&group_id='+o.group_id+'&modul_id='+o.modul_id );							
						 	uploadfile('form_update_'+grid, grid, formAction, base_url+'processor/plc/cek/dokumen/prareg?action=update&id='+o.last_id+'&foreign_key='+o.foreign_id+'&company_id='+o.company_id+'&group_id='+o.group_id+'&modul_id='+o.modul_id);
						 }
						 else{
							...
							$.get(base_url+'processor/plc/cek/dokumen/prareg?action=update&id='+o.last_id+'&foreign_key='+o.foreign_id+'&company_id='+o.company_id+'&group_id='+o.group_id+'&modul_id='+o.modul_id, function(data) {
				...
				.done(function(data) {
					...
					$('#grid_'+grid).trigger('reloadGrid');
						...
						$.get(base_url+'processor/plc/cek/dokumen/prareg?action=update&id='+last_id+'&foreign_key='+foreign_id+'&company_id='+company_id+'&group_id='+group_id+'&modul_id='+modul_id, function(data) {
	                            $('div#form_'+grid).html(data);
	                            $('html, body').animate({scrollTop:$('#'+grid).offset().top - 20}, 'slow');
	                    });
				});*/
...
function setuju(grid, url, dis, upbid, upbno,url2='processor/plc/cek/dokumen/prareg?action=view',psn='Anda Yakin Confirm UPB :',upbm=true){
	if(upbm==false){
		upbno='';
	}
	custom_confirm(psn+' '+upbno+'?',function(){
		$.ajax({
			...
			success: function(data) {
				...
				$.get(base_url+url2+'&id='+o.last_id+'&foreign_key='+o.foreign_id+'&company_id='+o.company_id+'&group_id='+o.group_id+'&modul_id='+o.modul_id, function(data) {
                        $('div#form_'+grid).html(data);
                        $('html, body').animate({scrollTop:$('#'+grid).offset().top - 20}, 'slow');
                });
			}		
