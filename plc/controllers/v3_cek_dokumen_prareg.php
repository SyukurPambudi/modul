...
class v3_cek_dokumen_prareg extends MX_Controller {
    ...
    function index($action = '') {
		...
		$grid->setTitle('Cek Dokumen Praregistrasi');		
		...
		$grid->addFields('iupb_id','vupb_nama','vgenerik','iNew_formula','vNew_formula','dCekPraregCheckLengkap');
		...
		$grid->setLabel('dCekPraregCheckLengkap', 'Tgl Terima Ceklist lengkap PPUB');
		...
		$grid->setQuery('plc2_upb.ineed_prareg', 1);
		...
		switch ($action) {
			...
			case 'confirmpac':
		        ...
		        $content="
		                Diberitahukan bahwa telah ada Approval oleh PPIC Manager pada Cek Dokumen Praregistrasi(aplikasi PLC) dengan rincian sebagai berikut :<br><br>
			...
			case 'cekReviseQA':
				...
				foreach ($row1 as $kv => $vvvv) {
					...
					foreach($_POST as $key=>$value) {
						if($key == 'v3_cek_dokumen_prareg_'.$filejenis.'_istatus'){
			...
			case 'cekReviseBD':
				...
				foreach ($row1 as $kv => $vvvv) {
					...
					$filejenis=$vvvv['filename'];
					foreach($_POST as $key=>$value) {
						if($key == 'v3_cek_dokumen_prareg_'.$filejenis.'_iconfirm_prareg'){
							foreach($value as $y=>$u) {
								if($y!=0){
									if($u[0]==0 or $u[0]==3){
										$nilaiVal[$filejenis]["nilai"][]=1;
		...
		function updatebox_v3_cek_dokumen_prareg_iNew_formula($field, $id, $value,$rowData) {
			$o="";
			$x=$this->auth->dept();
			if($this->auth->is_manager()){
				$x=$this->auth->dept();
				...
				if ($this->input->get('action') == 'view') {
						...
						if($rowData['iCopy_brand'] == 1){
							...
							$o = 'Diisi jika UPB Copy brand <input type="hidden" name="'.$field.'" id="'.$id.'" value="" class="input_rows1" size="30" />';
						}
		...
		function updatebox_v3_cek_dokumen_prareg_vNew_formula($field, $id, $value,$rowData) {
			if($rowData['iCopy_brand'] == 1){
				...
				$return .= '<script>';
						$return .= '
												$("#v3_cek_dokumen_prareg_vNew_formula").hide();		
												$("#v3_cek_dokumen_prareg_iNew_formula").die();
												$("#v3_cek_dokumen_prareg_iNew_formula").live("change",function(){
													
													if($(this).val() == 1){
														$("#v3_cek_dokumen_prareg_vNew_formula").show();
													}else{
														$("#v3_cek_dokumen_prareg_vNew_formula").hide();
													}
				...
				$return = 'Diisi jika UPB Copy brand  <input type="hidden" name="'.$field.'" id="'.$id.'" value="" class="input_rows1" size="30" />';
			}
...
/*Maniupulasi Gird end*/
	function listBox_v3_cek_dokumen_prareg_iconfirm_dok($value) {
		$vstatus="";
...
/*manipulasi view object form start*/
	...
	function updateBox_v3_cek_dokumen_prareg_dCekPraregCheckLengkap($field, $id, $value, $rowData) {
		$type="";
		$x=$this->auth->dept();
		if($this->auth->is_manager()){
			$x=$this->auth->dept();
		...
		$return = '<script>
			$("label[for=\''.$this->url.'_dCekPraregCheckLengkap\']").parent().remove();
		</script>';
		if($type=="BD" && $req=$rowData["ibe"]==1){
			$value=($value=='0000-00-00' || $value==NULL)?"":$value;
			$return="<input type='text' value='".$value."' name='".$id."' id='".$id."' class='required readonly' size='10' />";
			$return.='<script>$("#'.$id.'").datepicker({dateFormat:"yy-mm-dd"});</script>';
		}
		return $return;
	}
...
/*manipulasi proses object form start*/
	function manipulate_update_button($buttons, $rowData){
		...
		$js .= $this->load->view('js/cek_dok_prareg_js',$data,TRUE);
		...
		$update = '<button onclick="javascript:update_btn_back_cek_dok_prareg(\''.$this->url.'\', \' '.base_url().'processor/'.$this->urlpath.'?last_id='.$iupb_id.'&company_id='.$this->input->get('company_id').'&group_id='.$this->input->get('group_id').'&modul_id='.$this->input->get('modul_id').' \',this,true )"  id="button_update_submit_'.$this->url.'"  class="ui-button-text icon-save" >Update &amp; Submit</button>';
		$updateQA = '<button onclick="javascript:update_btn_back_v3_cekdok_prareg_QA(\''.$this->url.'\', \' '.base_url().'processor/'.$this->urlpath.'?last_id='.$iupb_id.'&company_id='.$this->input->get('company_id').'&group_id='.$this->input->get('group_id').'&modul_id='.$this->input->get('modul_id').' \',this,true )"  id="button_update_submit_'.$this->url.'"  class="ui-button-text icon-save" >Update &amp; Submit</button>';
		$updateBD = '<button onclick="javascript:update_btn_back_v3_cekdok_prareg_BD(\''.$this->url.'\', \' '.base_url().'processor/'.$this->urlpath.'?last_id='.$iupb_id.'&company_id='.$this->input->get('company_id').'&group_id='.$this->input->get('group_id').'&modul_id='.$this->input->get('modul_id').' \',this,true )"  id="button_update_submit_'.$this->url.'"  class="ui-button-text icon-save" >Update &amp; Submit</button>';
		
		$refresh="";
		if($rowData["iCopy_brand"]==1){
			$refresh = '<button onclick="javascript:reload_copy(\''.$iupb_id.'\',\''.$this->url.'\', \' '.base_url().'processor/'.$this->urlpath.'?last_id='.$iupb_id.'&company_id='.$this->input->get('company_id').'&group_id='.$this->input->get('group_id').'&modul_id='.$this->input->get('modul_id').' \',this,true )"  id="button_refresh_'.$this->url.'"  class="ui-button-text icon-save" >Refresh Copy</button>';
		}
		...
		if ($this->input->get('action') == 'view') {unset($buttons['update']);}
			...
			/*---------------------Cek Untuk PD----------------------------------*/
			/*List Tabel*/
			$pddetailtable=$this->v3_m_prareg->getDetailtable("PD",$iupb_id,1);
			...
			/*---------------------Cek Untuk QA----------------------------------*/
			/*List Tabel*/
			$pddetailtableqa=$this->v3_m_prareg->getDetailtable("QA",$iupb_id,1);
			$tableqa=$pddetailtableqa['table'];
			$pktableqa=$pddetailtableqa['pktable'];/*PK per table*/
			$nitableqa=$pddetailtableqa['nitable'];
			$iM_modul_filedsqa=$pddetailtableqa['iM_modul_fileds'];
			/*End Table QA*/
			/*---------------------Cek Untuk PAC----------------------------------*/
			/*List Tabel*/
			$pddetailtablepac=$this->v3_m_prareg->getDetailtable("PAC",$iupb_id,1);
			...
			foreach ($tableqa as $ktb => $vtb) {
				/*Cek Untuk dokumen PD minimal 1 File*/
				$sqlc="select * from plc2.group_file_upload where iDeleted=0 and iM_modul_fileds=".$iM_modul_filedsqa[$ktb]." AND idHeader_File=".$nitableqa[$ktb];
				//echo '<pre>'.$sqlc.";" ;
				if($this->db_plc0->query($sqlc)->num_rows()==0){
					$iia++;
				}
			}
			if($type=='QA'&&$rowData['iconfirm_dok_pd']>0&&$rowData['iconfirm_dok_qa']==0){
				$approve = '<button onclick="javascript:setuju(\'v3_cek_dokumen_prareg\', \''.base_url().'processor/'.$this->urlpath.'?action=confirmqa&last_id='.$this->input->get('id').'&iM_modul_activity='.$iM_modul_activity.'&iM_activity='.$iM_activity.'&foreign_key='.$this->input->get('foreign_key').'&company_id='.$this->input->get('company_id').'&group_id='.$this->input->get('group_id').'&modul_id='.$this->input->get('modul_id').'\', this, '.$dt['iupb_id'].', \''.$dt['vupb_nomor'].'\')" class="ui-button-text icon-save" id="button_v3_cek_dokumen_prareg">Approve</button>';
				
				$filter=array('de.ikategori_id=3 and de.ijenisdok=1 and de.jenisplc=1');
				$row=$this->v3_m_prareg->getDetailFiles($filter)->result_array();
				foreach ($row as $krow => $vrow) {
					$ipk=$this->v3_m_prareg->getAnotherUPB($vrow['fieldheader'],$rowData['iupb_id']);
					$iM_modul_fileds=$vrow['iM_modul_fileds'];
					$this->db_plc0->select("*")
								->from("plc2.group_file_upload")
								->where("iM_modul_fileds",$iM_modul_fileds)
				...
				//$buttons['update']='fuck this shit '.$iia.'  - '.$iSRejectQA;
			}
			if($type=='BD'&&$rowData['iconfirm_dok_pd']>0&&$rowData['iconfirm_dok_qa']>0&&$rowData['iconfirm_dok']==0){
				...
				if($iia==0 && $iSRejectBD==0){
					if($iDone==0){
						...
						$buttons['update']=$update_draft.$updateBD.$js;
					}
				...
				//$buttons['update']='fuck this shit BD'.$iia.'  - '.$iSRejectBD;
			}
		}
		
		/* ---------- Cek Untuk Module Sebelumnya --------- */
		/* Cek Untuk Produksi Pilot */
		$query="select plc2_upb_formula.iupb_id from plc2.plc2_upb_formula
		join plc2.plc2_upb_prodpilot on plc2_upb_prodpilot.ifor_id=plc2_upb_formula.ifor_id
		where plc2_upb_prodpilot.ldeleted=0 
		and plc2_upb_formula.ldeleted=0 
		and plc2_upb_prodpilot.iapppd_pp=2
		and plc2_upb_formula.iupb_id=".$rowData['iupb_id'];
		$row1=$this->db_plc0->query($query)->num_rows();
		$isisemua=0;
		$tittle="";
		$contectModule="";
		$ss=0;
		if($row1==0){
			$ss++;
			$contectModule.="<li>Module Produksi Pilot [PD]</li>";
		}
		...
		/* Cek Untuk Bahan Kemas Sekunder */
		$query3="select iupb_id from plc2.plc2_upb_bahan_kemas where iappqa=2 and iJenis_bk=2 and ldeleted=0 and iupb_id=".$rowData['iupb_id'];
		$row3=$this->db_plc0->query($query3)->num_rows();
		if($row3==0 && $type!="PD"){
			$ss++;
			$contectModule.="<li>Module Bahan Kemas Sekunder [Packdev]</li>";
		}
		/* Cek Untuk SOI Mikro BB Final*/
		$query4="select plc2_upb_request_sample.iupb_id 
		from plc2.plc2_upb_request_sample
		join plc2.plc2_upb_request_sample_detail on plc2_upb_request_sample.ireq_id=plc2_upb_request_sample_detail.ireq_id
		join plc2.plc2_upb_ro_detail on plc2_upb_ro_detail.ireq_id=plc2_upb_request_sample_detail.ireq_id
		where plc2_upb_request_sample.ldeleted=0 
		and plc2_upb_request_sample_detail.ldeleted=0 
		and plc2_upb_ro_detail.ldeleted=0
		AND (CASE WHEN plc2_upb_ro_detail.iUjiMikro_bb=1 THEN
			plc2_upb_request_sample_detail.ireq_id IN (
				select plc2_upb_request_sample_detail.ireq_id from plc2.uji_mikro_bb 
					join plc2.plc2_upb_request_sample_detail on plc2_upb_request_sample_detail.ireqdet_id=uji_mikro_bb.ireqdet_id
					where plc2_upb_request_sample_detail.ldeleted=0 and uji_mikro_bb.lDeleted=0 AND uji_mikro_bb.iApprove_mikro_final=2
			)	
			ELSE
			plc2_upb_ro_detail.ldeleted=0
			END)
		and plc2_upb_request_sample.iupb_id=".$rowData['iupb_id'];
		$row4=$this->db_plc0->query($query4)->num_rows();
		if($row4==0 && $type!="PD"){
			$ss++;
			$contectModule.="<li>Module SOI Mikro BB Final [QA]</li>";
		}
		
		/* Cek Untuk SOI Bahan Baku*/
		$query5="select iupb_id from plc2.plc2_upb_soi_bahanbaku where iappqa=2 and ldeleted=0 and iupb_id=".$rowData['iupb_id'];
		$row5=$this->db_plc0->query($query5)->num_rows();
		if($row5==0 && $type!="PD"){
			$ss++;
			$contectModule.="<li>Module SOI Bahan Baku [QA]</li>";
		}
		/* Cek Untuk Bahan Kemas Primer*/
		$query5="select iupb_id from plc2.plc2_upb_bahan_kemas where iappqa=2 and ldeleted=0 and iupb_id=".$rowData['iupb_id'];
		$row5=$this->db_plc0->query($query5)->num_rows();
		if($row5==0){
			$ss++;
			$contectModule.="<li>Module Bahan Kemas Primer [PD]</li>";
		}
		
		/* mansur 27 April 2019 cek dulu ke study literatur pd */
			$this->db_plc0->where('iupb_id',$rowData['iupb_id']);
			$upbdata=$this->db_plc0->get("plc2.plc2_upb")->row_array();							
			$sqlstPd= 'select a.iuji_mikro 
						from plc2.study_literatur_pd a 
						where a.iupb_id="'.$rowData['iupb_id'].'" ';
			if($upbdata['iCopy_brand']==1){
				$sqlstPd= 'select a.iuji_mikro 
						from plc2.study_literatur_pd a 
						join plc2.plc2_upb u on u.iupb_id_ref=a.iupb_id
						where u.iupb_id="'.$rowData['iupb_id'].'" ';
			}
			$dstPd=$this->db_plc0->query($sqlstPd)->row_array();
			if(isset($dstPd['iuji_mikro'])){
				if($dstPd['iuji_mikro']==1){
					/* Cek Untuk SOI Mikro FG*/
						$query5="select * from plc2.plc2_upb_formula
						join plc2.study_literatur_pd on study_literatur_pd.iupb_id=plc2_upb_formula.iupb_id
							...
							AND	(
									CASE WHEN study_literatur_pd.iuji_mikro=1 THEN
										plc2_upb_formula.ifor_id IN (select mikro_fg.ifor_id from plc2.mikro_fg where mikro_fg.iappqa_soi=2 and mikro_fg.lDeleted=0 )
									ELSE
					...
					AND study_literatur_pd.iupb_id=".$rowData['iupb_id'];
						...
						if($row5==0){
							$ss++;
							$contectModule.="<li>Module SOI Mikro FG [PD]</li>";
						}

						/* Cek Untuk SOI Finish Good*/
						$query5="select iupb_id from plc2.soi_fg where iappqa=2 and ldeleted=0 and iupb_id=".$rowData['iupb_id'];
						$row5=$this->db_plc0->query($query5)->num_rows();
						if($row5==0 && $type!="PD"){
							$ss++;
							$contectModule.="<li>Module SOI Finish Good [QA]</li>";
						}
		...
		/* Cek Untuk SOI Approval KSK*/
		$query5="select plc2_upb_request_sample.iupb_id from plc2.plc2_upb_ro_detail 
			join plc2.plc2_upb_request_sample on plc2_upb_request_sample.ireq_id=plc2_upb_ro_detail.ireq_id
			where plc2_upb_ro_detail.ldeleted=0 and plc2_upb_request_sample.ldeleted=0
			and plc2_upb_request_sample.iupb_id=".$rowData['iupb_id'];
		$row5=$this->db_plc0->query($query5)->num_rows();
		if($row5==0 && $type!="PD"){
			$ss++;
			$contectModule.="<li>Module Approval KSK[QA]</li>";
		}
		...
		$query4="select * from plc2.plc2_upb_prioritas_detail
		join plc2.plc2_upb_prioritas on plc2_upb_prioritas_detail.iprioritas_id=plc2_upb_prioritas.iprioritas_id
		where plc2_upb_prioritas_detail.ldeleted=0 and plc2_upb_prioritas_detail.ldeleted=0 and plc2_upb_prioritas.iappdir=2
		and plc2_upb_prioritas_detail.iupb_id=".$rowData['iupb_id'];
		$row4=$this->db_plc0->query($query4)->num_rows();
		if($row4==0){
			$nn++;
			$contectModule.="<li>Module Setting Prioritas [Direksi]</li>";
		}

		if($rowData['iCopy_brand']==1 && $nn>0){
			$tittle.="<span style='font-weight:bold'>Module belum dilalui :</span><br /></span><span style='color:red'><ul style='list-style-type: circle;;margin-left:10px;'>".$contectModule."</ul></span>";
		}
		/* $sButton = $iframe.$js;
		$sButton .= $update_draft.$update; */
		
		$dupb=$this->v3_m_prareg->getUPBDetails($rowData['iupb_id']);
		$filter=array('de.ikategori_id=3 and de.ijenisdok=1 and de.jenisplc=1');
		if($type=="PD"){
			$filter=array('de.ikategori_id=3 and de.ijenisdok=1 and de.jenisplc=1 and de.isPD=1');
		}
		$row=$this->v3_m_prareg->getDetailFiles($filter)->result_array();
		$get=$this->input->get();
		$belumissi=array();
		...
		if($rowData["iCopy_brand"]==1){
				...
				if($rowData['iconfirm_dok']==0){
					if($iia==0 && $iSRejectBD==0 && $iDone==0){
						...
						*/
						...
						if($dMother['iconfirm_dok'] == 1 ){
							$buttons['update'] = $refresh.$update_draft.$update.$approve.$js;
						}else{
							$buttons['update'] = $refresh.$update_draft.$update.$js.'<br>'.'UPB Original belum melewati Approval BD';
						}
			...
			elseif($type=="PD" && $rowData['iconfirm_dok_pd']==0){
				$approve = '<button onclick="javascript:setuju(\'v3_cek_dokumen_prareg\', \''.base_url().'processor/'.$this->urlpath.'?action=confirmpd&last_id='.$this->input->get('id').'&iM_modul_activity='.$iM_modul_activity.'&iM_activity='.$iM_activity.'&foreign_key='.$this->input->get('foreign_key').'&company_id='.$this->input->get('company_id').'&group_id='.$this->input->get('group_id').'&modul_id='.$this->input->get('modul_id').'\', this, '.$dt['iupb_id'].', \''.$dt['vupb_nomor'].'\')" class="ui-button-text icon-save" id="button_v3_cek_dokumen_prareg">Approve</button>';
			...
			else{ $buttons['update']=$buttons['update'] = $refresh.$js; }
		}
