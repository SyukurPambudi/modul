...
class v3_draft_soi_mikro_bb extends MX_Controller {
    function __construct() {
        ...
        $this->path = 'files/plc/local/draft_soi_mikro_bb/rpt_draft_soi_mikro';
        $this->tempat = 'local/draft_soi_mikro_bb/rpt_draft_soi_mikro';
        $this->folderApp = 'plc';

        $this->url = 'v3_draft_soi_mikro_bb';
    ...
    function index($action = '') {
                        ...
                        foreach ($dFields as $form_field) {
                            ...
                            if($get['formaction']=='update'){
                                ...
                                if($form_field['iM_jenis_field'] == 19){
                                    ...
                                    $data_field['qrFiles'] = 'SELECT id, iuji_mikro_bb, filename AS vFilename, keterangan AS vKeterangan, IF (vFilename_generate IS NOT NULL, vFilename_generate, filename) AS vFilename_generate FROM plc2.file_draft_soi_mikro WHERE '.$this->main_table_pk.' = '.$dataHead[$this->main_table_pk].' AND ( ldeleted = 0 OR ldeleted IS NULL)';
                                }else{
        ...
        function listBox_v3_draft_soi_mikro_bb_iappqc($value) {
        ...
        function listBox_v3_draft_soi_mikro_bb_iappqa($value) {
            if($value==0){$vstatus='Waiting for approval';}
            elseif($value==1){$vstatus='Rejected';}
            elseif($value==2){$vstatus='Approved';}
            return $vstatus;
        ...
        function listBox_v3_draft_soi_mikro_bb_itype($value) {
        ...
        function insertBox_v3_draft_soi_mikro_bb_form_detail($field,$id){
            $get=$this->input->get();
            $post=$this->input->post();
            ...
            $g=implode("&", $in);
            $return = '<script>
                var sebelum = $("label[for=\'v3_draft_soi_mikro_bb_form_detail\']").parent();
                $("label[for=\'v3_draft_soi_mikro_bb_form_detail\']").remove();
                sebelum.attr("id","'.$id.'");
                sebelum.html("");
                sebelum.removeAttr("class");
                sebelum.removeAttr("style");
                $.ajax({
                    url: base_url+"processor/plc/v3/draft/soi/mikro/bb?action=getFormDetail&formaction=addnew&'.$g.'",
                    type: "post",
                    data: iro_id=0,
        ...
        function updateBox_v3_draft_soi_mikro_bb_form_detail($field,$id,$value,$rowData){
            $get=$this->input->get();
            $post=$this->input->post();
            foreach ($get as $kget => $vget) {
                if($kget!="action"){
            ...
            $return = '<script>
                ...
                sebelum.removeAttr("style");
                $.ajax({
                    url: base_url+"processor/plc/v3/draft/soi/mikro/bb?action=getFormDetail&formaction=update&'.$g.'",
                    type: "post",
                    data: iro_id=0,
                    success: function(data) {
                        var o = $.parseJSON(data);
            ...
            function approve_view() { 
                             ...
                             function submit_ajax(form_id) {
                                return $.ajax({
                                    ...
                                    success: function(data) {
                                        ...
                                        var url = "'.base_url().'processor/plc/v3_draft_soi_mikro_bb";                             
                                        if(o.status == true) { 
                                            $("#alert_dialog_form").dialog("close");
                                                 $.get(url+"?action=update&id="+last_id+"&foreign_key=0&company_id=3&group_id="+group_id+"&modul_id="+modul_id, function(data) {
                                                 $("div#form_v3_draft_soi_mikro_bb").html(data);
                                                 
                                            });
                                            
                                        }
                                            reload_grid("grid_v3_draft_soi_mikro_bb");
                                    }
                ...
                $echo .= '<form id="form_v3_draft_soi_mikro_bb_approve" action="'.base_url().'processor/plc/v3_draft_soi_mikro_bb?action=approve_process" method="post">';
                ...
                <button type="button" onclick="submit_ajax(\'form_v3_draft_soi_mikro_bb_approve\')">Approve</button>';
            ...
            function confirm_view() { 
                ...
                $echo .= '<form id="form_v3_draft_soi_mikro_bb_confirm" action="'.base_url().'processor/plc/v3_draft_soi_mikro_bb?action=confirm_process" method="post">';
                $echo .= '<div style="vertical-align: top;">';
                $echo .= 'Remark : 
                        <input type="hidden" name="'.$this->main_table_pk.'" value="'.$this->input->get($this->main_table_pk).'" />
                        <input type="hidden" name="modul_id" value="'.$this->input->get('modul_id').'" />
                        ...
                        <textarea name="vRemark"></textarea>
                <button type="button" onclick="submit_ajax(\'form_v3_draft_soi_mikro_bb_confirm\')">Confirm</button>';
            ...
            function reject_view() {
                $echo = '<script type="text/javascript">
                             function submit_ajax(form_id) {
                                var remark = $("#v3_draft_soi_mikro_bb_remark").val();
                                if (remark=="") {
                ...
                $echo .= '<form id="form_v3_draft_soi_mikro_bb_reject" action="'.base_url().'processor/plc/v3_draft_soi_mikro_bb?action=reject_process" method="post">';
                $echo .= '<div style="vertical-align: top;">';
                $echo .= 'Remark : 
                        <input type="hidden" name="'.$this->main_table_pk.'" value="'.$this->input->get($this->main_table_pk).'" />
                        <input type="hidden" name="modul_id" value="'.$this->input->get('modul_id').'" />
                        ...
                        <textarea name="vRemark" id="reject_v3_draft_soi_mikro_bb_remark"></textarea>
                <button type="button" onclick="submit_ajax(\'form_v3_draft_soi_mikro_bb_reject\')">Reject</button>';
    ...
    function after_insert_processor($fields, $id, $postData) { 
        ...
        if ($postData['iSubmit_draft_soi'] == 1){
    ...
    function manipulate_insert_button($buttons) { 
        ...
        $iframe = '<iframe name="v3_draft_soi_mikro_bb_frame" id="v3_draft_soi_mikro_bb_frame" height="0" width="0"></iframe>';
        ...
        $save_draft = '<button onclick="javascript:save_draft_btn_multiupload(\'v3_draft_soi_mikro_bb\', \' '.base_url().'processor/plc/v3_draft_soi_mikro_bb?draft=true&modul_id='.$this->input->get('modul_id').'&iM_modul_activity='.$act['iM_modul_activity'].' \',this,true )"  id="button_save_draft_v3_draft_soi_mikro_bb"  class="ui-button-text icon-save" >Save as Draft</button>';
        $save = '<button onclick="javascript:save_btn_multiupload(\'v3_draft_soi_mikro_bb\', \' '.base_url().'processor/plc/v3_draft_soi_mikro_bb?company_id='.$this->input->get('company_id').'&modul_id='.$this->input->get('modul_id').'&iM_modul_activity='.$act['iM_modul_activity'].'&group_id='.$this->input->get('group_id').'modul_id='.$this->input->get('modul_id').' \',this,false )"  id="button_save_submit_v3_draft_soi_mikro_bb"  class="ui-button-text icon-save" >Save &amp; Submit</button>';
    ...
    function manipulate_update_button($buttons, $rowData) { 
        ...
        if ($this->input->get('action') == 'view') {
            unset($buttons['update']);
                ...
                if($isOpenEditing){
                    $update_draft = '<button onclick="javascript:update_draft_btn(\'v3_draft_soi_mikro_bb\', \' '.base_url().'processor/plc/v3_draft_soi_mikro_bb?draft=true \',this,true )"  id="button_update_draft_v3_draft_soi_mikro_bb"  class="ui-button-text icon-save" >Update open Editing</button>';
                    ...
                    foreach ($activities as $act) {
                        $update_draft = '<button onclick="javascript:update_draft_btn(\'v3_draft_soi_mikro_bb\', \' '.base_url().'processor/plc/v3_draft_soi_mikro_bb?draft=true&modul_id='.$this->input->get('modul_id').'&iM_modul_activity='.$act['iM_modul_activity'].' \',this,true )"  id="button_update_draft_v3_draft_soi_mikro_bb"  class="ui-button-text icon-save" >Update as Draft</button>';
                        $update = '<button onclick="javascript:update_btn_back(\'v3_draft_soi_mikro_bb\', \' '.base_url().'processor/plc/v3_draft_soi_mikro_bb?company_id='.$this->input->get('company_id').'&modul_id='.$this->input->get('modul_id').'&iM_modul_activity='.$act['iM_modul_activity'].'&group_id='.$this->input->get('group_id').'modul_id='.$this->input->get('modul_id').' \',this,false )"  id="button_update_submit_v3_draft_soi_mikro_bb"  class="ui-button-text icon-save" >Update &amp; Submit</button>';

                        $approve = '<button onclick="javascript:load_popup(\' '.base_url().'processor/plc/v3_draft_soi_mikro_bb?action=approve&iM_modul_activity='.$act['iM_modul_activity'].'&iM_activity='.$act['iM_activity'].'&'.$this->main_table_pk.'='.$peka.'&iupb_id='.$iupb_id.'&company_id='.$this->input->get('company_id').'&group_id='.$this->input->get('group_id').'&modul_id='.$this->input->get('modul_id').' \')"  id="button_approve_v3_draft_soi_mikro_bb"  class="ui-button-text icon-save" >Approve</button>';
                        $reject = '<button onclick="javascript:load_popup(\' '.base_url().'processor/plc/v3_draft_soi_mikro_bb?action=reject&iM_modul_activity='.$act['iM_modul_activity'].'&iM_activity='.$act['iM_activity'].'&'.$this->main_table_pk.'='.$peka.'&iupb_id='.$iupb_id.'&company_id='.$this->input->get('company_id').'&group_id='.$this->input->get('group_id').'&modul_id='.$this->input->get('modul_id').' \' )"  id="button_reject_v3_draft_soi_mikro_bb"  class="ui-button-text icon-save" >Reject</button>';

                        $confirm = '<button onclick="javascript:load_popup(\' '.base_url().'processor/plc/v3_draft_soi_mikro_bb?action=confirm&iM_modul_activity='.$act['iM_modul_activity'].'&iM_activity='.$act['iM_activity'].'&'.$this->main_table_pk.'='.$peka.'&iupb_id='.$iupb_id.'&company_id='.$this->input->get('company_id').'&group_id='.$this->input->get('group_id').'&modul_id='.$this->input->get('modul_id').' \')"  id="button_approve_v3_draft_soi_mikro_bb"  class="ui-button-text icon-save" >Confirm</button>';
