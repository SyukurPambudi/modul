...
class study_literatur_pd_popup extends MX_Controller {
...
$this->db_plc0 = $this->load->database('plc0',false, true);
    ...
    function index($action = '') {
		...
		$grid->setUrl('study_literatur_pd_popup');
...
where pros.idprivi_modules='.$this->input->get('modul_id').')',NULL);*/
		...
		$grid->setQuery('iupb_id not in (select iupb_id from plc2.study_literatur_pd where ldeleted=0 and iapppd !=1 )',null);
	...
	function listBox_study_literatur_pd_popup_pilih($value, $pk, $name, $rowData) {
