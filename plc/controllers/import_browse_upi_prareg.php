...
class import_browse_upi_prareg extends MX_Controller {
...
$this->db_plc0 = $this->load->database('plc0',false, true);
    ...
    function index($action = '') {
		...
		$grid->setUrl('import_browse_upi_prareg');
		...
		$grid->setQuery('iStatusUji = "0" ', null);
		$grid->setQuery('daftar_upi.iStatusKill = "0" ', null);
		//upi sudah approve prinsipal
		$grid->setQuery('daftar_upi.iupi_id in (SELECT b.iupi_id FROM plc2.analisa_prinsipal b WHERE b.iApprove_bdirm IN(2))', null);
		//upi tidak sedang aktif di prareg atau sudah approve
		$grid->setQuery('daftar_upi.iupi_id not in (select a.iupi_id from plc2.praregistrasi_upi a where a.iApprove_bdirm in(2,0)) ', null);
	...
	function listBox_import_browse_upi_prareg_pilih($value, $pk, $name, $rowData) {
