...
class v3_protokol_valpro extends MX_Controller {
    function __construct() {
		...
		$this->url = 'v3_protokol_valpro';
		$this->urlpath = 'plc/'.str_replace("_","/", $this->url);

		$this->maintable = 'plc2.protokol_valpro';	
		$this->main_table = $this->maintable;	
		$this->main_table_pk = 'iprotokol_id';	
		$datagrid['islist'] = array(
			...
			,'dmulai_protokol' => array('label'=>'Tgl Mulai','width'=>100,'align'=>'left','search'=>false)
			,'dselesai_protokol' => array('label'=>'Tgl selesai','width'=>100,'align'=>'left','search'=>false)
		...
		$datagrid['setQuery']=array(
								0=>array('vall'=>'protokol_valpro.lDeleted','nilai'=>0)
								);
		$datagrid['jointableinner']=array(
									0=>array('plc2.plc2_upb'=>'plc2_upb.iupb_id=protokol_valpro.iupb_id')
								);
	...
	/*List Box*/
	function listBox_v3_protokol_valpro_cPIC_AD($value, $pk, $name, $rowData) {
	...
	function listBox_v3_protokol_valpro_plc2_upb_iteampd_id($value, $pk, $name, $rowData) {
	...
	function insertBox_v3_protokol_valpro_form_detail($field,$id){
            $get=$this->input->get();
            $post=$this->input->post();
        ...
        function updateBox_v3_protokol_valpro_form_detail($field,$id,$value,$rowData){
            $get=$this->input->get();
            $post=$this->input->post();
            foreach ($get as $kget => $vget) {
                if($kget!="action"){
