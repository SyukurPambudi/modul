...
class v3_bk_primer extends MX_Controller {
    ...
    function index($action = '') {
        ...
        $grid = new Grid;
        $grid->setTitle('Bahan Kemas Primer');       
        $grid->setTable($this->_table);     
        $grid->setUrl('v3_bk_primer');
        ...
        /*basic required start*/
            ...
            $grid->setQuery('plc2_upb_bahan_kemas.iJenis_bk', 1);
        ...
        switch ($action) {
            ...
            case 'updateproses':
                $isUpload   = $this->input->get('isUpload'); 
                $post       = $this->input->post();
                $lastId     = $post['v3_bk_primer_ibk_id'];
        ...
        function copyOri_view() {
                         ...
                         function submit_ajax(form_id) {
                            return $.ajax({
                                ...
                                success: function(data) {
                                    ...
                                    var header = "Info";
                                    var info = "Info";
                                    var url = "'.base_url().'processor/plc/v3/bk/primer";                             
                                    if(o.status == true) {
                                        //alert($ibk_id);
                                        ...
                                        // $.get(url+"processor/plc/v3/bk/primer?action=update&id="+last_id+"&foreign_key="+foreign_id+"&company_id="+company_id+"&group_id="+group_id+"&modul_id="+modul_id, function(data) {
                                            // $("div#form_upb_daftar").html(data);
            ...
            $echo .= '<form id="form_v3_bk_primer_copyOri" action="'.base_url().'processor/plc/v3/bk/primer?action=copyOri_process" method="post">';
            ...
            <button type="button" onclick="submit_ajax(\'form_v3_bk_primer_copyOri\')">Simpan</button>';
        ...
        function copyKomp_view() {
            ...
            $echo .= '<form id="form_v3_bk_primer_copyKomp" action="'.base_url().'processor/plc/v3/bk/primer?action=copyKomp_process" method="post">';
            $echo .= '<div style="vertical-align: top;">';
            $echo .= '<input type="hidden" name="ibk_id" value="'.$this->input->get('upb_id').'" />
                    <input type="hidden" name="group_id" value="'.$this->input->get('group_id').'" />
                    <input type="hidden" name="modul_id" value="'.$this->input->get('modul_id').'" />
            ...
            <button type="button" onclick="submit_ajax(\'form_v3_bk_primer_copyKomp\')">Simpan</button>';
        ...
        function listBox_v3_bk_primer_iSubmit($value) {
            if($value==0){$vstatus='Need To Submit';}
            elseif($value==1){$vstatus='Submited';}
            return $vstatus;
        }
        function listBox_v3_bk_primer_iapppc($value) {
            if($value==0){$vstatus='Waiting for approval';}
            elseif($value==1){$vstatus='Rejected';}
            elseif($value==2){$vstatus='Approved';}
            return $vstatus;
        }
        function listBox_v3_bk_primer_iapppd($value) {
            if($value==0){$vstatus='Waiting for approval';}
            elseif($value==1){$vstatus='Rejected';}
            elseif($value==2){$vstatus='Approved';}
            return $vstatus;
        }
        function listBox_v3_bk_primer_iappqa($value) {
            if($value==0){$vstatus='Waiting for approval';}
            elseif($value==1){$vstatus='Rejected';}
            elseif($value==2){$vstatus='Approved';}
            return $vstatus;
        }
        function listBox_v3_bk_primer_iappbd($value) {
            if($value==0){$vstatus='Waiting for approval';}
            elseif($value==1){$vstatus='Rejected';}
            elseif($value==2){$vstatus='Approved';}
            return $vstatus;
        ...
        function insertBox_v3_bk_primer_form_detail($field,$id){
            $get=$this->input->get();
            $post=$this->input->post();
            ...
            $g=implode("&", $in);
            $return = '<script>
                var sebelum = $("label[for=\'v3_bk_primer_form_detail\']").parent();
                $("label[for=\'v3_bk_primer_form_detail\']").remove();
                sebelum.attr("id","'.$id.'");
                sebelum.html("");
                sebelum.removeAttr("class");
                sebelum.removeAttr("style");
                $.ajax({
                    url: base_url+"processor/plc/v3/bk/primer?action=getFormDetail&formaction=addnew&'.$g.'",
                    type: "post",
                    data: ibk_id=0,
                    success: function(data) {
                        var o = $.parseJSON(data);
        ...
        function updateBox_v3_bk_primer_form_detail($field,$id,$value,$rowData){
            $get=$this->input->get();
            $post=$this->input->post();
            foreach ($get as $kget => $vget) {
                if($kget!="action"){
            ...
            $return = '<script>
                ...
                sebelum.removeAttr("style");
                $.ajax({
                    url: base_url+"processor/plc/v3/bk/primer?action=getFormDetail&formaction=update&'.$g.'",
                    type: "post",
                    data: ibk_id=0,
                    success: function(data) {
                        var o = $.parseJSON(data);
            ...
            function approve_view() { 
                ...
                $echo .= '<form id="form_v3_bk_primer_approve" action="'.base_url().'processor/plc/v3_bk_primer?action=approve_process" method="post">';
                $echo .= '<div style="vertical-align: top;">';
                $echo .= 'Remark : 
                ...
                <button type="button" onclick="submit_ajax(\'form_v3_bk_primer_approve\')">Approve</button>';
            ...
            function confirm_view() {
                ...
                $echo .= '<form id="form_v3_bk_primer_confirm" action="'.base_url().'processor/plc/v3_bk_primer?action=confirm_process" method="post">';
                $echo .= '<div style="vertical-align: top;">';
                $echo .= 'Remark : 
                        <input type="hidden" name="'.$this->main_table_pk.'" value="'.$this->input->get($this->main_table_pk).'" />
                        <input type="hidden" name="modul_id" value="'.$this->input->get('modul_id').'" />
                        ...
                        <textarea name="vRemark"></textarea>
                <button type="button" onclick="submit_ajax(\'form_v3_bk_primer_confirm\')">Confirm</button>';
            ...
            function reject_view() {
                $echo = '<script type="text/javascript">
                             function submit_ajax(form_id) {
                                var remark = $("#v3_bk_primer_remark").val();
                                if (remark=="") {
                ...
                $echo .= '<form id="form_v3_bk_primer_reject" action="'.base_url().'processor/plc/v3_bk_primer?action=reject_process" method="post">';
                $echo .= '<div style="vertical-align: top;">';
                $echo .= 'Remark : 
                        <input type="hidden" name="'.$this->main_table_pk.'" value="'.$this->input->get($this->main_table_pk).'" />
                        <input type="hidden" name="modul_id" value="'.$this->input->get('modul_id').'" />
                        ...
                        <textarea name="vRemark" id="reject_v3_bk_primer_remark"></textarea>
                <button type="button" onclick="submit_ajax(\'form_v3_bk_primer_reject\')">Reject</button>';
    ...
    function after_insert_processor($fields, $id, $postData) { 
        $post = $this->input->post();
        $jenisbk = $post['jenisbk'];
        $modul_id = $this->modul_id;
        foreach($jenisbk as $k => $v) {
            if(!empty($v)) {
                $idet['ibk_id'] = $id;
                $idet['ijenis_bk_id'] = $jenisbk[$k];
                $this->db->insert('plc2.plc2_upb_bk_primer_detail', $idet);
            }
        }  
    ...
    function after_update_processor($fields, $id, $postData) { 
        ...
        $post = $this->input->post(); 
        $jenisbk = $post['jenisbk']; 
        $eRows = $this->db->get_where('plc2.plc2_upb_bk_primer_detail', array('ibk_id'=>$id, 'ldeleted'=>0))->result_array(); 
        foreach($eRows as $k => $v) {
            if(in_array($v['ibk_primer_id'], $jenisbk)) {
                $this->db->where('ibk_primer_id', $v['ibk_primer_id']);
                $key = array_search($v['ibk_primer_id'], $jenisbk);
                $this->db->update('plc2.plc2_upb_bk_primer_detail', array('iJenis_bk'=>$jenisbk[$key]));
            } else {
                $this->db->where('ibk_primer_id', $v['ibk_primer_id']);
                $this->db->update('plc2.plc2_upb_bk_primer_detail', array('ldeleted'=>1));
            }
        }
    ...
    function manipulate_insert_button($buttons) { 
        ...
        $iframe = '<iframe name="v3_bk_primer_frame" id="v3_bk_primer_frame" height="0" width="0"></iframe>';
        
        $save_draft = '<button onclick="javascript:save_draft_btn_multiupload(\'v3_bk_primer\', \' '.base_url().'processor/plc/v3_bk_primer?draft=true \',this,true )"  id="button_save_draft_v3_bk_primer"  class="ui-button-text icon-save" >Save As Draft</button>';
        $save = '<button onclick="javascript:save_btn_multiupload(\'v3_bk_primer\', \' '.base_url().'processor/plc/v3_bk_primer?company_id='.$this->input->get('company_id').'&group_id='.$this->input->get('group_id').'modul_id='.$this->input->get('modul_id').' \',this,true )"  id="button_save_submit_v3_bk_primer"  class="ui-button-text icon-save" >Save &amp; Submit</button>';
    ...
    function manipulate_update_button($buttons, $rowData) { 
        ...
        if ($this->input->get('action') == 'view') {
            unset($buttons['update']);
                ...
                if($isOpenEditing){
                    $update_draft = '<button onclick="javascript:update_draft_btn(\'v3_bk_primer\', \' '.base_url().'processor/plc/v3_bk_primer?draft=true \',this,true )"  id="button_update_draft_v3_bk_primer"  class="ui-button-text icon-save" >Update open Editing</button>';
                    ...
                    foreach ($activities as $act) {
                        $update_draft = '<button onclick="javascript:update_draft_btn(\'v3_bk_primer\', \' '.base_url().'processor/plc/v3_bk_primer?draft=true \',this,true )"  id="button_update_draft_v3_bk_primer"  class="ui-button-text icon-save" >Update as Draft</button>';
                        $update = '<button onclick="javascript:update_btn_back(\'v3_bk_primer\', \' '.base_url().'processor/plc/v3_bk_primer?company_id='.$this->input->get('company_id').'&group_id='.$this->input->get('group_id').'modul_id='.$this->input->get('modul_id').' \',this,true )"  id="button_update_submit_v3_bk_primer"  class="ui-button-text icon-save" >Update &amp; Submit</button>';



                        $approve = '<button onclick="javascript:load_popup(\' '.base_url().'processor/plc/v3_bk_primer?action=approve&iM_modul_activity='.$act['iM_modul_activity'].'&'.$this->main_table_pk.'='.$peka.'&company_id='.$this->input->get('company_id').'&group_id='.$this->input->get('group_id').'&modul_id='.$this->input->get('modul_id').' \')"  id="button_approve_v3_bk_primer"  class="ui-button-text icon-save" >Approve</button>';
                        $reject = '<button onclick="javascript:load_popup(\' '.base_url().'processor/plc/v3_bk_primer?action=reject&iM_modul_activity='.$act['iM_modul_activity'].'&'.$this->main_table_pk.'='.$peka.'&company_id='.$this->input->get('company_id').'&group_id='.$this->input->get('group_id').'&modul_id='.$this->input->get('modul_id').' \' )"  id="button_reject_v3_bk_primer"  class="ui-button-text icon-save" >Reject</button>';

                        $confirm = '<button onclick="javascript:load_popup(\' '.base_url().'processor/plc/v3_bk_primer?action=confirm&iM_modul_activity='.$act['iM_modul_activity'].'&'.$this->main_table_pk.'='.$peka.'&company_id='.$this->input->get('company_id').'&group_id='.$this->input->get('group_id').'&modul_id='.$this->input->get('modul_id').' \')"  id="button_approve_v3_bk_primer"  class="ui-button-text icon-save" >Confirm</button>';
