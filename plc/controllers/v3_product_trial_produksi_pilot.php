...
class v3_product_trial_produksi_pilot extends MX_Controller {
    function __construct() {
		...
		$this->url = 'v3_product_trial_produksi_pilot';
		$this->urlpath = 'plc/'.str_replace("_","/", $this->url);

		$this->maintable = 'plc2.plc2_upb_prodpilot';	
		$this->main_table = $this->maintable;	
		$this->main_table_pk = 'iprodpilot_id';	
		$datagrid['islist'] = array(
			...
			,'dtglmulai_prod' => array('label'=>'Tgl Mulai','width'=>100,'align'=>'left','search'=>false)
			// ,'dtglselesai_prod' => array('label'=>'Tgl selesai','width'=>100,'align'=>'left','search'=>false)
			,'iapppd_pp' => array('label'=>'Approval PD','width'=>100,'align'=>'left','search'=>false)
		);		

		$datagrid['setQuery']=array(
								0=>array('vall'=>'plc2_upb_prodpilot.ldeleted','nilai'=>0)
								);
		$datagrid['jointableinner']=array(
									0=>array('plc2.plc2_upb_formula'=>'plc2_upb_formula.ifor_id=plc2_upb_prodpilot.ifor_id')
    ...
    function index($action = '') {
		...
		$grid->changeFieldType('iapppd_pp', 'combobox','',array(''=>'--select--',0=>'Waiting for approval',1=>'Rejected', 2=>'Approved'));
                    ...
                    foreach ($dataFieldUpload as $kf => $vUpload) {
                        ...
                        if(!file_exists($path."/".$lastId)){
                            if (!mkdir($path."/".$lastId, 0777, true)) { //id review
                                die('Failed upload, try again! -----'.$vUpload['vNama_field']);
                            }
                        }
	...
	/*List Box*/
	function listBox_v3_product_trial_produksi_pilot_cPIC_AD($value, $pk, $name, $rowData) {
	...
	function listBox_v3_product_trial_produksi_pilot_plc2_upb_iteampd_id($value, $pk, $name, $rowData) {
	...
	function insertBox_v3_product_trial_produksi_pilot_form_detail($field,$id){
            $get=$this->input->get();
            $post=$this->input->post();
            $in=array();
        ...
        function updateBox_v3_product_trial_produksi_pilot_form_detail($field,$id,$value,$rowData){
            $get=$this->input->get();
            $post=$this->input->post();
            $in=array();
            foreach ($get as $kget => $vget) {
    ...
    function confirm_view() { 
     ...
     function confirm_process() {
        ...
        $sqlgetforId = 'select a.* ,b.iupb_id
                        from plc2.plc2_upb_prodpilot a 
                        join plc2.plc2_upb_formula b on b.ifor_id=a.ifor_id
                        where a.iprodpilot_id  = "'.$pk.'"';
        $datapp = $this->db->query($sqlgetforId)->row_array();

        
        if(!empty($datapp)){

            $ifor_id = $datapp['ifor_id'];
            $siupb_id = $datapp['iupb_id'];

            //Next To stabilita Pilot Detail
            $cek_form = "SELECT * FROM pddetail.`formula_process` fp WHERE fp.`lDeleted` = 0 AND fp.`iMaster_flow` IN (9,10,11) AND fp.`iupb_id` IN (".$siupb_id.")";
