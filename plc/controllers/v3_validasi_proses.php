...
class v3_validasi_proses extends MX_Controller {
    function __construct() {
		...
		$this->title = 'Laporan Validasi Proses';
		$this->url = 'v3_validasi_proses';
		$this->urlpath = 'plc/'.str_replace("_","/", $this->url);

		$this->maintable = 'plc2.validasi_proses';	
		$this->main_table = $this->maintable;	
		$this->main_table_pk = 'ivalidasi_id';	
		$datagrid['islist'] = array(
			...
			,'dmulai_validasi' => array('label'=>'Tgl Mulai','width'=>100,'align'=>'left','search'=>false)
			,'dselesai_validasi' => array('label'=>'Tgl selesai','width'=>100,'align'=>'left','search'=>false)
	...
	/*function listBox_v3_validasi_proses_cPIC_AD($value, $pk, $name, $rowData) {
	...
	function listBox_v3_validasi_proses_plc2_upb_iteampd_id($value, $pk, $name, $rowData) {
	...
	function insertBox_v3_validasi_proses_form_detail($field,$id){
            $get=$this->input->get();
            $post=$this->input->post();
        ...
        function updateBox_v3_validasi_proses_form_detail($field,$id,$value,$rowData){
            $get=$this->input->get();
            $post=$this->input->post();
            foreach ($get as $kget => $vget) {
                if($kget!="action"){
    ...
    function after_insert_processor($fields, $id, $postData) {
        ...
        $ivalpro_batch_id 	= $postData['ivalpro_batch_id'];
        $vno_batch 			= $postData['vno_batch'];
        $nip 				= $this->user->gNIP;
        ...
        foreach($ivalpro_batch_id as $k => $v) {
            if(empty($v)) {
                if(!empty($vno_batch[$k])) {
                    $batch['ivalidasi_id'] 	= $id;
                    $batch['vno_batch'] 	= $vno_batch[$k];
                    $batch['dcreate'] 		= $now;
                    $batch['ccreate']	 	= $nip;
                    $batch['ldeleted']	 	= 0;
                    $this->db->insert('plc2.validasi_proses_batch', $batch);
                }                       
            }
        } 
    ...
    function after_update_processor($fields, $id, $postData) {
        ...
        $eRows = $this->db->get_where('plc2.validasi_proses_batch', array('ivalidasi_id' => $id, 'ldeleted' => 0))->result_array();
        foreach($eRows as $k => $v) {
            if(in_array($v['ivalpro_batch_id'], $ivalpro_batch_id)) {
                $this->db->where('ivalpro_batch_id', $v['ivalpro_batch_id']);
                $key = array_search($v['ivalpro_batch_id'], $ivalpro_batch_id);
                $this->db->update('plc2.validasi_proses_batch', array('vno_batch' => $vno_batch[$key], 'dupdate' => $now, 'cupdate' => $nip));
            }
            else {
                $this->db->where('ivalpro_batch_id', $v['ivalpro_batch_id']);
                $this->db->update('plc2.validasi_proses_batch', array('ldeleted' => 1, 'dupdate' => $now, 'cupdate' => $nip));
            }
        }
