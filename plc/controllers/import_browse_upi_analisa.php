...
class import_browse_upi_analisa extends MX_Controller {
...
$this->db_plc0 = $this->load->database('plc0',false, true);
    ...
    function index($action = '') {
		...
		$grid->setUrl('import_browse_upi_analisa');
		...
		//upi tidak sedang aktif di analisa principal atau sudah approve
		$grid->setQuery('daftar_upi.iupi_id not in (select a.iupi_id from plc2.analisa_prinsipal a where a.iApprove_bdirm in(2,0)) ', null);
	...
	function listBox_import_browse_upi_analisa_pilih($value, $pk, $name, $rowData) {
		$url_header = base_url()."processor/plc/import/analisa/principal/?action=getdetil"; 
