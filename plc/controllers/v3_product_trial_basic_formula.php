...
class v3_product_trial_basic_formula extends MX_Controller {
    function __construct() {
		...
		$this->title = 'Basic Formula & Spesifikasi FG';
		$this->url = 'v3_product_trial_basic_formula';
		$this->urlpath = 'plc/'.str_replace("_","/", $this->url);

		$this->maintable = 'plc2.plc2_upb_formula';	
		$this->main_table = $this->maintable;	
		$this->main_table_pk = 'ifor_id';	
		$datagrid['islist'] = array(
			'vNo_formulaf' => array('label'=>'No Formulasi','width'=>150,'align'=>'left','search'=>true)
			...
			,'isubmit_basic' => array('label'=>'Status Submit','width'=>150,'align'=>'left','search'=>false)
			,'iapppd_basic' => array('label'=>'Approval PD','width'=>150,'align'=>'left','search'=>false)
		);		
		...
		$datagrid['jointableinner']=array(
									0=>array('plc2.plc2_upb'=>'plc2_upb.iupb_id=plc2_upb_formula.iupb_id')
									,1=>array('pddetail.formula_process'=>'formula_process.iFormula_process=plc2_upb_formula.iFormula_process')
                                    //,2=>array('pddetail.formula'=>'formula.iFormula_process=formula_process.iFormula_process')
                                    ,2=>array('(SELECT a.*
    ...
    function index($action = '') {
		...
		$grid->changeFieldType('isubmit_basic', 'combobox','',array(''=>'--select--',0=>'Draft - Need to be Submit', 1=>'Submited'));
		$grid->changeFieldType('iapppd_basic', 'combobox','',array(''=>'--select--',0=>'Waiting for approval',1=>'Rejected', 2=>'Approved'));
            ...
            unset($buttons['update']);
                ...
                if($isOpenEditing){
                    ...
                    foreach ($activities as $act) {
                        ...
                        $update2 = '<button onclick="javascript:update_btn_back(\''.$this->url.'\', \' '.base_url().'processor/'.$this->urlpath.'?company_id='.$this->input->get('company_id').'&iM_modul_activity='.$act['iM_modul_activity'].'&group_id='.$this->input->get('group_id').'&modul_id='.$this->input->get('modul_id').' \',this,true )"  id="button_update_submit_'.$this->url.'"  class="ui-button-text icon-save" >Update</button>';
    ...
    function listBox_v3_product_trial_basic_formula_vNo_formulaf($value, $pk, $name, $rowData) {
		$sql="select a.vNo_formula 
                from pddetail.formula a 
                where a.lDeleted=0
                and a.iFormula_process='".$rowData->iFormula_process."'
                ";
	...
	function listBox_v3_product_trial_basic_formula_cPIC_AD($value, $pk, $name, $rowData) {
	...
	function listBox_v3_product_trial_basic_formula_plc2_upb_iteampd_id($value, $pk, $name, $rowData) {
	...
	function insertBox_v3_product_trial_basic_formula_form_detail($field,$id){
            $get=$this->input->get();
            $post=$this->input->post();
        ...
        function updateBox_v3_product_trial_basic_formula_form_detail($field,$id,$value,$rowData){
            $get=$this->input->get();
            $post=$this->input->post();
            foreach ($get as $kget => $vget) {
                if($kget!="action"){
    ...
    function before_update_processor($row, $postData) {
        ...
        $postData['itype']=1;
        $this->db->where('ifor_id',$postData['v3_product_trial_basic_formula_ifor_id']);
        $ddd=$this->db->get('plc2.plc2_upb_formula')->row_array();
        $postData['iniSubmit']=isset($ddd['isubmit_basic'])?$ddd['isubmit_basic']:'0';
		$postData['iwithbasic']=isset($postData['product_trial_basic_formula_iwithbasic_pilot'])?$postData['product_trial_basic_formula_iwithbasic_pilot']:0;
        return $postData;
    }
    ...
    function confirm_view() { 
                ...
                $echo .= 'Remark : 
                        ...
                        <input type="hidden" name="iapprove" value="2" />
