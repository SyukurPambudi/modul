...
class v3_uji_mikro_bb extends MX_Controller {
    function __construct() {
        ...
        $this->main_table='plc2.uji_mikro_bb';
        $this->main_table_pk='iuji_mikro_bb';


        $this->title = 'Uji Mikro BB';
        $this->url = 'v3_uji_mikro_bb';
        $this->urlpath = 'plc/'.str_replace("_","/", $this->url);

        $this->maintable = 'plc2.uji_mikro_bb';  
        $datagrid['islist'] = array(
                                        'plc2_upb_request_sample.vreq_nomor' => array('label'=>'No Request','width'=>100,'align'=>'center','search'=>true)
                                        ,'plc2_raw_material.vnama' => array('label'=>'Nama Bahan Baku','width'=>200,'align'=>'left','search'=>true)
                                        ,'plc2_upb.vupb_nomor' => array('label'=>'No UPB','width'=>100,'align'=>'center','search'=>true)
                                        ,'plc2_upb.vupb_nama' => array('label'=>'Nama UPB','width'=>300,'align'=>'left','search'=>true)
                                        ,'iJumlah_terima' => array('label'=>'Jumlah Terima','width'=>100,'align'=>'center','search'=>true)
                                        ,'iSubmit' => array('label'=>'Status Submit','width'=>100,'align'=>'center','search'=>false)
                                        ,'iApprove_uji' => array('label'=>'Status Approval','width'=>100,'align'=>'center','search'=>false)
                                    );
        $datagrid['shortBy']=array('iuji_mikro_bb'=>'Desc');

        

        $datagrid['setQuery']=array(
                                0=>array('vall'=>'plc2_upb.ldeleted','nilai'=>0)
                               , 1=>array('vall'=>'uji_mikro_bb.lDeleted','nilai'=>0)
                                
                                );

        $datagrid['jointableinner']=array(
                                    #0=>array('plc2.plc2_upb_formula'=>'plc2_upb_formula.ifor_id=mikro_fg.ifor_id')
                                    0=>array('plc2.plc2_upb_request_sample_detail'=>'plc2.plc2_upb_request_sample_detail.ireqdet_id = plc2.uji_mikro_bb.ireqdet_id'),
        ...
        switch ($action) {
            ...
            case 'create':
             ...
             case 'createproses':
                    ...
                    if($isUpload) {
                        $lastId = $_GET["lastId"];
                        $path = realpath("files/plc/local/uji_mikro_bb/rpt_mikro_bb/");
                        $path1 = realpath("files/plc/local/uji_mikro_bb/rpt_study_literatur/");
                            ...
                            foreach($_POST as $key=>$value) {                       
                                if ($key == 'file_vKeterangan_file_filename') {
                                    foreach($value as $k=>$v) {
                                        $file_keterangan[$k] = $v;
                                    }
                                }
                                if ($key == 'file_vKeterangan_file_literatur') {
                                    foreach($value as $k=>$v) {
                            ...
                            $i = 0;
                            //upload form 1
                            foreach ($_FILES['fileupload_file_filename']["error"] as $key => $error) {
                                if ($error == UPLOAD_ERR_OK) {              
                                    $tmp_name = $_FILES['fileupload_file_filename']["tmp_name"][$key];
                                    $name = $_FILES['fileupload_file_filename']["name"][$key];
                                    $data['filename'] = $name;
                                    ...
                                    $name_generate = $i.'__'.$now_u.'__'.$_FILES['fileupload_file_filename']['name'][$key];

                                    if(move_uploaded_file($tmp_name, $path."/".$this->input->get('lastId')."/".$name_generate)) {    
                                        $sql[] = "INSERT INTO uji_mikro_bb_file(iuji_mikro_bb, filename,vFilename_generate, dInsertDate, keterangan,cInsert) 
                                                VALUES ('".$data['id']."', '".$data['filename']."','".$name_generate."','".$data['dInsertDate']."','".$file_keterangan[$i]."','".$data['nip']."')";
                                        $i++;                                                                           
                                    }
                                    else{
                            ...
                            //upload form 2
                            if (isset($_FILES['fileupload_file_literatur'])){
                                $i = 0;
                                foreach ($_FILES['fileupload_file_literatur']["error"] as $key => $error) {
                                    if ($error == UPLOAD_ERR_OK) {              
                                        $tmp_name1 = $_FILES['fileupload_file_literatur']["tmp_name"][$key];
                                        $name1 = $_FILES['fileupload_file_literatur']["name"][$key];
                                        ...
                                        if(move_uploaded_file($tmp_name1, $path1."/".$this->input->get('lastId')."/".$name_generate)) { 
                                            $sql1[] = "INSERT INTO uji_mikro_bb_file_literatur(iuji_mikro_bb, filename,vFilename_generate, dInsertDate, keterangan,cInsert) 
                                                    VALUES ('".$data1['id']."', '".$data1['filename']."','".$name_generate."','".$data1['dInsertDate']."','".$file_keterangan1[$i]."','".$data1['nip']."')";
                                            $i++;                                                                           
                                        }
                                        else{
                                        echo "Upload ke folder gagal";  
                ...
                if(count($dataFieldUpload)>0 && $lastId!="0" && $lastId!=""){
                    foreach ($dataFieldUpload as $kf => $vUpload) {
                        ...
                        if(count($validdetails)>0){
                            $this->db_plc0->where($vUpload['fieldheader'],$lastId)
                        ...
                        $qq=$this->db_plc0->get($vUpload['filetable']);
                        if($qq->num_rows() > 0){
                            ...
                            if ($error == UPLOAD_ERR_OK) {
                                ...
                                $name_generate = $this->lib_plc->generateFilename($name, $i);
                                    if(move_uploaded_file($tmp_name, $path."/".$lastId."/".$name_generate)) {
                                        ...
                                        $datainsert[$vUpload['fldeleted']]      = 0;
    ...
    function listBox_Action($row, $actions) {
        ...
        return $actions;
            ...
            function approve_view() {
                ...
                $echo .= '<form id="form_v3_uji_mikro_bb_approve" action="'.base_url().'processor/plc/v3_uji_mikro_bb?action=approve_process" method="post">';
                ...
                <button type="button" onclick="submit_ajax(\'form_v3_uji_mikro_bb_approve\')">Approve</button>';
            ...
            function approve_process() {
                ...
                $this->lib_plc->InsertActivityModule($arrUPB,$modul_id,$iuji_mikro_bb,$iM_activity,$iSort,$vRemark,2);
            ...
            function confirm_view() {
                ...
                $echo .= '<form id="form_v3_uji_mikro_bb_confirm" action="'.base_url().'processor/plc/v3_uji_mikro_bb?action=confirm_process" method="post">';
                $echo .= '<div style="vertical-align: top;">';
                $echo .= 'Remark : 
                        <input type="hidden" name="iuji_mikro_bb" value="'.$this->input->get('iuji_mikro_bb').'" />
                        <input type="hidden" name="modul_id" value="'.$this->input->get('modul_id').'" />
                        ...
                        <textarea name="vRemark"></textarea>
                <button type="button" onclick="submit_ajax(\'form_v3_uji_mikro_bb_confirm\')">Confirm</button>';
            ...
            function reject_view() {
                $echo = '<script type="text/javascript">
                             function submit_ajax(form_id) {
                                var remark = $("#v3_uji_mikro_bb_remark").val();
                                if (remark=="") {
                ...
                $echo .= '<form id="form_v3_uji_mikro_bb_reject" action="'.base_url().'processor/plc/v3_uji_mikro_bb?action=reject_process" method="post">';
                $echo .= '<div style="vertical-align: top;">';
                $echo .= 'Remark : 
                        <input type="hidden" name="iuji_mikro_bb" value="'.$this->input->get('iuji_mikro_bb').'" />
                        <input type="hidden" name="modul_id" value="'.$this->input->get('modul_id').'" />
                        ...
                        <textarea name="vRemark" id="reject_v3_uji_mikro_bb_remark"></textarea>
                <button type="button" onclick="submit_ajax(\'form_v3_uji_mikro_bb_reject\')">Reject</button>';
            ...
            function reject_process() {
                ...
                $arrUPB['iupb_id'] = $iupb_id;
                $this->lib_plc->InsertActivityModule($arrUPB,$modul_id,$iuji_mikro_bb,$iM_activity,$iSort,$vRemark,1);
    ...
    function after_insert_processor($fields, $id, $postData) {
        $ireqdet_id = $postData['ireqdet_id'];
        $sql = 'select d.iupb_id
                from plc2.plc2_upb_request_sample_detail a 
                join plc2.plc2_upb_request_sample b on b.ireq_id=a.ireq_id
                join plc2.plc2_raw_material c on c.raw_id=a.raw_id
                join plc2.plc2_upb d on d.iupb_id=b.iupb_id
    ...
    function before_update_processor($row, $postData) {
        ...
        $controller_name ='v3_uji_mikro_bb';
        $pk_field = 'iuji_mikro_bb';
    ...
    function manipulate_update_button($buttons, $rowData) { 
        $peka=$rowData['iuji_mikro_bb'];
        

        $ireqdet_id = $rowData['ireqdet_id'];
        $sql = 'select d.iupb_id
                from plc2.plc2_upb_request_sample_detail a 
                join plc2.plc2_upb_request_sample b on b.ireq_id=a.ireq_id
                join plc2.plc2_raw_material c on c.raw_id=a.raw_id
        ...
        $iframe = '<iframe name="v3_uji_mikro_bb_frame" id="v3_uji_mikro_bb_frame" height="0" width="0"></iframe>';
        ...
        if ($this->input->get('action') == 'view') {
            unset($buttons['update']);
                ...
                if($isOpenEditing){
                    $update_draft = '<button onclick="javascript:update_draft_btn(\'v3_uji_mikro_bb\', \' '.base_url().'processor/plc/v3_uji_mikro_bb?draft=true \',this,true )"  id="button_update_draft_v3_uji_mikro_bb"  class="ui-button-text icon-save" >Update open Editing</button>';
                    ...
                    foreach ($activities as $act) {
                        $update_draft = '<button onclick="javascript:update_draft_btn(\'v3_uji_mikro_bb\', \' '.base_url().'processor/plc/v3_uji_mikro_bb?draft=true \',this,true )"  id="button_update_draft_v3_uji_mikro_bb"  class="ui-button-text icon-save" >Update as Draft</button>';
                        $update = '<button onclick="javascript:update_btn_back(\'v3_uji_mikro_bb\', \' '.base_url().'processor/plc/v3_uji_mikro_bb?company_id='.$this->input->get('company_id').'&iM_modul_activity='.$act['iM_modul_activity'].'group_id='.$this->input->get('group_id').'modul_id='.$this->input->get('modul_id').' \',this,true )"  id="button_update_submit_v3_uji_mikro_bb"  class="ui-button-text icon-save" >Update &amp; Submit</button>';

                        $approve = '<button onclick="javascript:load_popup(\' '.base_url().'processor/plc/v3_uji_mikro_bb?action=approve&iM_modul_activity='.$act['iM_modul_activity'].'&iM_activity='.$act['iM_activity'].'&iuji_mikro_bb='.$peka.'&iupb_id='.$iupb_id.'&company_id='.$this->input->get('company_id').'&group_id='.$this->input->get('group_id').'&modul_id='.$this->input->get('modul_id').' \')"  id="button_approve_v3_uji_mikro_bb"  class="ui-button-text icon-save" >Approve</button>';
                        $reject = '<button onclick="javascript:load_popup(\' '.base_url().'processor/plc/v3_uji_mikro_bb?action=reject&iM_modul_activity='.$act['iM_modul_activity'].'&iM_activity='.$act['iM_activity'].'&iuji_mikro_bb='.$peka.'&iupb_id='.$iupb_id.'&company_id='.$this->input->get('company_id').'&group_id='.$this->input->get('group_id').'&modul_id='.$this->input->get('modul_id').' \' )"  id="button_reject_v3_uji_mikro_bb"  class="ui-button-text icon-save" >Reject</button>';

                        $confirm = '<button onclick="javascript:load_popup(\' '.base_url().'processor/plc/v3_uji_mikro_bb?action=confirm&iM_modul_activity='.$act['iM_modul_activity'].'&iM_activity='.$act['iM_activity'].'&iuji_mikro_bb='.$peka.'&iupb_id='.$iupb_id.'&company_id='.$this->input->get('company_id').'&group_id='.$this->input->get('group_id').'&modul_id='.$this->input->get('modul_id').' \')"  id="button_approve_v3_uji_mikro_bb"  class="ui-button-text icon-save" >Confirm</button>';
                        ...
                        if( !empty($cekDept) || in_array($this->user->gNIP, $arrNipAssign)  ){
                            $upbTeamArr=isset($upbTeamID[$act['vDept_assigned']])?$upbTeamID[$act['vDept_assigned']]:'';
                            // jika Dept id yang ditunjuk ada pada team id yang dimiliki
                            if(in_array( $upbTeamArr, $arrTeamID) || in_array($this->user->gNIP, $arrNipAssign) ){
    ...
    function insertBox_v3_uji_mikro_bb_form_detail($field,$id){
            $get=$this->input->get();
            $post=$this->input->post();
    ...
    function updateBox_v3_uji_mikro_bb_form_detail($field,$id,$value,$rowData){
        $get=$this->input->get();
        $post=$this->input->post();
        foreach ($get as $kget => $vget) {
            if($kget!="action"){
    ...
    function get_data_prev(){
            ...
            $row        = $this->db->get_where('plc2.sys_masterdok', array( 'filename' => $namefield ))->row_array();
            $sqlFile    = 'SELECT * FROM '.$row['filetable']. ' WHERE '.$row['fieldheader'].' = '.$post['id'].' AND '.$row['fldeleted'].' IN (0, NULL)';
            $dataFile   = $this->db->query($sqlFile);
            ...
            $data       = new StdClass;
            $data->records = $dataFile->num_rows();
            $i          = 0;

            foreach ($dataFile->result() as $k) {
                $data->rows[$i]['id'] = $i;
                $z      = 0;
                ...
                if($value != '') {
                    if (file_exists('./'.$row["filepath"].'/'.$id.'/'.$value)) {
                        ...
                        $linknya = '<a class="ui-button-text" style="color: blue;" href="javascript:;" onclick="window.location=\''.$link.'\'">[ Download ]</a>&nbsp;&nbsp;&nbsp;';
                    }
                }
                $linknya .= '<a class="ui-button-text" style="color: red;" href="javascript:;" onclick="javascript:hapus_row_'.$nmTable.'('.$i.')">[ Hapus ]</a><input type="hidden" class="num_rows_'.$nmTable.'" value="'.$i.'" />';
                $linknya .= '<input type="hidden" name="'.$row["fielddetail"].'[]" value="'.$k->{$row["fielddetail"]}.'" />';
