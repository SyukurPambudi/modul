...
$this->db_plc0 = $this->load->database('plc0',false, true);
    ...
    function index($action = '') {
		...
		$grid->addList('plc2_upb.vupb_nomor','plc2_upb.vupb_nama','plc2_upb.vgenerik','plc2_upb.iteampd_id','vkode_surat','itype','vrevisi', 'iapppd', 'iappqa', 'iapppd_f', 'iappqa_f');
		...
		$grid->addFields('iupb_id','vupb_nama','vgenerik','iteampd_id','vkode_surat','tberlaku','filename','vrevisi','itype', 'iuji_mikro','vnip_formulator','vnip_pd','vnip_qa','vnip_pd_f','vnip_qa_f','spesifikasi');
...
*/
    ...
    function listBox_Action($row, $actions) {
		...
		$iupbid=$row->iupb_id;
    	$sqle="select count(*) as jum from plc2.plc2_upb_spesifikasi_fg f where f.iupb_id=$iupbid and f.itype=1 and f.iappqa_f<>0";
    	...
    	if($this->auth->is_manager()){
    		$x=$this->auth->dept();
    		$manager=$x['manager'];
			//Man PD tetap bisa edit data walaupun sudah di approve
    		// Jika PDM sudah app, PDM tidak bisa edit
    		// if(in_array('PD', $manager)){
    			// if(($row->iapppd<>0)&&($row->iapppd<>0)){
    				// unset($actions['edit']);
    				// unset($actions['delete']);
    			// }
    		// }
    		// jika PDM blm app QA tidak edit
    		...
    		// jika PDM sudah approve/ reject, staff tidak bisa edit
    		if($row->iapppd<>0){
    			unset($actions['edit']);
    			unset($actions['delete']);
    		}
    ...
    function reject_view() {
    	...
    	$echo .= 'Remark : <input type="hidden" name="iupb_id" value="'.$this->input->get('iupb_id').'" />
    			...
    			<input type="hidden" name="group_id" value="'.$this->input->get('group_id').'" />
				<input type="hidden" name="modul_id" value="'.$this->input->get('modul_id').'" />
				<textarea name="remark"></textarea><button type="button" onclick="submit_ajax(\'form_spesifikasi_fg_tentative_reject\')">Reject</button>';
    	$echo .= '</div>';
    	$echo .= '</form>';
    	return $echo;
    }
	...
	function approve_view_final() {
    	...
    	$echo .= '<h1>Approval</h1><br />';
    	$echo .= '<form id="form_spesifikasi_fg_tentative_approve" action="'.base_url().'processor/plc/spesifikasi/fg/tentative?action=approve_process_final" method="post">';
    	$echo .= '<div style="vertical-align: top;">';
    	$echo .= 'Remark : <input type="hidden" name="ispekfg_id" value="'.$this->input->get('ispekfg_id').'" />
    			<input type="hidden" name="iupb_id" value="'.$this->input->get('iupb_id').'" />
				<input type="hidden" name="type" value="'.$this->input->get('type').'" />
    ...
    function approve_process_final() {
    	...
    	$this->db_plc0->where('iupb_id', $post['iupb_id']);
    	$iapprove = $post['type'] == 'PD' ? 'ispekpd' : 'ispekqa';
    	$this->db_plc0->update('plc2.plc2_upb', array($iapprove=>2)); // update data ke tabel upb
    	
		$nip = $this->user->gNIP; // update data di tabel spesifikasi_fg
		$skg=date('Y-m-d H:i:s');
    	$iapprove = $post['type'] == 'PD' ? 'iapppd_f' : 'iappqa_f';
		$iappnip = $post['type'] == 'PD' ? 'vnip_pd_f' : 'vnip_qa_f';
		$iappdate = $post['type'] == 'PD' ? 'tapp_pd_f' : 'tapp_qa_f';
		$istatus=$post['type'] == 'PD' ? '2' : '3';
		$this->db_plc0->where('ispekfg_id', $post['ispekfg_id']);
    	$this->db_plc0->update('plc2.plc2_upb_spesifikasi_fg', array('itype'=>1,$iapprove=>2, 'istatus'=>$istatus, $iappnip=>$nip, $iappdate=>$skg));
	...
	function after_insert_processor($row, $insertId, $postData) {
		...
		$bizsup=$getbp['idplc2_biz_process_sub'];
		$qcek="select f.ispekfg_id, f.itype, f.iapppd, f.iappqa from plc2.plc2_upb_spesifikasi_fg f where f.iupb_id=$iupb_id and f.ispekfg_id=$insertId";
		$rcek = $this->db_plc0->query($qcek)->row_array();
		if($rcek['itype']==1){ // kalo final hanya update last log
			//insert log
	...
	function after_update_processor($row, $updateId, $postData) {
		...
		$bizsup=$getbp['idplc2_biz_process_sub'];
		//echo $bizsup; 
		$qcek="select f.ispekfg_id, f.itype, f.iapppd, f.iappqa from plc2.plc2_upb_spesifikasi_fg f where f.ispekfg_id=$updateId";
		$rcek = $this->db_plc0->query($qcek)->row_array();
		if($rcek['itype']==1){ // kalo final hanya update last log
			//insert log
				$this->biz_process->insert_log($postData['iupb_id'], $bizsup, 7); // status 7 => submit
	...
	//upb yg bisa di pilh untuk create new hanya upb yg belum pernah di buat FG Final
	...
	function updateBox_spesifikasi_fg_tentative_vnip_qa($field, $id, $value, $rowData) {
		if($rowData['vnip_qa'] != null){
			...
			if($rowData['iappqa']==2){$st="Approve";}elseif($rowData['iappqa']==1){$st="Reject";
				$rowa = $this->db_plc0->get_where('plc2.plc2_upb_approve', array('vmodule'=>$this->input->get('modul_id'), 'iupb_id'=>$rowData['iupb_id']))->row_array();
				// $reason=$rowa['treason'];
			} 
