...
class report_prioritas_registrasi extends MX_Controller {
	private $sess_auth;
	private $dbset;
	private $urutan = 0;
	private $report;
...
$this->db_plc0 = $this->load->database('plc0',false, true);
        ...
        $this->url = 'report_prioritas_registrasi'; 
    ...
    function index($action = '') {		
		...
		$grid->setUrl('report_prioritas_registrasi');		
	...
	public function insertBox_report_prioritas_registrasi_imonth($field, $id) {
	...
	public function insertBox_report_prioritas_registrasi_iyear($field, $id){
		$thn_sekarang = date('Y');
	 ...
	 public function insertBox_report_prioritas_registrasi_iteambusdev_id($field, $id){
	...
	public function insertBox_report_prioritas_registrasi_iteampd_id($field, $id){
	...
	public function insertBox_report_prioritas_registrasi_format($field, $id) {
		$o = '
		...
		<script type="text/javascript">
			...
			var iteambusdev_id = $("#'.$this->url.'_iteambusdev_id option:selected").val(); 
				...
				if (iteampd_id == "") {
					...
					document.getElementById("iframe_pdf_plc_'.$this->url.'").src = "'.base_url().'processor/plc/report/prioritas/registrasi?action=print_pdf&imonth="+imonth+"&iyear="+iyear+"&iteambusdev_id="+iteambusdev_id+"&iteampd_id="+iteampd_id;
				}
			}
			...
			var iteambusdev_id = $("#'.$this->url.'_iteambusdev_id option:selected").val(); 
				...
				if (iteampd_id == "") {
					...
					document.getElementById("iframe_pdf_plc_'.$this->url.'").src = "'.base_url().'processor/plc/report/prioritas/registrasi?action=print_xls&imonth="+imonth+"&iyear="+iyear+"&iteambusdev_id="+iteambusdev_id+"&iteampd_id="+iteampd_id;
				}
			}
	...
	public function preview_report($cssstyle=false) {                
		...
		if($query->num_rows()>0) {
			...
			$html.='<tr style="background:#87CEFA"><td style="width:100%;">No.</td><td width="50px">No. UPB </td><td width="300px">Generik</td><td width="200px">Usulan Nama Produk</td><td width="30px">Kategori</td><td width="30px">Status</td></tr>';
			$no=0;
			foreach($rows as $row) {
				$no++;
	...
	function preview_report1() {
		...
		if($type=='PD' || $type=='QA'|| $type=='QC'|| $type=='DR'){
			...
			$reportAsal   = "report_registrasi_all.jrxml";
		...
		elseif($type=='BD'){
			...
			$params->put('iteampd_id', intval($this->input->get('iteampd_id')));
			$reportAsal   = "report_registrasi.jrxml";
