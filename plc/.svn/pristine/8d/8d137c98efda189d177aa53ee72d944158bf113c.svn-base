...
function add_row_lapori(table_id){		
		...
		if (n.length == 0) {
			...
			row_content  +=	'<select name="confirm12[]"><option value="0">Haven\'t been checked</option><option value="1">Yes</option><option value="3">No</option><option value="2">Unnecessary</option></select>';
			<?php } 
				else{?>
			row_content  +=	'<select name="confirm12[]"><option value="0">Haven\'t been checked</option></select>';
			<?		}
				}?>row_content  +=	'</td>';
...
<table class="hover_table" id="lapori" cellspacing="0" cellpadding="1" style="width: 98%; border: 1px solid #dddddd; text-align: center; margin-left: 5px; border-collapse: collapse">
	...
	<tr style="width: 98%; border: 1px solid #dddddd; background: #548cb6; border-collapse: collapse">
		<th colspan="7" style="border: 1px solid #dddddd;"><span style="font-weight: bold; color: #ffffff; text-shadow: 0 1px 1px rgba(0, 0, 0, 0.3); text-transform: uppercase;">Upload File lapori</span></th>
	</tr>
		...
		?>
				<tr style="border: 1px solid #dddddd; border-collapse: collapse; background: #ffffff; ">
					...
					<td style="border: 1px solid #dddddd; width: 4%; text-align: center;">
						...
						?>
						<input type="hidden" name="confirm12[]" value="<?php echo $row['iconfirm_busdev'];?>">
						<span>
	...
	<tfoot>
		...
		<?php } 
			...
			<td style="text-align: center">
				<?php echo '&nbsp;&nbsp;&nbsp; <span onclick="javascript:load_popup(\''.base_url().'processor/plc/cek/dokumen?action=dokDone&dok=lapori&id='.$idsp.'&upb_id='.$iupb_id.'&group_id='.$this->input->get('group_id').'&modul_id='.$this->input->get('modul_id').'\')" class="ui-button-text icon-save ui-button ui-widget ui-state-default ui-corner-all ui-button-text-icon-primary" id="button_doneOri">Done</span>';?>
			</td>
			<?php }?>
		</tr>
		<?php if(($iDone==1)) {?>
