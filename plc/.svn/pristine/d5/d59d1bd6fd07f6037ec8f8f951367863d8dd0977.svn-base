...
class v3_soi_fg_tentative extends MX_Controller {
    function __construct() {
		...
		$this->title = 'SOI FG Tentative';
		$this->url = 'v3_soi_fg_tentative';
		$this->urlpath = 'plc/'.str_replace("_","/", $this->url);

		$this->maintable = 'plc2.plc2_upb_soi_fg';	
		$this->main_table = $this->maintable;	
		$this->main_table_pk = 'isoi_id';	
		$datagrid['islist'] = array(
            ...
            ,'irevisi' => array('label'=>'Revisi','width'=>100,'align'=>'center','search'=>true)
			...
			/* ,'iappqa' => array('label'=>'Approval QA','width'=>100,'align'=>'left','search'=>false) */
		);		

		$datagrid['setQuery']=array(
								0=>array('vall'=>'plc2_upb_soi_fg.ldeleted','nilai'=>0)
								);
		$datagrid['jointableinner']=array(
									0=>array('plc2.plc2_upb'=>'plc2_upb.iupb_id=plc2_upb_soi_fg.iupb_id')
								);
	...
	/*List Box*/
	function listBox_v3_soi_fg_tentative_iappqa_uji($value) {
	...
	function listBox_v3_soi_fg_tentative_ini_nilai($value, $pk, $name, $rowData) {
	...
	function insertBox_v3_soi_fg_tentative_form_detail($field,$id){
            $get=$this->input->get();
            $post=$this->input->post();
        ...
        function updateBox_v3_soi_fg_tentative_form_detail($field,$id,$value,$rowData){
            $get=$this->input->get();
            $post=$this->input->post();
            foreach ($get as $kget => $vget) {
                if($kget!="action"){
    ...
    function before_insert_processor($row, $postData) {
        //ambil revisi terakhir dari upb
        $sqlCekRevisi = 'SELECT d.irevisi FROM plc2.plc2_upb_soi_fg d WHERE d.iupb_id = ? AND d.lDeleted = 0 ORDER BY d.isoi_id DESC ';
    ...
    function approve_view($id=0) { 
     ...
     function approve_process() {
        ...
        //fungsi ini untuk mengmbalikan upb langsung ke soi fg karena revisi nya > 0
        $getLastactivity = $this->lib_plc->getLastactivity($modul_id,$pk);
        if ($getLastactivity != 0){
            $data = $this->db->get_where($this->maintable, array($this->main_table_pk => $pk, 'ldeleted' => 0))->row_array();
            $revisi = (count($data) > 0)?intval($data['irevisi']):0;
            if ($revisi > 0 && $iapprove == 2){
                $soi['iupb_id'] = $data['iupb_id'];
                $soi['irevisi'] = $revisi;
                $soi['ineed_revisi'] = 0;
                $soi['itype'] = 1;
                $soi['vkode_surat'] = $data['vkode_surat'].' -- After Revisi';
                $soi['vnip_formulator'] = $data['vnip_formulator'];
                $soi['dmulai_draft'] = $data['dmulai_draft'];
                $soi['dselesai_draft'] = $data['dselesai_draft'];
                $this->db->insert('plc2.soi_fg', $soi);
            }
        }
        //close fungsi ini untuk mengmbalikan upb langsung ke soi fg karena revisi nya > 0
