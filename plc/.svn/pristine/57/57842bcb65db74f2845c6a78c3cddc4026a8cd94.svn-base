<?php if ( ! defined('BASEPATH')) exit('No direct script access allowed');
    class uji_mikro_bb extends MX_Controller {
...
$this->db_plc0 = $this->load->database('plc0',false, true);
    ...
    function index($action = '') {
        ...
        $grid = new Grid;		
        $grid->setUrl('uji_mikro_bb');
        $grid->setTitle('Uji Mikro BB');		
        $grid->setTable('plc2.uji_mikro_bb');		
        $grid->setUrl('uji_mikro_bb');
        $grid->addList('plc2_upb_request_sample.vreq_nomor','plc2_raw_material.vnama','iJumlah_terima','iSubmit','iApprove_uji');
        $grid->addFields('iApprove_uji','ireqdet_id','no_po','no_upb','iJumlah_terima','dMulai_literatur','dSelesai_literatur','dMulai_uji','dSelesai_uji','cPic_uji','iHasil_uji','iUji_spesifik','file_literatur','file_filename');
        ...
        switch ($action) {
								...
								foreach ($_FILES['fileupload1']["error"] as $key => $error) {
									if ($error == UPLOAD_ERR_OK) {				
										...
										if(move_uploaded_file($tmp_name1, $path1."/".$this->input->get('lastId')."/".$name1)) {	
											$sql1[] = "INSERT INTO uji_mikro_bb_file_literatur(iuji_mikro_bb, filename, dInsertDate, keterangan,cInsert) 
                ...
                case 'updateproses':
						...
						if (!file_exists( $path."/".$this->input->post('uji_mikro_bb_iuji_mikro_bb') )) {
							mkdir($path."/".$this->input->post('uji_mikro_bb_iuji_mikro_bb'), 0777, true);						 
						}
					
						if (!file_exists( $path1."/".$this->input->post('uji_mikro_bb_iuji_mikro_bb') )) { 
							mkdir($path1."/".$this->input->post('uji_mikro_bb_iuji_mikro_bb'), 0777, true);					    
						}
						...
						if($isUpload) {
							...
							if (isset($_FILES['fileupload'])) {
								$this->hapusfile($path, $file_name, 'uji_mikro_bb_file', $this->input->post('uji_mikro_bb_iuji_mikro_bb'));
								foreach ($_FILES['fileupload']["error"] as $key => $error) {	
									if ($error == UPLOAD_ERR_OK) {
										$tmp_name = $_FILES['fileupload']["tmp_name"][$key];
										$name = $_FILES['fileupload']["name"][$key];
										$data['filename'] = $name;
										$data['id']=$this->input->post('uji_mikro_bb_iuji_mikro_bb');
										$data['nip']=$this->user->gNIP;
										//$data['iupb_id'] = $insertId;
										$data['dInsertDate'] = date('Y-m-d H:i:s');
						 				//$file_tanggal[$i] = date('l, F jS, Y', strtotime($file_tanggal[$i]));		
						 				if(move_uploaded_file($tmp_name, $path."/".$this->input->post('uji_mikro_bb_iuji_mikro_bb')."/".$name)) 
						 				{				 					
							...
							if (isset($_FILES['fileupload1'])) {
								...
								$this->hapusfile($path1, $file_name1, 'uji_mikro_bb_file_literatur', $this->input->post('uji_mikro_bb_iuji_mikro_bb'));
								foreach ($_FILES['fileupload1']["error"] as $key => $error) {	
									if ($error == UPLOAD_ERR_OK) {
										$tmp_name1 = $_FILES['fileupload1']["tmp_name"][$key];
										$name1 = $_FILES['fileupload1']["name"][$key];
										$data1['filename'] = $name1;
										$data1['id']=$this->input->post('uji_mikro_bb_iuji_mikro_bb');
										$data1['nip']=$this->user->gNIP;
										$data1['dInsertDate'] = date('Y-m-d H:i:s');
						 				if(move_uploaded_file($tmp_name1, $path1."/".$this->input->post('uji_mikro_bb_iuji_mikro_bb')."/".$name1)) 
						 				{
											$sql1[] = "INSERT INTO uji_mikro_bb_file_literatur(iuji_mikro_bb, filename, dInsertDate, keterangan,cInsert) 
...
// manipulate object 
    function insertBox_uji_mikro_bb_iApprove_uji($field, $id) {
		$o='-';
		$o.='
	...
	function updateBox_uji_mikro_bb_iApprove_uji($field, $id, $value, $rowData) {
		//print_r($rowData);
	...
	function insertBox_uji_mikro_bb_ireqdet_id($field, $id) {
		$return = '<script>
		...
		$return .= '&nbsp;<button class="icon_pop"  onclick="browse(\''.base_url().'processor/plc/uji/mikro/bb/popup/?field=uji_mikro_bb\',\'List Bahan Baku\')" type="button">&nbsp;</button>';                
		
		return $return;
	}

	function updateBox_uji_mikro_bb_ireqdet_id($field, $id, $value, $rowData) {
		$sql = '
	...
	function insertBox_uji_mikro_bb_no_po($field, $id) {
		$return = '<input type="text" name="'.$field.'"  id="'.$id.'"  disabled="disabled" class="input_rows1 required" size="8" />';
		return $return;
	}
	//,'no_po','manuf',''
	function updateBox_uji_mikro_bb_no_po($field, $id, $value, $rowData) {
		$sql = '
		select a.ireqdet_id,b.vreq_nomor,e.vpo_nomor,f.vro_nomor,g.vnama,a.ijumlah,h.vupb_nomor,h.vupb_nama,a.* ,d.*
		from plc2.plc2_upb_request_sample_detail a
		join plc2.plc2_upb_request_sample b on b.ireq_id=a.ireq_id
	...
	function insertBox_uji_mikro_bb_no_upb($field, $id) {
		$return = '<input type="text" name="'.$field.'"  id="'.$id.'"  disabled="disabled" class="input_rows1 required" size="30" />';
		return $return;
	}
	//,'no_po','manuf','no_upb'
	function updateBox_uji_mikro_bb_no_upb($field, $id, $value, $rowData) {
		$sql = '
		select a.ireqdet_id,b.vreq_nomor,e.vpo_nomor,f.vro_nomor,g.vnama,a.ijumlah,h.vupb_nomor,h.vupb_nama,a.* ,d.*
		from plc2.plc2_upb_request_sample_detail a
		join plc2.plc2_upb_request_sample b on b.ireq_id=a.ireq_id
	...
	function insertBox_uji_mikro_bb_iJumlah_terima($field, $id) {
		$return = '<input type="text" name="'.$field.'"  id="'.$id.'" class=" angka input_rows1 required" size="8" />
		<input type="hidden" name="isdraft" id="isdraft">';
		return $return;
	}

	function updateBox_uji_mikro_bb_iJumlah_terima($field, $id, $value, $rowData) {
		if ($this->input->get('action') == 'view') {
			$return= $value;
	...
	function insertBox_uji_mikro_bb_dMulai_literatur($field, $id) {
		$return = '<input type="text" name="'.$field.'"  id="'.$id.'" class=" tanggal input_rows1 " size="8" />';
		return $return;
	}

	function updateBox_uji_mikro_bb_dMulai_literatur($field, $id, $value, $rowData) {
		if ($this->input->get('action') == 'view') {
			$return= $value;
	...
	function insertBox_uji_mikro_bb_dSelesai_literatur($field, $id) {
		$return = '<input type="text" name="'.$field.'"  id="'.$id.'" class=" tanggal input_rows1 " size="8" />';
		return $return;
	}

	function updateBox_uji_mikro_bb_dSelesai_literatur($field, $id, $value, $rowData) {
		if ($this->input->get('action') == 'view') {
			$return= $value;
	...
	function insertBox_uji_mikro_bb_dMulai_uji($field, $id) {
		$return = '<input type="text" name="'.$field.'"  id="'.$id.'" class=" tanggal input_rows1 required" size="8" />';
		return $return;
	}

	function updateBox_uji_mikro_bb_dMulai_uji($field, $id, $value, $rowData) {
		if ($this->input->get('action') == 'view') {
			$return= $value;
	...
	function insertBox_uji_mikro_bb_dSelesai_uji($field, $id) {
		$return = '<input type="text" name="'.$field.'"  id="'.$id.'" class=" tanggal input_rows1 required" size="8" />';
		return $return;
	}

	function updateBox_uji_mikro_bb_dSelesai_uji($field, $id, $value, $rowData) {
		if ($this->input->get('action') == 'view') {
			$return= $value;
	...
	function insertBox_uji_mikro_bb_cPic_uji($field, $id) {
		$return = '<script>
						$( "button.icon_pop" ).button({
							icons: {
								primary: "ui-icon-newwin"
		...
		$return .= '&nbsp;<button class="icon_pop"  onclick="browse(\''.base_url().'processor/plc/browse/pic/?field=uji_mikro_bb&pic_for=uji_mikro_bb&idfield='.$field.'\',\'List PIC\')" type="button">&nbsp;</button>';                
		
		return $return;
	}

	function updateBox_uji_mikro_bb_cPic_uji($field, $id, $value, $rowData) {
	...
	function insertBox_uji_mikro_bb_file_filename() {
		$data['mydept'] = $this->auth_localnon->my_depts(TRUE);	
		$data['date'] = date('Y-m-d H:i:s');	
		return $this->load->view('lokal/uji_mikro_bb_file',$data,TRUE); 
	}

	function updateBox_uji_mikro_bb_file_filename($field, $id, $value, $rowData) {
		if (!empty($_GET['idnya'])) {
		...
		$data['mydept'] = $this->auth_localnon->my_depts(TRUE);
		$data['rows'] = $this->db_plc0->get_where('plc2.uji_mikro_bb_file', array('iuji_mikro_bb'=>$iuji_mikro_bb))->result_array();
		return $this->load->view('lokal/uji_mikro_bb_file',$data,TRUE);				
	}	


	function insertBox_uji_mikro_bb_file_literatur() {
		$data['mydept'] = $this->auth_localnon->my_depts(TRUE);	
		$data['date'] = date('Y-m-d H:i:s');	
		return $this->load->view('lokal/uji_mikro_bb_file_literatur',$data,TRUE); 
	}

	function updateBox_uji_mikro_bb_file_literatur($field, $id, $value, $rowData) {
		if (!empty($_GET['idnya'])) {
			$iuji_mikro_bb = $_GET['idnya'];
		}else{
			$iuji_mikro_bb = $rowData['iuji_mikro_bb'];
		...
		$data['mydept'] = $this->auth_localnon->my_depts(TRUE);
		$data['rows'] = $this->db_plc0->get_where('plc2.uji_mikro_bb_file_literatur', array('iuji_mikro_bb'=>$iuji_mikro_bb))->result_array();
		return $this->load->view('lokal/uji_mikro_bb_file_literatur',$data,TRUE);				
	}	
...
// end manipulate     
	...
	function manipulate_insert_button($buttons) {
		...
		$save_draft = '<button onclick="javascript:save_draft_btn_multiupload(\'uji_mikro_bb\', \''.base_url().'processor/plc/uji/mikro/bb?draft=true\', this, true)" class="ui-button-text icon-save" id="button_save_draft_uji_mikro_bb">Save as Draft</button>';
		$save = '<button onclick="javascript:save_btn_multiupload(\'uji_mikro_bb\', \''.base_url().'processor/plc/uji/mikro/bb?company_id='.$this->input->get('company_id').'&group_id='.$this->input->get('group_id').'&modul_id='.$this->input->get('modul_id').'\', this)" class="ui-button-text icon-save" id="button_save_uji_mikro_bb">Save &amp; Submit</button>';
		$js = $this->load->view('lokal/uji_mikro_bb_js');
	...
	function manipulate_update_button($buttons, $rowData) {
		...
		$approve = '<button onclick="javascript:load_popup(\''.base_url().'processor/plc/uji/mikro/bb?action=approve&iuji_mikro_bb='.$rowData['iuji_mikro_bb'].'&cNip='.$cNip.'&lvl=2&status=1&group_id='.$this->input->get('group_id').'&modul_id='.$this->input->get('modul_id').'\')" class="ui-button-text icon-save" id="button_approve_uji_mikro_bb">Confirm</button>';
		$update = '<button onclick="javascript:update_btn_back(\'uji_mikro_bb\', \''.base_url().'processor/plc/uji/mikro/bb?company_id='.$this->input->get('company_id').'&group_id='.$this->input->get('group_id').'&modul_id='.$this->input->get('modul_id').'\', this)" class="ui-button-text icon-save" id="button_save_uji_mikro_bb">Update & Submit</button>';
		$updatedraft = '<button onclick="javascript:update_draft_btn(\'uji_mikro_bb\', \''.base_url().'processor/plc/uji/mikro/bb?company_id='.$this->input->get('company_id').'&draft=true&group_id='.$this->input->get('group_id').'&modul_id='.$this->input->get('modul_id').'\', this, true)" class="ui-button-text icon-save" id="button_save_uji_mikro_bb">Update as Draft</button>';
	...
	function approve_view() {
					 ...
					 function submit_ajax(form_id) {
						return $.ajax({
					 	 	...
					 	 	success: function(data) {
								...
								var url = "'.base_url().'processor/plc/uji/mikro/bb";								
								if(o.status == true) {
									
									$("#alert_dialog_form").dialog("close");
										 $.get(url+"?action=update&id="+last_id, function(data) {
										 $("div#form_uji_mikro_bb").html(data);
										 $("#button_approve_uji_mikro_bb").hide();
										 $("#button_reject_uji_mikro_bb").hide();
									});
									
								}
									reload_grid("grid_uji_mikro_bb");
							}
		...
		$echo .= '<form id="form_uji_mikro_bb_approve" action="'.base_url().'processor/plc/uji/mikro/bb?action=approve_process" method="post">';
		...
		<button type="button" onclick="submit_ajax(\'form_uji_mikro_bb_approve\')">Confirm</button>';
