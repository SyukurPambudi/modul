...
function save_draft_btn_multiupload(grid, url, dis, isdraft) {
	...
	if($("#import_tambahan_data_cPIC").val()==''){
			$("#import_tambahan_data_cPIC_text").addClass('error_text');	
			conf++;
		}
		if($("#import_tambahan_data_iupb_id").val()==''){
			$("#import_tambahan_data_iupb_id_dis").addClass('error_text');	
			conf++;
		}
			...
			if(isUpload && !isValidAFileSize('#form_create_'+grid+' input.multifile', uploadLimit)) {
				...
				$.ajax({
					...
					success: function(data) {	
						...
						if(o.status == true) {
							if(isUpload) {
								var iframe = $('<iframe name="import_tambahan_data_frame"/>');
								iframe.attr({'id':'import_tambahan_data_frame'});
								...
								$('#form_create_'+grid).attr('target','import_tambahan_data_frame');

								//import_tambahan_data_tryUPload('form_create_'+grid , grid, formAction, base_url+'processor/plc/study/literatur/ad?action=update&id='+o.last_id+'&foreign_key='+o.foreign_id+'&company_id='+o.company_id+'&group_id='+o.group_id+'&modul_id='+o.modul_id );
								uploadfile('form_create_'+grid, grid, formAction, base_url+'processor/plc/import/tambahan/data?action=update&id='+o.last_id+'&foreign_key='+o.foreign_id+'&company_id='+o.company_id+'&group_id='+o.group_id+'&modul_id='+o.modul_id);

							}else{
								_custom_alert('Data Berhasil Disimpan !',header,info, grid, 1, 20000);
								$.get(base_url+'processor/plc/import/tambahan/data?action=update&id='+o.last_id+'&foreign_key='+o.foreign_id+'&company_id='+o.company_id+'&group_id='+o.group_id+'&modul_id='+o.modul_id, function(data) {
				...
				success: function(data) {
					...
					if(o.status == true){
						if(isUpload) {
							...
							$('#form_update_'+grid).attr('target','import_tambahan_data_frame');

							//import_tambahan_data_tryUPload('form_update_'+grid , grid, formAction, base_url+'processor/plc/study/literatur/ad?action=update&id='+o.last_id+'&foreign_key='+o.foreign_id+'&company_id='+o.company_id+'&group_id='+o.group_id+'&modul_id='+o.modul_id );
							uploadfile('form_update_'+grid, grid, formAction, base_url+'processor/plc/import/tambahan/data?action=update&id='+o.last_id+'&foreign_key='+o.foreign_id+'&company_id='+o.company_id+'&group_id='+o.group_id+'&modul_id='+o.modul_id);
	...
	//return false;
		custom_confirm(comfirm_message,function(){
			...
			//if(!isUpload) {
				...
				$.ajax({
				url: base_url +'processor/plc/import/tambahan/data?action=updateproses2',
				type: 'post',
				data: $('#form_update_'+grid).serialize(),
				success: function(data) {
...
<iframe name="import_tambahan_data_frame" id="import_tambahan_data_frame" height="0" width="0"></iframe>
