...
class v3_pembuatan_mbr extends MX_Controller {
    function __construct() {
		...
		$this->title = 'Pembuatan MBR';
		$this->url = 'v3_pembuatan_mbr';
		$this->urlpath = 'plc/'.str_replace("_","/", $this->url);

		$this->maintable = 'plc2.plc2_upb_buat_mbr';	
		$this->main_table = $this->maintable;	
		$this->main_table_pk = 'ibuatmbr_id';	
		$datagrid['islist'] = array(
			'formula.vNo_formula' => array('label'=>'No Formulasi','width'=>150,'align'=>'left','search'=>true)
			,'plc2_upb.vupb_nomor' => array('label'=>'No UPB','width'=>75,'align'=>'center','search'=>true)
			,'no_mbr' => array('label'=>'No MBR','width'=>200,'align'=>'left','search'=>true)
			,'dbuat_mbr' => array('label'=>'Tanggal Buat MBR','width'=>125,'align'=>'center','search'=>true)
			,'isubmit' => array('label'=>'Status Submit','width'=>100,'align'=>'left','search'=>false)
			,'iapppd_bm' => array('label'=>'Approval PD','width'=>100,'align'=>'left','search'=>false)
		);		
		...
		$datagrid['jointableinner']=array(
									0=>array('plc2.plc2_upb_formula'=>'plc2_upb_formula.ifor_id=plc2_upb_buat_mbr.ifor_id')
									,1=>array('plc2.plc2_upb'=>'plc2_upb.iupb_id=plc2_upb_formula.iupb_id')
									//,2=>array('pddetail.formula_process'=>'formula_process.iupb_id=plc2_upb_formula.iupb_id')
									,2=>array('pddetail.formula'=>'formula.iFormula_process=plc2_upb_formula.iFormula_process')
								);
    ...
    function index($action = '') {
		...
		$grid->changeFieldType('iapppd_bm', 'combobox','',array(''=>'--select--',0=>'Waiting for approval',1=>'Rejected', 2=>'Approved'));
	...
	/*List Box*/
	function listBox_v3_pembuatan_mbr_cPIC_AD($value, $pk, $name, $rowData) {
	...
	function listBox_v3_pembuatan_mbr_plc2_upb_iteampd_id($value, $pk, $name, $rowData) {
	...
	function insertBox_v3_pembuatan_mbr_form_detail($field,$id){
            $get=$this->input->get();
            $post=$this->input->post();
        ...
        function updateBox_v3_pembuatan_mbr_form_detail($field,$id,$value,$rowData){
            $get=$this->input->get();
            $post=$this->input->post();
            foreach ($get as $kget => $vget) {
                if($kget!="action"){
            ...
            foreach ($dFields as $form_field) {
                ...
                if($get['formaction']=='update'){
                    ...
                    if($form_field['iM_jenis_field'] == 6){
                        ...
                        $sqlEdit = str_replace('$filter$', $this->lib_plc->queryFilterUPBbyTeam($this->modul_id), $sqlEdit);
                        $union = 'SELECT f.ifor_id AS valval , CONCAT( u.vupb_nomor," | ",p.vNo_formula) AS showshow, u.vupb_nomor, p.vNo_formula
									FROM plc2.plc2_upb_formula f
									JOIN plc2.plc2_upb u ON f.iupb_id = u.iupb_id
									JOIN pddetail.formula p ON f.iFormula_process = p.iFormula_process
									WHERE f.ifor_id = '.$dataHead['ifor_id'].' UNION ';
                    ...
                    if ($form_field['iM_jenis_field'] == 13){
                        $sqlQuery = str_replace('$filter$', $this->lib_plc->queryFilterUPBbyTeam($this->modul_id), $sqlQuery);
                    }
