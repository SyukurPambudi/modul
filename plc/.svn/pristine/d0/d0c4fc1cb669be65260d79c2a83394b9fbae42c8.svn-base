		...
		custom_confirm(comfirm_message,function(){
			if(isUpload && !isValidAFileSize('#form_create_'+grid+' input.multifile', uploadLimit)) {
				...
				success: function(data) {
					...
					var header = 'Info';
							if(isUpload) {
								var iframe = $('<iframe name="request_originator_frame"/>');
								iframe.attr({'id':'request_originator_frame'});
								...
								$('#form_create_'+grid).attr('target','request_originator_frame');								
			...
			if(isUpload && !isValidAFileSize('#form_update_'+grid+' input.multifile', uploadLimit)) {
				...
				success: function(data) {
					...
					if(isUpload) {
						...
						$('#form_update_'+grid).attr('target','request_originator_frame');								
...
<iframe name="request_originator_frame" id="request_originator_frame" height="0" width="0"></iframe>