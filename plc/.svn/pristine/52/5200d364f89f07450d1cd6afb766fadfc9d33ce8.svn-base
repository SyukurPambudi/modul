<?php
	...
	$rows = array();
	$iM_modul_fileds = $form_field['iM_modul_fileds'];
	$masterDok = $this->db->get_where('plc2.sys_masterdok', array('ldeleted' => 0, 'iM_modul_fileds' => $iM_modul_fileds))->row_array();

	if ($act != 'create'){
		$ibk_id 			= $rowDataH['ibk_id'];
		$rows 				= $this->db->get_where('plc2.group_file_upload', array('iDeleted' => 0, 'iM_modul_fileds' => $iM_modul_fileds, 'idHeader_File' => $ibk_id))->result_array();
...
function del_bahan_kemas(dis, conname, table_id) {
	//if($('.'+conname).length > 1) {
		...
		if((j_p<=1)){
			_custom_alert("File Upload Primer Minimal 1","INFO","INFO", "bahan_kemas", 1, 20000);
			return false;
		}else{
...
function add_row_bahan_kemas(table_id){		
	    ...
	    $("#ijenis_bk_id"+c).change(function(){
			...
			url=base_url+'processor/v3/bk/primer?action=cekjnsbk';
			$.ajax({
				url: url,
				type: 'post',
				data: 'id='+$(this).val(),
...
<table class="hover_table" id="bahan_kemas" cellspacing="0" cellpadding="1" style="width: 98%; border: 1px solid #dddddd; text-align: center; margin-left: 5px; border-collapse: collapse">
	...
	<tbody>
		...
		if((in_array('BD', $mydept))||(in_array('QA', $mydept))||(in_array('BIC', $mydept)) ||(in_array('PD', $mydept)) ||(in_array('PPC', $mydept))) {
			...
			$type = "bahan_kemas/bahan_kemas_primer";
			if($value != '') {
				if (file_exists('./'.$masterDok['filepath'].'/'.$idsp.'/'.$value)) {
					$link = base_url().'processor/plc/v3/bk/primer?action=download&id='.$idsp.'&path='.$type.'&file='.$value;
					...
					<td style="border: 1px solid #dddddd; width: 4%; text-align: center;">
						...
						<script type="text/javascript">
							$("#ijenis_bk_id<?php echo $i ?>").change(function(){
								$(this).find('option').attr('class','');
								url=base_url+'processor/plc/v3/bk/primer?action=cekjnsbk';
								$.ajax({
									url: url,
									type: 'post',
									data: 'id='+$(this).val(),
