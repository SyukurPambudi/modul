...
class v3_uji_mikro_fg extends MX_Controller {
    function __construct() {
		...
		$this->title = 'Uji Mikro FG [Non Steril]';
		$this->url = 'v3_uji_mikro_fg';
		...
		$datagrid['islist'] = array(
			...
			,'istatus' => array('label'=>'Hasil Pengujian','width'=>100,'align'=>'left','search'=>false)
			,'iappqa_uji' => array('label'=>'Approval QA','width'=>100,'align'=>'left','search'=>false)
		);
		$datagrid['shortBy']=array('ifor_id'=>'Desc');
		

		$datagrid['setQuery']=array(
								0=>array('vall'=>'mikro_fg.lDeleted','nilai'=>0)
								,1=>array('vall'=>'plc2_upb.iupb_id IN (select study_literatur_pd.iupb_id from plc2.study_literatur_pd where study_literatur_pd.lDeleted=0 and study_literatur_pd.iuji_mikro=1 and study_literatur_pd.ijenis_sediaan=1)','nilai'=>NULL)
								);
		$datagrid['jointableinner']=array(
									0=>array('plc2.plc2_upb_formula'=>'plc2_upb_formula.ifor_id=mikro_fg.ifor_id')
									,1=>array('plc2.plc2_upb'=>'plc2.plc2_upb_formula.iupb_id = plc2.plc2_upb.iupb_id')
								);
            ...
            unset($buttons['update']);
                ...
                if($isOpenEditing){
                    ...
                    foreach ($activities as $act) {
                        $update_draft = '<button onclick="javascript:update_draft_btn(\''.$this->url.'\', \' '.base_url().'processor/'.$this->urlpath.'?draft=true&company_id='.$this->input->get('company_id').'&group_id='.$this->input->get('group_id').'&modul_id='.$this->input->get('modul_id').'\',this,true )"  id="button_update_draft_'.$this->url.'"  class="ui-button-text icon-save" >Update</button>';
	...
	/*List Box*/
	function listBox_v3_uji_mikro_fg_iappqa_uji($value) {
	...
	function listBox_v3_uji_mikro_fg_ini_nilai($value, $pk, $name, $rowData) {
	...
	function insertBox_v3_uji_mikro_fg_form_detail($field,$id){
            $get=$this->input->get();
            $post=$this->input->post();
            $in=array();
        ...
        function updateBox_v3_uji_mikro_fg_form_detail($field,$id,$value,$rowData){
            $get=$this->input->get();
            $post=$this->input->post();
            $in=array();
            foreach ($get as $kget => $vget) {
    ...
    function before_insert_processor($row, $postData) {
        $arr=isset($postData['plc2_v3_uji_mikro_fg_iJenis_pengujian_iJenis_pengujian_ijenis_pengujian'])?$postData['plc2_v3_uji_mikro_fg_iJenis_pengujian_iJenis_pengujian_ijenis_pengujian']:array();
        if(count($arr)>0){
            $postData['iJenis_pengujian']=implode(',',$arr);
        }
    ...
    function getDataPengujian(){
		...
		$where=array('lDeleted'=>0,$this->main_table_pk=>$post["id"]);
		$this->db_plc0->where($where);
		$q=$this->db_plc0->get($this->main_table);
		$rsel=array("select_row",'iact');
        $data = new StdClass;
        
        $row=$q->row_array();
        $nilaiini=isset($row['iJenis_pengujian'])?explode(',',$row['iJenis_pengujian']):array();
        
        $data->records=count($nilaiini);
		$i=0;
		foreach ($nilaiini as $k => $valval) {
			$data->rows[$i]['id']=$i;
			$z=0;

			foreach ($rsel as $dsel => $vsel) {
				if($vsel=="iact"){
					$dataar[$dsel]='<a class="ui-button-text" href="javascript:;" onclick="javascript:hapus_row_'.$nmTable.'('.$i.')">[Hapus]</a>';
				}elseif($vsel=="select_row"){
                    $dataar[$dsel]='<input type="hidden" class="num_rows_tb_details_plc2_v3_uji_mikro_fg_iJenis_pengujian_iJenis_pengujian" value="'.$i.'" /><select id="plc2_v3_uji_mikro_fg_iJenis_pengujian_iJenis_pengujian_ijenis_pengujian_'.$i.'" class="required" name="plc2_v3_uji_mikro_fg_iJenis_pengujian_iJenis_pengujian_ijenis_pengujian[]">';
                    $dataar[$dsel].="<option value=''>---Pilih---</option>";
                    foreach ($datapengujian as $kkkk => $vvvv) {
                        $select=$valval==$vvvv['ijenis_mikro']?'selected':'';
                        $dataar[$dsel].="<option value='".$vvvv['ijenis_mikro']."' ".$select." >".$vvvv['vjenis_mikro']."</option>";
                    }
                    $dataar[$dsel].='</select>';
				}else{
					$dataar[$dsel]=$k->{$vsel};
