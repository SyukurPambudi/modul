<?php 
	...
	$sqlgetprivi = 'select *,b.idprivi_modules 
					from plc3.m_modul_fileds a 
					join plc3.m_modul b on b.iM_modul=a.iM_modul
					where a.lDeleted=0 
					and  a.iM_modul='.$form_field['iM_modul'].' ';
	$dPrivi = $this->db->query($sqlgetprivi)->row_array();
	...
	switch ($iM_jenis_field) {
		...
		case '6':
			...
			if($sql<>''){
				...
				$sFieldValidation_after = 'select m_modul_fileds_validation_after.vSource_input_validtation_after  
									from plc3.m_modul_fileds_validation_after
									where m_modul_fileds_validation_after.lDeleted=0 and m_modul_fileds_validation_after.iM_modul_fileds="'.$form_field['iM_modul_fileds'].'" 
									order by m_modul_fileds_validation_after.iSort DESC';
				...
				if($dPrivi['iMutu'] == 1 && $dPrivi['vNama_field'] == 'iupb_id'  ){

				
					$sql .= '
							union 
							';
					if($dPrivi['iM_modul']== '32'){
						$sql .=	'select a.iupb_id as valval , concat(a.vupb_nomor," | ",a.vupb_nama) as showshow , 1 as iexisting ';
					}else{
						$sql .=	'select a.iupb_id as valval , concat(a.vupb_nomor," | ",a.vupb_nama) as showshow ';
					}		
					


					$sql .= 'from plc2.log_mutu b 
							join plc2.plc2_upb a on a.iupb_id=b.iupb_id
							where b.iDone=0
							and b.idprivi_modules= $idprivi_modules$ ';

					$sql = str_replace('$idprivi_modules$', $dPrivi['idprivi_modules'], $sql);
		...
		case '13':
			...
			if($form_field['iFilter_log']==1){
				$sql.=$this->lib_plc->getUpbLog($form_field['iM_modul'],0);
			}
			if($sql<>''){
				if(!isset($isAdmin)){
					$isAdmin=false;
		...
		case '15':
			$return = '<input type="text"  name="'.$field.'" id="'.$id.'" class="input_rows1 '.$field_required.' " readonly="readonly" size="35" >';
			break;

		case '16':

			/*Get Details Form*/
			$sql='select * from plc2.sys_masterdok ma where ma.ldeleted = 0 and ma.iM_modul_fileds='.$form_field['iM_modul_fileds'];

			$qsql=$this->db->query($sql);
			if($qsql->num_rows()>0){
					$rowData=$qsql->row_array();
					//Config Variable//
					$grid="plc2_".$id."_".$rowData['filename'];
					$nmmodule="plc";
					...
					<script type="text/javascript">
					    ...
					    $(document).ready(function(){
					        $grid.jqGrid({
					            ...
					            postData: {id:'<?php echo $id ?>',grid:"<?php echo $grid ?>",nmTable:"<?php echo $nmTable ?>",namefield:"<?php echo $rowData['filename'] ?>",iReq:<?php echo $form_field['iRequired']?> },
					            datatype: "json",
					            mtype:'POST',
		...
		case '20':
			$return = '<input type="hidden"  name="'.$field.'" id="'.$id.'" value="0" class="input_rows1 '.$field_required.'" size="10"  readonly="readonly">';
