...
class v3_study_literatur_ad extends MX_Controller {
    function __construct() {
        ...
        $this->main_table='study_literatur_ad';
        $this->main_table_pk='istudy_ad_id';


        $this->title = 'Study Literatur AD';
        $this->url = 'v3_study_literatur_ad';
        $this->urlpath = 'plc/'.str_replace("_","/", $this->url);

        $this->maintable = 'plc2.study_literatur_ad';  
        
        

        $datagrid['islist'] = array(
                                'plc2_upb.vupb_nomor'  => array('label' => 'No UPB',           'width' => 75,    'align' => 'center', 'search' => true),
                                'plc2_upb.vupb_nama'   => array('label' => 'Nama UPB',         'width' => 250,   'align' => 'center', 'search' => true),
                                'plc2_upb.vgenerik'    => array('label' => 'Nama Generik',     'width' => 250,   'align' => 'center', 'search' => true),
                                ...
                                'iappad'               => array('label' => 'Status Approval',  'width' => 100,   'align' => 'center', 'search' => false),
                            );

        $datagrid['shortBy'] = array('istudy_ad_id' => 'Desc');

        

        $datagrid['setQuery'] = array(
                                    0 => array('vall' => 'plc2_upb.ldeleted',           'nilai' => 0),
                                    1 => array('vall' => 'study_literatur_ad.ldeleted', 'nilai' => 0),
                                    2 => array('vall' => 'plc2_upb.iHold',              'nilai' => 0),
                                );

        $datagrid['jointableinner']=array(
                                    #0=>array('plc2.plc2_upb_formula'=>'plc2_upb_formula.ifor_id=mikro_fg.ifor_id')
                                    0=>array('plc2.plc2_upb'=>'plc2.study_literatur_ad.iupb_id = plc2.plc2_upb.iupb_id')
                                );
    ...
    function index($action = '') {
       ...
       /* $grid->setGroupBy($this->setGroupBy);*/
        ...
        $grid->changeFieldType('iappad','combobox','',array(0=>'Waiting Approval',1=>'Rejected',2=>'Approved'));
        ...
        switch ($action) {
            ...
            case 'uploadFile':
                ...
                $dataFieldUpload    = $this->lib_plc->getUploadFileFromField($this->input->get('modul_id'));
                // $dataFieldUpload    = $this->lib_plc->getUploadFileFromField(4236);
    ...
    function listBox_v3_study_literatur_ad_iappad($value) {
        if ($value == 0) {
    ...
    function listBox_v3_study_literatur_ad_cPIC($value) {
        $data   = '-';
        $dt     = $this->db_plc0->get_where('hrd.employee', array('cNip' => $value))->row_array();
        if($dt){
        ...
        return $data;
            ...
            function approve_view() {
                             ...
                             function submit_ajax(form_id) {
                                return $.ajax({
                                    ...
                                    success: function(data) {
                                        ...
                                        var url = "'.base_url().'processor/plc/v3_study_literatur_ad";                             
                                        if(o.status == true) { 
                                            $("#alert_dialog_form").dialog("close");
                                                 $.get(url+"?action=update&id="+last_id+"&foreign_key=0&company_id=3&group_id="+group_id+"&modul_id="+modul_id, function(data) {
                                                 $("div#form_v3_study_literatur_ad").html(data);
                                                 
                                            });
                                            
                                        }
                                            reload_grid("grid_v3_study_literatur_ad");
                                    }
                ...
                $echo .= '<form id="form_v3_study_literatur_ad_approve" action="'.base_url().'processor/plc/v3_study_literatur_ad?action=approve_process" method="post">';
                ...
                <button type="button" onclick="submit_ajax(\'form_v3_study_literatur_ad_approve\')">Approve</button>';
            ...
            function approve_process() {
                ...
                $istudy_ad_id = $post['istudy_ad_id'];
                ...
                $data=array('iappad'=>'2');
                $this -> db -> where('istudy_ad_id', $istudy_ad_id);
                $updet = $this -> db -> update('plc2.study_literatur_ad', $data);
                ...
                $this->lib_plc->InsertActivityModule($arrUPB,$modul_id,$istudy_ad_id,$iM_activity,$iSort,$vRemark,2);
            ...
            function confirm_view() {
                ...
                $echo .= '<form id="form_v3_study_literatur_ad_confirm" action="'.base_url().'processor/plc/v3_study_literatur_ad?action=confirm_process" method="post">';
                $echo .= '<div style="vertical-align: top;">';
                $echo .= 'Remark : 
                        <input type="hidden" name="istudy_ad_id" value="'.$this->input->get('istudy_ad_id').'" />
                        <input type="hidden" name="modul_id" value="'.$this->input->get('modul_id').'" />
                        ...
                        <textarea name="vRemark"></textarea>
                <button type="button" onclick="submit_ajax(\'form_v3_study_literatur_ad_confirm\')">Confirm</button>';
            ...
            function reject_view() {
                $echo = '<script type="text/javascript">
                             function submit_ajax(form_id) {
                                var remark = $("#v3_study_literatur_ad_remark").val();
                                if (remark=="") {
                ...
                $echo .= '<form id="form_v3_study_literatur_ad_reject" action="'.base_url().'processor/plc/v3_study_literatur_ad?action=reject_process" method="post">';
                $echo .= '<div style="vertical-align: top;">';
                $echo .= 'Remark : 
                        <input type="hidden" name="istudy_ad_id" value="'.$this->input->get('istudy_ad_id').'" />
                        <input type="hidden" name="modul_id" value="'.$this->input->get('modul_id').'" />
                        ...
                        <textarea name="vRemark" id="reject_v3_study_literatur_ad_remark"></textarea>
                <button type="button" onclick="submit_ajax(\'form_v3_study_literatur_ad_reject\')">Reject</button>';
            ...
            function reject_process() {
                ...
                $arrUPB['iupb_id'] = $iupb_id;
                $this->lib_plc->InsertActivityModule($arrUPB,$modul_id,$istudy_ad_id,$iM_activity,$iSort,$vRemark,1);
    ...
    function before_update_processor($row, $postData) {
        ...
        $controller_name ='v3_study_literatur_ad';
        $pk_field = 'istudy_ad_id';
    ...
    function manipulate_update_button($buttons, $rowData) { 
        ...
        if ($this->input->get('action') == 'view') {
            unset($buttons['update']);
                ...
                if($isOpenEditing){
                    $update_draft = '<button onclick="javascript:update_draft_btn(\'v3_study_literatur_ad\', \' '.base_url().'processor/plc/v3_study_literatur_ad?draft=true \',this,true )"  id="button_update_draft_v3_study_literatur_ad"  class="ui-button-text icon-save" >Update open Editing</button>';
                    ...
                    foreach ($activities as $act) {
                        $update_draft = '<button onclick="javascript:update_draft_btn(\'v3_study_literatur_ad\', \' '.base_url().'processor/plc/v3_study_literatur_ad?draft=true \',this,true )"  id="button_update_draft_v3_study_literatur_ad"  class="ui-button-text icon-save" >Update as Draft</button>';
                        $update = '<button onclick="javascript:update_btn_back(\'v3_study_literatur_ad\', \' '.base_url().'processor/plc/v3_study_literatur_ad?company_id='.$this->input->get('company_id').'&iM_modul_activity='.$act['iM_modul_activity'].'group_id='.$this->input->get('group_id').'modul_id='.$this->input->get('modul_id').' \',this,true )"  id="button_update_submit_v3_study_literatur_ad"  class="ui-button-text icon-save" >Update &amp; Submit</button>';

                        $approve = '<button onclick="javascript:load_popup(\' '.base_url().'processor/plc/v3_study_literatur_ad?action=approve&iM_modul_activity='.$act['iM_modul_activity'].'&iM_activity='.$act['iM_activity'].'&istudy_ad_id='.$peka.'&iupb_id='.$iupb_id.'&company_id='.$this->input->get('company_id').'&group_id='.$this->input->get('group_id').'&modul_id='.$this->input->get('modul_id').' \')"  id="button_approve_v3_study_literatur_ad"  class="ui-button-text icon-save" >Approve</button>';
                        $reject = '<button onclick="javascript:load_popup(\' '.base_url().'processor/plc/v3_study_literatur_ad?action=reject&iM_modul_activity='.$act['iM_modul_activity'].'&iM_activity='.$act['iM_activity'].'&istudy_ad_id='.$peka.'&iupb_id='.$iupb_id.'&company_id='.$this->input->get('company_id').'&group_id='.$this->input->get('group_id').'&modul_id='.$this->input->get('modul_id').' \' )"  id="button_reject_v3_study_literatur_ad"  class="ui-button-text icon-save" >Reject</button>';

                        $confirm = '<button onclick="javascript:load_popup(\' '.base_url().'processor/plc/v3_study_literatur_ad?action=confirm&iM_modul_activity='.$act['iM_modul_activity'].'&iM_activity='.$act['iM_activity'].'&istudy_ad_id='.$peka.'&iupb_id='.$iupb_id.'&company_id='.$this->input->get('company_id').'&group_id='.$this->input->get('group_id').'&modul_id='.$this->input->get('modul_id').' \')"  id="button_approve_v3_study_literatur_ad"  class="ui-button-text icon-save" >Confirm</button>';
    ...
    function insertBox_v3_study_literatur_ad_form_detail($field,$id){
            $get=$this->input->get();
            $post=$this->input->post();
    ...
    function updateBox_v3_study_literatur_ad_form_detail($field,$id,$value,$rowData){
        $get=$this->input->get();
        $post=$this->input->post();
        foreach ($get as $kget => $vget) {
            if($kget!="action"){
        ...
        if(!empty($dFields)){
            ...
            foreach ($dFields as $form_field) {
                ...
                if($form_field['iM_jenis_field'] == 7){
                    ...
                    $path = 'files/plc/study_literatur_ad';
                    $createname_space =$this->url;
                    $tempat = 'study_literatur_ad';
                    $FOLDER_APP = 'plc';
