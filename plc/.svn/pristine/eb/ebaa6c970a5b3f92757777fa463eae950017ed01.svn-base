...
$("#<?php echo $tableId;?> .file_remove").click(function(){
	...
	var tmpDel = $("#draft_soi_mikro_fg_del");
	li.remove();
	var v = tmpDel.val();
	v+=','+fileid;
	tmpDel.val( v );
	alert( $("#draft_soi_mikro_fg_del").val() );
});

function add_row_draft_soi_mikro_fg_upload(table_id){		
		//alert(table_id);
		...
		if (n.length == 0) {
			...
			row_content	 += '<input type="file" id="fileupload'+table_id+c+'" class="fileupload multi multifile required" name="fileupload[]" style="width: 70%" />*';
			...
			row_content	 += '<span class="delete_btn"><a href="javascript:;" class="draft_soi_mikro_fg_del" onclick="del_row(this, \'draft_soi_mikro_fg_del\')">[Hapus]</a></span></td>';		
			row_content  += '</tr>';
...
<table class="hover_table" id="draft_soi_mikro_fg_upload" cellspacing="0" cellpadding="1" style="width: 98%; border: 1px solid #dddddd; text-align: center; margin-left: 5px; border-collapse: collapse">
	<thead>
	...
	<tbody>
		<?php
			...
			if(!empty($rows)) {
				...
				foreach($rows as $row) {
					...
					$value = $row['vFilename'];	
					if($value != '') {
						if (file_exists('./files/plc/draft_soi_mikro_fg/'.$id.'/'.$value)) {
							$link = base_url().'processor/plc/draft/soi/mikro/fg?action=download&id='.$id.'&file='.$value;
		...
		?>
				<tr style="border: 1px solid #dddddd; border-collapse: collapse; background: #ffffff; ">
					<td style="border: 1px solid #dddddd; width: 3%; text-align: center;">
						<span class="draft_soi_mikro_fg_upload_num"><?php echo $i ?></span>
					</td>		
					<td colspan="1" style="border: 1px solid #dddddd; width: 27%">
						...
						<input type="hidden" name="fileid[]" style="width: 70%" value="<?php echo $row['idraft_soi_mikro_fg_file_id'] ?>" />
					</td>	
					...
					<td style="border: 1px solid #dddddd; width: 10%">
						<?php if ($this->input->get('action') != 'view') { ?>
							<span class="delete_btn"><a href="javascript:;" class="draft_soi_mikro_fg_del" onclick="del_row1(this, 'draft_soi_mikro_fg_del')">[Hapus]</a></span>	
						<?php	} ?>
		...
		<tr style="border: 1px solid #dddddd; border-collapse: collapse; background: #ffffff; ">
					<td style="border: 1px solid #dddddd; width: 3%; text-align: center;">
						<span class="draft_soi_mikro_fg_upload_num">1</span>
					</td>		
					<td colspan="1" style="border: 1px solid #dddddd; width: 27%">
						<input type="file" id="fileupload_draft_soi_mikro_fg1" class="fileupload multi multifile required" name="fileupload[]" style="width: 70%" /> *
					...
					<td colspan="4" style="border: 1px solid #dddddd; width: 15%">
						<textarea class="" id="filekt1" name="fileketerangan[]" style="width: 240px; height: 50px;"size="250"></textarea>
						<br/>
	...
	<tfoot>
		<tr>
			...
			<td style="text-align: center">
				<?php if ($this->input->get('action') != 'view') { ?>
					<a href="javascript:;" onclick="javascript:add_row_draft_soi_mikro_fg_upload('draft_soi_mikro_fg_upload')">Tambah</a>	
				<?php } ?>
	...
	<script>
		...
		$("#fileupload_draft_soi_mikro_fg1").change(function () {
	        var fileExtension = ["pdf","jpeg", "jpg", "png", "gif", "bmp"];
	        if ($.inArray($(this).val().split(".").pop().toLowerCase(), fileExtension) == -1) {
	        	_custom_alert("Only formats are allowed : "+fileExtension.join(", "),"info", "info", "draft_soi_mikro_fg", 1, 20000);
	        	$(this).val('');
	        }
    	});
	</script>