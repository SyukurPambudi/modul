...
class master_prinsipal extends MX_Controller {
...
$this->db_plc0 = $this->load->database('plc0',false, true);
    ...
    function index($action = '') {
		...
		$grid = new Grid;
		$grid->setTitle('prinsipal');
		//dc.m_vendor  database.tabel
		$grid->setTable('plc2.prinsipal');		
		$grid->setUrl('master_prinsipal');
		$grid->addList('vNama_prinsipal','vKeterangan');
		$grid->setSortBy('vNama_prinsipal');
		$grid->setSortOrder('asc'); //sort ordernya

		$grid->addFields('vNama_prinsipal','vKeterangan');//,'dupdate','cUpdate');

		//setting widht grid
		$grid ->setWidth('vNama_prinsipal', '200'); 
		$grid ->setWidth('vKeterangan', '350'); 
		$grid->setWidth('ideleted', '110'); 
		
		
		//modif label
		$grid->setLabel('vNama_prinsipal','Nama Prinsipal'); //Ganti Label
		...
		$grid->setSearch('vNama_prinsipal');
	...
	//Field mandatori
		$grid->setRequired('vNama_prinsipal');	
		$grid->setRequired('vKeterangan');	
	...
	//	$grid->setQuery('asset.asset_sparepart.ldeleted', 0);
		...
		switch ($action) {
			...
			case 'create':
				...
				break;
			case 'createproses':
					$key = $this->input->post('vNama_prinsipal');
                        $cek_data = 'select * from prinsipal a where a.vNama_prinsipal="'.$key.'" ';
                        $data_cek = $this->dbset->query($cek_data)->row_array();
                        if (empty($data_cek) ) {
                             echo $grid->saved_form();
                        }else{
                            $r['status'] = FALSE;
                            $r['message'] = "Nama Prinsipal Sudah ada";
                            echo json_encode($r);
                        }
				break;
			case 'download':
			...
			case 'view':
				...
				break;
			case 'updateproses':
				$id=$_POST['master_prinsipal_iprinsipal_id']; 
                $key = $this->input->post('vNama_prinsipal');

                $cek_data = 'select * from plc2.prinsipal a where a.vNama_prinsipal="'.$key.'" and a.lDeleted=0 ';
                $result = $this->db_plc0->query($cek_data)->row_array();

                $sql2 = 'select * from plc2.prinsipal a where a.iprinsipal_id = "'.$id.'"';
                $old = $this->db_plc0->query($sql2)->row_array();


                if (empty($result) or $old['vNama_prinsipal'] == $key  ) {
...
function insertBox_master_prinsipal_vNama_prinsipal($field, $id) {
	$return = '<input type="text" name="'.$field.'"  id="'.$id.'"  class="input_rows1 required" size="30" />';
	return $return;
}
function updateBox_master_prinsipal_vNama_prinsipal($field, $id, $value, $rowData) {
...
function insertBox_master_prinsipal_dupdate($field, $id) {
...
function updateBox_master_prinsipal_dupdate($field, $id, $value, $rowData) {
		$skg=date('Y-m-d H:i:s');
		if ($this->input->get('action') == 'view') {
			$return= $value;
...
function insertBox_master_prinsipal_cUpdate($field, $id) {
		$skg=date('Y-m-d');
		$cNip = $this->user->gNIP;
...
function updateBox_master_prinsipal_cUpdate($field, $id, $value, $rowData) {
		$skg=date('Y-m-d');
		$cNip = $this->user->gNIP;
