...
function save_draft_btn_multiupload(grid, url, dis, isdraft) {
	...
	if($("#uji_mikro_fg_cnip_penyusun").val()==''){
		$("#uji_mikro_fg_cnip_penyusun_text").addClass('error_text');	
		conf++;
	}
			...
			if(isUpload && !isValidAFileSize('#form_create_'+grid+' input.multifile', uploadLimit)) {
				...
				$.ajax({
					...
					success: function(data) {	
						...
						if(o.status == true) {
							if(isUpload) {
								var iframe = $('<iframe name="uji_mikro_fg_frame"/>');
								iframe.attr({'id':'uji_mikro_fg_frame'});
								...
								$('#form_create_'+grid).attr('target','uji_mikro_fg_frame');								
								//$('#form_create_'+grid).submit();
								uploadfile('form_create_'+grid, grid, formAction, base_url+'processor/plc/uji/mikro/fg?action=update&id='+o.last_id+'&foreign_key='+o.foreign_id+'&company_id='+o.company_id+'&group_id='+o.group_id+'&modul_id='+o.modul_id);
							}else{
								_custom_alert(o.message,header,info, grid, 1, 20000);
								info = 'info';
								header = 'Info';
								
								$.get(base_url+'processor/plc/uji/mikro/fg?action=update&id='+o.last_id+'&foreign_key='+o.foreign_id+'&company_id='+o.company_id+'&group_id='+o.group_id+'&modul_id='+o.modul_id, function(data) {
				...
				.done(function(data) {
					...
					$('#grid_'+grid).trigger('reloadGrid');
						...
						$.get(base_url+'processor/plc/uji/mikro/fg?action=update&id='+last_id+'&foreign_key='+foreign_id+'&company_id='+company_id+'&group_id='+group_id+'&modul_id='+modul_id, function(data) {
	                            $('div#form_'+grid).html(data);
	                            $('html, body').animate({scrollTop:$('#'+grid).offset().top - 20}, 'slow');
	                    });
				});*/
				...
				success: function(data) {
					...
					if(o.status == true){
						if(isUpload) {
							...
							$('#form_update_'+grid).attr('target','uji_mikro_fg_frame');								
							//$('#form_update_'+grid).submit();
							uploadfile('form_update_'+grid, grid, formAction, base_url+'processor/plc/uji/mikro/fg?action=update&id='+o.last_id+'&foreign_key='+o.foreign_id+'&company_id='+o.company_id+'&group_id='+o.group_id+'&modul_id='+o.modul_id);
						}else{
	...
	custom_confirm('Anda Yakin Confirm UPB : '+upbno,function(){
		$.ajax({
			...
			success: function(data) {
				...
				$.get(base_url+'processor/plc/uji/mikro/fg?action=view&id='+o.last_id+'&foreign_key='+o.foreign_id+'&company_id='+o.company_id+'&group_id='+o.group_id+'&modul_id='+o.modul_id, function(data) {
                        $('div#form_'+grid).html(data);
                        $('html, body').animate({scrollTop:$('#'+grid).offset().top - 20}, 'slow');
                });
			}		
...
<iframe name="uji_mikro_fg_frame" id="study_literatur_pd_frame" height="0" width="0"></iframe>
