...
class v3_soi_mikro_bb_final extends MX_Controller {
    function __construct() {
        ...
        $this->main_table='uji_mikro_bb';
        $this->main_table_pk='iuji_mikro_bb';
        ...
        $this->_table = 'plc2.uji_mikro_bb';

        $this->path = 'files/plc/local/soi_mikro_bb_final/rpt_soi_mikro_final';
        $this->tempat = 'local/soi_mikro_bb_final/rpt_soi_mikro_final';
        $this->folderApp = 'plc';

        $this->url = 'v3_soi_mikro_bb_final';
    ...
    function index($action = '') {
        ...
        $grid = new Grid;
        $grid->setTitle('SOI Mikro BB Final');       
        $grid->setTable($this->_table);     
        $grid->setUrl($this->url);
        $grid->addList('plc2_upb_request_sample.vreq_nomor','plc2_raw_material.vnama','plc2_upb.vupb_nomor','plc2_upb.vupb_nama','iJumlah_terima','iSubmit_mikro_final','iApprove_mikro_final');
        $grid->setSortBy('iuji_mikro_bb');
        $grid->setSortOrder('DESC');
        $grid->setSearch('plc2_upb_request_sample.vreq_nomor', 'plc2_raw_material.vnama','plc2_upb.vupb_nomor','plc2_upb.vupb_nama');

        $grid->setLabel('iApprove_mikro_final', 'Status Approval'); //Ganti Label
        $grid->setAlign('iApprove_mikro_final', 'Center'); //Align nya
        $grid->setWidth('iApprove_mikro_final', '50'); // width nya
        ...
        $grid->setLabel('soi_mikro_final', 'Upload SOI Final Mikro BB Umum + Spesifik'); //Ganti Label
        $grid->setLabel('cPic_mikro_final', 'PIC Upload'); //Ganti Label
        ...
        $grid->setWidth('plc2_upb.vupb_nama', '250'); // width nya
        ...
        $grid->setLabel('iSubmit_mikro_final', 'Status Submit'); //Ganti Label
        $grid->setAlign('iSubmit_mikro_final', 'center'); //Align nya
        $grid->setWidth('iSubmit_mikro_final', '100'); // width nya

        $grid->setLabel('iApprove_mikro_final', 'Status Approval'); //Ganti Label
        $grid->setAlign('iApprove_mikro_final', 'center'); //Align nya
        $grid->setWidth('iApprove_mikro_final', '150'); // width nya
        ...
        $grid->changeFieldType('iSubmit_mikro_final','combobox','',array('0'=>'Draft','1'=>'Submitted'));
        ...
        $grid->changeFieldType('iApprove_mikro_final','combobox','',array(''=>'--Select--',0=>'Waiting Approval',1=>'Rejected', 2=>'Approved'));

        $grid->setQuery('uji_mikro_bb.lDeleted', 0);    
        $grid->setQuery('uji_mikro_bb.iApprove_uji', 2); 
        $grid->setQuery('plc2_upb.ldeleted', 0);
        $grid->setQuery('plc2_upb.iKill', 0);
                        ...
                        foreach ($dFields as $form_field) {
                            ...
                            if($get['formaction']=='update'){
                                ...
                                if($form_field['iM_jenis_field'] == 19){
                                    $sqlDet = 'SELECT rm.vnama, po.vpo_nomor, u.vupb_nomor, u.vupb_nama FROM plc2.plc2_upb_request_sample_detail rsd
                                                JOIN plc2.plc2_upb_request_sample rs ON rsd.ireq_id = rs.ireq_id
                                                JOIN plc2.plc2_raw_material rm ON rsd.raw_id = rm.raw_id
                                                JOIN plc2.plc2_upb_ro_detail rd ON rsd.ireq_id = rd.ireq_id AND rsd.raw_id = rd.raw_id
                                                LEFT JOIN plc2.plc2_upb_po po ON rd.ipo_id = po.ipo_id
                                                JOIN plc2.plc2_upb u ON rs.iupb_id = u.iupb_id
                                                WHERE rsd.ireqdet_id = ? AND rsd.ldeleted = 0 AND rs.ldeleted = 0 AND rm.ldeleted = 0
                                                    AND rd.ldeleted = 0 AND u.ldeleted = 0';
                                    $detail = $this->db->query($sqlDet, array($dataHead['ireqdet_id']))->row_array(); 
                                    switch ($form_field['vNama_field']) {
                                        case 'bahan_baku':
                                            $data_field['label'] = (isset($detail['vnama']))?$detail['vnama']:'-';
                                            break;
                                        case 'no_po':
                                            $data_field['label'] = (isset($detail['vpo_nomor']))?$detail['vpo_nomor']:'-';
                                            break;
                                        case 'no_upb':
                                            $data_field['label'] = (isset($detail['vupb_nomor']))?$detail['vupb_nomor'].' | '.$detail['vupb_nama']:'-';
                                            break;
                                        case 'cPic_uji':
                                            $nip = $dataHead[$form_field['vNama_field']];
                                            $sqlEmp = 'SELECT vName FROM hrd.employee WHERE cNip = ? ';
                                            $empl = $this->db->query($sqlEmp, array($nip))->row_array();
                                            $data_field['label'] = (count($empl)>0)?$nip.' | '.$empl['vName']:'-';
                                        ...
                                        case 'cPic_draft_soi':
                                            ...
                                            break;
                                        case 'iHasil_uji':
                                            $arrHasil = array(1=>'Gagal',2=>'Berhasil');
                                            $data_field['label'] = $arrHasil[$dataHead[$form_field['vNama_field']]];
                                            $data_field['default'] = $dataHead[$form_field['vNama_field']];
                                            break;
                                        case 'iUji_spesifik':
                                            $arrHasil = array(0=>'Tidak',1=>'Ya');
                                            $data_field['label'] = $arrHasil[$dataHead[$form_field['vNama_field']]];
                                            $data_field['default'] = $dataHead[$form_field['vNama_field']];
                                            break;
                                    ...
                                    $data_field['qrFiles'] = 'SELECT id, iuji_mikro_bb, filename AS vFilename, keterangan AS vKeterangan, IF (vFilename_generate IS NOT NULL, vFilename_generate, filename) AS vFilename_generate FROM plc2.file_soi_mikro_final WHERE '.$this->main_table_pk.' = '.$dataHead[$this->main_table_pk].' AND ( ldeleted = 0 OR ldeleted IS NULL)';
                                }else{
                                ...
                                if ($form_field['iM_jenis_field'] == 13){
                                    $sqlQuery .= $this->filterUPBByTeam('u');
        ...
        function listBox_v3_soi_mikro_bb_final_iappqc($value) {
        ...
        function listBox_v3_soi_mikro_bb_final_iappqa($value) {
            if($value==0){$vstatus='Waiting for approval';}
            elseif($value==1){$vstatus='Rejected';}
            elseif($value==2){$vstatus='Approved';}
            return $vstatus;
        ...
        function listBox_v3_soi_mikro_bb_final_itype($value) {
        ...
        function insertBox_v3_soi_mikro_bb_final_form_detail($field,$id){
            $get=$this->input->get();
            $post=$this->input->post();
            ...
            $g=implode("&", $in);
            $return = '<script>
                var sebelum = $("label[for=\'v3_soi_mikro_bb_final_form_detail\']").parent();
                $("label[for=\'v3_soi_mikro_bb_final_form_detail\']").remove();
                sebelum.attr("id","'.$id.'");
                sebelum.html("");
                sebelum.removeAttr("class");
                sebelum.removeAttr("style");
                $.ajax({
                    url: base_url+"processor/plc/v3/soi/mikro/bb/final?action=getFormDetail&formaction=addnew&'.$g.'",
                    type: "post",
                    data: iro_id=0,
        ...
        function updateBox_v3_soi_mikro_bb_final_form_detail($field,$id,$value,$rowData){
            $get=$this->input->get();
            $post=$this->input->post();
            foreach ($get as $kget => $vget) {
                if($kget!="action"){
            ...
            $return = '<script>
                ...
                sebelum.removeAttr("style");
                $.ajax({
                    url: base_url+"processor/plc/v3/soi/mikro/bb/final?action=getFormDetail&formaction=update&'.$g.'",
                    type: "post",
                    data: iro_id=0,
                    success: function(data) {
                        var o = $.parseJSON(data);
            ...
            function approve_view() { 
                             ...
                             function submit_ajax(form_id) {
                                return $.ajax({
                                    ...
                                    success: function(data) {
                                        ...
                                        var url = "'.base_url().'processor/plc/v3_soi_mikro_bb_final";                             
                                        if(o.status == true) { 
                                            $("#alert_dialog_form").dialog("close");
                                                 $.get(url+"?action=update&id="+last_id+"&foreign_key=0&company_id=3&group_id="+group_id+"&modul_id="+modul_id, function(data) {
                                                 $("div#form_v3_soi_mikro_bb_final").html(data);
                                                 
                                            });
                                            
                                        }
                                            reload_grid("grid_v3_soi_mikro_bb_final");
                                    }
                ...
                $echo .= '<form id="form_v3_soi_mikro_bb_final_approve" action="'.base_url().'processor/plc/v3_soi_mikro_bb_final?action=approve_process" method="post">';
                ...
                <button type="button" onclick="submit_ajax(\'form_v3_soi_mikro_bb_final_approve\')">Approve</button>';
            ...
            function confirm_view() { 
                ...
                $echo .= '<form id="form_v3_soi_mikro_bb_final_confirm" action="'.base_url().'processor/plc/v3_soi_mikro_bb_final?action=confirm_process" method="post">';
                $echo .= '<div style="vertical-align: top;">';
                $echo .= 'Remark : 
                        <input type="hidden" name="'.$this->main_table_pk.'" value="'.$this->input->get($this->main_table_pk).'" />
                        <input type="hidden" name="modul_id" value="'.$this->input->get('modul_id').'" />
                        ...
                        <textarea name="vRemark"></textarea>
                <button type="button" onclick="submit_ajax(\'form_v3_soi_mikro_bb_final_confirm\')">Confirm</button>';
            ...
            function reject_view() {
                $echo = '<script type="text/javascript">
                             function submit_ajax(form_id) {
                                var remark = $("#v3_soi_mikro_bb_final_remark").val();
                                if (remark=="") {
                ...
                $echo .= '<form id="form_v3_soi_mikro_bb_final_reject" action="'.base_url().'processor/plc/v3_soi_mikro_bb_final?action=reject_process" method="post">';
                $echo .= '<div style="vertical-align: top;">';
                $echo .= 'Remark : 
                        <input type="hidden" name="'.$this->main_table_pk.'" value="'.$this->input->get($this->main_table_pk).'" />
                        <input type="hidden" name="modul_id" value="'.$this->input->get('modul_id').'" />
                        ...
                        <textarea name="vRemark" id="reject_v3_soi_mikro_bb_final_remark"></textarea>
                <button type="button" onclick="submit_ajax(\'form_v3_soi_mikro_bb_final_reject\')">Reject</button>';
    ...
    function before_insert_processor($row, $postData) {
        ...
        if($postData['isdraft'] == true){
            $postData['iSubmit_mikro_final'] = 0;
        } else {
            $postData['iSubmit_mikro_final'] = 1;
        } 
    ...
    function before_update_processor($row, $postData) {
        ...
        unset($postData['bahan_baku']);
        unset($postData['no_po']);
        unset($postData['no_upb']);
        unset($postData['iJumlah_terima']);
        unset($postData['dMulai_literatur']);
        unset($postData['dSelesai_literatur']);
        unset($postData['dMulai_uji']);
        unset($postData['dSelesai_uji']);
        unset($postData['cPic_uji']);
        unset($postData['iHasil_uji']);
        unset($postData['iUji_spesifik']);
        // unset($postData['dMulai_draft_soi']);
        // unset($postData['dSelesai_draft_soi']);
        // unset($postData['cPic_draft_soi']);
    ...
    function after_insert_processor($fields, $id, $postData) { 
        ...
        if ($postData['iSubmit_mikro_final'] == 1){
    ...
    function manipulate_update_button($buttons, $rowData) { 
        ...
        if ($this->input->get('action') == 'view') {
            unset($buttons['update']);
                ...
                if($isOpenEditing){
                    $update_draft = '<button onclick="javascript:update_draft_btn(\'v3_soi_mikro_bb_final\', \' '.base_url().'processor/plc/v3_soi_mikro_bb_final?draft=true \',this,true )"  id="button_update_draft_v3_soi_mikro_bb_final"  class="ui-button-text icon-save" >Update open Editing</button>';
                    ...
                    foreach ($activities as $act) {
                        $update_draft = '<button onclick="javascript:update_draft_btn(\'v3_soi_mikro_bb_final\', \' '.base_url().'processor/plc/v3_soi_mikro_bb_final?draft=true&modul_id='.$this->input->get('modul_id').'&iM_modul_activity='.$act['iM_modul_activity'].' \',this,true )"  id="button_update_draft_v3_soi_mikro_bb_final"  class="ui-button-text icon-save" >Update as Draft</button>';
                        $update = '<button onclick="javascript:update_btn_back(\'v3_soi_mikro_bb_final\', \' '.base_url().'processor/plc/v3_soi_mikro_bb_final?company_id='.$this->input->get('company_id').'&modul_id='.$this->input->get('modul_id').'&iM_modul_activity='.$act['iM_modul_activity'].'&group_id='.$this->input->get('group_id').'modul_id='.$this->input->get('modul_id').' \',this,false )"  id="button_update_submit_v3_soi_mikro_bb_final"  class="ui-button-text icon-save" >Update &amp; Submit</button>';

                        $approve = '<button onclick="javascript:load_popup(\' '.base_url().'processor/plc/v3_soi_mikro_bb_final?action=approve&iM_modul_activity='.$act['iM_modul_activity'].'&iM_activity='.$act['iM_activity'].'&'.$this->main_table_pk.'='.$peka.'&iupb_id='.$iupb_id.'&company_id='.$this->input->get('company_id').'&group_id='.$this->input->get('group_id').'&modul_id='.$this->input->get('modul_id').' \')"  id="button_approve_v3_soi_mikro_bb_final"  class="ui-button-text icon-save" >Approve</button>';
                        $reject = '<button onclick="javascript:load_popup(\' '.base_url().'processor/plc/v3_soi_mikro_bb_final?action=reject&iM_modul_activity='.$act['iM_modul_activity'].'&iM_activity='.$act['iM_activity'].'&'.$this->main_table_pk.'='.$peka.'&iupb_id='.$iupb_id.'&company_id='.$this->input->get('company_id').'&group_id='.$this->input->get('group_id').'&modul_id='.$this->input->get('modul_id').' \' )"  id="button_reject_v3_soi_mikro_bb_final"  class="ui-button-text icon-save" >Reject</button>';

                        $confirm = '<button onclick="javascript:load_popup(\' '.base_url().'processor/plc/v3_soi_mikro_bb_final?action=confirm&iM_modul_activity='.$act['iM_modul_activity'].'&iM_activity='.$act['iM_activity'].'&'.$this->main_table_pk.'='.$peka.'&iupb_id='.$iupb_id.'&company_id='.$this->input->get('company_id').'&group_id='.$this->input->get('group_id').'&modul_id='.$this->input->get('modul_id').' \')"  id="button_approve_v3_soi_mikro_bb_final"  class="ui-button-text icon-save" >Confirm</button>';
...
/*                            }else{
    ...
    private function ViewUPB ($id=0){
        $sql = 'SELECT s.iupb_id FROM plc2.uji_mikro_bb m 
                JOIN plc2.plc2_upb_request_sample_detail d ON m.ireqdet_id = d.ireqdet_id
                JOIN plc2.plc2_upb_request_sample s ON d.ireq_id = s.ireq_id
                WHERE m.iuji_mikro_bb = ? AND m.lDeleted = 0';
