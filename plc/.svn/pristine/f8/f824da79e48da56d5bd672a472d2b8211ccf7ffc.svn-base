...
function add_row_ws(table_id){		
		...
		if (n.length == 0) {
			...
			row_content  +=	'<select name="confirmw[]"><option value="0">Haven\'t been checked</option><option value="1">Yes</option><option value="3">No</option><option value="2">Unnecessary</option></select>';
			<?php } 
				else{?>
			row_content  +=	'<select name="confirmw[]"><option value="0">Haven\'t been checked</option></select>';
			<?		}
				}?>
			row_content  +=	'</td>';
			row_content	 += '<td colspan="2" style="border: 1px solid #dddddd; width: 50%">';
			row_content	 += '<input type="file" class="fileupload multi multifile required" name="fileuploadw[]" style="width: 70%" />*max 5 mb';
			row_content  += '<input type="hidden" name="namafilew[]" style="width: 70%" value="" />';
			row_content  += '<input type="hidden" name="fileidw[]" style="width: 70%" value="" /></td>';	
			row_content	 += '<td colspan="2" style="border: 1px solid #dddddd; width: 8%">';
			row_content	 += '<textarea class="" name="fileketeranganw[]" style="width: 240px; height: 50px;"></textarea></td>';
			row_content	 += '<td style="border: 1px solid #dddddd; width: 10%">';
			row_content	 += '<span class="delete_btn"><a href="javascript:;" class="ws_del" onclick="del_row1(this, \'ws_del\')">[Hapus]</a></span></td>';		
			row_content  += '</tr>';
...
<table class="hover_table" id="ws" cellspacing="0" cellpadding="1" style="width: 98%; border: 1px solid #dddddd; text-align: center; margin-left: 5px; border-collapse: collapse">
	<thead>
	...
	<tbody>
		...
		if((in_array('BD', $mydept))||(in_array('PR', $mydept))) {
			...
			$type = "ws";
			if($value != '') {
				if (file_exists('./files/plc/ws/'.$idsp.'/'.$value)) {
		...
		?>
				<tr style="border: 1px solid #dddddd; border-collapse: collapse; background: #ffffff; ">
					...
					<td style="border: 1px solid #dddddd; width: 4%; text-align: center;">
						...
						?>
						<input type="hidden" name="confirmw[]" value="<?php echo $row['iconfirm_busdev'];?>">
						<span>
					...
					<td colspan="2" style="border: 1px solid #dddddd; width: 27%">
						...
						<input type="hidden" name="namafilew[]" style="width: 70%" value="<?php echo $row['filename'] ?>" />
						<input type="hidden" name="fileidw[]" style="width: 70%" value="<?php echo $row['id'] ?>" />
					</td>	
		...
		?>
				<tr style="border: 1px solid #dddddd; border-collapse: collapse; background: #ffffff; ">
					...
					<td colspan="2" style="border: 1px solid #dddddd; width: 27%">
						<input type="file" class="fileupload multi multifile required" name="fileuploadw[]" style="width: 70%" /> *max 5 mb
						<input type="hidden" name="namafilew[]" style="width: 70%" value="" />
						<input type="hidden" name="fileidw[]" style="width: 70%" value="" />
						</td>	
					<td colspan="2" style="border: 1px solid #dddddd; width: 15%">
						<textarea class="" name="fileketeranganw[]" style="width: 240px; height: 50px;"></textarea>
					</td>
					<td style="border: 1px solid #dddddd; width: 10%">
						<span class="delete_btn"><a href="javascript:;" class="ws_del" onclick="del_row1(this, 'ws_del')">[Hapus]</a></span>
					</td>	
				</tr>
		<?php }
		} 	
