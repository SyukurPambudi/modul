	...
	*/
	...
	switch ($iM_jenis_field) {
		...
		case '9':
			...
			if (isset($iM_modul_fileds)){
				if($iM_modul_fileds == 19){
					$type = 'BD';
					$usernip = $logged_nip;
					...
					<script type="text/javascript">
					    ...
					    $(document).ready(function(){
					        $grid.jqGrid({
					            ...
					            postData: {id:'<?php echo $dataHead[$main_table_pk] ?>',grid:"<?php echo $grid ?>",nmTable:"<?php echo $nmTable ?>",namefield:"<?php echo $rowData['filename'] ?>",iReq:<?php echo $form_field['iRequired']?> },
					            datatype: "json",
					            mtype:'POST',
