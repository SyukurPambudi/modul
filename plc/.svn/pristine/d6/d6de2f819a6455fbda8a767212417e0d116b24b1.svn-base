...
class Browse_upb_setreg extends MX_Controller {
...
$this->db_plc0 = $this->load->database('plc0',false, true);
    ...
    function index($action = '') {
		...
		$grid->setQuery('ihold', 0); // upb yg bisa dipilih hanya upb yg tdk di hold
		// upb yg bisa dipilih hanya upb yg blm approve DR utk launch produk 
		$grid->setQuery('iupb_id not in (select f.iupb_id from plc2.plc2_upb_bahan_kemas f where f.iappdr_launch=2 and f.ldeleted=0)',null); 
		...
		//sudah app prareg
		$grid->setQuery('iupb_id in (select uu.iupb_id from plc2.plc2_upb uu where uu.iappbusdev_prareg=2 and uu.ldeleted=0)',null); 
		//sudah app prareg atau tidak butuh prareg
		//$grid->setQuery('iupb_id in (select f.iupb_id from plc2.plc2_upb f where f.iappbusdev_prareg=2 or f.iappbusdev_prareg=3)', null); //
	...
	function listBox_browse_upb_setreg_pilih($value, $pk, $name, $rowData) {
