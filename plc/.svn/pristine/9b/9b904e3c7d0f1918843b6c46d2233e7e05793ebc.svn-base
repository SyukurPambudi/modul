...
function update_draft_btn(grid, url, dis, isdraft) {
	...
	if($("#soi_mikro_fg_vPIC_soi").val()==''){
		$("#soi_mikro_fg_vPIC_soi_text").addClass('error_text');
		$("#soi_mikro_fg_vPIC_soi_text").val("");
		conf++;
	}
	if(conf > 0) {
				...
				success: function(data) {
					...
					if(o.status == true){
						if(isUpload) {
							...
							var iframe = $('<iframe name="soi_mikro_fg_frame"/>');
							iframe.attr({'id':'soi_mikro_fg_frame'});
							...
							$('#form_update_'+grid).attr('target','soi_mikro_fg_frame');								
							//$('#form_update_'+grid).submit();
							uploadfile('form_update_'+grid, grid, formAction, base_url+'processor/plc/soi/mikro/fg?action=update&id='+o.last_id+'&foreign_key='+o.foreign_id+'&company_id='+o.company_id+'&group_id='+o.group_id+'&modul_id='+o.modul_id);

						}else{
							_custom_alert('Data Berhasil Disimpan !',header,info, grid, 1, 20000);
							$('#grid_'+grid).trigger('reloadGrid');	
	
							$.get(base_url+'processor/plc/soi/mikro/fg?action=update&id='+o.last_id+'&foreign_key='+o.foreign_id+'&company_id='+o.company_id+'&group_id='+o.group_id+'&modul_id='+o.modul_id, function(data) {
		...
		/*	.done(function(data) {
					...
					$('#grid_'+grid).trigger('reloadGrid');
						...
						$.get(base_url+'processor/plc/soi/mikro/fg?action=update&id='+last_id+'&foreign_key='+foreign_id+'&company_id='+company_id+'&group_id='+group_id+'&modul_id='+modul_id, function(data) {
	                            $('div#form_'+grid).html(data);
	                            $('html, body').animate({scrollTop:$('#'+grid).offset().top - 20}, 'slow');
	                    });
				});*/
	...
	custom_confirm('Anda Yakin Confirm UPB : '+upbno,function(){
		$.ajax({
			...
			success: function(data) {
				...
				$.get(base_url+'processor/plc/soi/mikro/fg?action=view&id='+o.last_id+'&foreign_key='+o.foreign_id+'&company_id='+o.company_id+'&group_id='+o.group_id+'&modul_id='+o.modul_id, function(data) {
                        $('div#form_'+grid).html(data);
                        $('html, body').animate({scrollTop:$('#'+grid).offset().top - 20}, 'slow');
                });
			}		
...
<iframe name="soi_mikro_fg_frame" id="soi_mikro_fg_frame" height="0" width="0"></iframe>
