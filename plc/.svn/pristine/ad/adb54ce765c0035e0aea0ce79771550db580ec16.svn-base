...
function add_row_file_reg(table_id){		
		//alert(table_id);
		...
		if (n.length == 0) {
			...
			row_content	 += '<input type="file" id="fileupload'+table_id+c+'" class="fileupload multi multifile" name="fileupload[]" style="width: 70%" />*max 5 mb';
...
<table class="hover_table" id="upi_file_reg" cellspacing="0" cellpadding="1" style="width: 98%; border: 1px solid #dddddd; text-align: center; margin-left: 5px; border-collapse: collapse">
	<thead>
	...
	<tbody>
		<?php
			...
			if(!empty($rows)) {
				...
				foreach($rows as $row) {
					...
					$id  = $row['iregistrasi_upi_id'];
					$value = $row['vFilename'];	
					if($value != '') {
						if (file_exists('./files/plc/import/upi/file_reg/'.$id.'/'.$value)) {
							$link = base_url().'processor/plc/import/registrasi/upi?action=download&id='.$id.'&file='.$value;
		...
		?>
				<tr style="border: 1px solid #dddddd; border-collapse: collapse; background: #ffffff; ">
					<td style="border: 1px solid #dddddd; width: 3%; text-align: center;">
						<span class="upi_file_reg_num"><?php echo $i ?></span>
					</td>		
		...
		<tr style="border: 1px solid #dddddd; border-collapse: collapse; background: #ffffff; ">
					<td style="border: 1px solid #dddddd; width: 3%; text-align: center;">
						<span class="upi_file_reg_num">1</span>
					</td>		
					<td colspan="1" style="border: 1px solid #dddddd; width: 27%">
						<input type="file" id="fileupload_study_ad_1" class="fileupload multi multifile" name="fileupload[]" style="width: 70%" /> *max 5 mb
	...
	<tfoot>
		<tr>
			<td colspan="5" style="text-align: left">*) Max File 5 MB : Format = pdf, docx, xlsx, jpeg, jpg, png</td><td style="text-align: center">
				<?php if ($this->input->get('action') != 'view') { ?>
					<a href="javascript:;" onclick="javascript:add_row_file_reg('upi_file_reg')">Tambah</a>	
				<?php } ?>
