<?php if ( ! defined('BASEPATH')) exit('No direct script access allowed');
    class soi_mikro_bb_final extends MX_Controller {
...
$this->db_plc0 = $this->load->database('plc0',false, true);
    ...
    function index($action = '') {
        ...
        $grid = new Grid;		
        $grid->setUrl('soi_mikro_bb_final');
        $grid->setTitle('SOI Mikro BB Final');		
        $grid->setTable('plc2.uji_mikro_bb');		
        $grid->setUrl('soi_mikro_bb_final');
        $grid->addList('plc2_upb_request_sample.vreq_nomor','plc2_raw_material.vnama','iJumlah_terima','iSubmit','iApprove_mikro_final');
        $grid->addFields('iApprove_mikro_final','ireqdet_id','no_po','no_upb','iJumlah_terima','dMulai_literatur','dSelesai_literatur','dMulai_uji','dSelesai_uji','cPic_uji','iHasil_uji','iUji_spesifik','dMulai_draft_soi','dSelesai_draft_soi','cPic_draft_soi','soi_mikro_final','cPic_mikro_final');
        ...
        switch ($action) {
                ...
                case 'createproses':
						...
						$sql1 = array();
		   				if($isUpload) {
							$path = realpath("files/plc/local/soi_mikro_bb_final/rpt_mikro_bb/");
							$path1 = realpath("files/plc/local/soi_mikro_bb_final/rpt_soi_mikro_final/");
								...
								foreach ($_FILES['fileupload1']["error"] as $key => $error) {
									if ($error == UPLOAD_ERR_OK) {				
										...
										if(move_uploaded_file($tmp_name1, $path1."/".$this->input->get('lastId')."/".$name1)) {	
											$sql1[] = "INSERT INTO file_soi_mikro_final(iuji_mikro_bb, filename, dInsertDate, keterangan,cInsert) 
                ...
                case 'updateproses':
						...
						if (!file_exists( $path1."/".$this->input->post('soi_mikro_bb_final_iuji_mikro_bb') )) { 
							mkdir($path1."/".$this->input->post('soi_mikro_bb_final_iuji_mikro_bb'), 0777, true);					    
						}
						...
						if($isUpload) {
							...
							if (isset($_FILES['fileupload1'])) {
								...
								$this->hapusfile($path1, $file_name1, 'file_soi_mikro_final', $this->input->post('soi_mikro_bb_final_iuji_mikro_bb'));
								foreach ($_FILES['fileupload1']["error"] as $key => $error) {	
									if ($error == UPLOAD_ERR_OK) {
										$tmp_name1 = $_FILES['fileupload1']["tmp_name"][$key];
										$name1 = $_FILES['fileupload1']["name"][$key];
										$data1['filename'] = $name1;
										$data1['id']=$this->input->post('soi_mikro_bb_final_iuji_mikro_bb');
										$data1['nip']=$this->user->gNIP;
										$data1['dInsertDate'] = date('Y-m-d H:i:s');
						 				if(move_uploaded_file($tmp_name1, $path1."/".$this->input->post('soi_mikro_bb_final_iuji_mikro_bb')."/".$name1)) 
						 				{
											$sql1[] = "INSERT INTO file_soi_mikro_final(iuji_mikro_bb, filename, dInsertDate, keterangan,cInsert) 
    ...
    function listBox_Action($row, $actions) {
		...
		if (isset($mydept)) {
			if((in_array('QA', $mydept)) ) {
				if ($row->iSubmit_mikro_final<>0) {
					if($this->auth_localnon->is_manager()){
						if ($row->iApprove_mikro_final<>0) {
							unset($actions['edit']);
...
// manipulate object 
    function insertBox_soi_mikro_bb_final_iApprove_mikro_final($field, $id) {
		$o='-';
		$o.='
	...
	function updateBox_soi_mikro_bb_final_iApprove_mikro_final($field, $id, $value, $rowData) {
		//print_r($rowData);
		if(($value <> 0) || (!empty($value))){
			$sql_dtapp = 'select * 
						from plc2.uji_mikro_bb a 
						join hrd.employee b on b.cNip=a.cApproval_mikro_final
						where
						a.lDeleted = 0
			...
			if($value==2){
				...
				$ret= $st.' oleh '.$row['vName'].' pada '.$row['dApproval_mikro_final'].'</br> Alasan: '.$row['vRemark_mikro_final'].'</p>';
			}
			elseif($value==1){
	...
	function insertBox_soi_mikro_bb_final_ireqdet_id($field, $id) {
		$return = '<script>
		...
		$return .= '&nbsp;<button class="icon_pop"  onclick="browse(\''.base_url().'processor/plc/soi/mikro/bb/final/popup/?field=soi_mikro_bb_final\',\'List Bahan Baku\')" type="button">&nbsp;</button>';                
		
		return $return;
	}

	function updateBox_soi_mikro_bb_final_ireqdet_id($field, $id, $value, $rowData) {
		$sql = '
	...
	function insertBox_soi_mikro_bb_final_no_po($field, $id) {
		$return = '<input type="text" name="'.$field.'"  id="'.$id.'"  disabled="disabled" class="input_rows1 required" size="8" />';
		return $return;
	}
	//,'no_po','manuf',''
	function updateBox_soi_mikro_bb_final_no_po($field, $id, $value, $rowData) {
		$sql = '
		select a.ireqdet_id,b.vreq_nomor,e.vpo_nomor,f.vro_nomor,g.vnama,a.ijumlah,h.vupb_nomor,h.vupb_nama,a.* ,d.*
		from plc2.plc2_upb_request_sample_detail a
		join plc2.plc2_upb_request_sample b on b.ireq_id=a.ireq_id
	...
	function insertBox_soi_mikro_bb_final_no_upb($field, $id) {
		$return = '<input type="text" name="'.$field.'"  id="'.$id.'"  disabled="disabled" class="input_rows1 required" size="30" />';
		return $return;
	}
	//,'no_po','manuf','no_upb'
	function updateBox_soi_mikro_bb_final_no_upb($field, $id, $value, $rowData) {
		$sql = '
		select a.ireqdet_id,b.vreq_nomor,e.vpo_nomor,f.vro_nomor,g.vnama,a.ijumlah,h.vupb_nomor,h.vupb_nama,a.* ,d.*
		from plc2.plc2_upb_request_sample_detail a
		join plc2.plc2_upb_request_sample b on b.ireq_id=a.ireq_id
	...
	function insertBox_soi_mikro_bb_final_iJumlah_terima($field, $id) {
		$return = '<input type="text" name="'.$field.'"  id="'.$id.'" class=" angka input_rows1 required" size="8" />
		<input type="hidden" name="isdraft" id="isdraft">';
		return $return;
	}

	function updateBox_soi_mikro_bb_final_iJumlah_terima($field, $id, $value, $rowData) {
		if ($this->input->get('action') == 'view') {
			$return= $value;
	...
	function insertBox_soi_mikro_bb_final_dMulai_literatur($field, $id) {
		$return = '<input type="text" name="'.$field.'"  id="'.$id.'" class=" tanggal input_rows1 " size="8" />';
		return $return;
	}

	function updateBox_soi_mikro_bb_final_dMulai_literatur($field, $id, $value, $rowData) {
		if ($this->input->get('action') == 'view') {
			$return= $value;
	...
	function insertBox_soi_mikro_bb_final_dSelesai_literatur($field, $id) {
		$return = '<input type="text" name="'.$field.'"  id="'.$id.'" class=" tanggal input_rows1 " size="8" />';
		return $return;
	}

	function updateBox_soi_mikro_bb_final_dSelesai_literatur($field, $id, $value, $rowData) {
		if ($this->input->get('action') == 'view') {
			$return= $value;
	...
	function insertBox_soi_mikro_bb_final_dMulai_uji($field, $id) {
		$return = '<input type="text" name="'.$field.'"  id="'.$id.'" class=" tanggal input_rows1 required" size="8" />';
		return $return;
	}

	function updateBox_soi_mikro_bb_final_dMulai_uji($field, $id, $value, $rowData) {
		if ($this->input->get('action') == 'view') {
			$return= $value;
	...
	function insertBox_soi_mikro_bb_final_dSelesai_uji($field, $id) {
		$return = '<input type="text" name="'.$field.'"  id="'.$id.'" class=" tanggal input_rows1 required" size="8" />';
		return $return;
	}

	function updateBox_soi_mikro_bb_final_dSelesai_uji($field, $id, $value, $rowData) {
		if ($this->input->get('action') == 'view') {
			$return= $value;
	...
	function insertBox_soi_mikro_bb_final_cPic_uji($field, $id) {
		$return = '<script>
						$( "button.icon_pop" ).button({
							icons: {
								primary: "ui-icon-newwin"
		...
		$return .= '&nbsp;<button class="icon_pop"  onclick="browse(\''.base_url().'processor/plc/browse/pic/?field=soi_mikro_bb_final&pic_for=soi_mikro_bb_final&idfield='.$field.'\',\'List PIC\')" type="button">&nbsp;</button>';                
		
		return $return;
	}

	function updateBox_soi_mikro_bb_final_cPic_uji($field, $id, $value, $rowData) {
	...
	function updateBox_soi_mikro_bb_final_iHasil_uji($field, $id, $value, $rowData) {
	...
	function updateBox_soi_mikro_bb_final_iUji_spesifik($field, $id, $value, $rowData) {
		
		if ($this->input->get('action') == 'view') {
			$return=$value;

		}else{
			$data=array(''=>'Select One','0'=>'Tidak','1'=>'Ya');
			$return=$data[$value];			
			$return .= '<input type="hidden" name="'.$field.'" id="'.$id.'" class="input_rows1 required" value="'.$value.'" />';
		}
	...
	function updateBox_soi_mikro_bb_final_soi_mikro_final($field, $id, $value, $rowData) {
		if (!empty($_GET['idnya'])) {
		...
		$data['rows'] = $this->db_plc0->get_where('plc2.file_soi_mikro_final', array('iuji_mikro_bb'=>$iuji_mikro_bb))->result_array();
		return $this->load->view('lokal/soi_mikro_bb_final_file',$data,TRUE);				
	}	

	function updateBox_soi_mikro_bb_final_dMulai_draft_soi($field, $id, $value, $rowData) {
		if ($this->input->get('action') == 'view') {
			$return= $value;
			...
			$return= $value;
			$return .= '<input type="hidden" name="'.$field.'" id="'.$id.'" class="input_rows1 " value="'.$value.'" />';
		}
		return $return;
	}

	function updateBox_soi_mikro_bb_final_dSelesai_draft_soi($field, $id, $value, $rowData) {
		if ($this->input->get('action') == 'view') {
			$return= $value;
	...
	function updateBox_soi_mikro_bb_final_cPic_draft_soi($field, $id, $value, $rowData) {
	...
	function updateBox_soi_mikro_bb_final_cPic_mikro_final($field, $id, $value, $rowData) {
		$emp = $this->db_plc0->get_where('hrd.employee', array('cNip'=>$rowData['cPic_mikro_final']))->row_array();
		if ($this->input->get('action') == 'view') {
			$return= $emp['cNip'].' - '.$emp['vName'];

		}else{
			
			if ($rowData['iSubmit_mikro_final'] == 0) {
				$return = '<script>
							$( "button.icon_pop" ).button({
								icons: {
									primary: "ui-icon-newwin"
			...
			$return .= '&nbsp;<button class="icon_pop"  onclick="browse(\''.base_url().'processor/plc/browse/pic/?field=soi_mikro_bb_final&pic_for=draft_soi_mikro_bb&idfield='.$field.'\',\'List PIC\')" type="button">&nbsp;</button>';                
...
// end manipulate     
	...
	function manipulate_insert_button($buttons) {
		...
		$save_draft = '<button onclick="javascript:save_draft_btn_multiupload(\'soi_mikro_bb_final\', \''.base_url().'processor/plc/soi/mikro/bb/final?draft=true\', this, true)" class="ui-button-text icon-save" id="button_save_draft_soi_mikro_bb_final">Save as Draft</button>';
		$save = '<button onclick="javascript:save_btn_multiupload(\'soi_mikro_bb_final\', \''.base_url().'processor/plc/soi/mikro/bb/final?company_id='.$this->input->get('company_id').'&group_id='.$this->input->get('group_id').'&modul_id='.$this->input->get('modul_id').'\', this)" class="ui-button-text icon-save" id="button_save_soi_mikro_bb_final">Save &amp; Submit</button>';
		$js = $this->load->view('lokal/soi_mikro_bb_final_js');
	...
	function manipulate_update_button($buttons, $rowData) {
		...
		$approve = '<button onclick="javascript:load_popup(\''.base_url().'processor/plc/soi/mikro/bb/final?action=approve&iuji_mikro_bb='.$rowData['iuji_mikro_bb'].'&cNip='.$cNip.'&lvl=2&status=1&group_id='.$this->input->get('group_id').'&modul_id='.$this->input->get('modul_id').'\')" class="ui-button-text icon-save" id="button_approve_soi_mikro_bb_final">Confirm</button>';
		$update = '<button onclick="javascript:update_btn_back(\'soi_mikro_bb_final\', \''.base_url().'processor/plc/soi/mikro/bb/final?company_id='.$this->input->get('company_id').'&group_id='.$this->input->get('group_id').'&modul_id='.$this->input->get('modul_id').'\', this)" class="ui-button-text icon-save" id="button_save_soi_mikro_bb_final">Update & Submit</button>';
		$updatedraft = '<button onclick="javascript:update_draft_btn(\'soi_mikro_bb_final\', \''.base_url().'processor/plc/soi/mikro/bb/final?company_id='.$this->input->get('company_id').'&draft=true&group_id='.$this->input->get('group_id').'&modul_id='.$this->input->get('modul_id').'\', this, true)" class="ui-button-text icon-save" id="button_save_soi_mikro_bb_final">Update as Draft</button>';
		...
		if ($this->input->get('action') == 'view') {unset($buttons['update']);}
			...
			if ($rowData['iSubmit_mikro_final']== 0) {
				...
				if ($rowData['iApprove_mikro_final']==0) {
	...
	function approve_view() {
					 ...
					 function submit_ajax(form_id) {
						return $.ajax({
					 	 	...
					 	 	success: function(data) {
								...
								var url = "'.base_url().'processor/plc/soi/mikro/bb/final";								
								if(o.status == true) {
									
									$("#alert_dialog_form").dialog("close");
										 $.get(url+"?action=update&id="+last_id, function(data) {
										 $("div#form_soi_mikro_bb_final").html(data);
										 $("#button_approve_soi_mikro_bb_final").hide();
										 $("#button_reject_soi_mikro_bb_final").hide();
									});
									
								}
									reload_grid("grid_soi_mikro_bb_final");
							}
		...
		$echo .= '<form id="form_soi_mikro_bb_final_approve" action="'.base_url().'processor/plc/soi/mikro/bb/final?action=approve_process" method="post">';
		...
		$echo .= 'Remark : 
				...
				<textarea name="vRemark_mikro_final"></textarea>
		<button type="button" onclick="submit_ajax(\'form_soi_mikro_bb_final_approve\')">Confirm</button>';
	...
	function approve_process() {
		...
		$lvl = $post['lvl'];
		$vRemark_mikro_final = $post['vRemark_mikro_final'];

		
		$data=array('iApprove_mikro_final'=>'2','cApproval_mikro_final'=>$cNip , 'dApproval_mikro_final'=>date('Y-m-d H:i:s'), 'vRemark_mikro_final'=>$vRemark_mikro_final);
...
*/
	...
	function before_update_processor($row, $postData) {
		...
		else{$postData['iSubmit_mikro_final']=1;} 
	...
	function download($filename) {
		...
		$id = $_GET['id'];
		$tempat = $_GET['path'];
		$path = file_get_contents('./files/plc/local/soi_mikro_bb_final/'.$tempat.'/'.$id.'/'.$name);	
