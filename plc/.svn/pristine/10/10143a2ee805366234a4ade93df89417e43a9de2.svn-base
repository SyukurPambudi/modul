...
class v3_soi_fg extends MX_Controller {
    function __construct() {
		...
		$this->maintable = 'plc2.soi_fg';	
		$this->main_table = $this->maintable;	
		$this->main_table_pk = 'isoifg_id';	
		$datagrid['islist'] = array(
			...
			,'vkode_surat' => array('label'=>'No. Draft','width'=>250,'align'=>'left','search'=>true)
			// ,'itype' => array('label'=>'Status','width'=>100,'align'=>'left','search'=>true)
            ,'ineed_revisi' => array('label'=>'Butuh Revisi','width'=>50,'align'=>'center','search'=>false)
            ,'irevisi' => array('label'=>'Revisi','width'=>50,'align'=>'center','search'=>true)
			...
			,'iappqc' => array('label'=>'Approval QC','width'=>150,'align'=>'left','search'=>true)
            ,'iappqa' => array('label'=>'Approval QA','width'=>150,'align'=>'left','search'=>true)
		);		

		$datagrid['setQuery']=array(
								0=>array('vall'=>'soi_fg.ldeleted','nilai'=>0)
								);
		$datagrid['jointableinner']=array(
									0=>array('plc2.plc2_upb'=>'plc2_upb.iupb_id=soi_fg.iupb_id')
								);
	...
	/*List Box*/
	function listBox_v3_soi_fg_iappqa_uji($value) {
    ...
    function searchBox_v3_soi_fg_iappqc($fields, $id) {
    ...
    function searchBox_v3_soi_fg_iappqa($fields, $id) {
        $arrStat    = array(0 => 'Waiting For Approval', 1 => 'Rejected', 2 => 'Approved');
        $echo       = '<select class="combobox" id="'.$id.'">';
        $echo      .= '<option value="">-- All Status --</option>';
    ...
    function listBox_v3_soi_fg_iappqc($value, $pk, $name, $rowData) {
        if ($value == 0){
    ...
    function listBox_v3_soi_fg_iappqa($value, $pk, $name, $rowData) {
        if ($value == 0){
            $vstatus = (intval($rowData->ineed_revisi) == 1)?'-':'Waiting For Approval';
        } else if($value == 1){
            $vstatus = 'Rejected';
	...
	function listBox_v3_soi_fg_ini_nilai($value, $pk, $name, $rowData) {
	...
	function insertBox_v3_soi_fg_form_detail($field,$id){
            $get=$this->input->get();
            $post=$this->input->post();
        ...
        function updateBox_v3_soi_fg_form_detail($field,$id,$value,$rowData){
            $get=$this->input->get();
            $post=$this->input->post();
            foreach ($get as $kget => $vget) {
                if($kget!="action"){
    ...
    function after_insert_processor($fields, $id, $postData) {
        ...
        if($postData['isubmit']){
            ...
            //fungsi untuk auto insert ke soi fg tentative ketika butuh revisi
            $ineed_revisi = $postData['ineed_revisi'];
            if ($ineed_revisi > 0){
                $draft['iupb_id'] = $postData['iupb_id'];
                $draft['irevisi'] = intval($postData['irevisi']) + 1;
                $draft['vkode_surat'] = $postData['vkode_surat'].' -- REVISI SOI';
                $draft['isubmit'] = 0;
                $draft['ldeleted'] = 0;
                $draft['iappqa'] = 0;
                $this->db->insert('plc2.plc2_upb_soi_fg', $draft);
            }
            //close fungsi untuk auto insert ke soi fg tentative ketika butuh revisi
    ...
    function approve_view($id=0) { 
     ...
     function approve_process() {
        ...
        $update = array($field => $iapprove);
        $this->db->where($this->main_table_pk, $pk);
        $run = $this->db->update($this->maintable, $update);
        ...
        if($iapprove==1){
            $this->db->select("*")
                            ...
                            ->where('idprivi_modules',$modul_id)
                            ->where('iApprove',1)
                            ->where('lDeleted',0);
            $row=$this->db->get()->row_array();

            if(isset($row['iM_modul_log_activity'])){
                /* Update Deleted=1 Untuk Yang baru di Insert */
                /* $dataupdate['lDeleted']=1;
                $this->db->where('iM_modul_log_activity',$row['iM_modul_log_activity']);
                $this->db->update("plc3.m_modul_log_activity",$dataupdate); */
            }
            ...
            //                         AND ac.vDept_assigned = "PD" AND mo.idprivi_modules = '.$modul_id;
            // $datarow=$this->db->query($sqlupdateReject)->result_array();

            // foreach ($datarow as $krow => $vrow) {
            //     $this->db->where("iM_modul_log_activity",$vrow['iM_modul_log_activity']);
            ...
            foreach ($sqlAct as $act) {
                ...
                if (strlen($vFieldName) > 0){
                    $this->db->where($this->main_table_pk, $pk);
                    $this->db->update($this->maintable, array($vFieldName => 0));
                // } else {
                //     $this->db->where($this->main_table_pk, $pk);
            ...
            foreach ($dFields as $form_field) {
                ...
                if($get['formaction']=='update'){
                    ...
                    if($form_field['iM_jenis_field'] == 6){
                        ...
                        $sqlEdit = str_replace('$filter$', $this->lib_plc->queryFilterUPBbyTeam($this->modul_id,'a'), $sqlEdit);
                        $union = 'SELECT u.iupb_id AS valval, CONCAT(u.vupb_nomor," | ",u.vupb_nama) AS showshow FROM plc2.plc2_upb u WHERE u.iupb_id = '.$dataHead['iupb_id'].' UNION ';
                        $sqlEdit = $union.$sqlEdit;
                    ...
                    if ($form_field['iM_jenis_field'] == 19){
                        ...
                        $sqlQuery = str_replace('$filter$', $this->lib_plc->queryFilterUPBbyTeam($this->modul_id,'a'), $sqlQuery);
                    }
