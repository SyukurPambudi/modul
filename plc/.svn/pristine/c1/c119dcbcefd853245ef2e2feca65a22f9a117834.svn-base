		...
		custom_confirm(comfirm_message,function(){
			if(isUpload && !isValidAFileSize('#form_create_'+grid+' input.multifile', uploadLimit)) {
				...
				success: function(data) {
					...
					var header = 'Error';
							if(isUpload) {
								var iframe = $('<iframe name="spek_fg_frame"/>');
								iframe.attr({'id':'spek_fg_frame'});
								...
								$('#form_create_'+grid).attr('target','spek_fg_frame');								
			...
			/*.done(function(data) {
					...
					info = 'info';
						...
						$.get(base_url+'processor/plc/spesifikasi/fg/tentative?action=update&foreign_key='+foreign_id+'&company_id='+company_id+'&id='+last_id+'&group_id='+group_id+'&modul_id='+modul_id, function(data) {
			...
			if(isUpload && !isValidAFileSize('#form_update_'+grid+' input.multifile', uploadLimit)) {
				...
				success: function(data) {
					...
					if(isUpload) {
						...
						$('#form_update_'+grid).attr('target','spek_fg_frame');								
...
<iframe name="spek_fg_frame" id="spek_fg_frame" height="0" width="0"></iframe>