	...
	*/
	...
	if (isset($act)){
		if ($act=='update' || $act == 'view') {
			...
			$sql = "SELECT d.ireqdet_id, d.raw_id, d.ijumlah, d.vsatuan, d.tspecification, s.vNmSatuan, d.plc2_master_satuan_id FROM plc2.plc2_upb_request_sample_detail d 
					LEFT JOIN plc2.plc2_master_satuan s ON d.plc2_master_satuan_id = s.plc2_master_satuan_id
					WHERE d.ldeleted = 0 AND d.ireq_id = ? AND ( s.ldeleted IN (0, NULL) ) ORDER BY d.ireqdet_id ASC";
			$rows = $this->db->query($sql, array($ireq_id))->result_array();
		}
	}
...
<script type="text/javascript">
	function browse_multi1_sample_permintaan_sample(url, title, dis, param) {
		var i = $('.btn_browse_bb permintaan_sample_edit').index(dis);	
...
<div class="v3_table_permintaan_sample_detail">
	<table class="hover_table" id="sample_permintaan_sample" cellspacing="0" cellpadding="1" style="width: 98%; border: 1px solid #dddddd; text-align: center; margin-left: 5px; border-collapse: collapse">
		...
		<tbody>
			<?php
				$browse_url = base_url().'processor/plc/browse/bb';
				$i = 0;
				if(!empty($rows)) {
					foreach($rows as $row) {
					$i++;				
			...
			?>
					<tr style="border: 1px solid #dddddd; border-collapse: collapse; background: #ffffff; ">
						<td style="border: 1px solid #dddddd; width: 3%; text-align: center;">
							<span class="sample_permintaan_sample_num"><?php echo $i ?></span>
						</td>		
						<td style="border: 1px solid #dddddd; width: 47%">
							<input type="hidden" name="ireqdet_id[]" value="<?php echo $row['ireqdet_id'] ?>" class="ireqdet_id" />
							...
							<button class="btn_browse_bb permintaan_sample_edit" type="button" onClick="javascript:get_rawmaterial_exists();javascript:browse_multi1_sample_permintaan_sample('<?php echo $browse_url ?>?field=sample_permintaan_sample&col=detraw_id','List Bahan Baku',this,'irawmat_id='+$('.list_raw_material_exists').val());return false;">...</button>
						...
						<td style="border: 1px solid #dddddd; width: 15%">
							<!-- <input type="text" name="detsatuan[]" value="<?php echo $row['vsatuan'] ?>"  class="detsatuan required" style="width: 100%" /> -->
						...
						<td style="border: 1px solid #dddddd; width: 50%">
							<input type="text" name="detspesifikasi[]" value="<?php echo $row['tspecification'] ?>" class="detspesifikasi" style="width: 100%" />
						</td>
						<td style="border: 1px solid #dddddd; width: 10%">
							<span class="permintaan_sample_detail_delete_btn permintaan_sample_edit"><a href="javascript:;" class="sample_permintaan_sample_del" onclick="del_row(this, 'sample_permintaan_sample_del')">[Hapus]</a></span>
						</td>		
					</tr>
			<?php
					}
			...
			<tr style="border: 1px solid #dddddd; border-collapse: collapse; background: #ffffff; ">
				<td style="border: 1px solid #dddddd; width: 3%; text-align: center;">
					<span class="sample_permintaan_sample_num">1</span>
				</td>		
				<td style="border: 1px solid #dddddd; width: 47%">
					<input type="hidden" name="ireqdet_id[]" value="" class="ireqdet_id" />
					...
					<!-- <button class="btn_browse_bb permintaan_sample_edit" type="button" onclick="javascript:browse_multiple('<?php //echo $browse_url ?>?field=sample_permintaan_sample&col=detraw_id','List Bahan Baku',this,'btn_browse_bb permintaan_sample_edit')">...</button> -->
					<button class="btn_browse_bb permintaan_sample_edit" type="button" onClick="javascript:get_rawmaterial_exists();javascript:browse_multi1_sample_permintaan_sample('<?php echo $browse_url ?>?upbid='+$('#sample_permintaan_sample_iupb_id').val()+'&field=sample_permintaan_sample&col=detraw_id','List Bahan Baku',this,'irawmat_id='+$('.list_raw_material_exists').val());return false;">...</button>
					<input type="hidden" name="list_raw_material_exists" class="list_raw_material_exists"/>
				</td>	
				<td style="border: 1px solid #dddddd; width: 10%">
				...
				<td style="border: 1px solid #dddddd; width: 15%">
					<!-- <input type="text" name="detsatuan[]" value="" class="detsatuan required" style="width: 100%" /> -->
				...
				<td style="border: 1px solid #dddddd; width: 10%">
					<span class="permintaan_sample_detail_delete_btn permintaan_sample_edit"><a href="javascript:;" class="sample_permintaan_sample_del" onclick="del_row(this, 'sample_permintaan_sample_del')">[Delete]</a></span>
				</td>		
			</tr>
			<?php } ?>
		</tbody>
		<tfoot class="permintaan_sample_detail_add_row permintaan_sample_edit">
			<tr>
