			...
			if(isUpload && !isValidFileSize('#form_create_'+grid+' input.multifile', uploadLimit)) {
				...
				$.ajax({
					...
					success: function(data) {	
						...
						if(o.status == true) {
							if(isUpload) {
								var iframe = $('<iframe name="spesifikasi_fgt_frame"/>');
								iframe.attr({'id':'spesifikasi_fgt_frame'});
								...
								$('#form_create_'+grid).attr('target','spesifikasi_fgt_frame');								
							...
							//alert(url)
							$.get(base_url+'processor/plc/spesifikasi/fg/tentatif?action=create&foreign_key=&company_id='+last_id+'&group_id='+group_id+'&modul_id='+modul_id, function(data) {
					...
					success: function(data) {	
						...
						if(o.status == true) {
							if(isUpload) {
								...
								$('#form_update_'+grid).attr('target','spesifikasi_fgt_frame');								
							...
							//alert(url)
							$.get(base_url+'processor/plc/spesifikasi/fgt/frame?action=update&foreign_key=&company_id='+last_id+'&group_id='+group_id+'&modul_id='+modul_id, function(data) {
	                            $('div#grid_wraper_'+grid).html(data);
	                            $('html, body').animate({scrollTop:$('#'+grid).offset().top - 20}, 'slow');
	                    	});
						}
...
<iframe name="spesifikasi_fgt_frame" id="spesifikasi_fgt_frame" height="0" width="0"></iframe>
