...
$("#<?php echo $tableId;?> .file_remove").click(function(){
	...
	var tmpDel = $("#valpro_del");
	li.remove();
	var v = tmpDel.val();
	v+=','+fileid;
	tmpDel.val( v );
	alert( $("#valpro_del").val() );
});
function del_valpro(dis, conname, table_id) {
	//if($('.'+conname).length > 1) {
		custom_confirm('Delete Selected Record?', function(){
			...
			if($('.'+conname).length==0) {
				...
				var n = $("span."+table_id+"_num:last").text();
					...
					row_content	 += '<input type="file" class="fileupload multi multifile" name="fileupload5[]" style="width: 70%" />*max 5 mb';
					row_content  += '<input type="hidden" name="namafile5[]" style="width: 70%" value="" />';
					row_content  += '<input type="hidden" name="fileid5[]" style="width: 70%" value="" /></td>';	
					row_content	 += '<td colspan="2" style="border: 1px solid #dddddd; width: 8%">';
					row_content	 += '<textarea class="" name="fileketerangan5[]" style="width: 240px; height: 50px;"></textarea></td>';
					row_content	 += '<td style="border: 1px solid #dddddd; width: 10%">';
					row_content	 += '<span class="delete_btn"><a href="javascript:;" class="valpro_del" onclick="del_valpro(this, \'valpro_del\',\'valpro\')">[Hapus]</a></span></td>';		
					row_content  += '</tr>';
...
function add_row_valpro(table_id){		
		//alert(table_id);
		var row = $('table#'+table_id+' tbody tr:last').clone();
		$("span."+table_id+"_num:first").text('1');
		var n = $("span."+table_id+"_num:last").text();
...
<table class="hover_table" id="valpro" cellspacing="0" cellpadding="1" style="width: 98%; border: 1px solid #dddddd; text-align: center; margin-left: 5px; border-collapse: collapse">
	<thead>
	...
	<tbody>
		...
		?>
				<tr style="border: 1px solid #dddddd; border-collapse: collapse; background: #ffffff; ">
					<td style="border: 1px solid #dddddd; width: 3%; text-align: center;">
						<span class="valpro_num"><?php echo $i ?></span>
					</td>							
					<td colspan="2" style="border: 1px solid #dddddd; width: 27%">
						<?php echo $row['filename'] ?> 
						<input type="hidden" name="namafile5[]" style="width: 70%" value="<?php echo $row['filename'] ?>" />
						<input type="hidden" name="fileid5[]" style="width: 70%" value="<?php echo $row['id'] ?>" />
					</td>	
					<td colspan="2" style="border: 1px solid #dddddd; width: 8%">
						<?php echo $row['keterangan'] ?>
					</td>
					<td style="border: 1px solid #dddddd; width: 10%">
						<span class="delete_btn"><a href="javascript:;" class="valpro_del" onclick="del_valpro(this,'valpro_del','valpro')">[Hapus]</a></span>
		...
		?>
				<tr style="border: 1px solid #dddddd; border-collapse: collapse; background: #ffffff; ">
					<td style="border: 1px solid #dddddd; width: 3%; text-align: center;">
						<span class="valpro_num">1</span>
					</td>		
					<td colspan="2" style="border: 1px solid #dddddd; width: 27%">
						<input type="file" class="fileupload multi multifile" name="fileupload5[]" style="width: 70%" /> *max 5 mb
						<input type="hidden" name="namafile5[]" style="width: 70%" value="" />
						<input type="hidden" name="fileid5[]" style="width: 70%" value="" />
						</td>	
					<td colspan="2" style="border: 1px solid #dddddd; width: 15%">
						<textarea class="" name="fileketerangan5[]" style="width: 240px; height: 50px;"></textarea>
					</td>
					<td style="border: 1px solid #dddddd; width: 10%">
						<span class="delete_btn"><a href="javascript:;" class="valpro_del" onclick="del_valpro(this,'valpro_del','valpro')">[Hapus]</a></span>
					</td>	
	...
	<tfoot>
		<tr>
			<td colspan="5"></td><td style="text-align: center"><a href="javascript:;" onclick="javascript:add_row_valpro('valpro')">Tambah</a></td>
		</tr>
	</tfoot>
</table>