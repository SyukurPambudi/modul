			...
			if(isUpload && !isValidAFileSize('#form_update_'+grid+' input.multifile', uploadLimit)) {
				...
				success: function(data) {
					...
					if(o.status == true){
						if(isUpload > 0) {				
							//alert('tes');								
							var iframe = $('<iframe name="applet_frame"/>');
							iframe.attr({'id':'applet_frame'});
							...
							$('#form_update_'+grid).attr('target','applet_frame');
						...
						$.get(base_url+'processor/plc/applet?action=update&id='+o.last_id+'&foreign_key='+o.foreign_id+'&company_id='+o.company_id+'&group_id='+o.group_id+'&modul_id='+o.modul_id, function(data) {
				...
				.done(function(data) {
					...
					$('#grid_'+grid).trigger('reloadGrid');
						...
						/*$.get(base_url+'processor/plc/applet?action=update&id='+last_id+'&foreign_key='+foreign_id+'&company_id='+company_id+'&group_id='+group_id+'&modul_id='+modul_id, function(data) {
	                            $('div#form_'+grid).html(data);
	                            $('html, body').animate({scrollTop:$('#'+grid).offset().top - 20}, 'slow');
	                    });*/
				});
				...
				success: function(data) {
					...
					if(o.status == true){
						if(isUpload > 0) {				
							...
							uploadfile('form_update_'+grid, grid, formAction, base_url+'processor/plc/applet?action=update&id='+o.last_id+'&foreign_key='+o.foreign_id+'&company_id='+o.company_id+'&group_id='+o.group_id+'&modul_id='+o.modul_id);
	...
	custom_confirm('Anda Yakin Confirm UPB : '+upbno,function(){
		$.ajax({
			...
			success: function(data) {
				...
				$.get(base_url+'processor/plc/applet?action=view&id='+o.last_id+'&foreign_key='+o.foreign_id+'&company_id='+o.company_id+'&group_id='+o.group_id+'&modul_id='+o.modul_id, function(data) {
                        $('div#form_'+grid).html(data);
                        $('html, body').animate({scrollTop:$('#'+grid).offset().top - 20}, 'slow');
                });
			}		
...
<iframe name="applet_frame" id="applet_frame" height="0" width="0"></iframe>
