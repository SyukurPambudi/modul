...
$("#<?php echo $tableId;?> .file_remove").click(function(){
	...
	var tmpDel = $("#draft_soi_mikro_del");
	li.remove();
	var v = tmpDel.val();
	v+=','+fileid;
	tmpDel.val( v );
	alert( $("#draft_soi_mikro_del").val() );
});

function del_upload_draft_soi_mikro(dis, conname, table_id) {
	if($('.'+conname).length > 1) {
		...
		var n = $("span."+table_id+"_num:last").text();
			...
			row_content	 += '<span class="delete_btn"><a href="javascript:;" class="draft_soi_mikro_del" onclick="del_upload_draft_soi_mikro(this, \'draft_soi_mikro_del\', \'draft_soi_mikro\')">[Hapus]</a></span></td>';		
			row_content  += '</tr>';
...
<table class="hover_table" id="draft_soi_mikro" cellspacing="0" cellpadding="1" style="width: 98%; border: 1px solid #dddddd; text-align: center; margin-left: 5px; border-collapse: collapse">
	<thead>
	...
	<tbody>
		...
		if(in_array('QA', $mydept)) {
			...
			$value = $row['filename'];
			$path = "rpt_draft_soi_mikro";	
			if($value != '') {
				if (file_exists('./files/plc/local/draft_soi_mikro_bb/rpt_draft_soi_mikro/'.$iuji_mikro_bb.'/'.$value)) {
					$link = base_url().'processor/plc/draft/soi/mikro/bb?action=download&id='.$iuji_mikro_bb.'&path='.$path.'&file='.$value;
		...
		?>
				<tr style="border: 1px solid #dddddd; border-collapse: collapse; background: #ffffff; ">
					<td style="border: 1px solid #dddddd; width: 3%; text-align: center;">
						<span class="draft_soi_mikro_num"><?php echo $i ?></span>
					</td>		
					...
					<td style="border: 1px solid #dddddd; width: 10%">
						<span class="delete_btn"><a href="javascript:;" class="draft_soi_mikro_del" onclick="del_upload_draft_soi_mikro(this, 'draft_soi_mikro_del', 'draft_soi_mikro')">[Hapus]</a></span>
		...
		<tr style="border: 1px solid #dddddd; border-collapse: collapse; background: #ffffff; ">
					<td style="border: 1px solid #dddddd; width: 3%; text-align: center;">
						<span class="draft_soi_mikro_num">1</span>
					</td>		
					<td colspan="2" style="border: 1px solid #dddddd; width: 27%">
	...
	<tfoot>
		<tr>
			<td colspan="5"></td><td style="text-align: center"><a href="javascript:;" onclick="javascript:add_row_upload_kimiawi('draft_soi_mikro')">Tambah</a></td>
		</tr>
	</tfoot>
</table>