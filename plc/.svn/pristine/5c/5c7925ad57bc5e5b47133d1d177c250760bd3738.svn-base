...
class Browse_stabilita_non_upd extends MX_Controller {
...
$this->db_plc0 = $this->load->database('plc0',false, true);
    ...
    function index($action = '') {
		...
		$grid->setUrl('browse_stabilita_non_upd');
		$grid->addList('idossier_review_id','dossier_upd.vUpd_no','dossier_upd.idossier_upd_id','dossier_upd.vNama_usulan','plc2_upb.dosis','mnf_sediaan.vSediaan','plc2_upb.vupb_nomor','pilih');
		...
		$grid->setSearch('dossier_upd.vUpd_no','dossier_upd.vNama_usulan','plc2_upb.dosis','mnf_sediaan.vSediaan');
		...
		$grid->setQuery("dossier_dok_list.idossier_review_id in (select distinct(lis.idossier_review_id) from dossier.dossier_dok_list lis
			...
			and ( dok.vNama_Dokumen='ACC ".$this->input->get("iBulanke")." (Batch ".$this->input->get("iBatch_no").") Non' OR dok.vNama_Dokumen='RT ".$this->input->get("iBulanke")." (Batch ".$this->input->get("iBatch_no").") Non') )",NULL);
		
		$grid->setGroupBy('idossier_upd_id');
		$grid->setQuery('dossier.dossier_kat_dok.idossier_kat_dok_id', 4);
		$grid->setQuery('dossier.dossier_kat_dok.ldeleted',0);
	...
	function listBox_browse_stabilita_non_upd_pilih($value, $pk, $name, $rowData) {
		$url_rincian=base_url()."processor/plc/stabilita/non/terbalik/cek?action=view";
		$url_header=base_url()."processor/plc/stabilita/non/terbalik?action=getdetil";
		$url_realtime=base_url()."processor/plc/stabilita/non/terbalik?action=get_detail_realtime";
		$url_realtime=base_url()."processor/plc/stabilita/non/terbalik?action=refresh_file_realtime";
		$url_accelerate=base_url()."processor/plc/stabilita/non/terbalik?action=refresh_file_accelerate";
		...
		<script type="text/javascript">
			function pilih_upd (id, idupd ,vUpdNo, vNamaUsulan, dosis, vSediaan, vUpbNo){				
				custom_confirm("Yakin ?", function(){
					...
					type: "post",
					data: {
							stabilita_non_terbalik_idossier_review_id: id,
							},
					beforeSend: function(){
		
					},
					success: function(data){
						$("#stabilita_non_terbalik_idossier_review_id").val(id);
						$("#stabilita_non_terbalik_idossier_upd_id").val(idupd);
						$("#stabilita_non_terbalik_vUpd_no").val(vUpdNo);
						$("#stabilita_non_terbalik_vNama_usulan").val(vNamaUsulan);
						$("#stabilita_non_terbalik_dosis").val(dosis);
						$("#stabilita_non_terbalik_vupb_ref").val(vUpbNo);
						$("#stabilita_non_terbalik_vSediaan").val(vSediaan);
