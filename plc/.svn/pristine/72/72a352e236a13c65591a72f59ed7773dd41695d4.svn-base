...
<table class="hover_table" id="registrasi_upb_dokumen" cellspacing="0" cellpadding="1" style="width: 98%; border: 1px solid #dddddd; text-align: center; margin-left: 5px; border-collapse: collapse">
	...
	<tbody>
		<?php
			$i = 0;
			$abb = $this->db_plc0->get_where('plc2.plc2_upb_prareg_dokumen_bb_detail', array('iupb_id'=>$iupb_id))->result_array();
			//print_r($abb);
			foreach($bb as $row) {
					if(search_array($row, $abb)) {
						$checked = 'checked';
						$notes = $this->db_plc0->get_where('plc2.plc2_upb_prareg_dokumen_bb_detail', array('iupb_id'=>$iupb_id, 'idoc_id'=>$row))->row_array();
						$note = $notes['tnote']; 
					}
					else {
						$checked = '';
...
<table class="hover_table" id="registrasi_upb_dokumen" cellspacing="0" cellpadding="1" style="width: 98%; border: 1px solid #dddddd; text-align: center; margin-left: 5px; border-collapse: collapse">
	...
	<tbody>
		<?php
			$i = 0;
			$asm = $this->db_plc0->get_where('plc2.plc2_upb_prareg_dokumen_sm_detail', array('iupb_id'=>$iupb_id))->result_array();
			//print_r($asm);
			foreach($sm as $row) {
				if(search_array($row, $asm)) {
					$checked = 'checked';
					$notes = $this->db_plc0->get_where('plc2.plc2_upb_prareg_dokumen_sm_detail', array('iupb_id'=>$iupb_id, 'idoc_id'=>$row))->row_array();
					$note = $notes['tnote']; 
				}
				else {
					$checked = '';
