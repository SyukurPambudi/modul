...
class v3_validasi_moa extends MX_Controller {
    function __construct() {
		...
		$this->title = 'Validasi MOA FG';
		$this->url = 'v3_validasi_moa';
		$this->urlpath = 'plc/'.str_replace("_","/", $this->url);

		$this->maintable = 'plc2.plc2_vamoa_fg';	
		$this->main_table = $this->maintable;	
		$this->main_table_pk = 'ivalmoa_fg_id';	
		$datagrid['islist'] = array(
			'vnomor_moa_fg' => array('label'=>'Validasi MOA','width'=>75,'align'=>'center','search'=>true)
			...
			,'plc2_upb.vgenerik' => array('label'=>'Nama Generik','width'=>250,'align'=>'left','search'=>true)
			,'plc2_upb.iteampd_id' => array('label'=>'Team PD','width'=>100,'align'=>'left','search'=>true)
			,'cPIC_AD' => array('label'=>'PIC - AD','width'=>250,'align'=>'left','search'=>true)
			,'isubmit' => array('label'=>'Status Submit','width'=>100,'align'=>'left','search'=>false)
			,'iapppd' => array('label'=>'Approval PD','width'=>100,'align'=>'left','search'=>false)
		);		

		$datagrid['setQuery']=array(
								0=>array('vall'=>'plc2_vamoa_fg.lDeleted','nilai'=>0)
								);
		$datagrid['jointableinner']=array(
									0=>array('plc2.plc2_upb'=>'plc2_upb.iupb_id=plc2_vamoa_fg.iupb_id')
								);
    ...
    function index($action = '') {
		...
		$grid->changeFieldType('iappqa', 'combobox','',array(''=>'--select--',0=>'Waiting for approval',1=>'Rejected', 2=>'Approved'));
		...
		switch ($action) {
            ...
            case 'confirm_process':
                ...
                break;
            case 'getDataPengujian':
				echo $this->getDataPengujian();
				break;
             /*Confirm*/
			case 'approve':
            ...
            unset($buttons['update']);
                ...
                if($isOpenEditing){
                    ...
                    foreach ($activities as $act) {
                        ...
                        $reject = '<button onclick="javascript:load_popup(\' '.base_url().'processor/'.$this->urlpath.'?action=reject&iM_modul_activity='.$act['iM_modul_activity'].'&iM_activity='.$act['iM_activity'].'&'.$fieldpeka.'='.$rowData[$fieldpeka].'&iupb_id='.$iupb_id.'&company_id='.$this->input->get('company_id').'&group_id='.$this->input->get('group_id').'&modul_id='.$this->input->get('modul_id').' \' )"  id="button_reject_'.$this->url.'"  class="ui-button-text icon-save" >Reject</button>';
	...
	/*List Box*/
	function listBox_v3_validasi_moa_cPIC_AD($value, $pk, $name, $rowData) {
	...
	function listBox_v3_validasi_moa_plc2_upb_iteampd_id($value, $pk, $name, $rowData) {
	...
	function insertBox_v3_validasi_moa_form_detail($field,$id){
            $get=$this->input->get();
            $post=$this->input->post();
        ...
        function updateBox_v3_validasi_moa_form_detail($field,$id,$value,$rowData){
            $get=$this->input->get();
            $post=$this->input->post();
            foreach ($get as $kget => $vget) {
                if($kget!="action"){
    ...
    function after_insert_processor($fields, $id, $postData) {
		...
		$sql = "UPDATE plc2.plc2_vamoa_fg SET vnomor_moa_fg = '".$nomor."' WHERE ivalmoa_fg_id=".$id." LIMIT 1";
		$query = $this->db->query($sql);

        // foreach ($postData['value_iretest'][0] as $kvv => $vvv) {
        //     $datainsert=array();
        //     $datainsert['iretest']=$vvv;
        //     $datainsert['iplc2_master_jenis_metode']=$postData['iplc2_master_jenis_metode'];
        //     $datainsert['ivalmoa_fg_id']=$id;
        //     $datainsert['dCreate']=date('Y-m-d H:i:s');
        //     $datainsert['cCreate']=$this->user->gNIP;
        //     $this->db_plc0->insert('plc2.plc2_vamoa_fg_metode',$datainsert);
        // }

        $post = $this->input->post();
        $idMetode = $post['iplc2_vamoa_fg_metode'];
    ...
    function after_update_processor($fields, $id, $postData) {
        $dataupdateawal['lDeleted']=1;
        // $this->db_plc0->where('ivalmoa_fg_id',$id);
        // $this->db_plc0->update('plc2.plc2_vamoa_fg_metode',$dataupdateawal);
        // foreach ($postData['value_iretest'] as $kvv => $vvv) {
        //     $arrayupdate=array();
        //     $arrayinsert=array();
        //     if($kvv!=0){
        //         foreach ($vvv as $kdet => $vdet){
        //             $arrayupdate['lDeleted']=0;
        //             $arrayupdate['iretest']=$vdet;
        //             $arrayupdate['iplc2_master_jenis_metode']=$postData['iplc2_master_jenis_metode'];
        //             $arrayupdate['dupdate']=date('Y-m-d H:i:s');
        //             $arrayupdate['cUpdate']=$this->user->gNIP;
        //             $this->db_plc0->where('iplc2_vamoa_fg_metode',$kvv)
        //                             ->update('plc2.plc2_vamoa_fg_metode',$arrayupdate);

        //         }
        //     }else{
        //         foreach ($vvv as $kdet => $vdet){
        //             $arrayinsert['iretest']=$vdet;
        //             $arrayinsert['iplc2_master_jenis_metode']=$postData['iplc2_master_jenis_metode'];
        //             $arrayinsert['ivalmoa_fg_id']=$id;
        //             $arrayinsert['dCreate']=date('Y-m-d H:i:s');
        //             $arrayinsert['cCreate']=$this->user->gNIP;
        //             $this->db_plc0->insert('plc2.plc2_vamoa_fg_metode',$arrayinsert);
        //         }
        //     }
        // }
        ...
        $eRows = $this->db->get_where('plc2.plc2_vamoa_fg_metode', array('ivalmoa_fg_id'=>$id, 'lDeleted'=>0))->result_array();
        foreach($eRows as $k => $v) {
            if(in_array($v['iplc2_vamoa_fg_metode'], $idMetode)) {
                $this->db->where('iplc2_vamoa_fg_metode', $v['iplc2_vamoa_fg_metode']);
                $key = array_search($v['iplc2_vamoa_fg_metode'], $idMetode);
                $this->db->update('plc2.plc2_vamoa_fg_metode', array('iplc2_master_jenis_metode'=>$jenisMetode[$key],'iretest'=>$reTest[$key],'ijumlah_val'=>$jmlValidasi[$key]));
            }
            else {
                $this->db->where('iplc2_vamoa_fg_metode', $v['iplc2_vamoa_fg_metode']);
                $this->db->update('plc2.plc2_vamoa_fg_metode', array('lDeleted'=>1));
            }
        }
        foreach($idMetode as $k => $v) {
            if(empty($v)) {
    ...
    function getFormDetail(){
        ...
        if($get['formaction']=='update' || $get['action']=='update'){
                $aidi = $get['id'];
        }else{
                $aidi = 0;
        }
    ...
    function getDataPengujian(){
		...
		$where=array('lDeleted'=>0,'ivalmoa_fg_id'=>$post["id"]);
		$this->db_plc0->where($where);
		$q=$this->db_plc0->get('plc2.plc2_vamoa_fg_metode');
		$rsel=array("iretest",'iact');
		$data = new StdClass;
		...
		foreach ($q->result() as $k) {
			$data->rows[$i]['id']=$i;
			$z=0;
			$linknya='<a class="ui-button-text" href="javascript:;" onclick="javascript:hapus_row_'.$nmTable.'('.$i.')">[Hapus]</a>';
			foreach ($rsel as $dsel => $vsel) {
				if($vsel=="iact"){
					$dataar[$dsel]=$linknya;
				}elseif($vsel=="iretest"){
					$dataar[$dsel]="<input type='hidden' class='num_rows_".$nmTable ."' value='".$i."' /><input type='text' name='value_iretest[".$k->iplc2_vamoa_fg_metode."][]' id='value_iretest_".$i."' class='numbersOnly required' value ='".$k->iretest."' />";
				}else{
					$dataar[$dsel]=$k->{$vsel};
				}
				$z++;
