...
class setup_modul extends MX_Controller {
    function __construct() {
        parent::__construct();
        $this->db_schedulercheck = $this->load->database('plc0',false, true);
    ...
    function index($action = '') {
        ...
        //List field
        $grid->addFields('vKode_modul','iM_flow','vNama_modul','idprivi_modules','vCodeModule','iType_modul','vDesciption','iM_notif','vSql_notif_custom','vContent_notif_custom','iMutu','vTable_name','vFieldPK','vDept_author','vNip_author','vDept_participant','vNip_participant','detail_setup'); 
        ...
        */
        ...
        $grid->setLabel('iMutu','Ulang karena Mutu ?');
        $grid->setLabel('iM_notif','Notifkasi Message');
        $grid->setLabel('vSql_notif_custom','Notifkasi Sql Get Content');
        $grid->setLabel('vContent_notif_custom','Notifkasi Custom Content');
...
*/
    ...
    //set required
        ...
        switch ($action) {
            ...
            case 'updateproses':
                    ...
                    //         }
                    //     }
                    //     if($kpost=="setup_modul_vFieldName"){
                    //         foreach ($vpost as $kplast => $vplast) {
                    //             if($kplast==0){
                    //                 foreach ($vplast as $kk => $vv) {
                    //                    $IvFieldName[$kk]=$vv;
                    ...
                    //             $dupdated['vFieldName']=isset($UvFieldName[$iddetail])?$UvFieldName[$iddetail]:"";
                    ...
                    //             $dinsertd['vFieldName']=isset($IvFieldName[$iddinsert])?$IvFieldName[$iddinsert]:"";
    ...
    function get_table(){
        ...
        $sql = 'SELECT CONCAT(TABLE_SCHEMA,".",TABLE_NAME) AS TABLE_NAME FROM information_schema.TABLES WHERE TABLE_NAME LIKE "%plc2%" AND ( TABLE_SCHEMA LIKE ? OR TABLE_NAME LIKE ? ) ORDER BY TABLE_NAME ASC';
        $dt = $this->db->query($sql, array('%'.$term.'%','%'.$term.'%'))->result_array(); 
        $data = array();

        foreach ($dt as $d) {
            $sqlPK = "SHOW KEYS FROM ".$d['TABLE_NAME']." WHERE Key_name = 'PRIMARY'";
            $pk = $this->db->query($sqlPK)->row_array();

            $row_array['value'] = trim($d['TABLE_NAME']);
            $row_array['id']    = (!empty($pk))?$pk['Column_name']:'';
    ...
    */
    ...
    function updateBox_setup_modul_detail_activity($field, $id, $value, $rowData) {
        $sql = "SELECT a.* FROM plc3.m_modul_activity a WHERE a.lDeleted = 0 AND a.iM_modul = ? ORDER BY a.iSort ASC ";

        $data['activities']     = $this->db_schedulercheck->get_where('plc3.m_activity', array('lDeleted' => 0))->result_array();
        $data['dept']           = $this->db_schedulercheck->get_where('plc3.m_dept', array('lDeleted' => 0))->result_array();
        $data['rows']           = $this->db_schedulercheck->query($sql, array($rowData['iM_modul']))->result_array();
        $data['get']            = $this->input->get();
        $data['id']             = $this->input->get('id');
        // return $this->load->view('template/grid/master_module_activity_details',$data,TRUE);
        return $this->load->view('template/grid/setup_modul_activity',$data,TRUE);
    }


    function updateBox_setup_modul_detail_field($field, $id, $value, $rowData) {
        $sql = "SELECT f.* FROM plc3.m_modul_fileds f 
                WHERE f.iM_modul = ? AND f.lDeleted = 0 
                ORDER BY f.iSort ASC ";

        $data['activities']     = $this->db_schedulercheck->get_where('plc3.m_activity', array('lDeleted' => 0))->result_array();
        $data['rows']           = $this->db_schedulercheck->query($sql, array($rowData['iM_modul']))->result_array();
        $data['listField']      = $this->db_schedulercheck->get_where('plc3.m_jenis_field', array('lDeleted' => 0))->result_array();
        $data['get']            = $this->input->get();
        $data['id']             = $this->input->get('id');
        // return $this->load->view('template/grid/master_module_fields_details',$data,TRUE);
        return $this->load->view('template/grid/setup_modul_field',$data,TRUE);
    }

    function updateBox_setup_modul_detail_setup($field, $id, $value, $rowData) {
        $url    = base_url().'processor/plc/setup/modul/activity/'; 
        $url2   = base_url().'processor/plc/setup/modul/field/';
    ...
    function updateBox_setup_modul_iM_notif($field, $id, $value, $rowData) {
        $dept = $this->db->get_where('plc3.m_notif', array('lDeleted' => 0))->result_array();

        $return  = '<select name="'.$field.'" id="'.$id.'" >';
        $return .= '    <option value=""> -- Pilih Author -- </option>';
        foreach ($dept as $d) {
            $selected =  (strtoupper($value) == strtoupper($d['iM_notif']))?'selected':'';
            $return .= '<option '.$selected.' value="'.$d['iM_notif'].'">'.$d['vNama_notif'].'</option>';
        }
        $return .= '</select>';
        return $return;
    }
    ...
    function insertBox_setup_modul_vSql_notif_custom($field, $id) {
        $return = '<textarea name="'.$field.'" id="'.$id.'" class="required" style="width: 850px; height: 300px;" size="250" maxlength ="250"></textarea>';
        return $return;
    }
    
    function updateBox_setup_modul_vSql_notif_custom($field, $id, $value, $rowData) {
            if ($this->input->get('action') == 'view') {
                 $return= '<label title="Note">'.nl2br($value).'</label>'; 
            }else{ 
                $return = '<textarea name="'.$field.'" id="'.$id.'" class="required" style="width: 850px; height: 300px;" size="250" maxlength ="250">'.$value.'</textarea>';
    ...
    function insertBox_setup_modul_vContent_notif_custom($field, $id) {
        $return = '<textarea name="'.$field.'" id="'.$id.'" class="required" style="width: 850px; height: 300px;" size="250" maxlength ="250"></textarea>';
        return $return;
    }
    
    function updateBox_setup_modul_vContent_notif_custom($field, $id, $value, $rowData) {
            if ($this->input->get('action') == 'view') {
                 $return= '<label title="Note">'.nl2br($value).'</label>'; 
            }else{ 
                $return = '<textarea name="'.$field.'" id="'.$id.'" class="required" style="width: 850px; height: 300px;" size="250" maxlength ="250">'.$value.'</textarea>';
    ...
    function updateBox_setup_modul_iMutu($field, $id, $value, $rowData) {
        $lmarketing = array(''=>'Pilih',1=>'Yes', 0=>'No');
        if ($this->input->get('action') == 'view') {
            $o = $lmarketing[$value];
        } else {
    ...
    function updateBox_setup_modul_vDept_author($field, $id, $value, $rowData) {
        ...
        $return .= '    <option value=""> -- Pilih Author -- </option>';
        foreach ($dept as $d) {
            $selected =  (strtoupper($value) == strtoupper($d['vKode_dept']))?'selected':'';
...
*/
    ...
    function dataDetailsModul(){
        ...
        $rsel=array('hapus','vKode_activity','iType','vDept_assigned','vNip_assigned','vFieldName','vDesciption','iSort');
        $data = new StdClass;
        ...
        foreach ($q->result() as $k) {
            ...
            foreach ($rsel as $dsel => $vsel) {
                ...
                }elseif($vsel=="vFieldName"){
                    $dataar[$dsel]="<input type='text' size='20' id='".$grid."_vFieldName_".$i."' value='".$k->vFieldName."' class='' name='".$grid."_vFieldName[".$k->iM_modul_activity."][]'>";
    ...
    function dataDetailsFields(){
        ...
        $listTypeyesno=array(1=>"Iya",0=>"Tidak");
        ...
        foreach ($q->result() as $k) {
            ...
            foreach ($rsel as $dsel => $vsel) {
                if($vsel=="hapus"){
                    ...
                    $dataar[$dsel]="<select id='".$grid."_iM_jenis_field_".$i."' class='numberOnly required' name='".$grid."_iM_jenis_field[".$k->iM_modul_fileds."][]'><?php echo $option ?></select>";
                    ...
                    $dataar[$dsel]="<input type='text' size='3' id='".$grid."_iSort_".$i."' class='required' name='".$grid."_iSort[".$k->iM_modul_fileds."][]' value='".$k->{$vsel}."' />";
                }else{
                    $dataar[$dsel]=$k->{$vsel};
                }
                $z++;
