...
class Lib_utilitas {
	...
	function send_email($to, $cc, $subject, $content) {
		...
		$to=$to;
		$cc = $cc;
		$bcc = "eka.yuni@novellpharm.com, yandi.prabowo@novellpharm.com, supri@novellpharm.com"; //$cc;
		$subject = $subject;
		$content = $content;
	...
	function get_email_team($team='') {
		...
		$sql = "select ti.vnip,e.vEmail from plc2.plc2_upb_team_item ti 
                ...
                where ti.iteam_id in (".$team.") and ti.ldeleted = 0 and ti.vnip not in ('".$this->sess_auth->gNIP."')";
		$query = $this->_ci->db->query( $sql );
		if($query->num_rows()>0) {
