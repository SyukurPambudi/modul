...
?>
<script>
function prareg_hpr_custome_save(grid, url, dis) {
    ...
    $.each(req, function(i,v){
        ...
        if($(this).val() == '') {
            ...
            label = label.replace('*','');
            if(id!="v3_prareg_hpr_ttarget_hpr"){  
    ...
    if(conf > 0) {
        ...
        custom_confirm(comfirm_message,function(){
            if(isUpload && !isValidAFileSize('#form_create_'+grid+' input.multifile', uploadLimit)) {
                ...
                $.ajax({
                url: base_url+'processor/plc/v3/prareg/hpr?action=updateinsert&foreign_key=<?php echo $get['foreign_key']; ?>&company_id=<?php echo $get['company_id']; ?>&group_id=<?php echo $get['group_id']; ?>&modul_id=<?php echo $get['modul_id']; ?>',
                type: 'post',
