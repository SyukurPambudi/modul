...
$("#<?php echo $tableId;?> .file_remove").click(function(){
	...
	var tmpDel = $("#study_literatur_mikro_del");
	li.remove();
	var v = tmpDel.val();
	v+=','+fileid;
	tmpDel.val( v );
	alert( $("#study_literatur_mikro_del").val() );
});

function add_row_study_literatur_mikro_upload(table_id){		
		//alert(table_id);
		...
		if (n.length == 0) {
			...
			row_content	 += '<span class="delete_btn"><a href="javascript:;" class="study_literatur_mikro_del" onclick="del_row(this, \'study_literatur_mikro_del\')">[Hapus]</a></span></td>';		
			row_content  += '</tr>';
...
<table class="hover_table" id="study_literatur_mikro_upload" cellspacing="0" cellpadding="1" style="width: 98%; border: 1px solid #dddddd; text-align: center; margin-left: 5px; border-collapse: collapse">
	<thead>
	...
	<tbody>
		<?php
			...
			if(!empty($rows)) {
				...
				foreach($rows as $row) {
					...
					$id  = $row['istudy_mikro_id'];
					$value = $row['vFilename'];	
					if($value != '') {
						if (file_exists('./files/plc/study_literatur_mikro/'.$id.'/'.$value)) {
							$link = base_url().'processor/plc/study/literatur/mikro?action=download&id='.$id.'&file='.$value;
		...
		?>
				<tr style="border: 1px solid #dddddd; border-collapse: collapse; background: #ffffff; ">
					<td style="border: 1px solid #dddddd; width: 3%; text-align: center;">
						<span class="study_literatur_mikro_upload_num"><?php echo $i ?></span>
					</td>		
					<td colspan="1" style="border: 1px solid #dddddd; width: 27%">
						...
						<input type="hidden" name="fileid[]" style="width: 70%" value="<?php echo $row['istudy_mikro_file_id'] ?>" />
					</td>	
					...
					<td style="border: 1px solid #dddddd; width: 10%">
						<?php if ($this->input->get('action') != 'view') { ?>
							<span class="delete_btn"><a href="javascript:;" class="study_literatur_mikro_del" onclick="del_row1(this, 'study_literatur_mikro_del')">[Hapus]</a></span>	
						<?php	} ?>
		...
		<tr style="border: 1px solid #dddddd; border-collapse: collapse; background: #ffffff; ">
					<td style="border: 1px solid #dddddd; width: 3%; text-align: center;">
						<span class="study_literatur_mikro_upload_num">1</span>
					</td>		
					<td colspan="1" style="border: 1px solid #dddddd; width: 27%">
	...
	<tfoot>
		<tr>
			<td colspan="5"></td><td style="text-align: center">
				<?php if ($this->input->get('action') != 'view') { ?>
					<a href="javascript:;" onclick="javascript:add_row_study_literatur_mikro_upload('study_literatur_mikro_upload')">Tambah</a>	
				<?php } ?>
