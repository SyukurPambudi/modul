...
$("#<?php echo $tableId;?> .file_remove").click(function(){
	...
	var tmpDel = $("#cek_dokumen_del");
	li.remove();
	var v = tmpDel.val();
	v+=','+fileid;
	tmpDel.val( v );
	alert( $("#cek_dokumen_del").val() );
});

function add_row_cek_dokumen(table_id){		
		//alert(table_id);
		...
		if (n.length == 0) {
			...
			row_content  +=	'<select name="confirm[]"><option value="0">Haven\'t been checked</option><option value="1">Yes</option><option value="3">No</option><option value="2">Unnecessary</option></select>';
			<?php } 
				else{?>
			row_content  +=	'<select name="confirm[]"><option value="0">Haven\'t been checked</option></select>';
			<?		}
				}?>
			row_content  +=	'</td>';
			row_content  +=	'<td style="border: 1px solid #dddddd; width: 4%; text-align: center;">';
			...
			row_content	 += '<textarea class="" name="fileketerangan[]" style="width: 150px; height: 50px;"></textarea></td>';
			row_content	 += '<td style="border: 1px solid #dddddd; width: 10%">';
			row_content	 += '<span class="delete_btn"><a href="javascript:;" class="cek_dokumen_del" onclick="del_row1(this, \'cek_dokumen_del\')">[Hapus]</a></span></td>';		
			row_content  += '</tr>';
...
<table class="hover_table" id="cek_dokumen" cellspacing="0" cellpadding="1" style="width: 98%; border: 1px solid #dddddd; text-align: center; margin-left: 5px; border-collapse: collapse">
	<thead>
	...
	<tbody>
		...
		if((in_array('BD', $mydept))||((in_array('PD', $mydept))&&(in_array($teamupb, $team)))){
			...
			if($value != '') {
				if (file_exists('./files/plc/bahan_kemas/'.$idsp.'/'.$value)) {
					$link = base_url().'processor/plc/cek/dokumen?action=download&id='.$idsp.'&path='.$type.'&file='.$value;
		...
		?>
				<tr style="border: 1px solid #dddddd; border-collapse: collapse; background: #ffffff; ">
					<td style="border: 1px solid #dddddd; width: 3%; text-align: center;">
						<span class="cek_dokumen_num"><?php echo $i ?></span>
					</td>
					<td style="border: 1px solid #dddddd; width: 4%; text-align: center;">
						...
						?>
						<input type="hidden" name="confirm[]" value="<?php echo $row['iconfirm_busdev'];?>">
						<span>
		...
		?>
				<tr style="border: 1px solid #dddddd; border-collapse: collapse; background: #ffffff; ">
					<td style="border: 1px solid #dddddd; width: 3%; text-align: center;">
						<span class="cek_dokumen_num">1</span>
					</td>
					<td style="border: 1px solid #dddddd; width: 4%; text-align: center;">
						<select name="confirm[]">
					...
					<td colspan="2" style="border: 1px solid #dddddd; width: 15%">
						<textarea class="" name="fileketerangan[]" style="width: 150px; height: 50px;"></textarea>
					</td>
					<td style="border: 1px solid #dddddd; width: 10%">
						<span class="delete_btn"><a href="javascript:;" class="cek_dokumen_del" onclick="del_row1(this, 'cek_dokumen_del')">[Hapus]</a></span>
					</td>	
				</tr>
		<?php }
		} 	
	...
	<tfoot>
		<td colspan="5"></td>
			...
			<td colspan="2" style="text-align: center"><a href="javascript:;" onclick="javascript:add_row_cek_dokumen('cek_dokumen')">Tambah</a></td>
		<?php } 
			if((in_array('BD', $mydept))&&($iDone==0)&&($cekconf==0)) {?>
			<td style="text-align: center">
				<?php echo '&nbsp;&nbsp;&nbsp; <span onclick="javascript:load_popup(\''.base_url().'processor/plc/cek/dokumen?action=dokDone&dok=bk&id='.$idsp.'&upb_id='.$iupb_id.'&group_id='.$this->input->get('group_id').'&modul_id='.$this->input->get('modul_id').'\')" class="ui-button-text icon-save ui-button ui-widget ui-state-default ui-corner-all ui-button-text-icon-primary" id="button_doneBk">Done</span>';?>
			</td>
			<?php }?>
		</tr>
		<?php if(($iDone==1)) {?>
