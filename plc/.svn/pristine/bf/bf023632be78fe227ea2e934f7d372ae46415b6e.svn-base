...
$("#<?php echo $tableId;?> .file_remove").click(function(){
	...
	var tmpDel = $("#proval_del");
	li.remove();
	var v = tmpDel.val();
	v+=','+fileid;
	tmpDel.val( v );
	alert( $("#proval_del").val() );
});

function add_row_proval(table_id){		
		//alert(table_id);
		...
		if (n.length == 0) {
			...
			row_content  +=	'<select name="confirmproval[]"><option value="0">Haven\'t been checked</option><option value="1">Yes</option><option value="3">No</option><option value="2">Unnecessary</option></select>';
			<?php } 
				else{?>
			row_content  +=	'<select name="confirmproval[]"><option value="0">Haven\'t been checked</option></select>';
			<?php		}
				}?>
			row_content  +=	'</td>';
			row_content	 += '<td colspan="2" style="border: 1px solid #dddddd; width: 50%">';
			row_content	 += '<input type="file" id="fileupload'+table_id+c+'" class="fileupload multi multifile required" name="fileuploadproval[]" style="width: 50%" />*';
			row_content  += '<input type="hidden" name="namafiledproval[]" style="width: 70%" value="" />';
			row_content  += '<input type="hidden" name="fileidproval[]" style="width: 70%" value="" /></td>';	
			row_content	 += '<td colspan="2" style="border: 1px solid #dddddd; width: 8%">';
			row_content	 += '<textarea class="" name="fileketeranganproval[]" style="width: 240px; height: 50px;"></textarea></td>';
			row_content	 += '<td style="border: 1px solid #dddddd; width: 10%">';
			row_content	 += '<span class="delete_btn"><a href="javascript:;" class="proval_del" onclick="del_row1(this, \'proval_del\')">[Hapus]</a></span></td>';		
			row_content  += '</tr>';
			...
			row_content	 += '<input type="file" id="fileupload'+table_id+c+'" class="fileupload multi multifile required" name="fileuploadproval[]" style="width: 50%" />*';
			row_content  += '<input type="hidden" name="namafileproval[]" style="width: 70%" value="" />';
			row_content  += '<input type="hidden" name="fileidproval[]" style="width: 70%" value="" /></td>';
			row_content	 += '<td colspan="2" style="border: 1px solid #dddddd; width: 8%">';
			row_content	 += '<textarea class="" name="fileketeranganproval[]" style="width: 240px; height: 50px;"></textarea></td>';
			row_content	 += '<td style="border: 1px solid #dddddd; width: 10%">';
...
<table class="hover_table" id="proval" cellspacing="0" cellpadding="1" style="width: 98%; border: 1px solid #dddddd; text-align: center; margin-left: 5px; border-collapse: collapse">
	<thead>
	<tr style="width: 98%; border: 1px solid #dddddd; background: #548cb6; border-collapse: collapse">
		<th colspan="7" style="border: 1px solid #dddddd;"><span style="font-weight: bold; color: #ffffff; text-shadow: 0 1px 1px rgba(0, 0, 0, 0.3); text-transform: uppercase;">Upload File Laporan Validasi Proses</span></th>
	</tr>
	...
	<tbody>
		...
		if((in_array('BD', $mydept))||(in_array('QA', $mydept))) {
			...
			$idsp  = $row['iprotokol_id'];
			$sql_spek="select * from plc2.protokol_valpro fo where fo.iprotokol_id='$idsp' and fo.iappqa=2 and fo.lDeleted=0
						order by fo.iprotokol_id desc limit 1";
			$q_spek=$this->db_plc0->query($sql_spek)->row_array();
			$iupb_id=$q_spek['iupb_id'];
			$value = $row['vFilename'];
			$type = "proval";
			if($value != '') {
				if (file_exists('./files/plc/protokol_valpro/'.$idsp.'/'.$value)) {
		...
		?>
				<tr style="border: 1px solid #dddddd; border-collapse: collapse; background: #ffffff; ">
					...
					<td style="border: 1px solid #dddddd; width: 4%; text-align: center;">
						...
						?>
						<input type="hidden" name="confirmproval[]" value="<?php echo $row['iconfirm_busdev'];?>">
						<span>
					...
					<td colspan="2" style="border: 1px solid #dddddd; width: 27%">
						...
						<input type="hidden" name="namafileproval[]" style="width: 70%" value="<?php echo $row['vFilename'] ?>" />
						<input type="hidden" name="fileidproval[]" style="width: 70%" value="<?php echo $row['iprotokol_detail_id'] ?>" />
					</td>	
					...
					<td style="border: 1px solid #dddddd; width: 10%">
						<?php if (($this->input->get('action') != 'view') && (in_array('QA', $mydept)) && (($row['iconfirm_busdev']==0)||($row['iconfirm_busdev']==3))) { ?>
							<span class="delete_btn"><a href="javascript:;" class="proval_del" onclick="del_row1(this, 'proval_del')">[Hapus]</a></span>	
						<?php	} ?>
		...
		?><!--
				<tr style="border: 1px solid #dddddd; border-collapse: collapse; background: #ffffff; ">
					...
					<td colspan="2" style="border: 1px solid #dddddd; width: 27%">
						<input type="file" id="fileuploadproval1" class="fileupload multi multifile required" name="fileuploadproval[]" style="width: 50%" /> *
						<input type="hidden" name="namafileproval[]" style="width: 70%" value="" />
						<input type="hidden" name="fileidproval[]" style="width: 70%" value="" />
						</td>	
					<td colspan="2" style="border: 1px solid #dddddd; width: 15%">
						<textarea class="" name="fileketeranganproval[]" style="width: 240px; height: 50px;"></textarea>
					</td>
					<td style="border: 1px solid #dddddd; width: 10%">
						<span class="delete_btn"><a href="javascript:;" class="proval_del" onclick="del_row1(this, 'proval_del')">[Hapus]</a></span>
					</td>	
	...
	<tfoot>
		<tr>
			...
			<td colspan="2" style="text-align: center"><a href="javascript:;" onclick="javascript:add_row_proval('proval')">Tambah</a></td>
			<?php }?>
			<?php if((in_array('BD', $mydept))&&($iDone==0)&&($cekconf==0)) {?>
			<td style="text-align: center">
				<?php echo '&nbsp;&nbsp;&nbsp; <span onclick="javascript:load_popup(\''.base_url().'processor/plc/cek/dokumen/prareg?action=dokDone&dok=proval&id='.$idsp.'&upb_id='.$iupb_id.'&group_id='.$this->input->get('group_id').'&modul_id='.$this->input->get('modul_id').'\')" class="ui-button-text icon-save ui-button ui-widget ui-state-default ui-corner-all ui-button-text-icon-primary" id="button_doneproval">Done</span>';?>
			</td>
			<?php }?>
		</tr>
		<?php if(($iDone==1)) {?>
			<tr>
			 <td colspan="3" style="text-align: left"><b><p style="color:green;font-size:120%;">Status Dokumen Form Validasi Proses : DONE </p></b></td>
			</tr>
		<?php }?>
...
<script>
	$("#fileuploadproval1").change(function () {
	    var fileExtension = ["pdf","jpeg", "jpg", "png", "gif", "bmp"];
	    if ($.inArray($(this).val().split(".").pop().toLowerCase(), fileExtension) == -1) {
