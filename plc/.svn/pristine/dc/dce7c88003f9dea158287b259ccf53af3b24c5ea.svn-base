		...
		custom_confirm(comfirm_message,function(){
			if(isUpload && !isValidAFileSize('#form_create_'+grid+' input.multifile', uploadLimit)) {
				...
				success: function(data) {
					...
					if(isUpload) {
								var iframe = $('<iframe name="product_trial_frame"/>');
								iframe.attr({'id':'product_trial_frame'});
								...
								$('#form_create_'+grid).attr('target','product_trial_frame');								
				...
				/*.done(function(data) {
					...
					$('#grid_'+grid).trigger('reloadGrid');
						...
						$.get(base_url+'processor/plc/product/trial/formula/skala/trial?action=update&id='+last_id+'&foreign_key='+foreign_id+'&company_id='+company_id+'&group_id='+group_id+'&modul_id='+modul_id, function(data) {
			...
			if(isUpload && !isValidAFileSize('#form_update_'+grid+' input.multifile', uploadLimit)) {
				...
				success: function(data) {
					...
					if(isUpload > 0) {				
						...
						$('#form_update_'+grid).attr('target','product_trial_frame');						
...
<iframe name="product_trial_frame" id="product_trial_frame" height="0" width="0">
</iframe>
