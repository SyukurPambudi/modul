...
foreach ($get as $kget => $vget) {
    if($kget!="act" && $kget!="formaction" && $kget!="action"){
        $in[]=$kget."=".$vget;
    }
}
$g=implode("&", $in);
...
$applet1 = $this->db_plc0->query($qp)->result_array();
...
$sqC = "SELECT DISTINCT (p.`vtipe`) FROM plc2.`plc2_upb_team` p JOIN plc2.`plc2_upb_team_item` pt 
    ...
    ?>
    <script>
        ini =$("label[for='v3_reg_applet_form_detail_dokumen_tambahan_applet']");
...
<table class="hover_table" id="lpo_upload" cellspacing="0" cellpadding="1" style="width: 98%; border: 1px solid #dddddd; text-align: center; margin-left: 5px; border-collapse: collapse">
        <thead>
            ...
            <?php }else{
                ...
                foreach ($applet1 as $applet) { ?>
                    ...
                    <td colspan="1" style="border: 1px solid #dddddd; width: 15%"> 
                        ...
                        if($value != '') {
                            if (file_exists('./files/plc/v3/reg/appletnew/'.$iupb_id.'/'.$value)) {
                                $link = base_url().'processor/plc/v3/reg/applet?action=download1&id='.$iupb_id.'&file='.$value;
                                                                        ...
                                                                        ?>
                                                                                <tr>
                                                                                    <td colspan="1" style="border: 1px solid #dddddd; width: 15%">
                                                                                        ...
                                                                                        $value = $idt['vfilename'];
                                                                                        if($value != '') {
                                                                                            if (file_exists('./files/plc/v3/reg/appletnew/'.$iupb_id.'/detail/'.$r2['iappletfile2'].'/'.$value)) {
                                                                                                $link = base_url().'processor/plc/v3/reg/applet?action=download2&id='.$iupb_id.'&id2='.$r2['iappletfile2'].'&file='.$value;
                ...
                <td colspan="5" style="text-align: right">  
                    <?php 
                        ...
                        //Cek Don File untuk Manager Khusus Dia
                            ...
                            if($c1==$c2 && $c11==$c22 && $c111==$c222 && $iappbusdev_registrasi==0 && $c222>0){  
                            ?>
                                <div>
	...
	var url2 =  "<?php echo base_url().'processor/plc/v3/reg/applet'; ?>";
	...
	function deleteRowtable(r,idhapus,idtable,itipe,idindex) {
		...
		if(jumlah>3){
			if(idhapus>0){
				$.ajax({
	            url: "<?php echo base_url().'processor/plc/v3/reg/applet?action=hapustableap' ?>",
	            type: 'POST',
	            data: {
	                id_file: idindex, 
	                tipe: itipe,
	            ...
	            success: function(data) {  
	            	...
	            	$.get(url2+"?action=update&id="+last_id+"&<?php echo $g; ?>", function(data) {
                            $("div#form_v3_reg_applet").html(data); 
                    });
	            }
	        });
			}else{   
	...
	function hilangap1(id){
		$.ajax({
            url: "<?php echo base_url().'processor/plc/v3/reg/applet?action=hilangap1' ?>",
            type: 'POST',
            data: {
                id_file: id, 
	...
	function donefileap(id,id2){
		$.ajax({
            url: "<?php echo base_url().'processor/plc/v3/reg/applet?action=donefileap' ?>",
            type: 'POST',
            data: {
                id_file: id,  
                id_file2: id2,   
	...
	function donefileap3(id){
		$.ajax({
            url: "<?php echo base_url().'processor/plc/v3/reg/applet?action=donefileapall' ?>",
            type: 'POST',
            data: {
                id_file: id,   
            },
	...
	function donefileap1(id){
		...
		if(hasilrev!=''){
			$.ajax({
	            url: "<?php echo base_url().'processor/plc/v3/reg/applet?action=donefileaprevisi' ?>",
	            type: 'POST',
	            data: {
	                id_file: id,  
	                revisi:hasilrev, 
