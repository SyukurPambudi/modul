...
class v3_bahan_baku_approval_ksk extends MX_Controller {
    function __construct() {
		...
		$this->url = 'v3_bahan_baku_approval_ksk';
		$this->urlpath = 'plc/'.str_replace("_","/", $this->url);

		$this->maintable = 'plc2.plc2_upb_ro_detail';	
		$this->main_table = $this->maintable;	
		$this->main_table_pk = 'irodet_id';
		$datagrid['islist'] = array(
			'plc2_upb_po.vpo_nomor' => array('label'=>'No. PO','width'=>80,'align'=>'left','search'=>true)
			,'plc2_upb_ro.vro_nomor' => array('label'=>'No. Penerimaan','width'=>90,'align'=>'left','search'=>false)
			,'plc2_upb.vupb_nomor' => array('label'=>'No. UPB','width'=>50,'align'=>'center','search'=>true)
			,'plc2_upb.vupb_nama' => array('label'=>'Nama Usulan','width'=>250,'align'=>'left','search'=>true)
			,'plc2_raw_material.vnama' => array('label'=>'Bahan Baku','width'=>250,'align'=>'left','search'=>true)
			,'isyarat' => array('label'=>'Kesimpulan QC','width'=>100,'align'=>'left','search'=>false)
			,'isubmit_ksk' => array('label'=>'Status','width'=>150,'align'=>'left','search'=>false)
			,'iappqa_ksk' => array('label'=>'Approval QA','width'=>120,'align'=>'left','search'=>false)
		);
		$datagrid['shortBy']=array('plc2_upb_ro_detail.irodet_id'=>'Desc');
		$datagrid['setQuery']=array(
								0=>array('vall'=>'plc2_upb_ro_detail.ldeleted','nilai'=>0)
								);
		$datagrid['jointableinner']=array(
									0=>array('plc2.plc2_raw_material'=>'plc2_raw_material.raw_id=plc2_upb_ro_detail.raw_id')
									,1=>array('plc2.plc2_upb_ro'=>'plc2.plc2_upb_ro.iro_id = plc2.plc2_upb_ro_detail.iro_id')
									,3=>array('plc2.plc2_upb_request_sample'=>'plc2.plc2_upb_request_sample.ireq_id = plc2.plc2_upb_ro_detail.ireq_id')
									,4=>array('plc2.plc2_upb'=>'plc2.plc2_upb.iupb_id = plc2.plc2_upb_request_sample.iupb_id')
                                    #,5=>array('plc2.plc2_upb_formula'=>'plc2_upb_formula.iupb_id=plc2_upb.iupb_id')
                                    //,6=>array('pddetail.formula_stabilita'=>'plc2.plc2_upb_formula.iFormula_process = pddetail.formula_stabilita.iFormula_process')
								);

		/*$datagrid['changeField']=array(
								'isyarat'=>array('jenis'=>'combobox','nilai'=>array(''=>'--select--', 0=>'-', 1=>'TMS', 2=>'MS'))
								);*/
    ...
    function index($action = '') {
		...
		foreach ($this->datagrid as $kv => $vv) {
			...
			if($kv=='setQuery'){
				foreach ($vv as $list => $vlist) { $ini[]=$kv;
			...
			/*if($kv=='changeField'){ 
				foreach ($vv as $list => $vlist) { 
					$grid->changeFieldType($list, $vlist['jenis'], $vlist['nilai']);
				}
			}*/
			$grid->setJoinTable('plc2.plc2_upb_po', 'plc2.plc2_upb_po.ipo_id = plc2.plc2_upb_ro_detail.ipo_id', 'left');
			$grid->changeFieldType('isyarat', 'combobox','',array(''=>'--select--', 0=>'-', 1=>'TMS', 2=>'MS'));
			$grid->changeFieldType('isubmit_ksk', 'combobox','',array(''=>'--select--',0=>'Draft - Need to be Submit', 1=>'Submited'));
			$grid->changeFieldType('iappqa_ksk', 'combobox','',array(''=>'--select--',0=>'Waiting for approval',1=>'Rejected', 2=>'Approved'));
		...
		$grid->setQuery('plc2_upb.iupb_id IN 
						...
						AND m_modul_log_activity.idprivi_modules IN (3591,3339))',NULL);
	...
	function insertBox_v3_bahan_baku_approval_ksk_form_detail($field,$id){
            $get=$this->input->get();
            $post=$this->input->post();
        ...
        function updateBox_v3_bahan_baku_approval_ksk_form_detail($field,$id,$value,$rowData){
            $get=$this->input->get();
            $post=$this->input->post();
            foreach ($get as $kget => $vget) {
                if($kget!="action"){
