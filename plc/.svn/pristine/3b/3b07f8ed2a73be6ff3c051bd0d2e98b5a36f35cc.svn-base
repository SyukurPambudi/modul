...
function add_row_lpo_upload(table_id,idFile){		 
		...
		if (n.length == 0) {
			...
			row_content	 += '	</td>		';
			row_content	 += '		<td colspan="1" style="border: 1px solid #dddddd; width: 15%">';
			row_content	 += '		<input type="file" id="fileupload_hpr" class="fileupload multi multifile required" name="fileupload[]" style="width: 70%" /> *';
	...
	<tbody class="lpo_upload_body">
		...
		<tr style="border: 1px solid #dddddd; border-collapse: collapse; background: #ffffff; ">
			...
			<td colspan="1" style="border: 1px solid #dddddd; width: 15%">
				<input type="file" id="fileupload_hpr" class="fileupload multi multifile" name="fileupload[]" style="width: 70%" /> *
				...
				<td colspan="1" style="border: 1px solid #dddddd; width: 15%"> 
					...
					if($value != '') {
						if (file_exists('./files/plc/hprnew/'.$iupb_id.'/'.$value)) {
							$link = base_url().'processor/plc/hpr?action=download1&id='.$iupb_id.'&file='.$value;
																				...
																				<td colspan="1" style="border: 1px solid #dddddd; width: 15%">
																					...
																					if($value != '') {
																						if (file_exists('./files/plc/hprnew/'.$iupb_id.'/detail/'.$r2['ihprfile2'].'/'.$value)) {
																							$link = base_url().'processor/plc/hpr?action=download2&id='.$iupb_id.'&id2='.$r2['ihprfile2'].'&file='.$value;
			...
			<td colspan="5" style="text-align: right">  
				<?php 
					...
					//Cek Don File untuk Manager Khusus Dia
						...
						if($c1==$c2 && $c11==$c22 && $c111==$c222 && $iappbd_hpr==0 && $c222>0){  
						?>
							<div>
...
<script>
	...
	var url2 =  "<?php echo base_url().'processor/plc/hpr'; ?>";
	...
	function deleteRowtable(r,idhapus,idtable,itipe,idindex) {
		...
		if(jumlah>3){
			if(idhapus>0){
				$.ajax({
	            url: "<?php echo base_url().'processor/plc/hpr?action=hapustable' ?>",
	            type: 'POST',
	            data: {
	                id_file: idindex, 
	                tipe: itipe,
	...
	function hilang1(id){ 
		$.ajax({
            url: "<?php echo base_url().'processor/plc/hpr?action=hilang1' ?>",
            type: 'POST',
	...
	function donefile(id,id2){
		$.ajax({
            url: "<?php echo base_url().'processor/plc/hpr?action=donefile' ?>",
            type: 'POST',
            data: {
                id_file: id,  
                id_file2: id2,   
	...
	function donefile3(id){
		$.ajax({
            url: "<?php echo base_url().'processor/plc/hpr?action=donefileall' ?>",
            type: 'POST',
            data: {
                id_file: id,   
            },
	...
	function donefile1(id){
		...
		if(hasilrev!=''){
			$.ajax({
	            url: "<?php echo base_url().'processor/plc/hpr?action=donefilerevisi' ?>",
	            type: 'POST',
	            data: {
	                id_file: id,  
	                revisi:hasilrev, 
