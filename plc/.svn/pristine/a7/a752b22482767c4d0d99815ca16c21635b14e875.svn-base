...
$("#<?php echo $tableId;?> .file_remove").click(function(){
	...
	var tmpDel = $("#fbasic_del");
	li.remove();
	var v = tmpDel.val();
	v+=','+fileid;
	tmpDel.val( v );
	alert( $("#fbasic_del").val() );
});



function add_row_fbasic_upload(table_id){		
		//alert(table_id);
		...
		if (n.length == 0) {
			...
			row_content	 += '<input type="file" id="fileupload'+table_id+c+'" class="fileupload multi multifile required" name="fileuploadfbasic[]" style="width: 70%" />*';
			row_content  += '<input type="hidden" name="namafilefbasic[]" style="width: 70%" value="" />';	
			row_content  += '<input type="hidden" name="fileidfbasic[]" style="width: 70%" value="" /></td>';
			row_content	 += '<td colspan="3" style="border: 1px solid #dddddd; width: 8%">';
			row_content	 += '<textarea id= "filefbasic'+c+'" class="" name="fileketeranganfbasic[]" style="width: 240px; height: 50px;"></textarea><br/>tersisa <span id="len'+c+'">250</span> karakter<br/></td>';
			row_content	 += '<td style="border: 1px solid #dddddd; width: 10%">';
			row_content	 += '<span class="delete_btn"><a href="javascript:;" class="fbasic_del" onclick="del_row(this, \'fbasic_del\')">[Hapus]</a></span></td>';		
			row_content  += '</tr>';
			...
			row_content  += '<input type="hidden" name="fileidfbasic[]" style="width: 70%" value="" /></td>';
			row_content	 += '<td colspan="3" style="border: 1px solid #dddddd; width: 8%">';
			row_content	 += '<textarea id= "filefbasic'+c+'"class="" name="fileketeranganfbasic[]" style="width: 240px; height: 50px;"></textarea><br/>tersisa <span id="len'+c+'">250</span> karakter<br/>';
			row_content  += '</td>';
...
<table class="hover_table" id="fbasic_upload" cellspacing="0" cellpadding="1" style="width: 98%; border: 1px solid #dddddd; text-align: center; margin-left: 5px; border-collapse: collapse">
	<thead>
	...
	<tbody>
		<?php
			...
			if(!empty($rows)) {
				...
				$type = "basic_formula";
				if($value != '') {
					if (file_exists('./files/plc/product_trial/basic_formula/filename/'.$idsp.'/'.$value)) {
		...
		?>
				<tr style="border: 1px solid #dddddd; border-collapse: collapse; background: #ffffff; ">
					<td style="border: 1px solid #dddddd; width: 3%; text-align: center;">
						<span class="fbasic_upload_num"><?php echo $i ?></span>
					</td>		
					<td colspan="1" style="border: 1px solid #dddddd; width: 27%">
						<?php echo $row['filename'] ?> 
						<input type="hidden" name="namafilefbasic[]" style="width: 70%" value="<?php echo $row['filename'] ?>" />
						<input type="hidden" name="fileidfbasic[]" style="width: 70%" value="<?php echo $row['id'] ?>" />
					</td>	
					...
					<td style="border: 1px solid #dddddd; width: 10%">
						<?php if ($this->input->get('action') != 'view') { ?>
							<span class="delete_btn"><a href="javascript:;" class="fbasic_del" onclick="del_row1(this, 'fbasic_del')">[Hapus]</a></span>	
						<?php	} ?>
		...
		<tr style="border: 1px solid #dddddd; border-collapse: collapse; background: #ffffff; ">
					<td style="border: 1px solid #dddddd; width: 3%; text-align: center;">
						<span class="fbasic_upload_num">1</span>
					</td>		
					<td colspan="1" style="border: 1px solid #dddddd; width: 27%">
						<input type="file" id="fileupload_fbasic1" class="fileupload multi multifile required" name="fileuploadfbasic[]" style="width: 70%" /> *
						<input type="hidden" name="namafilefbasic[]" style="width: 70%" value="" />
						<input type="hidden" name="fileidfbasic[]" style="width: 70%" value="" />
						</td>	
					<td colspan="3" style="border: 1px solid #dddddd; width: 15%">
						<textarea class="" id="filefbasic1"name="fileketeranganfbasic[]" style="width: 240px; height: 50px;"size="250"></textarea>
						<br/>
	...
	<tfoot>
		<tr>
			<td colspan="5" style="text-align: left">*) Max File 5 MB : Format = pdf,jpeg,jpg,png,gif,bmp</td><td style="text-align: center">
				<?php if ($this->input->get('action') != 'view') { ?>
					<a href="javascript:;" onclick="javascript:add_row_fbasic_upload('fbasic_upload')">Tambah</a>	
				<?php } ?>
	...
	<script>
       ...
       $("#fileupload_fbasic1").change(function () {
	        var fileExtension = ["pdf","jpeg", "jpg", "png", "gif", "bmp"];
	        if ($.inArray($(this).val().split(".").pop().toLowerCase(), fileExtension) == -1) {
	        	_custom_alert("Only formats are allowed : "+fileExtension.join(", "),"info", "info", "fbasic", 1, 20000);
	        	$(this).val('');
	        }
    	});
