<?php 
    ...
    <script>
<?php
       if($iupb_id!="0" && $iappbusdev_registrasi==2 && $iprareg_ulang_reg==1 && $imutu_prareg==0 ){?>
...
if ($iappbd_applet ==2 ){ 
      ...
      $("#<?php echo $id ?>").change(function(){
            if($(this).val()==1){
                ...
                $("label[for='v3_reg_applet_form_detail_tPrareg_req_doc']").parent().show();
            }else{
                $("label[for='v3_reg_applet_form_detail_iPic_Prareg_req_doc']").parent().hide();
                $("label[for='v3_reg_applet_form_detail_tPrareg_req_doc']").parent().hide();
            }
        });
      </script>
      <?php
