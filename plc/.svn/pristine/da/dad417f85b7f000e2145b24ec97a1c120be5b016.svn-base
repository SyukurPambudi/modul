			...
			if(isUpload && !isValidFileSize('#form_create_'+grid+' input.multifile', uploadLimit)) {
				...
				$.ajax({
					...
					success: function(data) {	
						...
						if(o.status == true) {
							if(isUpload) {
								var iframe = $('<iframe name="strest_test_frame"/>');
								iframe.attr({'id':'strest_test_frame'});
								...
								$('#form_create_'+grid).attr('target','strest_test_frame');								
							...
							//alert(url)
							$.get(base_url+'processor/plc/product/trial/stress/test?action=update&lastid='+last_id+'&foreign_key='+foreign_id+'&company_id='+company_id+'&group_id='+group_id+'&modul_id='+modul_id, function(data) {
					...
					success: function(data) {	
						...
						if(o.status == true) {
							if(isUpload) {
								...
								$('#form_update_'+grid).attr('target','strest_test_frame');								
...
<iframe name="strest_test_frame" id="strest_test_frame" height="0" width="0"></iframe>