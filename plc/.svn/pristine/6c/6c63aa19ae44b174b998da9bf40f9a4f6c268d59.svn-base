...
<div class="box-tambah">
	...
	<div class="clear"></div>
		<table style="width: 100%" id="vName_member_div_table">
			...
			<tbody>
	<?php
		if(count($effecs_proses) > 0) { // if udah ada di table effec action
			$i=1;
	...
	?>
				<tr>
						...
						<span class="effected_action">
							...
							<?php
								foreach($actions as $a) {
									$selected = $a['idplc2_status'] == $e['idplc2_status'] ? 'selected' : '';
									echo '<option '.$selected.' value="'.$a['idplc2_status'].'">'.$a['vCaption'].'</option>';
								}
							?>
							</select>
						</span>
					...
					<td style="text-align: center">
						...
						<?php
							foreach($rows as $r) {
								$selected = $r['idplc2_biz_process_sub'] == $e['iAffectedProcess'] ? 'selected' : '';
								echo '<option '.$selected.' value="'.$r['idplc2_biz_process_sub'].'">'.$r['vName'].' > '.$r['vStepName'].' > '.$r['vActivityName'].' ('.$r['vteam'].' - '.$r['vtipe'].')</option>';
							}
						?>
						</select>
					</td>				
				...
				<tr>
					...
					<td style="text-align: center">
						<span class="effected_action">
							...
							<?php
								foreach($actions as $a) {
									echo '<option value="'.$a['idplc2_status'].'">'.$a['vCaption'].'</option>';
								}
							?>
							</select>
						</span>
					...
					<td style="text-align: center">
						...
						<?php
							foreach($rows as $r) {
								echo '<option value="'.$r['idplc2_biz_process_sub'].'">'.$r['vName'].' > '.$r['vStepName'].' > '.$r['vActivityName'].' ('.$r['vteam'].' - '.$r['vtipe'].')</option>';
							}
						?>
						</select>
					</td>				
