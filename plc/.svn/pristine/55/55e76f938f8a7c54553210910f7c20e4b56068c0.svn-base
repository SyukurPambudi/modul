...
class v3_uji_mikro_fg_steril extends MX_Controller {
    function __construct() {
		...
		$this->title = 'Uji Mikro FG [Steril]';
		$this->url = 'v3_uji_mikro_fg_steril';
		...
		$datagrid['islist'] = array(
			...
			,'vno_surat' => array('label'=>'No Surat','width'=>200,'align'=>'left','search'=>false)
		...
		$datagrid['setQuery']=array(
								...
								,1=>array('vall'=>'study_literatur_pd.iuji_mikro','nilai'=>1)
								,2=>array('vall'=>'study_literatur_pd.ijenis_sediaan','nilai'=>0)
								);
		$datagrid['jointableinner']=array(
									...
									,2=>array('plc2.study_literatur_pd'=>'plc2.study_literatur_pd.iupb_id = plc2.plc2_upb.iupb_id')
								);
	...
	/*List Box*/
	function listBox_v3_uji_mikro_fg_steril_iappqa_uji($value) {
	...
	function listBox_v3_uji_mikro_fg_steril_ini_nilai($value, $pk, $name, $rowData) {
	...
	function insertBox_v3_uji_mikro_fg_steril_form_detail($field,$id){
            $get=$this->input->get();
            $post=$this->input->post();
        ...
        function updateBox_v3_uji_mikro_fg_steril_form_detail($field,$id,$value,$rowData){
            $get=$this->input->get();
            $post=$this->input->post();
            foreach ($get as $kget => $vget) {
                if($kget!="action"){
