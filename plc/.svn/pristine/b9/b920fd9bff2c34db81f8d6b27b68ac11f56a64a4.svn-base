...
$this->db_plc0 = $this->load->database('plc0',false, true);
	...
	function updateBox_upb_daftar_ihold($field, $id) {
			...
			//print_r($arrhak);
				if(empty($arrhak)){
					...
					if(empty($getbp)){}
						...
						if($this->auth->is_manager()){ //jika manager BD atau DR
							//if(($laststatus==7)&&($type=='BD')&&($rowData['iappbusdev']==0)){ // status udah submit baru bisa approve dkk
							if(($rowData['istatus']==1)&&($type=='BD')&&($rowData['iappbusdev']==0)){ // status udah submit baru bisa approve dkk
							...
							elseif(($rowData['istatus']==0)&&($type=='BD')&&($rowData['iappbusdev']==0)){ // status udah submit baru bisa approve dkk
							...
							if(($rowData['istatus']==0)&&($type=='BD')&&($rowData['iappbusdev']==0)){ //staff busdev, bisa edit
							...
							elseif(($rowData['istatus']==0)&&($type=='BD')&&($rowData['iappbusdev']==0)){ //staff busdev, bisa edit
								$update = '<button onclick="javascript:update_btn_back(\'upb_daftar\', \''.base_url().'processor/plc/upb/daftar?company_id='.$this->input->get('company_id').'&group_id='.$this->input->get('group_id').'&modul_id='.$this->input->get('modul_id').'\', this)" class="ui-button-text icon-save" id="button_save_daftar_upb">Update</button>';
