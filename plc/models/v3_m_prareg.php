...
class V3_m_prareg extends CI_Model {
	...
	function updateUploadFile($filejenis='',$type='',$dataretu=array(),$iupb_id=0){
        ...
        $sqldmf=array();
		if (isset($_FILES['fileupload_local_prareg_'.$filejenis])){
			$i=0;
			foreach ($_FILES['fileupload_local_prareg_'.$filejenis]["error"] as $key => $error) {	
				if ($error == UPLOAD_ERR_OK) {
					$tmp_name = $_FILES['fileupload_local_prareg_'.$filejenis]["tmp_name"][$key];
					$name =$_FILES['fileupload_local_prareg_'.$filejenis]["name"][$key];
					$data['filename'] = $name;
					...
					if(move_uploaded_file($tmp_name, $datafield['filepath']."/".$id."/".$name_generate)) {
						...
						if($ibk==1){
							...
							$sqldmf[] = "INSERT INTO plc2.group_file_upload(iM_modul_fileds, idHeader_File, vFilename,vFilename_generate ,dCreate, ijenis_bk_id, tKeterangan, cCreate, istatus, iconfirm_busdev, tKetRevQA) 
								VALUES (".$datafield['iM_modul_fileds'].",".$id.",'".$data['filename']."','".$name_generate."','".$data['dInsertDate']."','".$ijenisbk_upload[$i]."','".$fileketerangan[$i]."','".$this->user->gNIP."','".$istatus_upload[$i]."','".$iconfirm_prareg_upload[$i]."','".$tKetRevQA_upload_nilai."')";
						}else{
							$sqldmf[] = "INSERT INTO plc2.group_file_upload(iM_modul_fileds, idHeader_File, vFilename,vFilename_generate ,dCreate, tKeterangan, cCreate, istatus, iconfirm_busdev, tKetRevQA)
								VALUES (".$datafield['iM_modul_fileds'].",".$id.",'".$data['filename']."','".$name_generate."','".$data['dInsertDate']."','".$fileketerangan[$i]."','".$this->user->gNIP."','".$istatus_upload[$i]."','".$iconfirm_prareg_upload[$i]."','".$tKetRevQA_upload_nilai."')";
						}
						$i++;	
					}
					else{
	...
	function updateBeforeUpload($filejenis='',$type=''){
		...
		foreach($_POST as $key=>$value) {
			...
			if($key == 'v3_cek_dokumen_prareg_'.$filejenis.'_tKetRevQA'){
				foreach($value as $y=>$u) {
					if($y!=0){
						$tKetRevQA[$y]=$u[0];
					}else{
			...
			if($key == 'file_keterangan_local_prareg_'.$filejenis){
				foreach($value as $y=>$u) {
					$fileketeranganmain[] = $u;
				}
			}
			if($key == 'v3_cek_dokumen_prareg_'.$filejenis.'_ijenis_bk_id'){
				foreach($value as $y=>$u) {
					if($y!=0){
						$ijenisbk[$y]=$u[0];
					}else{
	...
	function get_v3_cek_dokumen_prareg_filemain(){
		$type="";
 		if($this->auth->is_manager()){
			$x=$this->auth->dept();
			$manager=$x['manager'];
			...
			foreach ($rsel as $dsel => $vsel) {
				...
				if($value != '') {
					if (file_exists('./'.$thisfilepath.'/'.$id.'/'.$value)) {
						$link = base_url().'processor/plc/v3/cek/dokumen/prareg?action=download&id='.$id.'&file='.$value.'&path='.$field;
				...
				if($vsel=="iact"){
					...
					$opt="<select id='v3_cek_dokumen_prareg_".$field."_iconfirm_prareg_".$n."' name='v3_cek_dokumen_prareg_".$field."_iconfirm_prareg[".$k->iFile."][]' ".$dis." >";
					...
					$opt="<select id='v3_cek_dokumen_prareg_".$field."_istatus_".$n."' name='v3_cek_dokumen_prareg_".$field."_istatus[".$k->iFile."][]' ".$dis." >";
					foreach ($arr as $karr => $varr) {
						$sel=$karr==$k->{$vsel}?'selected':'';
						$opt.="<option value='".$karr."' ".$sel.">".$varr."</option>";
					}
					...
					$now="<select id='v3_cek_dokumen_prareg_".$field."_ijenis_bk_id_".$n."' name='v3_cek_dokumen_prareg_".$field."_ijenis_bk_id[".$k->iFile."][]' ".$dis." >";
					...
					$now="<textarea id='v3_cek_dokumen_prareg_".$field."_tKetRevQA_".$n."' name='v3_cek_dokumen_prareg_".$field."_tKetRevQA[".$k->iFile."][]' style='width: 240px; height: 50px;' ".$dis.">".$k->{$vsel}."</textarea>";
					$dataar[$z]=$now;
	...
	function getDetailtable($team='PD',$iupb_id, $isrequired=0){
		...
		$filter=array('de.ikategori_id=3 and de.ijenisdok=1 and de.jenisplc=1 '.$and,'de.'.$iniTeam.'=1');
		...
		foreach ($row as $kteam => $vteam) {
			if($vteam['idetaildok']== 14 or $vteam['idetaildok']== 29 ){
				/*SOI FG required jika study literatur uji mikro Ya*/
				$this->db->where('iupb_id',$iupb_id);
				$upbdata=$this->db->get("plc2.plc2_upb")->row_array();							
				...
				$dUji = $this->db->query($sql)->row_array();
	...
	function getDetailtableUse($post){
		...
		$row=$this->m_prareg->getDetailFiles($filter)->result_array();
	...
	function getDetailFiles($array=array()){
		...
		$sql="select * from plc2.sys_detaildok de 
			...
			where de.ldeleted=0 and ma.ldeleted=0 ".$where;
