...
class pk_analytical_development_lokal extends MX_Controller {
    private $sess_auth;
    private $db_erp_pk;
    function __construct() {
        ...
        $this->url = 'pk_analytical_development_lokal'; 
        $this->urll= 'pk/'.str_replace("_", "/", $this->url);
        $this->load->library('lib_utilitas');
        $this->load->library('lib_pk_analytical_development_lokal');
    ...
    function index($action = '') {
        ...
        $grid = new Grid;
        $grid->setTitle('Analytical Development Lokal');    
        $grid->setTable('hrd.pk_trans');    
        $grid->setUrl('pk_analytical_development_lokal');
        ...
        $manager = explode(",",$this->lib_utilitas->getSysParam('FullPKMIS'));
...
*/
    ...
    switch ($action) {
      ...
      case 'saveSupportData';
        ...
        break;
      case 'getPerformanceObjective';
        $this->getPerformanceObjective();
        break;
      default:
        $grid->render_grid();
        break;
    ...
    function saveSupportData(){
      ...
      if ($parameter==1){
          $this->saveParameter1($_POST);
      }else if ($parameter==2){
        $this->saveParameter2($_POST);
      }else if ($parameter==3){
        $this->saveParameter3($_POST);
      }
      exit();
    }

    function download_excel(){
      $perode1 =$this->input->get('periode1');
      $perode2 =$this->input->get('periode2');
      $cNipNya =$this->input->get('nip');
            //Ambil Jenisnya 
            ...
            foreach ($dtJenis as $dj) {
                $html .= "<br><table cellspacing='0' cellpadding='3' width='200%'>
                            <tr>
                                <td><b>".$dj['vnama_jenis']."</b></td> 
                            </tr>
                          </table>
                ";
                $html .= "<table cellspacing='0' cellpadding='3' width='200%'>
                            <tr style='background: #C0C0C0;border: 1px solid #dddddd; border-collapse: collapse;'>
                                <td style='border: 1px solid #dddddd;' rowspan='2' width='2%'>No</td> 
                                <td style='border: 1px solid #dddddd;' rowspan='2' width='5%'>No Batch</td>
                                <td style='border: 1px solid #dddddd;' rowspan='2' width='5%'>Nama</td>";
                                ...
                                foreach ($dpro as $dp) {
                                     $html .= "<td style='border: 1px solid #dddddd;text-align:center' colspan='3'>".$dp['vProses_name']."</td>";
                                }
                $html .= "</tr>";
                $html .= "<tr style='background: #C0C0C0;border: 1px solid #dddddd; border-collapse: collapse;'>"; 
                                $dpro = $this->db_erp_pk->query($sqlP)->result_array();
                                foreach ($dpro as $dp) {
                                     $html .= "<td style='border: 1px solid #dddddd;text-align:center' width='5%'>Waiting</td>";
                                     $html .= "<td style='border: 1px solid #dddddd;text-align:center' width='5%'>Prosess</td>";
                                     $html .= "<td style='border: 1px solid #dddddd;text-align:center' width='3%'>Total</td>";
                                }
                $html .= "</tr>";
                        ...
                        $html .="<tr style='border: 1px solid #dddddd; border-collapse: collapse;'>
                                ...
                                foreach ($dpro as $dp) { 
                                    $html .= "<td style='border: 1px solid #dddddd;' >";
                                              $sqlTgl = "select kw.finish_time,kw.receive_time, TIMESTAMPDIFF(SECOND,kw.receive_time,kw.finish_time) as tot,kw.iNeedRework,kw.is_rework from kanban.kbn_mesin_waiting kw 
                                                 JOIN kanban.kbn_mbr km on kw.mbr_id = km.id
                                                 JOIN kanban.kbn_master_jenis kj on kj.ijenis_id = km.ijenis_id
                                                 where kj.isTipeSolid = 1 and kw.proses_qc_id NOT IN ('P0051','P0000','P0008') and km.iCompany=3
                                                 and km.id ='".$dm['id']."' 
                                                 ...
                                                 and kw.mbr_id IN( 
                                                    ...
                                                    BETWEEN '".$perode1."' and '".$perode2."')   
                                                 ";
                                                $dhari = $this->db_erp_pk->query($sqlTgl)->result_array();
                                                $tot   = $this->db_erp_pk->query($sqlTgl)->num_rows();
                                                $i = 0;
                                                $totalH1 = 0;
                                                foreach ($dhari as $dhr) { 
                                                   $i++;
                                                   if(!empty($dhr['receive_time']) || !empty($dhr['finish_time'])){
                                                    $warna = "";
                                                    if($dhr['iNeedRework']>0 || $dhr['is_rework']>0){
                                                      //Warna Merah
                                                      $warna = 'style="color:red;"';
                                                    }

                                                    $dhr['tot'] = number_format(($dhr['tot']/3600)/24,3);
                                                    if($dhr['tot']==0.000){
                                                        $dhr['tot'] = 0;
                                                    }
                                                    ...
                                                    if($tot>1){ 
                                                      if($tot==$i){
                                                        $html .= '<p '.$warna.'>'.$dhr['receive_time']." :: ".$dhr['finish_time']." = ".$dhr['tot'].' Hari</p><br>';  
                                                      }else{
                                                        $html .= '<p '.$warna.'>'.$dhr['receive_time']." :: ".$dhr['finish_time']." = ".$dhr['tot'].' Hari</p><hr>'; 
                                                      } 
                                                    }else{
                                                      $html .= '<p '.$warna.'>'.$dhr['receive_time']." :: ".$dhr['finish_time']." = ".$dhr['tot'].' Hari</p><br>'; 
                                                    } 
                                    ...
                                    $html .= "</td>"; 
                                    $totalHH = $totalH1+$totalH2;
                                    $html .= "<td style='border: 1px solid #dddddd;' >".$totalHH." Hari</td>"; 
                                } 
                                $html .= "</tr>";   
                                $totalHari += $hitung;
                    } 
            ...
            //header("Content-Disposition: attachment; filename=report_pk_chief_parameter3.xls");
            echo $html;
            exit;

    }

    function saveParameter1($post){
      ...
      foreach($post as $k=>$v) {
        if (preg_match('/^terima_berkas(.*)$/', $k, $match)) {
          $terima_berkas[] = $v;
        }

        if (preg_match('/^tgl_or(.*)$/', $k, $match)) {
          $tgl_or[] = $v;
        }

        if (preg_match('/^nama_or(.*)$/', $k, $match)) {
          $nama_or[] = $v;
        }

        if (preg_match('/^nomor_or(.*)$/', $k, $match)) {
          $nomor_or[] = $v;
        }

        if (preg_match('/^klaim_bulan(.*)$/', $k, $match)) {
          $klaim_bulan[] = $v;
        }
      ...
      foreach($terima_berkas as $key=>$value) {
        foreach($value as $k=>$v) {
          $suport_data = $terima_berkas[0][$k]."||".$tgl_or[0][$k]."||".$nama_or[0][$k]."||".$nomor_or[0][$k]."||".$klaim_bulan[0][$k];
    ...
    function saveParameter3($post){
      ...
      foreach($post as $k=>$v) {
        if (preg_match('/^tgl_kirim(.*)$/', $k, $match)) {
          $tgl_kirim[] = $v;
        }
      ...
      foreach($klaim_bulan as $key=>$value) {
        foreach($value as $k=>$v) {
          $suport_data = $klaim_bulan[0][$k]."||".$tgl_kirim[0][$k];
    ...
    function supportData(){
     ...
     /* if ($iAspekId=='118'){
      ...
      switch ($vFunctionLink) {
        case 'TSPERFM_03':
          $data['DataCollect']=$this->lib_pk_analytical_development_lokal->supportData_TSPERFM_03($data);
          $html .= $this->load->view('ts/support_data/TSPERFM_03',$data, TRUE);
          break;        
        default:
          $html .= '<br />Not Found';
          break;
    ...
    function proses_approve(){
      ...
      if ($jenis==0){ //Cek Before Sumbit
        ...
        (SELECT nResult FROM hrd.pk_pkresult WHERE iPkTransId='".$id."' AND iMsGroupAspek=a.iMsGroupAspekId) AS nResult 
    ...
    function SaveHasil(){
      ...
      $this->db_erp_pk->set('tUpdated','CURRENT_TIMESTAMP',FALSE);
      $this->db_erp_pk->set('cUpdatedBy',$cNip);
      if(isset($_POST['pk_analytical_development_lokal_vEvaKry'])){
        $vEvaKry     = $_POST['pk_analytical_development_lokal_vEvaKry'];
        $this->db_erp_pk->set('vEvaKry',$vEvaKry);
      }
      if(isset($_POST['pk_analytical_development_lokal_vRencana'])){
        $vRencana     = $_POST['pk_analytical_development_lokal_vRencana'];
        $this->db_erp_pk->set('vRencana',$vRencana);
      }
      if(isset($_POST['pk_analytical_development_lokal_vEvaAtasan'])){
        $vEvaAtasan   = $_POST['pk_analytical_development_lokal_vEvaAtasan'];
        $this->db_erp_pk->set('vEvaAtasan',$vEvaAtasan);
      }
      if(isset($_POST['pk_analytical_development_lokal_vSaranAtasan'])){
        $vSaranAtasan   = $_POST['pk_analytical_development_lokal_vSaranAtasan'];
        $this->db_erp_pk->set('vSaranAtasan',$vSaranAtasan);
      }
      if(isset($_POST['pk_analytical_development_lokal_vPelatihan'])){
        $vPelatihan   = $_POST['pk_analytical_development_lokal_vPelatihan'];
        $this->db_erp_pk->set('vPelatihan',$vPelatihan);
      }
      if (isset($_POST['pk_analytical_development_lokal_iRencana'])){
          $iRencana   = $_POST['pk_analytical_development_lokal_iRencana'];
          $this->db_erp_pk->set('iRencana',$iRencana);
      }
      if (isset($_POST['pk_analytical_development_lokal_iDivIdTo'])){
          $iDivIdTo   = $_POST['pk_analytical_development_lokal_iDivIdTo'];
          $this->db_erp_pk->set('iRencana',$iDivIdTo);
      }
      if (isset($_POST['pk_analytical_development_lokal_iPostIdTo'])){
          $iPostIdTo   = $_POST['pk_analytical_development_lokal_iPostIdTo'];
          $this->db_erp_pk->set('iRencana',$iPostIdTo);
      }
      if (isset($_POST['pk_analytical_development_lokal_vPertimbangan'])){
          $vPertimbangan   = $_POST['pk_analytical_development_lokal_vPertimbangan'];
          $this->db_erp_pk->set('iRencana',$vPertimbangan);
      }
      if (isset($_POST['pk_analytical_development_lokal_nFinal'])){
          $nFinal   = $_POST['pk_analytical_development_lokal_nFinal'];
          $this->db_erp_pk->set('iRencana',$nFinal);
      }
      $this->db_erp_pk->where('id',$iPkTransId);
      $this->db_erp_pk->update('hrd.pk_trans');
    ...
    function SaveBobot(){
      //print_r($_POST);
      //exit();
      $iPkTransId         = $_POST['pk_analytical_development_lokal_id'];
      ...
      foreach($iAspekId as $key=>$value) {
        foreach($value as $k=>$v) {
          if ($GroupAspekId[0][$k]==$iMsGroupAspekId){
            $querydtl[] = "UPDATE hrd.pk_aspek_dynamic set nBobot='".$nBobot[0][$k]."'
                where id = '".$v."'";
    ...
    function SaveNilaiCSE(){
      //print_r($_POST);
      //exit();
      $iPkTransId         = $_POST['pk_analytical_development_lokal_id'];
      $iMsGroupAspekId    = $_POST['_iMsGroupAspekId'];
      $nResult            = $_POST['pk_analytical_development_lokal_nilaicse'];
      $cNip               =  $this->sess_auth->gNIP;

      //save ke PK_Result;
      $sql = "SELECT b.cNip,b.cTahun,b.iSemester,b.iPostId,b.iSkemaId,c.nBobot,(".$nResult." * c.nBobot)/100 AS nNilai 
            FROM hrd.pk_trans AS b 
            INNER JOIN hrd.pk_group_aspek AS c ON c.iSkemaId=b.iSkemaId 
                    AND c.iMsGroupAspekId='".$iMsGroupAspekId."'
            WHERE b.id='".$iPkTransId."'";
    ...
    function uploadproses(){
      ...
      $files = $_FILES;
      $realname = $files["pk_analytical_development_lokal_lampiran"]["name"];
      $newName= "PK_".$id."_".$realname;
      $newName =  str_replace("/", "_", $newName);
      $upload_dir = "files/hdsystem/".$newName;

      move_uploaded_file($files["pk_analytical_development_lokal_lampiran"]["tmp_name"], $upload_dir);
    ...
    function proses_reject(){
      ...
      $sql    = "SELECT cNip FROM hrd.pk_analytical_development_lokal
      ...
      $sql = "INSERT INTO hrd.pk_analytical_development_lokal (iSkemaId,cNip,iStatus,vReason,dDate,iUlang,lDeleted,tCreated,cCreatedBy) VALUES
    ...
    public function listbox_pk_analytical_development_lokal_dPk($value, $id,$field,$rowData) {
    ...
    public function listbox_pk_analytical_development_lokal_iPostId($value, $id,$field,$rowData) {
      $vDescription = $value;
    ...
    public function insertbox_pk_analytical_development_lokal_iPostId($field, $id) {
      $cNip   =  $this->sess_auth->gNIP;
    ...
    public function updatebox_pk_analytical_development_lokal_iPostId($field, $id,$value,$data) {
    ...
    public function insertbox_pk_analytical_development_lokal_cNip($field, $id) {
      $cNip        =  $this->sess_auth->gNIP;
      $vDescription = $cNip." - ".$this->lib_utilitas->getvName($cNip);

      $url = $this->url;
      $url6 = base_url().'processor/'.$this->urll.'?action=searchcNip';
      $url2 = base_url().'processor/'.$this->urll.'?action=searchSkema';
      $url3 = base_url().'processor/'.$this->urll.'?action=cekJabatan';
      ...
      $o = "<script type='text/javascript'>
              $(document).ready(function() {
                /*var akses = cekJabatan('pk_analytical_development_lokal_iSkemaId', '".$iPostId."');
                if (akses=='TIDAK'){
                    custom_alert('Jabatan anda tidak terdaftar untuk input Penilaian Kinerja pada module ini');
                    //$('#form_pk_analytical_development_lokal').hide();
                    return false;
                }*/

                loadSkema('pk_analytical_development_lokal_iSkemaId', '".$iPostId."');
                ...
                var config = {
                  ...
                  select: function(event, ui){
                    ...
                    loadSkema('pk_analytical_development_lokal_iSkemaId', ui.item.iPostId);
    ...
    public function updatebox_pk_analytical_development_lokal_cNip($field, $id,$value,$data) {
      $cNip         =  $value;
      $vDescription = $cNip." - ".$this->lib_utilitas->getvName($cNip);
      $url = $this->url;
      $url6 = base_url().'processor/'.$this->urll.'?action=searchcNip';
    ...
    public function insertbox_pk_analytical_development_lokal_cTahun($field, $id) {
      $year = date('Y');
      $y1 = $year - 5;
      $y2 = $year + 5;
    ...
    public function updatebox_pk_analytical_development_lokal_cTahun($field, $id,$value,$data) {
      $year = $value;
    ...
    public function insertbox_pk_analytical_development_lokal_cPeriode($field, $id) {
    ...
    public function updatebox_pk_analytical_development_lokal_cPeriode($field, $id,$value,$data) {
    ...
    public function insertbox_pk_analytical_development_lokal_iSemester($field, $id) {
      
      $url2 = base_url().'processor/'.$this->urll.'?action=cekDataPk';
      $url  = base_url()."processor/".$this->urll."?company_id=".$this->input->get('company_id')."&group_id=".$this->input->get('group_id')."&modul_id=".$this->input->get('modul_id');
    ...
    public function updatebox_pk_analytical_development_lokal_iSemester($field, $id,$value,$data) {
      $sms = array(''=>'',1=>1,2=>2);
    ...
    public function insertbox_pk_analytical_development_lokal_dPk($field, $id) {
      $now = date('d-m-Y');
    ...
    public function updatebox_pk_analytical_development_lokal_dPk($field, $id,$value,$data) {
    ...
    public function insertbox_pk_analytical_development_lokal_iProbation($field, $id) {
    ...
    public function updatebox_pk_analytical_development_lokal_iProbation($field, $id,$value,$data) {
      //1=PK Probation, tahun dan semester bisa dikosongkan.
    ...
    public function insertbox_pk_analytical_development_lokal_lampiran($field, $id) {
    ...
    public function updatebox_pk_analytical_development_lokal_lampiran($field, $id,$value,$data) {
      $action = $this->input->get('action');
      $idnya = $data['id'];
      $url = base_url().'processor/'.$this->urll.'?action=uploadproses&id='.$idnya;
      $urlhapus = base_url().'processor/'.$this->urll.'?action=hapus';
      $urlgetLampiran = base_url().'processor/'.$this->urll.'?action=getLampiran';
      $o = '';  
      $o .= '<style>
  ...
  public function insertbox_pk_analytical_development_lokal_iSkemaId($field, $id) {
    ...
    public function updatebox_pk_analytical_development_lokal_iSkemaId($field, $id,$value,$data) {
      $iPostId = $data['iPostId'];
      $url = base_url().'processor/'.$this->urll.'?action=getDetail';
  ...
  public function insertbox_pk_analytical_development_lokal_detail($field, $id) {
...
border: 1px solid aquamarine;' >
  ...
  public function updatebox_pk_analytical_development_lokal_detail($field, $id,$value,$data) {
    $idnya      = $data['iSkemaId'];
    $iPkTransId = $data['id'];
    $cNip       = $data['cNip'];
    ...
    (SELECT lDynamic FROM hrd.pk_msgroup_aspek WHERE id=a.iMsGroupAspekId) AS lDynamic
    ...
    if ($query->num_rows > 0) {
      foreach($query->result_array() as $line) {
        $group_aspek      = $line['group_aspek'];
        $iMsGroupAspekId  = $line['iMsGroupAspekId'];
        $lDynamic  = $line['lDynamic'];
        if ($iMsGroupAspekId==7){//CSE
          $o .= '<div id="'.$this->url.'_'.$iMsGroupAspekId.'">
                  <div style="background-color: white;
                      border: solid 1px aquamarine;
                      padding: 5px;">
                   '.$this->generateCse($idnya,$iMsGroupAspekId,$iPkTransId,$cNip,$data).'
                   </div>
              </div>';
          ...
          $o .= '<div id="'.$this->url.'_'.$iMsGroupAspekId.'">
                  <div style="background-color: white;
                      border: solid 1px aquamarine;
                      padding: 5px;">
                   '.$this->generateAspekDivisi($idnya,$iMsGroupAspekId,$iPkTransId,$cNip,$data,$lDynamic).'
                   </div>
              </div>';
        }
  ...
  public function updatebox_pk_analytical_development_lokal_Approval($field, $id,$value,$data) {
    $id = $data['id'];
   $o = '<div class="box_cbox">
    ...
    $sql = "SELECT cNip,iStatus,vReason,dDate 
          FROM hrd.pk_analytical_development_lokal WHERE iSkemaId='".$id."' ORDER BY id ASC";
  ...
  public function manipulate_update_button($button,$rowData) {  
    ...
    $cNipNya = $rowData['cNip'];
      ...
      $btnSave .= "<button type='button'
                ...
                onclick='javascript:update_btn_back_".$this->url."(\"".$this->url."\", \"".base_url()."processor/".$this->urll."?company_id=".$this->input->get('company_id')."&group_id=".$this->input->get('group_id')."&modul_id=".$this->input->get('modul_id')."\", this)'>Update
                </button>";


      $urlsumbit  = base_url().'processor/'.$this->urll.'?action=proses_approve';
      ...
      $btnSubmit .= "<button type='button'
                ...
                id='button_update_".$this->url."'
                class='icon-save ui-button'
                onclick='javascript:btn_submit_".$this->url."(\"".$this->url."\", \"".base_url()."processor/".$this->urll."?company_id=".$this->input->get('company_id')."&group_id=".$this->input->get('group_id')."&modul_id=".$this->input->get('modul_id')."\", this)'>Submit
                </button>";
      ...
      $btnApprove .= "<button type='button'
                ...
                id='button_update_".$this->url."'
                class='icon-save ui-button'
                onclick='javascript:btn_submit_".$this->url."(\"".$this->url."\", \"".base_url()."processor/".$this->urll."?company_id=".$this->input->get('company_id')."&group_id=".$this->input->get('group_id')."&modul_id=".$this->input->get('modul_id')."\", this)'>Approve
                </button>";
      ...
      $btnAggrement .= "<button type='button'
                ...
                id='button_update_".$this->url."'
                class='icon-save ui-button'
                onclick='javascript:btn_agree_".$this->url."(\"".$this->url."\", \"".base_url()."processor/hdsystem/pk/transaksi/divisi?company_id=".$this->input->get('company_id')."&group_id=".$this->input->get('group_id')."&modul_id=".$this->input->get('modul_id')."\", this)'>Agreement 
                </button>";
      ...
      $btn_preview  = '
                      <script type="text/javascript">
                        ...
                        function print_'.$this->url.'() {
                          ...
                          document.getElementById("iframe_preview_'.$this->url.'").src = "'.base_url().'processor/'.$this->urll.'?action=get_preview_report&id="+id+"&vName="+vName;
                          
                        }   
                      </script>
                    ';
     
    $btn_preview .= '<iframe height="0" width="0" id="iframe_preview_'.$this->url.'"></iframe>';
    $btn_preview .= '<button onclick="javascript:print_'.$this->url.'();" class="ui-button-text icon-print" id="button_preview_'.$this->url.'">Print</button>';                  
    ...
    $btnSave  =  "<script type='text/javascript'>               
                            ...
                            function save_btn_back_".$this->url."(grid, url, dis) {
                                ...
                                if ($('#".$this->url."_iSemester').val() == '') {
                                    //custom_alert('Pilih Semester');
                                    //return false;     
                                    $('#".$this->url."_iSemester').addClass('error_text');
                                    var label = $('label[for=\"".$this->url."_iSemester\"]').text();
                                    label = label.replace('*','');
                                    alert_message += '<br /><b>'+label+'</b> Lenkapi karyawan'; 
                                    tot_err++;
                                }
                                ...
                                if ($('#".$this->url."_cTahun').val() == '') {
                                    //custom_alert('Pilih Tahun');
                                    //return false;
                                    $('#".$this->url."_cTahun').addClass('error_text');
                                    var label = $('label[for=\"".$this->url."_cTahun\"]').text();
                                    label = label.replace('*','');
                                    alert_message += '<br /><b>'+label+'</b> Pilih Tahun'; 
                                    tot_err++;
                                }
                                ...
                                if ($('#".$this->url."_dPeriode1').val() == '') {
                                    //custom_alert('Pilih Periode Penilaian');
                                    //return false;
                                    $('#".$this->url."_dPeriode1').addClass('error_text');
                                    var label = $('label[for=\"".$this->url."_cPeriode\"]').text();
                                    label = label.replace('*','');
                                    alert_message += '<br /><b>'+label+'</b> Pilih Periode Penilaian'; 
                                    tot_err++;
                                }
                                ...
                                if ($('#".$this->url."_dPeriode2').val() == '') {
                                    //custom_alert('Pilih Periode Penilaian');
                                    //return false;
                                    $('#".$this->url."_dPeriode2').addClass('error_text');
                                    var label = $('label[for=\"".$this->url."_cPeriode\"]').text();
                                    label = label.replace('*','');
                                    alert_message += '<br /><b>'+label+'</b> Pilih Periode Penilaian'; 
                                    tot_err++;
                                ...
                                if ($('#".$this->url."_iSkemaId').val() == '' || $('#".$this->url."_iSkemaId').val()==0) {
                                    //custom_alert('Pilih Skema PK');
                                    //return false;
                                   $('#".$this->url."_iSkemaId').addClass('error_text');
                                    var label = $('label[for=\"".$this->url."_iSkemaId\"]').text();
                                    label = label.replace('*','');
                                    alert_message += '<br /><b>'+label+'</b> Pilih Skema PK '; 
                                    tot_err++;
                                }
        ...
        $btnSave .= "<button type='button'
                            ...
                            class='icon-save ui-button'
                            onclick='javascript:save_btn_back_".$this->url."(\"".$this->url."\", \"".base_url()."processor/".$this->urll."?company_id=".$this->input->get('company_id')."&group_id=".$this->input->get('group_id')."&modul_id=".$this->input->get('modul_id')."\", this)'>Save
                            </button>";
    $button['save'] = $btnSave;
  ...
  function generateCse($iSkemaId,$iMsGroupAspekId,$iPkTransId,$cNip,$rowData){
    $action      = $this->input->get('action');
    $nipLogin    =  $this->sess_auth->gNIP;
    ...
    $sql = "SELECT nResult FROM hrd.pk_pkresult WHERE iPkTransId='".$iPkTransId."' 
            AND iMsGroupAspek='".$iMsGroupAspekId."' ";
    $nResult = '';        
    $query = $this->db_erp_pk->query($sql);
    if ($query->num_rows() > 0) {
        $row = $query->row();
    ...
    $html .= ' 
            <tr style="border: 1px solid #dddddd; border-collapse: collapse;">
                 ...
                 <td  style="border: 1px solid #dddddd; width: 90%;text-align: left">
                      <input type="text" name="'.$this->url.'_nilaicse" id="'.$this->url.'_nilaicse" size=5 value="'.$nResult.'" />
                 </td>
            </tr>';

    $html .= '</table>';

    $tombol = '<br /><a  href="javascript:void(0);" onclick="simpanPointCse('.$iMsGroupAspekId.')" class="icon-save ui-button ui-widget ui-state-default ui-corner-all ui-button-text-icon-primary">Simpan CSE</a>';

    if ($action=='view'){
        $tombol = '';
        $html   = '<b>NILAI CSE '.$nResult.'</b>';
    }
    ...
    $url = base_url().'processor/'.$this->urll.'?action=SaveNilaiCSE';
    $rUrl = base_url()."processor/".$this->urll."?company_id=".$this->input->get('company_id')."&group_id=".$this->input->get('group_id')."&modul_id=".$this->input->get('modul_id');

    $html .= "<script>
                $('#".$this->url."_nilaicse').autoNumeric('init',  {vMin:'-999.99', vMax:'999.99'});

              function simpanPointCse(iMsGroupAspekId){
                    
                  var iPkTransId = '".$iPkTransId."';
                    var nResult = $('#pk_analytical_development_lokal_nilaicse').val();
                    if (nResult==''){
                        custom_alert('Isi Nilai CSE');
                        return false;
                    }
                  $.ajax({
                     type: 'POST',
  ...
  function generateAspekDivisi($iSkemaId,$iMsGroupAspekId,$iPkTransId,$cNip,$rowData,$lDynamic){
    $action      = $this->input->get('action');
    $nipLogin    =  $this->sess_auth->gNIP;
    $aybs        = $this->lib_utilitas->getSingleAybs($cNip);
    ...
    if($lDynamic){
      $sql = "SELECT a.id,a.iSkemaId,a.iMsGroupAspekId,a.nBobot,
          (SELECT vDescription FROM hrd.pk_msgroup_aspek WHERE id=a.iMsGroupAspekId) AS group_aspek,
          (SELECT COALESCE(SUM(nBobot),0) FROM hrd.pk_aspek_dynamic WHERE iSkemaId=a.iSkemaId AND iMsGroupAspekId=a.iMsGroupAspekId AND iPkTransId='".$iPkTransId."') AS sum_bobot
          FROM hrd.pk_group_aspek AS a WHERE a.lDeleted=0 and a.iSkemaId='".$iSkemaId."' AND
          iMsGroupAspekId='".$iMsGroupAspekId."' ";
    ...
    if($lDynamic){
      $sql = "SELECT a.id,a.iSkemaId,a.iMsGroupAspekId,a.iUrut,a.vAspekName,a.nBobot,b.nBobot as bobot_saved,b.nPointKry,b.nPointAybs,b.ySource,
            ...
            FROM hrd.pk_aspek_dynamic as a
            ...
            AND a.lDeleted=0
            and a.iPkTransId='".$iPkTransId."' order by iUrut ASC";
    }else{
      $sql = "SELECT a.id,a.iSkemaId,a.iMsGroupAspekId,a.iUrut,a.vAspekName,a.nBobot,b.nBobot as bobot_saved,b.nPointKry,b.nPointAybs,b.ySource,
            b.nPointSepakat,b.nNilai,a.lAutoCalculation,a.vFunctionLink,a.iSupportData
    ...
    $empty=0;
      ...
      if ($iSupportData==1){
        if(substr($vFunctionLink,0,7)=='TSPERFM'){//Modify Support Data Function Button
          $html .= '<td colspan="3"  style="border: 1px solid #dddddd; width: 90%;text-align: center">
                      <span onClick="'.$this->url.'_input_supoortData_ts('.$iAspekId.','.$iPkTransId.')" class="icon-add ui-button ui-widget ui-state-default ui-corner-all ui-button-text-icon-primary">Support Data</span>
                 </td>';
        }else{
          $html .= '<td colspan="3"  style="border: 1px solid #dddddd; width: 90%;text-align: center">
        ...
        $html .= '<td colspan="3"  style="border: 1px solid #dddddd; width: 90%;text-align: left">
         ...
         $html .= '<td rowspan="6" style="border: 1px solid #dddddd; width: 5%;text-align: center">';
                    ...
                    if ($iStatus==3 && in_array($nipLogin, $manager)  && $lAutoCalculation==0){
      ...
      if($lDynamic) {
          $sqld = "SELECT iAspekId,vDescription,nPoint,
            (SELECT a.cWarna FROM hrd.pk_warna AS a WHERE a.iPoint=hrd.pk_aspek_dynamic_detail.nPoint ) AS cWarna
            FROM hrd.pk_aspek_dynamic_detail WHERE iAspekId='" . $iAspekId . "' AND ldeleted=0 ORDER BY nPoint DESC";
      }else{
          $sqld = "SELECT iAspekId,vDescription,nPoint,
          ...
          if ($lAutoCalculation==1 && $line['nPoint']==100){
            ...
            $html .='<td '.$rowspan.' style="width:10%;text-align: center;border: 1px solid #dddddd;">
                 ...
                 $html .= '<td  style="width:10%;text-align: center;border: 1px solid #dddddd;">
                          ...
                          <input '.$diskry.' type="radio" name="'.$this->url.'_nPointKry_'.$iAspekId.'[]" id="'.$this->url.'_nPointKry_'.$iAspekId.'" value="'.$line['nPoint'].'" class="'.$this->url.'_nPointKry" '.$selKry.' onClick="'.$this->url.'_pilihRadio('.$iAspekId.','.$nBobot.','.$nPoint.',0)"  />
                        </td>';
             ...
             $html .= '
                        ...
                        <td style="width:10%;text-align: center;border: 1px solid #dddddd;">
                          ...
                          onClick="'.$this->url.'_pilihRadioAybs('.$iAspekId.','.$nBobot.','.$nPoint.',0)" />
                        </td>
    ...
    $html .= ' </tbody>
                <tfoot>
                    ...
                    <td style="border: 1px solid #dddddd; width: 15%;text-align: center" >
                     ...
                     <td style="border: 1px solid #dddddd; width: 15%;text-align: center" colspan="8">';
                        $vlink = 'generatePO';
                    $tombolGenerate = '<a  href="javascript:void(0);" onClick="simpanDetail(\''.$vlink.'\','.$iMsGroupAspekId.')" class="icon-save ui-button ui-widget ui-state-default ui-corner-all ui-button-text-icon-primary">Generate '.ucwords(strtolower($group_aspek)).'</a>';
     ...
     $urlSD = base_url().'processor/'.$this->urll.'?action=supportData';
     $urlgetValuePk = base_url().'processor/'.$this->urll.'?action=getValuePk';
      $urlgetPerformanceObjective = base_url().'processor/'.$this->urll.'?action=getPerformanceObjective';
     $urlDetail = base_url().'processor/'.$this->urll.'?action=showDetail';
     $url = base_url().'processor/'.$this->urll.'?action=SaveNilai';
     $urlBobot = base_url().'processor/'.$this->urll.'?action=SaveBobot';
     $rUrl = base_url()."processor/".$this->urll."?company_id=".$this->input->get('company_id')."&group_id=".$this->input->get('group_id')."&modul_id=".$this->input->get('modul_id');

     $html .= "<script>

      $(document).ready(function() {
        $('.pk_analytical_development_lokal_nPointSepakat').autoNumeric('init', {vMin:'0', vMax:'100'});
     
      });

      $('.pk_analytical_development_lokal_nPointSepakat').live('keyup', function() {
        var ix = $('.pk_analytical_development_lokal_nPointSepakat').index($(this));
      ...
      $('.pk_analytical_development_lokal_nPointSepakat').live('blur', function() {
            var ix = $('.pk_analytical_development_lokal_nPointSepakat').index($(this));
            ...
            var pointKry    = parseFloat($('#".$this->url."_tmpPilih_'+iAspekId+'').val());
            var pointAybs   = parseFloat($('#".$this->url."_tmpAybs_'+iAspekId+'').val());
            var sepakat     = (pointKry+pointAybs)/2;
            var max = 100;
            var min = 20;
            if (pointKry > pointAybs){
                max = pointKry;
                min = pointAybs;
            }else{
                max = pointAybs;
            ...
            if (parseFloat(point) < min){
                custom_alert('Minimal Point Sepakat '+min);
                $('.".$this->url."_nPointSepakat').eq(ix).val(min);
                return false;
            }

           
            if (parseFloat(point) > max){
                custom_alert('Maksimal Point Sepakat '+max);
                $('.".$this->url."_nPointSepakat').eq(ix).val(max)
                return false;
      ...
      function ".$this->url."_pilihRadio(iAspekId,bobot,point, flag=0){
          var nilai  = (parseFloat(point) * parseFloat(bobot))/100;
          $('#".$this->url."_nNilai_'+iAspekId+'').val(nilai.toFixed(2));
          $('#".$this->url."_groupnilai_'+iAspekId+'').val(nilai.toFixed(2));
          $('#".$this->url."_tmpPilih_'+iAspekId+'').val(point);
      ...
      function ".$this->url."_pilihRadioAybs(iAspekId,bobot,point,flag=0){
        ...
        $.ajax({
           ...
           success: function(data)
           {
                custom_alert(data);
                 $.get('".$rUrl."&action=update&id='+iPkTransId, function(data) {
                      $('div#form_".$this->url."').html(data);
                      var index = $('#pk_analytical_development_lokal_list_detail a[href=\'#pk_analytical_development_lokal_'+iMsGroupAspekId+'\']').parent().index();
                      $('#pk_analytical_development_lokal_list_detail').tabs('option', 'active', index);                  
                  });
      ...
      function simpanDetail(vFunctionLink,iMsGroupAspekId){

        var iPkTransId = '".$iPkTransId."';
        var nip = $('#pk_analytical_development_lokal_cNip').val();
        var dPeriode1 = $('#".$this->url."_dPeriode1').val();
        var dPeriode2 = $('#".$this->url."_dPeriode2').val();
        var iPkTransId =  '".$iPkTransId."';
        var iSkemaId = '".$rowData['iSkemaId']."';
        $.ajax({
           type: 'POST',
           url: '".$urlgetPerformanceObjective."',
           data: $('#form_update_".$this->url."').serialize()+'&_iMsGroupAspekId='+iMsGroupAspekId+'&_vFunctionLink='+vFunctionLink+'&_cNipNya='+nip+'&_dPeriode1='+dPeriode1+'&_dPeriode2='+dPeriode2+'&_iPkTransId='+iPkTransId+'&_iSkemaId='+iSkemaId,
           success: function(data)
           {
                //custom_alert(data);
                 $.get('".$rUrl."&action=update&id='+iPkTransId, function(data) {
      ...
      function ".$this->url."_hitungPk(vFunctionLink,iAspekId,tampil,bobot){
          ...
          document.getElementById('vDescription_pk_analytical_development_lokal_'+iAspekId+'_100').style.backgroundColor='#ffffff';
          document.getElementById('vDescription_pk_analytical_development_lokal_'+iAspekId+'_80').style.backgroundColor='#ffffff';
          document.getElementById('vDescription_pk_analytical_development_lokal_'+iAspekId+'_60').style.backgroundColor='#ffffff';
          document.getElementById('vDescription_pk_analytical_development_lokal_'+iAspekId+'_40').style.backgroundColor='#ffffff';
          document.getElementById('vDescription_pk_analytical_development_lokal_'+iAspekId+'_20').style.backgroundColor='#ffffff';

          document.getElementById('vDescription_pk_analytical_development_lokal_'+iAspekId+'_'+point).style.backgroundColor = color;
      ...
      function ".$this->url."_getValuePk(vFunctionLink,iAspekId){
        ...
        return $.ajax({
            type: 'POST', 
            url: '".$urlgetValuePk."',
            data: '_vFunctionLink='+vFunctionLink+'&_iAspekId='+iAspekId+'&_cNipNya='+nip+'&_dPeriode1='+dPeriode1+'&_dPeriode2='+dPeriode2+'&_iPkTransId='+iPkTransId+'&_iSkemaId='+iSkemaId,
            async:false
        }).responseText
      ...
      function ".$this->url."_input_supoortData_ts(iAspekId,iPkTransId){
        load_popup_pk_analytical_development_lokal_insert('".$urlSD."&iAspekId='+iAspekId+'&iPkTransId='+iPkTransId+'&getaction=".$this->input->get('action')."','Support Data');
      }

     </script>";
     $data['grid']='pk_analytical_development_lokal';    
     //$html.=$this->load->view('func_tambahan_pk_analytical_development_lokal_js',$data);
      return $html;
  }
  ...
  function generateHasilAkhir($iPkTransId,$cNip,$rowData){
    ...
    $cNipNya = $rowData['cNip'];
    $listaybs =  $this->lib_utilitas->getAybs($cNipNya,$tmpNip=array());
    $disabled=$this->input->get('action')!='view'?'':'disabled="disabled"';
    $manager = $listaybs;
    $iStatus     = $rowData['iStatus'];
    ...
    $jumlah = 0;
     ...
     $html .= '<br /><table cellspacing="0" cellpadding="3">
      ...
      if($nipLogin==$cNip){
        if($iStatus==0){
          $disaybs=' disabled="disabled"';
          $disybs = '';
        }else{
          $disaybs=' disabled="disabled"';
          $disybs =' disabled="disabled"';
        }
      }else{
        if(in_array($nipLogin, $manager)){
          if($iStatus>=1){
            $disaybs='';
            $disybs =' disabled="disabled"';
          }else{
              ...
              <thead>
                ...
                <tr style="border: 1px solid #dddddd; border-collapse: collapse;">
                        <td style="border: 1px solid #dddddd" > 
                          ...
                          <br />
                          <textarea style="width:98%;" id="'.$this->url.'_vEvaKry" name="'.$this->url.'_vEvaKry" '.$disabled.$disybs.' >'.$row->vEvaKry.'</textarea>
                 ...
                 <tr style="border: 1px solid #dddddd; border-collapse: collapse;">
                        <td style="border: 1px solid #dddddd" > 
                          ...
                          <br />
                           <textarea style="width:98%;" id="'.$this->url.'_vRencana" name="'.$this->url.'_vRencana" '.$disabled.$disybs.' >'.$row->vRencana.'</textarea>
              ...
              <thead>
                ...
                <tr style="border: 1px solid #dddddd; border-collapse: collapse;">
                        <td style="border: 1px solid #dddddd" > 
                          Evaluasi Umum
                          <br />
                           <textarea style="width:98%;" id="'.$this->url.'_vEvaAtasan" name="'.$this->url.'_vEvaAtasan" '.$disabled.$disaybs.' >'.$row->vEvaAtasan.'</textarea>
                 ...
                 <tr style="border: 1px solid #dddddd; border-collapse: collapse;">
                        <td style="border: 1px solid #dddddd" > 
                          Saran/Perbaikan
                          <br />
                           <textarea style="width:98%;" id="'.$this->url.'_vSaranAtasan" name="'.$this->url.'_vSaranAtasan" '.$disabled.$disaybs.' >'.$row->vSaranAtasan.'</textarea>
                 ...
                 <tr style="border: 1px solid #dddddd; border-collapse: collapse;">
                        <td style="border: 1px solid #dddddd" > 
                          ...
                          <br />
                           <textarea style="width:98%;" id="'.$this->url.'_vPelatihan" name="'.$this->url.'_vPelatihan" '.$disabled.$disaybs.' >'.$row->vPelatihan.'</textarea>
                ...
                <tr style="border: 1px solid #dddddd; border-collapse: collapse;">
                        <td width="25%" rowspan="2" style="border: 1px solid #dddddd" > 
                          ...
                          foreach ($arr_iRencana as $key => $value) {
                            ...
                            $html .=' <input '.$sel.' type="radio" id="'.$this->url.'_iRencana" name="'.$this->url.'_iRencana" value="'.$key.'" '.$disabled.$disaybs.' />'.$value.'<br />';
                          }
                       ...
                       $html .=' </td>
                         <td style="border: 1px solid #dddddd" > 
                          ...
                          $html  .= '<select name="'.$this->url.'_iDivIdTo" id="'.$this->url.'_iDivIdTo" '.$disabled.$disaybs.'>';
                          ...
                          $html  .= '<select name="'.$this->url.'_iPostIdTo" id="'.$this->url.'_iPostIdTo" '.$disabled.$disaybs.'>';
                          $html  .= "<option value='0'>Pilih</option>";
                           $query = $this->db_erp_pk->query($sql);
                          if ($query->num_rows() > 0) {
                              $result = $query->result_array();
                 ...
                 <tr style="border: 1px solid #dddddd; border-collapse: collapse;">
                        <td style="border: 1px solid #dddddd" > 
                          Pertimbangan</br>
                           <textarea style="width:98%;" id="'.$this->url.'_vPertimbangan" name="'.$this->url.'_vPertimbangan" '.$disabled.$disaybs.' >'.$row->vPertimbangan.'</textarea>
                        </td>
    ...
    $urlHasil = base_url().'processor/'.$this->urll.'?action=SaveHasil';
    $rUrl = base_url()."processor/".$this->urll."?company_id=".$this->input->get('company_id')."&group_id=".$this->input->get('group_id')."&modul_id=".$this->input->get('modul_id');
    ...
    $html .= "<script>
                ...
                $(document).ready(function() {  
                  $('#pk_analytical_development_lokal_iDivIdTo').change(function() {
                      groupdeptid = getGroupDeptId($(this).val());
                      loadDataJbt('pk_analytical_development_lokal_iPostIdTo', groupdeptid, groupdeptid);
  ...
  public function searchcNip() {
    ...
    $data = array();
    //$divGa = $this->lib_utilitas->getSysParam('DIV_PK_SDM');
    $jabatan  =  $this->lib_utilitas->getSysParam("JABATAN_pk_analytical_development_lokal");
    ...
    $sql = "SELECT a.cNip,a.vName,a.dRealIn,a.iPostId,
        ...
        AND a.iCompanyId !=1 AND  a.iDivisionID in (".$DIV_PK.") and
  ...
  function getDetail(){
      ...
      $html .= "<script>
                  $(document).ready(function() {
                    ...
                    $('.pk_analytical_development_lokal_nResult').autoNumeric('init', {vMin:'0', vMax:'100.00'});
                  });


                  $('.pk_analytical_development_lokal_nResult').live('keyup', function() {
                    var ix = $('.pk_analytical_development_lokal_nResult').index($(this));
                    
                    var sisa_sblm   = 0;
                    var result  = $('.pk_analytical_development_lokal_nResult').eq(ix).val();
                    var bobot   = $('.pk_analytical_development_lokal_nBobot').eq(ix).val();
                    var nilai   = parseFloat((result * bobot) / 100);

                    $('.pk_analytical_development_lokal_nNilai').eq(ix).val(nilai.toFixed(2));
                    $('.nama_pk_analytical_development_lokal_nNilai').eq(ix).html(nilai.toFixed(2));
                  ...
                  function sum_".$this->url."(){
                    ...
                    $('.pk_analytical_development_lokal_nResult').each(function() {
                      if ($('.pk_analytical_development_lokal_nResult').eq(i).val() != '') {
                        total += parseFloat($('.pk_analytical_development_lokal_nNilai').eq(i).val());
                    ...
                    $('#pk_analytical_development_lokal_jumlah').html(total.toFixed(2));
                    $('#pk_analytical_development_lokal_nFinal').val(total.toFixed(2));
...
FROM hrd.pk_pkresult AS a WHERE a.iPkTransId='".$iPkTransId."' ";
      ...
      $html .= "<script>
                  $(document).ready(function() {
                    ...
                    $('.pk_create_manager_nResult').autoNumeric('init', {vMin:'0', vMax:'100.00'});
                  });
    ...
    function getValuePk(){
      ...
      $this->lib_pk_analytical_development_lokal->$vFunctionLink($_POST);
    }
    
    function getPerformanceObjective(){
        $vFunctionLink = $_POST['_vFunctionLink'];
        $this->lib_pk_analytical_development_lokal->$vFunctionLink($_POST);
    }
