...
class lib_pk_pd_spv_steril_nbl_201
{
    private $_ci;
    private $sess_auth;
    private $db_erp_pk;
    ...
    /*===============Start Function=======================*/ 
    function PD_SPV_STERIL_NBL_201_01($post)    {
         $noRow = 0;
        $totalJamAll = 0;
        $totalReqAll = 0;
        $tmpData = array();
        ...
        $sql2 ="select a.c_panumb,a.c_panumb1,a.c_iteno,b.c_itnam,a.c_type,b.c_toll,d.c_nmfloor,b.l_import,e.c_nmkateg,a.d_plandate
                ...
                and d.c_floor IN ('0002') # lantai 2
        ...
        $html .= "<table align='left' cellspacing='0' cellpadding='3' style='width: 300px;'>
                    ...
                    <tr style='border: 1px solid #dddddd; border-collapse: collapse;'>
                        <td style='width:150px;text-align: left;border: 1px solid #dddddd;'>(x) Total MBR steril close</td>
                        
                        <td style='width:100px;text-align: right;border: 1px solid #dddddd;'>".$totA."</td>
                    </tr>

                    <tr style='border: 1px solid #dddddd; border-collapse: collapse;'>
                        <td style='width:10%;text-align: left;border: 1px solid #dddddd;'>(y) Total MBR Steril</td> 
                    ...
                    <tr style='border: 1px solid #dddddd; border-collapse: collapse;'>
                        <td style='width:10%;text-align: left;border: 1px solid #dddddd;'>Persentase Produk (x/y * 100%)</td>
    ...
    function PD_SPV_STERIL_NBL_201_08($post)    {
        $noRow = 0;
        $totalJamAll = 0;
        $totalReqAll = 0;
        $tmpData = array();
        ...
        foreach ($batal as $b) {  
            ...
            if(array_key_exists($stringXo,$simpanXperiode)){ 
                $simpanXperiode[$stringXo]++;
            }else{
                $simpanXperiode[$stringXo]=1;
            }
        ...
        foreach ($turun as $t) {  
            ...
            if(array_key_exists($stringYo,$simpanYperiode)){ 
                $simpanYperiode[$stringYo]++;
            }else{
                $simpanYperiode[$stringYo]=1;
            }
            // $simpanYperiode[$stringYo]++; 
        }
        ...
        */
        ...
        $htmlxy = "<table align='left' cellspacing='0' cellpadding='3' style='width: 100%;'>
                        ...
                        if($cek_val!=$tgl_skr->format("Y-m")){
                            ...
                            if(isset($simpanXperiode[$tgl0])){
                                if(!empty($simpanXperiode[$tgl0]) || $simpanXperiode[$tgl0]!=0){
                                    ...
                                    $arrayXX[$tgl0]=$simpanXperiode[$tgl0];
                                }else{
                        ...
                        if($cek_val!=$tgl_skr->format("Y-m")){
                            ...
                            if(isset($simpanYperiode[$tgl0])){
                                if(!empty($simpanYperiode[$tgl0]) || $simpanYperiode[$tgl0]!=0){
                                    ...
                                    $arrayYY[$tgl0]=$simpanYperiode[$tgl0];
                                }else{
                                    $htmlxy .= "<td style='width:10%;text-align: right;border: 1px solid #dddddd;'>0</td>";
                                    $arrayYY[$tgl0]=0;
                            } 
         ...
         $htmlxy .= "</tr><tr><td style='width:2%;text-align: left;border: 1px solid #dddddd;'>(x/y)</td> ";
            $tgl_skr = new DateTime($perode2, new DateTimeZone("Europe/London"));
            $tgl_lalu = new DateTime($perode1, new DateTimeZone("Europe/London"));
            $cek_val ="";
            $hasil=0;
            do { 
                if($cek_val!=$tgl_skr->format("Y-m")){
                    $tgl = explode('-',$tgl_skr->format("Y-m"));  
                    $tgl0 = $tgl[0].$tgl[1];
                    $nilaixx=isset($arrayXX[$tgl0])?$arrayXX[$tgl0]:0;
                    $nilaiyy=isset($arrayYY[$tgl0])?$arrayYY[$tgl0]:0;
                    $nilaidetfin=0;
                    if($nilaixx>0 && $nilaiyy>0){
                        $nilaidetfin=$nilaixx/$nilaiyy;
                    }
                    $hasil=$hasil+$nilaidetfin;
                    $htmlxy .= "<td style='width:10%;text-align: right;border: 1px solid #dddddd;'>".number_format($nilaidetfin ,2)."</td>";
                }
                $cek_val=$tgl_skr->format("Y-m");  
                $tgl_skr->modify('-1 month');
            } while ($tgl_skr >= $tgl_lalu);
            $hasil= ($hasil/$bulan)*100;
