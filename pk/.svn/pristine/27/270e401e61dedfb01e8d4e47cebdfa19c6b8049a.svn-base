...
<table cellspacing='0' width="99%" class="table"> <!-- cellspacing='0' is important, must stay -->
	...
	<thead>
		<tr>
			...
			<th rowspan="2" colspan="2">Sikap Kerja</th>
	...
	$sql_j="select * from pk.pk_master ma where ma.idmaster_id=".$id." and ma.ldeleted=0";
	...
	$sql1="select * from pk.pk_nilai ni
		...
		where ni.idmaster_id=".$id." and pa.ikategori_id=1 and pa.ldeleted=0 and ni.lDeleted=0 and pa.iteam_id=".$teamid;
	...
	foreach ($dtni as $key => $val) {
		$sql2="select * from pk.pk_parameter_detail padet
			...
			where padet.iparameter_id=".$val['iparameter_id']." and padet.ldeleted=0 and padet.kategori_id=1 and kat.ldeleted=0";
		...
		echo "<td rowspan='".$r."'>".number_format($val['bobot']*$val['poin_sepakat_atasan'],2)."</td>";
		echo "</tr>";
		...
		$nilaitot[]=number_format($val['bobot']*$val['poin_sepakat_atasan'],2);
		$i++;
	}
	?>
	<tr style="padding:10px;border-top:1px solid #fafafa;border-bottom:1px solid #e0e0e0;border-left: 1px solid #e0e0e0;text-align: center;font-weight:bold;">
		<td colspan="7">Nilai akhir Aspek Sikap Kerja</td>
