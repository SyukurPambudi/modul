<?php
	...
	foreach ($dkat as $k) {
         ...
         */           
        if($idkat == 4){
    		$sql="select (ni.poin_sepakat_atasan) * pa.bobot as nilai from pk.pk_nilai ni
		...
		$jml=$this->dbset->query($sql)->result_array();
		$n=array();
		foreach ($jml as $jl => $j) {
			$n[]=$j['nilai'];
		}
		$naspek=array_sum($n);
		$s=floatval($k["bobot_sdm"])*floatval($naspek);
		$ni[]=$s;
		$o.='<tr>';
		$o.='<td>'.$k["kategori"].'</td>';
	...
	$o .= '<div style="overflow:auto;" class="rows_group"><label class="rows_label" for="tcatatan_umum_atasan">Evaluasi umum</label><div class="rows_input"><textarea name="tcatatan_umum_atasan" id="tcatatan_umum_atasan" class="tab_note" style="width:500px" >'.$dcat["tcatatan_umum_atasan"].'</textarea></div></div>';
	$o .= '<div style="overflow:auto;" class="rows_group"><label class="rows_label" for="tcatatan_saran_atasan">Saran/Perbaikan</label><div class="rows_input"><textarea name="tcatatan_saran_atasan" id="tcatatan_saran_atasan" class="tab_note" style="width:500px" >'.$dcat["tcatatan_saran_atasan"].'</textarea></div></div>';
	$o .= '<div style="overflow:auto;" class="rows_group"><label class="rows_label" for="tcatatan_pelatihan_atasan">Pelatihan yang diusulkan</label><div class="rows_input"><textarea name="tcatatan_pelatihan_atasan" id="tcatatan_pelatihan_atasan" class="tab_note" style="width:500px" >'.$dcat["tcatatan_pelatihan_atasan"].'</textarea></div></div>';
...
if ($this->input->get('action') == 'view') {
 		...
 		if($dcat["iSubmit"]<=3){
	 		echo '<button type="button" id="draft_tab_last_atasan" class="ui-button-text icon-save ui-button ui-widget ui-state-default ui-corner-all ui-button-text-icon-primary">Update as Draft</button>';
	 		echo '<button type="button" id="submit_draft_tab_last_atasan" class="ui-button-text icon-save ui-button ui-widget ui-state-default ui-corner-all ui-button-text-icon-primary">Update & Submit</button>';
	 	}
	 	}?>

<script type="text/javascript">
	$('#submit_draft_tab_last_atasan').die();
	$('#submit_draft_tab_last_atasan').live('click',function(){
		$.ajax({
			url: base_url+'processor/pk/pk_softdev_mgr?action=updatenote_ats',
			type: 'post',
			...
			success: function(data) {
				...
				$.get(base_url+'processor/pk/pk/softdev/mgr?action=update&foreign_key=0&company_id='+company_id+'&idnya='+o.last_id+'&id='+o.last_id+'&group_id='+group_id+'&modul_id='+modul_id, function(data) {
	...
	$('#draft_tab_last_atasan').die();
	$('#draft_tab_last_atasan').live('click',function(){
		$.ajax({
			url: base_url+'processor/pk/pk_softdev_mgr?action=updatenote_ats',
			type: 'post',
