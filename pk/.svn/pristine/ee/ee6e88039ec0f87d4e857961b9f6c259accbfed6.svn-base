...
//echo $mydept.'ini teamnya';
		$o='';
			...
			if($type==0){
				...
				where a.ldeleted=0 and a.ikategori_id=1 and kat.ldeleted=0 and a.iteam_id=".$teamid;
			}else{
				$sqlp='select * from pk.pk_nilai ni
						inner join pk.pk_parameter pa on ni.iparameter_id=pa.iparameter_id
						where ni.idmaster_id='.$this->input->get('id').' and pa.ikategori_id=1 and pa.iteam_id='.$teamid;
			}
		...
		if($queryp->num_rows()>0) {
			...
			$o .= '<tr>';
			$o .= '<th rowspan="2">I</th>';
			$o .= '<th rowspan="2" colspan="2">SIKAP KERJA</th>';
			...
			foreach($rows as $key => $row) {
				...
				$is_auto = $row['is_auto'];
				$bobot = $row['bobot'];
				$o.='<tr style="background-color:#EFEFEF;"><th width="10px">'.$nomor.'</th><th colspan="2">'.$parameter.'</th><th colspan="2"></th><th></th><th></th><th></th>';
				$o .='<input type="hidden" value="'.$iparameter_id.'" id="req_tab1_'.$iparameter_id.'" class="required_par_tab1">';
				$o .='</tr>';
				//Cek Nilai;
				$cek_nilai ="select * from pk.pk_nilai ni
				inner join pk.pk_parameter pa on ni.iparameter_id=pa.iparameter_id
				where pa.ikategori_id=1 and pa.ldeleted=0 and ni.ldeleted=0 and ni.iparameter_id=".$iparameter_id." and ni.idmaster_id=".$this->input->get('id')." LIMIT 1";
				...
				if($querypd->num_rows()>0) {
					...
					$parpar='<input id="parameter_'.$iparameter_id.'" class="parameters tabs1" type="hidden"  name="iparameter_id[]" value="'.$iparameter_id.'">';
					$hashas='<input id="hasil_'.$iparameter_id.'" class="hasils tabs1" type="hidden" name="hasil_calc[]" value="'.$nilai.'">';
					$poipoi='<input id="poin_'.$iparameter_id.'" class="poins tabs1" type="hidden" name="poin[]" value="'.$nilai.'">';
					...
					foreach($rows_ as $key_ =>$row_) {
						...
						$o.='<tr bgcolor="'.$bgcolor.'">
							...
							<input type="radio" id="mpoin_'.$iparameter_id.'"  name="poin[]_'.$iparameter_id.'" value="'.$poin.'"  onclick="calculate_all_rincian_hitung_tab1($(this),\''.$vFunction.'\',\''.$iparameter_id.'\',\''.$poin.'\',\''.$mcalc.'\');" '.$check.'>';
						$o.='<script>';
						$o.='$(function() {
					...
					$o.='</table></td>';
					$o.='<td align="center"><input type="hidden" value="'.$pk.'" id="poin_karyawan_'.$iparameter_id.'" class="tabs1" name="poin_karyawan_'.$iparameter_id.'" /><p id="lbl_poin_karyawan_'.$iparameter_id.'">'.$pk.'</p></td>';
					$o.='<td align="center"><input type="hidden" value="'.$ps.'" id="poin_pimpinan_'.$iparameter_id.'" class="tabs1" name="poin_pimpinan_'.$iparameter_id.'" /><p id="lbl_poin_pimpinan_'.$iparameter_id.'">'.$ps.'</p></td>';
					if($qt['vnip']==$vnip){
						$o.='<td align="center"><input type="hidden" value="'.$po.'" id="sepakat_'.$iparameter_id.'" class="tabs1" name="sepakat_'.$iparameter_id.'" /><p id="lbl_sepakat_'.$iparameter_id.'">'.$po.'</p></td>';
					}else{
						$o.='<td align="center"><input type="text" value="'.$po.'" id="sepakat_'.$iparameter_id.'" class="tabs1" name="sepakat_'.$iparameter_id.'" /><p id="lbl_sepakat_'.$iparameter_id.'"></td>';
						$o.='<script>';
					...
					$o.='<td align="center"><input type="hidden" value="'.$bobot.'" id="bobot_'.$iparameter_id.'" class="tabs1" name="bobot_'.$iparameter_id.'" />'.$bo.' %</td>';
					$o.='<td align="center"><input type="hidden" value="" id="nilai_'.$iparameter_id.'" class="tabs1" name="nilai_'.$iparameter_id.'" /><p id="lbl_nilai_'.$iparameter_id.'">'.$ni.'</p></td>';
					$o.='</tr>';
				}
			$nomor++;	
			}
			...
			$o.='';
			$o.= 'function calculate_all_rincian_hitung_tab1(o, vFunction,iparameter_id,mPoin,mCalc) {
						...
						success: function(data) {
							...
							if(!data.error) {
								...
								$o.='if (data.is_auto == 1) {
									...
									var url2 = "'.base_url().'processor/pk/pk_ts?action=calculate_bobot";
 ...
 <div><?php
	...
	$ca='<button type="button" id="submit_tab1" class="ui-button-text icon-save ui-button ui-widget ui-state-default ui-corner-all ui-button-text-icon-primary">Save</button>';
 	 if($type==0){
 	 	if($iSubmit==0){
 	 		echo $ca;
 	 	}
...
<script type="text/javascript">
	...
	$('#submit_draft_tab1').die();
	$('#submit_draft_tab1').live('click', function(){
		...
		var it = $("#it").val();
		return $.ajax({
			url: BASE_URL+'processor/pk/pk_ts?action=sikap_calc',
			type: 'post',
			//data: 'parameter='+iparameter_id+'&hasil_calc='+hasil_calc+'&poin='+poin,
			data: $('input.tabs1').serialize()+'&idmaster_id='+idmaster_id+'&iSubmit=1'+'&company_id='+company_id+'&group_id='+group_id+'&modul_id='+modul_id+'&itim='+itim_id+'&it='+it,
			beforeSend: function(xhr, opts) {
					var req = $(".required_par_tab1");
					var conf=0;
...
//					}
	...
	$('#submit_tab1').die();
	$('#submit_tab1').live('click', function(){
		var iparameter_id = $('input.parameters').serialize();
		var hasil_calc = $('input.hasils').serialize();
		var poin = $('input.poins').serialize();
		var BASE_URL = "<?php echo base_url();?>";
		...
		return $.ajax({
			...
			type: 'post',
			//data: 'parameter='+iparameter_id+'&hasil_calc='+hasil_calc+'&poin='+poin,
			data: $('input.tabs1').serialize()+'&idmaster_id='+idmaster_id+'&iSubmit=2'+'&company_id='+company_id+'&group_id='+group_id+'&modul_id='+modul_id+'&itim='+itim_id+'&it='+it,
			beforeSend: function(xhr, opts) {
					var req = $(".required_par_tab1");
					var conf=0;
					$.each(req, function(i,v){
