<?php
	...
	foreach ($dkat as $k) {
		...
		if($k["ikategori_id"]==2){
			$sql="select (ni.poin/2) * pa.bobot as nilai from plc2.pk_nilai ni
			...
			$sql="select ni.poin_sepakat as nilai from plc2.pk_nilai ni
				inner join plc2.pk_parameter pa on ni.iparameter_id=pa.iparameter_id
...
<script type="text/javascript">
	...
	$('#submit_draft_tab_last_karyawan').live('click',function(){
		$.ajax({
			url: base_url+'processor/pk/pk_busdev?action=updatenote',
			type: 'post',
			...
			success: function(data) {
				...
				$.get(base_url+'processor/pk/pk/busdev?action=update&foreign_key=0&company_id='+company_id+'&idnya='+o.last_id+'&id='+o.last_id+'&group_id='+group_id+'&modul_id='+modul_id, function(data) {
