...
class lib_pk_ass_manager_produksi_nbl_1_2
{
    private $_ci;
    private $sess_auth;
    private $db_erp_pk;
    ...
    function ASC_NBL12_P_01($post)    {
        ...
        $date1 = date($perode1);
        $newdate1 = strtotime ( '-1 months' , strtotime ( $date1 ) ) ;
        $perode1 = date ( 'Y-m-21' , $newdate1 );

        $perode2 = $x_prd2['2']."-".$x_prd2['1']."-20";
        ...
        $html .= "<table>
                    <tr>
                        <td><h4><b>Total RPB solid, Liquid, Softgel, dan Steril. (X)</b></td>
                    </tr>
                    <tr>
                        <td><p>Penarikan MBR langsung 6 bulan, dengan periode penarikan dari tanggal 21 di awal bulan semester, hingga tanggal 20 di akhir bulan semester, contoh untuk semester 1 penarikan dari tanggal 21 desember 2016 hingga tanggal 20 juni 2017</p></td>
                    </tr>
                </table>";
       ...
       /* $sql1 ="select a.c_panumb,a.c_panumb1,a.c_iteno,b.c_itnam, re.d_packing
        ...
        $html .= "<table align='left' cellspacing='0' cellpadding='3' style='width: 600px;'>
    ...
    function ASC_NBL12_P_07x($post)    {
        ...
        $html .= "<table cellspacing='0' cellpadding='3' width='850px'>
                    <tr>
                        <td><b>Jumlah Produk yang di DN Batal Turun (dengan yg membatalkan dari PRODUKSI)</b></td>
                    </tr>
                    </table>";
        $html .= "<table cellspacing='0' cellpadding='3' width='850px'>
                <tr style='width:100%; border: 1px solid #f86609; background: #b5f2a6; border-collapse: collapse;text-align: center;'>
        ...
        $sqlBatalProd = "SELECT r.id, r.d_dndate , r.d_datent, r.c_dnnumb, i.c_iteno,i.c_itnam, r.c_batchno, r.c_status, r.c_remark FROM sales.recent r
                 ...
                 JOIN purchase.pamsh p on r.c_batchno = p.c_panumb  
                     ...
                     and r.n_dncode = 4 and r.l_canmbr = 1 
                ORDER by r.d_dndate";
        ...
        $sqlJumlahTurun = "SELECT r.id, r.c_period, r.c_iteno, i.c_itnam, r.n_optbatch,r.c_userid, r3.n_realprod FROM mrp.rpb r 
        ...
        foreach ($turun as $t) {  
             ...
             $html .= "<tr style='border: 1px solid #dddddd; border-collapse: collapse;'>
                        ...
                        <td style='border: 1px solid #dddddd;' >".$t['c_period']."</td>  
                      </tr>"; 
        }
        $html .= "</table>";
        /*
        ...
        */
        ...
        $html .= "<table align='left' cellspacing='0' cellpadding='3' style='width: 850px;'>
                    ...
                    <tr style='border: 1px solid #dddddd; border-collapse: collapse;'>
                        <td style='width:10%;text-align: left;border: 1px solid #dddddd;'>(x)Jumlah Produk yang di DN Batal Turun (dengan yg membatalkan dari PPIC)</td> 
                    ...
                    <tr style='border: 1px solid #dddddd; border-collapse: collapse;'>
                        <td style='width:10%;text-align: left;border: 1px solid #dddddd;'>Persentase MBR (a/b * 100%) per bulan</td>
    ...
    function ASC_NBL12_P_07xx($post)    {
        $noRow = 0;
        $totalJamAll = 0;
        $totalReqAll = 0;
        $tmpData = array();
        ...
        $tgl1 = $x_prd1['2'].$x_prd1['1'];
        $tgl2 = $x_prd2['2'].$x_prd2['1'];
        $sqlJumlahTurun = "SELECT r.id, left(r.c_period,4) as perTahun, right(r.c_period,2) as perBulan, r.c_iteno, i.c_itnam, r.n_optbatch,r.c_userid, r3.n_realprod FROM mrp.rpb r 
                JOIN sales.itemas i on i.c_iteno = r.c_iteno
                JOIN mrp.rpb03 r3 on r3.c_iteno = r.c_iteno 
                where (i.c_export<>'Y' or i.l_import=0) and 
                i.c_status='A' and r3.n_realprod>0 
    ...
    function ASC_NBL12_P_07($post)    {
        ...
        $sqlBatalProd = "SELECT d.c_nmfloor, r.id, r.d_dndate , r.d_datent, r.c_dnnumb, i.c_iteno,i.c_itnam, r.c_batchno, r.c_status, r.c_remark FROM sales.recent r
                 ...
                 join purchase.floor d on d.c_floor=p.c_lokasi   
                     WHERE r.d_dndate >= '".$perode1."' and r.d_dndate <= '".$perode2."' and
                     i.l_tolletc <>1 and p.c_jnsmbr = 1   
                     and r.n_dncode = 1 and r.l_canmbr = 1 and d.c_floor='0002'
                ORDER by r.d_dndate";
        $batal = $this->db_erp_pk->query($sqlBatalProd)->result_array(); 
        $x = 0 ;
