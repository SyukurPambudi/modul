...
//echo $mydept.'ini teamnya';
		$o='';
			...
			if($type==0){
				...
				where a.ldeleted=0 and a.ikategori_id=3 and kat.ldeleted=0 and a.iteam_id=".$teamid;
			}else{
				$sqlp='select * from pk.pk_nilai ni
						inner join pk.pk_parameter pa on ni.iparameter_id=pa.iparameter_id
						where ni.idmaster_id='.$this->input->get('id').' and pa.ikategori_id=3 and pa.iteam_id='.$teamid;
			}
            //echo $sqlp;
		...
		if($queryp->num_rows()>0) {
			...
			foreach($rows as $key => $row) {
				...
				if($querypd->num_rows()>0) {
					...
					if($qt['vnip']==$vnip){
						$o.='<td align="center"><input type="hidden" value="'.$po.'" id="sepakat_'.$iparameter_id.'" class="tabs3" name="sepakat_'.$iparameter_id.'" /><p id="lbl_sepakat_'.$iparameter_id.'">'.$po.'</p></td>';
					}else{
						$o.='<td align="center"><input type="text" value="'.$po.'" id="sepakat_'.$iparameter_id.'" class="tabs3" name="sepakat_'.$iparameter_id.'" /><p id="lbl_sepakat_'.$iparameter_id.'"></td>';
						$o.='<script>';
					...
					$o.='<td align="center"><input type="hidden" value="" id="nilai_'.$iparameter_id.'" class="tabs3" name="nilai_'.$iparameter_id.'" /><p id="lbl_nilai_'.$iparameter_id.'">'.$ni.'</p></td>';
					$o.='</tr>';
				}
			$nomor++;	
			}
...
<script type="text/javascript">
	...
	$('#submit_draft_tab3').live('click', function(){
		...
		var it = $("#it").val();
		return $.ajax({
			url: BASE_URL+'processor/pk/pk_softdev_mgr?action=pimpin_calc',
			type: 'post',
