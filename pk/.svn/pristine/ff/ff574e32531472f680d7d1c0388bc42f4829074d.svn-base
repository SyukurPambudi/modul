...
<table cellspacing='0' width="98%" class="tablelast"> <!-- cellspacing='0' is important, must stay -->
	...
	<tbody>
		...
		foreach ($dkat as $k) {
			...
			echo '<tr>';
			echo '<td>'.$k["kategori"].'</td>';
			echo '<td align="center">'.$bobot.'%</td>';
			echo '<td align="center">'.floatval($j).'</td>';
			echo '<td align="center">'.number_format((floatval($bo)*floatval($j)),2).'</td>';
			echo '</tr>';
		}
		$n_final=array_sum($n);
	...
	$dcat=$this->db->query($qcat)->row_array();?>
		...
		<div id="catatan_atasan">
			...
			<p>Komentar/Evaluasi Umum</p><textarea name="tcatatan_umum_pengaju" id="tcatatan_umum_pengaju" class="" style="width:500px"><?php echo $dcat["tcatatan_umum_pengaju"] ?></textarea>
			<p>Rencana untuk periode yang akan datang</p><textarea name="tcatatan_rencana_pengaju" id="tcatatan_rencana_pengaju" class="" style="width:500px"><?php echo $dcat["tcatatan_rencana_pengaju"] ?></textarea>
		

		<p style="font-weight:bold">CATATAN UNTUK PIMPINAN:</p>
		<p>Evaluasi umum</p><textarea name="tcatatan_umum_atasan" id="tcatatan_umum_atasan" class="" style="width:500px"><?php echo  $dcat["tcatatan_umum_atasan"] ?></textarea>
		<p>Saran/Perbaikan</p><textarea name="tcatatan_saran_atasan" id="tcatatan_saran_atasan" class="" style="width:500px"><?php echo $dcat["tcatatan_saran_atasan"] ?></textarea>
		<p>Pelatihan yang diusulkan</p><textarea name="tcatatan_pelatihan_atasan" id="tcatatan_pelatihan_atasan" class="" style="width:500px"><?php echo $dcat["tcatatan_pelatihan_atasan"] ?></textarea>
		</div>
		<div id="catatan_atasan">
			...
			<tr>
				...
				<td style="vertical-align:top;border:1px solid #c5dbec;">
					...
					<p style="font-size:10px;margin-top:5px;margin-left:10px;font-weight: bold;">Posisi:
						<?php $que = "select * from hrd.position pos where pos.lDeleted=0 and pos.iPostId=".$dcat['iPostId'];
						$dt2=$this->db->query($que)->row_array();
						 echo $dt2['vDescription']; ?>
					</p>
					<p style="font-size:10px;margin-top:5px;margin-left:10px;font-weight: bold;">Departemen:
					<?php $que1 = "select * from hrd.msdepartement dep where dep.lDeleted=0 and dep.iDeptID=".$dcat['iDeptId'];
					$dt1 = $this->db->query($que1)->row_array();
...
<table width="100%" style="padding-left:10px;text-align:left" id="catatan_atasan">
	<tr>
		...
		<td width="50%">Atasan Langsung,</td>
	</tr>
	<tr height="100px" style="vertical-align:bottom;">
		<td width="50%" style="text-align:center"><br><br><br><br></td>
		<td width="50%" style="text-align:center"><br><br><br><br></td>
	</tr>
	<tr>
	<?php
	$npengusul=$dcat['vnip'];
	$sql='select * from hrd.employee em where em.cNIp="'.$npengusul.'"';
	$dpeng=$this->db->query($sql)->row_array();
	$natas=$dpeng['cUpper'];
	$sql='select * from hrd.employee em where em.cNIp="'.$natas.'"';
	$dupper=$this->db->query($sql)->row_array();
	?>
		<td width="50%"><div style="border-top:1px solid black; width:300px;"><?php echo "(".$npengusul." - ".$dpeng['vName'].")" ?></div></td>
		<td width="50%"><div style="border-top:1px solid black; width:300px;"><?php echo "(".$natas." - ".$dupper['vName'].")" ?></div></td>
	</tr>
</table>
