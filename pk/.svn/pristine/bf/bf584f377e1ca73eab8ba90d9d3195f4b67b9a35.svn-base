	...
	<tbody>
			...
			if($rcount>=1){
				foreach ($dataall as $kall => $vall) {
					...
					$date1=date_create($vall['dinput_applet']);
					...
					echo "<td>".date('Y-m-d',strtotime($vall['tregistrasi']))."</td>";
					echo "<td>".$selisih."</td>";
					echo "<tr>"; 
					$i++;
					$tot_upb++;
...
?>
<label class="lbl_detail">Pemerataan file pra registrasi per divisi (Jumlah Selisih / Jumlah UPB) : <?php echo $rata ?> Bulan</label>