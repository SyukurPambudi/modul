...
class browse_pk_ts extends MX_Controller {
    ...
    function index($action = '') {
		...
		switch ($action) {		
			...
			case 'confirm':
				...
				$sql="UPDATE pk.pk_master SET iSubmit=5, is_confirm=1, is_dinilai='".$post['ifinal']."', tcatatan_umum_atasan='".$post['tcatatan_umum_atasan']."', tcatatan_saran_atasan='".$post['tcatatan_saran_atasan']."',tcatatan_pelatihan_atasan='".$post['tcatatan_pelatihan_atasan']."', 
				iDeptId='".$post['iDeptID']."'".$ikarir.$uppostid.", tPertimbangan='".$post['tpertimbangan']."' WHERE  idmaster_id=".$post['ipk'];
...
public function viewform($id=0,$nipnya=0){
		...
		$qdkat="select * from pk.pk_kategori kat where kat.ldeleted=0 and kat.iPostId=".$qnip['iPostID'];
		...
		foreach ($dkat as $k) {
			...
			$o.='<td align="center">'.number_format(floatval($k["bobot"])*floatval($j),2).'</td>';
			$o.='</tr>';
		}
		$n_final=array_sum($n);
		$o .="<tr>";
		...
		$o .= '</form>';
		$o .= '</div>';
		$o .= '<button onclick="javascript:setuju(\'pk_ts\', \''.base_url().'processor/pk/browse/pk/ts?action=confirm&last_id='.$id.'&foreign_key='.$this->input->get('foreign_key').'&company_id='.$this->input->get('company_id').'&group_id='.$this->input->get('group_id').'&modul_id='.$this->input->get('modul_id').'\')" class="ui-button-text icon-save" id="button_save_pk_ts">Confirm</button>';
		$o .="<script>
			$('#iDeptID').change(function(){
				var val=$(this).val();
				url=base_url+'processor/pk/browse/pk/ts?action=getPosisi';
				if(val==''){
					var row ='';
					$('#iPostId').html(row);
					return false;
			...
			function setuju(grid, url){
				...
				//cek mail;
				$.ajax({
					url:base_url+'processor/pk/browse/pk/ts?action=cekpass',
					type:'post',
							...
							custom_confirm('Anda Yakin?',function(){
								$.ajax({
									...
									success: function(data) {
										...
										$.get(base_url+'processor/pk/pk/ts?action=view&id='+o.last_id+'&foreign_key='+o.foreign_id+'&company_id='+o.company_id+'&group_id='+o.group_id+'&modul_id='+o.modul_id, function(data) {
