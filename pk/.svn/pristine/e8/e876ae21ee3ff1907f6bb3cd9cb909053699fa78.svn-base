	...
	<tbody>
			...
			if($rcount>=1){
				foreach ($dataall as $kall => $vall) {
					...
					$date1=date_create($vall['tsubmit_prareg']);
					$date2=date_create($vall['ttanggal']);

					/*$diff =  $date1->diff($date2);
		    		$months = $diff->y * 12 + $diff->m + $diff->d / 30;*/

		    		$months=$controller->calc_bd3->selisihbulan($date1,$date2);
			...
			?>
		<tr>
			<td colspan="5">Jumlah Selisih</td>
			<td><?php echo $tsel ?></td>
		</tr>
	</tbody>
</table>
...
?>
<label class="lbl_detail">Rata-rata Jangka waktu antara UPB dan submit pre-registrasi (dihitung sejak UPB hingga tanggal diterimanya file oleh BPOM)  : <?php echo $rata ?> Bulan</label>