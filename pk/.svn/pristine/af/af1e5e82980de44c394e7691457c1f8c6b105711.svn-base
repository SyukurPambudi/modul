...
class lib_pk_pd3_asc {
    ...
    /*New Request By Assa*/
      function PD3_ASC_getParameter8($post){
     ...
     function PD3_ASC_getParameter7($post){
    ...
    /*AD N14615 Start*/
        function PD3_ASC_getParameter9($post){
            ...
            //DDS
            $sql2 ="SELECT pu.iteampd_id,pu.`iupb_id`,pu.vupb_nomor,pu.vupb_nama,pu.ikategoriupb_id, pu.imaster_delivery, pu.tconfirm_dok_pd FROM plc2.plc2_upb pu 
                    WHERE pu.`ldeleted` = 0 
                    #AND pu.`iteampd_id` = 2 
                    AND pu.iteampd_id = 2
                    AND pu.`itipe_id` NOT IN(6) 
            ...
            if(!empty($b)){
                foreach ($b as $v) {
                    ...
                    $k='';
                    $sqlk ="SELECT u.`vteam` FROM plc2.`plc2_upb_team` u WHERE u.`ldeleted`=0 AND  u.`iteam_id` = '".$v['iteampd_id']."'";
                    $kat = $this->db_erp_pk->query($sqlk)->row_array();
                    if(empty($kat['vteam'])){
                        $k = '-';
                    }else{
    ...
    /*AD N14615 End*/
     ...
     /*AD ARIES-------------------------------------------------------------------*/
    function PD3_ASC_getParameter6($post){
        ...
        $html .= "<table cellspacing='0' cellpadding='3' width='100%'>
            <tr style='width:100%; border: 1px solid #f86609; background: #b5f2a6; border-collapse: collapse;text-align: center;'>
                ...
                <td style='border: 1px solid #dddddd;' ><b>Nama UPB </b></td>
                <td style='border: 1px solid #dddddd;' ><b>Kategori UPB</b></td>
                <td style='border: 1px solid #dddddd;' ><b>Tanggal Aproval <br>Cek Dok. Prareg oleh PD</b></td>  
            </tr>
        ";
    ...
    function PD3_ASC_getParameter4($post){
    ...
    function PD3_ASC_getParameter5($post){
    ...
    /*AD N15748 Start*/
    function PD3_ASC_getParameter1($post){
        $iAspekId = $post['_iAspekId'];
        $cNipNya  = $post['_cNipNya'];
        //$cNipNya = 'N09484';
        $dPeriode1  = $post['_dPeriode1'];
    ...
    function PD3_ASC_getParameter2($post){ 
        $iAspekId = $post['_iAspekId'];
        $cNipNya  = $post['_cNipNya'];
        //$cNipNya = 'N09484';
        $dPeriode1  = $post['_dPeriode1'];
        ...
        $sqlto ="select u.dconfirm_registrasi_pd,u.* from plc2.plc2_upb u
            where u.ldeleted=0 #ora di hapus
            and u.ihold=0 #ora di hold
            and u.iteampd_id = 2 #team AD Ulujami
            and u.itipe_id not in (6) #group e Non OTC
            and u.dconfirm_registrasi_pd is not NULL #tanggal approve pd ora kosong
            and u.dconfirm_registrasi_pd !='0000-00-00' #tanggal approve pd ora kosong
            and u.dconfirm_registrasi_pd between '".$perode1."' AND '".$perode2."'"; 
    ...
    function PD3_ASC_getParameter3($post){ 
        $iAspekId = $post['_iAspekId'];
        $cNipNya  = $post['_cNipNya'];
        //$cNipNya = 'N09484';
        $dPeriode1  = $post['_dPeriode1'];
        ...
        $dbSql2 = $this->db_erp_pk->query($sql2, array($perode1, $perode2, $perode1, $perode2));
        $b = $dbSql2->result_array();
