<?php if ( ! defined('BASEPATH')) exit('No direct script access allowed');
    class pk_pd extends MX_Controller {
  private $sess_auth;
  private $db_erp_pk;
    ...
    /*$skemaPK = $this->lib_utilitas->getSysParam('SKEMA_PK_PD_MANAGER');

    $DIV_PK_PD = $this->lib_utilitas->getSysParam('DIV_PK_PD');
    $grid->setQuery('hrd.employee.iDivisionID in ('.$DIV_PK_PD.')',null);
    ...
    function saveSupportData(){
      ...
      switch ($parameter) {
        case 'PD1_getParameter6':
          $this->savePD1_getParameter6($_POST);
          break;

        case 'PD1_getParameter7':
          $this->savePD1_getParameter7($_POST);
          break;
    ...
    function savePD1_getParameter6($post){
    ...
    function savePD1_getParameter7($post){
      $iPkTransId = $post['_iPkTransId'];
      $iAspekId   = $post['_iAspekId'];
      $suport_data = $post['klaim_bulan'];
    ...
    function supportData(){
      ...
      switch ($vFunctionLink) {
        case 'PD1_getParameter6':
            $html .= $this->load->view('pd/support_data/PD1_getParameter6',$data, TRUE);
          break;

        case 'PD1_getParameter7':
            $html .= $this->load->view('pd/support_data/PD1_getParameter7',$data, TRUE);
          break;
    ...
    function SaveHasil(){
      ...
      $vEvaKry      = $_POST['pk_pd_vEvaKry'];
      $vRencana     = $_POST['pk_pd_vRencana'];
      $vEvaAtasan   = $_POST['pk_pd_vEvaAtasan'];
      $vSaranAtasan = $_POST['pk_pd_vSaranAtasan'];
      $vPelatihan   = $_POST['pk_pd_vPelatihan'];
      

      if (isset($_POST['pk_create_mekanik_engine_iRencana'])){
          $iRencana     = $_POST['pk_pd_iRencana'];
      }else{
          $iRencana = '';
      }

      $iDivIdTo     = $_POST['pk_pd_iDivIdTo'];
      $iPostIdTo     = $_POST['pk_pd_iPostIdTo'];
      $vPertimbangan = $_POST['pk_pd_vPertimbangan'];
      $nFinal        = $_POST['pk_pd_nFinal'];
    ...
    function uploadproses(){
      ...
      $files = $_FILES;
      $realname = $files["pk_pd_lampiran"]["name"];
      $newName= "PK_".$id."_".$realname;
      $newName =  str_replace("/", "_", $newName);
      $upload_dir = "files/hdsystem/".$newName;

      move_uploaded_file($files["pk_pd_lampiran"]["tmp_name"], $upload_dir);
    ...
    function cekJabatan(){
        ...
        $x_jabpk = explode(",", $this->lib_utilitas->getSysParam('JABATAN_PK_PD_MGR'));
    ...
    function proses_reject(){
      ...
      $sql    = "SELECT cNip FROM hrd.pk_pd 
      ...
      $sql = "INSERT INTO hrd.pk_pd (iSkemaId,cNip,iStatus,vReason,dDate,iUlang,lDeleted,tCreated,cCreatedBy) VALUES
    ...
    public function listbox_pk_pd_dPk($value, $id,$field,$rowData) {
    ...
    public function listbox_pk_pd_iPostId($value, $id,$field,$rowData) {
      $vDescription = $value;
    ...
    public function insertbox_pk_pd_iPostId($field, $id) {
    ...
    public function updatebox_pk_pd_iPostId($field, $id,$value,$data) {
    ...
    public function insertbox_pk_pd_cNip($field, $id) {
      $cNip        =  $this->sess_auth->gNIP;
      $vDescription = $cNip." - ".$this->lib_utilitas->getvName($cNip);

      $url = $this->url;
      $url6 = base_url().'processor/pk/pk/pd?action=searchcNip';
      $url2 = base_url().'processor/pk/pk/pd?action=searchSkema';
      $url3 = base_url().'processor/pk/pk/pd?action=cekJabatan';
      ...
      $o = "<script type='text/javascript'>
              $(document).ready(function() {
                ...
                var akses = cekJabatan('pk_pd_iSkemaId', '".$iPostId."');
                if (akses=='TIDAK'){
                ...
                loadSkema('pk_pd_iSkemaId', '".$iPostId."');
                ...
                var config = {
                  ...
                  select: function(event, ui){
                    ...
                    loadSkema('pk_pd_iSkemaId', ui.item.iPostId);
    ...
    public function updatebox_pk_pd_cNip($field, $id,$value,$data) {
      $cNip         =  $value;
      $vDescription = $cNip." - ".$this->lib_utilitas->getvName($cNip);
      $url = $this->url;
      $url6 = base_url().'processor/pk/pk/pd?action=searchcNip';
    ...
    public function insertbox_pk_pd_cTahun($field, $id) {
      $year = date('Y');
      $y1 = $year - 5;
      $y2 = $year + 5;
    ...
    public function updatebox_pk_pd_cTahun($field, $id,$value,$data) {
      $year = $value;
    ...
    public function insertbox_pk_pd_cPeriode($field, $id) {
    ...
    public function updatebox_pk_pd_cPeriode($field, $id,$value,$data) {
    ...
    public function insertbox_pk_pd_iSemester($field, $id) {
      
      $url2 = base_url().'processor/pk/pk/pd?action=cekDataPk';
      $url  = base_url()."processor/pk/pk/pd?company_id=".$this->input->get('company_id')."&group_id=".$this->input->get('group_id')."&modul_id=".$this->input->get('modul_id');
    ...
    public function updatebox_pk_pd_iSemester($field, $id,$value,$data) {
      $sms = array(''=>'',1=>1,2=>2);
    ...
    public function insertbox_pk_pd_dPk($field, $id) {
      $now = date('d-m-Y');
    ...
    public function updatebox_pk_pd_dPk($field, $id,$value,$data) {
    ...
    public function insertbox_pk_pd_iProbation($field, $id) {
    ...
    public function updatebox_pk_pd_iProbation($field, $id,$value,$data) {
      //1=PK Probation, tahun dan semester bisa dikosongkan.
    ...
    public function insertbox_pk_pd_lampiran($field, $id) {
    ...
    public function updatebox_pk_pd_lampiran($field, $id,$value,$data) {
      $action = $this->input->get('action');
      $idnya = $data['id'];
      $url = base_url().'processor/pk/pk/pd?action=uploadproses&id='.$idnya;
      $urlhapus = base_url().'processor/pk/pk/pd?action=hapus';
      $urlgetLampiran = base_url().'processor/pk/pk/pd?action=getLampiran';
      $o = '';  
      $o .= '<style>
  ...
  public function insertbox_pk_pd_iSkemaId($field, $id) {
    ...
    public function updatebox_pk_pd_iSkemaId($field, $id,$value,$data) {
      $iPostId = $data['iPostId'];
      $url = base_url().'processor/pk/pk/pd?action=getDetail';
  ...
  public function insertbox_pk_pd_detail($field, $id) {
...
border: 1px solid aquamarine;' >
  ...
  public function updatebox_pk_pd_detail($field, $id,$value,$data) {
    $idnya      = $data['iSkemaId'];
    $iPkTransId = $data['id'];
    $cNip       = $data['cNip'];
  ...
  public function updatebox_pk_pd_Approval($field, $id,$value,$data) {
    $id = $data['id'];
   $o = '<div class="box_cbox">
    ...
    $sql = "SELECT cNip,iStatus,vReason,dDate 
          FROM hrd.pk_pd WHERE iSkemaId='".$id."' ORDER BY id ASC";
  ...
  public function manipulate_update_button($button,$rowData) {  
    ...
    $cNipNya = $rowData['cNip'];
      ...
      $btnSave .= "<button type='button'
                ...
                onclick='javascript:update_btn_back_".$this->url."(\"".$this->url."\", \"".base_url()."processor/pk/pk/pd?company_id=".$this->input->get('company_id')."&group_id=".$this->input->get('group_id')."&modul_id=".$this->input->get('modul_id')."\", this)'>Update 
                </button>";


      $urlsumbit  = base_url().'processor/pk/pk/pd?action=proses_approve';
      ...
      $btnSubmit .= "<button type='button'
                ...
                id='button_update_".$this->url."'
                class='icon-save ui-button'
                onclick='javascript:btn_submit_".$this->url."(\"".$this->url."\", \"".base_url()."processor/pk/pk/pd?company_id=".$this->input->get('company_id')."&group_id=".$this->input->get('group_id')."&modul_id=".$this->input->get('modul_id')."\", this)'>Sumbit 
                </button>";
      ...
      $btnApprove .= "<button type='button'
                ...
                id='button_update_".$this->url."'
                class='icon-save ui-button'
                onclick='javascript:btn_submit_".$this->url."(\"".$this->url."\", \"".base_url()."processor/pk/pk/pd?company_id=".$this->input->get('company_id')."&group_id=".$this->input->get('group_id')."&modul_id=".$this->input->get('modul_id')."\", this)'>Approve 
                </button>";
      ...
      $btn_preview  = '
                      <script type="text/javascript">
                        ...
                        function print_'.$this->url.'() {
                          ...
                          document.getElementById("iframe_preview_ga_'.$this->url.'").src = "'.base_url().'processor/pk/pk/pd?action=get_preview_report&id="+id+"&vName="+vName;
  ...
  public function manipulate_insert_button($button) {
    ...
    $btnSave  =  "<script type='text/javascript'>               
        ...
        $btnSave .= "<button type='button'
                            ...
                            class='icon-save ui-button'
                            onclick='javascript:save_btn_back_".$this->url."(\"".$this->url."\", \"".base_url()."processor/pk/pk/pd?company_id=".$this->input->get('company_id')."&group_id=".$this->input->get('group_id')."&modul_id=".$this->input->get('modul_id')."\", this)'>Save
                            </button>";
    $button['save'] = $btnSave;
...
/*
    ...
    $html .= ' </tbody>
     ...
     $urlSD = base_url().'processor/pk/pk/pd?action=supportData';
     $urlgetValuePk = base_url().'processor/pk/pk/pd?action=getValuePk';
     $urlDetail = base_url().'processor/pk/pk/pd?action=showDetail';
     $url = base_url().'processor/pk/pk/pd?action=SaveNilai';
     $rUrl = base_url()."processor/pk/pk/pd?company_id=".$this->input->get('company_id')."&group_id=".$this->input->get('group_id')."&modul_id=".$this->input->get('modul_id');

     $html .= "<script>

      $(document).ready(function() {
        $('.pk_pd_nPointSepakat').autoNumeric('init', {vMin:'0', vMax:'100'});
     
      });

      $('.pk_pd_nPointSepakat').live('keyup', function() {                
        var ix = $('.pk_pd_nPointSepakat').index($(this));
      ...
      $('.pk_pd_nPointSepakat').live('blur', function() {  
            var ix = $('.pk_pd_nPointSepakat').index($(this));
      ...
      function ".$this->url."_hitungPk(vFunctionLink,iAspekId,tampil,bobot){
          ...
          document.getElementById('vDescription_pk_pd_'+iAspekId+'_100').style.backgroundColor='#ffffff';
          document.getElementById('vDescription_pk_pd_'+iAspekId+'_80').style.backgroundColor='#ffffff';
          document.getElementById('vDescription_pk_pd_'+iAspekId+'_60').style.backgroundColor='#ffffff';
          document.getElementById('vDescription_pk_pd_'+iAspekId+'_40').style.backgroundColor='#ffffff';
          document.getElementById('vDescription_pk_pd_'+iAspekId+'_20').style.backgroundColor='#ffffff';

          document.getElementById('vDescription_pk_pd_'+iAspekId+'_'+point).style.backgroundColor = color;
      ...
      function ".$this->url."_getValuePk(vFunctionLink,iAspekId){
        ...
        var nip = $('#pk_pd_cNip').val();
        var dPeriode1 = $('#pk_pd_dPeriode1').val();
        var dPeriode2 = $('#pk_pd_dPeriode2').val();
        return $.ajax({
            type: 'POST', 
            url: '".$urlgetValuePk."',
  ...
  function generateHasilAkhir($iPkTransId,$cNip,$rowData){
    ...
    $urlHasil = base_url().'processor/pk/pk/pd?action=SaveHasil';
    $rUrl = base_url()."processor/pk/pk/pd?company_id=".$this->input->get('company_id')."&group_id=".$this->input->get('group_id')."&modul_id=".$this->input->get('modul_id');
    ...
    $html .= "<script>
                ...
                $(document).ready(function() {  
                  $('#pk_pd_iDivIdTo').change(function() {
                      groupdeptid = getGroupDeptId($(this).val());
                      loadDataJbt('pk_pd_iPostIdTo', groupdeptid, groupdeptid);
  ...
  public function searchcNip() {
    ...
    $data = array();
    $divGa = $this->lib_utilitas->getSysParam('DIV_PK_PD');
    $jabatan  =  $this->lib_utilitas->getSysParam("JABATAN_PK_PD_MGR");
  ...
  function getDetail(){
      ...
      $html .= "<script>
                  $(document).ready(function() {
                    ...
                    $('.pk_pd_nResult').autoNumeric('init', {vMin:'0', vMax:'100.00'});
                  });


                  $('.pk_pd_nResult').live('keyup', function() {            
                    var ix = $('.pk_pd_nResult').index($(this));
                    
                    var sisa_sblm   = 0;
                    var result  = $('.pk_pd_nResult').eq(ix).val();
                    var bobot   = $('.pk_pd_nBobot').eq(ix).val();
                    var nilai   = parseFloat((result * bobot) / 100);

                    $('.pk_pd_nNilai').eq(ix).val(nilai.toFixed(2));
                    $('.nama_pk_pd_nNilai').eq(ix).html(nilai.toFixed(2));
                  ...
                  function sum_".$this->url."(){
                    ...
                    $('.pk_pd_nResult').each(function() {
                      if ($('.pk_pd_nResult').eq(i).val() != '') {        
                        total += parseFloat($('.pk_pd_nNilai').eq(i).val());
                    ...
                    $('#pk_pd_jumlah').html(total.toFixed(2));
                    $('#pk_pd_nFinal').val(total.toFixed(2));
...
FROM hrd.pk_pkresult AS a WHERE a.iPkTransId='".$iPkTransId."' ";
    ...
    function getValuePk(){
      ...
      $this->lib_pk_pd->$vFunctionLink($_POST);
    }
