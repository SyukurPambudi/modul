<?php if ( ! defined('BASEPATH')) exit('No direct script access allowed');
    class pk_chief_qc extends MX_Controller {
  private $sess_auth;
  private $db_erp_pk;
    function __construct() {
        ...
        $this->url = 'pk_chief_qc';
        //$this->load->helper('encrypt');
        $this->load->library('lib_utilitas');
        $this->load->library('lib_pk_chief_qc');
    ...
    function index($action = '') {
      ...
      $grid = new Grid;
      $grid->setTitle('PK Chief QC');
      $grid->setTable('hrd.pk_trans');
      $grid->setUrl('pk_chief_qc');
    ...
    $skemaPK = $this->lib_utilitas->getSysParam('SKEMA_PK_CHIEF_QC');

    $DIV_PK_CHIEF_QC = $this->lib_utilitas->getSysParam('DIV_PK_CHIEF_QC');
    $grid->setQuery('hrd.employee.iDivisionID in ('.$DIV_PK_CHIEF_QC.')',null);
    ...
    function saveSupportData(){
      ...
      switch ($parameter) {
        case 'AD_getParameter9':
          $this->saveAD_getParameter9($_POST);
          break;
        case 'AD_getParameter3':
          $this->saveAD_getParameter3($_POST);
          break;
    ...
    function download_excel(){
      ...
      $sql_par5 = "SELECT TIMESTAMPDIFF( DAY, p.d_datecon, s1.d_datecon ) AS selisih,
                  ...
                  and s1.iCompanyID=3
                  and im.iCompanyID=3
                  AND s1.d_datecon BETWEEN '".$periode1."' AND '".$periode2."' ";
    ...
    function saveAD_getParameter9($post){
    ...
    function supportData(){
      ...
      switch ($vFunctionLink) {
        case 'AD_getParameter9':
            $html .= $this->load->view('pd/support_data/AD_getParameter9',$data, TRUE);
          break;
        case 'AD_getParameter3':
            $html .= $this->load->view('pd/support_data/AD_getParameter3',$data, TRUE);
          break;
    ...
    function SaveHasil(){
      ...
      $vEvaKry      = $_POST['pk_chief_qc_vEvaKry'];
      $vRencana     = $_POST['pk_chief_qc_vRencana'];
      $vEvaAtasan   = $_POST['pk_chief_qc_vEvaAtasan'];
      $vSaranAtasan = $_POST['pk_chief_qc_vSaranAtasan'];
      $vPelatihan   = $_POST['pk_chief_qc_vPelatihan'];


      if (isset($_POST['pk_create_mekanik_engine_iRencana'])){
          $iRencana     = $_POST['pk_chief_qc_iRencana'];
      }else{
          $iRencana = '';
      }

      $iDivIdTo     = $_POST['pk_chief_qc_iDivIdTo'];
      $iPostIdTo     = $_POST['pk_chief_qc_iPostIdTo'];
      $vPertimbangan = $_POST['pk_chief_qc_vPertimbangan'];
      $nFinal        = $_POST['pk_chief_qc_nFinal'];
    ...
    function SaveNilai(){
      //print_r($_POST);
      //exit();
      $iPkTransId         = $_POST['pk_chief_qc_id'];
    ...
    function uploadproses(){
      ...
      $files = $_FILES;
      $realname = $files["pk_chief_qc_lampiran"]["name"];
      $newName= "PK_".$id."_".$realname;
      $newName =  str_replace("/", "_", $newName);
      $upload_dir = "files/hdsystem/".$newName;

      move_uploaded_file($files["pk_chief_qc_lampiran"]["tmp_name"], $upload_dir);
    ...
    function cekJabatan(){
        ...
        $x_jabpk = explode(",", $this->lib_utilitas->getSysParam('JABATAN_PK_CHIEF_QC'));
    ...
    function proses_reject(){
      ...
      $sql    = "SELECT cNip FROM hrd.pk_chief_qc
      ...
      $sql = "INSERT INTO hrd.pk_chief_qc (iSkemaId,cNip,iStatus,vReason,dDate,iUlang,lDeleted,tCreated,cCreatedBy) VALUES
    ...
    public function listbox_pk_chief_qc_dPk($value, $id,$field,$rowData) {
    ...
    public function listbox_pk_chief_qc_iPostId($value, $id,$field,$rowData) {
      $vDescription = $value;
    ...
    public function insertbox_pk_chief_qc_iPostId($field, $id) {
    ...
    public function updatebox_pk_chief_qc_iPostId($field, $id,$value,$data) {
    ...
    public function insertbox_pk_chief_qc_cNip($field, $id) {
      $cNip        =  $this->sess_auth->gNIP;
      $vDescription = $cNip." - ".$this->lib_utilitas->getvName($cNip);

      $url = $this->url;
      $url6 = base_url().'processor/pk/pk_chief/qc?action=searchcNip';
      $url2 = base_url().'processor/pk/pk_chief/qc?action=searchSkema';
      $url3 = base_url().'processor/pk/pk_chief/qc?action=cekJabatan';
      ...
      $o = "<script type='text/javascript'>
              $(document).ready(function() {
                ...
                var akses = cekJabatan('pk_chief_qc_iSkemaId', '".$iPostId."');
                if (akses=='TIDAK'){
                    custom_alert('Jabatan anda tidak terdaftar untuk input Penilaian Kinerja pada module ini');
                    //$('#form_pk_chief_qc').hide();
                    return false;
                }


                loadSkema('pk_chief_qc_iSkemaId', '".$iPostId."');
                ...
                var config = {
                  ...
                  select: function(event, ui){
                    ...
                    loadSkema('pk_chief_qc_iSkemaId', ui.item.iPostId);
    ...
    public function updatebox_pk_chief_qc_cNip($field, $id,$value,$data) {
      $cNip         =  $value;
      $vDescription = $cNip." - ".$this->lib_utilitas->getvName($cNip);
      $url = $this->url;
      $url6 = base_url().'processor/pk/pk_chief/qc?action=searchcNip';
    ...
    public function insertbox_pk_chief_qc_cTahun($field, $id) {
      $year = date('Y');
      $y1 = $year - 5;
      $y2 = $year + 5;
    ...
    public function updatebox_pk_chief_qc_cTahun($field, $id,$value,$data) {
      $year = $value;
    ...
    public function insertbox_pk_chief_qc_cPeriode($field, $id) {
    ...
    public function updatebox_pk_chief_qc_cPeriode($field, $id,$value,$data) {
    ...
    public function insertbox_pk_chief_qc_iSemester($field, $id) {

      $url2 = base_url().'processor/pk/pk_chief/qc?action=cekDataPk';
      $url  = base_url()."processor/pk/pk_chief/qc?company_id=".$this->input->get('company_id')."&group_id=".$this->input->get('group_id')."&modul_id=".$this->input->get('modul_id');
    ...
    public function updatebox_pk_chief_qc_iSemester($field, $id,$value,$data) {
      $sms = array(''=>'',1=>1,2=>2);
    ...
    public function insertbox_pk_chief_qc_dPk($field, $id) {
      $now = date('d-m-Y');
    ...
    public function updatebox_pk_chief_qc_dPk($field, $id,$value,$data) {
    ...
    public function insertbox_pk_chief_qc_iProbation($field, $id) {
    ...
    public function updatebox_pk_chief_qc_iProbation($field, $id,$value,$data) {
      //1=PK Probation, tahun dan semester bisa dikosongkan.
    ...
    public function insertbox_pk_chief_qc_lampiran($field, $id) {
    ...
    public function updatebox_pk_chief_qc_lampiran($field, $id,$value,$data) {
      $action = $this->input->get('action');
      $idnya = $data['id'];
      $url = base_url().'processor/pk/pk_chief/qc?action=uploadproses&id='.$idnya;
      $urlhapus = base_url().'processor/pk/pk_chief/qc?action=hapus';
      $urlgetLampiran = base_url().'processor/pk/pk_chief/qc?action=getLampiran';
      $o = '';
      $o .= '<style>
  ...
  public function insertbox_pk_chief_qc_iSkemaId($field, $id) {
    ...
    public function updatebox_pk_chief_qc_iSkemaId($field, $id,$value,$data) {
      $iPostId = $data['iPostId'];
      $url = base_url().'processor/pk/pk_chief/qc?action=getDetail';
  ...
  public function insertbox_pk_chief_qc_detail($field, $id) {
...
border: 1px solid aquamarine;' >
  ...
  public function updatebox_pk_chief_qc_detail($field, $id,$value,$data) {
    $idnya      = $data['iSkemaId'];
    $iPkTransId = $data['id'];
    $cNip       = $data['cNip'];
  ...
  public function updatebox_pk_chief_qc_Approval($field, $id,$value,$data) {
    $id = $data['id'];
   $o = '<div class="box_cbox">
    ...
    $sql = "SELECT cNip,iStatus,vReason,dDate
          FROM hrd.pk_chief_qc WHERE iSkemaId='".$id."' ORDER BY id ASC";
  ...
  public function manipulate_update_button($button,$rowData) {
    ...
    $cNipNya = $rowData['cNip'];
      ...
      $btnSave .= "<button type='button'
                ...
                onclick='javascript:update_btn_back_".$this->url."(\"".$this->url."\", \"".base_url()."processor/pk/pk_chief/qc?company_id=".$this->input->get('company_id')."&group_id=".$this->input->get('group_id')."&modul_id=".$this->input->get('modul_id')."\", this)'>Update
                </button>";


      $urlsumbit  = base_url().'processor/pk/pk_chief/qc?action=proses_approve';
      ...
      $btnSubmit .= "<button type='button'
                ...
                id='button_update_".$this->url."'
                class='icon-save ui-button'
                onclick='javascript:btn_submit_".$this->url."(\"".$this->url."\", \"".base_url()."processor/pk/pk_chief/qc?company_id=".$this->input->get('company_id')."&group_id=".$this->input->get('group_id')."&modul_id=".$this->input->get('modul_id')."\", this)'>Sumbit
                </button>";
      ...
      $btnApprove .= "<button type='button'
                ...
                id='button_update_".$this->url."'
                class='icon-save ui-button'
                onclick='javascript:btn_submit_".$this->url."(\"".$this->url."\", \"".base_url()."processor/pk/pk_chief/qc?company_id=".$this->input->get('company_id')."&group_id=".$this->input->get('group_id')."&modul_id=".$this->input->get('modul_id')."\", this)'>Approve
                </button>";
      ...
      $btn_preview  = '
                      <script type="text/javascript">
                        ...
                        function print_'.$this->url.'() {
                          ...
                          document.getElementById("iframe_preview_ga_'.$this->url.'").src = "'.base_url().'processor/pk/pk_chief/qc?action=get_preview_report&id="+id+"&vName="+vName;
  ...
  public function manipulate_insert_button($button) {
    ...
    $btnSave  =  "<script type='text/javascript'>
        ...
        $btnSave .= "<button type='button'
                            ...
                            class='icon-save ui-button'
                            onclick='javascript:save_btn_back_".$this->url."(\"".$this->url."\", \"".base_url()."processor/pk/pk_chief/qc?company_id=".$this->input->get('company_id')."&group_id=".$this->input->get('group_id')."&modul_id=".$this->input->get('modul_id')."\", this)'>Save
                            </button>";
    $button['save'] = $btnSave;
...
/*
    ...
    $html .= ' </tbody>
     ...
     $urlSD = base_url().'processor/pk/pk_chief/qc?action=supportData';
     $urlgetValuePk = base_url().'processor/pk/pk_chief/qc?action=getValuePk';
     $urlDetail = base_url().'processor/pk/pk_chief/qc?action=showDetail';
     $url = base_url().'processor/pk/pk_chief/qc?action=SaveNilai';
     $rUrl = base_url()."processor/pk/pk_chief/qc?company_id=".$this->input->get('company_id')."&group_id=".$this->input->get('group_id')."&modul_id=".$this->input->get('modul_id');

     $html .= "<script>

      $(document).ready(function() {
        $('.pk_chief_qc_nPointSepakat').autoNumeric('init', {vMin:'0', vMax:'100'});

      });

      $('.pk_chief_qc_nPointSepakat').live('keyup', function() {
        var ix = $('.pk_chief_qc_nPointSepakat').index($(this));
      ...
      $('.pk_chief_qc_nPointSepakat').live('blur', function() {
            var ix = $('.pk_chief_qc_nPointSepakat').index($(this));
      ...
      function ".$this->url."_hitungPk(vFunctionLink,iAspekId,tampil,bobot){
          ...
          document.getElementById('vDescription_pk_chief_qc_'+iAspekId+'_100').style.backgroundColor='#ffffff';
          document.getElementById('vDescription_pk_chief_qc_'+iAspekId+'_80').style.backgroundColor='#ffffff';
          document.getElementById('vDescription_pk_chief_qc_'+iAspekId+'_60').style.backgroundColor='#ffffff';
          document.getElementById('vDescription_pk_chief_qc_'+iAspekId+'_40').style.backgroundColor='#ffffff';
          document.getElementById('vDescription_pk_chief_qc_'+iAspekId+'_20').style.backgroundColor='#ffffff';

          document.getElementById('vDescription_pk_chief_qc_'+iAspekId+'_'+point).style.backgroundColor = color;
      ...
      function ".$this->url."_getValuePk(vFunctionLink,iAspekId){
        ...
        var nip = $('#pk_chief_qc_cNip').val();
        var dPeriode1 = $('#pk_chief_qc_dPeriode1').val();
        var dPeriode2 = $('#pk_chief_qc_dPeriode2').val();
        return $.ajax({
            type: 'POST',
            url: '".$urlgetValuePk."',
  ...
  function generateHasilAkhir($iPkTransId,$cNip,$rowData){
    ...
    $urlHasil = base_url().'processor/pk/pk_chief/qc?action=SaveHasil';
    $rUrl = base_url()."processor/pk/pk_chief/qc?company_id=".$this->input->get('company_id')."&group_id=".$this->input->get('group_id')."&modul_id=".$this->input->get('modul_id');
    ...
    $html .= "<script>
                ...
                $(document).ready(function() {
                  $('#pk_chief_qc_iDivIdTo').change(function() {
                      groupdeptid = getGroupDeptId($(this).val());
                      loadDataJbt('pk_chief_qc_iPostIdTo', groupdeptid, groupdeptid);
  ...
  public function searchcNip() {
    ...
    $data = array();
    $divGa = $this->lib_utilitas->getSysParam('DIV_PK_CHIEF_QC');
    $jabatan  =  $this->lib_utilitas->getSysParam("JABATAN_PK_CHIEF_QC");
  ...
  function getDetail(){
      ...
      $html .= "<script>
                  $(document).ready(function() {
                    ...
                    $('.pk_chief_qc_nResult').autoNumeric('init', {vMin:'0', vMax:'100.00'});
                  });


                  $('.pk_chief_qc_nResult').live('keyup', function() {
                    var ix = $('.pk_chief_qc_nResult').index($(this));

                    var sisa_sblm   = 0;
                    var result  = $('.pk_chief_qc_nResult').eq(ix).val();
                    var bobot   = $('.pk_chief_qc_nBobot').eq(ix).val();
                    var nilai   = parseFloat((result * bobot) / 100);

                    $('.pk_chief_qc_nNilai').eq(ix).val(nilai.toFixed(2));
                    $('.nama_pk_chief_qc_nNilai').eq(ix).html(nilai.toFixed(2));
                  ...
                  function sum_".$this->url."(){
                    ...
                    $('.pk_chief_qc_nResult').each(function() {
                      if ($('.pk_chief_qc_nResult').eq(i).val() != '') {
                        total += parseFloat($('.pk_chief_qc_nNilai').eq(i).val());
                    ...
                    $('#pk_chief_qc_jumlah').html(total.toFixed(2));
                    $('#pk_chief_qc_nFinal').val(total.toFixed(2));
...
FROM hrd.pk_pkresult AS a WHERE a.iPkTransId='".$iPkTransId."' ";
    ...
    function getValuePk(){
      ...
      $this->lib_pk_chief_qc->$vFunctionLink($_POST);
    }
