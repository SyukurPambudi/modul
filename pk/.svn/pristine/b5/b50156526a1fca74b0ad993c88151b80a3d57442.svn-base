	...
	<tbody>
			...
			if($rcount>=1){
				foreach ($dataall as $kall => $vall) {
					...
					$date1=date_create($vall['dApp_dir']);
					$date2=date_create($vall['dTanggalTerimaBD']);
					...
					echo "<td>".$a[$vall['iJenis_panel']]."</td>";   
					echo "<td>".date('Y-m-d',strtotime($vall['dApp_dir']))."</td>"; 
					echo "<td>".$vall['dTanggalTerimaBD']."</td>";
					echo "<td>".$selisih."</td>";
					echo "<tr>"; 
					$i++;
					$tot_upb++;
...
?>
<label class="lbl_detail">Panel (Target 1 bulan)  : <?php echo $rata ?> Bulan</label>