	...
	<tbody>
			...
			if($rcount>=1){
				foreach ($dataall as $kall => $vall) {
					...
					$date2=date_create($vall['tsubmit_prareg']);
					$date1=date_create($vall['tappbusdev']);
					...
					echo "<td>".date('Y-m-d',strtotime($vall['tappbusdev']))."</td>"; 
...
?>
<label class="lbl_detail">Rata - rata Jangka Waktu (Jumlah Selisih / Jumlah UPB) : <?php echo $rata ?> Bulan</label>