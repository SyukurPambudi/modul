...
<label class="lbl_detail">UPB Memenuhi Syarat ( Kategori B) </label>
<table class='tb_det'>
	<thead>
		<tr>
			<th>No</th>
	...
	<tbody>
			...
			$sel=0;
				foreach ($datas as $data) {
					...
					echo "<td>".$data['tsubmit_dokapplet']."</td>";
					echo "<td>".$data['iGroup_produk']."</td>";
					echo "<tr>";
...
<label class="lbl_detail">Jumlah UPB : <?php echo $c ?> UPB</label><br>
<?php 
$vc=array();
$ni='-';
if($c!='0'){
	foreach ($group as $kc => $vci) {
		$vc[]=$vci['groupnya'];
	}
	$ni=implode(',', $vc);
}
?>
<label class="lbl_detail">Group UPB : <?php echo $ni; ?></label>
