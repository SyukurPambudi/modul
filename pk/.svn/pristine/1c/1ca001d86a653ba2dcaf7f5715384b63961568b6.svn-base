	...
	<tbody>
			...
			if($rcount>=1){
				foreach ($dataall as $kall => $vall) {
					...
					$date1=date_create($vall['dsubmit_dok']);
					$date2=date_create($vall['dinput_applet']);

					/*$diff =  $date1->diff($date2);
		    		$months = ($diff->y * 12 + $diff->m + $diff->d / 30);*/

		    		$months=$controller->calc_bd2->selisihbulan($date1,$date2);
					...
					echo "<td>".date('Y-m-d',strtotime($vall['dsubmit_dok']))."</td>"; 
...
?>
<label class="lbl_detail">Rata-rata Jangka waktu diperolehnya nomor Applet (dihitung dari tanggal masuk jawaban tambahan data terakhir ke BPOM), untuk produk kategori B (termasuk OGB) : <?php echo $rata ?> Bulan</label>