<?php if ( ! defined('BASEPATH')) exit('No direct script access allowed');
    class pk_fg_steril extends MX_Controller {
  private $sess_auth;
  private $db_erp_pk;
    function __construct() {
        ...
        $this->url = 'pk_fg_steril';
        //$this->load->helper('encrypt');
        $this->load->library('lib_utilitas');
        $this->load->library('lib_pk_fg_steril');
    ...
    function index($action = '') {
      ...
      $grid = new Grid;
      $grid->setTitle('Penilaian Kerja Karyawan');
      $grid->setTable('hrd.pk_trans');
      $grid->setUrl('pk_fg_steril');
    ...
    $skemaPK = $this->lib_utilitas->getSysParam('SKEMA_PK_FG_SPV_STRL');

    $DIV_PK_FG_SPV_STRL = $this->lib_utilitas->getSysParam('DIV_PK_FG_SPV_STRL');
    $grid->setQuery('hrd.employee.iDivisionID in ('.$DIV_PK_FG_SPV_STRL.')',null);
    ...
    function SaveHasil(){
      ...
      $vEvaKry      = $_POST['pk_fg_steril_vEvaKry'];
      $vRencana     = $_POST['pk_fg_steril_vRencana'];
      $vEvaAtasan   = $_POST['pk_fg_steril_vEvaAtasan'];
      $vSaranAtasan = $_POST['pk_fg_steril_vSaranAtasan'];
      $vPelatihan   = $_POST['pk_fg_steril_vPelatihan'];


      if (isset($_POST['pk_create_mekanik_engine_iRencana'])){
          $iRencana     = $_POST['pk_fg_steril_iRencana'];
      }else{
          $iRencana = '';
      }

      $iDivIdTo     = $_POST['pk_fg_steril_iDivIdTo'];
      $iPostIdTo     = $_POST['pk_fg_steril_iPostIdTo'];
      $vPertimbangan = $_POST['pk_fg_steril_vPertimbangan'];
      $nFinal        = $_POST['pk_fg_steril_nFinal'];
    ...
    function SaveNilai(){
      //print_r($_POST);
      //exit();
      $iPkTransId         = $_POST['pk_fg_steril_id'];
    ...
    function uploadproses(){
      ...
      $files = $_FILES;
      $realname = $files["pk_fg_steril_lampiran"]["name"];
      $newName= "PK_".$id."_".$realname;
      $newName =  str_replace("/", "_", $newName);
      $upload_dir = "files/hdsystem/".$newName;

      move_uploaded_file($files["pk_fg_steril_lampiran"]["tmp_name"], $upload_dir);
    ...
    function cekJabatan(){
        ...
        $x_jabpk = explode(",", $this->lib_utilitas->getSysParam('JABATAN_PK_FG_SPV_STRL'));
    ...
    function proses_reject(){
      ...
      $sql    = "SELECT cNip FROM hrd.pk_fg_steril
      ...
      $sql = "INSERT INTO hrd.pk_fg_steril (iSkemaId,cNip,iStatus,vReason,dDate,iUlang,lDeleted,tCreated,cCreatedBy) VALUES
    ...
    public function listbox_pk_fg_steril_dPk($value, $id,$field,$rowData) {
    ...
    public function listbox_pk_fg_steril_iPostId($value, $id,$field,$rowData) {
      $vDescription = $value;
    ...
    public function insertbox_pk_fg_steril_iPostId($field, $id) {
    ...
    public function updatebox_pk_fg_steril_iPostId($field, $id,$value,$data) {
    ...
    public function insertbox_pk_fg_steril_cNip($field, $id) {
      $cNip        =  $this->sess_auth->gNIP;
      $vDescription = $cNip." - ".$this->lib_utilitas->getvName($cNip);

      $url = $this->url;
      $url6 = base_url().'processor/pk/pk/fg/steril?action=searchcNip';
      $url2 = base_url().'processor/pk/pk/fg/steril?action=searchSkema';
      $url3 = base_url().'processor/pk/pk/fg/steril?action=cekJabatan';
      ...
      $o = "<script type='text/javascript'>
              $(document).ready(function() {
                ...
                var akses = cekJabatan('pk_fg_steril_iSkemaId', '".$iPostId."');
                if (akses=='TIDAK'){
                    custom_alert('Jabatan anda tidak terdaftar untuk input Penilaian Kinerja pada module ini');
                    //$('#form_pk_fg_steril').hide();
                    return false;
                }


                loadSkema('pk_fg_steril_iSkemaId', '".$iPostId."');
                ...
                var config = {
                  ...
                  select: function(event, ui){
                    ...
                    loadSkema('pk_fg_steril_iSkemaId', ui.item.iPostId);
    ...
    public function updatebox_pk_fg_steril_cNip($field, $id,$value,$data) {
      $cNip         =  $value;
      $vDescription = $cNip." - ".$this->lib_utilitas->getvName($cNip);
      $url = $this->url;
      $url6 = base_url().'processor/pk/pk/fg/steril?action=searchcNip';
    ...
    public function insertbox_pk_fg_steril_cTahun($field, $id) {
      $year = date('Y');
      $y1 = $year - 5;
      $y2 = $year + 5;
    ...
    public function updatebox_pk_fg_steril_cTahun($field, $id,$value,$data) {
      $year = $value;
    ...
    public function insertbox_pk_fg_steril_cPeriode($field, $id) {
    ...
    public function updatebox_pk_fg_steril_cPeriode($field, $id,$value,$data) {
    ...
    public function insertbox_pk_fg_steril_iSemester($field, $id) {

      $url2 = base_url().'processor/pk/pk/fg/steril?action=cekDataPk';
      $url  = base_url()."processor/pk/pk/fg/steril?company_id=".$this->input->get('company_id')."&group_id=".$this->input->get('group_id')."&modul_id=".$this->input->get('modul_id');
    ...
    public function updatebox_pk_fg_steril_iSemester($field, $id,$value,$data) {
      $sms = array(''=>'',1=>1,2=>2);
    ...
    public function insertbox_pk_fg_steril_dPk($field, $id) {
      $now = date('d-m-Y');
    ...
    public function updatebox_pk_fg_steril_dPk($field, $id,$value,$data) {
    ...
    public function insertbox_pk_fg_steril_iProbation($field, $id) {
    ...
    public function updatebox_pk_fg_steril_iProbation($field, $id,$value,$data) {
      //1=PK Probation, tahun dan semester bisa dikosongkan.
    ...
    public function insertbox_pk_fg_steril_lampiran($field, $id) {
    ...
    public function updatebox_pk_fg_steril_lampiran($field, $id,$value,$data) {
      $action = $this->input->get('action');
      $idnya = $data['id'];
      $url = base_url().'processor/pk/pk/fg/steril?action=uploadproses&id='.$idnya;
      $urlhapus = base_url().'processor/pk/pk/fg/steril?action=hapus';
      $urlgetLampiran = base_url().'processor/pk/pk/fg/steril?action=getLampiran';
      $o = '';
      $o .= '<style>
  ...
  public function insertbox_pk_fg_steril_iSkemaId($field, $id) {
    ...
    public function updatebox_pk_fg_steril_iSkemaId($field, $id,$value,$data) {
      $iPostId = $data['iPostId'];
      $url = base_url().'processor/pk/pk/fg/steril?action=getDetail';
  ...
  public function insertbox_pk_fg_steril_detail($field, $id) {
...
border: 1px solid aquamarine;' >
  ...
  public function updatebox_pk_fg_steril_detail($field, $id,$value,$data) {
    $idnya      = $data['iSkemaId'];
    $iPkTransId = $data['id'];
    $cNip       = $data['cNip'];
  ...
  public function updatebox_pk_fg_steril_Approval($field, $id,$value,$data) {
    $id = $data['id'];
   $o = '<div class="box_cbox">
    ...
    $sql = "SELECT cNip,iStatus,vReason,dDate
          FROM hrd.pk_fg_steril WHERE iSkemaId='".$id."' ORDER BY id ASC";
  ...
  public function manipulate_update_button($button,$rowData) {
    ...
    $cNipNya = $rowData['cNip'];
      ...
      $btnSave .= "<button type='button'
                ...
                onclick='javascript:update_btn_back_".$this->url."(\"".$this->url."\", \"".base_url()."processor/pk/pk/fg/steril?company_id=".$this->input->get('company_id')."&group_id=".$this->input->get('group_id')."&modul_id=".$this->input->get('modul_id')."\", this)'>Update
                </button>";


      $urlsumbit  = base_url().'processor/pk/pk/fg/steril?action=proses_approve';
      ...
      $btnSubmit .= "<button type='button'
                ...
                id='button_update_".$this->url."'
                class='icon-save ui-button'
                onclick='javascript:btn_submit_".$this->url."(\"".$this->url."\", \"".base_url()."processor/pk/pk/fg/steril?company_id=".$this->input->get('company_id')."&group_id=".$this->input->get('group_id')."&modul_id=".$this->input->get('modul_id')."\", this)'>Sumbit
                </button>";
      ...
      $btnApprove .= "<button type='button'
                ...
                id='button_update_".$this->url."'
                class='icon-save ui-button'
                onclick='javascript:btn_submit_".$this->url."(\"".$this->url."\", \"".base_url()."processor/pk/pk/fg/steril?company_id=".$this->input->get('company_id')."&group_id=".$this->input->get('group_id')."&modul_id=".$this->input->get('modul_id')."\", this)'>Approve
                </button>";
      ...
      $btn_preview  = '
                      <script type="text/javascript">
                        ...
                        function print_'.$this->url.'() {
                          ...
                          document.getElementById("iframe_preview_ga_'.$this->url.'").src = "'.base_url().'processor/pk/pk/fg/steril?action=get_preview_report&id="+id+"&vName="+vName;
  ...
  public function manipulate_insert_button($button) {
    ...
    $btnSave  =  "<script type='text/javascript'>
        ...
        $btnSave .= "<button type='button'
                            ...
                            class='icon-save ui-button'
                            onclick='javascript:save_btn_back_".$this->url."(\"".$this->url."\", \"".base_url()."processor/pk/pk/fg/steril?company_id=".$this->input->get('company_id')."&group_id=".$this->input->get('group_id')."&modul_id=".$this->input->get('modul_id')."\", this)'>Save
                            </button>";
    $button['save'] = $btnSave;
...
/*
    ...
    $html .= ' </tbody>
     ...
     $urlSD = base_url().'processor/pk/pk/fg/steril?action=supportData';
     $urlgetValuePk = base_url().'processor/pk/pk/fg/steril?action=getValuePk';
     $urlDetail = base_url().'processor/pk/pk/fg/steril?action=showDetail';
     $url = base_url().'processor/pk/pk/fg/steril?action=SaveNilai';
     $rUrl = base_url()."processor/pk/pk/fg/steril?company_id=".$this->input->get('company_id')."&group_id=".$this->input->get('group_id')."&modul_id=".$this->input->get('modul_id');

     $html .= "<script>

      $(document).ready(function() {
        $('.pk_fg_steril_nPointSepakat').autoNumeric('init', {vMin:'0', vMax:'100'});

      });

      $('.pk_fg_steril_nPointSepakat').live('keyup', function() {
        var ix = $('.pk_fg_steril_nPointSepakat').index($(this));
      ...
      $('.pk_fg_steril_nPointSepakat').live('blur', function() {
            var ix = $('.pk_fg_steril_nPointSepakat').index($(this));
      ...
      function ".$this->url."_hitungPk(vFunctionLink,iAspekId,tampil,bobot){
          ...
          document.getElementById('vDescription_pk_fg_steril_'+iAspekId+'_100').style.backgroundColor='#ffffff';
          document.getElementById('vDescription_pk_fg_steril_'+iAspekId+'_80').style.backgroundColor='#ffffff';
          document.getElementById('vDescription_pk_fg_steril_'+iAspekId+'_60').style.backgroundColor='#ffffff';
          document.getElementById('vDescription_pk_fg_steril_'+iAspekId+'_40').style.backgroundColor='#ffffff';
          document.getElementById('vDescription_pk_fg_steril_'+iAspekId+'_20').style.backgroundColor='#ffffff';

          document.getElementById('vDescription_pk_fg_steril_'+iAspekId+'_'+point).style.backgroundColor = color;
      ...
      function ".$this->url."_getValuePk(vFunctionLink,iAspekId){
        ...
        var nip = $('#pk_fg_steril_cNip').val();
        var dPeriode1 = $('#pk_fg_steril_dPeriode1').val();
        var dPeriode2 = $('#pk_fg_steril_dPeriode2').val();
        return $.ajax({
            type: 'POST',
            url: '".$urlgetValuePk."',
  ...
  function generateHasilAkhir($iPkTransId,$cNip,$rowData){
    ...
    $urlHasil = base_url().'processor/pk/pk/fg/steril?action=SaveHasil';
    $rUrl = base_url()."processor/pk/pk/fg/steril?company_id=".$this->input->get('company_id')."&group_id=".$this->input->get('group_id')."&modul_id=".$this->input->get('modul_id');
    ...
    $html .= "<script>
                ...
                $(document).ready(function() {
                  $('#pk_fg_steril_iDivIdTo').change(function() {
                      groupdeptid = getGroupDeptId($(this).val());
                      loadDataJbt('pk_fg_steril_iPostIdTo', groupdeptid, groupdeptid);
  ...
  public function searchcNip() {
    ...
    $data = array();
    $divGa = $this->lib_utilitas->getSysParam('DIV_PK_FG_SPV_STRL');
    $jabatan  =  $this->lib_utilitas->getSysParam("JABATAN_PK_FG_SPV_STRL");
  ...
  function getDetail(){
      ...
      $html .= "<script>
                  $(document).ready(function() {
                    ...
                    $('.pk_fg_steril_nResult').autoNumeric('init', {vMin:'0', vMax:'100.00'});
                  });


                  $('.pk_fg_steril_nResult').live('keyup', function() {
                    var ix = $('.pk_fg_steril_nResult').index($(this));

                    var sisa_sblm   = 0;
                    var result  = $('.pk_fg_steril_nResult').eq(ix).val();
                    var bobot   = $('.pk_fg_steril_nBobot').eq(ix).val();
                    var nilai   = parseFloat((result * bobot) / 100);

                    $('.pk_fg_steril_nNilai').eq(ix).val(nilai.toFixed(2));
                    $('.nama_pk_fg_steril_nNilai').eq(ix).html(nilai.toFixed(2));
                  ...
                  function sum_".$this->url."(){
                    ...
                    $('.pk_fg_steril_nResult').each(function() {
                      if ($('.pk_fg_steril_nResult').eq(i).val() != '') {
                        total += parseFloat($('.pk_fg_steril_nNilai').eq(i).val());
                    ...
                    $('#pk_fg_steril_jumlah').html(total.toFixed(2));
                    $('#pk_fg_steril_nFinal').val(total.toFixed(2));
...
FROM hrd.pk_pkresult AS a WHERE a.iPkTransId='".$iPkTransId."' ";
    ...
    function getValuePk(){
      ...
      $this->lib_pk_fg_steril->$vFunctionLink($_POST);
    }
