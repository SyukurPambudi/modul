...
class pk_bd_new extends MX_Controller {
    private $sess_auth;
    private $db_erp_pk;
    function __construct() {
        ...
        $this->load->library('lib_pk_bd_new');
    ...
    function index($action = '') {
        ...
        $grid = new Grid;
        $grid->setTitle('Transaksi PK Business Development Manager');    
        $grid->setTable('hrd.pk_trans');    
        $grid->setUrl('pk_bd_new');
        ...
        $direksi = explode(",",$this->lib_utilitas->getSysParam('direksi'));
       ...
       // if (!in_array($nipLogin,$direksi)){  
        if ($nipLogin == 'N00923'){  

            //$grid->setQuery('hrd.employee.iDivisionID in ('.$DIV_PK.')',null);
            $grid->setQuery('hrd.pk_trans.iPostID in (14)',null);
            $grid->setQuery('hrd.pk_trans.cNip NOT IN("L00001")',null);
        }else{
            $grid->setQuery('(employee.cUpper = "'.$nipLogin.'" OR employee.cNip = "'.$nipLogin.'")',NULL);
            $grid->setQuery('hrd.pk_trans.cNip NOT IN("L00001")',null);
            //$grid->setQuery('hrd.employee.iDivisionID in ('.$nipLogin.')',null);
        }
        /*
        if($skemaPK){
...
*/
    ...
    function supportData(){
     ...
     /* if ($iAspekId=='118'){
      ...
      switch ($vFunctionLink) {
        case 'TSPERFM_03':
          $data['DataCollect']=$this->lib_pk_bd_new->supportData_TSPERFM_03($data);
    ...
    function SaveHasil(){
      ...
      $vEvaKry      = $_POST['pk_bd_new_vEvaKry'];
      $vRencana     = $_POST['pk_bd_new_vRencana'];
      $vEvaAtasan   = $_POST['pk_bd_new_vEvaAtasan'];
      $vSaranAtasan = $_POST['pk_bd_new_vSaranAtasan'];
      $vPelatihan   = $_POST['pk_bd_new_vPelatihan'];
      

      if (isset($_POST['pk_bd_new_iRencana'])){
          $iRencana     = $_POST['pk_bd_new_iRencana'];
      }else{
          $iRencana = '';
      }

      $iDivIdTo     = $_POST['pk_bd_new_iDivIdTo'];
      $iPostIdTo     = $_POST['pk_bd_new_iPostIdTo'];
      $vPertimbangan = $_POST['pk_bd_new_vPertimbangan'];
      $nFinal        = $_POST['pk_bd_new_nFinal'];
    ...
    function SaveNilaiCSE(){
      ...
      $iPkTransId         = $_POST['pk_bd_new_id'];
      $iMsGroupAspekId    = $_POST['_iMsGroupAspekId'];
      $nResult            = $_POST['pk_bd_new_nilaicse'];
      $cNip               =  $this->sess_auth->gNIP;
    ...
    function uploadproses(){
      ...
      $files = $_FILES;
      $realname = $files["pk_bd_new_lampiran"]["name"];
      $newName= "PK_".$id."_".$realname;
      $newName =  str_replace("/", "_", $newName);
      $upload_dir = "files/hdsystem/".$newName;

      move_uploaded_file($files["pk_bd_new_lampiran"]["tmp_name"], $upload_dir);
    ...
    function proses_reject(){
      ...
      $sql    = "SELECT cNip FROM hrd.pk_bd_new
      ...
      $sql = "INSERT INTO hrd.pk_bd_new (iSkemaId,cNip,iStatus,vReason,dDate,iUlang,lDeleted,tCreated,cCreatedBy) VALUES
    ...
    public function listbox_pk_bd_new_dPk($value, $id,$field,$rowData) {
    ...
    public function listbox_pk_bd_new_iPostId($value, $id,$field,$rowData) {
      $vDescription = $value;
    ...
    public function insertbox_pk_bd_new_iPostId($field, $id) {
      $cNip   =  $this->sess_auth->gNIP;
    ...
    public function updatebox_pk_bd_new_iPostId($field, $id,$value,$data) {
    ...
    public function insertbox_pk_bd_new_cNip($field, $id) {
      $cNip        =  $this->sess_auth->gNIP;
      $vDescription = $cNip." - ".$this->lib_utilitas->getvName($cNip);

      $url = $this->url;
      $url6 = base_url().'processor/pk/pk/bd/new?action=searchcNip';
      $url2 = base_url().'processor/pk/pk/bd/new?action=searchSkema';
      $url3 = base_url().'processor/pk/pk/bd/new?action=cekJabatan';
      ...
      $o = "<script type='text/javascript'>
              $(document).ready(function() {
                /*var akses = cekJabatan('pk_bd_new_iSkemaId', '".$iPostId."');
                if (akses=='TIDAK'){
                ...
                loadSkema('pk_bd_new_iSkemaId', '".$iPostId."');
                ...
                var config = {
                  ...
                  select: function(event, ui){
                    ...
                    loadSkema('pk_bd_new_iSkemaId', ui.item.iPostId);
    ...
    public function updatebox_pk_bd_new_cNip($field, $id,$value,$data) {
      $cNip         =  $value;
      $vDescription = $cNip." - ".$this->lib_utilitas->getvName($cNip);
      $url = $this->url;
      $url6 = base_url().'processor/pk/pk/bd/new?action=searchcNip';
    ...
    public function insertbox_pk_bd_new_cTahun($field, $id) {
      $year = date('Y');
      $y1 = $year - 5;
      $y2 = $year + 5;
    ...
    public function updatebox_pk_bd_new_cTahun($field, $id,$value,$data) {
      $year = $value;
    ...
    public function insertbox_pk_bd_new_cPeriode($field, $id) {
    ...
    public function updatebox_pk_bd_new_cPeriode($field, $id,$value,$data) {
    ...
    public function insertbox_pk_bd_new_iSemester($field, $id) {
      
      $url2 = base_url().'processor/pk/pk/bd/new?action=cekDataPk';
      $url  = base_url()."processor/pk/pk/bd/new?company_id=".$this->input->get('company_id')."&group_id=".$this->input->get('group_id')."&modul_id=".$this->input->get('modul_id');
    ...
    public function updatebox_pk_bd_new_iSemester($field, $id,$value,$data) {
      $sms = array(''=>'',1=>1,2=>2);
    ...
    public function insertbox_pk_bd_new_dPk($field, $id) {
      $now = date('d-m-Y');
    ...
    public function updatebox_pk_bd_new_dPk($field, $id,$value,$data) {
    ...
    public function insertbox_pk_bd_new_iProbation($field, $id) {
    ...
    public function updatebox_pk_bd_new_iProbation($field, $id,$value,$data) {
      //1=PK Probation, tahun dan semester bisa dikosongkan.
    ...
    public function insertbox_pk_bd_new_lampiran($field, $id) {
    ...
    public function updatebox_pk_bd_new_lampiran($field, $id,$value,$data) {
      $action = $this->input->get('action');
      $idnya = $data['id'];
      $url = base_url().'processor/pk/pk/bd/new?action=uploadproses&id='.$idnya;
      $urlhapus = base_url().'processor/pk/pk/bd/new?action=hapus';
      $urlgetLampiran = base_url().'processor/pk/pk/bd/new?action=getLampiran';
      $o = '';  
      $o .= '<style>
  ...
  public function insertbox_pk_bd_new_iSkemaId($field, $id) {
    ...
    public function updatebox_pk_bd_new_iSkemaId($field, $id,$value,$data) {
      $iPostId = $data['iPostId'];
      $url = base_url().'processor/pk/pk/bd/new?action=getDetail';
  ...
  public function insertbox_pk_bd_new_detail($field, $id) {
...
border: 1px solid aquamarine;' >
  ...
  public function updatebox_pk_bd_new_detail($field, $id,$value,$data) {
    $idnya      = $data['iSkemaId'];
    $iPkTransId = $data['id'];
    $cNip       = $data['cNip'];
  ...
  public function updatebox_pk_bd_new_Approval($field, $id,$value,$data) {
    $id = $data['id'];
   $o = '<div class="box_cbox">
    ...
    $sql = "SELECT cNip,iStatus,vReason,dDate 
          FROM hrd.pk_bd_new WHERE iSkemaId='".$id."' ORDER BY id ASC";
  ...
  public function manipulate_update_button($button,$rowData) {  
    ...
    $cNipNya = $rowData['cNip'];
      ...
      $btnSave .= "<button type='button'
                ...
                onclick='javascript:update_btn_back_".$this->url."(\"".$this->url."\", \"".base_url()."processor/pk/pk/bd/new?company_id=".$this->input->get('company_id')."&group_id=".$this->input->get('group_id')."&modul_id=".$this->input->get('modul_id')."\", this)'>Update
                </button>";


      $urlsumbit  = base_url().'processor/pk/pk/bd/new?action=proses_approve';
      ...
      $btnSubmit .= "<button type='button'
                ...
                id='button_update_".$this->url."'
                class='icon-save ui-button'
                onclick='javascript:btn_submit_".$this->url."(\"".$this->url."\", \"".base_url()."processor/pk/pk/bd/new?company_id=".$this->input->get('company_id')."&group_id=".$this->input->get('group_id')."&modul_id=".$this->input->get('modul_id')."\", this)'>Submit
                </button>";
      ...
      $btnApprove .= "<button type='button'
                ...
                id='button_update_".$this->url."'
                class='icon-save ui-button'
                onclick='javascript:btn_submit_".$this->url."(\"".$this->url."\", \"".base_url()."processor/pk/pk/bd/new?company_id=".$this->input->get('company_id')."&group_id=".$this->input->get('group_id')."&modul_id=".$this->input->get('modul_id')."\", this)'>Approve
                </button>";
      ...
      $btn_preview  = '
                      <script type="text/javascript">
                        ...
                        function print_'.$this->url.'() {
                          ...
                          document.getElementById("iframe_preview_'.$this->url.'").src = "'.base_url().'processor/pk/pk/bd/new?action=get_preview_report&id="+id+"&vName="+vName;
  ...
  public function manipulate_insert_button($button) {
    ...
    $btnSave  =  "<script type='text/javascript'>               
        ...
        $btnSave .= "<button type='button'
                            ...
                            class='icon-save ui-button'
                            onclick='javascript:save_btn_back_".$this->url."(\"".$this->url."\", \"".base_url()."processor/pk/pk/bd/new?company_id=".$this->input->get('company_id')."&group_id=".$this->input->get('group_id')."&modul_id=".$this->input->get('modul_id')."\", this)'>Save
                            </button>";
    $button['save'] = $btnSave;
  ...
  function generateCse($iSkemaId,$iMsGroupAspekId,$iPkTransId,$cNip,$rowData){
    ...
    $url = base_url().'processor/pk/pk/bd/new?action=SaveNilaiCSE';
    $rUrl = base_url()."processor/pk/pk/bd/new?company_id=".$this->input->get('company_id')."&group_id=".$this->input->get('group_id')."&modul_id=".$this->input->get('modul_id');
    ...
    $html .= "<script>
              ...
              function simpanPointCse(iMsGroupAspekId){
                  ...
                  var iPkTransId = '".$iPkTransId."';
                    var nResult = $('#pk_bd_new_nilaicse').val();
                    if (nResult==''){
  ...
  function generateAspekDivisi($iSkemaId,$iMsGroupAspekId,$iPkTransId,$cNip,$rowData,$lDynamic){
    ...
    $html .= ' </tbody>
     ...
     $urlSD = base_url().'processor/pk/pk/bd/new?action=supportData';
     $urlgetValuePk = base_url().'processor/pk/pk/bd/new?action=getValuePk';
      $urlgetPerformanceObjective = base_url().'processor/pk/pk/bd/new?action=getPerformanceObjective';
     $urlDetail = base_url().'processor/pk/pk/bd/new?action=showDetail';
     $url = base_url().'processor/pk/pk/bd/new?action=SaveNilai';
     $urlBobot = base_url().'processor/pk/pk/bd/new?action=SaveBobot';
     $rUrl = base_url()."processor/pk/pk/bd/new?company_id=".$this->input->get('company_id')."&group_id=".$this->input->get('group_id')."&modul_id=".$this->input->get('modul_id');

     $html .= "<script>

      $(document).ready(function() {
        $('.pk_bd_new_nPointSepakat').autoNumeric('init', {vMin:'0', vMax:'100'});
     
      });

      $('.pk_bd_new_nPointSepakat').live('keyup', function() {
        var ix = $('.pk_bd_new_nPointSepakat').index($(this));
      ...
      $('.pk_bd_new_nPointSepakat').live('blur', function() {
            var ix = $('.pk_bd_new_nPointSepakat').index($(this));
        ...
        $.ajax({
           ...
           success: function(data)
           {
                custom_alert(data);
                 $.get('".$rUrl."&action=update&id='+iPkTransId, function(data) {
                      $('div#form_".$this->url."').html(data);
                      var index = $('#pk_bd_new_list_detail a[href=\'#pk_bd_new_'+iMsGroupAspekId+'\']').parent().index();
                      $('#pk_bd_new_list_detail').tabs('option', 'active', index);                  
                  });
      ...
      function ".$this->url."_hitungPk(vFunctionLink,iAspekId,tampil,bobot){
          ...
          document.getElementById('vDescription_pk_bd_new_'+iAspekId+'_100').style.backgroundColor='#ffffff';
          document.getElementById('vDescription_pk_bd_new_'+iAspekId+'_80').style.backgroundColor='#ffffff';
          document.getElementById('vDescription_pk_bd_new_'+iAspekId+'_60').style.backgroundColor='#ffffff';
          document.getElementById('vDescription_pk_bd_new_'+iAspekId+'_40').style.backgroundColor='#ffffff';
          document.getElementById('vDescription_pk_bd_new_'+iAspekId+'_20').style.backgroundColor='#ffffff';

          document.getElementById('vDescription_pk_bd_new_'+iAspekId+'_'+point).style.backgroundColor = color;
      ...
      function ".$this->url."_input_supoortData_ts(iAspekId,iPkTransId){
        load_popup_pk_bd_new_insert('".$urlSD."&iAspekId='+iAspekId+'&iPkTransId='+iPkTransId+'&getaction=".$this->input->get('action')."','Support Data');
      }

     </script>";
     $data['grid']='pk_bd_new';    
     //$html.=$this->load->view('func_tambahan_pk_bd_new_js',$data);
      return $html;
  }
  ...
  function generateHasilAkhir($iPkTransId,$cNip,$rowData){
    ...
    $urlHasil = base_url().'processor/pk/pk/bd/new?action=SaveHasil';
    $rUrl = base_url()."processor/pk/pk/bd/new?company_id=".$this->input->get('company_id')."&group_id=".$this->input->get('group_id')."&modul_id=".$this->input->get('modul_id');
    ...
    $html .= "<script>
                ...
                $(document).ready(function() {  
                  $('#pk_bd_new_iDivIdTo').change(function() {
                      groupdeptid = getGroupDeptId($(this).val());
                      loadDataJbt('pk_bd_new_iPostIdTo', groupdeptid, groupdeptid);
  ...
  public function searchcNip() {
    ...
    $jabatan  =  $this->lib_utilitas->getSysParam("JABATAN_pk_bd_new_MANAGER");
  ...
  function getDetail(){
      ...
      $html .= "<script>
                  $(document).ready(function() {
                    ...
                    $('.pk_bd_new_nResult').autoNumeric('init', {vMin:'0', vMax:'100.00'});
                  });


                  $('.pk_bd_new_nResult').live('keyup', function() {
                    var ix = $('.pk_bd_new_nResult').index($(this));
                    
                    var sisa_sblm   = 0;
                    var result  = $('.pk_bd_new_nResult').eq(ix).val();
                    var bobot   = $('.pk_bd_new_nBobot').eq(ix).val();
                    var nilai   = parseFloat((result * bobot) / 100);

                    $('.pk_bd_new_nNilai').eq(ix).val(nilai.toFixed(2));
                    $('.nama_pk_bd_new_nNilai').eq(ix).html(nilai.toFixed(2));
                  ...
                  function sum_".$this->url."(){
                    ...
                    $('.pk_bd_new_nResult').each(function() {
                      if ($('.pk_bd_new_nResult').eq(i).val() != '') {
                        total += parseFloat($('.pk_bd_new_nNilai').eq(i).val());
                    ...
                    $('#pk_bd_new_jumlah').html(total.toFixed(2));
                    $('#pk_bd_new_nFinal').val(total.toFixed(2));
...
FROM hrd.pk_pkresult AS a WHERE a.iPkTransId='".$iPkTransId."' ";
    ...
    function getValuePk(){
      ...
      $this->lib_pk_bd_new->$vFunctionLink($_POST);
    }
