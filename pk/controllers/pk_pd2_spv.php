...
class pk_pd2_spv extends MX_Controller {
    private $sess_auth;
    private $db_erp_pk;
    function __construct() {
        ...
        $this->load->library('lib_pk_pd2_spv');
    ...
    function index($action = '') {
        ...
        $grid = new Grid;
        $grid->setTitle('Supervisor Product Development 2');    
        $grid->setTable('hrd.pk_trans');    
        $grid->setUrl('pk_pd2_spv');
...
*/
    ...
    function supportData(){
     ...
     /* if ($iAspekId=='118'){
      ...
      switch ($vFunctionLink) {
        case 'TSPERFM_03':
          $data['DataCollect']=$this->lib_pk_pd2_spv->supportData_TSPERFM_03($data);
    ...
    function SaveHasil(){
      ...
      if(isset($_POST['pk_pd2_spv_vEvaKry'])){
        $vEvaKry     = $_POST['pk_pd2_spv_vEvaKry'];
        $this->db_erp_pk->set('vEvaKry',$vEvaKry);
      }
      if(isset($_POST['pk_pd2_spv_vRencana'])){
        $vRencana     = $_POST['pk_pd2_spv_vRencana'];
        $this->db_erp_pk->set('vRencana',$vRencana);
      }
      if(isset($_POST['pk_pd2_spv_vEvaAtasan'])){
        $vEvaAtasan   = $_POST['pk_pd2_spv_vEvaAtasan'];
        $this->db_erp_pk->set('vEvaAtasan',$vEvaAtasan);
      }
      if(isset($_POST['pk_pd2_spv_vSaranAtasan'])){
        $vSaranAtasan   = $_POST['pk_pd2_spv_vSaranAtasan'];
        $this->db_erp_pk->set('vSaranAtasan',$vSaranAtasan);
      }
      if(isset($_POST['pk_pd2_spv_vPelatihan'])){
        $vPelatihan   = $_POST['pk_pd2_spv_vPelatihan'];
        $this->db_erp_pk->set('vPelatihan',$vPelatihan);
      }
      if (isset($_POST['pk_pd2_spv_iRencana'])){
          $iRencana   = $_POST['pk_pd2_spv_iRencana'];
          $this->db_erp_pk->set('iRencana',$iRencana);
      }
      if (isset($_POST['pk_pd2_spv_iDivIdTo'])){
          $iDivIdTo   = $_POST['pk_pd2_spv_iDivIdTo'];
          $this->db_erp_pk->set('iRencana',$iDivIdTo);
      }
      if (isset($_POST['pk_pd2_spv_iPostIdTo'])){
          $iPostIdTo   = $_POST['pk_pd2_spv_iPostIdTo'];
          $this->db_erp_pk->set('iRencana',$iPostIdTo);
      }
      if (isset($_POST['pk_pd2_spv_vPertimbangan'])){
          $vPertimbangan   = $_POST['pk_pd2_spv_vPertimbangan'];
          $this->db_erp_pk->set('iRencana',$vPertimbangan);
      }
      if (isset($_POST['pk_pd2_spv_nFinal'])){
          $nFinal   = $_POST['pk_pd2_spv_nFinal'];
    ...
    function SaveBobot(){
      //print_r($_POST);
      //exit();
      $iPkTransId         = $_POST['pk_pd2_spv_id'];
    ...
    function SaveNilaiCSE(){
      ...
      $iPkTransId         = $_POST['pk_pd2_spv_id'];
      $iMsGroupAspekId    = $_POST['_iMsGroupAspekId'];
      $nResult            = $_POST['pk_pd2_spv_nilaicse'];
      $cNip               =  $this->sess_auth->gNIP;
    ...
    function uploadproses(){
      ...
      $files = $_FILES;
      $realname = $files["pk_pd2_spv_lampiran"]["name"];
      $newName= "PK_".$id."_".$realname;
      $newName =  str_replace("/", "_", $newName);
      $upload_dir = "files/hdsystem/".$newName;

      move_uploaded_file($files["pk_pd2_spv_lampiran"]["tmp_name"], $upload_dir);
    ...
    function proses_reject(){
      ...
      $sql    = "SELECT cNip FROM hrd.pk_pd2_spv
      ...
      $sql = "INSERT INTO hrd.pk_pd2_spv (iSkemaId,cNip,iStatus,vReason,dDate,iUlang,lDeleted,tCreated,cCreatedBy) VALUES
    ...
    public function listbox_pk_pd2_spv_dPk($value, $id,$field,$rowData) {
    ...
    public function listbox_pk_pd2_spv_iPostId($value, $id,$field,$rowData) {
      $vDescription = $value;
    ...
    public function insertbox_pk_pd2_spv_iPostId($field, $id) {
      $cNip   =  $this->sess_auth->gNIP;
    ...
    public function updatebox_pk_pd2_spv_iPostId($field, $id,$value,$data) {
    ...
    public function insertbox_pk_pd2_spv_cNip($field, $id) {
      $cNip        =  $this->sess_auth->gNIP;
      ...
      $o = "<script type='text/javascript'>
              $(document).ready(function() {
                /*var akses = cekJabatan('pk_pd2_spv_iSkemaId', '".$iPostId."');
                if (akses=='TIDAK'){
                    custom_alert('Jabatan anda tidak terdaftar untuk input Penilaian Kinerja pada module ini');
                    //$('#form_pk_pd2_spv').hide();
                    return false;
                }*/

                loadSkema('pk_pd2_spv_iSkemaId', '".$iPostId."');
                ...
                var config = {
                  ...
                  select: function(event, ui){
                    ...
                    loadSkema('pk_pd2_spv_iSkemaId', ui.item.iPostId);
    ...
    public function updatebox_pk_pd2_spv_cNip($field, $id,$value,$data) {
      $cNip         =  $value;
      $vDescription = $cNip." - ".$this->lib_utilitas->getvName($cNip);
      $url = $this->url;
      $url6 = base_url().'processor/'.$this->urll.'?action=searchcNip';
    ...
    public function insertbox_pk_pd2_spv_cTahun($field, $id) {
      $year = date('Y');
      $y1 = $year - 5;
      $y2 = $year + 5;
    ...
    public function updatebox_pk_pd2_spv_cTahun($field, $id,$value,$data) {
      $year = $value;
    ...
    public function insertbox_pk_pd2_spv_cPeriode($field, $id) {
    ...
    public function updatebox_pk_pd2_spv_cPeriode($field, $id,$value,$data) {
    ...
    public function insertbox_pk_pd2_spv_iSemester($field, $id) {
    ...
    public function updatebox_pk_pd2_spv_iSemester($field, $id,$value,$data) {
      $sms = array(''=>'',1=>1,2=>2);
    ...
    public function insertbox_pk_pd2_spv_dPk($field, $id) {
      $now = date('d-m-Y');
    ...
    public function updatebox_pk_pd2_spv_dPk($field, $id,$value,$data) {
    ...
    public function insertbox_pk_pd2_spv_iProbation($field, $id) {
    ...
    public function updatebox_pk_pd2_spv_iProbation($field, $id,$value,$data) {
      //1=PK Probation, tahun dan semester bisa dikosongkan.
    ...
    public function insertbox_pk_pd2_spv_lampiran($field, $id) {
    ...
    public function updatebox_pk_pd2_spv_lampiran($field, $id,$value,$data) {
      $action = $this->input->get('action');
      $idnya = $data['id'];
  ...
  public function insertbox_pk_pd2_spv_iSkemaId($field, $id) {
    ...
    public function updatebox_pk_pd2_spv_iSkemaId($field, $id,$value,$data) {
  ...
  public function insertbox_pk_pd2_spv_detail($field, $id) {
...
border: 1px solid aquamarine;' >
  ...
  public function updatebox_pk_pd2_spv_detail($field, $id,$value,$data) {
    $idnya      = $data['iSkemaId'];
    $iPkTransId = $data['id'];
    $cNip       = $data['cNip'];
  ...
  public function updatebox_pk_pd2_spv_Approval($field, $id,$value,$data) {
    $id = $data['id'];
   $o = '<div class="box_cbox">
    ...
    $sql = "SELECT cNip,iStatus,vReason,dDate 
          FROM hrd.pk_pd2_spv WHERE iSkemaId='".$id."' ORDER BY id ASC";
    ...
    $html .= "<script>
              ...
              function simpanPointCse(iMsGroupAspekId){
                  ...
                  var iPkTransId = '".$iPkTransId."';
                    var nResult = $('#pk_pd2_spv_nilaicse').val();
                    if (nResult==''){
  ...
  function generateAspekDivisi($iSkemaId,$iMsGroupAspekId,$iPkTransId,$cNip,$rowData,$lDynamic){
    ...
    $html .= ' </tbody>
     ...
     $html .= "<script>
      ...
      $(document).ready(function() {
        $('.pk_pd2_spv_nPointSepakat').autoNumeric('init', {vMin:'0', vMax:'100'});
     
      });

      $('.pk_pd2_spv_nPointSepakat').live('keyup', function() {
        var ix = $('.pk_pd2_spv_nPointSepakat').index($(this));
      ...
      $('.pk_pd2_spv_nPointSepakat').live('blur', function() {
            var ix = $('.pk_pd2_spv_nPointSepakat').index($(this));
        ...
        $.ajax({
           ...
           success: function(data)
           {
                custom_alert(data);
                 $.get('".$rUrl."&action=update&id='+iPkTransId, function(data) {
                      $('div#form_".$this->url."').html(data);
                      var index = $('#pk_pd2_spv_list_detail a[href=\'#pk_pd2_spv_'+iMsGroupAspekId+'\']').parent().index();
                      $('#pk_pd2_spv_list_detail').tabs('option', 'active', index);                  
                  });
      ...
      function ".$this->url."_hitungPk(vFunctionLink,iAspekId,tampil,bobot){
          ...
          document.getElementById('vDescription_pk_pd2_spv_'+iAspekId+'_100').style.backgroundColor='#ffffff';
          document.getElementById('vDescription_pk_pd2_spv_'+iAspekId+'_80').style.backgroundColor='#ffffff';
          document.getElementById('vDescription_pk_pd2_spv_'+iAspekId+'_60').style.backgroundColor='#ffffff';
          document.getElementById('vDescription_pk_pd2_spv_'+iAspekId+'_40').style.backgroundColor='#ffffff';
          document.getElementById('vDescription_pk_pd2_spv_'+iAspekId+'_20').style.backgroundColor='#ffffff';

          document.getElementById('vDescription_pk_pd2_spv_'+iAspekId+'_'+point).style.backgroundColor = color;
      ...
      function ".$this->url."_input_supoortData_ts(iAspekId,iPkTransId){
        load_popup_pk_pd2_spv_insert('".$urlSD."&iAspekId='+iAspekId+'&iPkTransId='+iPkTransId+'&getaction=".$this->input->get('action')."','Support Data');
      }

     </script>";
     $data['grid']='pk_pd2_spv';    
     //$html.=$this->load->view('func_tambahan_pk_pd2_spv_js',$data);
      return $html;
  }
  ...
  function generateHasilAkhir($iPkTransId,$cNip,$rowData){
    ...
    $html .= "<script>
                ...
                $(document).ready(function() {  
                  $('#pk_pd2_spv_iDivIdTo').change(function() {
                      groupdeptid = getGroupDeptId($(this).val());
                      loadDataJbt('pk_pd2_spv_iPostIdTo', groupdeptid, groupdeptid);
  ...
  public function searchcNip() {
    ...
    $jabatan  =  $this->lib_utilitas->getSysParam("JABATAN_pk_pd2_spv");
  ...
  function getDetail(){
      ...
      $html .= "<script>
                  $(document).ready(function() {
                    ...
                    $('.pk_pd2_spv_nResult').autoNumeric('init', {vMin:'0', vMax:'100.00'});
                  });


                  $('.pk_pd2_spv_nResult').live('keyup', function() {
                    var ix = $('.pk_pd2_spv_nResult').index($(this));
                    
                    var sisa_sblm   = 0;
                    var result  = $('.pk_pd2_spv_nResult').eq(ix).val();
                    var bobot   = $('.pk_pd2_spv_nBobot').eq(ix).val();
                    var nilai   = parseFloat((result * bobot) / 100);

                    $('.pk_pd2_spv_nNilai').eq(ix).val(nilai.toFixed(2));
                    $('.nama_pk_pd2_spv_nNilai').eq(ix).html(nilai.toFixed(2));
                  ...
                  function sum_".$this->url."(){
                    ...
                    $('.pk_pd2_spv_nResult').each(function() {
                      if ($('.pk_pd2_spv_nResult').eq(i).val() != '') {
                        total += parseFloat($('.pk_pd2_spv_nNilai').eq(i).val());
                    ...
                    $('#pk_pd2_spv_jumlah').html(total.toFixed(2));
                    $('#pk_pd2_spv_nFinal').val(total.toFixed(2));
...
FROM hrd.pk_pkresult AS a WHERE a.iPkTransId='".$iPkTransId."' ";
    ...
    function getValuePk(){
      ...
      $this->lib_pk_pd2_spv->$vFunctionLink($_POST);
    }
