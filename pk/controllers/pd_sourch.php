<?php if ( ! defined('BASEPATH')) exit('No direct script access allowed');
    class pd_sourch extends MX_Controller {
  private $sess_auth;
  private $db_erp_pk;
    function __construct() {
        ...
        $this->load->library('lib_pd_sourch');
    ...
    function index($action = '') {
      ...
      $grid = new Grid;
      $grid->setTitle('PK PD Sourching');    
      $grid->setTable('hrd.pk_trans');    
      $grid->setUrl('pd_sourch');
    ...
    $skemaPK = $this->lib_utilitas->getSysParam('SKEMA_PK_PD_SOURCH');

    $DIV_PK_PD_SOURCH = $this->lib_utilitas->getSysParam('DIV_PK_PD_SOURCH');
    $grid->setQuery('hrd.employee.iDivisionID in ('.$DIV_PK_PD_SOURCH.')',null);
    ...
    function SaveHasil(){
      ...
      $vEvaKry      = $_POST['pd_sourch_vEvaKry'];
      $vRencana     = $_POST['pd_sourch_vRencana'];
      $vEvaAtasan   = $_POST['pd_sourch_vEvaAtasan'];
      $vSaranAtasan = $_POST['pd_sourch_vSaranAtasan'];
      $vPelatihan   = $_POST['pd_sourch_vPelatihan'];
      

      if (isset($_POST['pk_create_mekanik_engine_iRencana'])){
          $iRencana     = $_POST['pd_sourch_iRencana'];
      }else{
          $iRencana = '';
      }

      $iDivIdTo     = $_POST['pd_sourch_iDivIdTo'];
      $iPostIdTo     = $_POST['pd_sourch_iPostIdTo'];
      $vPertimbangan = $_POST['pd_sourch_vPertimbangan'];
      $nFinal        = $_POST['pd_sourch_nFinal'];
    ...
    function uploadproses(){
      ...
      $files = $_FILES;
      $realname = $files["pd_sourch_lampiran"]["name"];
      $newName= "PK_".$id."_".$realname;
      $newName =  str_replace("/", "_", $newName);
      $upload_dir = "files/hdsystem/".$newName;

      move_uploaded_file($files["pd_sourch_lampiran"]["tmp_name"], $upload_dir);
    ...
    function cekJabatan(){
        ...
        $x_jabpk = explode(",", $this->lib_utilitas->getSysParam('JABATAN_PK_PD_SOURCH'));
    ...
    function proses_reject(){
      ...
      $sql    = "SELECT cNip FROM hrd.pd_sourch 
      ...
      $sql = "INSERT INTO hrd.pd_sourch (iSkemaId,cNip,iStatus,vReason,dDate,iUlang,lDeleted,tCreated,cCreatedBy) VALUES
    ...
    public function listbox_pd_sourch_dPk($value, $id,$field,$rowData) {
    ...
    public function listbox_pd_sourch_iPostId($value, $id,$field,$rowData) {
      $vDescription = $value;
    ...
    public function insertbox_pd_sourch_iPostId($field, $id) {
    ...
    public function updatebox_pd_sourch_iPostId($field, $id,$value,$data) {
    ...
    public function insertbox_pd_sourch_cNip($field, $id) {
      $cNip        =  $this->sess_auth->gNIP;
      $vDescription = $cNip." - ".$this->lib_utilitas->getvName($cNip);

      $url = $this->url;
      $url6 = base_url().'processor/pk/pd/sourch?action=searchcNip';
      $url2 = base_url().'processor/pk/pd/sourch?action=searchSkema';
      $url3 = base_url().'processor/pk/pd/sourch?action=cekJabatan';
      ...
      $o = "<script type='text/javascript'>
              $(document).ready(function() {
                ...
                var akses = cekJabatan('pd_sourch_iSkemaId', '".$iPostId."');
                if (akses=='TIDAK'){
                ...
                loadSkema('pd_sourch_iSkemaId', '".$iPostId."');
                ...
                var config = {
                  ...
                  select: function(event, ui){
                    ...
                    loadSkema('pd_sourch_iSkemaId', ui.item.iPostId);
    ...
    public function updatebox_pd_sourch_cNip($field, $id,$value,$data) {
      $cNip         =  $value;
      $vDescription = $cNip." - ".$this->lib_utilitas->getvName($cNip);
      $url = $this->url;
      $url6 = base_url().'processor/pk/pd/sourch?action=searchcNip';
    ...
    public function insertbox_pd_sourch_cTahun($field, $id) {
      $year = date('Y');
      $y1 = $year - 5;
      $y2 = $year + 5;
    ...
    public function updatebox_pd_sourch_cTahun($field, $id,$value,$data) {
      $year = $value;
    ...
    public function insertbox_pd_sourch_cPeriode($field, $id) {
    ...
    public function updatebox_pd_sourch_cPeriode($field, $id,$value,$data) {
    ...
    public function insertbox_pd_sourch_iSemester($field, $id) {
      
      $url2 = base_url().'processor/pk/pd/sourch?action=cekDataPk';
      $url  = base_url()."processor/pk/pd/sourch?company_id=".$this->input->get('company_id')."&group_id=".$this->input->get('group_id')."&modul_id=".$this->input->get('modul_id');
    ...
    public function updatebox_pd_sourch_iSemester($field, $id,$value,$data) {
      $sms = array(''=>'',1=>1,2=>2);
    ...
    public function insertbox_pd_sourch_dPk($field, $id) {
      $now = date('d-m-Y');
    ...
    public function updatebox_pd_sourch_dPk($field, $id,$value,$data) {
    ...
    public function insertbox_pd_sourch_iProbation($field, $id) {
    ...
    public function updatebox_pd_sourch_iProbation($field, $id,$value,$data) {
      //1=PK Probation, tahun dan semester bisa dikosongkan.
    ...
    public function insertbox_pd_sourch_lampiran($field, $id) {
    ...
    public function updatebox_pd_sourch_lampiran($field, $id,$value,$data) {
      $action = $this->input->get('action');
      $idnya = $data['id'];
      $url = base_url().'processor/pk/pd/sourch?action=uploadproses&id='.$idnya;
      $urlhapus = base_url().'processor/pk/pd/sourch?action=hapus';
      $urlgetLampiran = base_url().'processor/pk/pd/sourch?action=getLampiran';
      $o = '';  
      $o .= '<style>
  ...
  public function insertbox_pd_sourch_iSkemaId($field, $id) {
    ...
    public function updatebox_pd_sourch_iSkemaId($field, $id,$value,$data) {
      $iPostId = $data['iPostId'];
      $url = base_url().'processor/pk/pd/sourch?action=getDetail';
  ...
  public function insertbox_pd_sourch_detail($field, $id) {
...
border: 1px solid aquamarine;' >
  ...
  public function updatebox_pd_sourch_detail($field, $id,$value,$data) {
    $idnya      = $data['iSkemaId'];
    $iPkTransId = $data['id'];
    $cNip       = $data['cNip'];
  ...
  public function updatebox_pd_sourch_Approval($field, $id,$value,$data) {
    $id = $data['id'];
   $o = '<div class="box_cbox">
    ...
    $sql = "SELECT cNip,iStatus,vReason,dDate 
          FROM hrd.pd_sourch WHERE iSkemaId='".$id."' ORDER BY id ASC";
  ...
  public function manipulate_update_button($button,$rowData) {  
    ...
    $cNipNya = $rowData['cNip'];
      ...
      $btnSave .= "<button type='button'
                ...
                onclick='javascript:update_btn_back_".$this->url."(\"".$this->url."\", \"".base_url()."processor/pk/pd/sourch?company_id=".$this->input->get('company_id')."&group_id=".$this->input->get('group_id')."&modul_id=".$this->input->get('modul_id')."\", this)'>Update 
                </button>";


      $urlsumbit  = base_url().'processor/pk/pd/sourch?action=proses_approve';
      ...
      $btnSubmit .= "<button type='button'
                ...
                id='button_update_".$this->url."'
                class='icon-save ui-button'
                onclick='javascript:btn_submit_".$this->url."(\"".$this->url."\", \"".base_url()."processor/pk/pd/sourch?company_id=".$this->input->get('company_id')."&group_id=".$this->input->get('group_id')."&modul_id=".$this->input->get('modul_id')."\", this)'>Sumbit 
                </button>";
      ...
      $btnApprove .= "<button type='button'
                ...
                id='button_update_".$this->url."'
                class='icon-save ui-button'
                onclick='javascript:btn_submit_".$this->url."(\"".$this->url."\", \"".base_url()."processor/pk/pd/sourch?company_id=".$this->input->get('company_id')."&group_id=".$this->input->get('group_id')."&modul_id=".$this->input->get('modul_id')."\", this)'>Approve 
                </button>";
      ...
      $btn_preview  = '
                      <script type="text/javascript">
                        ...
                        function print_'.$this->url.'() {
                          ...
                          document.getElementById("iframe_preview_ga_'.$this->url.'").src = "'.base_url().'processor/pk/pd/sourch?action=get_preview_report&id="+id+"&vName="+vName;
  ...
  public function manipulate_insert_button($button) {
    ...
    $btnSave  =  "<script type='text/javascript'>               
        ...
        $btnSave .= "<button type='button'
                            ...
                            class='icon-save ui-button'
                            onclick='javascript:save_btn_back_".$this->url."(\"".$this->url."\", \"".base_url()."processor/pk/pd/sourch?company_id=".$this->input->get('company_id')."&group_id=".$this->input->get('group_id')."&modul_id=".$this->input->get('modul_id')."\", this)'>Save
                            </button>";
    $button['save'] = $btnSave;
...
/*
    ...
    $html .= ' </tbody>
     ...
     $urlSD = base_url().'processor/pk/pd/sourch?action=supportData';
     $urlgetValuePk = base_url().'processor/pk/pd/sourch?action=getValuePk';
     $urlDetail = base_url().'processor/pk/pd/sourch?action=showDetail';
     $url = base_url().'processor/pk/pd/sourch?action=SaveNilai';
     $rUrl = base_url()."processor/pk/pd/sourch?company_id=".$this->input->get('company_id')."&group_id=".$this->input->get('group_id')."&modul_id=".$this->input->get('modul_id');

     $html .= "<script>

      $(document).ready(function() {
        $('.pd_sourch_nPointSepakat').autoNumeric('init', {vMin:'0', vMax:'100'});
     
      });

      $('.pd_sourch_nPointSepakat').live('keyup', function() {                
        var ix = $('.pd_sourch_nPointSepakat').index($(this));
      ...
      $('.pd_sourch_nPointSepakat').live('blur', function() {  
            var ix = $('.pd_sourch_nPointSepakat').index($(this));
      ...
      function ".$this->url."_hitungPk(vFunctionLink,iAspekId,tampil,bobot){
          ...
          document.getElementById('vDescription_pd_sourch_'+iAspekId+'_100').style.backgroundColor='#ffffff';
          document.getElementById('vDescription_pd_sourch_'+iAspekId+'_80').style.backgroundColor='#ffffff';
          document.getElementById('vDescription_pd_sourch_'+iAspekId+'_60').style.backgroundColor='#ffffff';
          document.getElementById('vDescription_pd_sourch_'+iAspekId+'_40').style.backgroundColor='#ffffff';
          document.getElementById('vDescription_pd_sourch_'+iAspekId+'_20').style.backgroundColor='#ffffff';

          document.getElementById('vDescription_pd_sourch_'+iAspekId+'_'+point).style.backgroundColor = color;
      ...
      function ".$this->url."_getValuePk(vFunctionLink,iAspekId){
        ...
        var nip = $('#pd_sourch_cNip').val();
        var dPeriode1 = $('#pd_sourch_dPeriode1').val();
        var dPeriode2 = $('#pd_sourch_dPeriode2').val();
        return $.ajax({
            type: 'POST', 
            url: '".$urlgetValuePk."',
  ...
  function generateHasilAkhir($iPkTransId,$cNip,$rowData){
    ...
    $urlHasil = base_url().'processor/pk/pd/sourch?action=SaveHasil';
    $rUrl = base_url()."processor/pk/pd/sourch?company_id=".$this->input->get('company_id')."&group_id=".$this->input->get('group_id')."&modul_id=".$this->input->get('modul_id');
    ...
    $html .= "<script>
                ...
                $(document).ready(function() {  
                  $('#pd_sourch_iDivIdTo').change(function() {
                      groupdeptid = getGroupDeptId($(this).val());
                      loadDataJbt('pd_sourch_iPostIdTo', groupdeptid, groupdeptid);
  ...
  public function searchcNip() {
    ...
    $data = array();
    $divGa = $this->lib_utilitas->getSysParam('DIV_PK_PD_SOURCH');
    $jabatan  =  $this->lib_utilitas->getSysParam("JABATAN_PK_PD_SOURCH");
  ...
  function getDetail(){
      ...
      $html .= "<script>
                  $(document).ready(function() {
                    ...
                    $('.pd_sourch_nResult').autoNumeric('init', {vMin:'0', vMax:'100.00'});
                  });


                  $('.pd_sourch_nResult').live('keyup', function() {            
                    var ix = $('.pd_sourch_nResult').index($(this));
                    
                    var sisa_sblm   = 0;
                    var result  = $('.pd_sourch_nResult').eq(ix).val();
                    var bobot   = $('.pd_sourch_nBobot').eq(ix).val();
                    var nilai   = parseFloat((result * bobot) / 100);

                    $('.pd_sourch_nNilai').eq(ix).val(nilai.toFixed(2));
                    $('.nama_pd_sourch_nNilai').eq(ix).html(nilai.toFixed(2));
                  ...
                  function sum_".$this->url."(){
                    ...
                    $('.pd_sourch_nResult').each(function() {
                      if ($('.pd_sourch_nResult').eq(i).val() != '') {        
                        total += parseFloat($('.pd_sourch_nNilai').eq(i).val());
                    ...
                    $('#pd_sourch_jumlah').html(total.toFixed(2));
                    $('#pd_sourch_nFinal').val(total.toFixed(2));
...
FROM hrd.pk_pkresult AS a WHERE a.iPkTransId='".$iPkTransId."' ";
    ...
    function getValuePk(){
      ...
      $this->lib_pd_sourch->$vFunctionLink($_POST);
    }
