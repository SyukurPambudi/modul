...
class Browse_pk_busdev_mgr extends MX_Controller {
...
public function viewform($id=0,$nipnya=0){
		...
		foreach ($dkat as $k) {
			...
			$n[]=floatval($k["bobot_bdm"])*floatval($j);
			$o.='<tr>';
			$o.='<td>'.$k["kategori"].'</td>';
			$o.='<td align="center">'.$bobot.'%</td>';
			$o.='<td align="center">'.$j.'</td>';
			$o.='<td align="center">'.number_format(floatval($k["bobot_bdm"])*floatval($j),2).'</td>';
			$o.='</tr>';
		}
		$n_final=array_sum($n);
		$o .="<tr>";
		...
		$o .= '</form>';
		$o .= '</div>';
		$o .= '<button onclick="javascript:setuju(\'pk_busdev_mgr\', \''.base_url().'processor/pk/browse/pk/busdev/mgr?action=confirm&last_id='.$id.'&foreign_key='.$this->input->get('foreign_key').'&company_id='.$this->input->get('company_id').'&group_id='.$this->input->get('group_id').'&modul_id='.$this->input->get('modul_id').'\')" class="ui-button-text icon-save" id="button_save_pk_ts">Confirm</button>';
		$o .="<script>
			$('#iDeptID').change(function(){
				var val=$(this).val();
				url=base_url+'processor/pk/browse/pk/busdev/mgr?action=getPosisi';
				if(val==''){
					var row ='';
					$('#iPostId').html(row);
					return false;
			...
			function setuju(grid, url){
				...
				//cek mail;
				$.ajax({
					url:base_url+'processor/pk/browse/pk/busdev/mgr?action=cekpass',
					type:'post',
							...
							custom_confirm('Anda Yakin?',function(){
								$.ajax({
									...
									success: function(data) {
										...
										$.get(base_url+'processor/pk/pk/busdev/mgr?action=view&id='+o.last_id+'&foreign_key='+o.foreign_id+'&company_id='+o.company_id+'&group_id='+o.group_id+'&modul_id='+o.modul_id, function(data) {
