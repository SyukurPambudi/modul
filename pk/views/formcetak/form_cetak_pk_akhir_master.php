...
<table cellspacing='0' width="98%" class="tablelast"> <!-- cellspacing='0' is important, must stay -->
	...
	<tbody>
		...
		$qdkat="select * from pk.pk_kategori kat where kat.ldeleted=0 and kat.bobot is not null";
		...
		foreach ($dkat as $k => $valo) {
			...
			$sql="select ni.poin_sepakat_atasan as atas, pa.bobot as bo from pk.pk_nilai ni
					...
					where ni.idmaster_id=".$id." and pa.ikategori_id=".$idkat." and ni.ldeleted=0 and pa.ldeleted=0 and pa.iteam_id=".$teamid;          
            }else {
			$sql="select ni.poin_sepakat_atasan as atas, pa.bobot as bo from pk.pk_nilai ni
