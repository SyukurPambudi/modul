...
<table cellspacing='0' width="98%" class="tablelast"> <!-- cellspacing='0' is important, must stay -->
	...
	<tbody>
		...
		$qdkat="select * from pk.pk_kategori kat where kat.ldeleted=0 and kat.ikategori_id in (1,2,3,4,8)";
		$dkat=$this->database->query($qdkat)->result_array();
		$n=array();
		foreach ($dkat as $k => $valo) {
			$s=array();
			$bobot=$valo["bobot_sdm"];
            ...
            if($idkat == 8){
                $sql = "select iCse as atas, 1 as bo from pk.pk_master where idmaster_id=".$id;
            }else{                
    			/*$sql="select ni.poin_sepakat_atasan as atas, pa.bobot as bo from pk.pk_nilai ni
		...
		<td align='center'><?php echo number_format($n_final,2); ?></td>
		</tr>
		<tr>
	...
	<div id="catatan_atasan">
            <b><?php echo "CATATAN UNTUK KARYAWAN:" ?></b><br>
            <?php echo "Komentar/Evaluasi Umum" ?><br>
            <textarea name="tcatatan" id="tcatatan" class="" style="width:100%"><?php echo $dcat['tcatatan_umum_pengaju']?></textarea><br>

            <?php echo "Rencana untuk periode yang akan datang" ?><br>
            <textarea name="tcatatan" id="tcatatan" class="" style="width:100%"><?php echo $dcat['tcatatan_rencana_pengaju']?></textarea><br><br>

             <b><?php echo "CATATAN UNTUK PIMPINAN:" ?></b><br>
            <?php echo "Evaluasi Umum" ?><br>
            <textarea name="tcatatan" id="tcatatan" class="" style="width:100%"><?php echo $dcat['tcatatan_umum_atasan']?></textarea><br>

            <?php echo "Saran/Perbaikan" ?><br>
            <textarea name="tcatatan" id="tcatatan" class="" style="width:100%"><?php echo $dcat['tcatatan_saran_atasan']?></textarea><br>
            
            <?php echo "Pelatihan yang diusulkan" ?><br>
            <textarea name="tcatatan" id="tcatatan" class="" style="width:100%"><?php echo $dcat['tcatatan_pelatihan_atasan']?></textarea>
