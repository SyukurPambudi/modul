...
<table cellspacing='0' width="99%" class="table"> <!-- cellspacing='0' is important, must stay -->
	...
	<thead>
		<tr>
			...
			<th rowspan="2" colspan="2">Performance Objective</th>
	...
	$sql1="select * from pk.pk_nilai ni
		...
		where ni.idmaster_id=".$id." and ni.ikategori_id=4 and pa.ldeleted=0 and ni.lDeleted=0";
	$dtni=$this->database->query($sql1)->result_array();
	$i=1;
	$nilaitot=array();
	foreach ($dtni as $key => $val) {
		$sql2="select * from pk.pk_po_deskripsi padet";
        ...
        $nPoint = $val['poin_bobot'] * $val['poin_sepakat'];
