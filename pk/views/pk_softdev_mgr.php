...
class pk_softdev_mgr extends MX_Controller {
    function __construct() {
        parent::__construct();
		$this->load->library('auth');
        $this->dbset = $this->load->database('general', true);
		$this->dbset1 = $this->load->database('pk', true);
		$this->dbset2 = $this->load->database('hrd', true);
		$this->load->library('lib_utilitas');
		$this->load->library('lib_calc');
		$this->user = $this->auth->user();
		$this->url='pk_softdev_mgr';
    }
    function index($action = '') {
		...
		$grid->setTitle('PK Softdev Manager');		
		$grid->setTable('pk.pk_master');		
		$grid->setUrl('pk_softdev_mgr');
		...
		switch ($action) {
			...
			case 'calculate_all_rincian_hitung':
				...
				$this->lib_calc->itim = $itim;
				$this->lib_calc->nippemohon = $nippemohon;
				$this->lib_calc->date1 = $date1;
				$this->lib_calc->date2 = $date2;
				$this->lib_calc->periode = $periode;
				$this->lib_calc->imaster_id = $imaster_id;
				$calc = $this->lib_calc->setCalc( $vFunction );
				if($calc == $this->lib_calc->error ) {
			...
			case 'pos_calc' :
					...
					$querydel = $this->dbset1->query( $sqldel );
			...
			case 'performance' :
					...
					foreach ($_POST['iparameter_id'] as $key => $error) {
						$sql[] = "INSERT INTO pk_nilai(idmaster_id, iparameter_id,poin,flag_dinilai,poin_bobot,poin_sepakat,poin_sepakat_atasan, ikategori_id) 
								VALUES ('".$idmaster_id."','".$iparameter_id[$i]."','".$p_k[$iparameter_id[$i]]."','".$it."','".$bobot[$iparameter_id[$i]]."','".$p_s[$iparameter_id[$i]]."','".$p_p[$iparameter_id[$i]]."',2)";
						$i++;	
						// update isubmit di tabelheader																		
					}			
					foreach($sql as $q) {
        ...
        case 'pimpin_calcs' :
			...
			case 'performance_obj_calc' :
					...
					foreach ($_POST['iparameter_id'] as $key => $error) {
					   $bot = $bobot[$iparameter_id[$i]]/100;
						$sql[] = "INSERT INTO pk_parameter_po(iparameter_id, idmaster_id, ikategori_id, iteam_id,parameter,is_auto, bobot, iUrut, vFunction) 
								VALUES ('".$iparameter_id[$i]."','".$idmaster_id."',4,'".$teamid."','".$par_name[$iparameter_id[$i]]."','".$it."','".$bot."','".$p_s[$iparameter_id[$i]]."','SDMPO')";
						$i++;	
						// update isubmit di tabelheader																		
					}
					...
					foreach ($_POST['iparameter_id'] as $key => $error) {
					   $bot = $bobot[$iparameter_id[$i]]/100;
						$sql[] = "INSERT INTO pk_nilai(idmaster_id, iparameter_id,poin,flag_dinilai,poin_bobot,poin_sepakat,poin_sepakat_atasan, ikategori_id) 
								VALUES ('".$idmaster_id."','".$iparameter_id[$i]."','".$p_k[$iparameter_id[$i]]."','".$it."','".$bot."','".$p_s[$iparameter_id[$i]]."','".$p_p[$iparameter_id[$i]]."',4)";
						$i++;	
						// update isubmit di tabelheader																		
					}			
					foreach($sql as $q) {
			...
			case 'cetak_pk_pdf':
				...
				$date=date('YmdHis');
				$pdffile = "report_pk_softdev_mgr-".$date.".pdf";
...
/*manipulasi view object form start*/ 
 	...
 	function insertBox_pk_softdev_mgr_lblIntro($field, $id){
 		$return=$this->load->view('create_master_pk_softdev_mgr_intro');
		$return.='
	...
	function updateBox_pk_softdev_mgr_lblIntro($field, $id, $value, $rowData) {
	...
	function insertBox_pk_softdev_mgr_lblpro($field, $id){
 		$return = '<script>
	...
	function updateBox_pk_softdev_mgr_lblpro($field, $id, $value, $rowData) {
 		$return = '<script>
			$("label[for=\''.$id.'\']").css({"border": "1px solid #dddddd", "background": "#548cb6", "border-collapse": "collapse","width":"98%","font-weight":"bold","color":"#ffffff","text-shadow": "0 1px 1px rgba(0, 0, 0, 0.3)","text-transform": "uppercase"});
		</script>';
		$return.='
	...
	function insertBox_pk_softdev_mgr_lbldes($field,$id){
	...
	function updateBox_pk_softdev_mgr_lbldes($field, $id, $value, $rowData) {
		$nipnya='';
	...
	function insertBox_pk_softdev_mgr_tgl_penilaian($field, $id) {
	...
	function updateBox_pk_softdev_mgr_tgl_penilaian($field, $id, $value, $rowData) {
	...
	function insertBox_pk_softdev_mgr_tgl1($field, $id) {
	...
	function updateBox_pk_softdev_mgr_tgl1($field, $id, $value, $rowData) {
		if ($this->input->get('action') == 'view') {
			$return= $value;
			...
			$return = '<input type="text" name="'.$field.'"  id="'.$id.'" value="'.$value.'"  readonly="readonly"class="  input_rows1 required" size="12" />';
			$return .= ' s/d ';
			$return .= '<input type="text" name="pk_softdev_mgr_tgl2"  id="pk_softdev_mgr_tgl2" value="'.$rowData['tgl2'].'" readonly="readonly" class="  input_rows1 required" size="12" />';
		}
		return $return;
	}

	function insertBox_pk_softdev_mgr_rincian_pk($field, $id) {
		$return = 'Save Record First';
		return $return;
	}

	function updateBox_pk_softdev_mgr_rincian_pk($field, $id, $value, $rowData) {
        ...
        $sql = "select id, vContent from pk.sysparam where cVariable ='SOFTDEV_MGR'";
        $row = $this->dbset2->query($sql)->row_array();
        $teamid = 0;
		...
		for ($x=0;$x<count($team);$x++) {
			...
			$datatab2['datemployee']=$this->dbset2->query($sqldatatab2)->row_array();
			...
			$datatablast['row']=$this->dbset1->query($sq)->row_array();      
            
            $sql = "select vContent 
                        from pk.sysparam where cVariable ='SOFTDEV_MGR_PK_CATEGORY'";
            $vContent=$this->dbset1->query($sql)->result_array();           
            ...
            // Sikap kerja
			$data['tab1'] = $this->load->view('tab1_sdm.php', $datatab1, TRUE); 
            //performance
			$data['tab2'] = $this->load->view('tab2_sdm_edit.php', $datatab2, TRUE);    
            //kepemimpinan
            $data['tab3'] = $this->load->view('tab3_sdm_kepemimpinan.php', $datatab3, TRUE);   
           //performance objective
            $data['tab4'] = $this->load->view('tab4_sdm_edit.php', $datatab4, TRUE);
                         
			$data['tab_karyawan']=$this->load->view('tab_last_karyawan_sdm.php', $datatablast, TRUE);
			$data['tab_atasan']=$this->load->view('tab_last_atasan_sdm.php', $datatablast, TRUE);
			$data['tab_last'] = $this->load->view('tab_last_sdm.php', $datatablast, TRUE);
			...
			$return .= $this->load->view('main_pk_softdev_mgr',$data,TRUE);	
...
/*manipulasi proses object form start*/
	function manipulate_update_button($buttons, $rowData){
		...
		$type='';
		$sqldatatab2="select * from hrd.employee em where cNip='".$rowData['vnip']."'";
		$datemployee=$this->dbset2->query($sqldatatab2)->row_array();
		$iatasan=0;
		if($datemployee['cUpper']==$cNip){
			$iatasan=1;
		}
		$query="select vName from hrd.employee where cNip='".$rowData['vnip']."' LIMIT 1";
		$dt=$this->dbset1->query($query)->row_array();
		$confirm='<button class="ui-button-text icon-save" onclick="$(\'#alert_dialog_form\').detach();browse(\''.base_url().'processor/pk/browse/pk/softdev/mgr?action=view&field=pk_softdev_mgr&id='.$rowData['idmaster_id'].'&vnip='.$rowData['vnip'].'&foreign_key='.$this->input->get('foreign_key').'&company_id='.$this->input->get('company_id').'&group_id='.$this->input->get('group_id').'&modul_id='.$this->input->get('modul_id').'\',\'\',\'\',\'Konfirmasi Penilaian Akhir Kerja\')" type="button">Confirm</button>';
		$note_bd='<button class="ui-button-text icon-save" onclick="$(\'#alert_dialog_form\').detach();browse(\''.base_url().'processor/pk/browse/note/karyawan/pk/softdev/mgr?action=view&field=pk_softdev_mgr&id='.$rowData['idmaster_id'].'&vnip='.$rowData['vnip'].'&foreign_key='.$this->input->get('foreign_key').'&company_id='.$this->input->get('company_id').'&group_id='.$this->input->get('group_id').'&modul_id='.$this->input->get('modul_id').'\',\'\',\'\',\'Catatan Dari Karyawan\')" type="button">Catatan</button>';
		//$btn = '<button onclick="javascript:cetak(\'pk_softdev_mgr\', \''.base_url().'processor/pk/pk/ts?action=cetak_pk&last_id='.$this->input->get('id').'&foreign_key='.$this->input->get('foreign_key').'&company_id='.$this->input->get('company_id').'&group_id='.$this->input->get('group_id').'&modul_id='.$this->input->get('modul_id').'\', this, \'pdf\', \''.$rowData["vnip"].'\')" class="ui-button-text icon-save" id="button_save_product_cetak_ts">Cetak PDF</button>';
		//$btn .= '<button onclick="javascript:cetak(\'pk_softdev_mgr\', \''.base_url().'processor/pk/pk/ts?action=cetak_pk&last_id='.$this->input->get('id').'&foreign_key='.$this->input->get('foreign_key').'&company_id='.$this->input->get('company_id').'&group_id='.$this->input->get('group_id').'&modul_id='.$this->input->get('modul_id').'\', this, \'xls\', \''.$rowData["vnip"].'\')" class="ui-button-text icon-save" id="button_save_product_cetak_ts">Cetak Excel</button>';
			...
			<script type="text/javascript">
				...
				function print_'.$this->url.'()	{
					...
					if (valid=="2") {
						...
						document.getElementById("iframe_preview_'.$this->url.'").src = "'.base_url().'processor/pk/pk/softdev/mgr?action=cetak_pk_pdf&format=pdf&id='.$rowData['idmaster_id'].'";
					}
				}		
			</script>
		';
	...
	function form_cetak_petunjuk($id){
		...
		$d=$this->dbset1->query($sqlp)->row_array();
		...
		$o.="<div style='page-break-after:always'>";
		$datapo['id']=$id;
		$o.=$this->load->view('form_cetak_pk_po_sdm',$datapo,TRUE);
		$o.='</div>';
                
		$datalast['id']=$id;
		$o.=$this->load->view('form_cetak_pk_akhir_sdm',$datalast,TRUE);
		//$o="sasa";
		return $o;
	}
...
function before_insert_processor($row, $postData) {
	...
	*/
    $sql = "select id, vContent from hrd.ss_sysparam where cVariable ='SOFTDEV_MGR'";
    $rows = $this->dbset2->query($sql)->row_array();
    $teamid = 20;
...
public function spv_pm2(){
	...
	$datamaster['tgl2']=$this->input->get('akhir');
		...
		$data['sql']=$sqlmain;
		$dupb = $this->dbset->query($sqlmain)->result_array();
