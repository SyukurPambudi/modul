...
<input type="hidden" name="modul_id" value="<?php echo $_GET['modul_id'] ?>" id="modul_id">
	...
	<?php
		...
		if($rowData['vnip']==$this->user->gNIP){
			if(($rowData['iSubmit']>=2)&&($rowData['iSubmit_Sikap']>=2)&&($rowData['is_confirm']==0)){
			echo '<li>
					  <a href="#tab_last">PROSES AKHIR</a>
				  </li>';
			}else if(($rowData['iSubmit']>=2)&&($rowData['iSubmit_Sikap']>=2)&&($rowData['is_confirm']==1)){
				echo '<li>
			...
			if($rowData['is_confirm']==1){
				...
				if($employee['cUpper']==$this->user->gNIP){
					echo '<li>
					  <a href="#tab_last">PROSES AKHIR</a>
				  </li>';
				}
