...
//echo $mydept.'ini teamnya';
		$o='';
			...
			// get parameter PK Busdev
				$sqlp ='select * from plc2.pk_parameter a where a.ldeleted=0 and a.ikategori_id=1 and a.iteam_id ="'.$itim.'" ';
			}else{
				$sqlp='select * from plc2.pk_nilai ni
						inner join plc2.pk_parameter pa on ni.iparameter_id=pa.iparameter_id
						where ni.idmaster_id='.$this->input->get('id').' and pa.ikategori_id=1';
			}
		...
		if($queryp->num_rows()>0) {
			...
			foreach($rows as $key => $row) {
				...
				if($querypd->num_rows()>0) {
					...
					$o.='<td align="center"><input type="hidden" value="'.$po.'" id="nilai_'.$iparameter_id.'" class="tabs1" name="nilai_'.$iparameter_id.'" /><p id="lbl_nilai_'.$iparameter_id.'">'.$po.'</p></td>';
					$o.='</tr>';
				}
			$nomor++;	
			}
...
<script type="text/javascript">
	...
	$('#submit_draft_tab1').live('click', function(){
		...
		var it = $("#it").val();
		return $.ajax({
			url: BASE_URL+'processor/pk/pk_busdev?action=sikap_calc',
			type: 'post',
