...
class auth { 	
	...
	function status_daftar() {
		$sqldaftar = "select * from koperasi.kop_master_anggota a where a.cNip='".$this->sess_auth->gNIP."' and a.ldeleted=0";
		$ms = $this->_ci->db->query($sqldaftar)->row_array();
		return $ms['istatus_form'];
	}
	function status_anggota() {
		$sqldaftar = "select * from koperasi.kop_master_anggota a where a.cNip='".$this->sess_auth->gNIP."' and a.ldeleted=0";
		$ms = $this->_ci->db->query($sqldaftar)->row_array();
		return $ms['istatus'];
	}
	function lama_gabung() {
		$sqldaftar = "select *,TIMESTAMPDIFF(MONTH,  a.dgabung,now()) as lama from koperasi.kop_master_anggota a where a.cNip='".$this->sess_auth->gNIP."' and a.ldeleted=0";
		$ms = $this->_ci->db->query($sqldaftar)->row_array();
		return $ms['lama'];
	}

	function is_resign() {
		$sqlmgr = "SELECT COUNT(1) c FROM hrd.employee t
				   WHERE t.cNip = '".$this->sess_auth->gNIP."' and t.ldeleted=0 and (t.dresign!='0000-00-00')";
		$ms = $this->_ci->db->query($sqlmgr)->row_array();
		return $ms['c'];
	}

	function anggota_id() {
		$sqlmgr = "select * from koperasi.kop_master_anggota a where a.cNip='".$this->sess_auth->gNIP."' and a.ldeleted=0";
		$ms = $this->_ci->db->query($sqlmgr)->row_array();
		return $ms['id'];
	}
	...
	function level() {
		$sqlmgr = "select b.level from koperasi.kop_master_anggota a 
					join koperasi.kop_master_jabatan b on a.jabatan_id=b.id
					where a.cNip='".$this->sess_auth->gNIP."' and a.ldeleted=0";
		$ms = $this->_ci->db->query($sqlmgr)->row_array();
		return $ms['level'];
	}

	function jabatan_id() {
		$sqlmgr = "select a.jabatan_id from koperasi.kop_master_anggota a 
					join koperasi.kop_master_jabatan b on a.jabatan_id=b.id
					where a.cNip='".$this->sess_auth->gNIP."' and a.ldeleted=0";

		$ms = $this->_ci->db->query($sqlmgr)->row_array();
		//return $ms['jabatan_id'] > 0;
		return $ms['jabatan_id'];
	}
