...
class auth_export { 	
	...
	function is_managerdept_id($dept,$id) {
		$sqlmgr = "SELECT COUNT(1) c FROM reformulasi.reformulasi_team t
				   WHERE t.vnip = '".$this->sess_auth->gNIP."' and t.vtipe='".$dept."' and t.ireformulasi_team='".$id."'  and t.ldeleted=0 and t.iTipe=2";
		$ms = $this->_ci->db->query($sqlmgr)->row_array();
		return $ms['c'] > 0;
	}
	...
	function is_dir() {
		$sqlmgr = "SELECT COUNT(1) c FROM reformulasi.reformulasi_team t
				   ...
				   and rmd.vkode_departement='DR' and t.ldeleted=0 and t.iTipe=2";
		$ms = $this->_ci->db->query($sqlmgr)->row_array();
		return $ms['c'] > 0;
	}
	function is_andev() {
		$sqlmgr = "SELECT COUNT(1) c FROM reformulasi.reformulasi_team t
				   join reformulasi.reformulasi_master_departement rmd on rmd.ireformulasi_master_departement=t.cDeptId
				   WHERE t.vnip = '".$this->sess_auth->gNIP."' and rmd.vkode_departement='AD' and t.ldeleted=0 and t.iTipe=2";
		$ms = $this->_ci->db->query($sqlmgr)->row_array();
		return $ms['c'] > 0;
	}
	function is_bdirm() {
		$sqlmgr = "SELECT COUNT(1) c FROM reformulasi.reformulasi_team t
					join reformulasi.reformulasi_master_departement rmd on rmd.ireformulasi_master_departement=t.cDeptId
				   WHERE t.vnip = '".$this->sess_auth->gNIP."' and rmd.vkode_departement='BDI' and t.ldeleted=0 and t.iTipe=2";
		$ms = $this->_ci->db->query($sqlmgr)->row_array();
		return $ms['c'] > 0;
	}
	function team() {
		...
		$sql = "SELECT t.ireformulasi_team FROM reformulasi.reformulasi_team_item t
				join reformulasi.reformulasi_team te on te.ireformulasi_team=t.ireformulasi_team
			   	WHERE t.vnip = '".$this->sess_auth->gNIP."' and t.ldeleted=0 and te.iTipe=2";
