...
class master_jenis_sediaan_refor extends MX_Controller {
    ...
    function index($action = '') {
        ...
        $grid = new Grid;
        $grid->setTitle('Master Jenis Sediaan Reformulasi');
        $grid->setTable('reformulasi.master_jenis_sediaan_refor');      
        $grid->setUrl('master_jenis_sediaan_refor');

        //List Table
        $grid->addList('jenis_sediaan','keterangan','status'); 
        $grid->setSortBy('jenis_sediaan','status');
        $grid->setSortOrder('DESC');  

        //List field
        $grid->addFields('jenis_sediaan','keterangan','status'); 
        ...
        */
        ...
        $grid->setWidth('jenis_sediaan', '250');
        $grid->setAlign('jenis_sediaan', 'left');
        $grid->setLabel('jenis_sediaan','Jenis Kesediaan');

        $grid->setWidth('keterangan', '250');
        $grid->setAlign('keterangan', 'left');
        $grid->setLabel('keterangan','Keterangan');

        $grid->setLabel('status','Status');
        $grid->changeFieldType('status','combobox', '', array(0=>'Active', 1=>'Deleted'));
    
//Example modifikasi GRID ERP
    //- Set Query
        $grid->setQuery('master_jenis_sediaan_refor.lDeleted = 0 ', null); 
        /*if ($this->validation) {
...
*/
    ...
    //   $grid->setJoinTable('axcelor.axl_rooms_type','axl_rooms_type.iRoomTypeID = axl_rooms.iRoomTypeID', 'inner');
    //set search
        $grid->setSearch('jenis_sediaan','status');
        
    //set required
        $grid->setRequired('id','jenis_sediaan','keterangan','status','dCreate','cCreated','dUpdate','cUpdate','lDeleted'); 
        ...
        switch ($action) {
                ...
                case 'createproses':/*
                    $post=$this->input->post();print_r($post);exit();*/
                      ...
                      //  $nama_alasan = ['nama_alasan'];
                        $name = $post['master_jenis_sediaan_refor_jenis_sediaan'];
                        $cek  = $this->db->get_where('reformulasi.master_jenis_sediaan_refor', array('jenis_sediaan' => $name, 'lDeleted' => 0))->num_rows(); 
                        if ($cek > 0){
                            $data['status']  = false;
            ...
            case 'updateproses':
                        $post   = $this->input->post(); 
                        $id     = $post['master_jenis_sediaan_refor_id_sediaan']; 
                        $name   = $post['master_jenis_sediaan_refor_jenis_sediaan']; 
                        $sql    = 'SELECT * FROM reformulasi.master_jenis_sediaan_refor WHERE id_sediaan <> '.$id.' AND jenis_sediaan = "'.$name.'" AND lDeleted = 0'; //echo $sql;exit();   
                        //echo ($sql);exit();
   ...
   /* function insertBox_master_jenis_sediaan_refor_jenis_sediaan($field, $id) {
        $teams = $this->db_schedulercheck->get_where('reformulasi.master_jenis_sediaan_refor', array('iDeleted' => 0))->result_array();
        $o = '<select class="required" name="'.$id.'" id="'.$id.'">';
        $o .= '<option value="">--Select--</option>';
        foreach ($teams as $t) {
            $o .= '<option value="'.$t['jenis_sediaan'].'">'.$t['jenis_sediaan'].'</option>';
        }
        $o .= '</select>';
        return $o;
    }
*/
    function insertBox_master_jenis_sediaan_refor_keterangan($field, $id) {
        if ($this->input->get('action') == 'view') { 
            $sql='select * from reformulasi ="'.$value.'" ';
    ...
    function updateBox_master_bentuk_sediaan_refor_id_sediaan($field, $id, $value, $rowData) {
        if ($this->input->get('action') == 'view') { 
            $sql='select * from reformulasi.master_jenis_sediaan_refor="'.$value.'" ';
            $data = $this->db->query($sql)->row_array();
    ...
    function updateBox_master_jenis_sediaan_refor_keterangan($field, $id, $value, $rowData) {
    if ($this->input->get('action') == 'view') { 
     ...
     public function insertbox_master_jenis_sediaan_refor_status($field, $id) {
    ...
    public function updatebox_master_jenis_sediaan_refor_status($field, $id, $value) {
        $lmarketing = array(0=>'Active', 1=>'lDeleted');
        if ($this->input->get('action') == 'view') {
            $o = $lmarketing[$value];
        } else {
