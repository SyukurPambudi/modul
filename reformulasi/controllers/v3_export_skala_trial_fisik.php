...
class v3_export_skala_trial_fisik extends MX_Controller {
    function __construct() {
		...
		$this->url = 'v3_export_skala_trial_fisik';
		...
		$datagrid['islist'] = array(
			...
			'isubmit_fisik'                         => array('label' => 'Submit',					'width' => 150,	'align' => 'left',		'search'=>true),
		);		
    ...
    function index($action = '') {
		...
		$grid->changeFieldType('isubmit_fisik', 'combobox','',array('' => '--select--', 0 => 'Draft - Need to be Submit', 1 => 'Submited'));
	...
	/*List Box*/
	function listBox_v3_export_skala_trial_fisik_cPIC_AD($value, $pk, $name, $rowData) {
	...
	function listBox_v3_export_skala_trial_fisik_dossier_upd_iteampd_id($value, $pk, $name, $rowData) {
	...
	function insertBox_v3_export_skala_trial_fisik_form_detail($field,$id){
            $get=$this->input->get();
            $post=$this->input->post();
        ...
        function updateBox_v3_export_skala_trial_fisik_form_detail($field,$id,$value,$rowData){
            $get=$this->input->get();
            $post=$this->input->post();
            foreach ($get as $kget => $vget) {
                if($kget!="action"){
