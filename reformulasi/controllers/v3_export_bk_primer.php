...
class v3_export_bk_primer extends MX_Controller {
    function __construct() {
		...
		$this->title = 'Bahan Kemas Primer';
		$this->url = 'v3_export_bk_primer';
		$this->urlpath = 'reformulasi/'.str_replace("_","/", $this->url);

		$this->maintable = 'reformulasi.export_bahan_kemas';	
		$this->main_table = $this->maintable;	
		$this->main_table_pk = 'iexport_bk_id';	
	...
	/*List Box*/
	function listBox_v3_export_bk_primer_cPIC_AD($value, $pk, $name, $rowData) {
	...
	function listBox_v3_export_bk_primer_dossier_upd_iteampd_id($value, $pk, $name, $rowData) {
	...
	function insertBox_v3_export_bk_primer_form_detail($field,$id){
            $get=$this->input->get();
            $post=$this->input->post();
        ...
        function updateBox_v3_export_bk_primer_form_detail($field,$id,$value,$rowData){
            $get=$this->input->get();
            $post=$this->input->post();
            foreach ($get as $kget => $vget) {
                if($kget!="action"){
