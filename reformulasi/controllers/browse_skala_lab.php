...
class browse_skala_lab extends MX_Controller {
    ...
    function index($action = '') {
        ...
        #Cek Maping Skala Lab
        $grid->setQuery('IF(`lokal_mapping_refor`.`imaster_proses_refor`=5,
             #Cek Melewati Stress Test
             IF((SELECT COUNT(lm.`ilokal_mapping_refor`) AS ck_skl_trial FROM reformulasi.`lokal_mapping_refor` lm WHERE lm.`lDeleted` = 0 
             AND lm.`imaster_proses_refor` = 3 AND lm.`iLocal_req_refor` = `local_req_refor`.`iLocal_req_refor`)>0,
                 
                #Kalo iya Harus Approve dan MS
                (SELECT COUNT(ls.iLocal_req_refor) FROM reformulasi.`lokal_refor_stress_test` ls WHERE ls.`lDeleted` = 0 AND 
                ls.iksm_stress_test = 1 AND ls.iflag_open=0 AND ls.`iappd` = 2 AND ls.`iLocal_req_refor` = `local_req_refor`.`iLocal_req_refor`)>0,
                ...
                AND lm.`imaster_proses_refor` = 4 AND lm.`iLocal_req_refor` = `local_req_refor`.`iLocal_req_refor`)>0,
                    ...
                    AND lm.`imaster_proses_refor` = 1 AND lm.`iLocal_req_refor` = `local_req_refor`.`iLocal_req_refor`)>0,
                        ...
                        #Kalo Tidak Kembali ke Kondisi Awal Tanpa Pengecakan
                         `lokal_mapping_refor`.`imaster_proses_refor`=5))),
              
              #Kalo Tidak Kembali ke Kondisi Awal Tanpa Pengecakan
             `lokal_mapping_refor`.`imaster_proses_refor`=5)  ', null); 
        
        #cek Ketersediaan Data
        $grid->setQuery('local_req_refor.iLocal_req_refor NOT IN(
            SELECT rl.`iLocal_req_refor` FROM reformulasi.`lokal_refor_skala_lab` rl WHERE 
            rl.`iLocal_req_refor` = local_req_refor.iLocal_req_refor
            AND IF(rl.`iappd`=2,IF(rl.`iksm_skala_lab`=0,rl.`iksm_skala_lab`=1,rl.`lDeleted`=0),rl.`lDeleted`=0) 
    ...
    function dt_upload(){
        ...
        echo $this->load->view('local/lokal_skala_lab_ref_file',$data,TRUE);
    }

    

	function listBox_browse_skala_lab_pilih($value, $pk, $name, $rowData) {
			  ...
			  <script type="text/javascript">
				function pilih_upb_detail (id, cIteno, vNo_req_refor, vBatch_no, vAlasan_refor, 
					...
					custom_confirm("Yakin ?", function(){
                        ...
                        $(".'.$this->input->get('field').'_cpic_skala_lab_").val(_cFormulator);  
                        $(".'.$this->input->get('field').'_cpic_skala_lab").val(cFormulator);  
