...
class local_draft_mbr_popup extends MX_Controller {
    ...
    function index($action = '') {
		...
		$grid->setUrl('local_draft_mbr_popup');
		...
		$grid->setQuery('lokal_refor_skala_trial.ilokal_refor_skala_trial IN (CASE
				#Apabila Stabilita Lab harus di lalui
				WHEN
				 	(select count(ma.iLocal_req_refor) as jml from reformulasi.lokal_mapping_refor ma
				 		...
				 		where ma.lDeleted=0 and m.lDeleted=0 and m.vkode_proses_refor="STBL" and ma.iLocal_req_refor=lokal_refor_skala_trial.iLocal_req_refor)>0
			 	THEN
				 	(select tr.ilokal_refor_skala_trial from reformulasi.lokal_refor_skala_trial tr
					 join reformulasi.lokal_refor_stabilita_lab lb on lb.ilokal_refor_skala_trial=tr.ilokal_refor_skala_trial
					 where tr.ldeleted=0 and lb.ldeleted=0 and tr.iflag_open=0 and lb.iflag_open=0 and lb.iapppd=2 and lb.ihasil=1 and lb.ilokal_refor_skala_trial=lokal_refor_skala_trial.ilokal_refor_skala_trial)
			...
			ELSE
			  		(select lokal_refor_skala_trial.ilokal_refor_skala_trial from reformulasi.lokal_refor_skala_trial tr where tr.ldeleted=0 and tr.iappd=2 and tr.iksm_skala_trial=1 and tr.iLocal_req_refor=lokal_refor_skala_trial.iLocal_req_refor)
			end)',NULL);
		$grid->setQuery('lokal_refor_skala_trial.ilokal_refor_skala_trial NOT IN (select st.ilokal_refor_skala_trial from reformulasi.lokal_ref_dok_mbr st where st.ldeleted=0)',NULL);
	...
	function listBox_local_draft_mbr_popup_pilih($value, $pk, $name, $rowData) {
		...
		$o = '<input type="radio" name="pilih" onClick="javascript:pilih_lokal_draft_mbr(\''.$pk.'\',\''.$dtrefor['iLocal_req_refor'].'\',\''.$dtrefor['vNo_req_refor'].'\',\''.$dtrefor['c_itnam'].'\',\''.$dtrefor['vBatch_no'].'\',\''.$dtrefor['cInisiator'].'\',\''.$iAm['vdepartemen'].'\',\''.$dtrefor['vAlasan_refor'].'\',\''.$iteam[$dtrefor['iteam_pd']].'\',\''.$dtrefor['dRequest'].'\',\''.$iapp['vName'].'\',\''.$req['vName'].'\',\''.$rowData->vnoFormulasi.'\') ;" />';
		$o.='<script>
				function pilih_lokal_draft_mbr(ilokal_refor_skala_trial,iLocal_req_refor,vNo_req_refor,cIteno,vBatch_no,cInisiator,vdepartemen,vAlasan_refor,iteam_pd,dRequest,cApproval,cFormulator,vnoFormulasi){
