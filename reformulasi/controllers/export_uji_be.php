...
class export_uji_be extends MX_Controller {
    function __construct() {
		...
		$this->filepath='files/reformulasi/export/uji_be';
		$this->filepathreq="files/reformulasi/export/export_reg_file";
		$this->urlpath="processor/reformulasi/export/uji/be";
		$this->gridname="export_uji_be";
    }
    function index($action = '') {
    	$action = $this->input->get('action');
    	//Bikin Object Baru Nama nya $grid		
		$grid = new Grid;
		$grid->setTitle('Reformulasi - Uji BE');		
		$grid->setTable('reformulasi.export_uji_be');		
		...
		$grid->addFields('iexport_refor_formula','vUpd_no','vNama_usulan','cInisiator_export','iDapartemen_export','tAlasan_export','vUploadfile','iTeamPD','dPermintaan_req_export','cApproval_ats_inisiator','cPicFormulator','lbl','dujebe','dselesaiujebe','vfile','istatus_ujibe','iperlu_sample','iapprove_pd');
		...
		$grid->setLabel('lbl', ' ');
		$grid->setLabel('dujebe', 'Tanggal Uji BE');
		$grid->setLabel('dselesaiujebe', 'Tanggal Selesai Uji BE');
		$grid->setLabel('vfile', 'File Upload');
		$grid->setLabel('istatus_ujibe', 'Status Approval');
		...
		$grid->setJoinTable('reformulasi.export_refor_formula', 'export_refor_formula.iexport_refor_formula=reformulasi.export_uji_be.iexport_refor_formula', 'inner');
		...
		$grid->setRequired('iexport_refor_formula','dujebe','dselesaiujebe','vfile','istatus_ujibe','iperlu_sample');
		...
		switch ($action) {
			...
			case 'createproses':
   				...
   				if($isUpload) {
					...
					foreach($_POST as $key=>$value) {						
						if ($key == 'export_uji_be_file_fileketerangan') {
					...
					$i=0;
					foreach ($_FILES['export_uji_be_file_upload_file']["error"] as $key => $error) {
						if ($error == UPLOAD_ERR_OK) {
							$tmp_name = $_FILES['export_uji_be_file_upload_file']["tmp_name"][$key];
							$name =$_FILES['export_uji_be_file_upload_file']["name"][$key];
							$data['filename'] = $name;
							$data['dInsertDate'] = date('Y-m-d H:i:s');

								if(move_uploaded_file($tmp_name, $path."/".$lastId."/".$name)) {
									$sql[]="INSERT INTO reformulasi.export_uji_be_file (iexport_uji_be, vfilename, tketerangan, dcreate, ccreated) 
			...
			case 'updateproses':
				...
				$lastId=$post['export_uji_be_iexport_uji_be'];
				$path = realpath($this->filepath);
				if(!file_exists($path."/".$lastId)){
					if (!mkdir($path."/".$lastId, 0777, true)) { //id review
						die('Failed upload, try again!');
                ...
                if($isUpload) {
                	if($fileid!=''){
                        ...
                        $sql1="update reformulasi.export_uji_be_file set ldeleted=1, dupdate='".$tgl."', cupdate='".$this->user->gNIP."' where iexport_uji_be='".$lastId."' and iexport_uji_be_file not in (".$fileid.")";
                        $this->dbset->query($sql1);
                    }else{
                        $tgl= date('Y-m-d H:i:s');
                        $sql1="update reformulasi.export_uji_be_file set ldeleted=1, dupdate='".$tgl."', cupdate='".$this->user->gNIP."' where iexport_uji_be='".$lastId."'";
                        $this->dbset->query($sql1);
                    }
                    $i=0;
                    if (isset($_FILES['export_uji_be_file_upload_file']))  {
                        foreach ($_FILES['export_uji_be_file_upload_file']["error"] as $key => $error) {
                            if ($error == UPLOAD_ERR_OK) {
                                $tmp_name = $_FILES['export_uji_be_file_upload_file']["tmp_name"][$key];
                                $name =$_FILES['export_uji_be_file_upload_file']["name"][$key];
	...
	function listBox_export_uji_be_isubmit($v, $pk, $name, $rowData) {
		$re="-";
		if($v==0){
	...
	function listBox_export_uji_be_iapprove_ad($v, $pk, $name, $rowData) {
		$sa=array(0=>'Waiting Approval',1=>'Rejected',2=>'Approved');
		$re=isset($sa[$v])?$sa[$v]:'-';
		return $re;
	}

	function listBox_export_uji_be_iapprove_qa($v, $pk, $name, $rowData) {
		$sa=array(0=>'Waiting Approval',1=>'Rejected',2=>'Approved');
		$re=isset($sa[$v])?$sa[$v]:'-';
		return $re;
	}
	...
	function insertBox_export_uji_be_iexport_refor_formula($f, $i){
		$r = '<script>
		...
		$r .= '&nbsp;<button class="icon_pop"  onclick="browse(\''.base_url().'processor/reformulasi/browse_export_uji_be?field=export_uji_be&modul_id='.$this->input->get('modul_id').'\',\'\')" type="button">&nbsp;</button>';
		return $r;
	}

	function insertBox_export_uji_be_vUpd_no($f, $i){
		return '<input type="text" disabled="TRUE" name="'.$i.'" id="'.$i.'" class="input_rows1" size="20" />';
	}

	function insertBox_export_uji_be_vNama_usulan($f, $i){
		return '<textarea disabled="TRUE" name="'.$i.'" id="'.$i.'"></textarea>';
	}

	function insertBox_export_uji_be_cInisiator_export($f, $i){
		return '<textarea disabled="TRUE" name="'.$i.'" id="'.$i.'"></textarea>';
	}

	function insertBox_export_uji_be_iDapartemen_export($f, $i){
		return '<input type="text" disabled="TRUE" name="'.$i.'" id="'.$i.'" class="input_rows1" size="20" />';
	}

	function insertBox_export_uji_be_tAlasan_export($f, $i){
		return '<textarea disabled="TRUE" name="'.$i.'" id="'.$i.'"></textarea>';
	}

	function insertBox_export_uji_be_vUploadfile($f, $i){
		$data['id']=0;
		$data['get']=$this->input->get();
		return $this->load->view('export/export_uji_be_file',$data,TRUE);
	}

	function insertBox_export_uji_be_iTeamPD($f, $i){
		return '<input type="text" disabled="TRUE" name="'.$i.'" id="'.$i.'" class="input_rows1" size="20" />';
	}

	function insertBox_export_uji_be_dPermintaan_req_export($f, $i){
		return '<input type="text" disabled="TRUE" name="'.$i.'" id="'.$i.'" class="input_rows1" size="20" />';
	}

	function insertBox_export_uji_be_cApproval_ats_inisiator($f, $i){
		return '<textarea disabled="TRUE" name="'.$i.'" id="'.$i.'"></textarea>';
	}

	function insertBox_export_uji_be_cPicFormulator($f, $i){
		$r= '<textarea disabled="TRUE" name="'.$i.'" id="'.$i.'"></textarea>';
		return $r;
	}

	function insertBox_export_uji_be_lbl($f, $i){
		$r = '<script>
	...
	function insertBox_export_uji_be_dujebe($f, $i){
	...
	function insertBox_export_uji_be_dselesaiujebe($f, $i){
		$r = '<input name="'.$i.'" id="'.$i.'" type="text" size="20" class="input_tgl datepicker required" style="width:130px"/>';
		$r .=	'<script>
						$("#'.$i.'").datepicker({dateFormat:"yy-mm-dd",changeMonth: true,changeYear: true});
					</script>';
	...
	function insertBox_export_uji_be_vfile(){
		$data['id']=0;
		$data['get']=$this->input->get();
		return $this->load->view('export/export_uji_be_file_upload',$data,TRUE);
	}

	function insertBox_export_uji_be_istatus_ujibe($f, $i){
		$sa=array(0=>"TMS",1=>"MS");
	...
	function insertBox_export_uji_be_iperlu_sample($f, $i){
		...
		$ret.='var sa = $("#export_uji_be_istatus_ujibe").val();';
		$ret.='if(sa != "0"){ $("#'.$i.'").parent().parent().hide() }';
		$ret.='$("#export_uji_be_istatus_ujibe").change(function(){ if($(this).val()=="0"){ $("#'.$i.'").parent().parent().show() }else{ $("#'.$i.'").parent().parent().hide() } });';
		$ret.='</script>';
		return $ret;
	}
	function insertBox_export_uji_be_iapprove_pd($f, $i){
		return "-";
	}
	...
	function manipulate_insert_button($buttons) {
		...
		$grid='export_uji_be';
		$data['grid']=$grid;
		$js = $this->load->view('export/js/export_uji_be_js',$data);
	...
	function after_insert_processor($r, $insertid, $postData){
		...
		if($postData['isubmit']==1){
			...
			$data['menuapp']="ERP -> Reformulasi -> Export -> Uji BE";
			$data['caption']="Diberitahukan telah ada Submit pada Aplikasi Reformulasi Export module Uji BE, dengan rincian sebagai berikut :";
			$subject = "Reformulasi Export - Uji BE (".$dform['export_req_refor__vno_export_req_refor'].")";
	...
	function updateBox_export_uji_be_iexport_refor_formula($f, $i, $v, $row){
		$val=$this->getDetailsForm($row['iexport_uji_be'],'export_req_refor__vno_export_req_refor');
		$r = '<script>
					$( "button.icon_pop" ).button({
						icons: {
							primary: "ui-icon-newwin"
	...
	function updateBox_export_uji_be_vUpd_no($f, $i, $v, $row){
	...
	function updateBox_export_uji_be_lbl($f, $i){
		$r = '<script>
			$("label[for=\''.$i.'\']").css({"border": "1px solid #dddddd", "background": "#548cb6", "border-collapse": "collapse","width":"99%","font-weight":"bold","color":"#ffffff","text-shadow": "0 1px 1px rgba(0, 0, 0, 0.3)","text-transform": "uppercase"});
		</script>';
		return $r;
	...
	function updateBox_export_uji_be_dujebe($f, $i, $v, $row){
		$dis="";
		if($this->input->get('action')=='view' or $row['isubmit']>0){
			$dis='disabled="TRUE"';
		}
	...
	function updateBox_export_uji_be_dselesaiujebe($f, $i, $v, $row){
		$dis="";
		if($this->input->get('action')=='view' or $row['isubmit']>0){
			$dis='disabled="TRUE"';
		}
	...
	function updateBox_export_uji_be_vfile($f, $i, $v, $row){
		$data['id']=$row['iexport_uji_be'];
		$data['get']=$this->input->get();
		$data['isubmit']=$row['isubmit'];
		return $this->load->view('export/export_uji_be_file_upload',$data,TRUE);
	}

	function updateBox_export_uji_be_istatus_ujibe($f, $i, $v, $row){
		$dis="";
		$ret="";
		$sa=array(0=>"TMS",1=>"MS");
		if($this->input->get('action')=='view' or $row['isubmit']>0){
	...
	function updateBox_export_uji_be_iperlu_sample($f, $i, $v, $row){
		$dis="";
		$ret="";
		$sa=array(0=>"Tidak",1=>"Ya");
		if($this->input->get('action')=='view' or $row['isubmit']>0){
	...
	function updateBox_export_uji_be_iapprove_pd($f, $i, $v, $row){
		$ret='';
		if($v==0){
	...
	function manipulate_update_button($btn, $row){
		...
		$approvead = '<button onclick="javascript:load_popup_'.$grid.'(\''.base_url().'processor/reformulasi/'.$grid.'?action=approve&iexport_uji_be='.$row['iexport_uji_be'].'&group_id='.$this->input->get('group_id').'&modul_id='.$this->input->get('modul_id').'&company_id='.$this->input->get('company_id').'\',\'\',\'Approve Draft Uji BE\')" class="ui-button-text icon-save" id="button_approve_'.$grid.'">Approve</button>';
		$btn2='';
		if($this->auth_export->is_manager()){
			$x=$this->auth_export->dept();
			$manager=$x['manager'];
	...
	function approve_view() {
		...
		$echo .= '<form id="form_approve_export_uji_be" action="'.base_url().$this->urlpath.'?action=approve_process" method="post">';
		$echo .= '<div style="vertical-align: top;">';
		$echo .= 'Remark : 
				<input type="hidden" name="approveBox_export_uji_be_iexport_uji_be" value="'.$this->input->get('iexport_uji_be').'" />
				<input type="hidden" name="approveBox_export_uji_be_group_id" value="'.$this->input->get('group_id').'" />
				<input type="hidden" name="approveBox_export_uji_be_modul_id" value="'.$this->input->get('modul_id').'" />
				<input type="hidden" name="approveBox_export_uji_be_company_id" value="'.$this->input->get('company_id').'" />
				<input type="hidden" name="approveBox_export_uji_be_foreign_id" value="'.$this->input->get('foreign_id').'" />
				<input type="hidden" name="approveBox_export_uji_be_iapprove_pd" value="2" />
				<textarea id="approveBox_export_uji_be_vRemark" name="approveBox_export_uji_be_vRemark"></textarea>';
		$echo .= '</div>';
		$echo .= '</form>';
	...
	function approve_process(){
		...
		$dataup=array();
		$dataup['iapprove_pd']=$post['approveBox_export_uji_be_iapprove_pd'];
		$dataup['tapprove_pd']=$post['approveBox_export_uji_be_vRemark'];
		...
		$this->dbset->where('iexport_uji_be',$post['approveBox_export_uji_be_iexport_uji_be']);
		if($this->dbset->update('reformulasi.export_uji_be',$dataup)){

			/*Cek Untuk Permintaan Sample*/

			$sqlpro="select * from reformulasi.export_uji_be r where r.ldeleted=0 and r.iexport_uji_be=".$post['approveBox_export_uji_be_iexport_uji_be'];
			$dr=$this->dbset->query($sqlpro)->row_array();
			$datasample=array();
			if(count($dr)>=1){
				if($dr['istatus_ujibe']==0 && $dr['iperlu_sample']==1){ // Insert Ke Sample Bahan Baku
                    ...
                    $insD['iTujuan_request'] = 6; // Tujuan ke Uji BE
				...
				if($dr['istatus_ujibe']==0 && $dr['iperlu_sample']==0){// Kembali Ke Trial
			...
			$data['group_id']=$post['approveBox_export_uji_be_group_id'];
			$data['modul_id']=$post['approveBox_export_uji_be_modul_id'];
			$data['foreign_id']=$post['approveBox_export_uji_be_foreign_id'];
			$data['company_id']=$post['approveBox_export_uji_be_company_id'];
			$data['last_id']=$post['approveBox_export_uji_be_iexport_uji_be'];

			/*Send Notif*/
			$id=$post['approveBox_export_uji_be_iexport_uji_be'];
			...
			$to='';
			if(count($dr)>=1){
				if($dr['istatus_ujibe']==0 && $dr['iperlu_sample']==1){ // Kirim Email Ke Sample
				...
				if($dr['istatus_ujibe']==0 && $dr['iperlu_sample']==0){ // Kirim Ke Trial
					$data['menuapp']="ERP -> Reformulasi -> Export -> Formula Skala Trial";
					$data['caption']="Diberitahukan telah ada Approval Pada Uji BE untuk dilakukan penginputan pada module Formula Skala Trial, dengan rincian sebagai berikut :";
				...
				if($dr['istatus_ujibe']==1){ // Kirim Untuk Hasil 
					$data['menuapp']="ERP -> Reformulasi -> Export -> Uji BE";
					$data['caption']="Diberitahukan telah ada Approval Pada Uji BE untuk dilakukan penginputan pada module Formula Skala Trial, dengan rincian sebagai berikut :";
					$subject = "Reformulasi Export - Uji BE (".$dform['export_req_refor__vno_export_req_refor'].")";
					$data['cdata'] = array(
    ...
    function getDetailsForm($id=0,$field='null'){
    	$sql="SELECT export_req_refor.vno_export_req_refor AS export_req_refor__vno_export_req_refor, dossier_upd.vNama_usulan AS dossier_upd__vNama_usulan, export_req_refor.iTeamPD as export_req_refor__iTeamPD, reformulasi.export_refor_formula.*, export_uji_be.iexport_uji_be
			...
			INNER JOIN reformulasi.export_uji_be ON export_uji_be.iexport_refor_formula=reformulasi.export_refor_formula.iexport_refor_formula
			WHERE export_uji_be.iexport_uji_be=".$id;
    ...
    function getdetailsprev(){
		...
		foreach ($q->result() as $k) {
			...
			foreach ($rsel as $dsel => $vsel) {
				if($vsel=="update"){
					$ini=$i+1;
					$btn1="<input type='hidden' class='num_rows_tb_details_export_uji_be' value='".$ini."' /><button id='ihapus_tb_details_harga_database' class='ui-button-text icon_hapus' style='width:75px' onclick='javascript:hapus_row_tb_details_export_uji_be(".$ini.")' type='button'>Hapus</button><input type='hidden' name='iexport_uji_be_file[]' value='".$k->iexport_req_refor_file."' />";
					$value=$k->tKeterangan;
					...
					if($value != '') {
						if (file_exists('./'.$this->filepathreq.'/'.$id.'/'.$value)) {
							...
							$btn2="<button id='ihapus_tb_details_harga_database' class='ui-button-text icon_hapus' style='width:100px' onclick='javascript:hapus_row_tb_details_export_uji_be(".$ini.")' type='button'>".$caption."</button>";
						}
						else {
							$btn2 = ' [No File!]';
						}
	...
	function getdetailsprevupload(){
    	$post=$this->input->post();
    	$get=$this->input->get();
		$sql_data="select * from reformulasi.export_uji_be_file p where p.ldeleted=0 and p.iexport_uji_be=".$post['id'];
		$q=$this->dbset->query($sql_data);
		$rsel=array('vfilename','tketerangan','update');
		$data = new StdClass;
		$data->records=$q->num_rows();
		$i=0;
		foreach ($q->result() as $k) {
			$data->rows[$i]['id']=$k->iexport_uji_be_file;
			$z=0;
			foreach ($rsel as $dsel => $vsel) {
				if($vsel=="update"){
					$ini=$i+1;
					$btn1="<input type='hidden' class='num_rows_tb_details_export_uji_be_file' value='".$ini."' />";
					if($get['isubmit']==0){
					$btn1=$btn1."<button id='ihapus_tb_details_".$this->gridname."' class='ui-button-text icon_hapus' style='width:75px' onclick='javascript:hapus_row_tb_details_".$this->gridname."_file(".$ini.")' type='button'>Hapus</button><input type='hidden' name='iexport_uji_be_file[]' value='".$k->iexport_uji_be_file."' />";
					}
					$value=$k->vfilename;
					$id=$k->iexport_uji_be;
					$caption='No File';
