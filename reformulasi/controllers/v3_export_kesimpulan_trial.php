...
class v3_export_kesimpulan_trial extends MX_Controller {
    function __construct() {
        ...
        $this->url = 'v3_export_kesimpulan_trial';
    ...
    function index($action = '') {
        ...
        $grid->addList('vno_formula','ikesimpulan_trial','itrial_ulang','ineed_sample_trial','isubmit_kesimpulan_trial','dsubmit_kesimpulan_trial'); 
        $grid->setSortBy('iexport_formulasi_detail');
        $grid->setSortOrder('DESC');  

        $grid->addFields('vno_formula','ikesimpulan_trial','itrial_ulang','ineed_sample_trial'); 
        ...
        $grid->setLabel('ikesimpulan_trial','Kesimpulan Skala Trial');

        $grid->setWidth('itrial_ulang', '100');
        $grid->setAlign('itrial_ulang', 'left');
        $grid->setLabel('itrial_ulang','Trial Ulang');

        $grid->setWidth('ineed_sample_trial', '100');
        $grid->setAlign('ineed_sample_trial', 'left');
        $grid->setLabel('ineed_sample_trial','Perlu Minta Sample');

        $grid->setWidth('isubmit_kesimpulan_trial', '100');
        $grid->setAlign('isubmit_kesimpulan_trial', 'left');
        $grid->setLabel('isubmit_kesimpulan_trial','Status');

        $grid->setWidth('dsubmit_kesimpulan_trial', '100');
        $grid->setAlign('dsubmit_kesimpulan_trial', 'left');
        $grid->setLabel('dsubmit_kesimpulan_trial','Tgl Submit');
    
        $grid->setQuery('lDeleted = 0 ', null); 
        $grid->setQuery('isubmit_fisik_trial = 1 ', null); 
        $grid->setQuery('isubmit_kimiawi_trial = 1 ', null); 
        ...
        $grid->changeFieldType('isubmit_kesimpulan_trial','combobox','',array('' => '--Pilih--', 0 => 'Need To Be Submit', 1 => 'Submitted'));
        ...
        $grid->changeFieldType('ineed_sample_trial','combobox','',array('' => '--Pilih--', 0 => 'No', 1 => 'Yes'));

        $grid->setSearch('vno_formula','isubmit_kesimpulan_trial');
        
        $grid->setRequired('vno_formula','ikesimpulan_trial','itrial_ulang','ineed_sample_trial'); 
    ...
    function listBox_Action($row, $actions) { 
        ...
        $sqlCek     = 'SELECT t.isubmit_kesimpulan_trial FROM reformulasi.export_refor_formulasi_detail f
        ...
        $submit     = (!empty($cekSubmit))?$cekSubmit['isubmit_kesimpulan_trial']:0;
        ...
        if ($submit == 1){
            ...
            if ($row['isubmit_kesimpulan_trial'] == 0){
                $actions['edit'] = $edit;
            }
        }
    ...
    function updateBox_v3_export_kesimpulan_trial_vno_formula($field, $id, $value, $rowData) {
    ...
    function updateBox_v3_export_kesimpulan_trial_ikesimpulan_trial($field, $id, $value, $rowData) {
        $idtrial = $this->url.'_itrial_ulang';
        $idsample = $this->url.'_ineed_sample_trial';
        ...
        $return .= '<script>
                        ...
                        function show_hide_trial(value){
                            if (value == 1){
                                ...
                                $("#'.$idtrial.'").val("'.$rowData['itrial_ulang'].'");
                            }
                        }
                    </script>';
    ...
    function updateBox_v3_export_kesimpulan_trial_itrial_ulang($field, $id, $value, $rowData) {
        $idsample = $this->url.'_ineed_sample_trial';
        ...
        $return .= '<script>
                        ...
                        function show_hide_sample(value){
                            if (value == 1){
                                ...
                                $("label[for=\''.$idsample.'\']").parent().hide();
                                $("#'.$idsample.'").val("'.$rowData['itrial_ulang'].'");
                            }
                        }
                    </script>';
    ...
    function before_update_processor($row, $postData) {
        ...
        if($postData['isdraft']==true){
            $postData['isubmit_kesimpulan_trial'] = 0;
        } else{
            $postData['isubmit_kesimpulan_trial'] = 1;
            $postData['dsubmit_kesimpulan_trial'] = date('Y-m-d H:i:s');
            $postData['csubmit_kesimpulan_trial'] = $this->user->gNIP;
        } 
    ...
    function manipulate_update_button($buttons, $rowData) { 
        ...
        if ($this->input->get('action') == 'view' || $rowData['isubmit_kesimpulan_trial'] == 1 || $submit == 1){
            unset($buttons['update_back']);
        }
