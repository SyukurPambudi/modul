...
class export_produksi_pilot extends MX_Controller {
    function __construct() {
		...
		$this->filepath='files/reformulasi/export/produksi_pilot';
		$this->filepathreq="files/reformulasi/export/export_reg_file";
		$this->urlpath="processor/reformulasi/export/produksi/pilot";
		$this->gridname="export_produksi_pilot";
    }
    function index($action = '') {
    	$action = $this->input->get('action');
    	//Bikin Object Baru Nama nya $grid		
		$grid = new Grid;
		$grid->setTitle('Reformulasi - Produksi Pilot');		
		$grid->setTable('reformulasi.export_produksi_pilot');		
		...
		$grid->addFields('iexport_refor_formula','vUpd_no','vNama_usulan','cInisiator_export','iDapartemen_export','tAlasan_export','vUploadfile','iTeamPD','dPermintaan_req_export','cApproval_ats_inisiator','cPicFormulator','lbl','dprodpil','dselesaiprodpil','vfile','istatus_prod','iperlu_sample','iapprove_pd');
		...
		$grid->setLabel('lbl', ' ');
		$grid->setLabel('dprodpil', 'Tanggal Produksi Pilot');
		$grid->setLabel('dselesaiprodpil', 'Tanggal Selesai Produksi Pilot');
		$grid->setLabel('vfile', 'File Upload');
		$grid->setLabel('istatus_prod', 'Status Approval');
		...
		$grid->setJoinTable('reformulasi.export_refor_formula', 'export_refor_formula.iexport_refor_formula=reformulasi.export_produksi_pilot.iexport_refor_formula', 'inner');
		...
		$grid->setRequired('iexport_refor_formula','dprodpil','dselesaiprodpil','vfile','istatus_prod','iperlu_sample');
		...
		switch ($action) {
			...
			case 'createproses':
   				...
   				if($isUpload) {
					...
					foreach($_POST as $key=>$value) {						
						if ($key == 'export_produksi_pilot_file_fileketerangan') {
					...
					$i=0;
					foreach ($_FILES['export_produksi_pilot_file_upload_file']["error"] as $key => $error) {
						if ($error == UPLOAD_ERR_OK) {
							$tmp_name = $_FILES['export_produksi_pilot_file_upload_file']["tmp_name"][$key];
							$name =$_FILES['export_produksi_pilot_file_upload_file']["name"][$key];
							$data['filename'] = $name;
							$data['dInsertDate'] = date('Y-m-d H:i:s');

								if(move_uploaded_file($tmp_name, $path."/".$lastId."/".$name)) {
									$sql[]="INSERT INTO reformulasi.export_produksi_pilot_file (iexport_produksi_pilot, vfilename, tketerangan, dcreate, ccreated) 
			...
			case 'updateproses':
				...
				$lastId=$post['export_produksi_pilot_iexport_produksi_pilot'];
				$path = realpath($this->filepath);
				if(!file_exists($path."/".$lastId)){
					if (!mkdir($path."/".$lastId, 0777, true)) { //id review
						die('Failed upload, try again!');
                ...
                foreach($_POST as $key=>$value) {     
                    ...
                    if ($key == 'iexport_produksi_pilot_file') {
                        $i=0;
                        foreach($value as $k=>$v) {
                            if($i==0){
                                $fileid .= "'".$v."'";
                ...
                if($isUpload) {
                	if($fileid!=''){
                        ...
                        $sql1="update reformulasi.export_produksi_pilot_file set ldeleted=1, dupdate='".$tgl."', cupdate='".$this->user->gNIP."' where iexport_produksi_pilot='".$lastId."' and iexport_produksi_pilot_file not in (".$fileid.")";
                        $this->dbset->query($sql1);
                    }else{
                        $tgl= date('Y-m-d H:i:s');
                        $sql1="update reformulasi.export_produksi_pilot_file set ldeleted=1, dupdate='".$tgl."', cupdate='".$this->user->gNIP."' where iexport_produksi_pilot='".$lastId."'";
                        $this->dbset->query($sql1);
                    }
                    $i=0;
                    if (isset($_FILES['export_produksi_pilot_file_upload_file']))  {
                        foreach ($_FILES['export_produksi_pilot_file_upload_file']["error"] as $key => $error) {
                            if ($error == UPLOAD_ERR_OK) {
                                $tmp_name = $_FILES['export_produksi_pilot_file_upload_file']["tmp_name"][$key];
                                $name =$_FILES['export_produksi_pilot_file_upload_file']["name"][$key];
	...
	function listBox_export_produksi_pilot_isubmit($v, $pk, $name, $rowData) {
		$re="-";
		if($v==0){
	...
	function listBox_export_produksi_pilot_iapprove_ad($v, $pk, $name, $rowData) {
		$sa=array(0=>'Waiting Approval',1=>'Rejected',2=>'Approved');
		$re=isset($sa[$v])?$sa[$v]:'-';
		return $re;
	}

	function listBox_export_produksi_pilot_iapprove_qa($v, $pk, $name, $rowData) {
		$sa=array(0=>'Waiting Approval',1=>'Rejected',2=>'Approved');
		$re=isset($sa[$v])?$sa[$v]:'-';
		return $re;
	}
	...
	function insertBox_export_produksi_pilot_iexport_refor_formula($f, $i){
		$r = '<script>
		...
		$r .= '&nbsp;<button class="icon_pop"  onclick="browse(\''.base_url().'processor/reformulasi/browse_export_produksi_pilot?field=export_produksi_pilot&modul_id='.$this->input->get('modul_id').'\',\'\')" type="button">&nbsp;</button>';
		return $r;
	}

	function insertBox_export_produksi_pilot_vUpd_no($f, $i){
		return '<input type="text" disabled="TRUE" name="'.$i.'" id="'.$i.'" class="input_rows1" size="20" />';
	}

	function insertBox_export_produksi_pilot_vNama_usulan($f, $i){
		return '<textarea disabled="TRUE" name="'.$i.'" id="'.$i.'"></textarea>';
	}

	function insertBox_export_produksi_pilot_cInisiator_export($f, $i){
		return '<textarea disabled="TRUE" name="'.$i.'" id="'.$i.'"></textarea>';
	}

	function insertBox_export_produksi_pilot_iDapartemen_export($f, $i){
		return '<input type="text" disabled="TRUE" name="'.$i.'" id="'.$i.'" class="input_rows1" size="20" />';
	}

	function insertBox_export_produksi_pilot_tAlasan_export($f, $i){
		return '<textarea disabled="TRUE" name="'.$i.'" id="'.$i.'"></textarea>';
	}

	function insertBox_export_produksi_pilot_vUploadfile($f, $i){
		$data['id']=0;
		$data['get']=$this->input->get();
		return $this->load->view('export/export_produksi_pilot_file',$data,TRUE);
	}

	function insertBox_export_produksi_pilot_iTeamPD($f, $i){
		return '<input type="text" disabled="TRUE" name="'.$i.'" id="'.$i.'" class="input_rows1" size="20" />';
	}

	function insertBox_export_produksi_pilot_dPermintaan_req_export($f, $i){
		return '<input type="text" disabled="TRUE" name="'.$i.'" id="'.$i.'" class="input_rows1" size="20" />';
	}

	function insertBox_export_produksi_pilot_cApproval_ats_inisiator($f, $i){
		return '<textarea disabled="TRUE" name="'.$i.'" id="'.$i.'"></textarea>';
	}

	function insertBox_export_produksi_pilot_cPicFormulator($f, $i){
		$r= '<textarea disabled="TRUE" name="'.$i.'" id="'.$i.'"></textarea>';
		return $r;
	}

	function insertBox_export_produksi_pilot_lbl($f, $i){
		$r = '<script>
	...
	function insertBox_export_produksi_pilot_dprodpil($f, $i){
	...
	function insertBox_export_produksi_pilot_dselesaiprodpil($f, $i){
		$r = '<input name="'.$i.'" id="'.$i.'" type="text" size="20" class="input_tgl datepicker required" style="width:130px"/>';
		$r .=	'<script>
						$("#'.$i.'").datepicker({dateFormat:"yy-mm-dd",changeMonth: true,changeYear: true});
					</script>';
	...
	function insertBox_export_produksi_pilot_vfile(){
		$data['id']=0;
		$data['get']=$this->input->get();
		return $this->load->view('export/export_produksi_pilot_file_upload',$data,TRUE);
	}

	function insertBox_export_produksi_pilot_istatus_prod($f, $i){
		$sa=array(0=>"TMS",1=>"MS");
	...
	function insertBox_export_produksi_pilot_iperlu_sample($f, $i){
		...
		$ret.='var sa = $("#export_produksi_pilot_istatus_prod").val();';
		$ret.='if(sa != "0"){ $("#'.$i.'").parent().parent().hide() }';
		$ret.='$("#export_produksi_pilot_istatus_prod").change(function(){ if($(this).val()=="0"){ $("#'.$i.'").parent().parent().show() }else{ $("#'.$i.'").parent().parent().hide() } });';
		$ret.='</script>';
		return $ret;
	}
	function insertBox_export_produksi_pilot_iapprove_pd($f, $i){
		return "-";
	}

	function manipulate_insert_button($buttons) {
		unset($buttons['save']);
		$grid='export_produksi_pilot';
		$data['grid']=$grid;
		$js = $this->load->view('export/js/export_produksi_pilot_js',$data);
	...
	function after_insert_processor($r, $insertid, $postData){
		...
		if($postData['isubmit']==1){
			...
			$data['menuapp']="ERP -> Reformulasi -> Export -> Produksi Pilot";
			$data['caption']="Diberitahukan telah ada Submit pada Aplikasi Reformulasi Export module Produksi Pilot, dengan rincian sebagai berikut :";
			$subject = "Reformulasi Export - Produksi Pilot (".$dform['export_req_refor__vno_export_req_refor'].")";
	...
	function updateBox_export_produksi_pilot_iexport_refor_formula($f, $i, $v, $row){
		$val=$this->getDetailsForm($row['iexport_produksi_pilot'],'export_req_refor__vno_export_req_refor');
		$r = '<script>
					$( "button.icon_pop" ).button({
						icons: {
							primary: "ui-icon-newwin"
	...
	function updateBox_export_produksi_pilot_vUpd_no($f, $i, $v, $row){
	...
	function updateBox_export_produksi_pilot_lbl($f, $i){
		$r = '<script>
			$("label[for=\''.$i.'\']").css({"border": "1px solid #dddddd", "background": "#548cb6", "border-collapse": "collapse","width":"99%","font-weight":"bold","color":"#ffffff","text-shadow": "0 1px 1px rgba(0, 0, 0, 0.3)","text-transform": "uppercase"});
		</script>';
		return $r;
	...
	function updateBox_export_produksi_pilot_dprodpil($f, $i, $v, $row){
		$dis="";
		if($this->input->get('action')=='view' or $row['isubmit']>0){
			$dis='disabled="TRUE"';
		}
	...
	function updateBox_export_produksi_pilot_dselesaiprodpil($f, $i, $v, $row){
		$dis="";
		if($this->input->get('action')=='view' or $row['isubmit']>0){
			$dis='disabled="TRUE"';
		}
	...
	function updateBox_export_produksi_pilot_vfile($f, $i, $v, $row){
		$data['id']=$row['iexport_produksi_pilot'];
		$data['get']=$this->input->get();
		$data['isubmit']=$row['isubmit'];
		return $this->load->view('export/export_produksi_pilot_file_upload',$data,TRUE);
	}

	function updateBox_export_produksi_pilot_istatus_prod($f, $i, $v, $row){
		$dis="";
		$ret="";
		$sa=array(0=>"TMS",1=>"MS");
		if($this->input->get('action')=='view' or $row['isubmit']>0){
	...
	function updateBox_export_produksi_pilot_iperlu_sample($f, $i, $v, $row){
		$dis="";
		$ret="";
		$sa=array(0=>"Tidak",1=>"Ya");
		if($this->input->get('action')=='view' or $row['isubmit']>0){
	...
	function updateBox_export_produksi_pilot_iapprove_pd($f, $i, $v, $row){
		$ret='';
		if($v==0){
	...
	function manipulate_update_button($btn, $row){
		...
		$approvead = '<button onclick="javascript:load_popup_'.$grid.'(\''.base_url().'processor/reformulasi/'.$grid.'?action=approve&iexport_produksi_pilot='.$row['iexport_produksi_pilot'].'&group_id='.$this->input->get('group_id').'&modul_id='.$this->input->get('modul_id').'&company_id='.$this->input->get('company_id').'\',\'\',\'Approve Draft Produksi Pilot\')" class="ui-button-text icon-save" id="button_approve_'.$grid.'">Approve</button>';
		$btn2='';
		if($this->auth_export->is_manager()){
			$x=$this->auth_export->dept();
			$manager=$x['manager'];
	...
	function approve_view() {
		...
		$echo .= '<form id="form_approve_export_produksi_pilot" action="'.base_url().$this->urlpath.'?action=approve_process" method="post">';
		$echo .= '<div style="vertical-align: top;">';
		$echo .= 'Remark : 
				<input type="hidden" name="approveBox_export_produksi_pilot_iexport_produksi_pilot" value="'.$this->input->get('iexport_produksi_pilot').'" />
				<input type="hidden" name="approveBox_export_produksi_pilot_group_id" value="'.$this->input->get('group_id').'" />
				<input type="hidden" name="approveBox_export_produksi_pilot_modul_id" value="'.$this->input->get('modul_id').'" />
				<input type="hidden" name="approveBox_export_produksi_pilot_company_id" value="'.$this->input->get('company_id').'" />
				<input type="hidden" name="approveBox_export_produksi_pilot_foreign_id" value="'.$this->input->get('foreign_id').'" />
				<input type="hidden" name="approveBox_export_produksi_pilot_iapprove_pd" value="2" />
				<textarea id="approveBox_export_produksi_pilot_vRemark" name="approveBox_export_produksi_pilot_vRemark"></textarea>';
		$echo .= '</div>';
		$echo .= '</form>';
	...
	function approve_process(){
		...
		$dataup=array();
		$dataup['iapprove_pd']=$post['approveBox_export_produksi_pilot_iapprove_pd'];
		$dataup['tapprove_pd']=$post['approveBox_export_produksi_pilot_vRemark'];
		...
		$this->dbset->where('iexport_produksi_pilot',$post['approveBox_export_produksi_pilot_iexport_produksi_pilot']);
		if($this->dbset->update('reformulasi.export_produksi_pilot',$dataup)){

			/*Cek Untuk Permintaan Sample*/

			$sqlpro="select * from reformulasi.export_produksi_pilot r where r.ldeleted=0 and r.iexport_produksi_pilot=".$post['approveBox_export_produksi_pilot_iexport_produksi_pilot'];
			$dr=$this->dbset->query($sqlpro)->row_array();
			$datasample=array();
			if(count($dr)>=1){
				if($dr['istatus_prod']==0 && $dr['iperlu_sample']==1){ // Insert Ke Sample Bahan Baku
                    ...
                    $insD['iTujuan_request'] = 5; // Tujuan ke Produksi Pilot
				...
				if($dr['istatus_prod']==0 && $dr['iperlu_sample']==0){// Kembali Ke Trial
			...
			$data['group_id']=$post['approveBox_export_produksi_pilot_group_id'];
			$data['modul_id']=$post['approveBox_export_produksi_pilot_modul_id'];
			$data['foreign_id']=$post['approveBox_export_produksi_pilot_foreign_id'];
			$data['company_id']=$post['approveBox_export_produksi_pilot_company_id'];
			$data['last_id']=$post['approveBox_export_produksi_pilot_iexport_produksi_pilot'];

			/*Send Notif*/
			$id=$post['approveBox_export_produksi_pilot_iexport_produksi_pilot'];
			...
			$to='';
			if(count($dr)>=1){
				if($dr['istatus_prod']==0 && $dr['iperlu_sample']==1){ // Kirim Email Ke Sample
				...
				if($dr['istatus_prod']==0 && $dr['iperlu_sample']==0){ // Kirim Ke Trial
					$data['menuapp']="ERP -> Reformulasi -> Export -> Formula Skala Trial";
					$data['caption']="Diberitahukan telah ada Approval Pada Produksi Pilot untuk dilakukan penginputan pada module Formula Skala Trial, dengan rincian sebagai berikut :";
				...
				if($dr['istatus_prod']==1){ // Kirim Untuk Hasil 
					$data['menuapp']="ERP -> Reformulasi -> Export -> Produksi Pilot";
					$data['caption']="Diberitahukan telah ada Approval Pada Produksi Pilot untuk dilakukan penginputan pada module Formula Skala Trial, dengan rincian sebagai berikut :";
					$subject = "Reformulasi Export - Produksi Pilot (".$dform['export_req_refor__vno_export_req_refor'].")";
					$data['cdata'] = array(
    ...
    function getDetailsForm($id=0,$field='null'){
    	$sql="SELECT export_req_refor.vno_export_req_refor AS export_req_refor__vno_export_req_refor, dossier_upd.vNama_usulan AS dossier_upd__vNama_usulan, export_req_refor.iTeamPD as export_req_refor__iTeamPD, reformulasi.export_refor_formula.*, export_produksi_pilot.iexport_produksi_pilot
			...
			INNER JOIN reformulasi.export_produksi_pilot ON export_produksi_pilot.iexport_refor_formula=reformulasi.export_refor_formula.iexport_refor_formula
			WHERE export_produksi_pilot.iexport_produksi_pilot=".$id;
    ...
    function getdetailsprev(){
		...
		foreach ($q->result() as $k) {
			...
			foreach ($rsel as $dsel => $vsel) {
				if($vsel=="update"){
					$ini=$i+1;
					$btn1="<input type='hidden' class='num_rows_tb_details_export_produksi_pilot' value='".$ini."' /><button id='ihapus_tb_details_harga_database' class='ui-button-text icon_hapus' style='width:75px' onclick='javascript:hapus_row_tb_details_export_produksi_pilot(".$ini.")' type='button'>Hapus</button><input type='hidden' name='iexport_produksi_pilot_file[]' value='".$k->iexport_req_refor_file."' />";
					$value=$k->tKeterangan;
					...
					if($value != '') {
						if (file_exists('./'.$this->filepathreq.'/'.$id.'/'.$value)) {
							...
							$btn2="<button id='ihapus_tb_details_harga_database' class='ui-button-text icon_hapus' style='width:100px' onclick='javascript:hapus_row_tb_details_export_produksi_pilot(".$ini.")' type='button'>".$caption."</button>";
						}
						else {
							$btn2 = ' [No File!]';
						}
	...
	function getdetailsprevupload(){
    	$post=$this->input->post();
    	$get=$this->input->get();
		$sql_data="select * from reformulasi.export_produksi_pilot_file p where p.ldeleted=0 and p.iexport_produksi_pilot=".$post['id'];
		$q=$this->dbset->query($sql_data);
		$rsel=array('vfilename','tketerangan','update');
		$data = new StdClass;
		$data->records=$q->num_rows();
		$i=0;
		foreach ($q->result() as $k) {
			$data->rows[$i]['id']=$k->iexport_produksi_pilot_file;
			$z=0;
			foreach ($rsel as $dsel => $vsel) {
				if($vsel=="update"){
					$ini=$i+1;
					$btn1="<input type='hidden' class='num_rows_tb_details_export_produksi_pilot_file' value='".$ini."' />";
					if($get['isubmit']==0){
					$btn1=$btn1."<button id='ihapus_tb_details_".$this->gridname."' class='ui-button-text icon_hapus' style='width:75px' onclick='javascript:hapus_row_tb_details_".$this->gridname."_file(".$ini.")' type='button'>Hapus</button><input type='hidden' name='iexport_produksi_pilot_file[]' value='".$k->iexport_produksi_pilot_file."' />";
					}
					$value=$k->vfilename;
					$id=$k->iexport_produksi_pilot;
					$caption='No File';
					$btn2="";
					if($value != '') {
	 ...
	 function getreqdata($id=0){
    	$sql="SELECT export_req_refor.iTeamPD,export_req_refor.iTeamAndev,export_req_refor.vno_export_req_refor AS export_req_refor__vno_export_req_refor, dossier_upd.vNama_usulan AS dossier_upd__vNama_usulan,dossier_upd.vUpd_no AS dossier_upd__vUpd_no, reformulasi.export_refor_formula.*, export_produksi_pilot.iexport_produksi_pilot
			FROM (reformulasi.export_refor_formula)
			INNER JOIN reformulasi.export_req_refor ON export_req_refor.iexport_req_refor=reformulasi.export_refor_formula.iexport_req_refor
			INNER JOIN dossier.dossier_upd ON dossier_upd.idossier_upd_id=reformulasi.export_req_refor.idossier_upd_id
			INNER JOIN reformulasi.export_produksi_pilot ON export_produksi_pilot.iexport_refor_formula=reformulasi.export_refor_formula.iexport_refor_formula
