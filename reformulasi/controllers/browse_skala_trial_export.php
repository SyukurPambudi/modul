...
class browse_skala_trial_export extends MX_Controller {
    ...
    function index($action = '') {
		...
		$grid->setUrl('browse_skala_trial_export');
        ...
        $grid->setQuery('export_req_refor.iexport_req_refor NOT IN (SELECT  
                    e.`iexport_req_refor`
                FROM
                  reformulasi.`export_refor_formula` e 
                WHERE e.`iflag_open_app` = 1 
                  AND IF(
                    e.iflag_open = 1,
                    e.iflag_open = 1,
                    IF(
                      e.`isSample_for_trial` = 1,
                      IF(
                        (SELECT 
                          (SELECT DISTINCT 
                            (
                          ...
                          WHERE rber.`lDeleted` = 0 
                            AND roer.`lDeleted` = 0 
                            AND rder.`lDeleted` = 0 
                            AND rber.iHasil_analisa_bb = 2 
                            AND rber.iApprove_analisa_bb = 2 
                            AND rder.iexport_request_sample = rser.iexport_request_sample) 
                        FROM
                          reformulasi.`export_request_sample` rser 
                        WHERE rser.`iTujuan_request` IN (2, 4, 6, 7) AND `rser`.`iexport_req_refor` =e.`iexport_req_refor`
                        ORDER BY `iexport_request_sample` DESC 
                        LIMIT 1) > 0,
                        e.`lDeleted` = 2,
                        e.`lDeleted` = 0
                      ),
        ...
        $grid->setQuery('IF(
            (SELECT (SELECT DISTINCT(COUNT(rbe.`iexport_ro_detail_batch`)) FROM  reformulasi.`export_request_sample_detail` rde 
            ...
            FROM reformulasi.`export_request_sample` rse WHERE rse.`iTujuan_request` =1 AND rse.`iexport_req_refor` = export_req_refor.`iexport_req_refor`
            ORDER BY `iexport_request_sample` DESC LIMIT 1)>0,
            `export_req_refor`.`lDeleted` = 0,
            `export_req_refor`.`lDeleted` = 3 
            )', null); 
	...
	function listBox_browse_skala_trial_export_pilih($value, $pk, $name, $rowData) {   
        $url_ = base_url().'processor/reformulasi/browse/skala/trial/export?action=dt_upload';

        $itname = "SELECT i.`c_itnam` FROM sales.`itemas` i JOIN `dossier`.`dossier_upd` d ON d.`iupb_id` = i.`c_iteno`
		...
		$o = '<input type="radio" name="pilih" onClick="javascript:pilih_upb_detail('.$pk.', \''.$rowData->vno_export_req_refor.'\' ,
            ...
            \''.$rowData->dPermintaan_req_export.'\',\''.$app['vName'].'\',\''.$rowData->cPicFormulator.'\',\''.$formu['vName'].'\',\''.$c_pd['vteam'].'\',\''.$c_itnam['c_itnam'].'\') ;" />
