...
class v2_exp_skala_trial_kesimpulan extends MX_Controller {
    function __construct() {
		...
		$this->title 		= 'Skala Trial - Kesimpulan';
		$this->url 			= 'v2_exp_skala_trial_kesimpulan';
		$this->urlpath 		= 'reformulasi/'.str_replace("_","/", $this->url);

		$this->maintable 	= 'reformulasi.export_refor_formulasi';	
		$this->main_table 	= $this->maintable;	
		$this->main_table_pk= 'iexport_formulasi';	
		...
		$datagrid['islist'] = array(
            ...
            'isubmit_kesimpulan_trial'              => array('label' => 'Submit Kesimpulan',    'width' => 150, 'align' => 'center', 'search' => true),
            'iapppd_kesimpulan_trial'               => array('label' => 'Approval Kesimpulan',  'width' => 150, 'align' => 'center', 'search' => false),
		);		

		$datagrid['setQuery'] = array(
			0 => array('vall' => 'export_refor_formulasi.ldeleted',              'nilai' => 0),
            0 => array('vall' => 'export_req_refor.lDeleted',                    'nilai' => 0),
            0 => array('vall' => 'export_refor_formulasi.iappad_kimiawi_trial',  'nilai' => 2),
		);

		$datagrid['jointableinner'] = array(
			0 => array('reformulasi.export_req_refor' 	=> 'export_refor_formulasi.iexport_req_refor = export_req_refor.iexport_req_refor'),
		);

		$datagrid['shortBy'] = array( "export_refor_formulasi.iexport_formulasi" => "DESC");
    ...
    function index($action = '') {
		...
		$grid->changeFieldType('isubmit_kesimpulan_trial', 'combobox','',array(''=>'--select--',0=>'Need to be Submit', 1=>'Submited'));
		$grid->changeFieldType('iapppd_kesimpulan_trial', 'combobox','',array(''=>'--select--',0=>'Waiting for approval',1=>'Rejected', 2=>'Approved'));
    ...
    function getFormDetail(){
        ...
        if(!empty($dFields)){
            ...
            foreach ($dFields as $form_field) {
                ...
                $vNama_field                    = $form_field['vNama_field'];
        ...
        function listBox_v2_exp_skala_trial_kesimpulan_iapppr($value) {
        ...
        function listBox_v2_exp_skala_trial_kesimpulan_cRequestor($value) {
        ...
        function insertBox_v2_exp_skala_trial_kesimpulan_form_detail($field,$id){
            $get=$this->input->get();
            $post=$this->input->post();
        ...
        function updateBox_v2_exp_skala_trial_kesimpulan_form_detail($field,$id,$value,$rowData){
            $get=$this->input->get();
            $post=$this->input->post();
            foreach ($get as $kget => $vget) {
                if($kget!="action"){
            ...
            function approve_view() { 
                             ...
                             function submit_ajax(form_id) {
                                return $.ajax({
                                    ...
                                    success: function(data) {
                                        ...
                                        var url = "'.base_url().'processor/reformulasi/v2_exp_skala_trial_kesimpulan";                             
                                        if(o.status == true) { 
                                            $("#alert_dialog_form").dialog("close");
                                                 $.get(url+"?action=update&id="+last_id+"&foreign_key=0&company_id=3&group_id="+group_id+"&modul_id="+modul_id, function(data) {
                                                 $("div#form_v2_exp_skala_trial_kesimpulan").html(data);
                                                 
                                            });
                                            
                                        }
                                            reload_grid("grid_v2_exp_skala_trial_kesimpulan");
                                    }
                ...
                $echo .= '<form id="form_v2_exp_skala_trial_kesimpulan_approve" action="'.base_url().'processor/reformulasi/v2_exp_skala_trial_kesimpulan?action=approve_process" method="post">';
                ...
                <button type="button" onclick="submit_ajax(\'form_v2_exp_skala_trial_kesimpulan_approve\')">Approve</button>';
            ...
            function approve_process() {
                ...
                $dataMain       = $this->db->get_where($this->maintable, array($this->main_table_pk => $pk))->row_array();
                $iskala_trial   = $dataMain['iskala_trial'];
                $ineed_sample   = $dataMain['ineed_sample_trial'];

                //jika butuh skala trial tapi tidak membutuhkan sample
                if ($iskala_trial == 1 && $ineed_sample == 0){

                    //mendapatkan semua field yang ada di activity fisik, kimiawi, dan kesimpulan
                    $sqlGetField = 'SELECT a.* FROM erp_privi.m_modul_activity a 
                                    JOIN erp_privi.m_modul m ON a.iM_modul = m.iM_modul
                                    WHERE a.lDeleted = 0 AND m.idprivi_modules IN ('.$activity['vDesciption'].')';
                    $getFields = $this->db->query($sqlGetField)->result_array();

                    $updateReset = array();
                    foreach ($getFields as $gf) {
                    ...
                    //reset formulasi supaya bisa diproses di uji fisik
                    $resetFomula['isubmit_fisik_trial']       = 0;
                    $resetFomula['dsubmit_fisik_trial']       = null;
                    $resetFomula['csubmit_fisik_trial']       = null;
                    $resetFomula['isubmit_kimiawi_trial']     = 0;
                    $resetFomula['dsubmit_kimiawi_trial']     = null;
                    $resetFomula['csubmit_kimiawi_trial']     = null;
                    $resetFomula['isubmit_kesimpulan_trial']  = 0;
                    $resetFomula['dsubmit_kesimpulan_trial']  = null;
                    $resetFomula['csubmit_kesimpulan_trial']  = null;
                    $this->db->where('iexport_formulasi', $pk);
                    $this->db->update('reformulasi.export_refor_formulasi_detail', $resetFomula);

                    //delete log fisik, kimiawi, kesimpulan
                    $sqlDeleteLog = $activity['vSql_get_passed'];
                    $this->db->query($sqlDeleteLog, array($cNip, $nowDate, $pk));
                }
            ...
            function confirm_view() { 
                ...
                $echo .= '<form id="form_v2_exp_skala_trial_kesimpulan_confirm" action="'.base_url().'processor/reformulasi/v2_exp_skala_trial_kesimpulan?action=confirm_process" method="post">';
                $echo .= '<div style="vertical-align: top;">';
                $echo .= 'Remark : 
                        <input type="hidden" name="'.$this->main_table_pk.'" value="'.$this->input->get($this->main_table_pk).'" />
                        <input type="hidden" name="modul_id" value="'.$this->input->get('modul_id').'" />
                        ...
                        <textarea name="vRemark"></textarea>
                <button type="button" onclick="submit_ajax(\'form_v2_exp_skala_trial_kesimpulan_confirm\')">Confirm</button>';
            ...
            function reject_view() {
                $echo = '<script type="text/javascript">
                             function submit_ajax(form_id) {
                                var remark = $("#v2_exp_skala_trial_kesimpulan_remark").val();
                                if (remark=="") {
                ...
                $echo .= '<form id="form_v2_exp_skala_trial_kesimpulan_reject" action="'.base_url().'processor/reformulasi/v2_exp_skala_trial_kesimpulan?action=reject_process" method="post">';
                $echo .= '<div style="vertical-align: top;">';
                $echo .= 'Remark : 
                        <input type="hidden" name="'.$this->main_table_pk.'" value="'.$this->input->get($this->main_table_pk).'" />
                        <input type="hidden" name="modul_id" value="'.$this->input->get('modul_id').'" />
                        ...
                        <textarea name="vRemark" id="reject_v2_exp_skala_trial_kesimpulan_remark"></textarea>
                <button type="button" onclick="submit_ajax(\'form_v2_exp_skala_trial_kesimpulan_reject\')">Reject</button>';
    ...
    function reject_process() {
        ...
        //update ke main table
        $update = $this->lib_refor->getFieldUpdate($this->maintable, $id_activity, 1, $vRemark);
        $this->db->where($this->main_table_pk, $pk);
        $this->db->update($this->maintable, $update);
    ...
    function manipulate_update_button($buttons, $rowData) { 
        ...
        $iframe 		= '<iframe name="v2_exp_skala_trial_kesimpulan_frame" id="v2_exp_skala_trial_kesimpulan_frame" height="0" width="0"></iframe>';

        $dataMain       = $this->db->get_where($this->maintable, array($this->main_table_pk => $peka))->row_array();
        $canSubmit      = ( $dataMain['isubmit_fisik_trial'] == 1 && $dataMain['isubmit_kimiawi_trial'] && $dataMain['iappad_kimiawi_trial'] == 2 ) ? true : false;
        ...
        if ($this->input->get('action') == 'view') {
            unset($buttons['update']);
                ...
                if($isOpenEditing){
                    $update_draft = '<button onclick="javascript:update_draft_btn(\'v2_exp_skala_trial_kesimpulan\', \' '.base_url().'processor/reformulasi/v2_exp_skala_trial_kesimpulan?draft=true \',this,true )"  id="button_update_draft_v2_exp_skala_trial_kesimpulan"  class="ui-button-text icon-save" >Update open Editing</button>';
                    ...
                    foreach ($activities as $act) {
                        $update_draft = '<button onclick="javascript:update_draft_btn(\'v2_exp_skala_trial_kesimpulan\', \' '.base_url().'processor/reformulasi/v2_exp_skala_trial_kesimpulan?draft=true&modul_id='.$this->input->get('modul_id').'&iM_modul_activity='.$act['iM_modul_activity'].' \',this,true )"  id="button_update_draft_v2_exp_skala_trial_kesimpulan"  class="ui-button-text icon-save" >Update as Draft</button>';
                        $update = '<button onclick="javascript:update_btn_back(\'v2_exp_skala_trial_kesimpulan\', \' '.base_url().'processor/reformulasi/v2_exp_skala_trial_kesimpulan?company_id='.$this->input->get('company_id').'&modul_id='.$this->input->get('modul_id').'&iM_modul_activity='.$act['iM_modul_activity'].'&group_id='.$this->input->get('group_id').'modul_id='.$this->input->get('modul_id').' \',this,false )"  id="button_update_submit_v2_exp_skala_trial_kesimpulan"  class="ui-button-text icon-save" >Update &amp; Submit</button>';

                        $approve = '<button onclick="javascript:load_popup(\' '.base_url().'processor/reformulasi/v2_exp_skala_trial_kesimpulan?action=approve&iM_modul_activity='.$act['iM_modul_activity'].'&iM_activity='.$act['iM_activity'].'&'.$this->main_table_pk.'='.$peka.'&iupb_id='.$iupb_id.'&company_id='.$this->input->get('company_id').'&group_id='.$this->input->get('group_id').'&modul_id='.$this->input->get('modul_id').' \')"  id="button_approve_v2_exp_skala_trial_kesimpulan"  class="ui-button-text icon-save" >Approve</button>';
                        $reject = '<button onclick="javascript:load_popup(\' '.base_url().'processor/reformulasi/v2_exp_skala_trial_kesimpulan?action=reject&iM_modul_activity='.$act['iM_modul_activity'].'&iM_activity='.$act['iM_activity'].'&'.$this->main_table_pk.'='.$peka.'&iupb_id='.$iupb_id.'&company_id='.$this->input->get('company_id').'&group_id='.$this->input->get('group_id').'&modul_id='.$this->input->get('modul_id').' \' )"  id="button_reject_v2_exp_skala_trial_kesimpulan"  class="ui-button-text icon-save" >Reject</button>';

                        $confirm = '<button onclick="javascript:load_popup(\' '.base_url().'processor/reformulasi/v2_exp_skala_trial_kesimpulan?action=confirm&iM_modul_activity='.$act['iM_modul_activity'].'&iM_activity='.$act['iM_activity'].'&'.$this->main_table_pk.'='.$peka.'&iupb_id='.$iupb_id.'&company_id='.$this->input->get('company_id').'&group_id='.$this->input->get('group_id').'&modul_id='.$this->input->get('modul_id').' \')"  id="button_approve_v2_exp_skala_trial_kesimpulan"  class="ui-button-text icon-save" >Confirm</button>';
    ...
    private function canSubmitKimiawi ($id){
        ...
        $trial_submit   = $this->db->get_where('reformulasi.export_refor_formulasi', array('iexport_formulasi' => $id))->row_array();
        if (!empty($trial_submit)){
            $submit_fisik_trial = $trial_submit['isubmit_fisik_trial'];
            if (intval($submit_fisik_trial) == 0){
                $status     = false;
                $message    = 'Skala Trial - Uji Fisik Masih Belum Di Submit';
            } else {
                $filter_submit  = array(
                    'iexport_formulasi'         => $id,
                    'isubmit_fisik_trial'       => 1,
                    'isubmit_kimiawi_trial'     => 1,
                    'isubmit_kesimpulan_trial'  => 0,
                    'ldeleted'                  => 0
                );
                $formula_submit = $this->db->get_where('reformulasi.export_refor_formulasi_detail', $filter_submit)->num_rows();
                if ($formula_submit > 0){
                    $status     = false;
                    $message    = 'Masih Ada '.$formula_submit.' Yang Belum Di Submit Di Kesimpulan Trial';
                } else {
                    $status     = true;
                    $message    = 'Berhasil';
                }
