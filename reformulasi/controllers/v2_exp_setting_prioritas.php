...
class v2_exp_setting_prioritas extends MX_Controller {
    function __construct() {
		...
		$this->title 		= 'Setting Prioritas';
		$this->url 			= 'v2_exp_setting_prioritas';
		$this->urlpath 		= 'reformulasi/'.str_replace("_","/", $this->url);

		$this->maintable 	= 'reformulasi.exp_setting_prioritas';	
		$this->main_table 	= $this->maintable;	
		$this->main_table_pk= 'iExp_setting_prioritas';	
		
		$datagrid['islist'] = array(
            'iyear'                     => array('label' => 'Tahun',	'width' => 100,	'align' => 'center', 'search' => true),
            'imonth' 	                => array('label' => 'Semester',	'width' => 100,	'align' => 'center', 'search' => true),
			'reformulasi_team.vteam' 	=> array('label' => 'Team PD',	'width' => 350,	'align' => 'left',   'search' => false),
            'iSubmit' 	                => array('label' => 'Status Submit',	'width' => 150,	'align' => 'center', 'search' => false),
            'iApproval' 	            => array('label' => 'Status Approval',	'width' => 150,	'align' => 'center', 'search' => false),
            
		);		

		$datagrid['setQuery']=array(
           /*  0 => array('vall' => 'exp_setting_prioritas.lDeleted', 'nilai' => 0),
            1 => array('vall' => 'exp_setting_prioritas.iApproval_ats_inisiator', 'nilai' => 2), */
			
		);

		$datagrid['jointableinner'] = array(
            0 => array('reformulasi.export_req_refor' => 'export_req_refor.iTeamPD = exp_setting_prioritas.iTeamPD'),
            1 => array('reformulasi.reformulasi_team' => 'reformulasi_team.ireformulasi_team = exp_setting_prioritas.iTeamPD'),
		);

		$datagrid['shortBy']=array("exp_setting_prioritas.iExp_setting_prioritas" => "DESC");
    ...
    function index($action = '') {
        ...
        $grid->changeFieldType('iApproval', 'combobox','',array(''=>'--select--',0=>'Waiting for approval',1=>'Rejected', 2=>'Approved'));
        ...
        /* if($this->auth_export->is_manager()){
            ...
            if(in_array('PD', $manager)){
                $grid->setQuery('exp_setting_prioritas.iTeamPD IN ('.$this->auth_export->my_teams().')', null);
            }
            else{$type='';}
        }
        else{
    ...
    function get_data_prev(){
        ...
        function listBox_v2_exp_setting_prioritas_imonth ($value) {
            return 'Semester '.$value;
        }

        function listBox_v2_exp_setting_prioritas_iapppr($value) {
        ...
        function listBox_Action($row, $actions) {
            ...
            $peka = $row->iExp_setting_prioritas;
        ...
        function insertBox_v2_exp_setting_prioritas_form_detail($field,$id){
            $get=$this->input->get();
            $post=$this->input->post();
        ...
        function updateBox_v2_exp_setting_prioritas_form_detail($field,$id,$value,$rowData){
            $get=$this->input->get();
            $post=$this->input->post();
            foreach ($get as $kget => $vget) {
                if($kget!="action"){
            ...
            function confirm_view() { 
                             ...
                             function submit_ajax(form_id) {
                                return $.ajax({
                                    ...
                                    success: function(data) {
                                        ...
                                        var url = "'.base_url().'processor/reformulasi/v2_exp_setting_prioritas";                             
                                        if(o.status == true) { 
                                            $("#alert_dialog_form").dialog("close");
                                                 $.get(url+"?action=update&id="+last_id+"&foreign_key=0&company_id=3&group_id="+group_id+"&modul_id="+modul_id, function(data) {
                                                 $("div#form_v2_exp_setting_prioritas").html(data);
                                                 
                                            });
                                            
                                        }
                                            reload_grid("grid_v2_exp_setting_prioritas");
                                    }
                ...
                $echo .= '<form id="form_v2_exp_setting_prioritas_approve" action="'.base_url().'processor/reformulasi/v2_exp_setting_prioritas?action=confirm_process" method="post">';
                ...
                <button type="button" onclick="submit_ajax(\'form_v2_exp_setting_prioritas_approve\')">Confirm</button>';
            ...
            function confirm_process() {
                ...
                /* $sqlHead = "SELECT s.`vRequest_no`, s.iexport_request_sample ,b.*
                    FROM reformulasi.`export_request_sample` s
                    JOIN reformulasi.exp_setting_prioritas b ON b.iExp_setting_prioritas=s.iExp_setting_prioritas
                    WHERE s.`lDeleted` = 0 
                    AND s.`iTujuan_request`=1 
                    AND s.`iExp_setting_prioritas` = '".$pk."' 
                    LIMIT 1";
                ...
                if(!empty($head['vRequest_no'])){
                    ...
                    $sqlU = "UPDATE reformulasi.`export_request_sample` SET
                        cRequestor = '".$head['cPicFormulator']."' ,
                        iSubmit = 1,
                        iApprove_pd = 2,
                    ...
                    $subject2="Konfirmasi Request Refor: No Request ".$head['vno_exp_setting_prioritas'];
                    $content1="
                    ...
                    Diberitahukan bahwa telah ada ada Konfirmasi dari Proses Request Reformulasi Export,
                        ...
                        $sqlDta ="SELECT s.`vRequest_no`,rw.`vnama`,sd.`raw_id`,sd.`vSatuan`,sd.`vSpesifikasi`,sd.`iJumlah` FROM reformulasi.`exp_setting_prioritas` r
                            JOIN reformulasi.`export_request_sample` s ON r.`iExp_setting_prioritas` = s.`iExp_setting_prioritas`
                            ...
                            WHERE s.`lDeleted` = 0 AND s.`iTujuan_request`=1 AND sd.`lDeleted` = 0  AND r.`iExp_setting_prioritas` = '".$pk."'";
            ...
            function approve_view() { 
                ...
                $echo .= '<form id="form_v2_exp_setting_prioritas_approve" action="'.base_url().'processor/reformulasi/v2_exp_setting_prioritas?action=approve_process" method="post">';
                $echo .= '<div style="vertical-align: top;">';
                $echo .= 'Remark : 
                        <input type="hidden" name="'.$this->main_table_pk.'" value="'.$this->input->get($this->main_table_pk).'" />
                        <input type="hidden" name="modul_id" value="'.$this->input->get('modul_id').'" />
                        ...
                        <textarea name="vRemark"></textarea>
                <button type="button" onclick="submit_ajax(\'form_v2_exp_setting_prioritas_approve\')">Approve</button>';
            ...
            function approve_process() {
                ...
                $this->lib_sub_core->InsertActivityModule($this->ViewUPB($pk),$modul_id,$pk,$activity['iM_activity'],$activity['iSort'],$vRemark,2);
                        ...
                        $sql = "SELECT e.`vName`, e.`cUpper`, r.`vno_exp_setting_prioritas`, d.`vUpd_no`, 
                            d.`vNama_usulan`, r.`iTeamPD`,  rt.`vnip` 
                            FROM reformulasi.`exp_setting_prioritas` r
                            ...
                            WHERE r.`lDeleted` = 0 AND d.`lDeleted` = 0 AND r.`iExp_setting_prioritas` = ".$post['iExp_setting_prioritas']; 
                        $mailDet = $this->db->query($sql)->row_array();  
                        
                        $to = $mailDet['vnip'];
                        $cc = $this->user->gNIP.','.$mailDet['cInisiator_export'];
                        $subject="Approved Request Reformulasi: No Request ".$mailDet['vno_exp_setting_prioritas']; 
                        $content="
                        Diberitahukan bahwa telah ada Approved Request Reformulasi Export, 
                            ...
                            <table border='0' style='width: 600px;'>
                                <tr>
                                        ...
                                        <td>".$mailDet['vno_exp_setting_prioritas']."</td>
                                </tr>
                                <tr>
            ...
            function reject_view() {
                $echo = '<script type="text/javascript">
                             function submit_ajax(form_id) {
                                var remark = $("#v2_exp_setting_prioritas_remark").val();
                                if (remark=="") {
                ...
                $echo .= '<form id="form_v2_exp_setting_prioritas_reject" action="'.base_url().'processor/reformulasi/v2_exp_setting_prioritas?action=reject_process" method="post">';
                $echo .= '<div style="vertical-align: top;">';
                $echo .= 'Remark : 
                        <input type="hidden" name="'.$this->main_table_pk.'" value="'.$this->input->get($this->main_table_pk).'" />
                        <input type="hidden" name="modul_id" value="'.$this->input->get('modul_id').'" />
                        ...
                        <textarea name="vRemark" id="reject_v2_exp_setting_prioritas_remark"></textarea>
                <button type="button" onclick="submit_ajax(\'form_v2_exp_setting_prioritas_reject\')">Reject</button>';
    ...
    function after_update_processor($fields, $id, $postData) {
        $post = $this->input->post();
            ...
            $this->db_reformulasi0->where('iExp_setting_prioritas', $id);
            if($this->db_reformulasi0->update('reformulasi.exp_setting_prioritas_detail', array('lDeleted'=>1))) {
                $bobot = 1;
                foreach($post['iexport_req_refor'] as $k=>$v) {

                    $det['iExp_setting_prioritas'] = $id;
                    $det['iexport_req_refor'] = $v;
                    $det['iSort'] = $bobot;
                    $bobot++;
                    $this->db_reformulasi0->insert('reformulasi.exp_setting_prioritas_detail', $det);
                }
            }
            ...
            if ($postData['iSubmit'] == 1){
                ...
                if ($postData['isdraft'] != true && count($activities) > 0){
                    $act = $activities[0];
                    $arr = array();
                    $this->lib_sub_core->InsertActivityModule($arr,$this->modul_id, $id, $act['iM_activity'], $act['iSort']);
                }
    ...
    function manipulate_insert_buttonx($buttons) { 
        ...
        $iframe = '<iframe name="v2_exp_setting_prioritas_frame" id="v2_exp_setting_prioritas_frame" height="0" width="0"></iframe>';
        ...
        $save_draft = '<button onclick="javascript:save_draft_btn_multiupload(\'v2_exp_setting_prioritas\', \' '.base_url().'processor/reformulasi/v2_exp_setting_prioritas?draft=true&modul_id='.$this->input->get('modul_id').'&iM_modul_activity='.$act['iM_modul_activity'].' \',this,true )"  id="button_save_draft_v2_exp_setting_prioritas"  class="ui-button-text icon-save" >Save as Draft</button>';
        $save = '<button onclick="javascript:save_btn_multiupload(\'v2_exp_setting_prioritas\', \' '.base_url().'processor/reformulasi/v2_exp_setting_prioritas?company_id='.$this->input->get('company_id').'&modul_id='.$this->input->get('modul_id').'&iM_modul_activity='.$act['iM_modul_activity'].'&group_id='.$this->input->get('group_id').'modul_id='.$this->input->get('modul_id').' \',this,false )"  id="button_save_submit_v2_exp_setting_prioritas"  class="ui-button-text icon-save" >Save &amp; Submit</button>';
    ...
    function manipulate_update_button($buttons, $rowData) { 
        ...
        if ($this->input->get('action') == 'view') {
            unset($buttons['update']);
                ...
                if($isOpenEditing){
                    $update_draft = '<button onclick="javascript:update_draft_btn(\'v2_exp_setting_prioritas\', \' '.base_url().'processor/reformulasi/v2_exp_setting_prioritas?draft=true \',this,true )"  id="button_update_draft_v2_exp_setting_prioritas"  class="ui-button-text icon-save" >Update open Editing</button>';
                    ...
                    foreach ($activities as $act) {
                        $update_draft = '<button onclick="javascript:update_draft_btn(\'v2_exp_setting_prioritas\', \' '.base_url().'processor/reformulasi/v2_exp_setting_prioritas?draft=true&modul_id='.$this->input->get('modul_id').'&iM_modul_activity='.$act['iM_modul_activity'].' \',this,true )"  id="button_update_draft_v2_exp_setting_prioritas"  class="ui-button-text icon-save" >Update as Draft</button>';
                        $update = '<button onclick="javascript:update_btn_back(\'v2_exp_setting_prioritas\', \' '.base_url().'processor/reformulasi/v2_exp_setting_prioritas?company_id='.$this->input->get('company_id').'&modul_id='.$this->input->get('modul_id').'&iM_modul_activity='.$act['iM_modul_activity'].'&group_id='.$this->input->get('group_id').'modul_id='.$this->input->get('modul_id').' \',this,false )"  id="button_update_submit_v2_exp_setting_prioritas"  class="ui-button-text icon-save" >Update &amp; Submit</button>';

                        $approve = '<button onclick="javascript:load_popup(\' '.base_url().'processor/reformulasi/v2_exp_setting_prioritas?action=approve&iM_modul_activity='.$act['iM_modul_activity'].'&iM_activity='.$act['iM_activity'].'&'.$this->main_table_pk.'='.$peka.'&iupb_id='.$iupb_id.'&company_id='.$this->input->get('company_id').'&group_id='.$this->input->get('group_id').'&modul_id='.$this->input->get('modul_id').' \')"  id="button_approve_v2_exp_setting_prioritas"  class="ui-button-text icon-save" >Approve</button>';
                        $reject = '<button onclick="javascript:load_popup(\' '.base_url().'processor/reformulasi/v2_exp_setting_prioritas?action=reject&iM_modul_activity='.$act['iM_modul_activity'].'&iM_activity='.$act['iM_activity'].'&'.$this->main_table_pk.'='.$peka.'&iupb_id='.$iupb_id.'&company_id='.$this->input->get('company_id').'&group_id='.$this->input->get('group_id').'&modul_id='.$this->input->get('modul_id').' \' )"  id="button_reject_v2_exp_setting_prioritas"  class="ui-button-text icon-save" >Reject</button>';

                        $confirm = '<button onclick="javascript:load_popup(\' '.base_url().'processor/reformulasi/v2_exp_setting_prioritas?action=confirm&iM_modul_activity='.$act['iM_modul_activity'].'&iM_activity='.$act['iM_activity'].'&'.$this->main_table_pk.'='.$peka.'&iupb_id='.$iupb_id.'&company_id='.$this->input->get('company_id').'&group_id='.$this->input->get('group_id').'&modul_id='.$this->input->get('modul_id').' \')"  id="button_approve_v2_exp_setting_prioritas"  class="ui-button-text icon-save" >Confirm</button>';
                        ...
                        if(in_array($act['vDept_assigned'], $arrTeam ) || in_array($this->user->gNIP, $arrNipAssign)  ){
                            ...
                            if(in_array($upbTeamID[$act['vDept_assigned']], $arrTeamID) || in_array($this->user->gNIP, $arrNipAssign) ){
                                ...
                                $sButton = '<span style="color:red;" arrTeamID="'.$this->teamID.'" dep="'.$act['vDept_assigned'].'" title="'.$upbTeamID[$act['vDept_assigned']].'" >You\'re Team not Authorized </span>';
                            }
