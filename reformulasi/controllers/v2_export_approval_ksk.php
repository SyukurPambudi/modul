...
class v2_export_approval_ksk extends MX_Controller {
    function __construct() {
		...
		$this->title 		= 'APPROVAL KSK';
		$this->url 			= 'v2_export_approval_ksk';
		$this->urlpath 		= 'reformulasi/'.str_replace("_","/", $this->url);


		$this->maintable 	= 'reformulasi.export_approval_ksk';	
		$this->main_table 	= $this->maintable;	
		$this->main_table_pk= 'iexport_approval_ksk';	
		
		$datagrid['islist'] = array(
            'export_req_refor.vno_export_req_refor'  => array('label' => 'No Reg Refor','width' => 100,	'align' => 'center', 'search' => true),
            'dossier.vUpd_no' => array('label' => 'No UPD','width' => 200,	'align' => 'left', 'search' => true),                      
			...
			'cApproval' => array('label' => 'Approval','width' => 110,	'align' => 'center', 'search' => false),
		);		

		$datagrid['setQuery']=array(
			0 => array('vall' => 'export_approval_ksk.lDeleted', 'nilai' => 0),
			
		);

		$datagrid['jointableinner'] = array(
			0 => $arrayName = array( 'reformulasi.export_req_refor' => 'export_approval_ksk.iexport_req_refor =export_req_refor.iexport_req_refor '),
			1 => $arrayName = array( 'dossier.dossier_upd' => 'export_req_refor.idossier_upd_id =dossier_upd.idossier_upd_id '),						
			//1 => array('dossier.dossier_upd' => 'dossier_upd.idossier_upd_id = export_req_refor.idossier_upd_id'),
			2 => array('sales.itemas' => 'itemas.c_iteno = dossier_upd.iupb_id'),
			
		);

		$datagrid['shortBy']=array("export_approval_ksk.iexport_approval_ksk" => "DESC");
...
/* 		if (!empty($dataField)){
    ...
    function get_data_prev(){
        ...
        function listBox_v2_export_approval_ksk_iapppr($value) {
...
/*aslinya*/
   /*         if ( $getLastactivity == 0 ) { 
        ...
        function insertBox_v2_export_approval_ksk_form_detail($field,$id){
            $get=$this->input->get();
            $post=$this->input->post();
        ...
        function updateBox_v2_export_approval_ksk_form_detail($field,$id,$value,$rowData){
            $get=$this->input->get();
            $post=$this->input->post();
            foreach ($get as $kget => $vget) {
                if($kget!="action"){
            ...
            function approve_view() { 
                             ...
                             function submit_ajax(form_id) {
                                return $.ajax({
                                    ...
                                    success: function(data) {
                                        ...
                                        var url = "'.base_url().'processor/reformulasi/v2_export_approval_ksk";                             
                                        if(o.status == true) { 
                                            $("#alert_dialog_form").dialog("close");
                                                 $.get(url+"?action=update&id="+last_id+"&foreign_key=0&company_id=3&group_id="+group_id+"&modul_id="+modul_id, function(data) {
                                                 $("div#form_v2_export_approval_ksk").html(data);
                                                 
                                            });
                                            
                                        }
                                            reload_grid("grid_v2_export_approval_ksk");
                                    }
                ...
                $echo .= '<form id="form_v2_export_approval_ksk_approve" action="'.base_url().'processor/reformulasi/v2_export_approval_ksk?action=approve_process" method="post">';
                ...
                <button type="button" onclick="submit_ajax(\'form_v2_export_approval_ksk_approve\')">Approve</button>';
           ...
           function approve_process() {
            ...
            function confirm_view() { 
                ...
                $echo .= '<form id="form_v2_export_approval_ksk_confirm" action="'.base_url().'processor/reformulasi/v2_export_approval_ksk?action=confirm_process" method="post">';
                $echo .= '<div style="vertical-align: top;">';
                $echo .= 'Remark : 
                        <input type="hidden" name="'.$this->main_table_pk.'" value="'.$this->input->get($this->main_table_pk).'" />
                        <input type="hidden" name="modul_id" value="'.$this->input->get('modul_id').'" />
                        ...
                        <textarea name="vRemark"></textarea>
                <button type="button" onclick="submit_ajax(\'form_v2_export_approval_ksk_confirm\')">Confirm</button>';
            ...
            function reject_view() {
                $echo = '<script type="text/javascript">
                             function submit_ajax(form_id) {
                                var remark = $("#v2_export_approval_ksk_remark").val();
                                if (remark=="") {
                ...
                $echo .= '<form id="form_v2_export_approval_ksk_reject" action="'.base_url().'processor/reformulasi/v2_export_approval_ksk?action=reject_process" method="post">';
                $echo .= '<div style="vertical-align: top;">';
                $echo .= 'Remark : 
                        <input type="hidden" name="'.$this->main_table_pk.'" value="'.$this->input->get($this->main_table_pk).'" />
                        <input type="hidden" name="modul_id" value="'.$this->input->get('modul_id').'" />
                        ...
                        <textarea name="vRemark" id="reject_v2_export_approval_ksk_remark"></textarea>
                <button type="button" onclick="submit_ajax(\'form_v2_export_approval_ksk_reject\')">Reject</button>';
    ...
    function after_insert_processor($fields, $id, $postData) { 
        ...
        if($postData['iSubmit']==1){
            $sql = "SELECT em.`vName`, sa.`dujebe`, sa.`dselesaiujebe`, sa.`iTeamAd`,
                e.`vno_export_req_refor` FROM reformulasi.`export_approval_ksk` sa
                ...
                WHERE sa.`lDeleted` = 0 AND sa.`iexport_approval_ksk` =  ".$id;
    ...
    function after_update_processor($fields, $id, $postData) {
        ...
        if($postData['isubmit']==1){
            ...
            $activity = $this->db->get_where('erp_privi.m_modul_activity', array('iM_modul_activity'=>$post['iM_modul_activity'], 'lDeleted'=>0))->row_array();
            $this->lib_sub_core->InsertActivityModule($this->ViewUPB($id),$modul_id,$id,$activity['iM_activity'],$activity['iSort']);
            $sql = "SELECT em.`vName`, sa.`dujebe`, sa.`dselesaiujebe`,sa.`iTeamAd`,   e.`vno_export_req_refor` FROM reformulasi.`export_approval_ksk` sa
                JOIN reformulasi.`export_req_refor` e ON sa.`iexport_req_refor` = e.`iexport_req_refor`
                JOIN hrd.`employee` em ON em.`cNip` = sa.`cpic_lapvalpro`
                WHERE sa.`lDeleted` = 0 AND sa.`iexport_approval_ksk` =  ".$id;
            $mailDet = $this->db->query($sql)->row_array();
    ...
    function manipulate_insert_buttonx($buttons){
		unset($buttons['save']);
        $js = $this->load->view('export/js/export_approval_ksk_js');
        $save_draft = '<button onclick="javascript:save_draft_btn_multiupload(\'export_approval_ksk\', \''.base_url().'processor/reformulasi/export/protokol/valpro?draft=true\', this, true)" class="ui-button-text icon-save" id="button_save_draft_export_approval_ksk_js">Save as Draft</button>';
        $save = '<button onclick="javascript:save_btn_multiupload(\'export_approval_ksk\', \''.base_url().'processor/reformulasi/export/protokol/valpro?company_id='.$this->input->get('company_id').'&group_id='.$this->input->get('group_id').'&modul_id='.$this->input->get('modul_id').'\', this)" class="ui-button-text icon-save" id="button_save_export_approval_ksk_js">Save &amp Submit</button>';
 ...
 function manipulate_update_button($buttons, $rowData) { 
        ...
        $iframe = '<iframe name="v2_export_approval_ksk" id="v2_export_approval_ksk" height="0" width="0"></iframe>';
        ...
        if ($this->input->get('action') == 'view') {
            unset($buttons['update']);
                ...
                if($isOpenEditing){
                    $update_draft = '<button onclick="javascript:update_draft_btn(\'v2_export_approval_ksk\', \' '.base_url().'processor/reformulasi/v2_export_approval_ksk?draft=true \',this,true )"  id="button_update_draft_v2_export_approval_ksk"  class="ui-button-text icon-save" >Update open Editing</button>';
                    ...
                    foreach ($activities as $act) {
                        $update_draft = '<button onclick="javascript:update_draft_btn(\'v2_export_approval_ksk\', \' '.base_url().'processor/reformulasi/v2_export_approval_ksk?draft=true&modul_id='.$this->input->get('modul_id').'&iM_modul_activity='.$act['iM_modul_activity'].' \',this,true )"  id="button_update_draft_v2_export_approval_ksk"  class="ui-button-text icon-save" >Update as Draft</button>';
                        $update = '<button onclick="javascript:update_btn_back(\'v2_export_approval_ksk\', \' '.base_url().'processor/reformulasi/v2_export_approval_ksk?company_id='.$this->input->get('company_id').'&modul_id='.$this->input->get('modul_id').'&iM_modul_activity='.$act['iM_modul_activity'].'&group_id='.$this->input->get('group_id').'modul_id='.$this->input->get('modul_id').' \',this,false )"  id="button_update_submit_v2_export_approval_ksk"  class="ui-button-text icon-save" >Update &amp; Submit</button>';

                        $approve = '<button onclick="javascript:load_popup(\' '.base_url().'processor/reformulasi/v2_export_approval_ksk?action=approve&iM_modul_activity='.$act['iM_modul_activity'].'&iM_activity='.$act['iM_activity'].'&'.$this->main_table_pk.'='.$peka.'&iupb_id='.$iupb_id.'&company_id='.$this->input->get('company_id').'&group_id='.$this->input->get('group_id').'&modul_id='.$this->input->get('modul_id').' \')"  id="button_approve_v2_export_approval_ksk"  class="ui-button-text icon-save" >Approve</button>';
                        $reject = '<button onclick="javascript:load_popup(\' '.base_url().'processor/reformulasi/v2_export_approval_ksk?action=reject&iM_modul_activity='.$act['iM_modul_activity'].'&iM_activity='.$act['iM_activity'].'&'.$this->main_table_pk.'='.$peka.'&iupb_id='.$iupb_id.'&company_id='.$this->input->get('company_id').'&group_id='.$this->input->get('group_id').'&modul_id='.$this->input->get('modul_id').' \' )"  id="button_reject_v2_export_approval_ksk"  class="ui-button-text icon-save" >Reject</button>';

                        $confirm = '<button onclick="javascript:load_popup(\' '.base_url().'processor/reformulasi/v2_export_approval_ksk?action=confirm&iM_modul_activity='.$act['iM_modul_activity'].'&iM_activity='.$act['iM_activity'].'&'.$this->main_table_pk.'='.$peka.'&iupb_id='.$iupb_id.'&company_id='.$this->input->get('company_id').'&group_id='.$this->input->get('group_id').'&modul_id='.$this->input->get('modul_id').' \')"  id="button_approve_v2_export_approval_ksk"  class="ui-button-text icon-save" >Confirm</button>';
    ...
    function manipulate_update_buttonx($buttons, $rowData) { 
        if ($this->input->get('action') == 'view') {
            ...
            $js = $this->load->view('export/js/export_approval_ksk_js');
            $update = '<button onclick="javascript:update_btn_back(\'export_approval_ksk\', \''.base_url().'processor/reformulasi/export/protokol/valpro?company_id='.$this->input->get('company_id').'&group_id='.$this->input->get('group_id').'&modul_id='.$this->input->get('modul_id').'\', this)" class="ui-button-text icon-save" id="button_update_export_approval_ksk">Update & Submit</button>';
            $updatedraft = '<button onclick="javascript:update_draft_btn(\'export_approval_ksk\', \''.base_url().'processor/reformulasi/export/protokol/valpro?company_id='.$this->input->get('company_id').'&draft=true&group_id='.$this->input->get('group_id').'&modul_id='.$this->input->get('modul_id').'\', this, true)" class="ui-button-text icon-save" id="button_save_export_approval_ksk">Update as Draft</button>';

           $approve = '<button onclick="javascript:load_popup(\''.base_url().'processor/reformulasi/export/protokol/valpro?action=approve&iexport_approval_ksk='.$rowData['iexport_approval_ksk'].'&cNip='.$cNip.'&lvl=1&status=1&group_id='.$this->input->get('group_id').'&modul_id='.$this->input->get('modul_id').'\')" class="ui-button-text icon-save" id="button_approve_export_approval_ksk">Confirm</button>';
            $reject = '<button onclick="javascript:load_popup(\''.base_url().'processor/reformulasi/export/protokol/valpro?action=reject&iexport_approval_ksk='.$rowData['iexport_approval_ksk'].'&cNip='.$cNip.'&lvl=1&status=2&group_id='.$this->input->get('group_id').'&modul_id='.$this->input->get('modul_id').'\')" class="ui-button-text icon-save" id="button_reject_export_approval_ksk">Reject</button>';
            ...
            if($rowData['iSubmit']==0){
                ...
                if($rowData['iApproval']==0 or empty($rowData['iApproval'])){
