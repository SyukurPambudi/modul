...
class v2_export_laporan_valpro extends MX_Controller {
    function __construct() {
		...
		$this->title 		= 'Laporan Validasi';
		$this->url 			= 'v2_export_laporan_valpro';
		$this->urlpath 		= 'reformulasi/'.str_replace("_","/", $this->url);


		$this->maintable 	= 'reformulasi.export_lap_valpro';	
		$this->main_table 	= $this->maintable;	
		$this->main_table_pk= 'iexport_lap_valpro';	
		
		$datagrid['islist'] = array(
           # 'export_req_refor.vno_export_req_refor'  => array('label' => 'No Req Formula','width' => 100,	'align' => 'center', 'search' => true),
           # 'itemas.c_itnam' => array('label' => 'Nama Produk','width' => 200,	'align' => 'left', 'search' => true ),
            'dmulai_lapvalpro'	=> array('label' => 'Tgl Mulai','width' => 100,	'align' => 'center', 'search' => true),
            'dselesai_lapvalpro' => array('label' => 'Tgl Selesai',	'width' => 100,	'align' => 'center', 'search' => true),
			'cpic_lapvalpro' => array('label' => 'NIP PIC','width' => 110,	'align' => 'center', 'search' => true),
			...
			'iapplapVP' => array('label' => 'Approval','width' => 110,	'align' => 'center', 'search' => false),
		);		

		$datagrid['setQuery']=array(
			0 => array('vall' => 'export_lap_valpro.lDeleted', 'nilai' => 0),
			
		);

		$datagrid['jointableinner'] = array(
            0 => array( 'reformulasi.export_protokol_valpro' => 'export_protokol_valpro.iexport_protokol_valpro = export_lap_valpro.iexport_protokol_valpro '),
            1 => array( 'reformulasi.export_dok_mbr' => 'export_dok_mbr.idok_mbr = export_protokol_valpro.idok_mbr '),
            2 => array( 'reformulasi.export_refor_basic_formula' => 'export_refor_basic_formula.iexport_refor_basic_formula =export_dok_mbr.iexport_refor_basic_formula '),
            3 => array( 'reformulasi.export_refor_formula' => 'export_refor_formula.iexport_refor_formula = export_refor_basic_formula.iexport_refor_formula '),
			4 => array( 'reformulasi.export_req_refor' => 'export_req_refor.iexport_req_refor =export_refor_formula.iexport_req_refor '),
			5 => array('hrd.employee' => 'employee.cNip = export_lap_valpro.cpic_lapvalpro'),
			6 => array('dossier.dossier_upd' => 'dossier_upd.idossier_upd_id = export_req_refor.idossier_upd_id'),
			7 => array('sales.itemas' => 'itemas.c_iteno = dossier_upd.iupb_id'),
			
		);

		$datagrid['shortBy']=array("export_lap_valpro.iexport_lap_valpro" => "DESC");
    ...
    function index($action = '') {
      ...
      /*  $grid->changeSearch('dPermintaan_req_export','between');*/
		/*$grid->changeSearch('cpic_lapvalpro','combobox');*/

        $grid->changeFieldType('iSubmit', 'combobox','',array(''=>'--select--',0=>'Draft - Need to be Submit', 1=>'Submited'));
		$grid->changeFieldType('iapplapVP', 'combobox','',array(''=>'--select--',0=>'Waiting for approval',1=>'Rejected', 2=>'Approved'));
...
/* 		if (!empty($dataField)){
    ...
    function get_data_prev(){
        ...
        function listBox_v2_export_laporan_valpro_iapppr($value) {
...
/*aslinya*/
   /*         if ( $getLastactivity == 0 ) { 
        ...
        function insertBox_v2_export_laporan_valpro_form_detail($field,$id){
            $get=$this->input->get();
            $post=$this->input->post();
        ...
        function updateBox_v2_export_laporan_valpro_form_detail($field,$id,$value,$rowData){
            $get=$this->input->get();
            $post=$this->input->post();
            foreach ($get as $kget => $vget) {
                if($kget!="action"){
            ...
            function approve_view() { 
                             ...
                             function submit_ajax(form_id) {
                                return $.ajax({
                                    ...
                                    success: function(data) {
                                        ...
                                        var url = "'.base_url().'processor/reformulasi/v2_export_laporan_valpro";                             
                                        if(o.status == true) { 
                                            $("#alert_dialog_form").dialog("close");
                                                 $.get(url+"?action=update&id="+last_id+"&foreign_key=0&company_id=3&group_id="+group_id+"&modul_id="+modul_id, function(data) {
                                                 $("div#form_v2_export_laporan_valpro").html(data);
                                                 
                                            });
                                            
                                        }
                                            reload_grid("grid_v2_export_laporan_valpro");
                                    }
                ...
                $echo .= '<form id="form_v2_export_laporan_valpro_approve" action="'.base_url().'processor/reformulasi/v2_export_laporan_valpro?action=approve_process" method="post">';
                ...
                <button type="button" onclick="submit_ajax(\'form_v2_export_laporan_valpro_approve\')">Approve</button>';
           ...
           function approve_process() {
            ...
            function confirm_view() { 
                ...
                $echo .= '<form id="form_v2_export_laporan_valpro_confirm" action="'.base_url().'processor/reformulasi/v2_export_laporan_valpro?action=confirm_process" method="post">';
                $echo .= '<div style="vertical-align: top;">';
                $echo .= 'Remark : 
                        <input type="hidden" name="'.$this->main_table_pk.'" value="'.$this->input->get($this->main_table_pk).'" />
                        <input type="hidden" name="modul_id" value="'.$this->input->get('modul_id').'" />
                        ...
                        <textarea name="vRemark"></textarea>
                <button type="button" onclick="submit_ajax(\'form_v2_export_laporan_valpro_confirm\')">Confirm</button>';
            ...
            function reject_view() {
                $echo = '<script type="text/javascript">
                             function submit_ajax(form_id) {
                                var remark = $("#v2_export_laporan_valpro_remark").val();
                                if (remark=="") {
                ...
                $echo .= '<form id="form_v2_export_laporan_valpro_reject" action="'.base_url().'processor/reformulasi/v2_export_laporan_valpro?action=reject_process" method="post">';
                $echo .= '<div style="vertical-align: top;">';
                $echo .= 'Remark : 
                        <input type="hidden" name="'.$this->main_table_pk.'" value="'.$this->input->get($this->main_table_pk).'" />
                        <input type="hidden" name="modul_id" value="'.$this->input->get('modul_id').'" />
                        ...
                        <textarea name="vRemark" id="reject_v2_export_laporan_valpro_remark"></textarea>
                <button type="button" onclick="submit_ajax(\'form_v2_export_laporan_valpro_reject\')">Reject</button>';
    ...
    function after_insert_processor($fields, $id, $postData) { 
        ...
        if($postData['iSubmit']==1){
            ...
            $sqlDetail  = 'SELECT lv.*
                                    ,em.vName
                                    ,d.vno_export_req_refor
                                    ,pv.iexport_protokol_valpro AS "No Protokol Valpro",c.vnoFormulasi AS "No Formula"
                                    ,e.vUpd_no AS "No UPD"
                                    ...
                                    FROM 
                                            reformulasi.export_lap_valpro lv 
                                            JOIN reformulasi.export_protokol_valpro pv ON pv.iexport_protokol_valpro=lv.iexport_protokol_valpro
                                    JOIN hrd.`employee` em ON em.`cNip` = lv.cpic_lapvalpro
                                    ...
                                    AND c.lDeleted=0
                                    AND d.lDeleted=0
                                    AND lv.iexport_lap_valpro= ?
                        ';
            $que        = $this->db->query($sqlDetail, array($id));
            $mailDet = $que->row_array();
            ...
            $cc = $this->user->gNIP;
            $subject="Submited Laporan Validasi ";
            $content="
            Diberitahukan bahwa telah ada Submited Modul Protokol Valpro,
                ...
                <table border='0' style='width: 600px;'>
                    ...
                    <tr>
                            <td><b>PIC Laporan Validasi  </b></td><td> : </td>
                            <td>".$mailDet['vName']."</td>
                    </tr>
                    <tr>
                            <td><b>Tanggal Mulai Laporan Validasi  </b></td><td> : </td>
                            <td>".$mailDet['dmulai_lapvalpro']."</td>
                    </tr>
                    <tr>
                            <td><b>Tanggal Selesai Laporan Validasi  </b></td><td> : </td>
                            <td>".$mailDet['dselesai_lapvalpro']."</td>
                    </tr>
                </table>
            <br/> <br/>
    ...
    function after_update_processor($fields, $id, $postData) {
        ...
        $sqH = "UPDATE reformulasi.`export_lap_valpro_detail` e 
                            SET e.lDeleted = 1, 
                            e.cUpdate='".$this->user->gNIP."',
                            e.dupdate='".date('Y-m-d H:i:s')."' 
                            WHERE e.iexport_lap_valpro = '".$id."'
                            ";
                $this->db->query($sqH);

        //Insert Foreach 
        if(isset($postData['detbatch'])){
            if(($postData['detbatch'][0])!=""){
                foreach($postData['detbatch'] as $k=>$v){ 
                    $pri2['iexport_lap_valpro']=$id;
                    $pri2['cpanumb']=$v;
                    $pri2['dCreate']=date('Y-m-d H:i:s');
                    $pri2['cCreated']=$this->user->gNIP;
                    $this->db->insert('reformulasi.export_lap_valpro_detail', $pri2);    
                }
            }
        }
        ...
        if($postData['iSubmit']==1){
            ...
            //$sql = "SELECT em.`vName`, sa.`dmulai_lapvalpro`, sa.`dselesai_lapvalpro`, sa.`iTeamPd`,
            //
            $sqlDetail  = 'SELECT lv.*
                                    ,em.vName
                                    ,d.vno_export_req_refor
    ...
    function manipulate_insert_buttonx($buttons){
		unset($buttons['save']);
        $js = $this->load->view('export/js/export_lap_valpro_js');
        $save_draft = '<button onclick="javascript:save_draft_btn_multiupload(\'export_lap_valpro\', \''.base_url().'processor/reformulasi/export/protokol/valpro?draft=true\', this, true)" class="ui-button-text icon-save" id="button_save_draft_export_lap_valpro_js">Save as Draft</button>';
        $save = '<button onclick="javascript:save_btn_multiupload(\'export_lap_valpro\', \''.base_url().'processor/reformulasi/export/protokol/valpro?company_id='.$this->input->get('company_id').'&group_id='.$this->input->get('group_id').'&modul_id='.$this->input->get('modul_id').'\', this)" class="ui-button-text icon-save" id="button_save_export_lap_valpro_js">Save &amp Submit</button>';
 ...
 function manipulate_update_button($buttons, $rowData) { 
        ...
        $iframe = '<iframe name="v2_export_laporan_valpro" id="v2_export_laporan_valpro" height="0" width="0"></iframe>';
        ...
        if ($this->input->get('action') == 'view') {
            unset($buttons['update']);
                ...
                if($isOpenEditing){
                    $update_draft = '<button onclick="javascript:update_draft_btn(\'v2_export_laporan_valpro\', \' '.base_url().'processor/reformulasi/v2_export_laporan_valpro?draft=true \',this,true )"  id="button_update_draft_v2_export_laporan_valpro"  class="ui-button-text icon-save" >Update open Editing</button>';
                    ...
                    foreach ($activities as $act) {
                        $update_draft = '<button onclick="javascript:update_draft_btn(\'v2_export_laporan_valpro\', \' '.base_url().'processor/reformulasi/v2_export_laporan_valpro?draft=true&modul_id='.$this->input->get('modul_id').'&iM_modul_activity='.$act['iM_modul_activity'].' \',this,true )"  id="button_update_draft_v2_export_laporan_valpro"  class="ui-button-text icon-save" >Update as Draft</button>';
                        $update = '<button onclick="javascript:update_btn_back(\'v2_export_laporan_valpro\', \' '.base_url().'processor/reformulasi/v2_export_laporan_valpro?company_id='.$this->input->get('company_id').'&modul_id='.$this->input->get('modul_id').'&iM_modul_activity='.$act['iM_modul_activity'].'&group_id='.$this->input->get('group_id').'modul_id='.$this->input->get('modul_id').' \',this,false )"  id="button_update_submit_v2_export_laporan_valpro"  class="ui-button-text icon-save" >Update &amp; Submit</button>';

                        $approve = '<button onclick="javascript:load_popup(\' '.base_url().'processor/reformulasi/v2_export_laporan_valpro?action=approve&iM_modul_activity='.$act['iM_modul_activity'].'&iM_activity='.$act['iM_activity'].'&'.$this->main_table_pk.'='.$peka.'&iupb_id='.$iupb_id.'&company_id='.$this->input->get('company_id').'&group_id='.$this->input->get('group_id').'&modul_id='.$this->input->get('modul_id').' \')"  id="button_approve_v2_export_laporan_valpro"  class="ui-button-text icon-save" >Approve</button>';
                        $reject = '<button onclick="javascript:load_popup(\' '.base_url().'processor/reformulasi/v2_export_laporan_valpro?action=reject&iM_modul_activity='.$act['iM_modul_activity'].'&iM_activity='.$act['iM_activity'].'&'.$this->main_table_pk.'='.$peka.'&iupb_id='.$iupb_id.'&company_id='.$this->input->get('company_id').'&group_id='.$this->input->get('group_id').'&modul_id='.$this->input->get('modul_id').' \' )"  id="button_reject_v2_export_laporan_valpro"  class="ui-button-text icon-save" >Reject</button>';

                        $confirm = '<button onclick="javascript:load_popup(\' '.base_url().'processor/reformulasi/v2_export_laporan_valpro?action=confirm&iM_modul_activity='.$act['iM_modul_activity'].'&iM_activity='.$act['iM_activity'].'&'.$this->main_table_pk.'='.$peka.'&iupb_id='.$iupb_id.'&company_id='.$this->input->get('company_id').'&group_id='.$this->input->get('group_id').'&modul_id='.$this->input->get('modul_id').' \')"  id="button_approve_v2_export_laporan_valpro"  class="ui-button-text icon-save" >Confirm</button>';
    ...
    function manipulate_update_buttonx($buttons, $rowData) { 
        if ($this->input->get('action') == 'view') {
            ...
            $js = $this->load->view('export/js/export_lap_valpro_js');
            $update = '<button onclick="javascript:update_btn_back(\'export_lap_valpro\', \''.base_url().'processor/reformulasi/export/protokol/valpro?company_id='.$this->input->get('company_id').'&group_id='.$this->input->get('group_id').'&modul_id='.$this->input->get('modul_id').'\', this)" class="ui-button-text icon-save" id="button_update_export_lap_valpro">Update & Submit</button>';
            $updatedraft = '<button onclick="javascript:update_draft_btn(\'export_lap_valpro\', \''.base_url().'processor/reformulasi/export/protokol/valpro?company_id='.$this->input->get('company_id').'&draft=true&group_id='.$this->input->get('group_id').'&modul_id='.$this->input->get('modul_id').'\', this, true)" class="ui-button-text icon-save" id="button_save_export_lap_valpro">Update as Draft</button>';

           $approve = '<button onclick="javascript:load_popup(\''.base_url().'processor/reformulasi/export/protokol/valpro?action=approve&iexport_lap_valpro='.$rowData['iexport_lap_valpro'].'&cNip='.$cNip.'&lvl=1&status=1&group_id='.$this->input->get('group_id').'&modul_id='.$this->input->get('modul_id').'\')" class="ui-button-text icon-save" id="button_approve_export_lap_valpro">Confirm</button>';
            $reject = '<button onclick="javascript:load_popup(\''.base_url().'processor/reformulasi/export/protokol/valpro?action=reject&iexport_lap_valpro='.$rowData['iexport_lap_valpro'].'&cNip='.$cNip.'&lvl=1&status=2&group_id='.$this->input->get('group_id').'&modul_id='.$this->input->get('modul_id').'\')" class="ui-button-text icon-save" id="button_reject_export_lap_valpro">Reject</button>';
            ...
            if($rowData['iSubmit']==0){
                ...
                if($rowData['iapplapVP']==0 or empty($rowData['iapplapVP'])){
