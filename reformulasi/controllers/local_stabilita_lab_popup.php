...
class local_stabilita_lab_popup extends MX_Controller {
    ...
    function index($action = '') {
		...
		$grid->setUrl('local_stabilita_lab_popup');
		...
		$grid->setQuery('lokal_refor_skala_trial.ilokal_refor_skala_trial NOT IN (select st.ilokal_refor_skala_trial from reformulasi.lokal_refor_stabilita_lab st where st.ldeleted=0)',NULL);
				...
				if($vsel=="iact"){
					...
					if($value != '') {
						if (file_exists('./files/reformulasi/local/request_refor/'.$k->iLocal_req_refor.'/'.$value)) {
							$link = base_url().'processor/reformulasi/local/stabilita/lab/popup?action=download&id='.$k->iLocal_req_refor.'&file='.$value;
	...
	function listBox_local_stabilita_lab_popup_pilih($value, $pk, $name, $rowData) {
