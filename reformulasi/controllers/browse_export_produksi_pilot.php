...
class browse_export_produksi_pilot extends MX_Controller {
    ...
    function index($action = '') {
        ...
        $grid->setUrl('browse_export_produksi_pilot');
        ...
        /*Filter untuk MBR*/
        $grid->setQuery('export_refor_formula.iexport_req_refor in (select vno_request from reformulasi.export_dok_mbr where ldeleted=0 and iappr_pd=2)',NULL);
        $grid->setQuery('export_refor_formula.iexport_refor_formula not in (select iexport_refor_formula from reformulasi.export_produksi_pilot where ldeleted=0)',NULL);
        ...
        $grid->setQuery('export_refor_formula.iksm_skala_trial', 1); // Skala Trial MS
        ...
        switch ($action) {
            ...
            default:
                $grid->render_grid();
                $url=base_url()."processor/reformulasi/browse/export/produksi/pilot?action=getdetails";
                $o='<script type="text/javascript">
                function pilih_request_produksi_pilot(id){    
                    custom_confirm("Yakin ?", function(){
                        $.ajax({
                            ...
                            success: function( data ) {
                                ...
                                $("#tb_details_export_produksi_pilot_id").val(o.iexport_req_refor);
                                reload_grid_tb_details_export_produksi_pilot();
                            }
                        });
    ...
    function listBox_browse_export_produksi_pilot_pilih($value, $pk, $name, $rowData) {
        $o = '<input type="radio" name="pilih" onClick="javascript:pilih_request_produksi_pilot('.$pk.') ;" />';
