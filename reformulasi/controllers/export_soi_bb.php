        ...
        class export_soi_bb extends MX_Controller {
            ...
            function index($action = '') {
                ...
                $grid->setTable('reformulasi.export_soi_bb');      
                $grid->setUrl('export_soi_bb');

                //List Table
                $grid->addList('export_req_refor.vno_export_req_refor','dossier_upd.vUpd_no','dossier_upd.vNama_usulan','vNo_soi','ireformulasi_team','iSubmit','iApprove'); 
                $grid->setSortBy('iexport_soi_bb');
                $grid->setSortOrder('DESC');  

                //List field
                $grid->addFields('iexport_req_refor','vNo_soi','dTgl_berlaku','iType','iRevisi','ireformulasi_team','cPic_penyusun','file_filename','iApprove'); 
                ...
                */
                ...
                $grid->setWidth('vNo_soi', '100');
                $grid->setAlign('vNo_soi', 'left');
                $grid->setLabel('vNo_soi','No SOI');
                ...
                $grid->setWidth('dTgl_berlaku', '150');
                $grid->setAlign('dTgl_berlaku', 'left');
                $grid->setLabel('dTgl_berlaku','Tgl Berlaku');
            
                $grid->setWidth('iRevisi', '150');
                $grid->setAlign('iRevisi', 'left');
                ...
                //Join Table
                    $grid->setJoinTable('reformulasi.export_req_refor', 'export_req_refor.iexport_req_refor = export_soi_bb.iexport_req_refor', 'inner');
                    ...
                    $grid->setRelation('ireformulasi_team','reformulasi.reformulasi_team','ireformulasi_team','vteam','vteam','inner',array('ldeleted'=>0),array('vteam'=>'asc'));

                // Change Field Name
                $grid->changeFieldType('iType','combobox','',array(''=>'--Pilih--','0'=>'Tentative','1'=>'Final'));
                ...
                */
                 ...
                 //set search
                $grid->setSearch('vNo_soi','ireformulasi_team');
                
                //set required
                $grid->setRequired('iexport_req_refor','vNo_soi','dTgl_berlaku','iRevisi','cPic_penyusun','ireformulasi_team','iApprove','cApprove','dApprove','vRemark','dCreate','cCreated','dupdate','cUpdate','lDeleted'); 
                ...
                switch ($action) {
                    ...
                    case 'createproses':
                            ...
                            $sql1 = array();
                            if($isUpload) {
                                $path = realpath("files/reformulasi/export/export_soi_bb/");
                                //$path1 = realpath("files/reformulasi/export/export_soi_bb/rpt_study_literatur/");
                                ...
                                foreach ($_FILES['fileupload']["error"] as $key => $error) {
                                    if ($error == UPLOAD_ERR_OK) {              
                                        ...
                                        if(move_uploaded_file($tmp_name, $path."/".$this->input->get('lastId')."/".$name)) {    
                                            $sql[] = "INSERT INTO reformulasi.export_soi_bb_file(iexport_soi_bb, vFilename, dCreate, vKeterangan,cCreate) 
                    ...
                    case 'updateproses':
                            $isUpload = $this->input->get('isUpload');
                            $lastId=$_POST['export_soi_bb_iexport_soi_bb'];
                            $sql = array();
                            $sql1 = array();
                            $file_name= "";
                            $file_name1= "";
                            ...
                            if (!file_exists( $path."/".$this->input->post('export_soi_bb_iexport_soi_bb') )) {
                                mkdir($path."/".$this->input->post('export_soi_bb_iexport_soi_bb'), 0777, true);                      
                            }
                            ...
                            if($isUpload) {
                                ...
                                if (isset($_FILES['fileupload'])) {
                                    $this->hapusfile($path, $file_name, 'export_soi_bb_file', $lastId);
                                    foreach ($_FILES['fileupload']["error"] as $key => $error) {    
                                        if ($error == UPLOAD_ERR_OK) {
                                            $tmp_name = $_FILES['fileupload']["tmp_name"][$key];
                                            $name = $_FILES['fileupload']["name"][$key];
                                            $data['vFilename'] = $name;
                                            $data['id']=$this->input->post('export_soi_bb_iexport_soi_bb');
                                            $data['nip']=$this->user->gNIP;
                                            $data['dCreate'] = date('Y-m-d H:i:s');
                                            //$file_tanggal[$i] = date('l, F jS, Y', strtotime($file_tanggal[$i]));     
                                            if(move_uploaded_file($tmp_name, $path."/".$this->input->post('export_soi_bb_iexport_soi_bb')."/".$name)) 
                                            {                                   
                                                $sql[] = "INSERT INTO reformulasi.export_soi_bb_file(iexport_soi_bb, vFilename, dCreate, vKeterangan,cCreate) 
            ...
            function download($vFilename) {
                ...
                $name = $vFilename;
                $id = $_GET['id']; 
                $path = file_get_contents('./files/reformulasi/export/export_soi_bb/'.$id.'/'.$name);    
            ...
            function hapusfile($path, $file_name, $table, $lastId){
                ...
                if (is_array($file_name)) {
                    ...
                    foreach($list_sql as $v) {
                        if (!in_array($v, $file_name)) {                
                            $del = "delete from reformulasi.".$table." where iexport_soi_bb = {$lastId} and vFilename= '{$v}'";
                            //echo $del;
                            mysql_query($del);  
                        }
            ...
            function readSQL($table, $lastId, $empty="") {
                $list_file = array();
                if (empty($empty)) {
                    $sql = "SELECT vFilename from reformulasi.".$table." where iexport_soi_bb=".$lastId;
                    ...
                    $sql2 = array();
                    while($row = mysql_fetch_array($query, MYSQL_ASSOC)) {
                        $sql2[] = "DELETE FROM reformulasi.".$table." where iexport_soi_bb=".$lastId." and vFilename='".$row['vFilename']."'";         
                    }
            ...
            function listBox_export_soi_bb_cPic_penyusun($value, $pk, $name, $rowData) {
            ...
            function insertBox_export_soi_bb_iexport_req_refor($field, $id) {     
                $uri = 'export_soi_bb';
                $o = "";  
                $o   .= "<div style='
                                padding: 3px;
                ...
                $o .= '         &nbsp;<button class="icon_pop"  onclick="browse(\''.base_url().'processor/reformulasi/browse_soi_bb/?field=export_soi_bb\',\'Request Reformulasi\')" type="button">Browse</button>';  
                $o  .="             </td>
                                </tr>
            ...
            function updateBox_export_soi_bb_iexport_req_refor($field, $id, $value, $rowData) {
                ...
                if(!empty($dt['iexport_req_refor'])){
                        ...
                        $itname = "SELECT i.`c_itnam`,d.vUpd_no FROM sales.`itemas` i 
                        ...
                        $_cIteno = $c_itnam['vUpd_no'].' - '.$c_itnam['c_itnam'];
            ...
            function insertBox_export_soi_bb_file_filename() {
                $data['mydept'] = $this->auth_export->my_depts(TRUE);   
                $data['date'] = date('Y-m-d H:i:s');    
                return $this->load->view('export/sample/export_soi_bb_file',$data,TRUE); 
            }

            function updateBox_export_soi_bb_file_filename($field, $id, $value, $rowData) {
                if (!empty($_GET['idnya'])) {
                    $iexport_soi_bb = $_GET['idnya'];
                }else{
                    $iexport_soi_bb = $rowData['iexport_soi_bb'];
                }
                $data['mydept'] = $this->auth_export->my_depts(TRUE);
                $data['rows'] = $this->db->get_where('reformulasi.export_soi_bb_file', array('iexport_soi_bb'=>$iexport_soi_bb))->result_array();
                return $this->load->view('export/sample/export_soi_bb_file',$data,TRUE);              
            }
            ...
            function insertBox_export_soi_bb_iexport_req_reforx($field, $id) {
            ...
            function updateBox_export_soi_bb_iexport_req_reforx($field, $id, $value, $rowData) {
            ...
            function insertBox_export_soi_bb_vNo_soi($field, $id) {
            ...
            function updateBox_export_soi_bb_vNo_soi($field, $id, $value, $rowData) {
                if ($this->input->get('action') == 'view') {
                     $return= $value; 
                }else{
            ...
            function insertBox_export_soi_bb_dTgl_berlaku($field, $id) {
            ...
            function updateBox_export_soi_bb_dTgl_berlaku($field, $id, $value, $rowData) {
                if ($this->input->get('action') == 'view') {
                     $return= $value; 
                }else{
            ...
            function insertBox_export_soi_bb_iRevisi($field, $id) {
                $return = '<input type="text" name="'.$field.'"  id="'.$id.'"  class="input_rows1 required" size="8" />';
                $return .= '<script>
                                $("#'.$id.'").numeric(); 
                             </script>';
            ...
            function updateBox_export_soi_bb_iRevisi($field, $id, $value, $rowData) {
                if ($this->input->get('action') == 'view') {
                     $return= $value; 
                }else{
                $return = '<input type="text" name="'.$field.'"  id="'.$id.'"  class="input_rows1 required" size="8" value="'.$value.'"/>';
            ...
            function insertBox_export_soi_bb_cPic_penyusunx($field, $id) {
                $return = '<input type="text" name="'.$field.'"  id="'.$id.'"  class="input_rows1 required" size="30" maxlength = "50" />';
                $return .= '<script>
                                $("#'.$id.'").keyup(function() {
                                    var len = this.value.length;
            ...
            function insertBox_export_soi_bb_cPic_penyusun($field, $id) {
                $url=base_url().'processor/reformulasi/export/soi/bb?action=getPicPenyusun';
                    ...
                    var config3 = {
                        source: function( request, response) {
                            $.ajax({
                                url: "'.$url.'",
                                beforeSend: function(){
                                    var idAD =  $("#export_soi_bb_ireformulasi_team").val();
                                ...
                                data: {
                                    term: request.term,
                                    idAD: $("#export_soi_bb_ireformulasi_team").val(),
            ...
            function updateBox_export_soi_bb_cPic_penyusun($field, $id, $value, $rowData) {
                $url=base_url().'processor/reformulasi/export/soi/bb?action=getPicPenyusun';
            ...
            function updateBox_export_soi_bb_cPic_penyusunx($field, $id, $value, $rowData) {
                if ($this->input->get('action') == 'view') {
                     $return= $value; 
                }else{
                $return = '<input type="text" name="'.$field.'"  id="'.$id.'"  class="input_rows1 required" size="30" maxlength = "50"  value="'.$value.'"/>';
            ...
            function insertBox_export_soi_bb_ireformulasi_team($field, $id) {
            ...
            function updateBox_export_soi_bb_ireformulasi_team($field, $id, $value, $rowData) {
                if ($this->input->get('action') == 'view') {
                    // $return= $value; 
                     $sql = "select * 
                            from reformulasi.reformulasi_team a 
            ...
            function insertBox_export_soi_bb_iApprove($field, $id) {
                $return = '-';
                $return .= '<input type="hidden" name="isdraft" id="isdraft">';
                return $return;
            }

            function updateBox_export_soi_bb_iApprove($field, $id, $value, $rowData) {
                //print_r($rowData);
                if(($value <> 0) || (!empty($value))){
                    $sql_dtapp = 'select * 
                                ...
                                where
                                a.lDeleted = 0
                                and  a.iexport_soi_bb = "'.$rowData['iexport_soi_bb'].'"';
            ...
            function insertBox_export_soi_bb_cApprove($field, $id) {
                $return = '<input type="text" name="'.$field.'"  id="'.$id.'"  class="input_rows1 required" size="30" maxlength = "50" />';
                $return .= '<script>
                                $("#'.$id.'").keyup(function() {
                                    var len = this.value.length;
            ...
            function updateBox_export_soi_bb_cApprove($field, $id, $value, $rowData) {
                if ($this->input->get('action') == 'view') {
                     $return= $value; 
                }else{
                $return = '<input type="text" name="'.$field.'"  id="'.$id.'"  class="input_rows1 required" size="30" maxlength = "50"  value="'.$value.'"/>';
            ...
            function insertBox_export_soi_bb_dApprove($field, $id) {
                $return = '<input type="text" name="'.$field.'"  id="'.$id.'"  class="input_rows1 required" size="30" />';
                $return .= '<script>
                                $("#'.$id.'").datepicker({  changeMonth:true,
                                        changeYear:true,
            ...
            function updateBox_export_soi_bb_dApprove($field, $id, $value, $rowData) {
                if ($this->input->get('action') == 'view') {
                     $return= $value; 
                }else{
                $return = '<input type="text" name="'.$field.'"  id="'.$id.'"  class="input_rows1 required" size="30" value="'.$value.'"/>';
            ...
            function insertBox_export_soi_bb_vRemark($field, $id) {
            ...
            function updateBox_export_soi_bb_vRemark($field, $id, $value, $rowData) {
                if ($this->input->get('action') == 'view') {
                     $return= $value; 
                }else{
            ...
            function insertBox_export_soi_bb_dCreate($field, $id) {
                $return = '<input type="text" name="'.$field.'"  id="'.$id.'"  class="input_rows1 required" size="30" />';
                $return .= '<script>
                                $("#'.$id.'").datepicker({  changeMonth:true,
                                        changeYear:true,
            ...
            function updateBox_export_soi_bb_dCreate($field, $id, $value, $rowData) {
                if ($this->input->get('action') == 'view') {
                     $return= $value; 
                }else{
                $return = '<input type="text" name="'.$field.'"  id="'.$id.'"  class="input_rows1 required" size="30" value="'.$value.'"/>';
            ...
            function insertBox_export_soi_bb_cCreated($field, $id) {
                $return = '<input type="text" name="'.$field.'"  id="'.$id.'"  class="input_rows1 required" size="30" maxlength = "50" />';
                $return .= '<script>
                                $("#'.$id.'").keyup(function() {
                                    var len = this.value.length;
            ...
            function updateBox_export_soi_bb_cCreated($field, $id, $value, $rowData) {
                if ($this->input->get('action') == 'view') {
                     $return= $value; 
                }else{
                $return = '<input type="text" name="'.$field.'"  id="'.$id.'"  class="input_rows1 required" size="30" maxlength = "50"  value="'.$value.'"/>';
            ...
            function insertBox_export_soi_bb_dupdate($field, $id) {
                $return = '<input type="text" name="'.$field.'"  id="'.$id.'"  class="input_rows1 required" size="30" />';
                $return .= '<script>
                                $("#'.$id.'").datepicker({  changeMonth:true,
                                        changeYear:true,
            ...
            function updateBox_export_soi_bb_dupdate($field, $id, $value, $rowData) {
                if ($this->input->get('action') == 'view') {
                     $return= $value; 
                }else{
                $return = '<input type="text" name="'.$field.'"  id="'.$id.'"  class="input_rows1 required" size="30" value="'.$value.'"/>';
            ...
            function insertBox_export_soi_bb_cUpdate($field, $id) {
                $return = '<input type="text" name="'.$field.'"  id="'.$id.'"  class="input_rows1 required" size="30" maxlength = "50" />';
                $return .= '<script>
                                $("#'.$id.'").keyup(function() {
                                    var len = this.value.length;
            ...
            function updateBox_export_soi_bb_cUpdate($field, $id, $value, $rowData) {
                if ($this->input->get('action') == 'view') {
                     $return= $value; 
                }else{
                $return = '<input type="text" name="'.$field.'"  id="'.$id.'"  class="input_rows1 required" size="30" maxlength = "50"  value="'.$value.'"/>';
            ...
            function insertBox_export_soi_bb_lDeleted($field, $id) {
                $return = '<input type="text" name="'.$field.'"  id="'.$id.'"  class="input_rows1 required" size="30" />';
                $return .= '<script>
                                $("#'.$id.'").keyup(function() {
                                     this.value = this.value.replace(/[^0-9\.]/g,"");
            ...
            function updateBox_export_soi_bb_lDeleted($field, $id, $value, $rowData) {
                if ($this->input->get('action') == 'view') {
                     $return= $value; 
                }else{
                $return = '<input type="text" name="'.$field.'"  id="'.$id.'"  class="input_rows1 required" size="30" value="'.$value.'"/>';
            ...
            function approve_view() {
                             ...
                             function submit_ajax(form_id) {
                                return $.ajax({
                                    ...
                                    success: function(data) {
                                        ...
                                        var url = "'.base_url().'processor/reformulasi/export/soi/bb";                                
                                        if(o.status == true) {
                                            
                                            $("#alert_dialog_form").dialog("close");
                                                 $.get(url+"?action=update&id="+last_id, function(data) {
                                                 $("div#form_export_soi_bb").html(data);
                                                 $("#button_approve_export_soi_bb").hide();
                                                 $("#button_reject_export_soi_bb").hide();
                                            });
                                            
                                        }
                                            reload_grid("grid_export_soi_bb");
                                    }
                ...
                $echo .= '<form id="form_export_soi_bb_approve" action="'.base_url().'processor/reformulasi/export/soi/bb?action=approve_process" method="post">';
                $echo .= '<div style="vertical-align: top;">';
                $echo .= 'Remark : 
                        <input type="hidden" name="iexport_soi_bb" value="'.$this->input->get('iexport_soi_bb').'" />
                        <input type="hidden" name="lvl" value="'.$this->input->get('lvl').'" />
                        <input type="hidden" name="group_id" value="'.$this->input->get('group_id').'" />
                        <input type="hidden" name="modul_id" value="'.$this->input->get('modul_id').'" />
                        <textarea name="vRemark"></textarea>
                <button type="button" onclick="submit_ajax(\'form_export_soi_bb_approve\')">Confirm</button>';
            ...
            function approve_process() {
                ...
                $iexport_soi_bb = $post['iexport_soi_bb'];
                $lvl = $post['lvl'];
                $vRemark = $post['vRemark'];

                
                $data=array('iApprove'=>'2','cApprove'=>$cNip , 'dApprove'=>date('Y-m-d H:i:s'), 'vRemark'=>$vRemark);
                $this -> db -> where('iexport_soi_bb', $iexport_soi_bb);
                $updet = $this -> db -> update('reformulasi.export_soi_bb', $data);

                if ($updet) {
                    $qsql="
                        select a.*,b.*,c.cNip,c.vName 
                        from reformulasi.export_soi_bb a 
                        join reformulasi.export_req_refor b on b.iexport_req_refor=a.iexport_req_refor
                        join hrd.employee c on c.cNip=a.cPic_penyusun
                        where a.lDeleted=0
                        and b.lDeleted=0
                        and  a.iexport_soi_bb = '".$iexport_soi_bb."'
                ";
                ...
                if ($rsql['iSubmit']) {
                    ...
                    $sqlEmpAr = 'select * from reformulasi.mailsparam a where a.cVariable="export_soi_bb_approve"';
                    ...
                    $subject = 'Reformulasi - Approval SOI BB '.$rsql['vNo_soi'];
                    $content="
                        Diberitahukan bahwa telah ada Approval SOI BB dengan rincian sebagai berikut :<br><br>  
                            <table border='0' style='width: 600px;'>
                                <tr>
                                        <td style='width: 110px;'><b>PIC Penyusun</b></td><td style='width: 20px;'> : </td>
                                        <td>".$rsql['cNip'].' || '.$rsql['vName']."</td>
                                </tr>
                                <tr>
                                        <td><b>No Request Refor  </b></td><td> : </td>
                                        <td>".$rsql['vno_export_req_refor']."</td>
                                </tr> 
                                <tr>
                                        <td><b>NO SOI</b></td><td> : </td>
                                        <td>".$rsql['vNo_soi']."</td>
                                </tr> 
                                <tr>
                                        <td><b>Tanggal Berlaku  </b></td><td> : </td>
                                        <td>".$rsql['dTgl_berlaku']."</td>
                                </tr> 
                            </table> 
                ...
                $data['status']  = true;
                $data['last_id'] = $post['iexport_soi_bb'];
            ...
            /*
                ...
                function approve_view() {
                                 ...
                                 function submit_ajax(form_id) {
                                    return $.ajax({
                                        ...
                                        success: function(data) {
                                            var o = $.parseJSON(data);
                                            var last_id = o.last_id;
                                            var url = "'.base_url().'processor/home/export_soi_bb";                             
                                            if(o.status == true) { 
                                                $("#alert_dialog_form").dialog("close");
                                                     $.get(url+"?action=update&id="+last_id, function(data) {
                                                     $("div#form_export_soi_bb").html(data);
                    ...
                    $echo .= '<form id="form_export_soi_bb_approve" action="'.base_url().'processor/home/export_soi_bb?action=approve_process" method="post">';
                    $echo .= '<div style="vertical-align: top;">';
                    $echo .= 'Remark : 
                            <input type="hidden" name="iexport_soi_bb" value="'.$this->input->get('iexport_soi_bb').'" />
                            <input type="hidden" name="lvl" value="'.$this->input->get('lvl').'" />
                            ...
                            <input type="hidden" name="modul_id" value="'.$this->input->get('modul_id').'" />
                            <textarea name="vRemark"></textarea>
                    <button type="button" onclick="submit_ajax(\'form_export_soi_bb_approve\')">Approve</button>';
            ...
            */
            ...
            /*
                ...
                function reject_view() {
                    $echo = '<script type="text/javascript">
                                 function submit_ajax(form_id) {
                                    var remark = $("#export_soi_bb_remark").val();
                                    if (remark=="") {
                    ...
                    $echo .= '<form id="form_export_soi_bb_reject" action="'.base_url().'processor/home/export_soi_bb?action=reject_process" method="post">';
                    $echo .= '<div style="vertical-align: top;">';
                    $echo .= 'Remark : 
                            <input type="hidden" name="iexport_soi_bb" value="'.$this->input->get('iexport_soi_bb').'" />
                            <input type="hidden" name="lvl" value="'.$this->input->get('lvl').'" />
                            <input type="hidden" name="group_id" value="'.$this->input->get('group_id').'" />
                            <input type="hidden" name="modul_id" value="'.$this->input->get('modul_id').'" />
                            <textarea name="vRemark" id="reject_export_soi_bb_remark"></textarea>
                    <button type="button" onclick="submit_ajax(\'form_export_soi_bb_reject\')">Reject</button>';
            ...
            */
            ...
            function after_insert_processor($fields, $id, $postData) {
                ...
                $qsql="
                        ...
                        where a.lDeleted=0
                        and b.lDeleted=0
                        and  a.iexport_soi_bb = '".$id."'
                ";

                $rsql = $this->db->query($qsql)->row_array();

                if ($rsql['iType']==0) {
                    // jika tentative maka ubah jadi draft
                    $sql = "UPDATE reformulasi.export_soi_bb SET iSubmit = '0' WHERE iexport_soi_bb=$id LIMIT 1";
                    $query = $this->db->query( $sql );
                ...
                if ($rsql['iSubmit']) {
                    // kirim notifikasi message ERp
                    $sqlEmpAr = 'select * from reformulasi.mailsparam a where a.cVariable="export_soi_bb_submit "';
                    $dEmpAr =  $this->db->query($sqlEmpAr)->row_array();
                    ...
                    $subject = 'Reformulasi - New SOI BB '.$rsql['vnama'];
                    $content="
                        Diberitahukan bahwa telah ada Submitted SOI BB dengan rincian sebagai berikut :<br><br>  
                            <table border='0' style='width: 600px;'>
                                <tr>
                                        <td style='width: 110px;'><b>PIC Penyusun</b></td><td style='width: 20px;'> : </td>
                                        <td>".$rsql['cNip'].' || '.$rsql['vName']."</td>
            ...
            function after_update_processor($fields, $id, $postData) {
                ...
                if ($rsql['iSubmit']) {
                    ...
                    $subject = 'Reformulasi - New SOI BB '.$rsql['vNo_soi'];
                    $content="
                        Diberitahukan bahwa telah ada Submitted SOI BB dengan rincian sebagai berikut :<br><br>  
                            <table border='0' style='width: 600px;'>
                                <tr>
            ...
            function manipulate_insert_button($buttons) {
                ...
                $save_draft = '<button onclick="javascript:save_draft_btn_multiupload(\'export_soi_bb\', \''.base_url().'processor/reformulasi/export/soi/bb?draft=true\', this, true)" class="ui-button-text icon-save" id="button_save_draft_export_soi_bb">Save as Draft</button>';
                $save = '<button onclick="javascript:save_btn_multiupload(\'export_soi_bb\', \''.base_url().'processor/reformulasi/export/soi/bb?company_id='.$this->input->get('company_id').'&group_id='.$this->input->get('group_id').'&modul_id='.$this->input->get('modul_id').'\', this)" class="ui-button-text icon-save" id="button_save_export_soi_bb">Save &amp; Submit</button>';
                $js = $this->load->view('export/sample/js/export_soi_bb_js');
            ...
            function manipulate_update_button($buttons, $rowData) {
                ...
                $approve = '<button onclick="javascript:load_popup(\''.base_url().'processor/reformulasi/export/soi/bb?action=approve&iexport_soi_bb='.$rowData['iexport_soi_bb'].'&cNip='.$cNip.'&lvl=2&status=1&group_id='.$this->input->get('group_id').'&modul_id='.$this->input->get('modul_id').'\')" class="ui-button-text icon-save" id="button_approve_export_soi_bb">Confirm</button>';
                $update = '<button onclick="javascript:update_btn_back(\'export_soi_bb\', \''.base_url().'processor/reformulasi/export/soi/bb?company_id='.$this->input->get('company_id').'&group_id='.$this->input->get('group_id').'&modul_id='.$this->input->get('modul_id').'\', this)" class="ui-button-text icon-save" id="button_save_export_soi_bb">Update & Submit</button>';
                $updatedraft = '<button onclick="javascript:update_draft_btn(\'export_soi_bb\', \''.base_url().'processor/reformulasi/export/soi/bb?company_id='.$this->input->get('company_id').'&draft=true&group_id='.$this->input->get('group_id').'&modul_id='.$this->input->get('modul_id').'\', this, true)" class="ui-button-text icon-save" id="button_save_export_soi_bb">Update as Draft</button>';
