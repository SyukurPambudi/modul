...
class v2_exp_stress_test_kesimpulan extends MX_Controller {
    function __construct() {
		...
		$this->title 		= 'Stress Test - Kesimpulan';
		$this->url 			= 'v2_exp_stress_test_kesimpulan';
		...
		$datagrid['islist'] = array(
            ...
            'isubmit_kesimpulan_stress'             => array('label' => 'Submit Kesimpulan',    'width' => 150, 'align' => 'center', 'search' => true),
            'iapppd_kesimpulan_stress'              => array('label' => 'Approval Kesimpulan',  'width' => 150, 'align' => 'center', 'search' => false),
		);		

		$datagrid['setQuery'] = array(
			0 => array('vall' => 'export_refor_formulasi.ldeleted',         'nilai' => 0),
            1 => array('vall' => 'export_req_refor.lDeleted',           'nilai' => 0),
            2 => array('vall' => 'export_refor_formulasi.iappad_kimiawi_stress',   'nilai' => 2),
		);
    ...
    function index($action = '') {
		...
		$grid->changeFieldType('isubmit_kesimpulan_stress', 'combobox','',array(''=>'--select--',0=>'Need to be Submit', 1=>'Submited'));
		$grid->changeFieldType('iapppd_kesimpulan_stress', 'combobox','',array(''=>'--select--',0=>'Waiting for approval',1=>'Rejected', 2=>'Approved'));
    ...
    function getFormDetail(){
        ...
        function listBox_v2_exp_stress_test_kesimpulan_iapppr($value) {
        ...
        function listBox_v2_exp_stress_test_kesimpulan_cRequestor($value) {
        ...
        function insertBox_v2_exp_stress_test_kesimpulan_form_detail($field,$id){
            $get=$this->input->get();
            $post=$this->input->post();
        ...
        function updateBox_v2_exp_stress_test_kesimpulan_form_detail($field,$id,$value,$rowData){
            $get=$this->input->get();
            $post=$this->input->post();
            foreach ($get as $kget => $vget) {
                if($kget!="action"){
            ...
            function approve_view() { 
                             ...
                             function submit_ajax(form_id) {
                                return $.ajax({
                                    ...
                                    success: function(data) {
                                        ...
                                        var url = "'.base_url().'processor/reformulasi/v2_exp_stress_test_kesimpulan";                             
                                        if(o.status == true) { 
                                            $("#alert_dialog_form").dialog("close");
                                                 $.get(url+"?action=update&id="+last_id+"&foreign_key=0&company_id=3&group_id="+group_id+"&modul_id="+modul_id, function(data) {
                                                 $("div#form_v2_exp_stress_test_kesimpulan").html(data);
                                                 
                                            });
                                            
                                        }
                                            reload_grid("grid_v2_exp_stress_test_kesimpulan");
                                    }
                ...
                $echo .= '<form id="form_v2_exp_stress_test_kesimpulan_approve" action="'.base_url().'processor/reformulasi/v2_exp_stress_test_kesimpulan?action=approve_process" method="post">';
                ...
                <button type="button" onclick="submit_ajax(\'form_v2_exp_stress_test_kesimpulan_approve\')">Approve</button>';
            ...
            function confirm_view() { 
                ...
                $echo .= '<form id="form_v2_exp_stress_test_kesimpulan_confirm" action="'.base_url().'processor/reformulasi/v2_exp_stress_test_kesimpulan?action=confirm_process" method="post">';
                $echo .= '<div style="vertical-align: top;">';
                $echo .= 'Remark : 
                        <input type="hidden" name="'.$this->main_table_pk.'" value="'.$this->input->get($this->main_table_pk).'" />
                        <input type="hidden" name="modul_id" value="'.$this->input->get('modul_id').'" />
                        ...
                        <textarea name="vRemark"></textarea>
                <button type="button" onclick="submit_ajax(\'form_v2_exp_stress_test_kesimpulan_confirm\')">Confirm</button>';
            ...
            function reject_view() {
                $echo = '<script type="text/javascript">
                             function submit_ajax(form_id) {
                                var remark = $("#v2_exp_stress_test_kesimpulan_remark").val();
                                if (remark=="") {
                ...
                $echo .= '<form id="form_v2_exp_stress_test_kesimpulan_reject" action="'.base_url().'processor/reformulasi/v2_exp_stress_test_kesimpulan?action=reject_process" method="post">';
                $echo .= '<div style="vertical-align: top;">';
                $echo .= 'Remark : 
                        <input type="hidden" name="'.$this->main_table_pk.'" value="'.$this->input->get($this->main_table_pk).'" />
                        <input type="hidden" name="modul_id" value="'.$this->input->get('modul_id').'" />
                        ...
                        <textarea name="vRemark" id="reject_v2_exp_stress_test_kesimpulan_remark"></textarea>
                <button type="button" onclick="submit_ajax(\'form_v2_exp_stress_test_kesimpulan_reject\')">Reject</button>';
    ...
    function reject_process() {
        ...
        $resetFomula['isubmit_fisik_stress']       = 0;
        $resetFomula['dsubmit_fisik_stress']       = null;
        $resetFomula['csubmit_fisik_stress']       = null;
        $resetFomula['isubmit_kimiawi_stress']     = 0;
        $resetFomula['dsubmit_kimiawi_stress']     = null;
        $resetFomula['csubmit_kimiawi_stress']     = null;
        $resetFomula['isubmit_kesimpulan_stress']  = 0;
        $resetFomula['dsubmit_kesimpulan_stress']  = null;
        $resetFomula['csubmit_kesimpulan_stress']  = null;
    ...
    function manipulate_update_button($buttons, $rowData) { 
        ...
        $iframe 		= '<iframe name="v2_exp_stress_test_kesimpulan_frame" id="v2_exp_stress_test_kesimpulan_frame" height="0" width="0"></iframe>';

        $dataMain       = $this->db->get_where($this->maintable, array($this->main_table_pk => $peka))->row_array();
        $canSubmit      = ( $dataMain['isubmit_fisik_stress'] == 1 && $dataMain['isubmit_kimiawi_stress'] && $dataMain['iappad_kimiawi_stress'] == 2 ) ? true : false;
        ...
        if ($this->input->get('action') == 'view') {
            unset($buttons['update']);
                ...
                if($isOpenEditing){
                    $update_draft = '<button onclick="javascript:update_draft_btn(\'v2_exp_stress_test_kesimpulan\', \' '.base_url().'processor/reformulasi/v2_exp_stress_test_kesimpulan?draft=true \',this,true )"  id="button_update_draft_v2_exp_stress_test_kesimpulan"  class="ui-button-text icon-save" >Update open Editing</button>';
                    ...
                    foreach ($activities as $act) {
                        $update_draft = '<button onclick="javascript:update_draft_btn(\'v2_exp_stress_test_kesimpulan\', \' '.base_url().'processor/reformulasi/v2_exp_stress_test_kesimpulan?draft=true&modul_id='.$this->input->get('modul_id').'&iM_modul_activity='.$act['iM_modul_activity'].' \',this,true )"  id="button_update_draft_v2_exp_stress_test_kesimpulan"  class="ui-button-text icon-save" >Update as Draft</button>';
                        $update = '<button onclick="javascript:update_btn_back(\'v2_exp_stress_test_kesimpulan\', \' '.base_url().'processor/reformulasi/v2_exp_stress_test_kesimpulan?company_id='.$this->input->get('company_id').'&modul_id='.$this->input->get('modul_id').'&iM_modul_activity='.$act['iM_modul_activity'].'&group_id='.$this->input->get('group_id').'modul_id='.$this->input->get('modul_id').' \',this,false )"  id="button_update_submit_v2_exp_stress_test_kesimpulan"  class="ui-button-text icon-save" >Update &amp; Submit</button>';

                        $approve = '<button onclick="javascript:load_popup(\' '.base_url().'processor/reformulasi/v2_exp_stress_test_kesimpulan?action=approve&iM_modul_activity='.$act['iM_modul_activity'].'&iM_activity='.$act['iM_activity'].'&'.$this->main_table_pk.'='.$peka.'&iupb_id='.$iupb_id.'&company_id='.$this->input->get('company_id').'&group_id='.$this->input->get('group_id').'&modul_id='.$this->input->get('modul_id').' \')"  id="button_approve_v2_exp_stress_test_kesimpulan"  class="ui-button-text icon-save" >Approve</button>';
                        $reject = '<button onclick="javascript:load_popup(\' '.base_url().'processor/reformulasi/v2_exp_stress_test_kesimpulan?action=reject&iM_modul_activity='.$act['iM_modul_activity'].'&iM_activity='.$act['iM_activity'].'&'.$this->main_table_pk.'='.$peka.'&iupb_id='.$iupb_id.'&company_id='.$this->input->get('company_id').'&group_id='.$this->input->get('group_id').'&modul_id='.$this->input->get('modul_id').' \' )"  id="button_reject_v2_exp_stress_test_kesimpulan"  class="ui-button-text icon-save" >Reject</button>';

                        $confirm = '<button onclick="javascript:load_popup(\' '.base_url().'processor/reformulasi/v2_exp_stress_test_kesimpulan?action=confirm&iM_modul_activity='.$act['iM_modul_activity'].'&iM_activity='.$act['iM_activity'].'&'.$this->main_table_pk.'='.$peka.'&iupb_id='.$iupb_id.'&company_id='.$this->input->get('company_id').'&group_id='.$this->input->get('group_id').'&modul_id='.$this->input->get('modul_id').' \')"  id="button_approve_v2_exp_stress_test_kesimpulan"  class="ui-button-text icon-save" >Confirm</button>';
    ...
    private function canSubmitKimiawi ($id){
        ...
        if (!empty($trial_submit)){
            $submit_fisik_stress = $trial_submit['isubmit_fisik_stress'];
            if (intval($submit_fisik_stress) == 0){
                $status     = false;
                $message    = 'Skala Trial - Uji Fisik Masih Belum Di Submit';
            } else {
                $filter_submit  = array(
                    'iexport_formulasi'   => $id,
                    'isubmit_fisik_stress'         => 1,
                    'isubmit_kimiawi_stress'       => 1,
                    'isubmit_kesimpulan_stress'    => 0,
                    'ldeleted'              => 0
                );
