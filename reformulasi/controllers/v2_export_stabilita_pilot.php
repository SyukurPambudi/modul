...
class v2_export_stabilita_pilot extends MX_Controller {
    function __construct() {
		...
		$this->title 		= 'STABILITA PILOT';
		$this->url 			= 'v2_export_stabilita_pilot';
		$this->urlpath 		= 'reformulasi/'.str_replace("_","/", $this->url);


		$this->maintable 	= 'reformulasi.export_soi_fg';	
		$this->main_table 	= $this->maintable;	
		$this->main_table_pk= 'export_stabilita_pilot';	
		...
		$datagrid['islist'] = array(
            ...
            'cBulan'	=> array('label' => 'Bulan','width' => 100,	'align' => 'center', 'search' => true),
            'ireal'	=> array('label' => 'Real Time Test','width' => 100,	'align' => 'center', 'search' => true),
            'dstablabreal1' => array('label' => 'Tgl Test Lab Real Time',	'width' => 100,	'align' => 'center', 'search' => true),
            'dstablabreal2' => array('label' => 'Tgl Test Lab Real Time',	'width' => 100,	'align' => 'center', 'search' => true),
            'iacc'	=> array('label' => 'Accelerated Test','width' => 100,	'align' => 'center', 'search' => true),
            'dstablabacc1' => array('label' => 'Tgl Test Lab Accelerated',	'width' => 100,	'align' => 'center', 'search' => true),
            'dstablabacc2' => array('label' => 'Tgl Test Lab Accelerated',	'width' => 100,	'align' => 'center', 'search' => true),
            'iinter'	=> array('label' => 'Intermediate Test','width' => 100,	'align' => 'center', 'search' => true),
            'dstablabinter1' => array('label' => 'Tgl Test Lab Intermediate',	'width' => 100,	'align' => 'center', 'search' => true),
            'dstablabinter2' => array('label' => 'Tgl Test Lab Intermediate',	'width' => 100,	'align' => 'center', 'search' => true),

			'cPicstabpilot' => array('label' => 'PIC Penyusun','width' => 110,	'align' => 'center', 'search' => true),			
			'iSubmit'=> array('label' => 'Status Submit','width' => 150,	'align' => 'left',   'search' => false),
			'iapprove_pd' => array('label' => 'Approval','width' => 110,	'align' => 'center', 'search' => false),
		);		

		$datagrid['setQuery']=array(
			0 => array('vall' => 'export_soi_fg.lDeleted', 'nilai' => 0),
			
		);

		$datagrid['jointableinner'] = array(
			0 => $arrayName = array( 'reformulasi.export_req_refor' => 'export_soi_fg.iexport_req_refor =export_req_refor.iexport_req_refor '),
			1 => array('purchase.pamsh' => 'pamsh.c_panumb = export_soi_fg.cpanumb'),
		...
		$datagrid['shortBy']=array("export_soi_fg.export_stabilita_pilot" => "DESC");
...
/* 		if (!empty($dataField)){
    ...
    function get_data_prev(){
        ...
        function listBox_v2_export_stabilita_pilot_iapppr($value) {
...
/*aslinya*/
   /*         if ( $getLastactivity == 0 ) { 
        ...
        function insertBox_v2_export_stabilita_pilot_form_detail($field,$id){
            $get=$this->input->get();
            $post=$this->input->post();
        ...
        function updateBox_v2_export_stabilita_pilot_form_detail($field,$id,$value,$rowData){
            $get=$this->input->get();
            $post=$this->input->post();
            foreach ($get as $kget => $vget) {
                if($kget!="action"){
            ...
            function approve_view() { 
                             ...
                             function submit_ajax(form_id) {
                                return $.ajax({
                                    ...
                                    success: function(data) {
                                        ...
                                        var url = "'.base_url().'processor/reformulasi/v2_export_stabilita_pilot";                             
                                        if(o.status == true) { 
                                            $("#alert_dialog_form").dialog("close");
                                                 $.get(url+"?action=update&id="+last_id+"&foreign_key=0&company_id=3&group_id="+group_id+"&modul_id="+modul_id, function(data) {
                                                 $("div#form_v2_export_stabilita_pilot").html(data);
                                                 
                                            });
                                            
                                        }
                                            reload_grid("grid_v2_export_stabilita_pilot");
                                    }
                ...
                $echo .= '<form id="form_v2_export_stabilita_pilot_approve" action="'.base_url().'processor/reformulasi/v2_export_stabilita_pilot?action=approve_process" method="post">';
                ...
                <button type="button" onclick="submit_ajax(\'form_v2_export_stabilita_pilot_approve\')">Approve</button>';
           ...
           function approve_process() {
            ...
            function confirm_view() { 
                ...
                $echo .= '<form id="form_v2_export_stabilita_pilot_confirm" action="'.base_url().'processor/reformulasi/v2_export_stabilita_pilot?action=confirm_process" method="post">';
                $echo .= '<div style="vertical-align: top;">';
                $echo .= 'Remark : 
                        <input type="hidden" name="'.$this->main_table_pk.'" value="'.$this->input->get($this->main_table_pk).'" />
                        <input type="hidden" name="modul_id" value="'.$this->input->get('modul_id').'" />
                        ...
                        <textarea name="vRemark"></textarea>
                <button type="button" onclick="submit_ajax(\'form_v2_export_stabilita_pilot_confirm\')">Confirm</button>';
            ...
            function reject_view() {
                $echo = '<script type="text/javascript">
                             function submit_ajax(form_id) {
                                var remark = $("#v2_export_stabilita_pilot_remark").val();
                                if (remark=="") {
                ...
                $echo .= '<form id="form_v2_export_stabilita_pilot_reject" action="'.base_url().'processor/reformulasi/v2_export_stabilita_pilot?action=reject_process" method="post">';
                $echo .= '<div style="vertical-align: top;">';
                $echo .= 'Remark : 
                        <input type="hidden" name="'.$this->main_table_pk.'" value="'.$this->input->get($this->main_table_pk).'" />
                        <input type="hidden" name="modul_id" value="'.$this->input->get('modul_id').'" />
                        ...
                        <textarea name="vRemark" id="reject_v2_export_stabilita_pilot_remark"></textarea>
                <button type="button" onclick="submit_ajax(\'form_v2_export_stabilita_pilot_reject\')">Reject</button>';
    ...
    function after_insert_processor($fields, $id, $postData) { 
        ...
        if($postData['iSubmit']==1){
            $sql = "SELECT em.`vName`, sa.`dujebe`, sa.`dselesaiujebe`, sa.`iTeamAd`,
                e.`vno_export_req_refor` FROM reformulasi.`export_soi_fg` sa
                JOIN reformulasi.`export_req_refor` e ON sa.`iexport_req_refor` = e.`iexport_req_refor`
                JOIN hrd.`employee` em ON em.`cNip` = sa.`cpic_lapvalpro`
                WHERE sa.`lDeleted` = 0 AND sa.`export_stabilita_pilot` =  ".$id;
            ...
            Diberitahukan bahwa telah ada Submited Modul Protokol Valpro,
                ...
                <table border='0' style='width: 600px;'>
                    ...
                    <tr>
                            ...
                            <td>".$mailDet['dujebe']."</td>
                    </tr>
                    <tr>
                            <td><b>Tanggal Selesai Protokol Valpro  </b></td><td> : </td>
                            <td>".$mailDet['dselesaiujebe']."</td>
                    </tr>
                </table>
            <br/> <br/>
    ...
    function after_update_processor($fields, $id, $postData) {
       $post = $this->input->post();
        ...
        if($postData['isubmit']==1){
            ...
            $sql = "SELECT em.`vName`, sa.`dujebe`, sa.`dselesaiujebe`,sa.`iTeamAd`,   e.`vno_export_req_refor` FROM reformulasi.`export_soi_fg` sa
                JOIN reformulasi.`export_req_refor` e ON sa.`iexport_req_refor` = e.`iexport_req_refor`
                JOIN hrd.`employee` em ON em.`cNip` = sa.`cpic_lapvalpro`
                WHERE sa.`lDeleted` = 0 AND sa.`export_stabilita_pilot` =  ".$id;
            $mailDet = $this->db->query($sql)->row_array();
    ...
    function manipulate_insert_buttonx($buttons){
		unset($buttons['save']);
        $js = $this->load->view('export/js/export_soi_fg_js');
        $save_draft = '<button onclick="javascript:save_draft_btn_multiupload(\'export_soi_fg\', \''.base_url().'processor/reformulasi/export/protokol/valpro?draft=true\', this, true)" class="ui-button-text icon-save" id="button_save_draft_export_soi_fg_js">Save as Draft</button>';
        $save = '<button onclick="javascript:save_btn_multiupload(\'export_soi_fg\', \''.base_url().'processor/reformulasi/export/protokol/valpro?company_id='.$this->input->get('company_id').'&group_id='.$this->input->get('group_id').'&modul_id='.$this->input->get('modul_id').'\', this)" class="ui-button-text icon-save" id="button_save_export_soi_fg_js">Save &amp Submit</button>';
 ...
 function manipulate_update_button($buttons, $rowData) { 
        ...
        $iframe = '<iframe name="v2_export_stabilita_pilot" id="v2_export_stabilita_pilot" height="0" width="0"></iframe>';
        ...
        if ($this->input->get('action') == 'view') {
            unset($buttons['update']);
                ...
                if($isOpenEditing){
                    $update_draft = '<button onclick="javascript:update_draft_btn(\'v2_export_stabilita_pilot\', \' '.base_url().'processor/reformulasi/v2_export_stabilita_pilot?draft=true \',this,true )"  id="button_update_draft_v2_export_stabilita_pilot"  class="ui-button-text icon-save" >Update open Editing</button>';
                    ...
                    foreach ($activities as $act) {
                        $update_draft = '<button onclick="javascript:update_draft_btn(\'v2_export_stabilita_pilot\', \' '.base_url().'processor/reformulasi/v2_export_stabilita_pilot?draft=true&modul_id='.$this->input->get('modul_id').'&iM_modul_activity='.$act['iM_modul_activity'].' \',this,true )"  id="button_update_draft_v2_export_stabilita_pilot"  class="ui-button-text icon-save" >Update as Draft</button>';
                        $update = '<button onclick="javascript:update_btn_back(\'v2_export_stabilita_pilot\', \' '.base_url().'processor/reformulasi/v2_export_stabilita_pilot?company_id='.$this->input->get('company_id').'&modul_id='.$this->input->get('modul_id').'&iM_modul_activity='.$act['iM_modul_activity'].'&group_id='.$this->input->get('group_id').'modul_id='.$this->input->get('modul_id').' \',this,false )"  id="button_update_submit_v2_export_stabilita_pilot"  class="ui-button-text icon-save" >Update &amp; Submit</button>';

                        $approve = '<button onclick="javascript:load_popup(\' '.base_url().'processor/reformulasi/v2_export_stabilita_pilot?action=approve&iM_modul_activity='.$act['iM_modul_activity'].'&iM_activity='.$act['iM_activity'].'&'.$this->main_table_pk.'='.$peka.'&iupb_id='.$iupb_id.'&company_id='.$this->input->get('company_id').'&group_id='.$this->input->get('group_id').'&modul_id='.$this->input->get('modul_id').' \')"  id="button_approve_v2_export_stabilita_pilot"  class="ui-button-text icon-save" >Approve</button>';
                        $reject = '<button onclick="javascript:load_popup(\' '.base_url().'processor/reformulasi/v2_export_stabilita_pilot?action=reject&iM_modul_activity='.$act['iM_modul_activity'].'&iM_activity='.$act['iM_activity'].'&'.$this->main_table_pk.'='.$peka.'&iupb_id='.$iupb_id.'&company_id='.$this->input->get('company_id').'&group_id='.$this->input->get('group_id').'&modul_id='.$this->input->get('modul_id').' \' )"  id="button_reject_v2_export_stabilita_pilot"  class="ui-button-text icon-save" >Reject</button>';

                        $confirm = '<button onclick="javascript:load_popup(\' '.base_url().'processor/reformulasi/v2_export_stabilita_pilot?action=confirm&iM_modul_activity='.$act['iM_modul_activity'].'&iM_activity='.$act['iM_activity'].'&'.$this->main_table_pk.'='.$peka.'&iupb_id='.$iupb_id.'&company_id='.$this->input->get('company_id').'&group_id='.$this->input->get('group_id').'&modul_id='.$this->input->get('modul_id').' \')"  id="button_approve_v2_export_stabilita_pilot"  class="ui-button-text icon-save" >Confirm</button>';
    ...
    function manipulate_update_buttonx($buttons, $rowData) { 
        if ($this->input->get('action') == 'view') {
            ...
            $js = $this->load->view('export/js/export_soi_fg_js');
            $update = '<button onclick="javascript:update_btn_back(\'export_soi_fg\', \''.base_url().'processor/reformulasi/export/protokol/valpro?company_id='.$this->input->get('company_id').'&group_id='.$this->input->get('group_id').'&modul_id='.$this->input->get('modul_id').'\', this)" class="ui-button-text icon-save" id="button_update_export_soi_fg">Update & Submit</button>';
            $updatedraft = '<button onclick="javascript:update_draft_btn(\'export_soi_fg\', \''.base_url().'processor/reformulasi/export/protokol/valpro?company_id='.$this->input->get('company_id').'&draft=true&group_id='.$this->input->get('group_id').'&modul_id='.$this->input->get('modul_id').'\', this, true)" class="ui-button-text icon-save" id="button_save_export_soi_fg">Update as Draft</button>';

           $approve = '<button onclick="javascript:load_popup(\''.base_url().'processor/reformulasi/export/protokol/valpro?action=approve&export_stabilita_pilot='.$rowData['export_stabilita_pilot'].'&cNip='.$cNip.'&lvl=1&status=1&group_id='.$this->input->get('group_id').'&modul_id='.$this->input->get('modul_id').'\')" class="ui-button-text icon-save" id="button_approve_export_soi_fg">Confirm</button>';
            $reject = '<button onclick="javascript:load_popup(\''.base_url().'processor/reformulasi/export/protokol/valpro?action=reject&export_stabilita_pilot='.$rowData['export_stabilita_pilot'].'&cNip='.$cNip.'&lvl=1&status=2&group_id='.$this->input->get('group_id').'&modul_id='.$this->input->get('modul_id').'\')" class="ui-button-text icon-save" id="button_reject_export_soi_fg">Reject</button>';
            ...
            if($rowData['isubmit']==0){
                ...
                if($rowData['iapprove_pd']==0 or empty($rowData['iapprove_pd'])){
