...
class v2_exp_bk_primer extends MX_Controller {
    function __construct() {
        ...
        $this->title        = 'Request Reformulasi';
        $this->url          = 'v2_exp_bk_primer';
        $this->urlpath      = 'reformulasi/'.str_replace("_","/", $this->url);

        $this->maintable    = 'reformulasi.export_bk_primer';   
        $this->main_table   = $this->maintable; 
        $this->main_table_pk= 'iexport_bk_primer_id';  
        
        $datagrid['islist'] = array(
           'vtitle'     => array('label' => 'Title',    'width' => 100, 'align' => 'center', 'search' => true),
            'vNo_bk_primer'     => array('label' => 'No BK Sekunder',    'width' => 100, 'align' => 'center', 'search' => true),
        ...
        $datagrid['setQuery']=array(
            0 => array('vall' => 'export_bk_primer.lDeleted', 'nilai' => 0),
            
        );

        $datagrid['jointableinner'] = array(
            0 => array('dossier.dossier_upd' => 'dossier_upd.idossier_upd_id = export_bk_primer.iDossier_upd_id'),
            1 => array('export_req_refor' => 'export_req_refor.iexport_req_refor = export_bk_primer.iexport_req_refor'),
        );

        $datagrid['shortBy']=array("export_bk_primer.iexport_bk_primer_id" => "DESC");
    ...
    function index($action = '') {
        ...
        $grid->changeFieldType('iapproved', 'combobox','',array(''=>'--select--',0=>'Waiting for approval',1=>'Rejected', 2=>'Approved'));
    ...
    function get_data_prev(){
        ...
        function listBox_v2_exp_bk_primer_iapppr($value) {
        ...
        function insertBox_v2_exp_bk_primer_form_detail($field,$id){
            $get=$this->input->get();
            $post=$this->input->post();
        ...
        function updateBox_v2_exp_bk_primer_form_detail($field,$id,$value,$rowData){
            $get=$this->input->get(); 
            $post=$this->input->post(); 
            foreach ($get as $kget => $vget) {
                if($kget!="action"){
            ...
            function approve_view() { 
                             ...
                             function submit_ajax(form_id) {
                                return $.ajax({
                                    ...
                                    success: function(data) {
                                        ...
                                        var url = "'.base_url().'processor/reformulasi/v2_exp_bk_primer";                             
                                        if(o.status == true) { 
                                            $("#alert_dialog_form").dialog("close");
                                                 $.get(url+"?action=update&id="+last_id+"&foreign_key=0&company_id=3&group_id="+group_id+"&modul_id="+modul_id, function(data) {
                                                 $("div#form_v2_exp_bk_primer").html(data);
                                                 
                                            });
                                            
                                        }
                                            reload_grid("grid_v2_exp_bk_primer");
                                    }
                ...
                $echo .= '<form id="form_v2_exp_bk_primer_approve" action="'.base_url().'processor/reformulasi/v2_exp_bk_primer?action=approve_process" method="post">';
                ...
                <button type="button" onclick="submit_ajax(\'form_v2_exp_bk_primer_approve\')">Approve</button>';
           ...
           /* function approve_process() {
                ...
                $this->lib_sub_core->InsertActivityModule($this->ViewUPB($pk),$modul_id,$pk,$activity['iM_activity'],$activity['iSort'],$vRemark,2);
                        ...
                        $sql = "SELECT e.`vName`, e.`cUpper`, r.`vno_export_bk_primer`, d.`vUpd_no`, 
                            d.`vNama_usulan`, r.`iTeamPD`,  rt.`vnip` 
                            FROM reformulasi.`export_bk_primer` r
                            ...
                            WHERE r.`lDeleted` = 0 AND d.`lDeleted` = 0 AND r.`iexport_bk_primer_id` = ".$post['iexport_bk_primer_id']; 

                        $mailDet = $this->db->query($sql)->row_array();  
                        
                        $to = $mailDet['vnip'];
                        $cc = $this->user->gNIP.','.$mailDet['cInisiator_export'];
                        $subject="Approved Request Reformulasi: No Request ".$mailDet['vno_export_bk_primer']; 
                        $content="
                        Diberitahukan bahwa telah ada Approved Request Reformulasi Export, 
                            ...
                            <table border='0' style='width: 600px;'>
                                <tr>
                                        ...
                                        <td>".$mailDet['vno_export_bk_primer']."</td>
                                </tr>
                                <tr>
            ...
            function confirm_view() { 
                ...
                $echo .= '<form id="form_v2_exp_bk_primer_confirm" action="'.base_url().'processor/reformulasi/v2_exp_bk_primer?action=confirm_process" method="post">';
                $echo .= '<div style="vertical-align: top;">';
                $echo .= 'Remark : 
                        <input type="hidden" name="'.$this->main_table_pk.'" value="'.$this->input->get($this->main_table_pk).'" />
                        <input type="hidden" name="modul_id" value="'.$this->input->get('modul_id').'" />
                        ...
                        <textarea name="vRemark"></textarea>
                <button type="button" onclick="submit_ajax(\'form_v2_exp_bk_primer_confirm\')">Confirm</button>';
            ...
            function reject_view() {
                $echo = '<script type="text/javascript">
                             function submit_ajax(form_id) {
                                var remark = $("#v2_exp_bk_primer_remark").val();
                                if (remark=="") {
                ...
                $echo .= '<form id="form_v2_exp_bk_primer_reject" action="'.base_url().'processor/reformulasi/v2_exp_bk_primer?action=reject_process" method="post">';
                $echo .= '<div style="vertical-align: top;">';
                $echo .= 'Remark : 
                        <input type="hidden" name="'.$this->main_table_pk.'" value="'.$this->input->get($this->main_table_pk).'" />
                        <input type="hidden" name="modul_id" value="'.$this->input->get('modul_id').'" />
                        ...
                        <textarea name="vRemark" id="reject_v2_exp_bk_primer_remark"></textarea>
                <button type="button" onclick="submit_ajax(\'form_v2_exp_bk_primer_reject\')">Reject</button>';
    ...
    function after_insert_processor($fields, $id, $postData) { 
        ...
        $sql = "UPDATE reformulasi.export_bk_primer SET vno_export_bk_primer = '".$nomor."' WHERE iexport_bk_primer_id=$id LIMIT 1";
        $query = $this->db->query( $sql );

        $insD['iexport_bk_primer_id'] = $id;
        ...
        if ($postData['iSubmit'] == 1){
            ...
            $sql = "SELECT e.`vName`, e.`cUpper`, d.`vUpd_no`, r.`vno_export_bk_primer`,  d.`vNama_usulan` FROM reformulasi.`export_bk_primer` r
                JOIN hrd.`employee` e ON e.`cNip` = r.`cInisiator_export`
                JOIN dossier.`dossier_upd` d ON d.`idossier_upd_id` = r.`idossier_upd_id` 
                WHERE r.`lDeleted` = 0 AND d.`lDeleted` = 0 AND r.`iexport_bk_primer_id` = ".$id;
               // print_r($sql); 
            $mailDet = $this->db->query($sql)->row_array();  
            $to = $mailDet['cUpper'];
            $cc = $this->user->gNIP;
            $subject="Submited Request Reformulasi: No Request ".$mailDet['vno_export_bk_primer']; 
            $content="
    ...
    function after_update_processor($fields, $id, $postData) {
        ...
        $sql = "SELECT e.iexport_bk_primer_id 
                FROM reformulasi.`export_bk_primer` e 
                WHERE e.lDeleted = 0 
                AND e.iexport_bk_primer_id = '".$id."' 
                LIMIT 1";
                //print_r($sql); exit();
                $dtsample = $this->db->query($sql)->row_array(); 
                if(empty($dtsample['export_bk_primer'])){
                    $insD['iexport_bk_primer_id'] = $id;
                    $insD['vtitle'] = date('Y-m-d H:i:s');
                    $insD['vKode_kemas'] = 1;
                    $insD['iRev_bk'] ;
                    $this->db->insert('reformulasi.export_bk_primer', $insD);
                    $id2 = $this->db->insert_id(); 

                    $nomor2 = "RS".str_pad($id2, 7, "0", STR_PAD_LEFT);
                    $sql = "UPDATE reformulasi.export_bk_primer 
    ...
    function manipulate_insert_buttonx($buttons) { 
        ...
        $iframe = '<iframe name="v2_exp_bk_primer_frame" id="v2_exp_bk_primer_frame" height="0" width="0"></iframe>';
        ...
        $save_draft = '<button onclick="javascript:save_draft_btn_multiupload(\'v2_exp_bk_primer\', \' '.base_url().'processor/reformulasi/v2_exp_bk_primer?draft=true&modul_id='.$this->input->get('modul_id').'&iM_modul_activity='.$act['iM_modul_activity'].' \',this,true )"  id="button_save_draft_v2_exp_bk_primer"  class="ui-button-text icon-save" >Save as Draft</button>';
        $save = '<button onclick="javascript:save_btn_multiupload(\'v2_exp_bk_primer\', \' '.base_url().'processor/reformulasi/v2_exp_bk_primer?company_id='.$this->input->get('company_id').'&modul_id='.$this->input->get('modul_id').'&iM_modul_activity='.$act['iM_modul_activity'].'&group_id='.$this->input->get('group_id').'modul_id='.$this->input->get('modul_id').' \',this,false )"  id="button_save_submit_v2_exp_bk_primer"  class="ui-button-text icon-save" >Save &amp; Submit</button>';
    ...
    function manipulate_update_button($buttons, $rowData) { 
        ...
        if ($this->input->get('action') == 'view') {
            unset($buttons['update']);
                ...
                if($isOpenEditing){
                    $update_draft = '<button onclick="javascript:update_draft_btn(\'v2_exp_bk_primer\', \' '.base_url().'processor/reformulasi/v2_exp_bk_primer?draft=true \',this,true )"  id="button_update_draft_v2_exp_bk_primer"  class="ui-button-text icon-save" >Update open Editing</button>';
                    ...
                    foreach ($activities as $act) {
                        $update_draft = '<button onclick="javascript:update_draft_btn(\'v2_exp_bk_primer\', \' '.base_url().'processor/reformulasi/v2_exp_bk_primer?draft=true&modul_id='.$this->input->get('modul_id').'&iM_modul_activity='.$act['iM_modul_activity'].' \',this,true )"  id="button_update_draft_v2_exp_bk_primer"  class="ui-button-text icon-save" >Update as Draft</button>';
                        $update = '<button onclick="javascript:update_btn_back(\'v2_exp_bk_primer\', \' '.base_url().'processor/reformulasi/v2_exp_bk_primer?company_id='.$this->input->get('company_id').'&modul_id='.$this->input->get('modul_id').'&iM_modul_activity='.$act['iM_modul_activity'].'&group_id='.$this->input->get('group_id').'modul_id='.$this->input->get('modul_id').' \',this,false )"  id="button_update_submit_v2_exp_bk_primer"  class="ui-button-text icon-save" >Update &amp; Submit</button>';

                        $approve = '<button onclick="javascript:load_popup(\' '.base_url().'processor/reformulasi/v2_exp_bk_primer?action=approve&iM_modul_activity='.$act['iM_modul_activity'].'&iM_activity='.$act['iM_activity'].'&'.$this->main_table_pk.'='.$peka.'&iupb_id='.$iupb_id.'&company_id='.$this->input->get('company_id').'&group_id='.$this->input->get('group_id').'&modul_id='.$this->input->get('modul_id').' \')"  id="button_approve_v2_exp_bk_primer"  class="ui-button-text icon-save" >Approve</button>';
                        $reject = '<button onclick="javascript:load_popup(\' '.base_url().'processor/reformulasi/v2_exp_bk_primer?action=reject&iM_modul_activity='.$act['iM_modul_activity'].'&iM_activity='.$act['iM_activity'].'&'.$this->main_table_pk.'='.$peka.'&iupb_id='.$iupb_id.'&company_id='.$this->input->get('company_id').'&group_id='.$this->input->get('group_id').'&modul_id='.$this->input->get('modul_id').' \' )"  id="button_reject_v2_exp_bk_primer"  class="ui-button-text icon-save" >Reject</button>';

                        $confirm = '<button onclick="javascript:load_popup(\' '.base_url().'processor/reformulasi/v2_exp_bk_primer?action=confirm&iM_modul_activity='.$act['iM_modul_activity'].'&iM_activity='.$act['iM_activity'].'&'.$this->main_table_pk.'='.$peka.'&iupb_id='.$iupb_id.'&company_id='.$this->input->get('company_id').'&group_id='.$this->input->get('group_id').'&modul_id='.$this->input->get('modul_id').' \')"  id="button_approve_v2_exp_bk_primer"  class="ui-button-text icon-save" >Confirm</button>';
