...
class v3_export_stress_fisik extends MX_Controller {
    function __construct() {
        ...
        $this->url = 'v3_export_stress_fisik';
        ...
        $this->pathfile = 'files/reformulasi/export/stress_test';
    ...
    function index($action = '') {
        ...
        $grid->addList('vno_formula','dmulai_stress_test','dselesai_stress_test','cpic_stress_test','isubmit_fisik_stress','dsubmit_fisik_stress'); 
        $grid->setSortBy('iexport_formulasi_detail');
        $grid->setSortOrder('DESC');  

        $grid->addFields('vno_formula','dmulai_stress_test','dselesai_stress_test','cpic_stress_test','formulasi_uji_fisik', 'ldeleted'); 

        $grid->setWidth('vno_formula', '100');
        $grid->setAlign('vno_formula', 'left');
        $grid->setLabel('vno_formula','No. Formulasi');
    
        $grid->setWidth('dmulai_stress_test', '100');
        $grid->setAlign('dmulai_stress_test', 'left');
        $grid->setLabel('dmulai_stress_test','Tgl Mulai Orientasi Stress Test ');

        $grid->setWidth('dselesai_stress_test', '100');
        $grid->setAlign('dselesai_stress_test', 'left');
        $grid->setLabel('dselesai_stress_test','Tgl Selesai Orientasi Stress Test ');

        $grid->setWidth('cpic_stress_test', '100');
        $grid->setAlign('cpic_stress_test', 'left');
        $grid->setLabel('cpic_stress_test','PIC Stress Test ');
        ...
        $grid->setWidth('isubmit_fisik_stress', '100');
        $grid->setAlign('isubmit_fisik_stress', 'left');
        $grid->setLabel('isubmit_fisik_stress','Status');

        $grid->setWidth('dsubmit_fisik_stress', '100');
        $grid->setAlign('dsubmit_fisik_stress', 'left');
        $grid->setLabel('dsubmit_fisik_stress','Tgl Submit');
        ...
        $grid->changeFieldType('isubmit_fisik_stress', 'combobox','',array('' => 'Pilih', 0 => 'Need To Be Submit', 1 => 'Submitted'));
        $grid->changeFieldType('ldeleted', 'combobox','',array( 0 => 'Tidaik', 1 => 'Ya'));

        $grid->setSearch('vno_formula','isubmit_fisik_stress');
        
        $grid->setRequired('vno_formula','dmulai_stress_test','dselesai_stress_test','cpic_stress_test'); 
    ...
    public function manipulate_grid_button($button) {  
        ...
        $submitFisik= (!empty($cekSubmit))?$cekSubmit['isubmit_fisik_stress']:0;
    ...
    function listBox_Action($row, $actions) { 
        ...
        $sqlCek     = 'SELECT t.isubmit_fisik_stress FROM reformulasi.export_refor_formulasi_detail f
    ...
    function insertBox_v3_export_stress_fisik_vno_formula($field, $id) {
    ...
    function updateBox_v3_export_stress_fisik_vno_formula($field, $id, $value, $rowData) {
    ...
    function insertBox_v3_export_stress_fisik_dmulai_stress_test($field, $id) {
        $return = '<input type="date" name="'.$field.'"  id="'.$id.'"  class="input_rows1 tanggal required" size="15" />';
        return $return;
    }
    
    function updateBox_v3_export_stress_fisik_dmulai_stress_test($field, $id, $value, $rowData) {
        if ($this->input->get('action') == 'view') {
             $return= $value; 
        }else{ 
    ...
    function insertBox_v3_export_stress_fisik_dselesai_stress_test($field, $id) {
        $return = '<input type="date" name="'.$field.'"  id="'.$id.'"  class="input_rows1 tanggal required" size="15" />';
        return $return;
    }
    
    function updateBox_v3_export_stress_fisik_dselesai_stress_test($field, $id, $value, $rowData) {
        if ($this->input->get('action') == 'view') {
             $return= $value; 
        }else{ 
            $return = '<input type="date" name="'.$field.'"  id="'.$id.'"  class="input_rows1 tanggal  required" size="15" value="'.$value.'"/>';
    ...
    function insertBox_v3_export_stress_fisik_cpic_stress_test($field, $id) {
    ...
    function insertBox_v3_export_stress_fisik_formulasi_uji_fisik($field, $id) { 
    ...
    function updateBox_v3_export_stress_fisik_formulasi_uji_fisik($field, $id, $value, $rowData) {
        $sql_dok    = "SELECT d.* FROM erp_privi.m_modul m 
                        JOIN erp_privi.m_modul_fields f ON m.iM_modul = f.iM_modul
                        JOIN erp_privi.sys_masterdok d ON f.iM_modul_fields = d.iM_modul_fields
                        WHERE m.lDeleted = 0 AND f.lDeleted = 0 AND d.ldeleted = 0
    ...
    function updateBox_v3_export_stress_fisik_cpic_stress_test($field, $id, $value, $rowData) {
        ...
        if ($this->input->get('action') == 'view') {
            ...
            $return   .= '<input type="text" name="'.$field.'_dis"  id="'.$id.'_dis"  class="input_rows1 required" size="50" maxlength = "50"  value="'.$pic.'"/>'; 
    ...
    function before_insert_processor($row, $postData) {
        ...
        if(filter_var($postData['isdraft'], FILTER_VALIDATE_BOOLEAN) == true){
            $postData['isubmit_fisik_stress'] = 0;
        } else{
            $postData['isubmit_fisik_stress'] = 1;
            $postData['dsubmit_fisik_stress'] = date('Y-m-d H:i:s');
            $postData['csubmit_fisik_stress'] = $this->user->gNIP;
        } 
    ...
    function manipulate_update_button($buttons, $rowData) { 
        ...
        if ($this->input->get('action') == 'view' || $rowData['isubmit_fisik_stress'] == 1 || $submitFisik == 1){
            unset($buttons['update_back']);
        }
