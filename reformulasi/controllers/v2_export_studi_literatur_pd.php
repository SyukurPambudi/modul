...
class v2_export_studi_literatur_pd extends MX_Controller {
    function __construct() {
		...
		$this->title 		= 'Studi Literatur pd';
		$this->url 			= 'v2_export_studi_literatur_pd';
		$this->urlpath 		= 'reformulasi/'.str_replace("_","/", $this->url);


		$this->maintable 	= 'reformulasi.export_refor_studi_literatur_pd';	
		$this->main_table 	= $this->maintable;	
		$this->main_table_pk= 'iexport_refor_studi_literatur_pd';	
		...
		$datagrid['islist'] = array(
            ...
            'dmulaistudiPd'	=> array('label' => 'Tgl Mulai','width' => 100,	'align' => 'center', 'search' => true),
            'dselesaistudiPd' => array('label' => 'Tgl Selesai',	'width' => 100,	'align' => 'center', 'search' => true),
			'cPicstudiPd' => array('label' => 'NIP PIC','width' => 110,	'align' => 'center', 'search' => true),
			...
			'iappPd' => array('label' => 'Approval','width' => 110,	'align' => 'center', 'search' => false),
		);		

		$datagrid['setQuery']=array(
			0 => array('vall' => 'export_refor_studi_literatur_pd.lDeleted', 'nilai' => 0),
			
		);

		$datagrid['jointableinner'] = array(
			0 => $arrayName = array( 'reformulasi.export_req_refor' => 'export_refor_studi_literatur_pd.iexport_req_refor =export_req_refor.iexport_req_refor '),
			1 => array('hrd.employee' => 'employee.cNip = export_refor_studi_literatur_pd.cPicstudiPd'),
		...
		$datagrid['shortBy']=array("export_refor_studi_literatur_pd.iexport_refor_studi_literatur_pd" => "DESC");
    ...
    function index($action = '') {
      ...
      /*  $grid->changeSearch('dPermintaan_req_export','between');*/
		/*$grid->changeSearch('cPicstudiPd','combobox');*/
    ...
    function get_data_prev(){
        ...
        function listBox_v2_export_studi_literatur_pd_iapppr($value) {
        ...
        function listBox_Action($row, $actions) {
            ...
            $peka = $row->iexport_refor_studi_literatur_pd;
...
/*aslinya*/
   /*         if ( $getLastactivity == 0 ) { 
        ...
        function insertBox_v2_export_studi_literatur_pd_form_detail($field,$id){
            $get=$this->input->get();
            $post=$this->input->post();
        ...
        function updateBox_v2_export_studi_literatur_pd_form_detail($field,$id,$value,$rowData){
            $get=$this->input->get();
            $post=$this->input->post();
            foreach ($get as $kget => $vget) {
                if($kget!="action"){
            ...
            function approve_view() { 
                             ...
                             function submit_ajax(form_id) {
                                return $.ajax({
                                    ...
                                    success: function(data) {
                                        ...
                                        var url = "'.base_url().'processor/reformulasi/v2_export_studi_literatur_pd";                             
                                        if(o.status == true) { 
                                            $("#alert_dialog_form").dialog("close");
                                                 $.get(url+"?action=update&id="+last_id+"&foreign_key=0&company_id=3&group_id="+group_id+"&modul_id="+modul_id, function(data) {
                                                 $("div#form_v2_export_studi_literatur_pd").html(data);
                                                 
                                            });
                                            
                                        }
                                            reload_grid("grid_v2_export_studi_literatur_pd");
                                    }
                ...
                $echo .= '<form id="form_v2_export_studi_literatur_pd_approve" action="'.base_url().'processor/reformulasi/v2_export_studi_literatur_pd?action=approve_process" method="post">';
                ...
                <button type="button" onclick="submit_ajax(\'form_v2_export_studi_literatur_pd_approve\')">Approve</button>';
    ...
    */
            ...
            function confirm_view() { 
                ...
                $echo .= '<form id="form_v2_export_studi_literatur_pd_confirm" action="'.base_url().'processor/reformulasi/v2_export_studi_literatur_pd?action=confirm_process" method="post">';
                $echo .= '<div style="vertical-align: top;">';
                $echo .= 'Remark : 
                        <input type="hidden" name="'.$this->main_table_pk.'" value="'.$this->input->get($this->main_table_pk).'" />
                        <input type="hidden" name="modul_id" value="'.$this->input->get('modul_id').'" />
                        ...
                        <textarea name="vRemark"></textarea>
                <button type="button" onclick="submit_ajax(\'form_v2_export_studi_literatur_pd_confirm\')">Confirm</button>';
            ...
            function reject_view() {
                $echo = '<script type="text/javascript">
                             function submit_ajax(form_id) {
                                var remark = $("#v2_export_studi_literatur_pd_remark").val();
                                if (remark=="") {
                ...
                $echo .= '<form id="form_v2_export_studi_literatur_pd_reject" action="'.base_url().'processor/reformulasi/v2_export_studi_literatur_pd?action=reject_process" method="post">';
                $echo .= '<div style="vertical-align: top;">';
                $echo .= 'Remark : 
                        <input type="hidden" name="'.$this->main_table_pk.'" value="'.$this->input->get($this->main_table_pk).'" />
                        <input type="hidden" name="modul_id" value="'.$this->input->get('modul_id').'" />
                        ...
                        <textarea name="vRemark" id="reject_v2_export_studi_literatur_pd_remark"></textarea>
                <button type="button" onclick="submit_ajax(\'form_v2_export_studi_literatur_pd_reject\')">Reject</button>';
        ...
        if($postData['iSubmit']==1){
            ...
            Diberitahukan bahwa telah ada Submited Modul Study Literatur PD,
                ...
                <table border='0' style='width: 600px;'>
                    ...
                    <tr>
                            <td><b>Tanggal Selesai Studi Literatur  </b></td><td> : </td>
 ...
 function manipulate_update_button($buttons, $rowData) { 
        ...
        $iframe = '<iframe name="v2_export_studi_literatur_pd" id="v2_export_studi_literatur_pd" height="0" width="0"></iframe>';
        ...
        if ($this->input->get('action') == 'view') {
            unset($buttons['update']);
                ...
                if($isOpenEditing){
                    $update_draft = '<button onclick="javascript:update_draft_btn(\'v2_export_studi_literatur_pd\', \' '.base_url().'processor/reformulasi/v2_export_studi_literatur_pd?draft=true \',this,true )"  id="button_update_draft_v2_export_studi_literatur_pd"  class="ui-button-text icon-save" >Update open Editing</button>';
                    ...
                    foreach ($activities as $act) {
                        $update_draft = '<button onclick="javascript:update_draft_btn(\'v2_export_studi_literatur_pd\', \' '.base_url().'processor/reformulasi/v2_export_studi_literatur_pd?draft=true&modul_id='.$this->input->get('modul_id').'&iM_modul_activity='.$act['iM_modul_activity'].' \',this,true )"  id="button_update_draft_v2_export_studi_literatur_pd"  class="ui-button-text icon-save" >Update as Draft</button>';
                        $update = '<button onclick="javascript:update_btn_back(\'v2_export_studi_literatur_pd\', \' '.base_url().'processor/reformulasi/v2_export_studi_literatur_pd?company_id='.$this->input->get('company_id').'&modul_id='.$this->input->get('modul_id').'&iM_modul_activity='.$act['iM_modul_activity'].'&group_id='.$this->input->get('group_id').'modul_id='.$this->input->get('modul_id').' \',this,false )"  id="button_update_submit_v2_export_studi_literatur_pd"  class="ui-button-text icon-save" >Update &amp; Submit</button>';

                        $approve = '<button onclick="javascript:load_popup(\' '.base_url().'processor/reformulasi/v2_export_studi_literatur_pd?action=approve&iM_modul_activity='.$act['iM_modul_activity'].'&iM_activity='.$act['iM_activity'].'&'.$this->main_table_pk.'='.$peka.'&iupb_id='.$iupb_id.'&company_id='.$this->input->get('company_id').'&group_id='.$this->input->get('group_id').'&modul_id='.$this->input->get('modul_id').' \')"  id="button_approve_v2_export_studi_literatur_pd"  class="ui-button-text icon-save" >Approve</button>';
                        $reject = '<button onclick="javascript:load_popup(\' '.base_url().'processor/reformulasi/v2_export_studi_literatur_pd?action=reject&iM_modul_activity='.$act['iM_modul_activity'].'&iM_activity='.$act['iM_activity'].'&'.$this->main_table_pk.'='.$peka.'&iupb_id='.$iupb_id.'&company_id='.$this->input->get('company_id').'&group_id='.$this->input->get('group_id').'&modul_id='.$this->input->get('modul_id').' \' )"  id="button_reject_v2_export_studi_literatur_pd"  class="ui-button-text icon-save" >Reject</button>';

                        $confirm = '<button onclick="javascript:load_popup(\' '.base_url().'processor/reformulasi/v2_export_studi_literatur_pd?action=confirm&iM_modul_activity='.$act['iM_modul_activity'].'&iM_activity='.$act['iM_activity'].'&'.$this->main_table_pk.'='.$peka.'&iupb_id='.$iupb_id.'&company_id='.$this->input->get('company_id').'&group_id='.$this->input->get('group_id').'&modul_id='.$this->input->get('modul_id').' \')"  id="button_approve_v2_export_studi_literatur_pd"  class="ui-button-text icon-save" >Confirm</button>';
