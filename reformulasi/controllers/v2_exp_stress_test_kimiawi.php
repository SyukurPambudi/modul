...
class v2_exp_stress_test_kimiawi extends MX_Controller {
    function __construct() {
		...
		$this->title 		= 'Stress Test - Uji Kimiawi';
		$this->url 			= 'v2_exp_stress_test_kimiawi';
		...
		$datagrid['islist'] = array(
            ...
            'isubmit_fisik_stress'                  => array('label' => 'Submit Uji Fisik',     'width' => 150, 'align' => 'center', 'search' => true),
            'isubmit_kimiawi_stress'                => array('label' => 'Submit Uji Kimiawi',   'width' => 150, 'align' => 'center', 'search' => true),
            'iappad_kimiawi_stress'                 => array('label' => 'Approval Uji Kimiawi', 'width' => 150, 'align' => 'center', 'search' => false),
		);		

        $sql_check_submit = '( SELECT COUNT(*) AS submitted FROM reformulasi.export_refor_formulasi_detail f WHERE f.iexport_formulasi = export_refor_formulasi.iexport_formulasi AND f.isubmit_fisik_stress = 1 AND f.ldeleted = 0 ) > 0';
    ...
    function index($action = '') {
		...
		$grid->changeFieldType('isubmit_fisik_stress', 'combobox','',array(''=>'--select--',0=>'Need to be Submit', 1=>'Submited'));
        $grid->changeFieldType('isubmit_kimiawi_stress', 'combobox','',array(''=>'--select--',0=>'Need to be Submit', 1=>'Submited'));
		$grid->changeFieldType('iappad_kimiawi_stress', 'combobox','',array(''=>'--select--',0=>'Waiting for approval',1=>'Rejected', 2=>'Approved'));
    ...
    function getFormDetail(){
        ...
        function listBox_v2_exp_stress_test_kimiawi_iapppr($value) {
        ...
        function listBox_v2_exp_stress_test_kimiawi_cRequestor($value) {
        ...
        function insertBox_v2_exp_stress_test_kimiawi_form_detail($field,$id){
            $get=$this->input->get();
            $post=$this->input->post();
        ...
        function updateBox_v2_exp_stress_test_kimiawi_form_detail($field,$id,$value,$rowData){
            $get=$this->input->get();
            $post=$this->input->post();
            foreach ($get as $kget => $vget) {
                if($kget!="action"){
            ...
            function approve_view() { 
                             ...
                             function submit_ajax(form_id) {
                                return $.ajax({
                                    ...
                                    success: function(data) {
                                        ...
                                        var url = "'.base_url().'processor/reformulasi/v2_exp_stress_test_kimiawi";                             
                                        if(o.status == true) { 
                                            $("#alert_dialog_form").dialog("close");
                                                 $.get(url+"?action=update&id="+last_id+"&foreign_key=0&company_id=3&group_id="+group_id+"&modul_id="+modul_id, function(data) {
                                                 $("div#form_v2_exp_stress_test_kimiawi").html(data);
                                                 
                                            });
                                            
                                        }
                                            reload_grid("grid_v2_exp_stress_test_kimiawi");
                                    }
                ...
                $echo .= '<form id="form_v2_exp_stress_test_kimiawi_approve" action="'.base_url().'processor/reformulasi/v2_exp_stress_test_kimiawi?action=approve_process" method="post">';
                ...
                <button type="button" onclick="submit_ajax(\'form_v2_exp_stress_test_kimiawi_approve\')">Approve</button>';
            ...
            function confirm_view() { 
                ...
                $echo .= '<form id="form_v2_exp_stress_test_kimiawi_confirm" action="'.base_url().'processor/reformulasi/v2_exp_stress_test_kimiawi?action=confirm_process" method="post">';
                $echo .= '<div style="vertical-align: top;">';
                $echo .= 'Remark : 
                        <input type="hidden" name="'.$this->main_table_pk.'" value="'.$this->input->get($this->main_table_pk).'" />
                        <input type="hidden" name="modul_id" value="'.$this->input->get('modul_id').'" />
                        ...
                        <textarea name="vRemark"></textarea>
                <button type="button" onclick="submit_ajax(\'form_v2_exp_stress_test_kimiawi_confirm\')">Confirm</button>';
            ...
            function reject_view() {
                $echo = '<script type="text/javascript">
                             function submit_ajax(form_id) {
                                var remark = $("#v2_exp_stress_test_kimiawi_remark").val();
                                if (remark=="") {
                ...
                $echo .= '<form id="form_v2_exp_stress_test_kimiawi_reject" action="'.base_url().'processor/reformulasi/v2_exp_stress_test_kimiawi?action=reject_process" method="post">';
                $echo .= '<div style="vertical-align: top;">';
                $echo .= 'Remark : 
                        <input type="hidden" name="'.$this->main_table_pk.'" value="'.$this->input->get($this->main_table_pk).'" />
                        <input type="hidden" name="modul_id" value="'.$this->input->get('modul_id').'" />
                        ...
                        <textarea name="vRemark" id="reject_v2_exp_stress_test_kimiawi_remark"></textarea>
                <button type="button" onclick="submit_ajax(\'form_v2_exp_stress_test_kimiawi_reject\')">Reject</button>';
    ...
    function reject_process() {
        ...
        $update['isubmit_kimiawi_stress']       = 0;
        $this->db->where($this->main_table_pk, $pk);
        $this->db->update($this->maintable, $update);
        ...
        //update Stress Test formula jadi draft
        $resetFormula['isubmit_kimiawi_stress']    = 0;
        $resetFormula['dsubmit_kimiawi_stress']    = null;
        $resetFormula['csubmit_kimiawi_stress']    = null;
        $this->db->where('iexport_formulasi', $pk);
        $this->db->update('reformulasi.export_refor_formulasi_detail', $resetFormula);
    ...
    function manipulate_update_button($buttons, $rowData) { 
        ...
        $iframe 		= '<iframe name="v2_exp_stress_test_kimiawi_frame" id="v2_exp_stress_test_kimiawi_frame" height="0" width="0"></iframe>';
        ...
        if ($this->input->get('action') == 'view') {
            unset($buttons['update']);
                ...
                if($isOpenEditing){
                    $update_draft = '<button onclick="javascript:update_draft_btn(\'v2_exp_stress_test_kimiawi\', \' '.base_url().'processor/reformulasi/v2_exp_stress_test_kimiawi?draft=true \',this,true )"  id="button_update_draft_v2_exp_stress_test_kimiawi"  class="ui-button-text icon-save" >Update open Editing</button>';
                    ...
                    foreach ($activities as $act) {
                        $update_draft = '<button onclick="javascript:update_draft_btn(\'v2_exp_stress_test_kimiawi\', \' '.base_url().'processor/reformulasi/v2_exp_stress_test_kimiawi?draft=true&modul_id='.$this->input->get('modul_id').'&iM_modul_activity='.$act['iM_modul_activity'].' \',this,true )"  id="button_update_draft_v2_exp_stress_test_kimiawi"  class="ui-button-text icon-save" >Update as Draft</button>';
                        $update = '<button onclick="javascript:update_btn_back(\'v2_exp_stress_test_kimiawi\', \' '.base_url().'processor/reformulasi/v2_exp_stress_test_kimiawi?company_id='.$this->input->get('company_id').'&modul_id='.$this->input->get('modul_id').'&iM_modul_activity='.$act['iM_modul_activity'].'&group_id='.$this->input->get('group_id').'modul_id='.$this->input->get('modul_id').' \',this,false )"  id="button_update_submit_v2_exp_stress_test_kimiawi"  class="ui-button-text icon-save" >Update &amp; Submit</button>';

                        $approve = '<button onclick="javascript:load_popup(\' '.base_url().'processor/reformulasi/v2_exp_stress_test_kimiawi?action=approve&iM_modul_activity='.$act['iM_modul_activity'].'&iM_activity='.$act['iM_activity'].'&'.$this->main_table_pk.'='.$peka.'&iupb_id='.$iupb_id.'&company_id='.$this->input->get('company_id').'&group_id='.$this->input->get('group_id').'&modul_id='.$this->input->get('modul_id').' \')"  id="button_approve_v2_exp_stress_test_kimiawi"  class="ui-button-text icon-save" >Approve</button>';
                        $reject = '<button onclick="javascript:load_popup(\' '.base_url().'processor/reformulasi/v2_exp_stress_test_kimiawi?action=reject&iM_modul_activity='.$act['iM_modul_activity'].'&iM_activity='.$act['iM_activity'].'&'.$this->main_table_pk.'='.$peka.'&iupb_id='.$iupb_id.'&company_id='.$this->input->get('company_id').'&group_id='.$this->input->get('group_id').'&modul_id='.$this->input->get('modul_id').' \' )"  id="button_reject_v2_exp_stress_test_kimiawi"  class="ui-button-text icon-save" >Reject</button>';

                        $confirm = '<button onclick="javascript:load_popup(\' '.base_url().'processor/reformulasi/v2_exp_stress_test_kimiawi?action=confirm&iM_modul_activity='.$act['iM_modul_activity'].'&iM_activity='.$act['iM_activity'].'&'.$this->main_table_pk.'='.$peka.'&iupb_id='.$iupb_id.'&company_id='.$this->input->get('company_id').'&group_id='.$this->input->get('group_id').'&modul_id='.$this->input->get('modul_id').' \')"  id="button_approve_v2_exp_stress_test_kimiawi"  class="ui-button-text icon-save" >Confirm</button>';
    ...
    private function canSubmitKimiawi ($id){
        ...
        if (!empty($trial_submit)){
            ...
            if (intval($submit_fisik_stress) == 0){
                $status     = false;
                $message    = 'Stress Test - Uji Fisik Masih Belum Di Submit';
            } else {
                $filter_submit  = array(
                    'iexport_formulasi'   => $id,
                    'isubmit_fisik_stress'         => 1,
                    'isubmit_kimiawi_stress'       => 0,
                    'ldeleted'              => 0
                );
     ...
     private function sendNotifKesimpulan ($id){
        ...
        if (count($arrPD) > 0){
            $sql        = 'SELECT r.vno_export_req_refor, u.vUpd_no, u.vNama_usulan, s.ccreate
                            FROM reformulasi.export_refor_formulasi s 
                            ...
                            WHERE s.iexport_formulasi = ? ';
            $data       = $this->db->query($sql, array($id))->row_array();

            if (!empty($data)){
                $subject  = 'Stress Test - Uji Kimiawi Need Process';

                $content  = '<p>Kepada Yth. Bapak / Ibu</p>';
                $content .= '<p>Diberitahukan kepada PD Export untuk melakukan Proses Stress Test - Kesimpulan dengan rincian sebagai berikut : </p>';
                $content .= '<table>';
                $content .= '    <tr>';
    ...
    private function sendNotif ($id){
        ...
        if (count($adManager) > 0){
            ...
            if (!empty($data)){
                $subject  = 'Stress Test - Uji Kimiawi Need Approval';

                $content  = '<p>Kepada Yth. Bapak / Ibu</p>';
                $content .= '<p>Diberitahukan kepada AD Export Manager untuk melakukan Approval pada Modul Stress Test - Uji Kimiawi dengan rincian sebagai berikut : </p>';
                $content .= '<table>';
                $content .= '    <tr>';
                $content .= '       <td> Nomor Reformulasi </td>';
                $content .= '       <td> : </td>';
