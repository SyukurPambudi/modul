...
class v3_export_skala_lab_fisik extends MX_Controller {
    function __construct() {
        ...
        $this->url = 'v3_export_skala_lab_fisik';
        ...
        $this->pathfile = 'files/reformulasi/export/skala_lab';
    ...
    function index($action = '') {
        ...
        $grid->addList('vno_formula','dmulai_skala_lab','dselesai_skala_lab','cpic_skala_lab','isubmit_fisik','dsubmit_fisik'); 
        $grid->setSortBy('iexport_skala_lab_formula');
        $grid->setSortOrder('DESC');  

        $grid->addFields('vno_formula','dmulai_skala_lab','dselesai_skala_lab','cpic_skala_lab','formulasi_uji_fisik', 'ldeleted'); 

        $grid->setWidth('vno_formula', '100');
        $grid->setAlign('vno_formula', 'left');
        $grid->setLabel('vno_formula','No. Formulasi');
    
        $grid->setWidth('dmulai_skala_lab', '100');
        $grid->setAlign('dmulai_skala_lab', 'left');
        $grid->setLabel('dmulai_skala_lab','Tgl Mulai Orientasi Skala Lab ');

        $grid->setWidth('dselesai_skala_lab', '100');
        $grid->setAlign('dselesai_skala_lab', 'left');
        $grid->setLabel('dselesai_skala_lab','Tgl Selesai Orientasi Skala Lab ');

        $grid->setWidth('cpic_skala_lab', '100');
        $grid->setAlign('cpic_skala_lab', 'left');
        $grid->setLabel('cpic_skala_lab','PIC Skala Lab ');
        ...
        $grid->setRequired('vno_formula','dmulai_skala_lab','dselesai_skala_lab','cpic_skala_lab'); 
    ...
    public function manipulate_grid_button($button) {  
        unset($button['create']);
        $url        = base_url()."processor/reformulasi/".str_replace('_', '/', $this->url)."?action=create&foreign_key=".$this->input->get('iexport_skala_lab')."&iexport_skala_lab=".$this->input->get('iexport_skala_lab')."&company_id=".$this->input->get('company_id')."&group_id=".$this->input->get('group_id')."&modul_id=".$this->input->get('modul_id');
    ...
    function listBox_Action($row, $actions) { 
        ...
        $sqlCek     = 'SELECT t.isubmit_fisik FROM reformulasi.export_skala_lab_formula f
    ...
    function insertBox_v3_export_skala_lab_fisik_vno_formula($field, $id) {
    ...
    function updateBox_v3_export_skala_lab_fisik_vno_formula($field, $id, $value, $rowData) {
    ...
    function insertBox_v3_export_skala_lab_fisik_dmulai_skala_lab($field, $id) {
        $return = '<input type="date" name="'.$field.'"  id="'.$id.'"  class="input_rows1 tanggal required" size="15" />';
        return $return;
    }
    
    function updateBox_v3_export_skala_lab_fisik_dmulai_skala_lab($field, $id, $value, $rowData) {
        if ($this->input->get('action') == 'view') {
             $return= $value; 
        }else{ 
    ...
    function insertBox_v3_export_skala_lab_fisik_dselesai_skala_lab($field, $id) {
        $return = '<input type="date" name="'.$field.'"  id="'.$id.'"  class="input_rows1 tanggal required" size="15" />';
        return $return;
    }
    
    function updateBox_v3_export_skala_lab_fisik_dselesai_skala_lab($field, $id, $value, $rowData) {
        if ($this->input->get('action') == 'view') {
             $return= $value; 
        }else{ 
            $return = '<input type="date" name="'.$field.'"  id="'.$id.'"  class="input_rows1 tanggal  required" size="15" value="'.$value.'"/>';
    ...
    function insertBox_v3_export_skala_lab_fisik_cpic_skala_lab($field, $id) {
    ...
    function insertBox_v3_export_skala_lab_fisik_formulasi_uji_fisik($field, $id) { 
    ...
    function updateBox_v3_export_skala_lab_fisik_formulasi_uji_fisik($field, $id, $value, $rowData) {
        $sql_dok    = "SELECT d.* FROM erp_privi.m_modul m 
                        JOIN erp_privi.m_modul_fields f ON m.iM_modul = f.iM_modul
                        JOIN erp_privi.sys_masterdok d ON f.iM_modul_fields = d.iM_modul_fields
                        WHERE m.lDeleted = 0 AND f.lDeleted = 0 AND d.ldeleted = 0
    ...
    function updateBox_v3_export_skala_lab_fisik_cpic_skala_lab($field, $id, $value, $rowData) {
    ...
    function before_insert_processor($row, $postData) { //print_r(json_encode($postData));exit();
