...
class master_pengujian_refor extends MX_Controller {
    ...
    function index($action = '') {
        ...
        $grid = new Grid;
        $grid->setTitle('Master Pengujian Reformulasi');
        $grid->setTable('reformulasi.master_pengujian_refor');      
        $grid->setUrl('master_pengujian_refor');


        //List Table
        $grid->addList('master_bentuk_sediaan_refor.vBentuk_sediaan','vNama_pengujian','vKeterangan','iStatus'); 
        $grid->setSortBy('id_pengujian','id_bentuk_sediaan','vNama_pengujian','iStatus');
        $grid->setSortOrder('DESC');  

        //List field
        $grid->addFields('id_bentuk_sediaan','vNama_pengujian','vKeterangan','iStatus'); 
        ...
        */
        ...
        $grid->setLabel('master_bentuk_sediaan_refor.vBentuk_sediaan','Nama Bentuk Sediaan');
        $grid->setWidth('id_bentuk_sediaan', '250');
        $grid->setAlign('id_bentuk_sediaan', 'left');
        $grid->setLabel('id_bentuk_sediaan','Nama Bentuk Sediaan');

        $grid->setWidth('vNama_pengujian', '250');
        $grid->setAlign('vNama_pengujian', 'left');
        $grid->setLabel('vNama_pengujian','Pengujian');
...
//Example modifikasi GRID ERP
    //- Set Query
        $grid->setQuery('master_pengujian_refor.lDeleted = 0 ', null); 
        /*if ($this->validation) {
...
*/
    ...
    //set search
        ...
        $grid->setSearch('master_bentuk_sediaan_refor.vBentuk_sediaan','vNama_pengujian','iStatus');
        
    //set required
        $grid->setRequired('id_bentuk_sediaan','vNama_pengujian','iStatus','dCreate','cCreated','dUpdate','cUpdate','lDeleted'); 
           ...
           /* case 'createproses':
            ...
            case 'createproses':
                      ...
                      //  $nama_alasan = ['nama_alasan'];
                        $name = $post['master_pengujian_refor_vNama_pengujian']; //print_r($_POST);exit();
                        $cek  = $this->db->get_where('reformulasi.master_pengujian_refor', array('vNama_pengujian' => $name, 'lDeleted' => 0))->num_rows(); 
                        if ($cek > 0){
                            $data['status']  = false;
            ...
            case 'updateproses':
                        $post   = $this->input->post(); 
                        $id     = $post['master_pengujian_refor_id_pengujian']; 
                        $name   = $post['master_pengujian_refor_vNama_pengujian']; 
                        $sql    = 'SELECT * FROM reformulasi.master_pengujian_refor WHERE id_pengujian <> '.$id.' AND vNama_pengujian = "'.$name.'" AND lDeleted = 0'; //echo $sql;exit();   
                        //echo ($sql);exit();
    ...
    */
    ...
    function insertBox_master_pengujian_refor_id_bentuk_sediaan($field, $id) {
        $teams = $this->db_schedulercheck->get_where('reformulasi.master_bentuk_sediaan_refor', array('lDeleted' => 0))->result_array();
    ...
    function updateBox_master_pengujian_refor_id_bentuk_sediaan($field, $id, $value, $rowData) {
        if ($this->input->get('action') == 'view') { 
            ...
            $echo .= '<option '.$selected.' value="'.$t['id_bentuk_sediaan'].'">'.$t['vBentuk_sediaan'].'</option>';
        }
        $echo .= '</select>';
