...
class browse_export_uji_be extends MX_Controller {
    ...
    function index($action = '') {
        ...
        $grid->setUrl('browse_export_uji_be');
        ...
        $grid->setQuery('export_refor_formula.iexport_refor_formula not in (select iexport_refor_formula from reformulasi.export_uji_be where ldeleted=0)',NULL);
        ...
        /*Perlu Uji BE*/
        $grid->setQuery('export_req_refor.iUjibe',1);
        ...
        switch ($action) {
            ...
            default:
                $grid->render_grid();
                $url=base_url()."processor/reformulasi/browse/export/uji/be?action=getdetails";
                $o='<script type="text/javascript">
                function pilih_request_uji_be(id){    
                    custom_confirm("Yakin ?", function(){
                        $.ajax({
                            ...
                            success: function( data ) {
                                ...
                                $("#tb_details_export_uji_be_id").val(o.iexport_req_refor);
                                reload_grid_tb_details_export_uji_be();
                            }
                        });
    ...
    function listBox_browse_export_uji_be_pilih($value, $pk, $name, $rowData) {
        $o = '<input type="radio" name="pilih" onClick="javascript:pilih_request_uji_be('.$pk.') ;" />';
