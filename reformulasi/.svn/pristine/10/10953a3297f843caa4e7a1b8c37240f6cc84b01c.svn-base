...
class v3_export_request_sample extends MX_Controller {
    function __construct() {
		...
		$this->url = 'v3_export_request_sample';
		...
		$this->main_table_pk = 'export_request_sample';	

		$datagrid['islist'] = array(
			'iTujuan_request' 						=> array('label' => 'Tujuan Request', 			'width' => 150,	'align' => 'center',	'search'=>true),
			'vRequest_no' 							=> array('label' => 'Nomor Permintaan Sample', 	'width' => 100,	'align' => 'center',	'search'=>true),
			'dTgl_request' 							=> array('label' => 'Tgl Request', 				'width' => 100,	'align' => 'center',	'search'=>true),
			'export_req_refor.vno_export_req_refor' => array('label' => 'No request Refor',			'width' => 100,	'align' => 'center',	'search'=>true),
			'dossier_upd.vUpd_no' 					=> array('label' => 'No UPB',					'width' => 75,	'align' => 'center',	'search'=>true),
			'dossier_upd.vNama_usulan' 				=> array('label' => 'Nama Usulan',				'width' => 250,	'align' => 'left',		'search'=>true),
			'reformulasi_team.vteam'	 			=> array('label' => 'Team',						'width' => 100,	'align' => 'left',		'search'=>true),
			'iApprove_pd'	 						=> array('label' => 'Approve',					'width' => 100,	'align' => 'left',		'search'=>true),
		);		
    ...
    function load_detail(){
    	$post 		= $this->input->post();
    	$export_req = $post['iexport_req_refor'];

    	$sql 		= "SELECT r.vno_export_req_refor AS 'Nomor Request', u.vUpd_no AS 'Kode Produk', u.vNama_usulan AS 'Nama Produk' 
						FROM reformulasi.export_req_refor r 
						JOIN dossier.dossier_upd u ON r.idossier_upd_id = u.idossier_upd_id
						WHERE r.iexport_req_refor = ? ";
		$detail 	= $this->db->query($sql, array($export_req))->row_array();
    ...
    function load_upd(){
        ...
        $is_create 	= $post['is_create'];
       	...
       	$fieldData 	= $this->db->get_where('erp_privi.m_modul_fields', array('lDeleted' => 0, 'iM_modul_fields' => $field_id))->row_array();
       	$basicSQL 	= (intval($is_create) == 1)?$fieldData['vSource_input']:$fieldData['vSource_input_edit'];

       	$getVal 	= $this->db->get_where('erp_privi.m_modul_fields_validation', array('lDeleted' => 0, 'iM_modul_fields' => $field_id))->result_array();
       	foreach ($getVal as $val) {
       		$basicSQL .= ' ';
       		$basicSQL .= (intval($is_create) == 1)?$val['vSource_input_validtation']:$val['vSource_input_edit_validtation'];
       	}

       	$getFilter 	= $this->db->get_where('erp_privi.m_modul_fields_validation_after', array('lDeleted' => 0, 'iM_modul_fields' => $field_id, 'iSort' => $tujuan))->result_array();
       	foreach ($getFilter as $fil) {
       		$basicSQL .= ' ';
       		$basicSQL .= (intval($is_create) == 1)?$fil['vSource_input_validtation_after']:$fil['vSource_input_edit_validtation_after'];
       	}

       	$arrUPD 	= $this->db->query($basicSQL)->result_array();
        $html = '<select name="'.$fieldData['vNama_field'].'" id="'.$this->url.'_'.$fieldData['vNama_field'].'" required class="input_rows1 choose">';
        foreach ($arrUPD as $u) {
	...
	/*List Box*/
	function listBox_v3_export_request_sample_cPIC_AD($value, $pk, $name, $rowData) {
	...
	function listBox_v3_export_request_sample_dossier_upd_iteampd_id($value, $pk, $name, $rowData) {
	...
	function insertBox_v3_export_request_sample_form_detail($field,$id){
            $get=$this->input->get();
            $post=$this->input->post();
        ...
        function updateBox_v3_export_request_sample_form_detail($field,$id,$value,$rowData){
            $get=$this->input->get();
            $post=$this->input->post();
            foreach ($get as $kget => $vget) {
                if($kget!="action"){
    ...
    function after_insert_processor($fields, $id, $postData) {
		...
		$sql = "UPDATE reformulasi.plc2_vamoa SET vnomor_moa = '".$nomor."' WHERE ivalmoa_id=".$id." LIMIT 1";
