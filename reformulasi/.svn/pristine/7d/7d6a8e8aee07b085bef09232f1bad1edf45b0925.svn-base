<?php 
	...
	if (strtolower($act) != 'create'){
		...
		$dataRows 	= $this->db->query($sqlRows, array($rowDataH['iexport_skala_trial']))->result_array();

		foreach ($dataRows as $row) {
			$dataFiles 		= $this->db->query($sqlFile, array($row['iexport_skala_trial_formula']))->result_array();
			$files 			= array();
