<?php 
	...
	if (strtolower($act) != 'create'){
		...
		$dataRows 	= $this->db->query($sqlRows, array($rowDataH['iexport_formulasi']))->result_array();

		foreach ($dataRows as $row) {
			$dataFiles 		= $this->db->query($sqlFile, array($row['iexport_formulasi_detail']))->result_array();
			$files 			= array();
	...
	$arrKesimpulan = array( 0 => 'TMS', 1 => 'MS');
...
<div id="<?php echo $id; ?>">
	...
	<table id="table_trial_uji_fisik" cellspacing="0" cellpadding="1">
		<thead>
			<tr>
				<th colspan="14"><?php echo $form_field['vDesciption']; ?></th>
			</tr>
			<tr>
				<th rowspan="2">No.</th>
				<th rowspan="2">Nomor Formulasi</th>
				<th rowspan="2">Bentuk Sediaan</th>
				<th rowspan="2">Parameter Pengujian</th>
				<th rowspan="2">Tanggal Mulai Analisa</th>
				<th rowspan="2">Tanggal Selesai Analisa</th>
				<th rowspan="2">PIC Analisa</th>
				<th colspan="4">Upload File</th>
				<th rowspan="2">Kesimpulan Analisa</th>
				<th rowspan="2">Tanggal Kesimpulan Analisa</th>
		...
		<tbody>
			<?php
				foreach ($rows as $num => $row) {
					...
					$rowHead	.= '<td rowspan="'.$span.'">'.$row['vBentuk_sediaan'].'</td>';
					$rowHead	.= '<td rowspan="'.$span.'">'.$row['vNama_pengujian'].'</td>';
					$rowHead	.= '<td rowspan="'.$span.'">'.$row['dmulai_analisa_stress'].'</td>';
					$rowHead	.= '<td rowspan="'.$span.'">'.$row['dselesai_analisa_stress'].'</td>';
					$rowHead	.= '<td rowspan="'.$span.'">'.$row['nip_pic'].' - '.$row['name_pic'].'</td>';

					$rowFoot	 = '<td rowspan="'.$span.'">'.$arrKesimpulan[$row['ikesimpulan_analisa_stress']].'</td>';
					$rowFoot	.= '<td rowspan="'.$span.'">'.$row['dkesimpulan_analisa_stress'].'</td>';
					$rowFoot	.= '<td rowspan="'.$span.'">'.$row['dsubmit_kimiawi_stress'].'</td>';
