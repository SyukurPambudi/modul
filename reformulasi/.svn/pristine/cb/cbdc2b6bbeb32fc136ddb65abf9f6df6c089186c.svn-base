...
class v2_exp_request_refor extends MX_Controller {
    function __construct() {
		...
		$datagrid['islist'] = array(
			...
			'iApproval_ats_inisiator' 	=> array('label' => 'Approval',		'width' => 150,	'align' => 'center', 'search' => false),
		);		
    ...
    function get_data_prev(){
        ...
        function listBox_v2_exp_request_refor_iapppr($value) {
        ...
        function insertBox_v2_exp_request_refor_form_detail($field,$id){
            $get=$this->input->get();
            $post=$this->input->post();
        ...
        function updateBox_v2_exp_request_refor_form_detail($field,$id,$value,$rowData){
            $get=$this->input->get();
            $post=$this->input->post();
            foreach ($get as $kget => $vget) {
                if($kget!="action"){
            ...
            function approve_view() { 
                             ...
                             function submit_ajax(form_id) {
                                return $.ajax({
                                    ...
                                    success: function(data) {
                                        ...
                                        var url = "'.base_url().'processor/reformulasi/v2_exp_request_refor";                             
                                        if(o.status == true) { 
                                            $("#alert_dialog_form").dialog("close");
                                                 $.get(url+"?action=update&id="+last_id+"&foreign_key=0&company_id=3&group_id="+group_id+"&modul_id="+modul_id, function(data) {
                                                 $("div#form_v2_exp_request_refor").html(data);
                                                 
                                            });
                                            
                                        }
                                            reload_grid("grid_v2_exp_request_refor");
                                    }
                ...
                $echo .= '<form id="form_v2_exp_request_refor_approve" action="'.base_url().'processor/reformulasi/v2_exp_request_refor?action=approve_process" method="post">';
                ...
                <button type="button" onclick="submit_ajax(\'form_v2_exp_request_refor_approve\')">Approve</button>';
            ...
            function confirm_view() { 
                ...
                $echo .= '<form id="form_v2_exp_request_refor_confirm" action="'.base_url().'processor/reformulasi/v2_exp_request_refor?action=confirm_process" method="post">';
                $echo .= '<div style="vertical-align: top;">';
                $echo .= 'Remark : 
                        <input type="hidden" name="'.$this->main_table_pk.'" value="'.$this->input->get($this->main_table_pk).'" />
                        <input type="hidden" name="modul_id" value="'.$this->input->get('modul_id').'" />
                        ...
                        <textarea name="vRemark"></textarea>
                <button type="button" onclick="submit_ajax(\'form_v2_exp_request_refor_confirm\')">Confirm</button>';
            ...
            function reject_view() {
                $echo = '<script type="text/javascript">
                             function submit_ajax(form_id) {
                                var remark = $("#v2_exp_request_refor_remark").val();
                                if (remark=="") {
                ...
                $echo .= '<form id="form_v2_exp_request_refor_reject" action="'.base_url().'processor/reformulasi/v2_exp_request_refor?action=reject_process" method="post">';
                $echo .= '<div style="vertical-align: top;">';
                $echo .= 'Remark : 
                        <input type="hidden" name="'.$this->main_table_pk.'" value="'.$this->input->get($this->main_table_pk).'" />
                        <input type="hidden" name="modul_id" value="'.$this->input->get('modul_id').'" />
                        ...
                        <textarea name="vRemark" id="reject_v2_exp_request_refor_remark"></textarea>
                <button type="button" onclick="submit_ajax(\'form_v2_exp_request_refor_reject\')">Reject</button>';
    ...
    function manipulate_insert_buttonx($buttons) { 
        ...
        $iframe = '<iframe name="v2_exp_request_refor_frame" id="v2_exp_request_refor_frame" height="0" width="0"></iframe>';
        ...
        $save_draft = '<button onclick="javascript:save_draft_btn_multiupload(\'v2_exp_request_refor\', \' '.base_url().'processor/reformulasi/v2_exp_request_refor?draft=true&modul_id='.$this->input->get('modul_id').'&iM_modul_activity='.$act['iM_modul_activity'].' \',this,true )"  id="button_save_draft_v2_exp_request_refor"  class="ui-button-text icon-save" >Save as Draft</button>';
        $save = '<button onclick="javascript:save_btn_multiupload(\'v2_exp_request_refor\', \' '.base_url().'processor/reformulasi/v2_exp_request_refor?company_id='.$this->input->get('company_id').'&modul_id='.$this->input->get('modul_id').'&iM_modul_activity='.$act['iM_modul_activity'].'&group_id='.$this->input->get('group_id').'modul_id='.$this->input->get('modul_id').' \',this,false )"  id="button_save_submit_v2_exp_request_refor"  class="ui-button-text icon-save" >Save &amp; Submit</button>';
    ...
    function manipulate_update_button($buttons, $rowData) { 
        ...
        if ($this->input->get('action') == 'view') {
            unset($buttons['update']);
                ...
                if($isOpenEditing){
                    $update_draft = '<button onclick="javascript:update_draft_btn(\'v2_exp_request_refor\', \' '.base_url().'processor/reformulasi/v2_exp_request_refor?draft=true \',this,true )"  id="button_update_draft_v2_exp_request_refor"  class="ui-button-text icon-save" >Update open Editing</button>';
                    ...
                    foreach ($activities as $act) {
                        $update_draft = '<button onclick="javascript:update_draft_btn(\'v2_exp_request_refor\', \' '.base_url().'processor/reformulasi/v2_exp_request_refor?draft=true&modul_id='.$this->input->get('modul_id').'&iM_modul_activity='.$act['iM_modul_activity'].' \',this,true )"  id="button_update_draft_v2_exp_request_refor"  class="ui-button-text icon-save" >Update as Draft</button>';
                        $update_aja = '<button onclick="javascript:update_draft_btn(\'v2_exp_request_refor\', \' '.base_url().'processor/reformulasi/v2_exp_request_refor?draft=true&modul_id='.$this->input->get('modul_id').'&iM_modul_activity='.$act['iM_modul_activity'].' \',this,true )"  id="button_update_draft_v2_exp_request_refor"  class="ui-button-text icon-save" >Update</button>';
                        
                        $update = '<button onclick="javascript:update_btn_back(\'v2_exp_request_refor\', \' '.base_url().'processor/reformulasi/v2_exp_request_refor?company_id='.$this->input->get('company_id').'&modul_id='.$this->input->get('modul_id').'&iM_modul_activity='.$act['iM_modul_activity'].'&group_id='.$this->input->get('group_id').'modul_id='.$this->input->get('modul_id').' \',this,false )"  id="button_update_submit_v2_exp_request_refor"  class="ui-button-text icon-save" >Update &amp; Submit</button>';

                        $approve = '<button onclick="javascript:load_popup(\' '.base_url().'processor/reformulasi/v2_exp_request_refor?action=approve&iM_modul_activity='.$act['iM_modul_activity'].'&iM_activity='.$act['iM_activity'].'&'.$this->main_table_pk.'='.$peka.'&iupb_id='.$iupb_id.'&company_id='.$this->input->get('company_id').'&group_id='.$this->input->get('group_id').'&modul_id='.$this->input->get('modul_id').' \')"  id="button_approve_v2_exp_request_refor"  class="ui-button-text icon-save" >Approve</button>';
                        $reject = '<button onclick="javascript:load_popup(\' '.base_url().'processor/reformulasi/v2_exp_request_refor?action=reject&iM_modul_activity='.$act['iM_modul_activity'].'&iM_activity='.$act['iM_activity'].'&'.$this->main_table_pk.'='.$peka.'&iupb_id='.$iupb_id.'&company_id='.$this->input->get('company_id').'&group_id='.$this->input->get('group_id').'&modul_id='.$this->input->get('modul_id').' \' )"  id="button_reject_v2_exp_request_refor"  class="ui-button-text icon-save" >Reject</button>';

                        $confirm = '<button onclick="javascript:load_popup(\' '.base_url().'processor/reformulasi/v2_exp_request_refor?action=confirm&iM_modul_activity='.$act['iM_modul_activity'].'&iM_activity='.$act['iM_activity'].'&'.$this->main_table_pk.'='.$peka.'&iupb_id='.$iupb_id.'&company_id='.$this->input->get('company_id').'&group_id='.$this->input->get('group_id').'&modul_id='.$this->input->get('modul_id').' \')"  id="button_approve_v2_exp_request_refor"  class="ui-button-text icon-save" >Confirm</button>';
                        ...
                        switch ($act['iType']) {
                            ...
                            case '2':
                                ...
                                if($getLastStatusApprove){
                                    $sButton .= $update_aja.$approve.$reject;
                                }else{
    ...
    private function ViewUPB ($id=0){
        ...
        $arrUPB = array();
        foreach ($upb as $u) {
            if (isset($u['iexport_req_refor'])){
                array_push($arrUPB, $u['iexport_req_refor']);
            }
        }
        return $arrUPB;
    }
