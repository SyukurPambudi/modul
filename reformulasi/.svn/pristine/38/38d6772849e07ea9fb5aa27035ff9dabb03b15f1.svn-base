...
class v2_exp_skala_trial_fisik extends MX_Controller {
    function __construct() {
		...
		$this->title 		= 'Skala Trial - Uji Fisik';
		$this->url 			= 'v2_exp_skala_trial_fisik';
		...
		$datagrid['islist'] = array(
            ...
            'isubmit_fisik_trial'                   => array('label' => 'Submit Uji Fisik',     'width' => 100, 'align' => 'center', 'search' => true),
		);		
    ...
    function index($action = '') {
		...
		switch ($action) {
			...
			case 'updateproses':
                ...
                $isdraft        = filter_var($post['isdraft'], FILTER_VALIDATE_BOOLEAN);
                $get            = $this->input->get();
                $lastId         = isset($post[$this->url."_".$this->main_table_pk])?$post[$this->url."_".$this->main_table_pk]:"0";
                $cekFormula     = $this->db->get_where('reformulasi.export_refor_formulasi_detail', array('ldeleted' => 0, 'isubmit_fisik_trial' => 1, $this->main_table_pk => $lastId))->num_rows();
                ...
                if (!$isdraft && $cekFormula == 0){
                    $msg['status']  = false;
                    $msg['message'] = 'Formulasi yang Sudah Submit / Send Minimal 1 !';
    ...
    function getFormDetail(){
        ...
        function listBox_v2_exp_skala_trial_fisik_iapppr($value) {
        ...
        function listBox_v2_exp_skala_trial_fisik_cRequestor($value) {
        ...
        function insertBox_v2_exp_skala_trial_fisik_form_detail($field,$id){
            $get=$this->input->get();
            $post=$this->input->post();
        ...
        function updateBox_v2_exp_skala_trial_fisik_form_detail($field,$id,$value,$rowData){
            $get=$this->input->get();
            $post=$this->input->post();
            foreach ($get as $kget => $vget) {
                if($kget!="action"){
            ...
            function approve_view() { 
                             ...
                             function submit_ajax(form_id) {
                                return $.ajax({
                                    ...
                                    success: function(data) {
                                        ...
                                        var url = "'.base_url().'processor/reformulasi/v2_exp_skala_trial_fisik";                             
                                        if(o.status == true) { 
                                            $("#alert_dialog_form").dialog("close");
                                                 $.get(url+"?action=update&id="+last_id+"&foreign_key=0&company_id=3&group_id="+group_id+"&modul_id="+modul_id, function(data) {
                                                 $("div#form_v2_exp_skala_trial_fisik").html(data);
                                                 
                                            });
                                            
                                        }
                                            reload_grid("grid_v2_exp_skala_trial_fisik");
                                    }
                ...
                $echo .= '<form id="form_v2_exp_skala_trial_fisik_approve" action="'.base_url().'processor/reformulasi/v2_exp_skala_trial_fisik?action=approve_process" method="post">';
                ...
                <button type="button" onclick="submit_ajax(\'form_v2_exp_skala_trial_fisik_approve\')">Approve</button>';
            ...
            function confirm_view() { 
                ...
                $echo .= '<form id="form_v2_exp_skala_trial_fisik_confirm" action="'.base_url().'processor/reformulasi/v2_exp_skala_trial_fisik?action=confirm_process" method="post">';
                $echo .= '<div style="vertical-align: top;">';
                $echo .= 'Remark : 
                        <input type="hidden" name="'.$this->main_table_pk.'" value="'.$this->input->get($this->main_table_pk).'" />
                        <input type="hidden" name="modul_id" value="'.$this->input->get('modul_id').'" />
                        ...
                        <textarea name="vRemark"></textarea>
                <button type="button" onclick="submit_ajax(\'form_v2_exp_skala_trial_fisik_confirm\')">Confirm</button>';
            ...
            function reject_view() {
                $echo = '<script type="text/javascript">
                             function submit_ajax(form_id) {
                                var remark = $("#v2_exp_skala_trial_fisik_remark").val();
                                if (remark=="") {
                ...
                $echo .= '<form id="form_v2_exp_skala_trial_fisik_reject" action="'.base_url().'processor/reformulasi/v2_exp_skala_trial_fisik?action=reject_process" method="post">';
                $echo .= '<div style="vertical-align: top;">';
                $echo .= 'Remark : 
                        <input type="hidden" name="'.$this->main_table_pk.'" value="'.$this->input->get($this->main_table_pk).'" />
                        <input type="hidden" name="modul_id" value="'.$this->input->get('modul_id').'" />
                        ...
                        <textarea name="vRemark" id="reject_v2_exp_skala_trial_fisik_remark"></textarea>
                <button type="button" onclick="submit_ajax(\'form_v2_exp_skala_trial_fisik_reject\')">Reject</button>';
    ...
    function manipulate_insert_buttonx($buttons) { 
        ...
        $iframe = '<iframe name="v2_exp_skala_trial_fisik_frame" id="v2_exp_skala_trial_fisik_frame" height="0" width="0"></iframe>';
        ...
        $save_draft = '<button onclick="javascript:save_draft_btn_multiupload(\'v2_exp_skala_trial_fisik\', \' '.base_url().'processor/reformulasi/v2_exp_skala_trial_fisik?draft=true&modul_id='.$this->input->get('modul_id').'&iM_modul_activity='.$act['iM_modul_activity'].' \',this,true )"  id="button_save_draft_v2_exp_skala_trial_fisik"  class="ui-button-text icon-save" >Save as Draft</button>';
        $save = '<button onclick="javascript:save_btn_multiupload(\'v2_exp_skala_trial_fisik\', \' '.base_url().'processor/reformulasi/v2_exp_skala_trial_fisik?company_id='.$this->input->get('company_id').'&modul_id='.$this->input->get('modul_id').'&iM_modul_activity='.$act['iM_modul_activity'].'&group_id='.$this->input->get('group_id').'modul_id='.$this->input->get('modul_id').' \',this,false )"  id="button_save_submit_v2_exp_skala_trial_fisik"  class="ui-button-text icon-save" >Save &amp; Submit</button>';
    ...
    function manipulate_update_button($buttons, $rowData) { 
        ...
        if ($this->input->get('action') == 'view') {
            unset($buttons['update']);
                ...
                if($isOpenEditing){
                    $update_draft = '<button onclick="javascript:update_draft_btn(\'v2_exp_skala_trial_fisik\', \' '.base_url().'processor/reformulasi/v2_exp_skala_trial_fisik?draft=true \',this,true )"  id="button_update_draft_v2_exp_skala_trial_fisik"  class="ui-button-text icon-save" >Update open Editing</button>';
                    ...
                    foreach ($activities as $act) {
                        $update_draft = '<button onclick="javascript:update_draft_btn(\'v2_exp_skala_trial_fisik\', \' '.base_url().'processor/reformulasi/v2_exp_skala_trial_fisik?draft=true&modul_id='.$this->input->get('modul_id').'&iM_modul_activity='.$act['iM_modul_activity'].' \',this,true )"  id="button_update_draft_v2_exp_skala_trial_fisik"  class="ui-button-text icon-save" >Update as Draft</button>';
                        $update = '<button onclick="javascript:update_btn_back(\'v2_exp_skala_trial_fisik\', \' '.base_url().'processor/reformulasi/v2_exp_skala_trial_fisik?company_id='.$this->input->get('company_id').'&modul_id='.$this->input->get('modul_id').'&iM_modul_activity='.$act['iM_modul_activity'].'&group_id='.$this->input->get('group_id').'modul_id='.$this->input->get('modul_id').' \',this,false )"  id="button_update_submit_v2_exp_skala_trial_fisik"  class="ui-button-text icon-save" >Update &amp; Submit</button>';

                        $approve = '<button onclick="javascript:load_popup(\' '.base_url().'processor/reformulasi/v2_exp_skala_trial_fisik?action=approve&iM_modul_activity='.$act['iM_modul_activity'].'&iM_activity='.$act['iM_activity'].'&'.$this->main_table_pk.'='.$peka.'&iupb_id='.$iupb_id.'&company_id='.$this->input->get('company_id').'&group_id='.$this->input->get('group_id').'&modul_id='.$this->input->get('modul_id').' \')"  id="button_approve_v2_exp_skala_trial_fisik"  class="ui-button-text icon-save" >Approve</button>';
                        $reject = '<button onclick="javascript:load_popup(\' '.base_url().'processor/reformulasi/v2_exp_skala_trial_fisik?action=reject&iM_modul_activity='.$act['iM_modul_activity'].'&iM_activity='.$act['iM_activity'].'&'.$this->main_table_pk.'='.$peka.'&iupb_id='.$iupb_id.'&company_id='.$this->input->get('company_id').'&group_id='.$this->input->get('group_id').'&modul_id='.$this->input->get('modul_id').' \' )"  id="button_reject_v2_exp_skala_trial_fisik"  class="ui-button-text icon-save" >Reject</button>';

                        $confirm = '<button onclick="javascript:load_popup(\' '.base_url().'processor/reformulasi/v2_exp_skala_trial_fisik?action=confirm&iM_modul_activity='.$act['iM_modul_activity'].'&iM_activity='.$act['iM_activity'].'&'.$this->main_table_pk.'='.$peka.'&iupb_id='.$iupb_id.'&company_id='.$this->input->get('company_id').'&group_id='.$this->input->get('group_id').'&modul_id='.$this->input->get('modul_id').' \')"  id="button_approve_v2_exp_skala_trial_fisik"  class="ui-button-text icon-save" >Confirm</button>';
