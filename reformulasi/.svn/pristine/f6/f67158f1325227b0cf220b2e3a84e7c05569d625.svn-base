...
class browse_export_soi_fg extends MX_Controller {
    ...
    function index($action = '') {
		...
		$grid->setUrl('browse_export_soi_fg');
		$grid->addList('pilih','vnoFormulasi','export_req_refor.vno_export_req_refor','dossier_upd.vUpd_no','dossier_upd.vNama_usulan','export_req_refor.iTeamPD'); 
        ...
        $grid->setQuery('export_refor_formula.iexport_refor_formula in (select iexport_refor_formula from reformulasi.export_draft_soi_fg where ldeleted=0 and iapprove_ad=2)',NULL);
        $grid->setQuery('export_refor_formula.iexport_refor_formula not in (select iexport_refor_formula from reformulasi.export_soi_fg where ldeleted=0)',NULL);
    ...
    function listBox_browse_export_soi_fg_export_req_refor_iTeamPD($value, $pk, $name, $rowData) {
        return $this->getTeamByID($value);
    }

	function listBox_browse_export_soi_fg_pilih($value, $pk, $name, $rowData) {
		$o = '<input type="radio" name="pilih" onClick="javascript:pilih_request('.$pk.',\''.$rowData->vnoFormulasi.'\',\''.$rowData->export_req_refor__vno_export_req_refor.'\',\''.$rowData->dossier_upd__vNama_usulan.'\',\''.$rowData->dossier_upd__vUpd_no.'\',\''.$this->getTeamByID($rowData->export_req_refor__iTeamPD).'\') ;" />';
		$o.='<script type="text/javascript">
                function pilih_request (id, vnoformulasi, vno_export_req_refor, vNama_usulan,vUpd_no,iTeamPD){	
					custom_confirm("Yakin ?", function(){
                        ...
                        $("#'.$this->input->get('field').'_iTeam_pd").val(iTeamPD); 
