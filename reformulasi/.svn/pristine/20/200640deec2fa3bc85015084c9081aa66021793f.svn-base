...
class auth_localnon { 	
	...
	function is_managerdept_id($dept,$id) {
		$sqlmgr = "SELECT COUNT(1) c FROM plc2.plc2_upb_team t
				   WHERE t.vnip = '".$this->sess_auth->gNIP."' and t.vtipe='".$dept."' and t.iteam_id='".$id."'  and t.ldeleted=0 and t.iTipe=1";
		$ms = $this->_ci->db->query($sqlmgr)->row_array();
		return $ms['c'] > 0;
	}
	...
	function is_dir() {
		$sqlmgr = "SELECT COUNT(1) c FROM plc2.plc2_upb_team t
				   WHERE t.vnip = '".$this->sess_auth->gNIP."' and t.cDeptId='DR' and t.ldeleted=0 and t.iTipe=1";
		$ms = $this->_ci->db->query($sqlmgr)->row_array();
		return $ms['c'] > 0;
	}
	function is_andev() {
		$sqlmgr = "SELECT COUNT(1) c FROM plc2.plc2_upb_team t
				   WHERE t.vnip = '".$this->sess_auth->gNIP."' and t.cDeptId='AD' and t.ldeleted=0 and t.iTipe=1";
		$ms = $this->_ci->db->query($sqlmgr)->row_array();
		return $ms['c'] > 0;
	}
	function is_bdirm() {
		$sqlmgr = "SELECT COUNT(1) c FROM plc2.plc2_upb_team t
				   WHERE t.vnip = '".$this->sess_auth->gNIP."' and t.cDeptId='BDI' and t.ldeleted=0 and t.iTipe=1";
		$ms = $this->_ci->db->query($sqlmgr)->row_array();
		return $ms['c'] > 0;
	}
	function team() {
		...
		$sql = "SELECT t.iteam_id FROM plc2.plc2_upb_team_item t
				join plc2.plc2_upb_team te on te.iteam_id=t.iteam_id
