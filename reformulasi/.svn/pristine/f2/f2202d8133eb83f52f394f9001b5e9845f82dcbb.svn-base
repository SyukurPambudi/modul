							  	 ...
							  	 ?>
											  	<tr>
											  		...
											  		<!-- read status analisa BB jika sudah ada -->
												  		...
												  		<td>
												  				<?php if ( $dataBacth['iTerimaPd'] == 0  and (  in_array('PD', $manager) or in_array('PD', $team) )  ) {	?>
												  		...
												  		<td>	
												  				<?php if ( $dataBacth['iTerimaPd'] == 0  and (  in_array('PD', $manager) or in_array('PD', $team) )  ) {	
																	    	...
																	    	$echo = '<select class="" name="iSatuan_pd[]" >';
												  		...
												  		<td class="cb">
												  				<?php if ( $dataBacth['iTerimaPd'] == 0  and (  in_array('PD', $manager) or in_array('PD', $team) )  ) {	?>
												  					<?php 
												  						$sql = "
												  						 ...
												  						 ";
																	    	$teams = $this->db->query($sql)->result_array();
																	    	$echo = '<select  class="chosen-select cb2" name="cPenerima_pd[]" >';
																	    	$echo .= '<option value="">--Pilih--</option>';
																	    	foreach($teams as $t) {
																	    		$selected = $dataBacth['cPenerima_pd'] == $t['vnip'] ? 'selected' : '';
												  				...
												  				?>
												  							...
												  							<?php 
														  							...
														  							if (empty($dataBacth['cPenerima_pd'])) {
																			    		$ret = '-';
																			    	}else{
																			    		$ret = $iam['vName'];
																			    	}
												  		...
												  		<!-- BAGIAN TERIMA SAMPLE OLEH ANDEV START  -->
												  		<td>
												  				<?php if ( $dataBacth['iTerimaAd'] == 0  and (  in_array('AD', $manager) or in_array('AD', $team) )  ) {	?>
												  		...
												  		<td>	
												  				<?php if ( $dataBacth['iTerimaAd'] == 0  and (  in_array('AD', $manager) or in_array('AD', $team) )  ) {	
												  							$sql = "select * from plc2.plc2_master_satuan a where a.ldeleted=0 ";
																	    	$teams = $this->db->query($sql)->result_array();
																	    	$echo = '<select class="" name="iSatuan_ad[]" >';
																	    	$echo .= '<option value="">--Pilih--</option>';
																	    	foreach($teams as $t) {
												  		...
												  		<td class="cb">
												  				<?php if ( $dataBacth['iTerimaAd'] == 0  and (  in_array('AD', $manager) or in_array('AD', $team) )  ) {	?>
												  						<?php 
												  						 ...
												  						 ";
																	    	$teams = $this->db->query($sql)->result_array();
																	    	$echo = '<select  class="chosen-select cb2" name="cPenerima_ad[]" >';
																	    	$echo .= '<option value="">--Pilih--</option>';
																	    	foreach($teams as $t) {
																	    		$selected = $dataBacth['cPenerima_ad'] == $t['vnip'] ? 'selected' : '';
																	    		$echo .= '<option '.$selected.' value="'.$t['vnip'].'">'.$t['vName'].'</option>';
																	    	}
																	    	$echo .= '</select>';
												  				...
												  				?>
												  					<?php 
											  							...
											  							if (empty($dataBacth['cPenerima_ad'])) {
																    		$ret = '-';
																    	}else{
																    		$ret = $iam['vName'];
																    	}
												  		...
												  		<td>	
												  				<?php if ( ($dataBacth['iTerimaQa'] > 0)  ) {	
																	    	...
																	    	$ret .= '<input type="hidden" name="iSatuan_qa[]" class="angka" value="'.$dataBacth['iSatuan_qa'].'" size="8">';
														  					...
														  					$sql = "select * from plc2.plc2_master_satuan a where a.ldeleted=0 ";
																	    	$teams = $this->db->query($sql)->result_array();
																	    	$echo = '<input type="hidden" name="iSatuan_qa[]" class="angka" value="'.$dataBacth['iSatuan_qa'].'" size="8">';
