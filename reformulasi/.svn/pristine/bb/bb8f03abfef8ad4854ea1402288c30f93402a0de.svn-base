<?php
	...
	if ($act != 'create'){
		$ibk_id 			= $rowDataH['iexport_bk_id'];
		$rows 				= $this->db->get_where('reformulasi.group_file_upload', array('iDeleted' => 0, 'iM_modul_fileds' => $iM_modul_fileds, 'idHeader_File' => $ibk_id))->result_array();
	}
...
$("#<?php echo $tableId;?> .file_remove").click(function(){
	...
	var tmpDel = $("#export_bk_primer_del");
	li.remove();
	var v = tmpDel.val();
	v+=','+fileid;
	tmpDel.val( v );
	alert( $("#export_bk_primer_del").val() );
});
function del_export_bk_primer(dis, conname, table_id) {
	//if($('.'+conname).length > 1) {
		var j_p=0;
		var j_k=0;
		...
		if((j_p<=1)){
			_custom_alert("File Upload Primer Minimal 1","INFO","INFO", "export_bk_primer", 1, 20000);
			return false;
		}else{
...
function add_row_export_bk_primer(table_id){		
		//alert(table_id);
		...
		if (n.length == 0) {
			...
			row_content	 += '<span class="delete_btn"><a href="javascript:;" class="export_bk_primer_del" onclick="del_export_bk_primer(this, \'export_bk_primer_del\', \'export_bk_primer\')">[Hapus]</a></span></td>';		
			row_content  += '</tr>';
	    ...
	    $("#ijenis_bk_id"+c).change(function(){
			...
			url=base_url+'processor/reformulasi/v3/export/bk/primer?action=cekjnsbk';
			$.ajax({
				url: url,
				type: 'post',
				data: 'id='+$(this).val(),
...
<table class="hover_table" id="export_bk_primer" cellspacing="0" cellpadding="1" style="width: 98%; border: 1px solid #dddddd; text-align: center; margin-left: 5px; border-collapse: collapse">
	<thead>
	<tr style="width: 98%; border: 1px solid #dddddd; background: #548cb6; border-collapse: collapse">
		<th colspan="5" style="border: 1px solid #dddddd;"><span style="font-weight: bold; color: #ffffff; text-shadow: 0 1px 1px rgba(0, 0, 0, 0.3); text-transform: uppercase;"><?php echo $title; ?></span></th>
	...
	<tbody>
		...
		if((in_array('BD', $mydept))||(in_array('QA', $mydept))||(in_array('BIC', $mydept)) ||(in_array('PD', $mydept)) ||(in_array('PPC', $mydept))) {
			...
			if($value != '') {
				if (file_exists('./'.$vPath_upload.'/'.$idsp.'/'.$value)) {
					$link = base_url().'processor/reformulasi/v3/export/bk/primer?action=download&id='.$idsp.'&path='.$vPath_upload.'&file='.$value;
		...
		?>
				<tr style="border: 1px solid #dddddd; border-collapse: collapse; background: #ffffff; ">
					<td style="border: 1px solid #dddddd; width: 3%; text-align: center;">
						<span class="export_bk_primer_num"><?php echo $i ?></span>
					</td>
					...
					<td style="border: 1px solid #dddddd; width: 10%">
						<span class="delete_btn"><a href="javascript:;" class="export_bk_primer_del" onclick="del_export_bk_primer(this, 'export_bk_primer_del', 'export_bk_primer')">[Hapus]</a></span>
		...
		?>
				<tr style="border: 1px solid #dddddd; border-collapse: collapse; background: #ffffff; ">
					<td style="border: 1px solid #dddddd; width: 3%; text-align: center;">
						<span class="export_bk_primer_num">1</span>
					</td>
					<td style="border: 1px solid #dddddd; width: 4%; text-align: center;">
	...
	<tfoot>
		...
		<tr>
			...
			<td style="text-align: center"><a href="javascript:;" onclick="javascript:add_row_export_bk_primer('export_bk_primer')">Tambah</a></td>
		</tr><?php //}?>
	</tfoot>
</table>
...
<script>
	 $("#fileuploadbk1").change(function () {
	        var fileExtension = ["pdf","jpeg", "jpg", "png", "gif", "bmp"];
	        if ($.inArray($(this).val().split(".").pop().toLowerCase(), fileExtension) == -1) {
	        	_custom_alert("Only formats are allowed : "+fileExtension.join(", "),"info", "info", "export_bk_primer", 1, 20000);
	        	$(this).val('');
	        }
    	});
