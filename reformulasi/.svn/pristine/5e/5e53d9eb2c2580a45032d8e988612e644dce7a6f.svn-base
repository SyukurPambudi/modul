...
class v3_export_request_refor extends MX_Controller {
    function __construct() {
		...
		$this->title = 'Request Refor';
		$this->url = 'v3_export_request_refor';
		...
		$datagrid['islist'] = array(
			...
			// 'tAlasan_export' 			=> array('label' => 'Alasan Request Reformulasi','width'=>350,'align'=>'left','search'=>true),
			...
			'iApproval_ats_inisiator' 	=> array('label' => 'Approval',		'width' => 100,	'align' => 'center', 'search' => false),
		);		
		...
		$datagrid['shortBy']=array("iexport_req_refor"=>"DESC");
    ...
    function index($action = '') {
		...
		switch ($action) {
			...
			case 'updateproses':
                ...
                $dataFieldUpload=$this->lib_refor->getUploadFileFromField($this->input->get('modul_id'));

                if(count($dataFieldUpload) > 0 && $lastId != "0" && $lastId != ""){
                    foreach ($dataFieldUpload as $kf => $vUpload) { 
                        $pathf      = $vUpload['vPath_upload'];
                        $iddetails  = $this->url.'_'.$vUpload['iM_modul_fields'].'_iFile'; 
                        ...
                        if(count($validdetails) > 0){
                            $this->db_plc0->where('idHeader_File',$lastId)
                                            ...
                                            ->update('reformulasi.group_file_upload',$dataupdate);
                        }else{
                            $this->db_plc0->where('idHeader_File',$lastId)
                                            ->where('iM_modul_fields',$vUpload['iM_modul_fields'])
                                            ->update('reformulasi.group_file_upload',$dataupdate);
                        ...
                        $qq     = $this->db_plc0->get('reformulasi.group_file_upload');
			...
			case 'download':
                ...
                $files 	= file_get_contents('./'.$path.'/'.$id.'/'.$name); 
                ...
                if(count($dataFieldUpload) > 0){
                    foreach ($dataFieldUpload as $kf => $vUpload) {
                        ...
                        foreach($_POST as $key=>$value) {						
                            if ($key == 'reformulasi_'.$this->url.'_'.$vUpload['vNama_field']."_".$vUpload['iM_modul_fields'].'_fileketerangan') {
                        ...
                        $i=0;
                        foreach ($_FILES['reformulasi_'.$this->url.'_'.$vUpload['vNama_field']."_".$vUpload['iM_modul_fields'].'_upload_file']["error"] as $key => $error) {
                            if ($error == UPLOAD_ERR_OK) {
                                $tmp_name           = $_FILES['reformulasi_'.$this->url.'_'.$vUpload['vNama_field']."_".$vUpload['iM_modul_fields'].'_upload_file']["tmp_name"][$key];
                                $name               = $_FILES['reformulasi_'.$this->url.'_'.$vUpload['vNama_field']."_".$vUpload['iM_modul_fields'].'_upload_file']["name"][$key];
                                $data['filename']   = $name;
                                $data['dInsertDate']= date('Y-m-d H:i:s');
                                $filenameori        = $name;
                                $now_u              = date('Y_m_d__H_i_s');
                                $name_generate      = $this->lib_refor->generateFilename($name, $i);
                                ...
                                if(move_uploaded_file($tmp_name, $path."/".$lastId."/".$name_generate)) {
                                    ...
                                    $this->db_plc0->insert('reformulasi.group_file_upload',$datainsert);
                                    $i++;	
	...
	/*List Box*/
	function listBox_v3_export_request_refor_cPIC_AD($value, $pk, $name, $rowData) {
	...
	function listBox_v3_export_request_refor_dossier_upd_iteampd_id($value, $pk, $name, $rowData) {
	...
	function insertBox_v3_export_request_refor_form_detail($field,$id){
            $get=$this->input->get();
            $post=$this->input->post();
        ...
        function updateBox_v3_export_request_refor_form_detail($field,$id,$value,$rowData){
            $get=$this->input->get();
            $post=$this->input->post();
            foreach ($get as $kget => $vget) {
                if($kget!="action"){
    ...
    /*After Insert*/
    function after_insert_processor($row,$id, $postData) {
        ...
        $iupb_id[]=$this->lib_refor->getUpbId($peka,$id);
        $modul_id=$this->input->get("modul_id");
        $iKey_id=$id;
        $iM_activity=0;
        $iSort=1;
        $activities = $this->lib_refor->get_current_module_activities($this->modul_id,$id);
        foreach ($activities as $kk => $vv) {
        	$iM_activity=isset($vv['iM_activity'])?$vv['iM_activity']:$iM_activity;
        }
        if($postData['iSubmit']){
        	$this->lib_refor->InsertActivityModule($iupb_id,$modul_id,$iKey_id,$iM_activity,$iSort);
        }
    }
    ...
    function confirm_view() { 
     ...
     function confirm_process() {
        ...
        $peka=$this->main_table_pk;
        $iupb_id[]=$this->lib_refor->getUpbId($peka,$pk);

        $this->lib_refor->InsertActivityModule($iupb_id,$modul_id,$pk,$activity['iM_activity'],$activity['iSort'],$vRemark,2);
    ...
    function approve_view($id=0) { 
     ...
     function approve_process() {
        ...
        $this->lib_refor->InsertActivityModule($iupb_id,$modul_id,$pk,$activity['iM_activity'],$activity['iSort'],$vRemark,$iapprove);
    ...
    function getFormDetail(){
        ...
        $data["html"] .= $hate_emel; //print_r($hate_emel);exit();
        return json_encode($data);
    }

    /*function getMax(){
	$iexport_request_sample_detail=$_POST['iexport_request_sample_detail'];
	$data = array();
    ...
    function get_data_prev(){
        ...
        $q 			= $this->db->get_where('reformulasi.group_file_upload', array('iDeleted' => 0, 'idHeader_File' => $post["id"], 'iM_modul_fields' => $namefield));
        ...
        foreach ($q->result() as $k) {
            ...
            if($value != '') {
                if (file_exists('./'.$filepath.'/'.$id.'/'.$value)) {
                    $link       = base_url().'processor/'.$this->urlpath.'?action=download&id='.$id.'&file='.$value.'&path='.$filepath;
                    $linknya    = '<a class="ui-button-text" href="javascript:;" onclick="window.location=\''.$link.'\'">[Download]</a>&nbsp;&nbsp;&nbsp;';
                }
            }

            $linknya = $linknya.'<a class="ui-button-text" href="javascript:;" onclick="javascript:hapus_row_'.$nmTable.'('.$i.')">[Hapus]</a><input type="hidden" class="num_rows_'.$nmTable.'" value="'.$i.'" /><input type="hidden" name="'.$this->url.'_'.$post["namefield"].'_iFile[]" value="'.$k->iFile.'" />';
