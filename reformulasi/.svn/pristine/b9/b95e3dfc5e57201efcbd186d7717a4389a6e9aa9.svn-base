...
class v3_export_skala_trial_kimiawi extends MX_Controller {
    function __construct() {
		...
		$this->url = 'v3_export_skala_trial_kimiawi';
		...
		$datagrid['islist'] = array(
			...
			'isubmit_kimiawi'                       => array('label' => 'Submit',					'width' => 150,	'align' => 'left',		'search'=>true),
            'iappad_kimiawi'                        => array('label' => 'Approval Andev',           'width' => 150, 'align' => 'left',      'search'=>true),
		);		

		$datagrid['setQuery'] = array(
			0 => array('vall' => 'export_req_refor.lDeleted', 		'nilai' => 0),
			1 => array('vall' => 'dossier_upd.lDeleted', 			'nilai' => 0),
            1 => array('vall' => 'export_skala_trial.isubmit_fisik','nilai' => 1),
		);
    ...
    function index($action = '') {
		...
		$grid->changeFieldType('isubmit_kimiawi', 'combobox','',array('' => '--select--', 0 => 'Draft - Need to be Submit', 1 => 'Submited'));
	...
	/*List Box*/
	function listBox_v3_export_skala_trial_kimiawi_cPIC_AD($value, $pk, $name, $rowData) {
	...
	function listBox_v3_export_skala_trial_kimiawi_dossier_upd_iteampd_id($value, $pk, $name, $rowData) {
	...
	function insertBox_v3_export_skala_trial_kimiawi_form_detail($field,$id){
            $get=$this->input->get();
            $post=$this->input->post();
        ...
        function updateBox_v3_export_skala_trial_kimiawi_form_detail($field,$id,$value,$rowData){
            $get=$this->input->get();
            $post=$this->input->post();
            foreach ($get as $kget => $vget) {
                if($kget!="action"){
