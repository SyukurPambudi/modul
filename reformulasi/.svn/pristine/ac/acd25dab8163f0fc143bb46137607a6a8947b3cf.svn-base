...
class v3_export_skala_trial_kesimpulan extends MX_Controller {
    function __construct() {
        ...
        $this->title = 'Skala Trial - Kesimpulan Trial';
        $this->url = 'v3_export_skala_trial_kesimpulan';
        $this->urlpath = 'reformulasi/'.str_replace("_","/", $this->url);

        $this->maintable = 'reformulasi.export_skala_trial';    
        $this->main_table = $this->maintable;   
        $this->main_table_pk = 'iexport_skala_trial';   
        ...
        $datagrid['islist'] = array(
            ...
            'isubmit_kesimpulan'                    => array('label' => 'Submit',                   'width' => 150, 'align' => 'left',      'search'=>true),
            'iapppd_kesimpulan'                     => array('label' => 'Approval PD',              'width' => 150, 'align' => 'left',      'search'=>true),
        );      

        $datagrid['setQuery'] = array(
            0 => array('vall' => 'export_req_refor.lDeleted',       'nilai' => 0),
            1 => array('vall' => 'dossier_upd.lDeleted',            'nilai' => 0),
            2 => array('vall' => 'export_skala_trial.isubmit_fisik','nilai' => 1),
            3 => array('vall' => 'export_skala_trial.isubmit_kimiawi','nilai' => 1),
            4 => array('vall' => 'export_skala_trial.iappad_kimiawi','nilai' => 2),
        );

        $datagrid['jointableinner'] = array(
            0 => array('reformulasi.export_req_refor'   => 'export_skala_trial.iexport_req_refor = export_req_refor.iexport_req_refor'),
            1 => array('dossier.dossier_upd'            => 'export_req_refor.idossier_upd_id = export_req_refor.idossier_upd_id'),
        );

        $datagrid['shortBy'] = array("export_skala_trial.iexport_skala_trial" => "DESC");
    ...
    function index($action = '') {
        ...
        $grid->changeFieldType('isubmit_kesimpulan', 'combobox','',array('' => '--select--', 0 => 'Draft - Need to be Submit', 1 => 'Submited'));
    ...
    /*List Box*/
    function listBox_v3_export_skala_trial_kesimpulan_cPIC_AD($value, $pk, $name, $rowData) {
    ...
    function listBox_v3_export_skala_trial_kesimpulan_dossier_upd_iteampd_id($value, $pk, $name, $rowData) {
    ...
    function insertBox_v3_export_skala_trial_kesimpulan_form_detail($field,$id){
            $get=$this->input->get();
            $post=$this->input->post();
        ...
        function updateBox_v3_export_skala_trial_kesimpulan_form_detail($field,$id,$value,$rowData){
            $get=$this->input->get();
            $post=$this->input->post();
            foreach ($get as $kget => $vget) {
                if($kget!="action"){
    ...
    function before_update_processor($row, $postData) {
        $activity            = $this->db->get_where('erp_privi.m_modul_activity', array('iM_modul_activity'=>$postData['iM_modul_activity'], 'lDeleted'=>0))->row_array();
        $field_submit        = $activity['vFieldName'];

        $postData['dupdate'] = date('Y-m-d H:i:s');
        $postData['cupdate'] = $this->user->gNIP;

        if($postData['isdraft'] == true){
            $postData[$field_submit] = 0;
        } else{
            $postData[$field_submit] = 1;
        }
    ...
    function after_update_processor($fields, $id, $postData) { 
        $activity = $this->db->get_where('erp_privi.m_modul_activity', array('iM_modul_activity'=>$postData['iM_modul_activity'], 'lDeleted'=>0))->row_array();
        $modul_id = $this->modul_id;
        if ($postData[$activity['vFieldName']] == 1){
            $this->lib_refor->InsertActivityModule($this->ViewUPD($id), $modul_id, $id, $activity['iM_activity'], $activity['iSort']);
        }
    }
    ...
    function approve_view($id=0) {
                     ...
                     function submit_ajax(form_id) {
                        return $.ajax({
                            ...
                            success: function(data) {
                                ...
                                if(o.status == true) { 
                                    $("#alert_dialog_form").dialog("close");
                                         $.get(url+"?action=view&id="+last_id+"&foreign_key=0&company_id=3&group_id="+group_id+"&modul_id="+modul_id, function(data) {
                                         $("div#form_'.$this->url.'").html(data);
     ...
     function approve_process() {
        ...
        if ($iapprove == 1){
            ...
            $sqlAct     = $this->db->get_where('erp_privi.m_modul_activity', array('lDeleted' => 0, 'iM_modul' => $activity['iM_modul']))->result_array();
            foreach ($sqlAct as $act) {
                $vFieldName = $act['vFieldName'];
                if (strlen($vFieldName) > 0){
                    $this->db->where($this->main_table_pk, $pk);
                    $this->db->update($this->main_table, array($vFieldName => 0));
                } else {
                    $this->db->where($this->main_table_pk, $pk);
                    $this->db->update($this->main_table, array('iSubmit' => 0));
                }
            }
            ...
            $this->db->update('erp_privi.m_modul_log_activity', array('lDeleted' => 1));
        }
    ...
    function getFormDetail(){
        ...
        $hate_emel .= '
            <table class="hover_table" style="width:99%; border: 1px solid #dddddd; text-align: center; margin-left: 5px; border-collapse: collapse" cellspacing="0" cellpadding="1">
                ...
                $hate_emel .= $this->lib_refor->getHistoryActivity($this->modul_id,$aidi,true);
        ...
        if(!empty($dFields)){
            ...
            foreach ($dFields as $form_field) {
                ...
                if ($form_field['vRequirement_field']<> "") {
                 ...
                 if ($form_field['iM_jenis_field'] == 8 && $form_field['vNama_field'] == 'formulasi_kesimpulan_trial'){
                    $hate_emel .=' </label>
