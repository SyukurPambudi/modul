...
class master_alasan_refor extends MX_Controller {
    ...
    function index($action = '') {
        ...
        $grid = new Grid;
        $grid->setTitle('Master Alasan Reformulasi');
        $grid->setTable('reformulasi.master_alasan_refor');      
        $grid->setUrl('master_alasan_refor');



        //List Table
        $grid->addList('nama_alasan','vKeterangan','iStatus'); 
        $grid->setSortBy('nama_alasan','vKeterangan','iStatus');
        $grid->setSortOrder('DESC');  

        //List field
        $grid->addFields('nama_alasan','vKeterangan','iStatus'); 
        ...
        */
        ...
        $grid->setWidth('nama_alasan', '350');
        $grid->setAlign('nama_alasan', 'left');
        $grid->setLabel('nama_alasan','Nama Alasan Reformulasi');
        
        $grid->setWidth('vKeterangan', '325');
...
//Example modifikasi GRID ERP
    //- Set Query
        $grid->setQuery('master_alasan_refor.lDeleted = 0 ', null); 
        /*if ($this->validation) {
...
*/
    ...
    //set search
        ...
        $grid->setSearch('nama_alasan','vKeterangan','iStatus');
        
    //set required
        $grid->setRequired('nama_alasan','vKeterangan','iStatus','dCreate','cCreated','dUpdate','cUpdate','lDeleted'); 
        ...
        switch ($action) {
                ...
                case 'createproses':
                    /*$post=$this->input->post();print_r($post);exit();*/
                      ...
                      //  $nama_alasan = ['nama_alasan'];
                        $name = $post['master_alasan_refor_nama_alasan'];
                        $cek  = $this->db->get_where('reformulasi.master_alasan_refor', array('nama_alasan' => $name, 'lDeleted' => 0))->num_rows(); 
                        if ($cek > 0){
                            $data['status']  = false;
            ...
            case 'updateproses':
                        $post   = $this->input->post(); 
                        $id     = $post['master_alasan_refor_id_alasan'];
                        $name   = $post['master_alasan_refor_nama_alasan']; 
                        //$sql    = 'SELECT * FROM asset.master_kategori WHERE id <> ? AND vName = ? AND lDeleted = 0';
                        $sql    = 'SELECT * FROM reformulasi.master_alasan_refor WHERE id_alasan <> '.$id.' AND nama_alasan = "'.$name.'" AND lDeleted = 0';   
                        //echo ($sql);exit();
    ...
    */
    /*function updateBox_master_pengujian_refor_id_bentuk_sediaan($field, $id, $value, $rowData) {
        if ($this->input->get('action') == 'view') { 
            $sql='select * from reformulasi.master_alasan_refor a where a.id_alasam="'.$value.'" ';
    ...
    function manipulate_insert_button($buttons) { 
     ...
     /*$cNip= $this->user->gNIP;
      $cNip= $this->user->gNIP;
        ...
        $AuthModul = $this->lib_preventive->getAuthorModul($this->modul_id);
