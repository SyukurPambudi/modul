...
class v2_exp_terima_sample extends MX_Controller {
    function __construct() {
		...
		$this->title 		= 'Penerimaan Sample Bahan Baku';
		$this->url 			= 'v2_exp_terima_sample';
		$this->urlpath 		= 'reformulasi/'.str_replace("_","/", $this->url);

		$this->maintable 	= 'reformulasi.export_terima_sample';	
		$this->main_table 	= $this->maintable;	
		$this->main_table_pk= 'iexport_terima_sample';	
		
		$datagrid['islist'] = array(
			'lpb_header.cLPB_Number' 	=> array('label' => 'Nomor Penerimaan',	'width' => 100,	'align' => 'center', 'search' => true),
			'warehouse.vWareHouse'		=> array('label' => 'Warehouse', 		'width' => 150,	'align' => 'center', 'search' => false),
			'poir_header.cPONumber'		=> array('label' => 'Nomor PO', 		'width' => 100,	'align' => 'center', 'search' => false),
			'supplier.vSupplierName' 	=> array('label' => 'Supplier',			'width' => 250,	'align' => 'center', 'search' => false),
		...
		$datagrid['setQuery']=array(
			0 => array('vall' => 'lpb_header.lDeleted', 		'nilai' => 0),
			1 => array('vall' => 'warehouse.lDeleted', 			'nilai' => 0),
			2 => array('vall' => 'supplier.lDeleted', 			'nilai' => 0),
			3 => array('vall' => 'lpb_detail.lDeleted', 		'nilai' => 0),
			4 => array('vall' => 'poir_header.lDeleted', 		'nilai' => 0),
			5 => array('vall' => 'inreq_header.lDeleted', 		'nilai' => 0),
		);

		$datagrid['jointableinner'] = array(
			0 => array('pd_source.lpb_header' 	=> 'export_terima_sample.ilpb_id = lpb_header.id'),
			1 => array('pd_source.warehouse' 	=> 'lpb_header.iWareHouseID = warehouse.Id'),
			2 => array('pd_source.supplier' 	=> 'lpb_header.iSupplierId = supplier.Id'),
			3 => array('pd_source.lpb_detail' 	=> 'lpb_header.cLPB_Number = lpb_detail.cLPB_Number'),
			4 => array('pd_source.poir_header' 	=> 'lpb_detail.cPONumber = poir_header.cPONumber'),
			5 => array('pd_source.inreq_header' => 'lpb_detail.cRequestNo = inreq_header.cRequestNo'),
			6 => array('dossier.dossier_upd' 	=> 'inreq_header.iSampleID = dossier_upd.idossier_upd_id'),
		);

		$datagrid['shortBy'] = array("export_terima_sample.iexport_terima_sample" => "DESC");
			...
			case 'updateproses':
                ...
                if(count($dataFieldUpload) > 0 && $lastId != "0" && $lastId != ""){
                    foreach ($dataFieldUpload as $kf => $vUpload) {
                        ...
                        $this->db->where($where);
                        $qq     = $this->db->get($row2['vTable_file']);
    ...
    function getFormDetail(){
        ...
        if(!empty($dFields)){
            ...
            foreach ($dFields as $form_field) {
                ...
                if ($form_field['iM_jenis_field'] == 8 && $get['formaction']=='update'){
        ...
        function listBox_v2_exp_terima_sample_iapppr($value) {
        ...
        function listBox_v2_exp_terima_sample_cRequestor($value) {
        ...
        function insertBox_v2_exp_terima_sample_form_detail($field,$id){
            $get=$this->input->get();
            $post=$this->input->post();
        ...
        function updateBox_v2_exp_terima_sample_form_detail($field,$id,$value,$rowData){
            $get=$this->input->get();
            $post=$this->input->post();
            foreach ($get as $kget => $vget) {
                if($kget!="action"){
            ...
            function approve_view() { 
                             ...
                             function submit_ajax(form_id) {
                                return $.ajax({
                                    ...
                                    success: function(data) {
                                        ...
                                        var url = "'.base_url().'processor/reformulasi/v2_exp_terima_sample";                             
                                        if(o.status == true) { 
                                            $("#alert_dialog_form").dialog("close");
                                                 $.get(url+"?action=update&id="+last_id+"&foreign_key=0&company_id=3&group_id="+group_id+"&modul_id="+modul_id, function(data) {
                                                 $("div#form_v2_exp_terima_sample").html(data);
                                                 
                                            });
                                            
                                        }
                                            reload_grid("grid_v2_exp_terima_sample");
                                    }
                ...
                $echo .= '<form id="form_v2_exp_terima_sample_approve" action="'.base_url().'processor/reformulasi/v2_exp_terima_sample?action=approve_process" method="post">';
                ...
                <button type="button" onclick="submit_ajax(\'form_v2_exp_terima_sample_approve\')">Approve</button>';
            ...
            function confirm_view() { 
                ...
                $echo .= '<form id="form_v2_exp_terima_sample_confirm" action="'.base_url().'processor/reformulasi/v2_exp_terima_sample?action=confirm_process" method="post">';
                $echo .= '<div style="vertical-align: top;">';
                $echo .= 'Remark : 
                        <input type="hidden" name="'.$this->main_table_pk.'" value="'.$this->input->get($this->main_table_pk).'" />
                        <input type="hidden" name="modul_id" value="'.$this->input->get('modul_id').'" />
                        ...
                        <textarea name="vRemark"></textarea>
                <button type="button" onclick="submit_ajax(\'form_v2_exp_terima_sample_confirm\')">Confirm</button>';
            ...
            function reject_view() {
                $echo = '<script type="text/javascript">
                             function submit_ajax(form_id) {
                                var remark = $("#v2_exp_terima_sample_remark").val();
                                if (remark=="") {
                ...
                $echo .= '<form id="form_v2_exp_terima_sample_reject" action="'.base_url().'processor/reformulasi/v2_exp_terima_sample?action=reject_process" method="post">';
                $echo .= '<div style="vertical-align: top;">';
                $echo .= 'Remark : 
                        <input type="hidden" name="'.$this->main_table_pk.'" value="'.$this->input->get($this->main_table_pk).'" />
                        <input type="hidden" name="modul_id" value="'.$this->input->get('modul_id').'" />
                        ...
                        <textarea name="vRemark" id="reject_v2_exp_terima_sample_remark"></textarea>
                <button type="button" onclick="submit_ajax(\'form_v2_exp_terima_sample_reject\')">Reject</button>';
    ...
    function after_insert_processor($fields, $id, $postData) { 
        ...
        $sqlRaw 	= 'SELECT d.id FROM pd_source.lpb_header h 
						JOIN pd_source.lpb_detail d ON h.cLPB_Number = d.cLPB_Number
						WHERE h.id = ? AND d.lDeleted = 0';
		$dataRaw 	= $this->db->query($sqlRaw, array($postData['ilpb_id']))->result_array();

		foreach ($dataRaw as $raw) {
			$insRaw['iexport_terima_sample'] 	= $id;
			$insRaw['ilpb_detail_id'] 			= $raw['id'];
        	$insRaw['dcreate']					= date('Y-m-d H:i:s');
        	$insRaw['ccreate']					= $this->user->gNIP;
        	$insRaw['ldeleted']					= 0;
        	$this->db->insert('reformulasi.export_terima_sample_detail', $insRaw);
		}
    ...
    function after_update_processor($fields, $id, $postData) { //print_r(json_encode($postData));exit();
        ...
        $idDetails 	= $post['id_details'];
        $tanggalPD 	= $post['tanggal_pd'];
        $detailPD 	= $post['detail_pd'];
        $tanggalAD 	= $post['tanggal_ad'];
        $detailAD 	= $post['detail_ad'];
        $tanggalQA 	= $post['tanggal_qa'];
        $detailQA 	= $post['detail_qa'];

        foreach ($idDetails as $index => $idDetail) {
        	$rawUpdate['dtgl_kirim_pd'] 	= $tanggalPD[$index];
        	$rawUpdate['tdetail_pd'] 		= $detailPD[$index];
        	$rawUpdate['dtgl_terima_ad'] 	= $tanggalAD[$index];
        	$rawUpdate['tdetail_ad']	 	= $detailAD[$index];
        	$rawUpdate['dtgl_terima_qa'] 	= $tanggalQA[$index];
        	$rawUpdate['tdetail_qa']	 	= $detailQA[$index];
	        $rawUpdate['dupdate'] 			= date('Y-m-d H:i:s');
	        $rawUpdate['cupdate'] 			= $this->user->gNIP;
        	$this->db->where('iexport_terima_sample_detail', $idDetail);
            $this->db->update('reformulasi.export_terima_sample_detail', $rawUpdate);
        }
    ...
    function manipulate_insert_buttonx($buttons) { 
        ...
        $iframe = '<iframe name="v2_exp_terima_sample_frame" id="v2_exp_terima_sample_frame" height="0" width="0"></iframe>';
        ...
        $save_draft = '<button onclick="javascript:save_draft_btn_multiupload(\'v2_exp_terima_sample\', \' '.base_url().'processor/reformulasi/v2_exp_terima_sample?draft=true&modul_id='.$this->input->get('modul_id').'&iM_modul_activity='.$act['iM_modul_activity'].' \',this,true )"  id="button_save_draft_v2_exp_terima_sample"  class="ui-button-text icon-save" >Save as Draft</button>';
        $save = '<button onclick="javascript:save_btn_multiupload(\'v2_exp_terima_sample\', \' '.base_url().'processor/reformulasi/v2_exp_terima_sample?company_id='.$this->input->get('company_id').'&modul_id='.$this->input->get('modul_id').'&iM_modul_activity='.$act['iM_modul_activity'].'&group_id='.$this->input->get('group_id').'modul_id='.$this->input->get('modul_id').' \',this,false )"  id="button_save_submit_v2_exp_terima_sample"  class="ui-button-text icon-save" >Save &amp; Submit</button>';
    ...
    function manipulate_update_button($buttons, $rowData) { 
        ...
        if ($this->input->get('action') == 'view') {
            unset($buttons['update']);
                ...
                if($isOpenEditing){
                    $update_draft = '<button onclick="javascript:update_draft_btn(\'v2_exp_terima_sample\', \' '.base_url().'processor/reformulasi/v2_exp_terima_sample?draft=true \',this,true )"  id="button_update_draft_v2_exp_terima_sample"  class="ui-button-text icon-save" >Update open Editing</button>';
                    ...
                    foreach ($activities as $act) {
                        $update_draft = '<button onclick="javascript:update_draft_btn(\'v2_exp_terima_sample\', \' '.base_url().'processor/reformulasi/v2_exp_terima_sample?draft=true&modul_id='.$this->input->get('modul_id').'&iM_modul_activity='.$act['iM_modul_activity'].' \',this,true )"  id="button_update_draft_v2_exp_terima_sample"  class="ui-button-text icon-save" >Update as Draft</button>';
                        $update = '<button onclick="javascript:update_btn_back(\'v2_exp_terima_sample\', \' '.base_url().'processor/reformulasi/v2_exp_terima_sample?company_id='.$this->input->get('company_id').'&modul_id='.$this->input->get('modul_id').'&iM_modul_activity='.$act['iM_modul_activity'].'&group_id='.$this->input->get('group_id').'modul_id='.$this->input->get('modul_id').' \',this,false )"  id="button_update_submit_v2_exp_terima_sample"  class="ui-button-text icon-save" >Update &amp; Submit</button>';

                        $approve = '<button onclick="javascript:load_popup(\' '.base_url().'processor/reformulasi/v2_exp_terima_sample?action=approve&iM_modul_activity='.$act['iM_modul_activity'].'&iM_activity='.$act['iM_activity'].'&'.$this->main_table_pk.'='.$peka.'&iupb_id='.$iupb_id.'&company_id='.$this->input->get('company_id').'&group_id='.$this->input->get('group_id').'&modul_id='.$this->input->get('modul_id').' \')"  id="button_approve_v2_exp_terima_sample"  class="ui-button-text icon-save" >Approve</button>';
                        $reject = '<button onclick="javascript:load_popup(\' '.base_url().'processor/reformulasi/v2_exp_terima_sample?action=reject&iM_modul_activity='.$act['iM_modul_activity'].'&iM_activity='.$act['iM_activity'].'&'.$this->main_table_pk.'='.$peka.'&iupb_id='.$iupb_id.'&company_id='.$this->input->get('company_id').'&group_id='.$this->input->get('group_id').'&modul_id='.$this->input->get('modul_id').' \' )"  id="button_reject_v2_exp_terima_sample"  class="ui-button-text icon-save" >Reject</button>';

                        $confirm = '<button onclick="javascript:load_popup(\' '.base_url().'processor/reformulasi/v2_exp_terima_sample?action=confirm&iM_modul_activity='.$act['iM_modul_activity'].'&iM_activity='.$act['iM_activity'].'&'.$this->main_table_pk.'='.$peka.'&iupb_id='.$iupb_id.'&company_id='.$this->input->get('company_id').'&group_id='.$this->input->get('group_id').'&modul_id='.$this->input->get('modul_id').' \')"  id="button_approve_v2_exp_terima_sample"  class="ui-button-text icon-save" >Confirm</button>';

                        
                        $iujiMikro 	= $rowData['iuji_mikro'];
                        $iSort 		= $act['iSort'];

                        switch ($act['iType']) {
                            case '1':
                                # Update
                                $sButton .= ( $iSort == 3 && $iujiMikro == 0 ) ? '' : $update_draft.$update;
                                break;
                            case '2':
                                # Approval
