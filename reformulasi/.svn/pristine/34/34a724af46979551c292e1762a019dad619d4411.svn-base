			...
			if(isUpload && !isValidAFileSize('#form_create_'+grid+' input.multifile', uploadLimit)) {
				...
				$.ajax({
					...
					success: function(data) {	
						...
						if(o.status == true) {
							if(isUpload) {
								var iframe = $('<iframe name="export_po_sample_frame"/>');
								iframe.attr({'id':'export_po_sample_frame'});
								...
								$('#form_create_'+grid).attr('target','export_po_sample_frame');								
							...
							$.get(base_url+'processor/reformulasi/export/po/sample?action=update&foreign_key=0&company_id='+company_id+'&id='+last_id+'&group_id='+group_id+'&modul_id='+modul_id, function(data) {
			...
			//if(!isUpload) {
				...
				success: function(data) {
					...
					if(isUpload) {
						...
						$('#form_update_'+grid).attr('target','export_po_sample_frame');								
			...
			//if(!isUpload) {
				...
				.done(function(data) {
					...
					$('#grid_'+grid).trigger('reloadGrid');
						...
						$.get(base_url+'processor/reformulasi/export/po/sample?action=update&id='+last_id+'&foreign_key='+foreign_id+'&company_id='+company_id+'&group_id='+group_id+'&modul_id='+modul_id, function(data) {
	                            $('div#form_'+grid).html(data);
	                            $('html, body').animate({scrollTop:$('#'+grid).offset().top - 20}, 'slow');
	                    });
				});
...
<iframe name="export_po_sample_frame" id="export_po_sample_frame" height="0" width="0"></iframe>
