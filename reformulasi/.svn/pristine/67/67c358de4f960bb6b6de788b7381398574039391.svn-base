...
class browse_skala_trial extends MX_Controller {
    ...
    function index($action = '') {
		...
		$grid->setUrl('browse_skala_trial');
        ...
        #Cek Maping Skala Trial 
        $grid->setQuery('IF(`lokal_mapping_refor`.`imaster_proses_refor`=4,
            #Cek Melewati literatur
             ...
             AND lm.`imaster_proses_refor` = 1 AND lm.`iLocal_req_refor` = `local_req_refor`.`iLocal_req_refor`)>0,
                ...
                #Kalo Tidak Kembali ke Kondisi Awal Tanpa Pengecakan
                 `lokal_mapping_refor`.`imaster_proses_refor`=4),
              
              #Kalo Tidak Kembali ke Kondisi Awal Tanpa Pengecakan
             `lokal_mapping_refor`.`imaster_proses_refor`=4)  ', null); 
        
        #cek Ketersediaan Data
        $grid->setQuery('local_req_refor.iLocal_req_refor NOT IN(
            SELECT rl.`iLocal_req_refor` FROM reformulasi.`lokal_refor_skala_trial` rl WHERE 
            rl.`iLocal_req_refor` = local_req_refor.iLocal_req_refor
            AND IF(rl.`iappd`=2,IF(rl.`iksm_skala_trial`=0,rl.`iksm_skala_trial`=1,rl.`lDeleted`=0),rl.`lDeleted`=0) 
	...
	function listBox_browse_skala_trial_pilih($value, $pk, $name, $rowData) {
		...
		$o = '<input type="radio" name="pilih" onClick="javascript:pilih_upb_detail('.$pk.',\''.$rowData->cIteno.'\',
                ...
                \''.$cdapart['vDescription'].'\',\''.$c_pd['vteam'].'\', \''.$app['vName'].'\', \''.$formu['vName'].'\', \''.$rowData->cFormulator.'\'
                ) ;" />
			  <script type="text/javascript">
				function pilih_upb_detail (id, cIteno, vNo_req_refor, vBatch_no, vAlasan_refor, 
                    dRequest, _cIteno, _cInisiator, _cdapart, _iteam_pd, _iStatus, _cFormulator, cFormulator){					
					custom_confirm("Yakin ?", function(){
                        ...
                        $(".'.$this->input->get('field').'_cpic_skala_trial").val(cFormulator);
