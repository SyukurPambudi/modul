<?php
$nmTable="table_file_study_details_edit";
$pager="pager_".$nmTable;
$caption = "File Study Literatur";
$gridurl=base_url()."processor/reformulasi/local_study_literatur_pd?action=getdetailsMR&id=".$get['id']."&acc=".$get['action'];
   ...
   <?php if($rowData['iapppd']==0 || $get['action']!='view'){ ?>
            .navButtonAdd('#<?php echo $pager; ?>',{
               ...
               onClickButton: function(){ 
                 addrow_detail_files_study_edit();
               }, 
               position:"last"
            })
    <?php 
    ...
    function addrow_detail_files_study_edit(){
        //get last num rows
        var n=[];
        var i=0;
        $.each($(".num_rows_details_file_study_edit"),function(){
             /*if(i==0){
                n[i]=$(this).val();
            }else{
                n[i]=','+$(this).val();
        ...
        var sa=[["<input type='file' id='fileupload_local_study_pd_"+rlast+"' class='fileupload multi multifile' name='fileupload_local_study_pd[]' style='width: 90%' />","<textarea id='file_keterangan_local_study_"+rlast+"' name='file_keterangan_local_study[]' style='width: 240px; height: 50px;'></textarea>","<input type='hidden' class='num_rows_details_file_study_edit' value='"+rlast+"' /><button id='ihapus_<?php echo $nmTable ?>' class='ui-button-text icon_hapus' style='width:75px' onclick='javascript:hapus_row_details_file_study_edit("+rlast+")' type='button'>Hapus</button>"]];
    ...
    function hapus_row_details_file_study_edit(rowId){
