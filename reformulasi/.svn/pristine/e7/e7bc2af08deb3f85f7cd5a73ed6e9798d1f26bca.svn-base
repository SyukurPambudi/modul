...
class v2_exp_confirm_refor extends MX_Controller {
    function __construct() {
		...
		$this->title 		= 'Confirm Request Reformulasi';
		$this->url 			= 'v2_exp_confirm_refor';
		$this->urlpath 		= 'reformulasi/'.str_replace("_","/", $this->url);

		$this->maintable 	= 'reformulasi.export_req_refor';	
		$this->main_table 	= $this->maintable;	
		$this->main_table_pk= 'iexport_req_refor';	
		
		$datagrid['islist'] = array(
            'vno_export_req_refor'     => array('label' => 'No Request',	'width' => 100,	'align' => 'center', 'search' => true),
            'dPermintaan_req_export' 	=> array('label' => 'Tgl Request',	'width' => 100,	'align' => 'center', 'search' => true),
			'cInisiator_export' 		=> array('label' => 'Inisiator',	'width' => 110,	'align' => 'center', 'search' => true),
			'dossier_upd.vNama_usulan' 	=> array('label' => 'Nama Usulan',	'width' => 350,	'align' => 'left',   'search' => true),
            'iSubmitPD' 	            => array('label' => 'Submit PD',	'width' => 150,	'align' => 'center', 'search' => false),
            
		);		

		$datagrid['setQuery']=array(
            0 => array('vall' => 'export_req_refor.lDeleted', 'nilai' => 0),
            1 => array('vall' => 'export_req_refor.iApproval_ats_inisiator', 'nilai' => 2),
			
		);

		$datagrid['jointableinner'] = array(
			0 => array('dossier.dossier_upd' => 'dossier_upd.idossier_upd_id = export_req_refor.idossier_upd_id'),
		);

		$datagrid['shortBy']=array("export_req_refor.iexport_req_refor" => "DESC");
    ...
    function get_data_prev(){
        ...
        function listBox_v2_exp_confirm_refor_iapppr($value) {
        ...
        function insertBox_v2_exp_confirm_refor_form_detail($field,$id){
            $get=$this->input->get();
            $post=$this->input->post();
        ...
        function updateBox_v2_exp_confirm_refor_form_detail($field,$id,$value,$rowData){
            $get=$this->input->get();
            $post=$this->input->post();
            foreach ($get as $kget => $vget) {
                if($kget!="action"){
            ...
            function confirm_view() { 
                             ...
                             function submit_ajax(form_id) {
                                return $.ajax({
                                    ...
                                    success: function(data) {
                                        ...
                                        var url = "'.base_url().'processor/reformulasi/v2_exp_confirm_refor";                             
                                        if(o.status == true) { 
                                            $("#alert_dialog_form").dialog("close");
                                                 $.get(url+"?action=update&id="+last_id+"&foreign_key=0&company_id=3&group_id="+group_id+"&modul_id="+modul_id, function(data) {
                                                 $("div#form_v2_exp_confirm_refor").html(data);
                                                 
                                            });
                                            
                                        }
                                            reload_grid("grid_v2_exp_confirm_refor");
                                    }
                ...
                $echo .= '<form id="form_v2_exp_confirm_refor_approve" action="'.base_url().'processor/reformulasi/v2_exp_confirm_refor?action=confirm_process" method="post">';
                ...
                <button type="button" onclick="submit_ajax(\'form_v2_exp_confirm_refor_approve\')">Confirm</button>';
            ...
            function confirm_process() {
                ...
                $sqlHead = "SELECT s.`vRequest_no`, s.iexport_request_sample ,b.*
                    ...
                    JOIN reformulasi.export_req_refor b ON b.iexport_req_refor=s.iexport_req_refor
                    WHERE s.`lDeleted` = 0 
                    AND s.`iTujuan_request`=1 
                    AND s.`iexport_req_refor` = '".$pk."' 
                    LIMIT 1";
                ...
                if(!empty($head['vRequest_no'])){
                    ...
                    $subject2="Konfirmasi Request Refor: No Request ".$head['vno_export_req_refor'];
                    $content1="
                    ...
                    Diberitahukan bahwa telah ada ada Konfirmasi dari Proses Request Reformulasi Export,
                        ...
                        $sqlDta ="SELECT s.`vRequest_no`,rw.`vnama`,sd.`raw_id`,sd.`vSatuan`,sd.`vSpesifikasi`,sd.`iJumlah` FROM reformulasi.`export_req_refor` r
                            ...
                            WHERE s.`lDeleted` = 0 AND s.`iTujuan_request`=1 AND sd.`lDeleted` = 0  AND r.`iexport_req_refor` = '".$pk."'";
            ...
            function approve_view() { 
                ...
                $echo .= '<form id="form_v2_exp_confirm_refor_approve" action="'.base_url().'processor/reformulasi/v2_exp_confirm_refor?action=approve_process" method="post">';
                $echo .= '<div style="vertical-align: top;">';
                $echo .= 'Remark : 
                        <input type="hidden" name="'.$this->main_table_pk.'" value="'.$this->input->get($this->main_table_pk).'" />
                        <input type="hidden" name="modul_id" value="'.$this->input->get('modul_id').'" />
                        ...
                        <textarea name="vRemark"></textarea>
                <button type="button" onclick="submit_ajax(\'form_v2_exp_confirm_refor_approve\')">Approve</button>';
            ...
            function reject_view() {
                $echo = '<script type="text/javascript">
                             function submit_ajax(form_id) {
                                var remark = $("#v2_exp_confirm_refor_remark").val();
                                if (remark=="") {
                ...
                $echo .= '<form id="form_v2_exp_confirm_refor_reject" action="'.base_url().'processor/reformulasi/v2_exp_confirm_refor?action=reject_process" method="post">';
                $echo .= '<div style="vertical-align: top;">';
                $echo .= 'Remark : 
                        <input type="hidden" name="'.$this->main_table_pk.'" value="'.$this->input->get($this->main_table_pk).'" />
                        <input type="hidden" name="modul_id" value="'.$this->input->get('modul_id').'" />
                        ...
                        <textarea name="vRemark" id="reject_v2_exp_confirm_refor_remark"></textarea>
                <button type="button" onclick="submit_ajax(\'form_v2_exp_confirm_refor_reject\')">Reject</button>';
    ...
    function after_update_processor($fields, $id, $postData) {
        ...
        $sql = "SELECT e.iexport_request_sample 
                FROM reformulasi.`export_request_sample` e 
                WHERE e.lDeleted = 0 
                AND e.iexport_req_refor = '".$id."' 
                LIMIT 1";
                ...
                if(empty($dtsample['iexport_request_sample'])){
                    ...
                    $sql = "UPDATE reformulasi.export_request_sample 
                ...
                if(isset($postData['detrawid'])){
                    if(($postData['detrawid'][0])!=""){
                        foreach($postData['detrawid'] as $k=>$v){ 
                            ...
                            $pri2['vSatuan']=$postData['detsatuan'][$k];
                            $pri2['vSpesifikasi']=$postData['detspesifikasi'][$k];
                            $pri2['iMaterialMR']=$postData['detsource'][$k];
                            $pri2['dCreate']=date('Y-m-d H:i:s');
                            $pri2['cCreated']=$this->user->gNIP;
                            $this->db->insert('reformulasi.export_request_sample_detail', $pri2);    
                        }
                ...
                $sqH = "UPDATE reformulasi.`kategori_refor_export` e 
                            SET e.lDeleted = 1, 
                            e.cUpdate='".$this->user->gNIP."',
                            e.dupdate='".date('Y-m-d H:i:s')."' 
                            WHERE e.iexport_req_refor = '".$id."'
                            ";
                $this->db->query($sqH);

                if(isset($postData['kategori'])){
                    //echo 'masook';
                    if(($postData['kategori'][0])!=""){
                        //echo 'masooook lagi';
                        foreach($postData['kategori'] as $k=>$v){ 
                            $pri22['iexport_req_refor']=$id;
                            $pri22['idKategori']=$v;
                            $pri22['vRemark']=(isset($postData['vRemark_'.$v]) ? $postData['vRemark_'.$v] : NULL);
                            $pri22['dCreate']=date('Y-m-d H:i:s');
                            $pri22['cCreated']=$this->user->gNIP;
                            $this->db->insert('reformulasi.kategori_refor_export', $pri22);    
                        }
                    }
                }


                $sqH = "UPDATE reformulasi.`refor_bahan_kemas` e 
                            SET e.lDeleted = 1, 
                            e.cUpdate='".$this->user->gNIP."',
                            e.dupdate='".date('Y-m-d H:i:s')."' 
                            WHERE e.iexport_req_refor = '".$id."'
                ...
                if(isset($postData['detjenis'])){
                    if(($postData['detjenis'][0])!=""){
                        foreach($postData['detjenis'] as $k=>$v){ 
                            $pri23['iexport_req_refor']=$id;
                            $pri23['vJenis']=$v;
                            $pri23['vRemark']=$postData['detremark'][$k];
                            $pri23['dCreate']=date('Y-m-d H:i:s');
                            $pri23['cCreated']=$this->user->gNIP;
                            $this->db->insert('reformulasi.refor_bahan_kemas', $pri23);    
                        }
                    }
                }
    ...
    function manipulate_insert_buttonx($buttons) { 
        ...
        $iframe = '<iframe name="v2_exp_confirm_refor_frame" id="v2_exp_confirm_refor_frame" height="0" width="0"></iframe>';
        ...
        $save_draft = '<button onclick="javascript:save_draft_btn_multiupload(\'v2_exp_confirm_refor\', \' '.base_url().'processor/reformulasi/v2_exp_confirm_refor?draft=true&modul_id='.$this->input->get('modul_id').'&iM_modul_activity='.$act['iM_modul_activity'].' \',this,true )"  id="button_save_draft_v2_exp_confirm_refor"  class="ui-button-text icon-save" >Save as Draft</button>';
        $save = '<button onclick="javascript:save_btn_multiupload(\'v2_exp_confirm_refor\', \' '.base_url().'processor/reformulasi/v2_exp_confirm_refor?company_id='.$this->input->get('company_id').'&modul_id='.$this->input->get('modul_id').'&iM_modul_activity='.$act['iM_modul_activity'].'&group_id='.$this->input->get('group_id').'modul_id='.$this->input->get('modul_id').' \',this,false )"  id="button_save_submit_v2_exp_confirm_refor"  class="ui-button-text icon-save" >Save &amp; Submit</button>';
    ...
    function manipulate_update_button($buttons, $rowData) { 
        ...
        if ($this->input->get('action') == 'view') {
            unset($buttons['update']);
                ...
                if($isOpenEditing){
                    $update_draft = '<button onclick="javascript:update_draft_btn(\'v2_exp_confirm_refor\', \' '.base_url().'processor/reformulasi/v2_exp_confirm_refor?draft=true \',this,true )"  id="button_update_draft_v2_exp_confirm_refor"  class="ui-button-text icon-save" >Update open Editing</button>';
                    ...
                    foreach ($activities as $act) {
                        $update_draft = '<button onclick="javascript:update_draft_btn(\'v2_exp_confirm_refor\', \' '.base_url().'processor/reformulasi/v2_exp_confirm_refor?draft=true&modul_id='.$this->input->get('modul_id').'&iM_modul_activity='.$act['iM_modul_activity'].' \',this,true )"  id="button_update_draft_v2_exp_confirm_refor"  class="ui-button-text icon-save" >Update as Draft</button>';
                        $update = '<button onclick="javascript:update_btn_back(\'v2_exp_confirm_refor\', \' '.base_url().'processor/reformulasi/v2_exp_confirm_refor?company_id='.$this->input->get('company_id').'&modul_id='.$this->input->get('modul_id').'&iM_modul_activity='.$act['iM_modul_activity'].'&group_id='.$this->input->get('group_id').'modul_id='.$this->input->get('modul_id').' \',this,false )"  id="button_update_submit_v2_exp_confirm_refor"  class="ui-button-text icon-save" >Update &amp; Submit</button>';

                        $approve = '<button onclick="javascript:load_popup(\' '.base_url().'processor/reformulasi/v2_exp_confirm_refor?action=approve&iM_modul_activity='.$act['iM_modul_activity'].'&iM_activity='.$act['iM_activity'].'&'.$this->main_table_pk.'='.$peka.'&iupb_id='.$iupb_id.'&company_id='.$this->input->get('company_id').'&group_id='.$this->input->get('group_id').'&modul_id='.$this->input->get('modul_id').' \')"  id="button_approve_v2_exp_confirm_refor"  class="ui-button-text icon-save" >Approve</button>';
                        $reject = '<button onclick="javascript:load_popup(\' '.base_url().'processor/reformulasi/v2_exp_confirm_refor?action=reject&iM_modul_activity='.$act['iM_modul_activity'].'&iM_activity='.$act['iM_activity'].'&'.$this->main_table_pk.'='.$peka.'&iupb_id='.$iupb_id.'&company_id='.$this->input->get('company_id').'&group_id='.$this->input->get('group_id').'&modul_id='.$this->input->get('modul_id').' \' )"  id="button_reject_v2_exp_confirm_refor"  class="ui-button-text icon-save" >Reject</button>';

                        $confirm = '<button onclick="javascript:load_popup(\' '.base_url().'processor/reformulasi/v2_exp_confirm_refor?action=confirm&iM_modul_activity='.$act['iM_modul_activity'].'&iM_activity='.$act['iM_activity'].'&'.$this->main_table_pk.'='.$peka.'&iupb_id='.$iupb_id.'&company_id='.$this->input->get('company_id').'&group_id='.$this->input->get('group_id').'&modul_id='.$this->input->get('modul_id').' \')"  id="button_approve_v2_exp_confirm_refor"  class="ui-button-text icon-save" >Confirm</button>';
