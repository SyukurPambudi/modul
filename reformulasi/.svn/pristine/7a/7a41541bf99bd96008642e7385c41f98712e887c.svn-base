...
class browse_lpp_export extends MX_Controller {
    ...
    function index($action = '') {
        ...
        //Grid harus end LPP
        $grid->setQuery('export_req_refor.iexport_req_refor IN(select f.iexport_req_refor from reformulasi.export_produksi_pilot e 
                    join reformulasi.export_refor_formula f on f.iexport_refor_formula=e.iexport_refor_formula
                    where e.ldeleted=0 and f.lDeleted=0 and e.iapprove_pd=2)', null); 

        $grid->setQuery('export_req_refor.iexport_req_refor NOT IN(SELECT el.`iexport_req_refor` 
            FROM reformulasi.`export_refor_lpp` el WHERE el.`lDeleted` = 0)', null); 
	...
	function listBox_browse_lpp_export_pilih($value, $pk, $name, $rowData) {   
