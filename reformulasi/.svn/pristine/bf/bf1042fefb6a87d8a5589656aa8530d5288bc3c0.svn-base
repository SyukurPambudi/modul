...
$("#<?php echo $tableId;?> .file_remove").click(function(){
	...
	var tmpDel = $("#terima_bb_pd_ad_matrik_<?php echo $iexport_ro_detail ?>_del");
	li.remove();
	var v = tmpDel.val();
	v+=','+fileid;
	tmpDel.val( v );
	alert( $("#terima_bb_pd_ad_matrik_<?php echo $iexport_ro_detail ?>_del").val() );
});

function add_row_terima_bb_pd_ad_matrik_<?php echo $iexport_ro_detail ?>_upload(table_id){		
		//alert(table_id);
		...
		if (n.length == 0) {
			...
			row_content	 += '<input type="file" id="fileupload'+table_id+c+'" class="fileupload multi multifile" name="fileupload<?php echo $iexport_ro_detail ?>[]" style="width: 70%" />*';
			...
			row_content	 += '<textarea id= "filekt_request_reforpd'+c+'" class="" name="file_vKeterangan<?php echo $iexport_ro_detail ?>[]" style="width: 240px; height: 50px;"></textarea><br/>tersisa <span id="len_request_reforpd'+c+'">250</span> karakter<br/></td>';
			row_content	 += '<td style="border: 1px solid #dddddd; width: 10%">';
			row_content	 += '<span class="delete_btn"><a href="javascript:;" class="terima_bb_pd_ad_matrik_<?php echo $iexport_ro_detail ?>_del" onclick="del_row(this, \'terima_bb_pd_ad_matrik_<?php echo $iexport_ro_detail ?>_del\')">[Hapus]</a></span></td>';		
			row_content  += '</tr>';
			...
			row_content  += '<input type="hidden" name="fileid[]" style="width: 70%" value="" /></td>';
			row_content	 += '<td colspan="3" style="border: 1px solid #dddddd; width: 8%">';
			row_content	 += '<textarea id= "filekt_request_reforpd'+c+'"class="" name="file_vKeterangan<?php echo $iexport_ro_detail ?>[]" style="width: 240px; height: 50px;"></textarea><br/>tersisa <span id="len_request_reforpd'+c+'">250</span> karakter<br/>';
			row_content  += '</td>';
...
<table class="hover_table" id="terima_bb_pd_ad_matrik_<?php echo $iexport_ro_detail ?>_upload" cellspacing="0" cellpadding="1" style="width: 98%; border: 1px solid #dddddd; text-align: center; margin-left: 5px; border-collapse: collapse">
	<thead>
	...
	<tbody>
		<?php
			...
			if(!empty($rows)) {
				foreach($rows as $row) {
					...
					$id  = $row['iexport_ro_detail'];
					$value = $row['vFilename'];	
					$tempat = 'ro_detail_matrik';	
					if($value != '') {
						if (file_exists('./files/reformulasi/export/'.$tempat.'/'.$id.'/'.$value)) {
							$link = base_url().'processor/reformulasi/export/terima/bb/pd/ad?action=download&id='.$id.'&file='.$value.'&path='.$tempat;
		...
		?>
				<tr style="border: 1px solid #dddddd; border-collapse: collapse; background: #ffffff; ">
					<td style="border: 1px solid #dddddd; width: 3%; text-align: center;">
						<span class="terima_bb_pd_ad_matrik_<?php echo $iexport_ro_detail ?>_upload_num"><?php echo $i ?></span>
					</td>		
					<td colspan="1" style="border: 1px solid #dddddd; width: 27%">
						...
						<input type="hidden" name="fileid[]" style="width: 70%" value="<?php echo $row['iexport_ro_detail_matrik'] ?>" />
					</td>	
					<td colspan="3" style="border: 1px solid #dddddd; width: 8%">
						<?php echo $row['vKeterangan'] ?>
					</td>
					<td style="border: 1px solid #dddddd; width: 10%">
						<?php if ($this->input->get('action') != 'view' and ( in_array('PD', $manager) or in_array('PD', $team)) ) { ?>
							<span class="delete_btn"><a href="javascript:;" class="terima_bb_pd_ad_matrik_<?php echo $iexport_ro_detail ?>_del" onclick="del_row1(this, 'terima_bb_pd_ad_matrik_<?php echo $iexport_ro_detail ?>_del')">[Hapus]</a></span>	
						<?php	} ?>
		...
		?>
				<!-- <tr style="border: 1px solid #dddddd; border-collapse: collapse; background: #ffffff; ">
					<td style="border: 1px solid #dddddd; width: 3%; text-align: center;">
						<span class="terima_bb_pd_ad_matrik_<?php echo $iexport_ro_detail ?>_upload_num">1</span>
					</td>		
					<td colspan="1" style="border: 1px solid #dddddd; width: 27%">
						<input type="file" id="fileupload_terima_bb_pd_ad_matrik_<?php echo $iexport_ro_detail ?>_1" class="fileupload multi multifile" name="fileupload<?php echo $iexport_ro_detail ?>[]" style="width: 70%" /> *
						<input type="hidden" name="namafile[]" style="width: 70%" value="" />
						<input type="hidden" name="fileid[]" style="width: 70%" value="" />
						<input type="hidden" name="iexport_ro_detailF[]" style="width: 70%" value="<?php $iexport_ro_detail ?>" />
						</td>	
					<td colspan="3" style="border: 1px solid #dddddd; width: 15%">
						<textarea class="" id="filekt_request_reforpd1" name="file_vKeterangan<?php echo $iexport_ro_detail ?>[]" style="width: 240px; height: 50px;"size="250"></textarea>
						<br/>
	...
	<tfoot>
		<tr>
			<td colspan="5" style="text-align: left">*) Max File 5 MB : Format = pdf,jpeg,jpg,png,gif,bmp</td><td style="text-align: center">
				<?php if ( ($this->input->get('action') != 'view') and ( in_array('PD', $manager) or in_array('PD', $team)   ) ) { ?>
					<a href="javascript:;" onclick="javascript:add_row_terima_bb_pd_ad_matrik_<?php echo $iexport_ro_detail ?>_upload('terima_bb_pd_ad_matrik_<?php echo $iexport_ro_detail ?>_upload')">Tambah</a>	
				<?php } ?>
	...
	<script>
		...
		$("#fileupload_terima_bb_pd_ad_matrik_<?php echo $iexport_ro_detail ?>_1").change(function () {
	        var fileExtension = ["pdf","jpeg", "jpg", "png", "gif", "bmp", "xls", "xlsx","doc","docx"];
	        if ($.inArray($(this).val().split(".").pop().toLowerCase(), fileExtension) == -1) {
	        	_custom_alert("Only formats are allowed : "+fileExtension.join(", "),"info", "info", "terima_bb_pd_ad_matrik_<?php echo $iexport_ro_detail ?>", 1, 20000);
	        	$(this).val('');
	        }
    	});
	</script>