...
<script type="text/javascript">
   ...
   <?php //if($get['action']!='view'){ ?>
    ...
    function addrow_<?php echo $nmTable; ?>(){
            ...
            var sa=[["<input type='hidden' class='num_rows_<?php echo $nmTable ?>' value='"+rlast+"' /><input type='file' id='<?php echo $grid; ?>_upload_file_"+rlast+"' class='fileupload1 multi multifile required' name='<?php echo $grid; ?>_upload_file[]' style='width: 90%' /> *","<textarea class='' id='<?php echo $grid; ?>_fileketerangan_"+rlast+"' name='<?php echo $grid; ?>_fileketerangan[]' style='width: 290px; height: 50px;' size='290'></textarea>", "<button id='ihapus_<?php echo $nmTable ?>' class='ui-button-text icon_hapus' style='width:75px' onclick='javascript:hapus_row_<?php echo $nmTable ?>("+rlast+")' type='button'>Hapus</button><input type='hidden' name='iexport_produksi_pilot_file[]' value='0' />"]];
    ...
    function reload_grid_<?php echo $nmTable ?>(){
        var id=$("#tb_details_export_produksi_pilot_id").val();
