...
class v2_export_soi_fg extends MX_Controller {
    function __construct() {
		...
		$this->title 		= 'SOI FINISH GOOD';
		$this->url 			= 'v2_export_soi_fg';
		$this->urlpath 		= 'reformulasi/'.str_replace("_","/", $this->url);


		$this->maintable 	= 'reformulasi.export_soi_fg';	
		$this->main_table 	= $this->maintable;	
		$this->main_table_pk= 'iexport_soi_fg';	
		
		$datagrid['islist'] = array(
            'export_req_refor.vno_export_req_refor'  => array('label' => 'Nama Bahan Baku','width' => 100,	'align' => 'center', 'search' => true),
            'itemas.c_itnam' => array('label' => 'Nama Produk','width' => 200,	'align' => 'left', 'search' => true ),           
            'vno_soi_fg'	=> array('label' => 'No SOI','width' => 100,	'align' => 'center', 'search' => true),
            'dberlaku'	=> array('label' => 'Tgl Berlaku','width' => 100,	'align' => 'center', 'search' => true),
            'vrevisi' => array('label' => 'Revisi',	'width' => 100,	'align' => 'center', 'search' => true),
			'iteamad' => array('label' => 'Team Andev','width' => 110,	'align' => 'center', 'search' => true),
			'cpenyusun' => array('label' => 'PIC Penyusun','width' => 110,	'align' => 'center', 'search' => true),			
			'iSubmit'=> array('label' => 'Status Submit','width' => 150,	'align' => 'left',   'search' => false),
			'iapprove_qa' => array('label' => 'Approval','width' => 110,	'align' => 'center', 'search' => false),
		);		
		...
		$datagrid['shortBy']=array("export_soi_fg.iexport_soi_fg" => "DESC");
...
/* 		if (!empty($dataField)){
    ...
    function get_data_prev(){
        ...
        function listBox_v2_export_soi_fg_iapppr($value) {
...
/*aslinya*/
   /*         if ( $getLastactivity == 0 ) { 
        ...
        function insertBox_v2_export_soi_fg_form_detail($field,$id){
            $get=$this->input->get();
            $post=$this->input->post();
        ...
        function updateBox_v2_export_soi_fg_form_detail($field,$id,$value,$rowData){
            $get=$this->input->get();
            $post=$this->input->post();
            foreach ($get as $kget => $vget) {
                if($kget!="action"){
            ...
            function approve_view() { 
                             ...
                             function submit_ajax(form_id) {
                                return $.ajax({
                                    ...
                                    success: function(data) {
                                        ...
                                        var url = "'.base_url().'processor/reformulasi/v2_export_soi_fg";                             
                                        if(o.status == true) { 
                                            $("#alert_dialog_form").dialog("close");
                                                 $.get(url+"?action=update&id="+last_id+"&foreign_key=0&company_id=3&group_id="+group_id+"&modul_id="+modul_id, function(data) {
                                                 $("div#form_v2_export_soi_fg").html(data);
                                                 
                                            });
                                            
                                        }
                                            reload_grid("grid_v2_export_soi_fg");
                                    }
                ...
                $echo .= '<form id="form_v2_export_soi_fg_approve" action="'.base_url().'processor/reformulasi/v2_export_soi_fg?action=approve_process" method="post">';
                ...
                <button type="button" onclick="submit_ajax(\'form_v2_export_soi_fg_approve\')">Approve</button>';
           ...
           function approve_process() {
            ...
            function confirm_view() { 
                ...
                $echo .= '<form id="form_v2_export_soi_fg_confirm" action="'.base_url().'processor/reformulasi/v2_export_soi_fg?action=confirm_process" method="post">';
                $echo .= '<div style="vertical-align: top;">';
                $echo .= 'Remark : 
                        <input type="hidden" name="'.$this->main_table_pk.'" value="'.$this->input->get($this->main_table_pk).'" />
                        <input type="hidden" name="modul_id" value="'.$this->input->get('modul_id').'" />
                        ...
                        <textarea name="vRemark"></textarea>
                <button type="button" onclick="submit_ajax(\'form_v2_export_soi_fg_confirm\')">Confirm</button>';
            ...
            function reject_view() {
                $echo = '<script type="text/javascript">
                             function submit_ajax(form_id) {
                                var remark = $("#v2_export_soi_fg_remark").val();
                                if (remark=="") {
                ...
                $echo .= '<form id="form_v2_export_soi_fg_reject" action="'.base_url().'processor/reformulasi/v2_export_soi_fg?action=reject_process" method="post">';
                $echo .= '<div style="vertical-align: top;">';
                $echo .= 'Remark : 
                        <input type="hidden" name="'.$this->main_table_pk.'" value="'.$this->input->get($this->main_table_pk).'" />
                        <input type="hidden" name="modul_id" value="'.$this->input->get('modul_id').'" />
                        ...
                        <textarea name="vRemark" id="reject_v2_export_soi_fg_remark"></textarea>
                <button type="button" onclick="submit_ajax(\'form_v2_export_soi_fg_reject\')">Reject</button>';
    ...
    function after_insert_processor($fields, $id, $postData) { 
        ...
        if($postData['iSubmit']==1){
            $sql = "SELECT em.`vName`, sa.`dujebe`, sa.`dselesaiujebe`, sa.`iTeamAd`,
                ...
                WHERE sa.`lDeleted` = 0 AND sa.`iexport_soi_fg` =  ".$id;
 ...
 function manipulate_update_button($buttons, $rowData) { 
        ...
        $iframe = '<iframe name="v2_export_soi_fg" id="v2_export_soi_fg" height="0" width="0"></iframe>';
        ...
        if ($this->input->get('action') == 'view') {
            unset($buttons['update']);
                ...
                if($isOpenEditing){
                    $update_draft = '<button onclick="javascript:update_draft_btn(\'v2_export_soi_fg\', \' '.base_url().'processor/reformulasi/v2_export_soi_fg?draft=true \',this,true )"  id="button_update_draft_v2_export_soi_fg"  class="ui-button-text icon-save" >Update open Editing</button>';
                    ...
                    foreach ($activities as $act) {
                        $update_draft = '<button onclick="javascript:update_draft_btn(\'v2_export_soi_fg\', \' '.base_url().'processor/reformulasi/v2_export_soi_fg?draft=true&modul_id='.$this->input->get('modul_id').'&iM_modul_activity='.$act['iM_modul_activity'].' \',this,true )"  id="button_update_draft_v2_export_soi_fg"  class="ui-button-text icon-save" >Update as Draft</button>';
                        $update = '<button onclick="javascript:update_btn_back(\'v2_export_soi_fg\', \' '.base_url().'processor/reformulasi/v2_export_soi_fg?company_id='.$this->input->get('company_id').'&modul_id='.$this->input->get('modul_id').'&iM_modul_activity='.$act['iM_modul_activity'].'&group_id='.$this->input->get('group_id').'modul_id='.$this->input->get('modul_id').' \',this,false )"  id="button_update_submit_v2_export_soi_fg"  class="ui-button-text icon-save" >Update &amp; Submit</button>';

                        $approve = '<button onclick="javascript:load_popup(\' '.base_url().'processor/reformulasi/v2_export_soi_fg?action=approve&iM_modul_activity='.$act['iM_modul_activity'].'&iM_activity='.$act['iM_activity'].'&'.$this->main_table_pk.'='.$peka.'&iupb_id='.$iupb_id.'&company_id='.$this->input->get('company_id').'&group_id='.$this->input->get('group_id').'&modul_id='.$this->input->get('modul_id').' \')"  id="button_approve_v2_export_soi_fg"  class="ui-button-text icon-save" >Approve</button>';
                        $reject = '<button onclick="javascript:load_popup(\' '.base_url().'processor/reformulasi/v2_export_soi_fg?action=reject&iM_modul_activity='.$act['iM_modul_activity'].'&iM_activity='.$act['iM_activity'].'&'.$this->main_table_pk.'='.$peka.'&iupb_id='.$iupb_id.'&company_id='.$this->input->get('company_id').'&group_id='.$this->input->get('group_id').'&modul_id='.$this->input->get('modul_id').' \' )"  id="button_reject_v2_export_soi_fg"  class="ui-button-text icon-save" >Reject</button>';

                        $confirm = '<button onclick="javascript:load_popup(\' '.base_url().'processor/reformulasi/v2_export_soi_fg?action=confirm&iM_modul_activity='.$act['iM_modul_activity'].'&iM_activity='.$act['iM_activity'].'&'.$this->main_table_pk.'='.$peka.'&iupb_id='.$iupb_id.'&company_id='.$this->input->get('company_id').'&group_id='.$this->input->get('group_id').'&modul_id='.$this->input->get('modul_id').' \')"  id="button_approve_v2_export_soi_fg"  class="ui-button-text icon-save" >Confirm</button>';
    ...
    function manipulate_update_buttonx($buttons, $rowData) { 
        if ($this->input->get('action') == 'view') {
           ...
           $approve = '<button onclick="javascript:load_popup(\''.base_url().'processor/reformulasi/export/protokol/valpro?action=approve&iexport_soi_fg='.$rowData['iexport_soi_fg'].'&cNip='.$cNip.'&lvl=1&status=1&group_id='.$this->input->get('group_id').'&modul_id='.$this->input->get('modul_id').'\')" class="ui-button-text icon-save" id="button_approve_export_soi_fg">Confirm</button>';
            $reject = '<button onclick="javascript:load_popup(\''.base_url().'processor/reformulasi/export/protokol/valpro?action=reject&iexport_soi_fg='.$rowData['iexport_soi_fg'].'&cNip='.$cNip.'&lvl=1&status=2&group_id='.$this->input->get('group_id').'&modul_id='.$this->input->get('modul_id').'\')" class="ui-button-text icon-save" id="button_reject_export_soi_fg">Reject</button>';
