...
$("#<?php echo $tableId;?> .file_remove").click(function(){
	...
	var tmpDel = $("#export_analisa_bb_pd_del");
	li.remove();
	var v = tmpDel.val();
	v+=','+fileid;
	tmpDel.val( v );
	alert( $("#export_analisa_bb_pd_del").val() );
});

function add_row_export_analisa_bb_pd_upload(table_id){		
		//alert(table_id);
		...
		if (n.length == 0) {
			...
			row_content	 += '<textarea id= "filekt_export_analisa_bbpd'+c+'" class="" name="file_vKeterangan[]" style="width: 240px; height: 50px;"></textarea><br/>tersisa <span id="len_export_analisa_bbpd'+c+'">250</span> karakter<br/></td>';
			row_content	 += '<td style="border: 1px solid #dddddd; width: 10%">';
			row_content	 += '<span class="delete_btn"><a href="javascript:;" class="export_analisa_bb_pd_del" onclick="del_row(this, \'export_analisa_bb_pd_del\')">[Hapus]</a></span></td>';		
			row_content  += '</tr>';
			...
			row_content  += '<input type="hidden" name="fileid[]" style="width: 70%" value="" /></td>';
			row_content	 += '<td colspan="3" style="border: 1px solid #dddddd; width: 8%">';
			row_content	 += '<textarea id= "filekt_export_analisa_bbpd'+c+'"class="" name="file_vKeterangan[]" style="width: 240px; height: 50px;"></textarea><br/>tersisa <span id="len_export_analisa_bbpd'+c+'">250</span> karakter<br/>';
			row_content  += '</td>';
			...
			$("span."+table_id+"_num:last").text(c);		
				...
				$("#filekt_export_analisa_bbpd"+c).keyup(function() {
					var len = this.value.length;
					if (len >= 250) {
					this.value = this.value.substring(0, 250);
					}
					$("#len_export_analisa_bbpd"+c).text(250 - len);
				});
...
<table class="hover_table" id="export_analisa_bb_pd_upload" cellspacing="0" cellpadding="1" style="width: 98%; border: 1px solid #dddddd; text-align: center; margin-left: 5px; border-collapse: collapse">
	<thead>
	...
	<tbody>
		<?php
			...
			if(!empty($rows)) {
				foreach($rows as $row) {
					...
					$value = $row['vFilename'];	
					$tempat = 'export_analisa_bb';	
					if($value != '') {
						if (file_exists('./files/reformulasi/export/export_analisa_bb/'.$id.'/'.$value)) {
							$link = base_url().'processor/reformulasi/export/analisa/bb?action=download&id='.$id.'&file='.$value.'&path='.$tempat;
		...
		?>
				<tr style="border: 1px solid #dddddd; border-collapse: collapse; background: #ffffff; ">
					<td style="border: 1px solid #dddddd; width: 3%; text-align: center;">
						<span class="export_analisa_bb_pd_upload_num"><?php echo $i ?></span>
					</td>		
					<td colspan="1" style="border: 1px solid #dddddd; width: 27%">
						...
						<input type="hidden" name="fileid[]" style="width: 70%" value="<?php echo $row['iexport_analisa_bb_pd_file_id'] ?>" />
					</td>	
					...
					<td style="border: 1px solid #dddddd; width: 10%">
						<?php if ($this->input->get('action') != 'view') { ?>
							<span class="delete_btn"><a href="javascript:;" class="export_analisa_bb_pd_del" onclick="del_row1(this, 'export_analisa_bb_pd_del')">[Hapus]</a></span>	
						<?php	} ?>
		...
		<!-- <tr style="border: 1px solid #dddddd; border-collapse: collapse; background: #ffffff; ">
					<td style="border: 1px solid #dddddd; width: 3%; text-align: center;">
						<span class="export_analisa_bb_pd_upload_num">1</span>
					</td>		
					<td colspan="1" style="border: 1px solid #dddddd; width: 27%">
						<input type="file" id="fileupload_export_analisa_bb_pd_1" class="fileupload multi multifile" name="fileupload[]" style="width: 70%" /> *
					...
					<td colspan="3" style="border: 1px solid #dddddd; width: 15%">
						<textarea class="" id="filekt_export_analisa_bbpd1" name="file_vKeterangan[]" style="width: 240px; height: 50px;"size="250"></textarea>
						<br/>
						tersisa <span id="len_export_analisa_bbpd1">250</span> karakter<br/>
	...
	<tfoot>
		<tr>
			<td colspan="5" style="text-align: left">*) Max File 5 MB : Format = pdf,jpeg,jpg,png,gif,bmp</td><td style="text-align: center">
				<?php if ($this->input->get('action') != 'view') { ?>
					<a href="javascript:;" onclick="javascript:add_row_export_analisa_bb_pd_upload('export_analisa_bb_pd_upload')">Tambah</a>	
				<?php } ?>
	...
	<script>
		$('#filekt_export_analisa_bbpd1').keyup(function() {
		var len = this.value.length;
		if (len >= 250) {
		this.value = this.value.substring(0, 250);
		}
		$('#len_export_analisa_bbpd1').text(250 - len);
		});
		$("#fileupload_export_analisa_bb_pd_1").change(function () {
	        var fileExtension = ["pdf","jpeg", "jpg", "png", "gif", "bmp", "xls", "xlsx","doc","docx"];
	        if ($.inArray($(this).val().split(".").pop().toLowerCase(), fileExtension) == -1) {
	        	_custom_alert("Only formats are allowed : "+fileExtension.join(", "),"info", "info", "export_analisa_bb_pd", 1, 20000);
	        	$(this).val('');
	        }
    	});
	</script>