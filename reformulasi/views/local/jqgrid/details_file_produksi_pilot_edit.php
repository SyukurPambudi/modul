<?php
$nmTable="table_file_produksi_pilot_details_edit";
$pager="pager_".$nmTable;
$caption = "File Produksi Pilot";
$gridurl=base_url()."processor/reformulasi/local_produksi_pilot?action=getdetailsMR&id=".$get['id']."&acc=".$get['action'];
   ...
   <?php if($rowData['iapppd']==0 || $get['action']!='view'){ ?>
            .navButtonAdd('#<?php echo $pager; ?>',{
               ...
               onClickButton: function(){ 
                 addrow_detail_files_produksi_pilot_edit();
               }, 
               position:"last"
            })
    <?php 
    ...
    function addrow_detail_files_produksi_pilot_edit(){
        //get last num rows
        var n=[];
        var i=0;
        $.each($(".num_rows_details_file_produksi_pilot_edit"),function(){
            /*if(i==0){
                n[i]=$(this).val();
            }else{
                n[i]=','+$(this).val();
        ...
        var sa=[["<input type='file' id='fileupload_local_produksi_pilot_"+rlast+"' class='fileupload multi multifile' name='fileupload_local_produksi_pilot[]' style='width: 90%' />","<textarea id='file_keterangan_local_produksi_pilot_"+rlast+"' name='file_keterangan_local_produksi_pilot[]' style='width: 240px; height: 50px;'></textarea>","<input type='hidden' class='num_rows_details_file_produksi_pilot_edit' value='"+rlast+"' /><button id='ihapus_<?php echo $nmTable ?>' class='ui-button-text icon_hapus' style='width:75px' onclick='javascript:hapus_row_details_file_produksi_pilot_edit("+rlast+")' type='button'>Hapus</button>"]];
    ...
    function hapus_row_details_file_produksi_pilot_edit(rowId){
