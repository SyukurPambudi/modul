...
$("#<?php echo $tableId;?> .file_remove1").click(function(){
	...
	var fileid_lokal_stress_test_file1 = li.attr('fileid_lokal_stress_test_file1');
	var tmpDel = $("#lokal_stress_test_file_");
	li.remove();
	var v = tmpDel.val();
	v+=','+fileid_lokal_stress_test_file1;
	tmpDel.val( v );
	alert( $("#lokal_stress_test_file_").val() );
});

function add_row_lokal_stress_test_filess1(table_id){		
		//alert(table_id);
		...
		if (n.length == 0) {
			...
			row_content	 += '<input type="file" id="fileupload1'+table_id+c+'" class="fileupload1 multi multifile " name="fileid_lokal_stress_test_file_upload_lokal_stress_test_file[]" style="width: 70%" />*';
			row_content  += '<input type="hidden" name="namefile_lokal_stress_test_file[]" style="width: 70%" value="" />';	
			row_content  += '<input type="hidden" name="fileid_lokal_stress_test_file[]" style="width: 70%" value="" /></td>';

			row_content	 += '<td colspan="1" style="border: 1px solid #dddddd; width: 8%">';
			row_content	 += '<textarea id= "filekt1'+c+'" class="" name="fileketerangan_lokal_stress_test_file[]" style="width: 240px; height: 50px;"></textarea><br/>tersisa <span id="len1'+c+'">250</span> karakter<br/></td>';
			
			row_content	 += '<td style="border: 1px solid #dddddd; width: 10%">';
			row_content	 += '<span class="delete_btn"><a href="javascript:;" class="lokal_stress_test_file_" onclick="del_row(this, \'lokal_stress_test_file_\')">[Hapus]</a></span></td>';		
			row_content  += '</tr>';
...
<table class="hover_table" id="lokal_stress_test_file" cellspacing="0" cellpadding="1" style="width: 98%; border: 1px solid #dddddd; text-align: center; margin-left: 5px; border-collapse: collapse">
	<thead>
	...
	<tbody>
		<?php
			...
			if(!empty($rows)) {
				...
				foreach($rows as $row) {
					...
					$value = $row['vFilename'];	
					if($value != '') {
						if (file_exists('./files/reformulasi/local/stress_test/trial/'.$id.'/'.$value)) {
							$link = base_url().'processor/reformulasi/lokal/stress/test?action=download&filepath=trial&id='.$id.'&file='.$value;
		...
		?>
				<tr style="border: 1px solid #dddddd; border-collapse: collapse; background: #ffffff; ">
					<td style="border: 1px solid #dddddd; width: 3%; text-align: center;">
						<span class="lokal_stress_test_file_numasd"><?php echo $i ?></span>
					</td>		
					<td colspan="1" style="border: 1px solid #dddddd; width: 27%">
						<?php echo $row['vFilename'] ?> 
						<input type="hidden" name="namefile_lokal_stress_test_file[]" style="width: 70%" value="<?php echo $row['vFilename'] ?>" />
						<input type="hidden" name="fileid_lokal_stress_test_file[]" style="width: 70%" value="<?php echo $row['ilocal_refor_stress_test_file'] ?>" />
					</td>
					...
					<td style="border: 1px solid #dddddd; width: 10%">
						<?php if ($this->input->get('action') != 'view') { ?>
							<span class="delete_btn"><a href="javascript:;" class="lokal_stress_test_file_" onclick="del_row1(this, 'lokal_stress_test_file_')">[Hapus]</a></span>	
						<?php	} ?>
		...
		<tr style="border: 1px solid #dddddd; border-collapse: collapse; background: #ffffff; ">
					<td style="border: 1px solid #dddddd; width: 3%; text-align: center;">
						<span class="lokal_stress_test_file_numasd">1</span>
					</td>		
					<td colspan="1" style="border: 1px solid #dddddd; width: 27%">
						<input type="file" id="fileupload1_study_pd_1" class="fileupload1 multi multifile " name="fileid_lokal_stress_test_file_upload_lokal_stress_test_file[]" style="width: 70%" /> *
						<input type="hidden" name="namefile_lokal_stress_test_file[]" style="width: 70%" value="" />
						<input type="hidden" name="fileid_lokal_stress_test_file[]" style="width: 70%" value="" />
						</td>	

						<td colspan="1" style="border: 1px solid #dddddd; width: 15%">
						<textarea class="" id="filekt1" name="fileketerangan_lokal_stress_test_file[]" style="width: 240px; height: 50px;"size="250">
					...
					<td style="border: 1px solid #dddddd; width: 5%">
						<span class="delete_btn"><a href="javascript:;" class="lokal_stress_test_file_" onclick="del_row(this, 'lokal_stress_test_file_')">[Hapus]</a></span>
					</td>	
				</tr>
		<?php } }?>
	</tbody>
	<tfoot>
		<tr>
			<td colspan="3" style="text-align: left">*) Max File 5 MB : Format = pdf, docx, xlsx, jpeg, jpg, png</td><td style="text-align: center">
				<?php if ($this->input->get('action') != 'view') { ?>
					<a href="javascript:;" onclick="javascript:add_row_lokal_stress_test_filess1('lokal_stress_test_file')">Tambah</a>	
				<?php } ?>
