...
function save_draft_btn_multiupload(grid, url, dis, isdraft) {
	...
	if($("#lokal_maping_proses_cPIC").val()==''){
			$("#lokal_maping_proses_cPIC_text").addClass('error_text');	
			conf++;
		}
		if($("#lokal_maping_proses_iupb_id").val()==''){
			$("#lokal_maping_proses_iupb_id_dis").addClass('error_text');	
			conf++;
		}
			...
			if(isUpload && !isValidAFileSize('#form_create_'+grid+' input.multifile', uploadLimit)) {
				...
				$.ajax({
					...
					success: function(data) {	
						...
						if(o.status == true) {
							if(isUpload) {
								var iframe = $('<iframe name="lokal_maping_proses_frame"/>');
								iframe.attr({'id':'lokal_maping_proses_frame'});
								...
								$('#form_create_'+grid).attr('target','lokal_maping_proses_frame');

								uploadfile('form_create_'+grid, grid, formAction, base_url+'processor/reformulasi/lokal/maping/proses?action=update&id='+o.last_id+'&foreign_key='+o.foreign_id+'&company_id='+o.company_id+'&group_id='+o.group_id+'&modul_id='+o.modul_id);							    

							
							}else{
								_custom_alert('Data Berhasil Disimpan !',header,info, grid, 1, 20000);
								$.get(base_url+'processor/reformulasi/lokal/maping/proses?action=update&id='+o.last_id+'&foreign_key='+o.foreign_id+'&company_id='+o.company_id+'&group_id='+o.group_id+'&modul_id='+o.modul_id, function(data) {
				...
				success: function(data) {
					...
					if(o.status == true){
						if(isUpload) {
							...
							$('#form_update_'+grid).attr('target','lokal_maping_proses_frame');

							uploadfile('form_update_'+grid, grid, formAction, base_url+'processor/reformulasi/lokal/maping/proses?action=update&id='+o.last_id+'&foreign_key='+o.foreign_id+'&company_id='+o.company_id+'&group_id='+o.group_id+'&modul_id='+o.modul_id);
							//lokal_maping_proses_tryUPload('form_update_'+grid , grid, formAction, base_url+'processor/reformulasi/lokal/maping/proses?action=update&id='+o.last_id+'&foreign_key='+o.foreign_id+'&company_id='+o.company_id+'&group_id='+o.group_id+'&modul_id='+o.modul_id );
...
<iframe name="lokal_maping_proses_frame" id="lokal_maping_proses_frame" height="0" width="0"></iframe>
