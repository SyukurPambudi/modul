...
function save_draft_btn_multiupload(grid, url, dis, isdraft) {
	...
	if($("#export_req_refor_cPIC").val()==''){
			$("#export_req_refor_cPIC_text").addClass('error_text');	
			conf++;
		}
		if($("#export_req_refor_iupb_id").val()==''){
			$("#export_req_refor_iupb_id_dis").addClass('error_text');	
			conf++;
		}
			...
			if(isUpload && !isValidAFileSize('#form_create_'+grid+' input.multifile', uploadLimit)) {
				...
				$.ajax({
					...
					success: function(data) {	
						...
						if(o.status == true) {
							if(isUpload) {
								var iframe = $('<iframe name="export_req_refor_frame"/>');
								iframe.attr({'id':'export_req_refor_frame'});
								...
								$('#form_create_'+grid).attr('target','export_req_refor_frame');

								uploadfile('form_create_'+grid, grid, formAction, base_url+'processor/reformulasi/export/req/refor?action=update&id='+o.last_id+'&foreign_key='+o.foreign_id+'&company_id='+o.company_id+'&group_id='+o.group_id+'&modul_id='+o.modul_id);							    

							
							}else{
								_custom_alert('Data Berhasil Disimpan !',header,info, grid, 1, 20000);
								$.get(base_url+'processor/reformulasi/export/req/refor?action=update&id='+o.last_id+'&foreign_key='+o.foreign_id+'&company_id='+o.company_id+'&group_id='+o.group_id+'&modul_id='+o.modul_id, function(data) {
				...
				success: function(data) {
					...
					if(o.status == true){
						if(isUpload) {
							...
							$('#form_update_'+grid).attr('target','export_req_refor_frame');

							uploadfile('form_update_'+grid, grid, formAction, base_url+'processor/reformulasi/export/req/refor?action=update&id='+o.last_id+'&foreign_key='+o.foreign_id+'&company_id='+o.company_id+'&group_id='+o.group_id+'&modul_id='+o.modul_id);
							//export_req_refor_tryUPload('form_update_'+grid , grid, formAction, base_url+'processor/reformulasi/export/req/refor?action=update&id='+o.last_id+'&foreign_key='+o.foreign_id+'&company_id='+o.company_id+'&group_id='+o.group_id+'&modul_id='+o.modul_id );
...
<iframe name="export_req_refor_frame" id="export_req_refor_frame" height="0" width="0"></iframe>
