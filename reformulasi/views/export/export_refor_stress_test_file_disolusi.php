...
?>
<script>
$("#<?php echo $tableId;?> .fileupload1_export_refor_stress_test_file_disolusi").MultiFile();
$("#<?php echo $tableId;?> .file_remove1").click(function(){
	var li = $(this).closest('li');
	var fileid_export_refor_stress_test_file_disolusi1 = li.attr('fileid_export_refor_stress_test_file_disolusi1');
	var tmpDel = $("#study_pd_del1");
	li.remove();
	var v = tmpDel.val();
	v+=','+fileid_export_refor_stress_test_file_disolusi1;
	tmpDel.val( v );
	alert( $("#study_pd_del1").val() );
});

function add_row_export_refor_stress_test_file_disolusiss1(table_id){		
		//alert(table_id);
		...
		if (n.length == 0) {
			...
			row_content	 += '<input type="file" id="fileupload1_export_refor_stress_test_file_disolusi'+table_id+c+'" class="fileupload1_export_refor_stress_test_file_disolusi multi multifile " name="fileid_export_refor_stress_test_file_disolusi_upload_export_refor_stress_test_file_disolusi[]" style="width: 70%" />*';
			row_content  += '<input type="hidden" name="namefile_export_refor_stress_test_file_disolusi[]" style="width: 70%" value="" />';	
			row_content  += '<input type="hidden" name="fileid_export_refor_stress_test_file_disolusi[]" style="width: 70%" value="" /></td>';

			row_content	 += '<td colspan="1" style="border: 1px solid #dddddd; width: 8%">';
			row_content	 += '<textarea id= "filekt1'+c+'" class="" name="fileketerangan_export_refor_stress_test_file_disolusi[]" style="width: 240px; height: 50px;"></textarea><br/>tersisa <span id="len1'+c+'">250</span> karakter<br/></td>';
			...
			$("span."+table_id+"_numasd:last").text(c);		
				...
				$("#fileupload1_export_refor_stress_test_file_disolusi"+table_id+c).change(function () {
...
<table class="hover_table" id="export_refor_stress_test_file_disolusi" cellspacing="0" cellpadding="1" style="width: 98%; border: 1px solid #dddddd; text-align: center; margin-left: 5px; border-collapse: collapse">
	<thead>
	...
	<tbody>
		<?php
			...
			if(!empty($rows)) {
				...
				foreach($rows as $row) {
					...
					$value = $row['vFilename'];	
					if($value != '') {
						if (file_exists('./files/reformulasi/export/skala_test/disolusi/'.$id.'/'.$value)) {
							$link = base_url().'processor/reformulasi/export/refor/stress/test?action=download&filepath=disolusi&id='.$id.'&file='.$value;
		...
		?>
				<tr style="border: 1px solid #dddddd; border-collapse: collapse; background: #ffffff; ">
					<td style="border: 1px solid #dddddd; width: 3%; text-align: center;">
						<span class="export_refor_stress_test_file_disolusi_numasd"><?php echo $i ?></span>
					</td>		
					<td colspan="1" style="border: 1px solid #dddddd; width: 27%">
						<?php echo $row['vFilename'] ?> 
						<input type="hidden" name="namefile_export_refor_stress_test_file_disolusi[]" style="width: 70%" value="<?php echo $row['vFilename'] ?>" />
						<input type="hidden" name="fileid_export_refor_stress_test_file_disolusi[]" style="width: 70%" value="<?php echo $row['iexport_refor_stress_test_file'] ?>" />
					</td>
		...
		<tr style="border: 1px solid #dddddd; border-collapse: collapse; background: #ffffff; ">
					<td style="border: 1px solid #dddddd; width: 3%; text-align: center;">
						<span class="export_refor_stress_test_file_disolusi_numasd">1</span>
					</td>		
					<td colspan="1" style="border: 1px solid #dddddd; width: 27%">
						<input type="file" id="fileupload1_export_refor_stress_test_file_disolusi_study_pd_1" class="fileupload1_export_refor_stress_test_file_disolusi multi multifile " name="fileid_export_refor_stress_test_file_disolusi_upload_export_refor_stress_test_file_disolusi[]" style="width: 70%" /> *
						<input type="hidden" name="namefile_export_refor_stress_test_file_disolusi[]" style="width: 70%" value="" />
						<input type="hidden" name="fileid_export_refor_stress_test_file_disolusi[]" style="width: 70%" value="" />
						</td>	

						<td colspan="1" style="border: 1px solid #dddddd; width: 15%">
						<textarea class="" id="filekt1" name="fileketerangan_export_refor_stress_test_file_disolusi[]" style="width: 240px; height: 50px;"size="250">
	...
	<tfoot>
		<tr>
			<td colspan="3" style="text-align: left">*) Max File 5 MB : Format = pdf, docx, xlsx, jpeg, jpg, png</td><td style="text-align: center">
				<?php if ($this->input->get('action') != 'view') { ?>
					<a href="javascript:;" onclick="javascript:add_row_export_refor_stress_test_file_disolusiss1('export_refor_stress_test_file_disolusi')">Tambah</a>	
				<?php } ?>
...
<script>
	...
	$("#fileupload1_export_refor_stress_test_file_disolusi_study_pd_1").change(function () {
       var fileExtension = ["pdf","docx","xlsx","jpeg", "jpg", "png"];
        if ($.inArray($(this).val().split(".").pop().toLowerCase(), fileExtension) == -1) {
