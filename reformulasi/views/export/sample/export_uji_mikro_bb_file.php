...
$("#<?php echo $tableId;?> .file_remove").click(function(){
	...
	var tmpDel = $("#export_uji_mikro_bb_file_filename_del");
	li.remove();
	var v = tmpDel.val();
	v+=','+fileid;
	tmpDel.val( v );
	alert( $("#export_uji_mikro_bb_file_filename_del").val() );
});

function del_upload_export_uji_mikro_bb_file_filename(dis, conname, table_id) {
	if($('.'+conname).length > 1) {
		...
		var n = $("span."+table_id+"_num:last").text();
			...
			row_content	 += '<span class="delete_btn"><a href="javascript:;" class="export_uji_mikro_bb_file_filename_del" onclick="del_upload_export_uji_mikro_bb_file_filename(this, \'export_uji_mikro_bb_file_filename_del\', \'export_uji_mikro_bb_file_filename\')">[Hapus]</a></span></td>';		
			row_content  += '</tr>';
...
function add_row_export_uji_mikro_bb_file_filename(table_id){		
		//alert(table_id);
		var row = $('table#'+table_id+' tbody tr:last').clone();
		$("span."+table_id+"_num:first").text('1');
		var n = $("span."+table_id+"_num:last").text();
...
<table class="hover_table" id="export_uji_mikro_bb_file_filename" cellspacing="0" cellpadding="1" style="width: 98%; border: 1px solid #dddddd; text-align: center; margin-left: 5px; border-collapse: collapse">
	<thead>
	...
	<tbody>
		...
		if (isset($mydept)) {
			...
			if(!empty($rows)) {
				...
				//tambahan untuk download file
					...
					$path = "rpt_mikro_bb";	
					if($value != '') {
						if (file_exists('./files/reformulasi/export/export_uji_mikro_bb/rpt_mikro_bb/'.$iexport_uji_mikro_bb.'/'.$value)) {
		...
		?>
				<tr style="border: 1px solid #dddddd; border-collapse: collapse; background: #ffffff; ">
					<td style="border: 1px solid #dddddd; width: 3%; text-align: center;">
						<span class="export_uji_mikro_bb_file_filename_num"><?php echo $i ?></span>
					</td>							
					...
					<td style="border: 1px solid #dddddd; width: 10%">
						<span class="delete_btn"><a href="javascript:;" class="export_uji_mikro_bb_file_filename_del" onclick="del_upload_export_uji_mikro_bb_file_filename(this, 'export_uji_mikro_bb_file_filename_del' , 'export_uji_mikro_bb_file_filename')">[Hapus]</a></span>
		...
		?>
				<tr style="border: 1px solid #dddddd; border-collapse: collapse; background: #ffffff; ">
					<td style="border: 1px solid #dddddd; width: 3%; text-align: center;">
						<span class="export_uji_mikro_bb_file_filename_num">1</span>
					</td>		
					<td colspan="2" style="border: 1px solid #dddddd; width: 27%">
	...
	<tfoot>
		<tr>
			<td colspan="5"></td><td style="text-align: center"><a href="javascript:;" onclick="javascript:add_row_export_uji_mikro_bb_file_filename('export_uji_mikro_bb_file_filename')">Tambah</a></td>
		</tr>
	</tfoot>
</table>