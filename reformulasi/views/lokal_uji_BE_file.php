...
$("#<?php echo $tableId;?> .file_remove3").click(function(){
	...
	var tmpDel = $("#Local_ujibe_del");
	li.remove();
	var v = tmpDel.val();
	v+=','+fileid3;
	tmpDel.val( v );
	alert( $("#Local_ujibe_del").val() );
});


function add_row_Local_ujibe_upload(table_id){		
		//alert(table_id);
		...
		if (n.length == 0) {
			...
			row_content	 += '<span class="delete_btn"><a href="javascript:;" class="lokal_ujibe_del" onclick="del_row(this, \'lokal_ujibe_del\')">[Hapus]</a></span></td>';		
			row_content  += '</tr>';
...
<table class="hover_table" id="lokal_ujibe_upload" cellspacing="0" cellpadding="1" style="width: 98%; border: 1px solid #dddddd; text-align: center; margin-left: 5px; border-collapse: collapse">
	<thead>
	...
	<tbody>
		<?php
			...
			if(!empty($rows)) {
				...
				foreach($rows as $row) {
					...
					$value = $row['vFilename'];	
					if($value != '') {
						if (file_exists('./files/reformulasi/local/lokal_uji_be'.$id.'/'.$value)) {
							$link = base_url().'processor/reformulasi/lokal/req/ujibe?action=downloadbkt&id='.$id.'&filebkt='.$value;
		...
		?>
				<tr style="border: 1px solid #dddddd; border-collapse: collapse; background: #ffffff; ">
					<td style="border: 1px solid #dddddd; width: 3%; text-align: center;">
						<span class="lokal_ujibe_upload_num"><?php echo $i ?></span>
					</td>		
					<td colspan="1" style="border: 1px solid #dddddd; width: 27%">
						...
						<input type="hidden" name="fileid_studypd[]" style="width: 70%" value="<?php echo $row['iUjibe_file'] ?>" />
					</td>	
		...
		<tr style="border: 1px solid #dddddd; border-collapse: collapse; background: #ffffff; ">
					<td style="border: 1px solid #dddddd; width: 3%; text-align: center;">
						<span class="lokal_ujibe_upload_num">1</span>
					</td>		
					<td colspan="1" style="border: 1px solid #dddddd; width: 27%">
	...
	<tfoot>
		<tr>
			<td colspan="5" style="text-align: left">*) Max File 5 MB : Format = pdf,jpeg,jpg,png,gif,bmp</td><td style="text-align: center">
				<?php if ($this->input->get('action') != 'view') { ?>
					<a href="javascript:;" onclick="javascript:add_row_lokal_ujibe_upload('lokal_ujibe_upload')">Tambah</a>	
				<?php } ?>
