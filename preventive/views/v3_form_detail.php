...
<?php 
	...
	*/
	...
	switch ($iM_jenis_field) {
		case '1':
			$return = '<input type="text" value="" name="'.$field.'" id="'.$id.'" class="input_rows1 '.$field_required.'" '.$readonly.' size="35" >';
			break;
		...
		case '14':
			$sqlem = 'SELECT a.*,b.*,b.vDescription AS vdepartemen
					FROM hrd.employee a
					JOIN hrd.msdepartement b ON b.iDeptID=a.iDepartementID
					where a.cNip="'.$this->user->gNIP.'" ';
			$iAm=$this->db->query($sqlem)->row_array();
			...
			$return .= $this->user->gNIP.' - '.$this->user->gName.' - '.$iAm['vdepartemen'];
			break;
