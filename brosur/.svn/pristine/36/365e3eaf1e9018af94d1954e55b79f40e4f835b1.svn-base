<?php
    $urlGetProsesKopi = base_url()."processor/plc/copy_brand?action=getProsesKopi"; 
...
function copy_brand_btn_multiupload(grid, url, dis, isdraft) {
    ...
    if(conf > 0) {
        ...
        custom_confirm(comfirm_message,function(){
                ...
                $.ajax({
                    ...
                    // data: {    
                    //             iupb_id : $("#copy_brand_iupb_id").val()
                    //         },
                    beforeSend: function() {
...
function copy_brand_btn_multiuploadcc(grid, url, dis, isdraft) {
    var urlc = new URL(url);
