...
class jenis_field extends MX_Controller {
    function __construct() {
        ...
        $this->main_table='erp_privi.m_jenis_field';
        $this->main_table_pk='iM_jenis_field';


        $this->title = 'Jenis Field';
        $this->url = 'jenis_field';
        $this->urlpath = 'plc/'.str_replace("_","/", $this->url);

        $this->maintable = 'erp_privi.m_jenis_field';  
        $datagrid['islist'] = array(
                                        'vKode_field' => array('label'=>'Code','width'=>100,'align'=>'center','search'=>false)
                                        ,'vNama_field' => array('label'=>'Nama','width'=>250,'align'=>'left','search'=>false)
                                        ,'vDesciption' => array('label'=>'Description','width'=>400,'align'=>'left','search'=>false)
                                    );
        $datagrid['shortBy']=array('iM_jenis_field'=>'Desc');
        ...
        $datagrid['isField'] = array(
			'vKode_field' => array('label'=>'Code','require'=>true)
            ,'vNama_field' => array('label'=>'Nama','require'=>true)
      ...
      /*   $datagrid['jointableinner']=array(
                                    ...
                                    0=>array('plc2.plc2_upb_request_sample_detail'=>'plc2.plc2_upb_request_sample_detail.ireqdet_id = erp_privi.m_jenis_field.ireqdet_id'),
    ...
    function approve_view() {
        ...
        $echo .= '<form id="form_jenis_field_approve" action="'.base_url().'processor/plc/jenis_field?action=approve_process" method="post">';
        $echo .= '<div style="vertical-align: top;">';
        $echo .= 'Remark : 
                <input type="hidden" name="iM_jenis_field" value="'.$this->input->get('iM_jenis_field').'" />
        ...
        <button type="button" onclick="submit_ajax(\'form_jenis_field_approve\')">Approve</button>';
    ...
    function approve_process() {
        ...
        $iM_jenis_field = $post['iM_jenis_field'];
        ...
        $this -> db -> where('iM_jenis_field', $iM_jenis_field);
        $updet = $this -> db -> update('erp_privi.m_jenis_field', $data);

        $iM_modul_activity = $post['iM_modul_activity'];
        $isAndSort = $this->lib_plc->getIDActivityAndSort($iM_modul_activity);
        $iM_jenis_field = $isAndSort['iM_jenis_field'];
        $iSort = $isAndSort['iSort'];

        $arrUPB['iupb_id'] = $iupb_id;
        $this->lib_plc->InsertActivityModule($arrUPB,$modul_id,$iM_jenis_field,$iM_jenis_field,$iSort,$vRemark,2);

        $data['status']  = true;
        $data['last_id'] = $post['iM_jenis_field'];
    ...
    function confirm_view() {
        ...
        $echo .= '<form id="form_jenis_field_confirm" action="'.base_url().'processor/plc/jenis_field?action=confirm_process" method="post">';
        $echo .= '<div style="vertical-align: top;">';
        $echo .= 'Remark : 
                <input type="hidden" name="iM_jenis_field" value="'.$this->input->get('iM_jenis_field').'" />
                <input type="hidden" name="modul_id" value="'.$this->input->get('modul_id').'" />
                ...
                <textarea name="vRemark"></textarea>
        <button type="button" onclick="submit_ajax(\'form_jenis_field_confirm\')">Confirm</button>';
    ...
    function reject_view() {
        $echo = '<script type="text/javascript">
                     function submit_ajax(form_id) {
                        var remark = $("#jenis_field_remark").val();
                        if (remark=="") {
        ...
        $echo .= '<form id="form_jenis_field_reject" action="'.base_url().'processor/plc/jenis_field?action=reject_process" method="post">';
        $echo .= '<div style="vertical-align: top;">';
        $echo .= 'Remark : 
                <input type="hidden" name="iM_jenis_field" value="'.$this->input->get('iM_jenis_field').'" />
                <input type="hidden" name="modul_id" value="'.$this->input->get('modul_id').'" />
                ...
                <textarea name="vRemark" id="reject_jenis_field_remark"></textarea>
        <button type="button" onclick="submit_ajax(\'form_jenis_field_reject\')">Reject</button>';
    ...
    function reject_process() {
        ...
        $arrUPB['iupb_id'] = $iupb_id;
        $this->lib_plc->InsertActivityModule($arrUPB,$modul_id,$iM_jenis_field,$iM_jenis_field,$iSort,$vRemark,1);
    ...
    function getHistory($iupb_id,$iTujuan_req){
        ...
        $sql = 'select a.vreq_ori_no , date(a.tcreate) as tcreate , a.tapppd as tapppd,a.vnip_apppd  
                from erp_privi.m_jenis_field a 
                where 
                a.ldeleted=0
                and a.iupb_id= "'.$iupb_id.'"  
                and a.iTujuan_req= "'.$iTujuan_req.'"  
                order by iM_jenis_field
        ';
    ...
    function output(){
        ...
        function insertBox_jenis_field_vKode_field($field, $id) {
        $return = '<input type="text" name="'.$field.'"  id="'.$id.'"  class="input_rows1 required" size="4"  />';
        return $return;
    }
    
    function updateBox_jenis_field_vKode_field($field, $id, $value, $rowData) {
    ...
    function insertBox_jenis_field_vNama_field($field, $id) {
        $return = '<input type="text" name="'.$field.'"  id="'.$id.'"  class="input_rows1 required" size="30"  />';
        return $return;
    }
    
    function updateBox_jenis_field_vNama_field($field, $id, $value, $rowData) {
            if ($this->input->get('action') == 'view') {
                    $return= $value; 
            }else{ 
    ...
    function insertBox_jenis_field_vDesciption($field, $id) {
        $return = '<textarea name="'.$field.'" id="'.$id.'" class="required" style="width: 240px; height: 75px;" size="250" maxlength ="250"></textarea>';
        return $return;
    }
    
    function updateBox_jenis_field_vDesciption($field, $id, $value, $rowData) {
            if ($this->input->get('action') == 'view') {
    ...
    function before_update_processor($row, $postData) {
        ...
        $controller_name ='jenis_field';
        $pk_field = 'iM_jenis_field';
    ...
    function insertBox_jenis_field_form_detail($field,$id){
            $get=$this->input->get();
            $post=$this->input->post();
    ...
    function updateBox_jenis_field_form_detail($field,$id,$value,$rowData){
        $get=$this->input->get();
        $post=$this->input->post();
        foreach ($get as $kget => $vget) {
            if($kget!="action"){
