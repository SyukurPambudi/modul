...
class master_activity extends MX_Controller {
    function __construct() {
        ...
        $this->main_table='erp_privi.m_activity';
        $this->main_table_pk='iM_activity';


        $this->title = 'Activity';
        $this->url = 'master_activity';
        $this->urlpath = 'plc/'.str_replace("_","/", $this->url);

        $this->maintable = 'erp_privi.m_activity';  
        $datagrid['islist'] = array(
                                        'vKode_activity' => array('label'=>'Code','width'=>100,'align'=>'center','search'=>false)
                                        ,'vNama_activity' => array('label'=>'Name','width'=>250,'align'=>'left','search'=>false)
                                        ,'vDesciption' => array('label'=>'Description','width'=>400,'align'=>'left','search'=>false)
                                    );
        $datagrid['shortBy']=array('iM_activity'=>'Desc');
        ...
        $datagrid['isField'] = array(
			'vKode_activity' => array('label'=>'Code','require'=>true)
            ,'vNama_activity' => array('label'=>'Name','require'=>true)
      ...
      /*   $datagrid['jointableinner']=array(
                                    ...
                                    0=>array('plc2.plc2_upb_request_sample_detail'=>'plc2.plc2_upb_request_sample_detail.ireqdet_id = erp_privi.m_activity.ireqdet_id'),
    ...
    function approve_view() {
        ...
        $echo .= '<form id="form_master_activity_approve" action="'.base_url().'processor/plc/master_activity?action=approve_process" method="post">';
        ...
        <button type="button" onclick="submit_ajax(\'form_master_activity_approve\')">Approve</button>';
    ...
    function approve_process() {
        ...
        $this -> db -> where('iM_activity', $iM_activity);
        $updet = $this -> db -> update('erp_privi.m_activity', $data);
        ...
        $this->lib_plc->InsertActivityModule($arrUPB,$modul_id,$iM_activity,$iM_activity,$iSort,$vRemark,2);
    ...
    function confirm_view() {
        ...
        $echo .= '<form id="form_master_activity_confirm" action="'.base_url().'processor/plc/master_activity?action=confirm_process" method="post">';
        $echo .= '<div style="vertical-align: top;">';
        $echo .= 'Remark : 
                <input type="hidden" name="iM_activity" value="'.$this->input->get('iM_activity').'" />
                <input type="hidden" name="modul_id" value="'.$this->input->get('modul_id').'" />
                ...
                <textarea name="vRemark"></textarea>
        <button type="button" onclick="submit_ajax(\'form_master_activity_confirm\')">Confirm</button>';
    ...
    function reject_view() {
        $echo = '<script type="text/javascript">
                     function submit_ajax(form_id) {
                        var remark = $("#master_activity_remark").val();
                        if (remark=="") {
        ...
        $echo .= '<form id="form_master_activity_reject" action="'.base_url().'processor/plc/master_activity?action=reject_process" method="post">';
        $echo .= '<div style="vertical-align: top;">';
        $echo .= 'Remark : 
                <input type="hidden" name="iM_activity" value="'.$this->input->get('iM_activity').'" />
                <input type="hidden" name="modul_id" value="'.$this->input->get('modul_id').'" />
                ...
                <textarea name="vRemark" id="reject_master_activity_remark"></textarea>
        <button type="button" onclick="submit_ajax(\'form_master_activity_reject\')">Reject</button>';
    ...
    function reject_process() {
        ...
        $arrUPB['iupb_id'] = $iupb_id;
        $this->lib_plc->InsertActivityModule($arrUPB,$modul_id,$iM_activity,$iM_activity,$iSort,$vRemark,1);
    ...
    function output(){
        ...
        function insertBox_master_activity_vKode_activity($field, $id) {
        $return = '<input type="text" name="'.$field.'"  id="'.$id.'"  class="input_rows1 required" size="4"  />';
        return $return;
    }
    
    function updateBox_master_activity_vKode_activity($field, $id, $value, $rowData) {
    ...
    function insertBox_master_activity_vNama_activity($field, $id) {
        $return = '<input type="text" name="'.$field.'"  id="'.$id.'"  class="input_rows1 required" size="30"  />';
        return $return;
    }
    
    function updateBox_master_activity_vNama_activity($field, $id, $value, $rowData) {
            if ($this->input->get('action') == 'view') {
                    $return= $value; 
            }else{ 
    ...
    function insertBox_master_activity_vDesciption($field, $id) {
        $return = '<textarea name="'.$field.'" id="'.$id.'" class="required" style="width: 240px; height: 75px;" size="250" maxlength ="250"></textarea>';
        return $return;
    }
    
    function updateBox_master_activity_vDesciption($field, $id, $value, $rowData) {
            if ($this->input->get('action') == 'view') {
    ...
    function before_update_processor($row, $postData) {
        ...
        $controller_name ='master_activity';
        $pk_field = 'iM_activity';
    ...
    function insertBox_master_activity_form_detail($field,$id){
            $get=$this->input->get();
            $post=$this->input->post();
    ...
    function updateBox_master_activity_form_detail($field,$id,$value,$rowData){
        $get=$this->input->get();
        $post=$this->input->post();
        foreach ($get as $kget => $vget) {
            if($kget!="action"){
